...
chr_orglist = unique(SNP_info$CHR)
num_orgchr = length(chr_orglist)
chrlabs_give = as.character(c(strsplit(opt$chrlabs,",")[[1]]))
num_lab = length(chrlabs_give)
if(num_lab>=num_orgchr){
chrlabs_list = chrlabs_give[c(1:num_orgchr)]
}else{
star = num_lab+1
chrlabs_list = c(chrlabs_give,chr_orglist[star:num_orgchr]) 
}
}
####################
