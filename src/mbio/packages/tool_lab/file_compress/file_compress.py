...
class FileCompress(object):
    ...
    def uncompress(self,uncompress,out_dir):
        print(uncompress)
        if uncompress.endswith("tgz") or uncompress.endswith("tar.gz"):
            self.un_tgz(uncompress,out_dir)
        elif uncompress.endswith("gz"):
        ...
        elif uncompress.endswith("zip"):
            self.un_zip(uncompress, out_dir)
        else:
            raise Exception("不支持{}该格式压缩文件,目前仅支持tgz(tar.gz),gz,tar,rar,zip五种压缩格式".format(uncompress))
    ...
    def un_rar(self,file_name,out_dir):
        rar_path = Config().SOFTWARE_DIR + "/bioinfo/rar/rar/"
    ...
    def un_tar(self,file_name,out_dir):
        ...
        if os.path.isdir(out_dir):
            ...
            os.mkdir(out_dir)
        #因为解压后是很多文件，因此创建待存放目录
        for name in names:
            tar.extract(name, out_dir)
        tar.close()
