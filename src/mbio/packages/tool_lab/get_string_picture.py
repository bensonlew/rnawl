...
class string_api(object):
    def __init__(self, specie, list_path, vsstring, identity, useblast, max_num, out):
        ...
        self.vstringfile = ''
        if  self.specie is None and not os.path.exists(vsstring):
    ...
    def get_protein_info_from_net(self, string_dict, out):
        ...
        for n,ids in enumerate(id_lists):
            ...
            for line in response.text.strip().split("\n"):
                ...
                string2acc[string_identifier] = string_dict.get(input_identifier, "")
                ow.write(node + '\t' + string_dict.get(input_identifier, "") + '\t' + string_identifier + '\t' + annotation + '\n')
    ...
    def get_identifiers_bitscore_and_filter(self, identifiers, string2acc, out):
        tmp_spe = Counter([ide.split('.')[0] for ide in identifiers]).most_common(1)[0][0]
        if self.specie:
            self.specie = int(tmp_spe)
            print(u'选定物种为%s' % tmp_spe)
        id_lists = list()
        l = len(identifiers)
        for i in range(0, l, 50):
            if i + 50 <= l:
        ...
        for ind in score_df.index:
            ...
            for string, acc in string2acc.items():
                if '.' + sa in string or ('.' in sa and sa in string):
                    pa = acc
                if '.' + sb in string or ('.' in sb and sb in string):
                    pb = acc
    ...
    def get_picture_from_net(self, identifiers, out):
        ...
        params = {
            ...
            "identifiers": "%0d".join(identifiers),  # your protein
            # "species": str(self.specie),  # species NCBI identifier
            "add_white_nodes": 10,  # add 15 white nodes to my protein
            "network_flavor": "confidence",  # show confidence links
            "caller_identity": "www.awesome_app.org"  # your app name
        }
        if self.specie:
            params.update({'species': str(self.specie)})
        response = requests.post(request_url, data=params)
        file_name = "%s.network.png" % out
    ...
    def get_interaction_from_net(self, identifiers, string2acc, out):
        ...
        with open(out, 'w') as ow:
            ...
            for line in response.readlines():
                ...
                pa,pb='',''
                for string,acc in string2acc.items():
                    if '.' + sa in string or ('.' in sa and string == sa):
                        pa = acc
                    if '.' + sb in string or ('.' in sb and string == sb):
                        pb = acc
                ow.write(pa.strip() + '\t' + pb.strip() + '\t' + line)
                # try:
