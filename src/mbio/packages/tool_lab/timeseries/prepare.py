...
parser.add_option('--normalized_method', dest='nor_method', type=str, help='normalized_method')
...
#Interface Options
Gene_display_policy_on_main_interface[Do not display,Display only selected,Display all] Display all
...
Y_Scale_Max 3.0
Tick_interval 1.0
X-axis_scale_should_be[Uniform,Based on real time] Based on real time
'''
...
def main(opts):
    ...
    if opts.nor_method == "log_normal":
        kwargs.update({'scale': 'Log normalize data'})
    elif opts.nor_method == "normal":
        kwargs.update({'scale': 'Normalize data'})
    else:
        kwargs.update({'scale': 'No normalization/add 0'})
...
def export_group_matrix(raw_matrix, pheno_table, log_base, out_matrix, geneset_list=None):
    ...
    phedf.columns = ['Sample','Group','Order']
    ...
    if 'seq_id' in df.columns.values:
        ...
        df = df.reindex(['seq_id'] + samples, axis=1).set_index('seq_id')
    elif 'gene_id' or 'gene id' in df.columns.values:
        df.rename(columns={'gene id': 'seq_id'}, inplace=True)
        df = df.reindex(['seq_id'] + samples, axis=1).set_index('seq_id')
    else:
        raw_id = list(df.columns.values)[0]
        df.rename(columns={raw_id: 'seq_id'}, inplace=True)
        df = df.reindex(['seq_id'] + samples, axis=1).set_index('seq_id')
