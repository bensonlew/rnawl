...
def chang_result(data, sample, prefix):
    with open(data, "r") as f:
        ...
        st = ''
        list1 = []
        for key, value in data['pmlst']['results'].items():
            ...
            if key == "allele_profile":
                for key2, value2 in value.items():
                    ...
                    for key3 in sorted(value2.keys()):
                        if key3 == "alternative_hit":
                           continue
                        list2.append(value2[key3])
                    list1.append(list2)
        data = DataFrame(list1)
        data.columns = ['locus', "align_len", "allele", "allele_name", "coverage", "gaps", "identity", "sbj_len"]
        ...
        ss = ["sample", "ST"] + names
        print(data3)
        data3.to_csv(prefix + ".pmlst.ST.xls", sep='\t', header=True, index=False, columns=ss)
        del data['allele']
        print(data)
        data.to_csv(prefix + ".pmlst.detail.xls", sep='\t', header=True, index=False,
