...
diff_exp_example1$raw_name<-diff_exp_example1$gene_id
#diff_exp_example1 <- diff_exp_example1[which(diff_exp_example1$FDR < FDR),]
data_mapped <- string_db$map(diff_exp_example1, "gene_id",takeFirst=FALSE, removeUnmappedRows=TRUE, quiet = TRUE)
data_mapped <- data_mapped[,2:3]    # raw_name,STRING_id
colnames(data_mapped)[1]<-'gene_id'
...
unmapped_gene <- setdiff(diff_exp_example1$gene_id, data_mapped$gene_id)
