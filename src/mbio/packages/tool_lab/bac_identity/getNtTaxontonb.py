...
parser.add_argument("-t", "--taxonkit", help="taxonkit路径", required=True)
parser.add_argument("-d", "--datadir", help="data_dir路径", required=True)
args = vars(parser.parse_args())


In_NT = args["input"]
out_NT = args["out"]
taxonkit = args["taxonkit"]
datadir = args["datadir"]
# taxonkit = "/mnt/ilustre/users/sanger-dev/app/bioinfo/tool_lab/taxonkit"
# taxonkit = "/mnt/ilustre/users/sanger-dev/app/bioinfo/ref_rna_v2/miniconda3/bin/taxonkit"
# taxonkit = "/mnt/lustre/users/sanger-dev/app/bioinfo/ref_rna_v2/miniconda3/bin/taxonkit"
# temp = "/mnt/ilustre/users/sanger-dev/sg-users/xueqinwen/NT/test/temp"

def getTaxon(taxid,is_print):
    code = subprocess.Popen("echo {}|{} --data-dir {} lineage|{} --data-dir {} reformat -f \"{{k}};{{p}};{{c}};{{o}};{{f}};{{g}};{{s}}\" -P -r norank|cut -f 3".format(taxid,taxonkit,datadir,taxonkit,datadir),shell = True, stdout = subprocess.PIPE,
