...
class DiffExpToolbox(PvalueCorrect):
    ...
    """
    def __init__(self, count_matrix, group_info, cmp_info, is_duplicate, batch_info=None,
                 sig_type='pvalue', stat_cutoff=0.05, fc_cutoff=2, padjust_way=3, pool_size=5,
                 normalized='tmm', prob=0.9):
        """
        initial inputs
        ...
        with open(self.count) as f:
            count_samples = f.readline().strip().split('\t')[1:]
    ...
    def DEGseq(self, sep='\t', method='MARS', threshold_kind=0, output=None):
        ...
        for ctrl, test in self.cmp_list:
            ...
            counts_cmp_total.to_csv(os.path.join(output, '{}_vs_{}.counts_filted'.format(ctrl, test)), sep="\t")
    ...
    def edgeR(self, dispersion=0.1, output=None, sep='\t', normalized='tmm', model='glmQLfit'):
        #if self.count_filtered is None:
        #    count_table = self.count
        #else:
        #    count_table = self.count_filtered
       # with open(count_table) as f:
        ...
        #for ctrl, test in self.cmp_list:
            ...
            f.write('y <- calcNormFactors(y, method="{}")\n'.format(normalized))
            # 输出normfactors
    ...
    def DESeq2(self, output=None, sep='\t', padjust_way=None, test_method='Wald'):
        """
    ...
    def limma(self, voom, dispersion=0.1, output=None, sep='\t', normalized='tmm', ):
        print type(voom)
        print voom
        if output is None:
            output = os.getcwd()
        ...
        #for ctrl, test in self.cmp_list:
            ...
            if self.batch is not None:
                f.write('tmp_batch <- c({})\n'.format(','.join(ctrl_batch + test_batch)))
                f.write('tmp_batch <- as.factor(tmp_batch)\n')
            f.write('y_deglist <- DGEList(counts=tmp_counts, group=tmp_group)\n')
            # f.write('y <- calcNormFactors(y_deglist)\n')
            f.write('y <- calcNormFactors(y_deglist, method="{}")\n'.format(normalized))
            # 输出normfactors
            ...
            if (',' not in ctrl_ind) and (',' not in test_ind):
                ...
                if voom is True:
                    ...
                    f.write('y_voom <- cpm(y, normalized.lib.sizes = TRUE)\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--is_duplicate', type=str)
    parser.add_argument('--prob', type=float, default=0.9)
    parser.add_argument('-model', type=str, default='glmQLfit')
    parser.add_argument('-test', type=str, default='Wald')
    parser.add_argument('-voom', type=bool, )
...
#        toolbox.filter()
    ...
    if args.method.lower() == 'degseq':
        ...
        toolbox.edgeR(dispersion=args.dispersion, output=args.output, normalized=args.normalized, model=args.model)
    elif args.method.lower() == 'deseq2':
        toolbox.DESeq2(output=args.output, padjust_way=args.deseq2_padjust_way, test_method=args.test)
    elif args.method == 'limma':
        toolbox.limma(args.voom, dispersion=args.dispersion, output=args.output, normalized=args.normalized)
    elif args.method == 'svaseqlimma':
        toolbox.svaseq_limma(dispersion=args.dispersion, output=args.output)
    elif args.method.lower() == 'noiseq':
