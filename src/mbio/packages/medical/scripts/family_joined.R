...
# 更改表头
get_bed <- function(file, err.min=err_min, rf.min=0.1, rf.max=0.9, dp.min=12){
...
#######joined family函数
dad.bed <- get_bed(file_F)
mom.bed <- get_bed(file_M)
preg.bed <- get_bed(file_S)
...
file_name_Rdata = paste(bed$dad.id, bed$mom.id, bed$preg.id, 'family_joined_tab.Rdata',sep = "_")
file_name_txt = paste(bed$dad.id, bed$mom.id, bed$preg.id, 'family_joined_tab.txt',sep = "_")
save(bed, file= paste(out_path,file_name_Rdata[1],sep="/"))
# write.xlsx(bed,file = 'family_joined_tab.xlsx')
write.table(bed, file = paste(out_path,file_name_txt[1],sep="/"), row.names = F, quote = F, sep="\t")
# write.csv(bed,file='family_joined_tab.csv',quote=F,row.names= F)
