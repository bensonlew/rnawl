...
plot_family <- function(prepared_family, by.dp=F){
  ...
  d.dads = joined_family[dad.geno %in% c("0/0","1/1", "0/1")][, .(dp=dad.dp, n, rf=dad.rf, name=dad.id, alpha=1)]
  d.mom = joined_family[mom.geno %in% c("0/0","1/1", "0/1"), .(dp= mom.dp, n, rf=mom.rf, name=mom.id, alpha=1)]
  d.preg = joined_family[, .(dp= preg.dp, n, rf=preg.rf, name=preg.id, alpha=1+is.test)]
  if (by.dp)
