# use: pt_dup_new.R WQ123M.tab WQ123S.tab 2 /mnt/ilustre/users/sanger-dev/sg-users/zhoumoli/pt/targets.bed.rda WQ123F WQ123-F1,WQ123-F2
...
# 下面开始进行查重模块
dad.ids <- character(0)
for (m in strsplit(dad_list, split=",")){dad.ids <- append(dad.ids, m)} 
cat("本批次要进行查重的父本")
cat("\n")
cat(dad.ids)
