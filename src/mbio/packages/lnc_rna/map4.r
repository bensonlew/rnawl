...
plot_polygon <- function(choose_xml, i, i_max, bg_col, height, err_x_location, err_y_location, magnify,border, ...) {
  x <- choose_xml$x
  y <- choose_xml$y
  w <- choose_xml$width+2
  h <- choose_xml$height+1
  polygon(
    c(x - w / 2 * magnify + w / i_max * (i - 1) * magnify + err_x_location,
      x - w / 2 * magnify + w / i_max * i * magnify + err_x_location,
      x - w / 2 * magnify + w / i_max * i * magnify + err_x_location,
      x - w / 2 * magnify + w / i_max * (i - 1) * magnify + err_x_location),
    c(height - y - h / 2 * magnify - err_y_location,
      height - y - h / 2 * magnify - err_y_location,
      height - y + h / 2 * magnify - err_y_location,
      height - y + h / 2 * magnify - err_y_location),
    col = bg_col[i],
    border = border[i],
...
plot_segments <- function(choose_xml, i, i_max, height, err_x_location, err_y_location, magnify, border, ...) {
  x <- choose_xml$x
  y <- choose_xml$y
  w <- choose_xml$width + 2
  h <- choose_xml$height + 1
...
plot_KO <-function(xml_KO, bg_col, fg_col, height, err_x_location, err_y_location) {
  ...
  if (i_max == 1) {
    for (i in 1:i_max) {
      plot_polygon(xml_KO, i = i, i_max = i_max, bg_col = bg_col, height = height, err_x_location = err_x_location, err_y_location = err_y_location, magnify = 1, border = fg_col)
    }
  } else {
    for (i in 1:i_max) {
      plot_polygon(xml_KO, i = i, i_max = i_max, bg_col = bg_col, height = height, err_x_location = err_x_location, err_y_location = err_y_location, magnify = 1, border = NA)
    }
    i_max <- length(fg_col)
    for (i in 1:i_max) {
      plot_segments(xml_KO, i = i, i_max = i_max, height = height, err_x_location = err_x_location, err_y_location = err_y_location, magnify = 1, border = fg_col)
    }
  }
}
...
path_KOs <- read.table(kos, head = TRUE, comment.char = "")
