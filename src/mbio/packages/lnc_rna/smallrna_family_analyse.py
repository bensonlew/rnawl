...
class family_analyse(object):
    def __init__(self, miRfamily, miR_pre_mature,  mature_fa):
    ...
    def creat_known_txt(self, mirna_fa, fam2pre, pre2mi, spe2pre):
        ...
        for seq in SeqIO.parse(mirna_fa, "fasta"):
            tmp_pre = list()
            tmp_fam = list()
            mir = seq.id
            for key in pre2mi:
    ...
    def creat_novel_txt(self, mature_fa, novo_fa, fam_txt):
        ...
        def matct(fam2seed, seq):
            ...
            if score2fam and max(score2fam) >= 6:
    ...
    def run(self):
        ...
        fam_txt, spe_txt = self.creat_known_txt(self.mature_fa, fam2pre, pre2mi, spe2pre)
        return fam_txt
...
if __name__ == '__main__':
    ...
    FA = family_analyse(args.family, args.pre, args.mature_fa)
    fam_txt = FA.run()
