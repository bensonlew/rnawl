            ...
            for area in areas:
                ...
                if area_dict['shape'] == 'rect':
                    ...
                    if len(change_kos) > 0:
                        ...
                        mark.write("\t".join([self.map_id, "rect", bg_color, str(), area_dict['coords'], all_title_mark,
                                              ",".join(change_kos), area_dict['href']]) + "\n")
                    else:
                        mark.write("\t".join([self.map_id, "rect", str(), str(), area_dict['coords'], area_dict['title'], str(),
                                              area_dict['href']]) + "\n")
                elif area_dict['shape'] == "circle":
                    mark.write("\t".join([self.map_id, "circle", str(), str(), area_dict['coords'], area_dict['title'], str(),
                                          area_dict['href']]) + "\n")
        ...
        with open("temp.mark", 'w') as mark:
            for area in areas:
                ...
                if area_dict['shape'] == 'rect':
                    ...
                    bg_color = self.get_bg_color(change_kos)
                    mark.write("\t".join(
                        [self.map_id, "rect", bg_color, str(), area_dict['coords'], all_title, ",".join(change_kos),
                         area_dict['href']]) + "\n")
                elif area_dict['shape'] == "circle":
                    mark.write("\t".join([self.map_id, "circle", str(), str(), area_dict['coords'], area_dict['title'], str(),
                                          area_dict['href']]) + "\n")
        ...
        with open("temp.mark", 'w') as mark:
            for area in areas:
                ...
                if area_dict['shape'] == 'rect':
                    ...
                    bg_color = str()
                    if ":" in all_title:
                        if all_title.startswith("map"):
                            pass
                        elif "MSTRG" in all_title or "XLOC" in all_title or "TCONS" in all_title:
