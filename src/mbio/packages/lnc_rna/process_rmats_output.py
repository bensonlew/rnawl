...
parser.add_option('-g', '--gtf', dest='gtf', help='Input GTF file')
...
def main(root, gtf, method, cutoff, psi):
...
def modify_id_for_txt(ref_gtf, infile, outfile):
    # 将gtf文件中包含的染色体名去重后存到列表chr_set中
    chr_set = [e.strip() for e in subprocess.check_output(
        'awk -F \'\\t\'  \'$0!~/^#/{print $1}\' %s | uniq | sort | uniq' % ref_gtf, shell=True
    ).strip().split('\n')]
    f_name_match = re.match(r'^.*?(A3SS|A5SS|MXE|RI|SE).*?\.txt$', os.path.basename(infile).strip())
    if f_name_match:
        # 读取fromGTF.*.txt和*.MATS.*.txt作为infile，将infile内的全部半角双引号去除后存为infile.temp作为临时文件
        # 一通操作后，结果存为infile.alter_id.txt作为outfile
        tmp = outfile + '.temp'
...
def check_rmats_out_dir(dirpath):
    if OUTPUT_FILES.difference(set(os.listdir(dirpath))):
        ...
        for each in event_types:
            ...
            # modify 'fromGTF.{}.txt'
            remove_quotation(target_file1)
            target_file1_table = pandas.read_table(target_file1, index_col=0, sep='\t', header=0)
            target_file1_table = target_file1_table.query("index in @id_list")
            target_file1_table.to_csv(target_file1, sep='\t')
            # modify fromGTF.novelEvents.{}.txt
            remove_quotation(target_file2)
            target_file2_table = pandas.read_table(target_file2, index_col=0, sep='\t', header=0)
            target_file2_table = target_file2_table.query("index in @id_list")
            target_file2_table.to_csv(target_file2, sep='\t')

    to_be_altered_id_files = set(
        ...
        [os.path.join(dirpath, name) for name in os.listdir(dirpath) if
         re.match(r'^(A3SS|A5SS|MXE|RI|SE)\.MATS\.(JCEC|JC)\.txt$', name.strip())]
    )
    info_dic = defaultdict(str)
    for f in to_be_altered_id_files:
        alter_id_file = os.path.join(
            os.path.dirname(f),
            re.match(r'^(\S+)\.txt$', os.path.basename(f).strip()).group(1) + '.alter_id.txt')
        info_dic[f] = alter_id_file
    return info_dic

def remove_quotation(file_path):
    lines = [line.replace('"', '') for line in open(file_path)]
...
def get_event_stats(files, pvalue_fdr="fdr", fdr=0.05, psi=0):
    ...
    for as_type in LEGAL_EVENT_TYPE:
        d[as_type]['JunctionCountOnly_and_ReadsOnTargetAndJunctionCounts_set'] = d[as_type]['JunctionCountOnly_event_id_set'] & \
                                                                                 d[as_type]['ReadsOnTargetAndJunctionCounts_event_id_set']
        ...
        d[as_type]['JunctionCountOnly_or_ReadsOnTargetAndJunctionCounts_set'] = d[as_type]['JunctionCountOnly_event_id_set'] | \
                                                                                d[as_type]['ReadsOnTargetAndJunctionCounts_event_id_set']
...
if __name__ == '__main__':
    if opts.root and opts.gtf and opts.method and opts.cutoff and opts.psi:
        main(opts.root, opts.gtf, opts.method, opts.cutoff, opts.psi)
    else:
        parser.print_help()