...
def hcluster(exp_pd, transpose=False, n_clusters=10, method='average',
             metric='correlation', output=None, prefix='', use_sub=True):
    """
    ...
    try:
        ...
        try:
            z = hclust.linkage(exp_pd, method=method, metric=metric,optimal_ordering=True)
        except:
            print("enhen? fastcluster failed again, we will try scipy.cluster.hierarchy")
            z = sch.linkage(exp_pd, method=method, metric=metric,optimal_ordering=True)
    except:
    ...
    '''
    print sch.leaves_list(z)
    print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
    newtree = sch.to_tree(z)
    print newtree.__dict__
    print newtree.get_right()
    rootnode, nodelist = sch.to_tree(z, rd=True)
    print nodelist,"-------------",rootnode
    # tree = R_to_tree(exp_pd, metric, method)
    '''
    if use_sub:
        subcluster = get_subcluster(z, labels, num=n_clusters,prefix=prefix)
    else:
        subcluster = {}
    ...
    for k, v in subcluster.items():
        ...
        if transpose:
            sub = sub.transpose()
        sub.to_csv(out_dir, sep='\t', header=True, index=True, quoting=3)
    # write out tree
...
def to_tree(zclust, labels):
    ...
    for i, j, h, _ in zclust:
        ...
        if i in clust_step_detail:
            ...
            tmp_list = i.split("):")
            last_h_1 = float(h) - float(tmp_list[-1])
            i = "):".join(tmp_list[0:len(tmp_list)-1]) + "):" + str(last_h_1)
        if j in clust_step_detail:
            j = clust_step_detail[j]
            tmp_list = j.split("):")
            last_h = float(h) - float(tmp_list[-1])
            j = "):".join(tmp_list[0:len(tmp_list)-1]) + "):" + str(last_h)
...
def r_hcluster(exp_pd, transpose=False, n_clusters=10, method='average',
               metric='euclidean', output=None, prefix='', do_scale=True, use_sub=True):
    """
        'fastcluster' was used, http://www.danifold.net/fastcluster.html?section=3.
        scipy.cluster.hierarchy.linkage could also do the same thing but slower.
        However, the documentation of scipy.cluster.hierarchy.linkage is pretty good.
...
def get_subcluster(zclust, labels, num=2,prefix=''):
    """
    ...
    ## zouguanqing 20190618  生成cluster列表文件
    cluster_out = prefix + 'cluster.txt'
...
def corr(exp_pd, method='pearson', output=None):
    ...
    result.to_csv(out_dir, sep='\t', header=True, index=True, quoting=3)
    return result
...
def exp_cluster(exp_pd, n_clusters=10, sn_clusters=10,s_cluster=True, g_cluster=True,
    ...
    if s_cluster:
        ...
        if s_cluster_type == 'hierarchy':
            s_cluster_result = hcluster(exp_pd, transpose=True, n_clusters=n_clusters, method=s_method,
                                        metric=s_metric, output=output, prefix="sample.", use_sub=use_sub)
        else:
            s_cluster_result = kmeans(exp_pd, n_clusters=sn_clusters,transpose=True, output=output, prefix='sample.')
    ...
    if g_cluster:
        ...
        if g_cluster_type == 'hierarchy':
            g_cluster_result = hcluster(exp_pd, transpose=False, n_clusters=n_clusters, method=g_method,
                                        metric=g_metric, output=output, prefix="seq.", use_sub=use_sub)
        else:
    ...
    exp_pd.to_csv(out_dir, sep='\t', header=True, index=True, quoting=3)
    return result
...
if __name__ == '__main__':
    ...
    parser.add_argument('-sn_clusters', type=int, default=10, metavar="cluster_num", help="sample expected_cluster_number")
    ...
    exp_pd = process_exp_matrix(args.exp, log_base=log_base, group_dict=group_dict)
    #
    ...
    if args.ngc:
        g_cluster = False
    print "args:",args.n_clusters
    exp_cluster(exp_pd, n_clusters=args.n_clusters,sn_clusters=args.sn_clusters,
