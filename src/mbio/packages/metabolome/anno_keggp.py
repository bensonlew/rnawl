...
class AnnoKeggp(Base):
    ...
    def run(self, table, organisms, level_path, stat_path, color='green',html_db=None, db_v="v94.2"):
        """
        table: 所用的数据详情表
    ...
    def get_ko_list(self, organisms):
        ...
        ref_org_db = self.ref_db["kegg_{}_organisms".format(self.db_v)]
        ko_list = []
        if len(tmp_list) == 1:
            ko_set = set()
    ...
    def find_compound_ko(self):
        ...
        """
        if self.db_v == 'v94.2':
            ref_kegg_compound = self.ref_db["kegg_v202007_br08001"]
        else:
            ref_kegg_compound = self.ref_db["kegg_{}_br08001".format(self.db_v)]
        ko_list = []
    ...
    def find_kegg(self, ko_list=None):
        ...
        :return:
        """
        ref_kegg_db = self.ref_db["kegg_{}_pathway_level".format(self.db_v)]
    ...
    def run_img_v1(self, ko, path_C, outdir):
        ...
        png_coll = self.ref_db['kegg_v94.2_pathway_png']
    ...
    def run_img(self, ko, path_C, outdir,html_db=None):  #20190612
        ...
        """
        # print "pathway: %s" % ko
        print("***run_img: ko: {} path_C: {}".format(ko, path_C))
        if not html_db:
        ...
        if  os.path.exists(kgml_ori_path) and os.path.exists(png_ori_path) and os.path.exists(html_ori_path) and os.path.getsize(kgml_ori_path) !=0:
            ...
            for each in path_C:
                ...
                compound_color_dic[each] = "#FF0000" #red   #"#00FF00" green
            canvas = KGMLCanvas(p_kgml, import_imagemap=True, label_compounds=False, label_orthologs=False,
                                label_reaction_entries=False, label_maps=True, show_maps=False,
                                draw_relations=True, show_orthologs=True, show_compounds=True, show_genes=True,
                                show_reaction_entries=False)
            ...
            if os.path.exists(html_path):
                ...
                map_html.get_html_compound_mark(compound_color_dic)
...
class KeggHtml:
    ...
    def read_html(self):
        ...
        maps = body.find('map')
        if maps == None:
            maps = body.find_class('image-block')[0].find('map')
        ...
        with open(self.out_file, 'w') as mark:
            ...
            for area in areas:
                ...
                if area_dict['shape'] in ['circle']:
                    kos = area_dict['href'].split('/')[-1].split('?')[-1].split('+')
                    hit_cs = list(set(kos) & set(self.compounds))
                    print("***circle: hit_cs:{}".format(hit_cs))
                    color =''
