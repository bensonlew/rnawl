    ...
    def get_genome_fasta(self, query, blast, ani, database, out):
        ...
        with open(database, "r") as g:
            ...
            for line in lines[1:]:
                ...
                tson[lin[0]] = [lin[1],lin[2]]
        blast1 = {}
        with open(blast, "r") as d:
            lines = d.readlines()
            for line in lines:
                lin = line.strip().split("\t")
                name = "_".join(lin[1].split("_")[1:])
                blast1[name] = lin[2]
        ...
        with open(out+".taxon.xls", "w") as f,open(out+".anno.xls", "w") as g:
            g.write("sample\tref\tsimilarity\tani\tGTDB taxon\tNCBI taxon\n")
            if data2.shape[0] ==0:
                f.write("{}\t{}\n".format(query, "new species"))
                for dd in top20:
                    sample = dd.split("/")[-1].split("_genomic.fna")[0]
                    ani = data.ix[top20.index(dd),2]
                    des = "\t".join(tson[sample])
                    g.write("{}\t{}\t{}\t{}\t{}\n".format(query, sample, str(blast1[sample]), str(ani), des))
            elif data2.shape[0] >0:
                name = data2.ix[0,1].split("/")[-1].split("_genomic.fna")[0]
                gtdb = tson[name][0]
                f.write("{}\t{}\n".format(query, gtdb))
                for ss in top20:
                    sample = ss.split("/")[-1].split("_genomic.fna")[0]
