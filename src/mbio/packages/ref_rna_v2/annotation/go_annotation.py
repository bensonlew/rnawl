...
def main(query, host, user, passwd, db, dir_out):
    ...
    level4_handle = open(os.path.join(dir_out, 'level4.stat.tsv'), 'w')
    level3_handle = open(os.path.join(dir_out, 'level3.stat.tsv'), 'w')
    level2_handle = open(os.path.join(dir_out, 'level2.stat.tsv'), 'w')
    level4_handle.write('GO (Lev1)\tGO Term (Lev2)\tGO ID (Lev2)\tGO Term (Lev3)\tGO ID (Lev3)\tGO Term (Lev4)\tGO ID (Lev4)\tSeq Number\tPercent\tSeq List\n')
    level3_handle.write('GO (Lev1)\tGO Term (Lev2)\tGO ID (Lev2)\tGO Term (Lev3)\tGO ID (Lev3)\tSeq Number\tPercent\tSeq List\n')
    level2_handle.write('GO (Lev1)\tGO Term (Lev2)\tGO ID (Lev2)\tSeq Number\tPercent\tSeq List\n')
    majorterm = {
                    ...
                    if j == 2:
                        if zterm.has_key(y):
                            if zterm[y] != '':
                                ...
                                level2_handle.write(tname + '\t' + y + '\t' + majorterm[majorkey][2][y] + '\t' + str(zterm[y]) + '\t' + str('%.8f' % pct) + '\t' + genes[y] + '\n')
                    for x in xterm:
                        ...
                        if j == 3:
                            if zterm.has_key(x):
                                if zterm[x] != '':
                                    ...
                                    level3_handle.write(tname + '\t' + y + '\t' + majorterm[majorkey][2][y] + '\t' + x + '\t' + majorterm[majorkey][3][x] + '\t' + str(zterm[x]) + '\t' + str('%.8f' % pct) + '\t' + genes[x] + '\n')
                        for w in wterm:
                            if j == 4:
                                if zterm.has_key(w):
                                    if zterm[w] != '':
                                        pct = float(zterm[w]) / float(len(all_gene))
                                        level4_handle.write(tname + '\t' + y + '\t' + majorterm[majorkey][2][y] + '\t' + x + '\t' + majorterm[majorkey][3][x] + '\t' + w + '\t' + majorterm[majorkey][4][w] + '\t' + str(zterm[w]) + '\t' + str('%.8f' % pct) + '\t' + genes[w] + '\n')
    else:
        level4_handle.close()
        level3_handle.close()
        level2_handle.close()
