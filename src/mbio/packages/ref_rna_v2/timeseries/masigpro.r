...
sig.rsq <- NBg[["sig.genes"]][[comparison]][["sig.pvalues"]]["R-squared"]
...
for (cluster in unique(vec.cluster)) {
  ...
  pdf(paste(opts$output, paste("profile", cluster, "pdf", sep = "."), sep = "/"))
  PlotProfiles(df.plot, cond = rownames(df.design), main = cluster, ylim = vec.ylim, repvect = df.design[["Replicate"]])
  dev.off()
  png(paste(opts$output, paste("profile", cluster, "png", sep = "."), sep = "/"), width = 4200, height = 4200, res = 600)
  PlotProfiles(df.plot, cond = rownames(df.design), main = cluster, ylim = vec.ylim, repvect = df.design[["Replicate"]])
  dev.off()
  svg(paste(opts$output, paste("profile", cluster, "svg", sep = "."), sep = "/"))
  PlotProfiles(df.plot, cond = rownames(df.design), main = cluster, ylim = vec.ylim, repvect = df.design[["Replicate"]])
  dev.off()
  if (flag) {
    pdf(paste(opts$output, paste("groups", cluster, "pdf", sep = "."), sep = "/"))
    self.plot.groups(df.plot, lst.design)
    dev.off()
    png(paste(opts$output, paste("groups", cluster, "png", sep = "."), sep = "/"), width = 4200, height = 4200, res = 600)
    self.plot.groups(df.plot, lst.design)
    dev.off()
    svg(paste(opts$output, paste("groups", cluster, "svg", sep = "."), sep = "/"))
    self.plot.groups(df.plot, lst.design)
    dev.off()
  } else {
    pdf(paste(opts$output, paste("groups", cluster, "pdf", sep = "."), sep = "/"))
...
write.table(df.out, paste(opts$output, 'result.tsv', sep = "/"), quote = FALSE, sep = "\t", row.names = FALSE)
