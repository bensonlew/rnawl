...
if (length(unique(df.design[["Time"]])) != 1){
col.Time <- colorRamp2(
  c(min(df.design[["Time"]]), median(df.design[["Time"]]), max(df.design[["Time"]])),
  topo.colors(3))
}
int.unit <- max(apply(df.design['Sample'], 1, nchar)) + 10
if (length(unique(df.design[["Time"]])) != 1){
