...
class RmatsToolkit(object):
    ...
    def get_event_transcripts(self):
        ...
        for event_type in self.process_func_dict:
            ...
            for i, row in df.iterrows():
                ...
                event_txpt_data.update({'{}_{}'.format(event_type, _id): txpt_info})
            else:
    ...
    def get_sample_event_coordinates(self):
        test_conf = os.path.join(self.lastod, 'B_group_bam.txt')
        ctrl_conf = os.path.join(self.lastod, 'A_group_bam.txt')
        ...
        ctrl_samples = [os.path.basename(p)[:-4] for p in open(test_conf).read().split(',')]
        ...
        for n, row in df.iterrows():
            ijc1s = map(int, row['IJC_SAMPLE_1'].split(',')) if not pd.isnull(row['IJC_SAMPLE_1'])\
                else [0] * len(test_samples)
            sjc1s = map(int, row['SJC_SAMPLE_1'].split(',')) if not pd.isnull(row['SJC_SAMPLE_1'])\
                else [0] * len(test_samples)
            ijc2s = map(int, row['IJC_SAMPLE_2'].split(',')) if not pd.isnull(row['IJC_SAMPLE_2'])\
                else [0] * len(ctrl_samples)
            sjc2s = map(int, row['SJC_SAMPLE_2'].split(',')) if not pd.isnull(row['SJC_SAMPLE_2'])\
                else [0] * len(ctrl_samples)
            ic1s = map(int, row['IC_SAMPLE_1'].split(',')) if not pd.isnull(row['IC_SAMPLE_1'])\
                else [0] * len(test_samples)
            sc1s = map(int, row['SC_SAMPLE_1'].split(',')) if not pd.isnull(row['SC_SAMPLE_1'])\
                else [0] * len(test_samples)
            ic2s = map(int, row['IC_SAMPLE_2'].split(',')) if not pd.isnull(row['IC_SAMPLE_2'])\
                else [0] * len(ctrl_samples)
            sc2s = map(int, row['IC_SAMPLE_2'].split(',')) if not pd.isnull(row['IC_SAMPLE_2'])\
                else [0] * len(ctrl_samples)
...
if __name__ == '__main__':
    ...
    inst = RmatsToolkit(gtf=args.gtf, lastod=args.od)
    inst.run()
