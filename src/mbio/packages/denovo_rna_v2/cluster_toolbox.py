...
def hcluster(exp_pd, transpose=False, n_clusters=10, method='average',
    ...
    try:
        ...
        z = sch.linkage(exp_pd, method=method, metric=metric)
    labels = exp_pd.index
    tree = to_tree(z, labels) ## to_tree方法有问题，应该直接调用R包里生成树的方法
...
def to_tree2(merge, height, labels):
    """
    tree, subcluster, z
    :param zclust： hclust.linkage result
    :param labels: leaf label from DataFrame.columns
    ...
    for i, j in merge:
        ...
        if j in clust_step_detail:
            j = clust_step_detail[j]
        h_str = '{:g}'.format(h/2)
        clust_step_detail[new_node] = '({i}:{h},{j}:{h})'.format(i=i, j=j, h=h_str)
        # print clust_step_detail[new_node]
    else:
        tree = clust_step_detail[new_node]
...
def r_hcluster(exp_pd, transpose=False, n_clusters=10, method='average',
    ...
    def r_hclust_func(exp_pd, method='complete', metric='euclidean', n_clusters=None):
        ...
        try:
            ...
            print("r_write_tree 失败, 使用to_tree2生成树")
            merge = np.array(hc[0])
            height = np.array(hc[1])
            label = np.array(hc[3])
            tree_str = to_tree2(merge, height, label)
            tree_list = re.findall('[(,]([^(]*?):', tree_str)
        # get subcluster
        if n_clusters:
            r_cut_tree = r['cutree']
    ...
    # write out tree
    out_dir = os.path.join(output, prefix + "cluster_tree.txt")
    out_dit_tre = os.path.join(output, prefix + 'cluster_tree.tre')
    with open(out_dir, 'w') as f:
        f.write(tree_str + '\n')
        f.write(";".join(tree_list) + '\n')
    with open(out_dit_tre, 'w') as f1:
        f1.write(tree_str + ';' + '\n')
    return z
...
def exp_cluster(exp_scale, n_clusters=10, s_cluster=True, g_cluster=True,
                ...
                use_r=True,output=None, static_p=False, r=None, rscrip=None, group4heatmap=None):
    result = []
    if use_r == True:
        hcluster = r_hcluster
    if s_cluster:
    ...
    if static_p :
        ...
        if ("seq.cluster_tree.txt" in results) or ("seq.kmeans_cluster.txt" in results):
            ...
            sub_path = os.path.join(output, 'sub_group')
            with open(sub_path, 'w') as sub_group:
                sub_group.write('seq_id' + '\t' + 'group2' + '\n')
                for sub in sub_clusters:
                    sub_cluster_id = int(sub.split('_')[1])
                    target_file = os.path.join(output, sub)
                    with open(target_file, 'r') as tar:
                        for line in tar.readlines():
                            if line.strip().startswith('seq_id'):
                                continue
                            seq_id = line.strip().split('\t')[0]
                            sub_group.write(seq_id + '\t' + str(sub_cluster_id) + '\n')
        else:
            sub_path = 'False'


        if g_cluster_type == 'hierarchy':
            seq_tree = os.path.join(output, 'seq.cluster_tree.tre')
        if g_cluster_type == 'kmeans' :
            seq_tree = 'kmeans'
        if not g_cluster:
            seq_tree = 'False'
        if not s_cluster:
            sample_tree = 'False'
        if s_cluster_type == 'hierarchy':
            sample_tree = os.path.join(output, 'sample.cluster_tree.tre')
        #     cmd = '{} {} -i {} -s {} -m {} -o {} -g {} -q {}'.format(
        #         r, rscrip, out_dir, sample_tree, seq_tree, output, group4heatmap, sub_path
        #     )
        # if g_cluster_type == 'kmeans':
        #     cmd = '{} {} -i {} -s {} -m {} -o {} -g {} -q {}'.format(
        #         r, rscrip, out_dir, sample_tree, 'False', output, group4heatmap, sub_path
        #     )
...
if __name__ == '__main__':
    ...
    parser.add_argument('-log_base', type=int, metavar="log_base", default=None, help="log base for transformation exp ")
    ...
    parser.add_argument('-R',dest='r', help='R')
    parser.add_argument('-Rscrip', dest='rscrip', help='Rscrip')
    parser.add_argument('-static', dest='static', help='plot heatmap')
    parser.add_argument('-group4heatmap', dest='group4heatmap', help="file with two col: sample\tgroup")
    ...
    if args.static:
        exp_cluster(exp_scale, n_clusters=args.n_clusters,
                    ...
                    s_method=args.scm, g_method=args.gcm,
                    s_metric=args.scd, g_metric=args.gcd,
                    output=args.out, static_p=args.static, r=args.r, rscrip=args.rscrip, group4heatmap=args.group4heatmap)
    else:
        exp_cluster(exp_scale, n_clusters=args.n_clusters,
                    s_cluster=s_cluster, g_cluster=g_cluster,
                    s_cluster_type=args.sct, g_cluster_type=args.gct,
