...
class AllAnnoStat(object):
    ...
    def get_anno_stat(self, gene2trans, tran_outpath, gene_outpath, new_gtf_path, ref_gtf_path, length_path, gene_file, cog_list=None, kegg_table=None, gos_list=None, blast_nr_table=None, blast_swissprot_table=None, pfam_domain=None):
        """
        ...
        """
        if gene2trans:
            self.get_unigene(gene2trans=gene2trans)
        else:
        ...
        if length_path and not gene2trans :
            self.get_length(length_path=length_path)
        with open(tran_outpath, 'wb') as w:
            if gene2trans:
                head = 'transcript\tgene_id\tis_gene\tlength\tcds\tcog\tcog_description\tKO_id\tKO_name\tpaths\tpfam\tgo\tnr\tswissprot\n'
            else:
                head = 'transcript\tgene_id\tgene_name\tlength\tcog\tcog_description\tKO_id\tKO_name\tpaths\tpfam\tgo\tnr\tswissprot\n'
            w.write(head)
        ...
        with open(gene_outpath, 'wb') as w:
            if gene2trans:
                head = 'gene_id\tcog\tcog_description\tKO_id\tKO_name\tpaths\tpfam\tgo\tnr\tswissprot\n'
            else:
                head = 'gene_id\tgene_name\tcog\tcog_description\tKO_id\tKO_name\tpaths\tpfam\tgo\tnr\tswissprot\n'
            w.write(head)
    ...
    def get_unigene(self, gene2trans):
        ...
        with open (gene2trans, 'r') as f:
            ...
            for line in lines:
                ...
                if len(line) > 4:
                    self.stat_info[line[0]].cds = line[4]
                else:
                    self.stat_info[line[0]].cds = ""
...
if __name__ == '__main__':
    ...
    if sys.argv[12] == 'None':
        sys.argv[12] = None
    if len(sys.argv) > 14:
        inst = AllAnnoStat(kegg_version=sys.argv[14])
    else:
        inst = AllAnnoStat()
