...
class KeggAnnotation(object):
                                    ...
                                    if i in ko_list:
                                        ...
                                        if map_id not in self.gloabl:
                                            ...
                                            if map_id in self.path2ko:
                                                if item["koid"] in self.path2ko[map_id]:
                                                    pass
                                                else:
                                                    self.path2ko[map_id].append(item["koid"])
                                            else:
                                                self.path2ko[map_id]=[item["koid"]]
                                else:
                                    map_id = re.sub("ko", "map", i)
                                    if map_id not in self.gloabl:
                                        self.path[map_id] = result['pathway_category'][index]
        ...
        for key in path_table:
            if key:
                ...
                with open(kos_path, "w") as w:
                    w.write("#KO\tbg\tfg\n")
                    for k in koids:
                        if key in self.path2ko and k in self.path2ko[key]:
                ...
                self.get_pic(r_path, map_path, key, kos_path, png_path, html_path)
                # if image_magick:
    ...
    def get_pic(self, r_path, map_path, path, kos_path, png_path, html_path):
        """
        画通路图
        """
    ...
    def run(self, r_path, map_path, blast_xml, kegg_ids, kegg_table, pidpath, pathwaydir, pathway_table, layerfile, taxonomy=None, link_bgcolor="green", png_bgcolor= "#00CD00", image_magick=None, html_path="/database/KEGG/kegg_2017-05-01/kegg/pathway/map"):
        ...
        self.html_path = Config().SOFTWARE_DIR + html_path
...
if __name__ == '__main__':
    ...
    kegg_anno.run(r_path=sys.argv[1], map_path=sys.argv[2], blast_xml=sys.argv[3], kegg_ids=sys.argv[4], kegg_table=sys.argv[5],
                  ...
                  layerfile=sys.argv[9], taxonomy=sys.argv[10], link_bgcolor=sys.argv[11], png_bgcolor=sys.argv[12], image_magick=sys.argv[13],html_path=sys.argv[14])
