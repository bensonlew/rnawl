...
class nr_stat(object):
    ...
    def nr_stat_info2(self, tran_taxonfile, outpath, level=7):
        """
        传入只含有域界门纲目科属种的转录本的注释信息，以及基因序列的列表，并且设置分类水平，即可统计在该分类水平的物种对应的转录本、基因的数目和百分比
        tran_taxonfile：只含有域界门纲目科属种的转录本的注释信息文件
        gene_list：基因序列的列表，type：list
        ...
        with open(tran_taxonfile, 'rb') as f:
            for line in f:
                ...
                species2class[species_name] = class_name
                if species_name in count_info:
                    count_info[species_name]['tran'] += 1
                else:
                    new_dict = {}
        ...
        for species in count_info:
            count_info[species]['species'] = species
            count_list.append(count_info[species])
        count_list_sorted = sorted(count_list, key=itemgetter('tran'), reverse=True)

        with open(outpath, 'wb') as w:
            w.write('taxon\tseq_num\tseq_percent\tclass\n')
            for species in count_list_sorted:
                w.write('{}\t{}\t{}\t{}\n'.format(species['species'], species['tran'],  '%0.4g' % (species['tran'] / tran_num * 100), species2class[species['species']]))


    def nr_stat_species(self, tran_taxonfile, gene_list, exp, outpath, level=7):
        '''
        根据表达统计物种丰度
        '''
        if gene_list != "":
            with open(gene_list, 'r') as gf:
                tran2gene  = {line.strip().split("\t")[0]: line.strip().split("\t")[1] for line in gf if line.strip().split("\t")[2] == "yes"}
            with open(tran_taxonfile, 'r') as taxon_f:
                seq2species =  {line.strip("\n").split("\t")[0]: line.strip("\n").split("\t")[2].split(';')[-1].split("{")[0]   for line in taxon_f}
            transs = set(tran2gene.keys()).intersection(set(seq2species.keys()))
            seq2species = {tran2gene[trans]: seq2species[trans] for trans in transs}
        else:
            with open(tran_taxonfile, 'r') as taxon_f:
                seq2species =  {line.strip("\n").split("\t")[0]: line.strip("\n").split("\t")[2].split(';')[-1].split("{")[0]   for line in taxon_f}

        species_list = seq2species.values()
        spe_dict = {spe: species_list.count(spe) for spe in list(set(species_list))}

        species_list_sorted =  sorted(spe_dict.keys(), key=lambda x:spe_dict[x], reverse=True)
        count_df = pd.read_table(exp, header=0)
        count_df["species"] = count_df["seq_id"].map(seq2species)
        count_dfs = count_df.groupby("species")

        with open(outpath, 'w') as spe_sum:
            spe_sum.write("species\tseq_num\t" + "\t".join(list(count_df.columns)[1:-1]) + "\n")
            for spe in species_list_sorted:
                spe_df = count_dfs.get_group(spe)
                sum_list = spe_df.drop(["seq_id", "species"], axis=1).sum()
                sum_list = map(int, sum_list)
                spe_sum.write(spe + "\t" + str(spe_dict[spe]) + "\t" + "\t".join(map(str, sum_list)) + "\n")

    def nr_stat_species2(self, tran_taxonfile, gene_list, exp, outpath, level=7):
        '''
        根据表达统计物种丰度
        '''
        if gene_list != "":
        ...
        seq_num = len(seq2species)
        species_list = seq2species.values()
        spe_dict = {spe: species_list.count(spe) for spe in list(set(species_list))}

        species_list_sorted = sorted(spe_dict.keys(), key=lambda x:spe_dict[x], reverse=True)
        count_df = pd.read_table(exp, header=0)
        samples = list(count_df.columns)[1:]
        # print seq2species
        count_df["species"] = count_df["seq_id"].map(seq2species)
        ...
        with open(outpath, 'w') as spe_sum:
            spe_sum.write("species\tnum\tpercent\tclass\t" + "\t".join(list(count_df.columns)[1:-1]) + "\n")
            for spe in species_list_sorted:
                try:
                    spe_df = count_dfs.get_group(spe)
                    sum_list = spe_df.drop(["seq_id", "species"], axis=1).sum()
                    sum_list = map(int, sum_list)
                    spe_sum.write(spe + "\t" + str(spe_dict[spe]) + "\t" + str(round(float(spe_dict[spe])/float(seq_num), 4))  + "\t" + species2class[spe] + "\t" + "\t".join(map(str, sum_list)) + "\n")
                except:
                    print "{} not in exp_file".format(spe)
                    pass
...
if __name__ == '__main__':
    nr1 = nr_stat()
    nr1.nr_stat_species(sys.argv[1], "gene_list", sys.argv[2], "outpath", level=7)
