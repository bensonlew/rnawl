...
class Expresscor(object):
    def __init__(self, padjust_way="fdr_bh", corr_way="spearmanr",
                 output=None, exp_matrix=None, cor_cutoff=None,
    ...
    def calculate_corr(self, pvalue_cutoff=None, qvalue_cutoff=None):
        ...
        if not self.sig_type:
            df_selected_temp = df_info[(df_info["cor"] > self.cor_cutoff) & (df_info["p_value"] < self.pvalue_cutoff)]
        ...
        if self.sig_type:
            df_selected_temp = df_info[(df_info["cor"] > self.cor_cutoff) & (df_info["q_value"] < self.qvalue_cutoff)]
            df_selected_temp1 = df_selected_temp[df_selected_temp['gene_id_1'] != df_selected_temp['gene_id_2']]
            df_selected_temp1.to_csv(out_dir_file2, sep="\t", index=False)
            with open(out_dir_file2) as f_tmp, open(out_dir_file3, "w") as f_tmp1:
                temp_list = list()
        ...
        if self.g_or_t.lower() == "t":
            df_gene_pre = df_anno.loc[:, ['transcript', 'gene_name']]
            idx = df_gene_pre[df_gene_pre['gene_name'].isnull()].index
            df_gene_pre.loc[idx, 'gene_name'] = df_gene_pre.loc[idx, 'transcript']
            df_gene_tmp = df_gene_pre.drop_duplicates('transcript', keep='first', inplace=False)
            id2name_dict = OrderedDict(zip(df_gene_tmp.transcript, df_gene_tmp.gene_name))
