...
GetOptions (\%opts,"i=s","env=s","pe=i","o=s","rg=s","rtop=i","method=s","d_method=s","binary=s");
my $usage = <<"USAGE";
Program : $0
Version : $VERSION
...
Usage:perl $0 [options]
		...
		-env *        env.txt/map.txt
		-o *          output dir name
		-rg	      row group,given a file with two column[tax1     group1
                                                                     tax2     group2] 
								    tax3	group3]
...
USAGE
die $usage if ( !(defined $opts{i}&&$opts{env}&&$opts{o}));
