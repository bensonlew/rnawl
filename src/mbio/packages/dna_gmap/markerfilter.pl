...
my $homo="aa";
# $Seg||=0.05;
$Seg=0.05 if($Seg ne "0" && !$Seg);
#$Miss||=0.3;
$Miss=0.3 if($Miss ne "0" && !$Miss);
$Pop||="F2";
$minPMdep=10 if($minPMdep ne "0" && !$minPMdep);
$minOdep=5 if($minOdep ne "0" && !$minOdep);
$maxPMdep||=1000000;
$maxOdep||=1000000;
$minPMdep=~s/_//;
$minOdep=~s/_//;
...
while (<In>) {
	...
	if (/^#/) {
		...
		next if ($tdep == 0);
		next if ($tdep/scalar@info < $minOdep);#子代深度
		next if ($tdep/scalar@info > $maxOdep);#子代深度
		$Type{$gtype}{$vtype}++;
