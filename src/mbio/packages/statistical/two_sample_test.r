library(parallel)
system.time({
otu_data <- read.table("${inputfile}",sep = "\t",comment.char = '', colClasses="character", quote="")
samp <- t(otu_data[1,-1])
otu_data <- otu_data[-1,]
...
two_sample_test <- function(i){
  ...
  samp1 <- c(signif(pro1*100,4))
  samp2 <- c(signif(pro2*100,4))
...
if(test == "chi"){
  colnames(result) <- c(" ",paste(s1,"-propotion",sep=''),paste(s2,"-propotion",sep=''),"statistic","pvalue","corrected_pvalue")
result_order <- result[order(-(as.numeric(result[,2])+as.numeric(result[,3]))),]
}else{
  colnames(result) <- c(" ",paste(s1,"-propotion",sep=''),paste(s2,"-propotion",sep=''),"odds_ratio","pvalue","corrected_pvalue")
result_order <- result[order(-(as.numeric(result[,2])+as.numeric(result[,3]))),]
}
if(lendata == 1){
