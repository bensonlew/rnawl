...
class CodeMap(dict):
    ...
    def trace(self, code, lineno, prev_lineno):
        ...
        time_now = time.strftime("%X", time.localtime()) # add time @ 20180724
        self[code][lineno] = (previous_inc + (memory-prev_line_memory), max(memory, previous_memory), time_now)
    ...
    def wrap_function(self, func):
        ...
        def f(*args, **kwds):
            ...
            try:
                ...
                timestamps.append(_get_memory(os.getpid(), self.backend, timestamps=True, filename=filename))  # add
                self.disable_by_count()

        return f

    # add function
    def show_result(self, stream=None):
        if stream is None:
            stream = sys.stdout
        for func, timestamps in self.functions.items():
            function_name = "%s.%s" % (func.__module__, func.__name__)
...
def show_results(prof, stream=None, precision=1):
    if stream is None:
        stream = sys.stdout
    template = '{0:>6} {1:>12} {2:>12} {3:>12}   {4:<}'

    for (filename, lines) in prof.code_map.items():
        header = template.format('Line #', 'Mem usage', 'Increment', 'End time',
                                 'Line Contents')
                                # 'Time now')
        ...
        for (lineno, mem) in lines:
            if mem:
                ...
                # tmp = template.format(lineno, mem, inc, timestr)
                # stream.write(unicode(tmp, 'UTF-8'))
                # stream.write("debug %s \n localtime: %s \n" % (timestr, time_now))
            else:
                mem = u''
                inc = u''
                timestr = u''
                # time_now = u'\n'  # add time @ 20180720
            tmp = template.format(lineno, mem, inc, timestr, all_lines[lineno - 1])
            stream.write(unicode(tmp, 'UTF-8'))
        stream.write(u'\n\n')
...
def profile(func=None, stream=None, precision=1, backend='psutil'):
    ...
    if func is not None:
        def wrapper(*args, **kwargs):
            ...
            # prof_time = TimeStamper(backend)
            val = prof(func)(*args, **kwargs)
            show_results(prof, stream=stream, precision=precision)
            # prof_time.show_results(stream=stream)
            # prof.show_result(stream=stream)
            return val
