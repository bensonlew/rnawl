...
my ( $type, $community, $dist, $environment, $outdir, $group, $color, $shape, $point_lab, $priorSpe_pick, $pick_number, $rda_threshold, $width, $height, $label_font, $label_cex, $pca_env, $env_labs, $envfit, $scale, $summary);
GetOptions(
    "type=s"          => \$type,
...
my $usage = <<"USAGE";
    ...
    Usage : perl $0 -type pca-pcoa-nmds-rda [options]
        ...
        -summary        [T/F] whether to output summary file
...
if (r_pca == \"T\") {
    ...
    data_pca <- data_community[,apply(data_community,2,var)!= 0] # 将方差设置为不等于零
    ...
    site_data <- pca_sites
    eigen <- pca_analysis\$sdev
    eigen <- eigen[1:dim(site_data)[2]]
    eigenpre <- pca_importance[1:dim(site_data)[2]]
    pca_rotation <- pca_summary\$rotation
    pca_rotation_all <- pca_rotation
    if(nrow(pca_rotation) > 100) {
        pca_rotation <- pca_rotation[1:100, ]
    }
    summaryname <- paste(o_dir, \"/pca/pca.txt\", sep = \"\")
    if (summary==\"T\") {
        eig_header <- paste(\"Eigvals\",length(eigen),sep=\"\\t\")
        prop_header <- paste(\"Proportion explained\",length(eigenpre),sep=\"\\t\")
        spe_header <- paste(\"Species\",\"0\",\"0\",sep=\"\\t\")
        site_header <- paste(\"Site\",dim(site_data)[1],dim(site_data)[2],sep=\"\\t\")
        bi_header <- paste(\"Biplot\",\"0\",\"0\",sep=\"\\t\")
        constraints_header <-  paste(\"Site constraints\",\"0\",\"0\",sep=\"\\t\")
        sink(summaryname)
        cat(eig_header,sep=\"\\n\",append=TRUE)
        cat(eigen,sep=\"\\t\",append=TRUE)
        cat(\"\\n\\n\",append=TRUE)
        cat(prop_header,sep=\"\\n\",append=TRUE)
        cat(eigenpre,sep=\"\\t\",append=TRUE)
        cat(\"\\n\\n\",append=TRUE)
        cat(spe_header,sep=\"\\n\",append=TRUE)
        cat(\"\\n\",append=TRUE)
        cat(site_header,sep=\"\\n\",append=TRUE)
        sink()
        write.table(as.data.frame(site_data),summaryname,sep=\"\\t\",append=TRUE,col.names = FALSE,quote = FALSE)
        df_append <- data.frame(x=c(\"0\",0),y=c(\"0\\n\",\"0\\n\"),row.names=c(\"\\nBiplot\",\"Site constraints\"))
        write.table(df_append, summaryname, sep=\"\\t\",append=TRUE,col.names = FALSE,quote = FALSE)
    }
    if ( g_design != \"\" ) {
...
if (r_pcoa == \"T\") {
    ...
    site_data <- pcoa_point
    eigenvalues <- pcoa_analysis\$eig
    eigen <- eigenvalues[1:dim(site_data)[2]]
    eigenvaluespre <- abs(pcoa_analysis\$eig)\/sum(abs(pcoa_analysis\$eig))
    eigenpre <- eigenvaluespre[1:dim(site_data)[2]]
    ...
    summaryname <- paste(o_dir, \"/pcoa/pcoa.txt\", sep = \"\")
    if (summary==\"T\") {
        eig_header <- paste(\"Eigvals\",length(eigen),sep=\"\\t\")
        prop_header <- paste(\"Proportion explained\",length(eigenpre),sep=\"\\t\")
        spe_header <- paste(\"Species\",\"0\",\"0\",sep=\"\\t\")
