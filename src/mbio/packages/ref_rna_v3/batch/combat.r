...
#   which(apply(matrix(counts[, batch==b]), 1, function(x){!all(length(levels(as.factor(x)))==1)}))
#   } else{
#   which(apply(counts[, batch==b], 1, function(x){!all(length(levels(as.factor(x)))==1)}))
# }})
# keep_batch <- Reduce(intersect, keep_batch)
# keep_group = lapply(levels(group), function(b){
#   if (class(counts[,group==b])=='numeric'){
#   which(apply(matrix(counts[, group==b]), 1, function(x){!all(length(levels(as.factor(x)))==1)}))
#   } else{
#   which(apply(counts[, group==b], 1, function(x){!all(length(levels(as.factor(x)))==1)}))
# }})
#修改写法
keep_batch <- lapply(levels(batch), function(b){
      if (class(counts[,batch==b])=='numeric'){
  which(apply(matrix(counts[, batch==b]), 1, function(x){!sum(x)==0}))
  } else{
  which(apply(counts[, batch==b], 1, function(x){!sum(x)==0}))
}})
keep_batch <- Reduce(intersect, keep_batch)
keep_group = lapply(levels(group), function(b){
  if (class(counts[,group==b])=='numeric'){
  which(apply(matrix(counts[, group==b]), 1, function(x){!sum(x)==0}))
  } else{
  which(apply(counts[, group==b], 1, function(x){!sum(x)==0}))
}})
