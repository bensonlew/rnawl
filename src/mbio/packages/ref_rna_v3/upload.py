...
class Upload(object):
    ...
    def set_rmats(self,idir, odir, task_id):
        ...
        for document in self.database['sg_splicing_rmats'].find({'task_id': task_id}).batch_size(10):
    ...
    def get_gid2des_dct(self,task_id):
        query_id = self.database['sg_annotation_query'].find_one({'task_id': task_id})['main_id']
        lst = list({(document['gene_id'], document['description']) for document in
                    self.database['sg_annotation_query_detail'].find({'query_id': query_id})})
    ...
    def export_rmats_detail(self,splicing_id, gid2des_dct,gid2name_dct, s2, s1, output_dir):
        ...
        for d in self.database['sg_splicing_rmats_detail'].find({'splicing_id': ObjectId(splicing_id)}):
            eid = d['event_id']
