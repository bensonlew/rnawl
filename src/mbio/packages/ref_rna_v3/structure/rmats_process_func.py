...
def check_rmats_out_dir(dirpath):
    if OUTPUT_FILES.difference(set(os.listdir(dirpath))):
        ...
        def drop_quotations(filepath):
            lines = [line.replace('"', '') for line in open(filepath)]
...
def get_event_stats(files, pvalue_fdr='fdr', fdr=0.05, psi=0):
    ...
    for filepath in files:
        m = re.match(r'(\S+?)\.MATS.(JC|JCEC)\.alter_id\.txt', os.path.basename(filepath))
        if m:
            print ('start reading {}'.format(filepath))
            event_type = m.group(1)
            dct[event_type]['{}_file'.format(m.group(2))] = filepath
...
def get_event_type(files):
    ...
    for filepath in files:
        m = re.match(r'fromGTF\.(novelEvents\.)?(A3SS|A5SS|MXE|RI|SE)\.alter_id\.txt', os.path.basename(filepath))
        if m:
            print ('INFO: start reading {}'.format(filepath))
            event_type = m.group(2)
            df = pd.read_table(filepath)
            if m.group(1):
                dct[event_type]['novel_event_file'] = filepath
                dct[event_type]['novel_event_id_no'] = len(set(df['ID']))
                dct[event_type]['novel_event_id_set'] = set(df['ID'])
            else:
                dct[event_type]['all_event_file'] = filepath
...
def write_big_detail_file(out, big_file, pvalue_fdr="fdr", fdr=0.05, psi=0):
    ...
    for event_type in LEGAL_EVENT_TYPE:
        ...
        all_event_file_type = os.path.join(out, 'fromGTF.%s.alter_id.txt' % event_type)
        novel_event_file_type = os.path.join(out, 'fromGTF.novelEvents.%s.alter_id.txt' % event_type)
        ...
        for event_id in all_event_set_type:
            ...
            if event_id in ReadsOnTargetAndJunctionCounts_event_set_type:
                if pvalue_fdr.lower() == 'fdr':
                    if float(ReadsOnTargetAndJunctionCounts_info_type.loc[event_id]['FDR']) <= fdr and abs(
                            float(ReadsOnTargetAndJunctionCounts_info_type.loc[event_id]['IncLevelDifference'])) >= psi:
                        diff_ReadsOnTargetAndJunctionCounts = 'yes'
                    else:
                        diff_ReadsOnTargetAndJunctionCounts = 'no'
                else:
                    if float(ReadsOnTargetAndJunctionCounts_info_type.loc[event_id]['PValue']) <= fdr and abs(
                            float(ReadsOnTargetAndJunctionCounts_info_type.loc[event_id]['IncLevelDifference'])) >= psi:
                        diff_ReadsOnTargetAndJunctionCounts = 'yes'
                    else:
                        diff_ReadsOnTargetAndJunctionCounts = 'no'
            if event_id in JunctionCountOnly_event_set_type:
                if pvalue_fdr.lower() == 'fdr':
                    if float(JunctionCountOnly_info_type.loc[event_id]['FDR']) <= fdr and abs(
                            float(JunctionCountOnly_info_type.loc[event_id]['IncLevelDifference'])) >= psi:
                        diff_JunctionCountOnly = 'yes'
                    else:
                        diff_JunctionCountOnly = 'no'
                else:
                    if float(JunctionCountOnly_info_type.loc[event_id]['PValue']) <= fdr and abs(
                            float(JunctionCountOnly_info_type.loc[event_id]['IncLevelDifference'])) >= psi:
                        diff_JunctionCountOnly = 'yes'
                    else:
                        diff_JunctionCountOnly = 'no'
            ...
            event_str = '\t'.join(
                [str(e) for e in [event_id, event_type, novel, old, all_event_info_type.at[event_id, 'GeneID'],
