...
sub printCMD {
	...
	print STDERR "\n\t-backward (backward distance from Tss)\n";
	print STDERR "\n\t-forward (forward distance from Tss)\n";
...
open(OUT, "> promoter.bed") or die "can not open promoter.bed";
if ($mode eq 'tss' || $mode eq 'tts') {
	foreach(keys %genes) {
		...
		print OUT "$c\t$s\t$e\t$printID\t$score\t$d\n";
	}
} elsif ($mode eq 'rna') {
