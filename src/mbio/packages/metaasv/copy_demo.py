...
class CopyDemo(Base):
    ...
    def _change_old_doc(self, collection, old_doc_dict):
        ...
        if 'asv_id' in old_doc_dict  and collection != 'asv' :
            old_otu_id = str(old_doc_dict['asv_id'])
            old_doc_dict['asv_id'] = ObjectId(self.otu_id_map[old_otu_id])
        ...
        if 'params' in old_doc_dict:
            ...
            if params_str and params_str != "null" and len(params_str) > 1:
                ...
                if 'asv_id' in params_dict and collection != 'asv':  # 后面再单独更新asv 表中的params 的otu_id
                    old_otu_id = params_dict['asv_id']
                    params_dict['asv_id'] = self.otu_id_map[old_otu_id]
    ...
    def copy_target_main_table(self, collection,update_status=False, origin_filter=True):   # 不要copy sg_status
        ...
        for doc_dict in doc_dict_list:
            ...
            if collection == "specimen":
                ...
                self.copy_detail(old_sample, main_table_id, 'specimen_detail', 'specimen_id') ##用名称想对应
                specimen_details2 = self.db['specimen_detail'].find({"specimen_id": main_table_id})
                specimen_details = self.db['specimen_detail'].find({"specimen_id": ObjectId(old_sample)})
                origin_specimen_dict = {}
                if specimen_details:
                    for s_detail in specimen_details:
                        old_specimen_id = str(s_detail["_id"])
                        origin_name = s_detail["specimen"]
                        if origin_name not in origin_specimen_dict:
                            origin_specimen_dict[origin_name] = old_specimen_id
                print('origin_specimen_dict: %s'%(origin_specimen_dict))
                if specimen_details2:
                    for o_detail in specimen_details2:
                        new_specimen_id = str(o_detail["_id"])
                        new_name = o_detail["specimen"]
                        if new_name in origin_specimen_dict:
                            old_specimen_id = origin_specimen_dict[new_name]
                            if old_specimen_id not in self.sample_map:
                                print('insert %s : %s'%(old_specimen_id, str(new_specimen_id)))
                                self.sample_map[old_specimen_id] = str(new_specimen_id)
                self.db['specimen'].update({"_id":main_table_id},{"$set":{"main_id":main_table_id}})
                print('specimen_detail: %s'%(self.sample_map))

            elif collection == 'specimen_group':
                old_group_id = str(new_doc_dict['main_id'])
                self.group_map[old_group_id] = str(main_table_id)
                self.db['specimen_group'].update({"_id":main_table_id},{"$set":{"main_id":main_table_id}})

            elif collection =='specimen_pair_group':
                old_pair_group_id = str(new_doc_dict['main_id'])
                self.pair_group_map[old_pair_group_id] = str(main_table_id)
                self.db['specimen_pair_group'].update({"_id":main_table_id},{"$set":{"main_id":main_table_id}})

            elif collection == 'env':
                old_env_id = str(new_doc_dict['main_id'])
                self.env_map[old_env_id] = str(main_table_id)
                details = self.db['env_detail'].find({"env_id":new_doc_dict['main_id']},{"_id":0})
                insert_data = []
                for detail in details:
                    detail['env_id'] = main_table_id
                    insert_data.append(detail)
                self.db['env_detail'].insert_many(insert_data)
                self.db['env'].update({"_id":main_table_id},{"$set":{"main_id":main_table_id}})

            elif collection == 'env_group':
                old_env_group_id = str(new_doc_dict['main_id'])
                self.env_group_map[old_env_group_id] = str(main_table_id)
                self.db['env_group'].update({"_id":main_table_id},{"$set":{"main_id":main_table_id}})


            elif collection == 'asv':
                old_main_id = new_doc_dict['main_id']
                details = self.db['asv_specimen'].find({"asv_id":old_main_id},{"_id":0})
                insert_data = []
                for detail in details:
                    detail['asv_id'] = main_table_id
                    old_sample_id = str(detail['specimen_id'])
                    detail['specimen_id'] = ObjectId(self.sample_map[old_sample_id])
                    insert_data.append(detail)
                self.db['asv_specimen'].insert_many(insert_data)

                self.copy_detail(old_main_id, main_table_id, 'asv_detail','asv_id')
                self.copy_detail(old_main_id, main_table_id, 'asv_detail_level','asv_id')
                # self.copy_detail(old_main_id, main_table_id, 'asv_seq','otu_id')
                self.copy_detail(old_main_id, main_table_id, 'asv_specimen_stat','asv_id')

                self.otu_id_map[str(old_main_id)] = str(main_table_id)
                self.db['asv'].update({"_id":main_table_id},{"$set": {"main_id": main_table_id}})
    ...
    def copy_detail(self, ori_main_id, new_main_id,detail_collection, main_id_name):
        ...
        for detail in details:
            ...
            if detail_collection in ['specimen_detail','asv_detail_level']:
    ...
    def updata_sg_otu_otu_id(self):
        sg_otus = self.db['asv'].find({"task_id" : self._new_task_id})
        for sg_otu in sg_otus:
            update_dic = {}
            if 'params' in sg_otu:
                params = json.loads(sg_otu['params'])
                old_otu_id = params['asv_id']
                params['asv_id'] = self.otu_id_map[old_otu_id]
                update_dic['params'] = json.dumps(params, sort_keys=True, separators=(',', ':'))
            if 'asv_id' in sg_otu:
                new_from_id = self.otu_id_map[str(sg_otu['asv_id'])]
                update_dic['asv_id'] = ObjectId(new_from_id)
            if 'asv_id' in sg_otu:
                new_otu_id = self.otu_id_map[str(sg_otu['asv_id'])]
                update_dic['asv_id'] = ObjectId(new_otu_id)

            if update_dic:
                self.db['asv'].update({"_id": sg_otu['_id']},{"$set": update_dic})
    ...
    def run(self):
        ...
        for c in ['sg_task', 'specimen','specimen_group','specimen_pair_group','env','env_group','asv']:  ##先复制样本，分组，在 sg_otu ,再是其他的主表
            if c == 'asv':
