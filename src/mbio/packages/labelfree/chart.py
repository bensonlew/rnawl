...
class Chart(object):
	def __init__(self):
		...
		self.mode_dir = chart_dir + "/labelfree"
		self.mode_mode = chart_dir + "/model"
		self.mode_mode2 = chart_dir + "/labelfree/highchart_model"
		self.js_list = list()
		self.work_dir = os.environ.get("PWD", "") + "/"
		self.output_dir = os.environ.get("PWD", "") + "/output/"
		self.rcolor_dict = self.get_rcolor_dict()

	def os_path_exists(self, afile):
		if os.path.exists(afile):
			if len(open(afile, 'r').readlines()) > 1:
				return True
			else:
				print "this file is empty or no innerContent: "+str(afile)
				return False
		else:
			return False
	...
	def chart_pfam_domain(self,pfam_domain_xls):
		...
		pfam_series =  pd.value_counts(pfam_domain_df.iloc[:,3])
		pfam_df = pfam_series.to_frame()
		pfam_df['index'] = pfam_df.index
		pfam_df = pfam_df.sort_values(by = ['Domain', 'index'], ascending = [False, True]).head(20)
		categories = pfam_df['index'].tolist()
		num = pfam_df['Domain'].tolist()
	...
	def chart_diff(self,group_list,num_summary_file,allsummary_file,group_file_list,sig_type="pvalue"):
		p_column = "log10pvalue"
		if sig_type == "pvalue":
			p_column = "log10pvalue"
		if sig_type == "padjust":
			p_column = "log10padjust"
		...
		for i,this_group_name in enumerate(group_list):
			...
			data_nosig = diff_df[diff_df[tmp_group_name] == 'no'][[diff_df.columns[2], diff_df.columns[1], diff_df.columns[0], diff_df.columns[6], p_column]].values.tolist()
			data_down = diff_df[diff_df[tmp_group_name+'_down'] == 'yes'][[diff_df.columns[2], diff_df.columns[1], diff_df.columns[0], diff_df.columns[6], p_column]].values.tolist()
			data_up = diff_df[diff_df[tmp_group_name+'_up'] == 'yes'][[diff_df.columns[2], diff_df.columns[1], diff_df.columns[0], diff_df.columns[6], p_column]].values.tolist()
			...
			# data_nosig_volcano = diff_df[diff_df[tmp_group_name] == 'no'][[diff_df.columns[5], diff_df.columns[p_column], diff_df.columns[0], diff_df.columns[5], diff_df.columns[p_column]]].values.tolist()
			# data_down_volcano = diff_df[diff_df[tmp_group_name+'_down'] == 'yes'][[diff_df.columns[5], diff_df.columns[p_column], diff_df.columns[0], diff_df.columns[5], diff_df.columns[p_column]]].values.tolist()
			# data_up_volcano = diff_df[diff_df[tmp_group_name+'_up'] == 'yes'][[diff_df.columns[5], diff_df.columns[p_column], diff_df.columns[0], diff_df.columns[5], diff_df.columns[p_column]]].values.tolist()
			diff_df[p_column] = -diff_df[p_column]
			data_nosig_volcano = diff_df[diff_df[tmp_group_name] == 'no'][["log2fc", p_column, "accession_id", "log2fc", p_column]].values.tolist()
			data_down_volcano = diff_df[diff_df[tmp_group_name+'_down'] == 'yes'][["log2fc", p_column, "accession_id", "log2fc", p_column]].values.tolist()
			data_up_volcano = diff_df[diff_df[tmp_group_name+'_up'] == 'yes'][["log2fc", p_column, "accession_id", "log2fc", p_column]].values.tolist()
	...
	def chart_proteinsetcluster_goclass(self, diff_group_list, proteinsetgo_class_all_files_list, proteinsetgo_class_updown_files_list):
		for i,this_group_name in enumerate(diff_group_list):
			...
			proteinsetgo_class_updown_df_head20 = proteinsetgo_class_updown_df.sort_values(by = [proteinsetgo_class_updown_df.columns[3],proteinsetgo_class_updown_df.columns[1]], ascending=[False,True]).iloc[:,[0,1,6,7,3,4]].drop_duplicates(subset=None,keep='first',inplace=False).iloc[:20,:]
			...
			proteinsetgo_class_updown_td_df_head20_ = proteinsetgo_class_updown_df.iloc[:,[0,1,3,6]].drop_duplicates(subset=None,keep='first',inplace=False).sort_values(by = [proteinsetgo_class_updown_df.columns[3],proteinsetgo_class_updown_df.columns[0]], ascending=False).iloc[:20,:]
			def get_sort(x):
	...
	def chart_proteinsetcluster_goclass_web(self, proteinsetgo_class_updown_file):
		...
		# proteinsetgo_class_updown_df_head20 = proteinsetgo_class_updown_df.sort_values(by = [proteinsetgo_class_updown_df.columns[need_list_1[-1]],proteinsetgo_class_updown_df.columns[1]], ascending=[False,True]).iloc[:,need_list_2].drop_duplicates(subset=None,keep='first',inplace=False).iloc[:20,:]
		proteinsetgo_class_updown_df_head20 = proteinsetgo_class_updown_df.sort_values(by = [proteinsetgo_class_updown_df.columns[need_list_1[-1]],proteinsetgo_class_updown_df.columns[1]], ascending=[False,True]).iloc[:,need_list_2].iloc[:20,:]
	...
	def chart_proteinsetcluster_gorich(self, diff_group_list, go_enrich_all_files, go_enrich_down_files, go_enrich_up_files):
		for i,this_group_name in enumerate(diff_group_list):
			...
			if go_enrich_all_file_df.shape[0] > 0:
				...
				self.chart_proteinsetcluster_gorichBar_tojs(this_group_name+"_all____"+this_group_name+'_go_enrich_bar', go_enrich_all_data, "proteinsetgo_richBar.json")
			...
			if go_enrich_down_file_df.shape[0] > 0:
				...
				self.chart_proteinsetcluster_gorichBar_tojs(this_group_name+"_down____"+this_group_name+'_go_enrich_bar', go_enrich_down_data, "proteinsetgo_richBar.json")
			...
			if go_enrich_up_file_df.shape[0] > 0:
				...
				self.chart_proteinsetcluster_gorichBar_tojs(this_group_name+"_up____"+this_group_name+'_go_enrich_bar', go_enrich_up_data, "proteinsetgo_richBar.json")
			...
			if go_enrich_all_file_df.shape[0] > 0:
				...
				# add blank list
				go_enrich_all_data_category = copy.deepcopy(go_enrich_all_data)
				existed_cate = list(set(list(go_enrich_all_data_.iloc[:,5])))
				add_blank_list_index = [{'BP':0,'CC':1,'MF':2}[i] for i in ['BP','CC','MF'] if i not in existed_cate]
				for abli in add_blank_list_index:
					go_enrich_all_data_category.insert(abli, [])
				#
				self.chart_proteinsetcluster_gorichDenseBubble_tojs(this_group_name+"_all____"+this_group_name+'_go_enrich_bubble', go_enrich_all_data, False,'densebubble', "proteinsetgo_DenseBubble.json")
				self.chart_proteinsetcluster_gorichDenseBubble_tojs(this_group_name+"_all____"+this_group_name+'_go_enrich_bubble2', go_enrich_all_data_category, True,'scatterbubble', "proteinsetgo_DenseBubble.json")
			...
			if go_enrich_down_file_df.shape[0] > 0:
				...
				# add blank list
				go_enrich_down_data_category = copy.deepcopy(go_enrich_down_data)
				existed_cate = list(set(list(go_enrich_down_data_.iloc[:,5])))
				add_blank_list_index = [{'BP':0,'CC':1,'MF':2}[i] for i in ['BP','CC','MF'] if i not in existed_cate]
				for abli in add_blank_list_index:
					go_enrich_down_data_category.insert(abli, [])
				#
				self.chart_proteinsetcluster_gorichDenseBubble_tojs(this_group_name+"_down____"+this_group_name+'_go_enrich_bubble', go_enrich_down_data, False,'densebubble', "proteinsetgo_DenseBubble.json")
				self.chart_proteinsetcluster_gorichDenseBubble_tojs(this_group_name+"_down____"+this_group_name+'_go_enrich_bubble2', go_enrich_down_data_category, True, 'scatterbubble',"proteinsetgo_DenseBubble.json")
			...
			if go_enrich_up_file_df.shape[0] > 0:
				...
				# add blank list
				go_enrich_up_data_category = copy.deepcopy(go_enrich_up_data)
				existed_cate = list(set(list(go_enrich_up_data_.iloc[:,5])))
				add_blank_list_index = [{'BP':0,'CC':1,'MF':2}[i] for i in ['BP','CC','MF'] if i not in existed_cate]
				for abli in add_blank_list_index:
					go_enrich_up_data_category.insert(abli, [])
				#
				self.chart_proteinsetcluster_gorichDenseBubble_tojs(this_group_name+"_up____"+this_group_name+'_go_enrich_bubble', go_enrich_up_data, False,'densebubble', "proteinsetgo_DenseBubble.json")
				self.chart_proteinsetcluster_gorichDenseBubble_tojs(this_group_name+"_up____"+this_group_name+'_go_enrich_bubble2', go_enrich_up_data_category, True,'scatterbubble', "proteinsetgo_DenseBubble.json")
	...
	def chart_proteinsetcluster_gorich_web(self, go_enrich_all_file):
		...
		self.chart_proteinsetcluster_gorichDenseBubble_tojs('enrichgo', go_enrich_all_data_category, True,'scatterbubble', "proteinsetgo_DenseBubble.json")
	...
	def chart_proteinsetcluster_keggclass(self, diff_group_list, kegg_class_stat_all_files, kegg_class_stat_updown_files, kegg_class_level_file):
		...
		for index,this_group_name in enumerate(diff_group_list):
			...
			if self.os_path_exists(kegg_stat_all_xls):
			...
			if self.os_path_exists(kegg_stat_updown_xls):
	...
	def chart_proteinsetcluster_keggrich(self, diff_group_list, kegg_enrich_all_files, kegg_enrich_down_files, kegg_enrich_up_files):
		...
		for i,this_group_name in enumerate(diff_group_list):
			...
			if self.os_path_exists(kegg_enrich_all_file):
				# Bar pdf
				kegg_enrich_all_file_df = pd.read_table(kegg_enrich_all_file)
				if kegg_enrich_all_file_df.shape[0] > 0:
			...
			if self.os_path_exists(kegg_enrich_down_file):
				kegg_enrich_down_file_df = pd.read_table(kegg_enrich_down_file)
				if kegg_enrich_down_file_df.shape[0] > 0:
			...
			#########################################################################################################
			if self.os_path_exists(kegg_enrich_up_file):
				kegg_enrich_up_file_df = pd.read_table(kegg_enrich_up_file)
				if kegg_enrich_up_file_df.shape[0] > 0:
	...
	def chart_proteinset_circ(self,diff_group_list, geneset_circ_choose_files, geneset_circ_zscore_files):
		for index,this_group_name in enumerate(diff_group_list):
			for ite in [['all','go'],['all','kegg'],['down','go'],['down','kegg'],['up','go'],['up','kegg']]:
				...
				if self.os_path_exists(geneset_circ_choose) and self.os_path_exists(geneset_circ_zscore):
	...
	def chart_pfam(self,diff_group_list, pfam_all_files, pfam_up_down_files):
		for index,this_group_name in enumerate(diff_group_list):
			if self.os_path_exists(pfam_all_files[index]):
			...
			#
			if self.os_path_exists(pfam_up_down_files[index]):
	...
	def chart_sample_pca(self, sample_pca_sites_file, sample_pca_propor_file,group_table):
		...
		for index, category in enumerate(list(set(group_dict.values()))):
			if index < len(shape):
				...
				shape_color_dic[category] = [shape[index%len(shape) - len(shape)], color[index%len(color) - len(color)]]
		# shape_color_dic = {category:[shape[index],color[index]] for index,category in  enumerate(list(set([i[:-1] for i in this_pca_sites_df.iloc[:,0].values.tolist()])))}
	...
	def chart_pt_goenrich(self, pt_goenrich_dir):
		...
		if gene_xls_file != pt_goenrich_dir and protein_xls_file != pt_goenrich_dir:
			...
			if protein_xls_df.shape[0] > 0 and gene_xls_df.shape[0] > 0:
	...
	def chart_pt_goenrich_bubble_tojs(self, name, data, json_mode):
		...
		with open(json_mode, 'r') as f, open(self.output_dir + name + ".bubble_groups.js", 'w') as fo:
			...
			fo.write("var options = ")
			fo.write(json.dumps(a, indent=4))
			self.js_list.append([self.output_dir + name + ".bubble_groups.js", {"model":"highchart","highchart_type":"bubble_groups","width":1050,"height":650}])
