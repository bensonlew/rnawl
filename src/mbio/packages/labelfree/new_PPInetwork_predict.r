...
#if(length(args) < 5)
#{
#            stop("Rscript  PPInetwork_predict.r <gene_list.txt> <species> <outdir> <combined_score>")
#}
...
cat("\n")
database <- args[5]
string_db<-try(STRINGdb$new(version="10", species = spec, input_directory=database))
if ('try-error' %in% class(string_db)){cat('wrong')}else{
diff_exp_example1 <- read.table(args[1], header = T, sep = "\t", colClass=c('STRING_id'='character'))
...
g <- try(graph.data.frame(relations, directed = F, vertices = actors))
if ('try-error' %in% class(g)){cat('wrong')}else{
