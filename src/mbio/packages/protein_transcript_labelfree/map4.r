...
path_KOs <- read.table(kos,head = T,comment.char = "")
...
for(i in 1:length(choose_xml[,1])){
  ...
  #print(choose_xml[i,])
  #print(bg_col)
  #print(choose_xml[i,]$name)
  fg_col = unlist(strsplit(as.character(path_KOs[choose_xml[i,]$name,]$fg),","))
  xy<-paste(choose_xml[i,]$x,choose_xml[i,]$y,sep="-")
  if (xy %in% names(ploted)){if (ploted[xy] == bg_col) {next} else {bg_col <- '#FFA500'}} else {tmp <-c(bg_col);names(tmp)<-xy; ploted <- c(ploted, tmp)}
  #print(ploted)
