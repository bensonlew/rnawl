...
def check_pollution_pip(summary_dir,task_id,out_dir,is_sanger='tsanger',sanger_mongo=None):
    ...
    os.system('cat {0}/otu_taxon_Genus.percent.full.xls {0}/otu_taxon_Family.percent.full.xls {0}/otu_taxon_Phylum.percent.full.xls > {1}/g_f_p.xls'.format(summary_dir,out_dir))
...
def get_pollution_from_mongo(task_ids,pol_list=None,sanger_mongo=None):
    ...
    if sanger_mongo:
        ...
        db_name = 'meta'
        db = Config().get_mongo_client(mtype=db_name)[Config().get_mongo_dbname(db_name)]
    otu_main_table = db['sg_otu']
    otu_detail_table = db['sg_otu_detail']
    otu_seq_table = db['sg_otu_seq']
    task_ids_result_files = {}
    for id in task_ids:
        print id
        try:
            ret_main = otu_main_table.find_one({'task_id': id,'name':'OTU_Taxon_Origin'})
            main_id = ret_main['_id']

            samples_info = otu_seq_table.find({"otu_id":main_id})
            samples_reads = {}
            if samples_info:
            ...
            with open(str(id)+'_pollution.xls','w') as fw:
                ...
                fw.write('#OTU ID\t'+'\t'.join(sample_list)+'\n')
...
if __name__ == '__main__':
    #check_pollution_pip(sys.argv[1],sys.argv[2],sys.argv[3])  #summary_dir,task_id,out_dir,
    

    #tsg
    #update_old_project_mongo_pip()

    '''
    #sanger
    end_time = "2019-9-8 00:42:35"
    start_time = "2019-8-8 00:42:35"
    task_mongo=sys.argv[2]
    sanger_mongo = sys.argv[3]

    update_old_project_mongo_pip(start_time=start_time,sanger_mongo=sanger_mongo, task_mongo=task_mongo)
    '''
