...
class KeggRegulate(object):
    ...
    def get_regulate_table(self, ko_gene, path_ko, regulate_gene, output, gene_list=None):
        ...
        with open(output, 'wb') as w:
            ...
            for path in path_ko:
                ...
                # 去除冗余，不同ko对应同一个基因会重复
                for gn in genelist_names:
                    write_dict[gn] = list(set(write_dict[gn]))
                # print link
                for gn in genelist_names:
    ...
    def get_regulate_table2(self, ko_gene, path_ko, regulate_gene, gene_kegg_gene, output, gene_list=None):
        """
        生成kegg调控统计表, 特定物种
        ko_gene:ko对应的gene信息:{'ko1': [gene1,gene2], ...,'ko2': [gene1,gene2]}
        path_ko:path对应的ko信息:{'pathway1': [ko1,ko2], ...,'pathway2': [ko1,ko2]}
        ...
        with open(output, 'wb') as w:
            ...
            print "path_ko is {}".format(path_ko)

            for path in path_ko:
                ko_list = []
                for ko_id in path_ko[path]:
                    ko_list.extend(ko_id.split(";"))
                ko_ids = set(ko_list)
                # up_genes = []
                # down_genes = []
                write_dict = {}
