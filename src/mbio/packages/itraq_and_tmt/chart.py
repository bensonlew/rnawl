...
class Chart(object):
	def __init__(self):
		...
		self.mode_dir = chart_dir + "/itraq_tmt"
		self.mode_mode = chart_dir + "/model"
		self.mode_mode2 = chart_dir + "/itraq_tmt/highchart_model"
		self.js_list = list()
	...
	def chart_protein_infomation(self, Protein_infomation_xls):
		infomation_df = pd.read_table(Protein_infomation_xls)
	...
	def chart_go12level_statistics(self, golevel_statistics_xls):
		...
		def get_ten(x):
		  df = x.drop_duplicates(subset=None,keep='first',inplace=False).sort_values(by = [x.columns[2],x.columns[1]], ascending=False)
		  return df.iloc[:10,:]
	...
	def chart_diff(self,group_list,num_summary_file,allsummary_file,group_file_list,sig_type="pvalue"):
		p_column = 9
		if sig_type == "pvalue":
			p_column = 9
		if sig_type == "padjust":
		...
		for i,this_group_name in enumerate(group_list):
			...
			data_nosig = diff_df[diff_df[tmp_group_name] == 'no'][[diff_df.columns[2], diff_df.columns[1], diff_df.columns[0], diff_df.columns[6], diff_df.columns[p_column]]].values.tolist()
			data_down = diff_df[diff_df[tmp_group_name+'_down'] == 'yes'][[diff_df.columns[2], diff_df.columns[1], diff_df.columns[0], diff_df.columns[6], diff_df.columns[p_column]]].values.tolist()
			data_up = diff_df[diff_df[tmp_group_name+'_up'] == 'yes'][[diff_df.columns[2], diff_df.columns[1], diff_df.columns[0], diff_df.columns[6], diff_df.columns[p_column]]].values.tolist()
			data = [data_nosig,data_down,data_up]
			self.chart_diff_tojs(this_group_name+'_scatter', categories, data, this_group_name, "diff.json")
			#
			diff_df.iloc[:,[p_column]] = -diff_df.iloc[:,[p_column]]
			data_nosig_volcano = diff_df[diff_df[tmp_group_name] == 'no'][[diff_df.columns[6], diff_df.columns[p_column], diff_df.columns[0], diff_df.columns[6], diff_df.columns[p_column]]].values.tolist()
			data_down_volcano = diff_df[diff_df[tmp_group_name+'_down'] == 'yes'][[diff_df.columns[6], diff_df.columns[p_column], diff_df.columns[0], diff_df.columns[6], diff_df.columns[p_column]]].values.tolist()
			data_up_volcano = diff_df[diff_df[tmp_group_name+'_up'] == 'yes'][[diff_df.columns[6], diff_df.columns[p_column], diff_df.columns[0], diff_df.columns[6], diff_df.columns[p_column]]].values.tolist()
	...
	def chart_proteinsetcluster(self, proteinsetcluster_group_list, expression_matrix_files_list, seq_tree_files_list, sample_tree_files_list):
		for i,this_group_name in enumerate(proteinsetcluster_group_list):
			...
			if os.path.exists(this_seq_tree_file):
				...
				this_tree_1 = ""
				this_tree_rows = this_expression_matrix_df.index.tolist()
			if os.path.exists(this_sample_tree_file):
				...
				this_tree_2 = ""
				this_tree_columns = this_expression_matrix_df.columns.tolist()

			this_expression_matrix_df = this_expression_matrix_df[this_tree_columns]
	...
	def chart_proteinsetcluster_goclass(self, diff_group_list, proteinsetgo_class_all_files_list, proteinsetgo_class_updown_files_list):
		for i,this_group_name in enumerate(diff_group_list):
			...
			if not proteinsetgo_class_all_df.empty:
				# ____________单向直方图-all__________
				proteinsetgo_class_all_df_head20 = proteinsetgo_class_all_df.iloc[:, [0, 1, 3, 4]].drop_duplicates(
					subset=None, keep='first', inplace=False).sort_values(
					by=[proteinsetgo_class_all_df.columns[3], proteinsetgo_class_all_df.columns[1]],
					ascending=[False, True]).iloc[:20, :]
				proteinsetgo_class_all_df_head20[proteinsetgo_class_all_df_head20.columns[3]] = \
				proteinsetgo_class_all_df_head20[proteinsetgo_class_all_df_head20.columns[3]].map(
					lambda x: x.split('(')[0]).astype(float)
				list_list = proteinsetgo_class_all_df_head20.values.tolist()
				self.chart_proteinsetcluster_goclass_tojs(this_group_name + "____" + this_group_name + '_all_go',
														  list_list, "proteinsetgo_class.json")
			if not proteinsetgo_class_updown_df.empty:
				# ____________单向直方图-up_down__________
				proteinsetgo_class_updown_df['tmp_sum'] = proteinsetgo_class_updown_df.iloc[:,
														  3] + proteinsetgo_class_updown_df.iloc[:, 6]
				proteinsetgo_class_updown_df_head20 = proteinsetgo_class_updown_df.sort_values(
					by=['tmp_sum', proteinsetgo_class_updown_df.columns[1]], ascending=[False, True]).iloc[:,
													  [0, 1, 6, 7, 3, 4]].drop_duplicates(subset=None, keep='first',
																						  inplace=False).iloc[:20, :]
				proteinsetgo_class_updown_df_head20[proteinsetgo_class_updown_df_head20.columns[3]] = \
				proteinsetgo_class_updown_df_head20[proteinsetgo_class_updown_df_head20.columns[3]].map(
					lambda x: x.split('(')[0]).astype(float)
				proteinsetgo_class_updown_df_head20[proteinsetgo_class_updown_df_head20.columns[5]] = \
				proteinsetgo_class_updown_df_head20[proteinsetgo_class_updown_df_head20.columns[5]].map(
					lambda x: x.split('(')[0]).astype(float)
				sub_categories = [proteinsetgo_class_updown_df_head20.columns[2].replace("num", ''),
								  proteinsetgo_class_updown_df_head20.columns[4].replace("num", '')]
				list_list = proteinsetgo_class_updown_df_head20.values.tolist()
				self.chart_proteinsetcluster_goclass_tojs(this_group_name + "____" + this_group_name + '_up_down_go',
														  list_list, "proteinsetgo_class_updown.json",
														  sub_categories=sub_categories)
				# ___________双向直方图-up_down___________
				proteinsetgo_class_updown_td_df_head20_ = proteinsetgo_class_updown_df.iloc[:,
														  [0, 1, 3, 6]].drop_duplicates(subset=None, keep='first',
																						inplace=False).sort_values(
					by=[proteinsetgo_class_updown_df.columns[3], proteinsetgo_class_updown_df.columns[1]],
					ascending=[False, True]).iloc[:20, :]

				def get_sort(x):
					df = x.sort_values(by=x.columns[2], ascending=False)
					return df

				proteinsetgo_class_updown_td_df_head20 = proteinsetgo_class_updown_td_df_head20_.groupby(
					proteinsetgo_class_updown_td_df_head20_.columns[0], as_index=False).apply(get_sort)
				color_dic = {'biological_process': '#FF00FF', 'cellular_component': '#388E3C',
							 'molecular_function': '#F44336'}
				abbr_dic = {'biological_process': 'BP', 'cellular_component': 'CC', 'molecular_function': 'MF'}
				categories = [i[1] + ':' + abbr_dic[i[0]] for i in
							  proteinsetgo_class_updown_td_df_head20.iloc[:, [0, 1]].values.tolist()]
				categories_color = [
					{'color': color_dic[i[0]], 'group': abbr_dic[i[0]], 'value': i[1] + ':' + abbr_dic[i[0]]} for i in
					proteinsetgo_class_updown_td_df_head20.iloc[:, [0, 1]].values.tolist()]
				...
				if colors_range_1:
					colors_range_11 = {'start': min(colors_range_1), 'end': max(colors_range_1), 'top_color': '#FF00FF',
									   'bottom_color': '#FF00FF'}
				...
				if colors_range_2:
					colors_range_22 = {'start': min(colors_range_2), 'end': max(colors_range_2), 'top_color': '#388E3C',
									   'bottom_color': '#388E3C'}
				...
				if colors_range_3:
					colors_range_33 = {'start': min(colors_range_3), 'end': max(colors_range_3), 'top_color': '#F44336',
									   'bottom_color': '#F44336'}
					colors_range.append(colors_range_33)

				proteinsetgo_class_updown_td_df_head20.iloc[:, [3]] = -proteinsetgo_class_updown_td_df_head20.iloc[:,
																	   [3]]
				data = proteinsetgo_class_updown_td_df_head20.iloc[:, [2, 3]].T.values.tolist()
				self.chart_proteinsetgo_class_updown_td_tojs(
					this_group_name + "____" + this_group_name + '_up_down_go_neg', data, colors_range, categories,
					categories_color, "proteinsetgo_class_updown_td.json")
			if not proteinsetgo_class_all_df.empty and not proteinsetgo_class_updown_df.empty:
				#____________单向直方图-all__________
				...
				list_list = proteinsetgo_class_all_df_head20.values.tolist()
				self.chart_proteinsetcluster_goclass_tojs(this_group_name+"____"+this_group_name+'_all_go', list_list, "proteinsetgo_class.json")

				#____________单向直方图-up_down__________
				proteinsetgo_class_updown_df['tmp_sum'] = proteinsetgo_class_updown_df.iloc[:,3] + proteinsetgo_class_updown_df.iloc[:,6]
				proteinsetgo_class_updown_df_head20 = proteinsetgo_class_updown_df.sort_values(by = ['tmp_sum',proteinsetgo_class_updown_df.columns[1]], ascending=[False,True]).iloc[:,[0,1,6,7,3,4]].drop_duplicates(subset=None,keep='first',inplace=False).iloc[:20,:]
				...
				sub_categories = [proteinsetgo_class_updown_df_head20.columns[2].replace("num",''),proteinsetgo_class_updown_df_head20.columns[4].replace("num",'')]
				list_list = proteinsetgo_class_updown_df_head20.values.tolist()
				self.chart_proteinsetcluster_goclass_tojs(this_group_name+"____"+this_group_name+'_up_down_go', list_list, "proteinsetgo_class_updown.json", sub_categories=sub_categories)

				#___________双向直方图-up_down___________
				proteinsetgo_class_updown_td_df_head20_ = proteinsetgo_class_updown_df.iloc[:,[0,1,3,6]].drop_duplicates(subset=None,keep='first',inplace=False).sort_values(by = [proteinsetgo_class_updown_df.columns[3],proteinsetgo_class_updown_df.columns[1]], ascending=[False,True]).iloc[:20,:]
				def get_sort(x):
				  df = x.sort_values(by = x.columns[2], ascending=False)
				  return df
	...
	def chart_proteinsetcluster_goclass_web(self, proteinsetgo_class_updown_file):
		...
		sub_categories = [list(proteinsetgo_class_updown_df.columns)[i].split(' ')[0] for i in need_list_1]
		...
		proteinsetgo_class_updown_df_head20 = proteinsetgo_class_updown_df.sort_values(by = ['tmp_sum',proteinsetgo_class_updown_df.columns[1]], ascending=[False,True]).iloc[:,need_list_2].drop_duplicates(subset=None,keep='first',inplace=False).iloc[:20,:]
		...
		list_list = proteinsetgo_class_updown_df_head20.values.tolist()
		self.chart_proteinsetcluster_goclass_tojs('webmulti_go', list_list, "proteinsetgo_class_updown.json",sub_categories=sub_categories)

		if colnum == 3:
			#___________双向直方图-up_down___________
			proteinsetgo_class_updown_td_df_head20_ = proteinsetgo_class_updown_df.iloc[:,[0,1,3,6]].drop_duplicates(subset=None,keep='first',inplace=False).sort_values(by = [proteinsetgo_class_updown_df.columns[6],proteinsetgo_class_updown_df.columns[1]], ascending=[False,True]).iloc[:20,:]
			def get_sort(x):
			  df = x.sort_values(by = x.columns[3], ascending=False)
			  return df
			proteinsetgo_class_updown_td_df_head20 = proteinsetgo_class_updown_td_df_head20_.groupby(proteinsetgo_class_updown_td_df_head20_.columns[0],as_index=False).apply(get_sort)
			color_dic = {'biological_process':'#FF00FF', 'cellular_component':'#388E3C', 'molecular_function':'#F44336'}
			abbr_dic = {'biological_process':'BP', 'cellular_component':'CC', 'molecular_function':'MF'}
			...
			proteinsetgo_class_updown_td_df_head20.iloc[:,[2]] = -proteinsetgo_class_updown_td_df_head20.iloc[:,[2]]
			data = proteinsetgo_class_updown_td_df_head20.iloc[:,[3,2]].T.values.tolist()
	...
	def chart_proteinsetcluster_goclass_tojs(self, name, list_list, json_mode, sub_categories = []):
		def randomcolor():
		...
		with open(json_mode, 'r') as f, open(self.output_dir + name + ".go_bar.js", 'w') as fo:
			...
			if len(sub_categories) >0:
				a["sub_categories"] = sub_categories
				a["sub_colors"] = [randomcolor() for i in sub_categories]*20
			fo.write("var options = ")
			fo.write(json.dumps(a, indent=4, sort_keys=True))
			self.js_list.append([self.output_dir + name + ".go_bar.js", {"model":"highchart","highchart_type":"go_bar","width":1100,"height":600}])
	...
	def chart_proteinsetcluster_gorich(self, diff_group_list, go_enrich_all_files, go_enrich_down_files, go_enrich_up_files):
		for i,this_group_name in enumerate(diff_group_list):
			...
			if not go_enrich_all_file_df.empty and not go_enrich_down_file_df.empty and not go_enrich_up_file_df.empty:
				go_enrich_all_file_df = go_enrich_all_file_df[(go_enrich_all_file_df.iloc[:, 2] == 'e')]
				go_enrich_all_file_df['new_percent'] = go_enrich_all_file_df.iloc[:, 4].str.split(pat="/",
																								  expand=True).iloc[:,
													   0].astype(int) / go_enrich_all_file_df.iloc[:, 5].str.split(
					pat="/", expand=True).iloc[:, 0].astype(int)
				go_enrich_all_data_ = go_enrich_all_file_df.sort_values(
					by=[go_enrich_all_file_df.columns[6], go_enrich_all_file_df.columns[3]], ascending=True).iloc[:20,
									  [1, 3, 11, 6]]
				...
				self.chart_proteinsetcluster_gorichBar_tojs(
					this_group_name + "_all____" + this_group_name + '_go_enrich_bar', go_enrich_all_data,
					"proteinsetgo_richBar.json")

				go_enrich_down_file_df = go_enrich_down_file_df[(go_enrich_down_file_df.iloc[:, 2] == 'e')]
				go_enrich_down_file_df['new_percent'] = go_enrich_down_file_df.iloc[:, 4].str.split(pat="/",
																									expand=True).iloc[:,
														0].astype(int) / go_enrich_down_file_df.iloc[:, 5].str.split(
					pat="/", expand=True).iloc[:, 0].astype(int)
				go_enrich_down_data_ = go_enrich_down_file_df.sort_values(
					by=[go_enrich_down_file_df.columns[6], go_enrich_down_file_df.columns[3]], ascending=True).iloc[:20,
									   [1, 3, 11, 6]]
				go_enrich_down_data_.iloc[:, 1] = go_enrich_down_data_.iloc[:, 1] + ':' + go_enrich_down_data_.iloc[:,
																						  0]
				go_enrich_down_data = go_enrich_down_data_.values.tolist()
				self.chart_proteinsetcluster_gorichBar_tojs(
					this_group_name + "_down____" + this_group_name + '_go_enrich_bar', go_enrich_down_data,
					"proteinsetgo_richBar.json")

				go_enrich_up_file_df = go_enrich_up_file_df[(go_enrich_up_file_df.iloc[:, 2] == 'e')]
				go_enrich_up_file_df['new_percent'] = go_enrich_up_file_df.iloc[:, 4].str.split(pat="/",
																								expand=True).iloc[:,
													  0].astype(int) / go_enrich_up_file_df.iloc[:, 5].str.split(
					pat="/", expand=True).iloc[:, 0].astype(int)
				go_enrich_up_data_ = go_enrich_up_file_df.sort_values(
					by=[go_enrich_up_file_df.columns[6], go_enrich_up_file_df.columns[3]], ascending=True).iloc[:20,
									 [1, 3, 11, 6]]
				...
				self.chart_proteinsetcluster_gorichBar_tojs(
					this_group_name + "_up____" + this_group_name + '_go_enrich_bar', go_enrich_up_data,
					"proteinsetgo_richBar.json")
			...
			if not go_enrich_all_file_df.empty and not go_enrich_down_file_df.empty and not go_enrich_up_file_df.empty:
				...
				def get_sort(x):
					...
					return [{'y': i[0], 'x': i[1], 'desc': i[2], 'name': i[3], 'size': i[4]} for i in
							df.values.tolist()]

				go_enrich_all_file_df['logUnpvalue'] = -np.log2(go_enrich_all_file_df.iloc[:, 6])
				go_enrich_all_data_ = go_enrich_all_file_df.sort_values(
					by=[go_enrich_all_file_df.columns[6], go_enrich_all_file_df.columns[1]], ascending=True).iloc[:20,
									  [12, 11, 3, 0, 8, 1, 6]]
				go_enrich_all_data__ = go_enrich_all_data_.sort_values(
					by=[go_enrich_all_data_.columns[5], go_enrich_all_data_.columns[6]], ascending=True).groupby(
					go_enrich_all_data_.columns[5], as_index=False).apply(get_sort)
				go_enrich_all_data = go_enrich_all_data__.values.tolist()
				self.chart_proteinsetcluster_gorichDenseBubble_tojs(
					this_group_name + "_all____" + this_group_name + '_go_enrich_bubble', go_enrich_all_data, False,
					'densebubble', "proteinsetgo_DenseBubble.json")
				self.chart_proteinsetcluster_gorichDenseBubble_tojs(
					this_group_name + "_all____" + this_group_name + '_go_enrich_bubble2', go_enrich_all_data, True,
					'scatterbubble', "proteinsetgo_DenseBubble.json")

				go_enrich_down_file_df = go_enrich_down_file_df[(go_enrich_down_file_df.iloc[:, 2] == 'e')]
				go_enrich_down_file_df['logUnpvalue'] = -np.log2(go_enrich_down_file_df.iloc[:, 6])
				go_enrich_down_data_ = go_enrich_down_file_df.sort_values(
					by=[go_enrich_down_file_df.columns[6], go_enrich_down_file_df.columns[1]], ascending=True).iloc[:20,
									   [12, 11, 3, 0, 8, 1, 6]]
				go_enrich_down_data__ = go_enrich_down_data_.sort_values(
					by=[go_enrich_down_data_.columns[5], go_enrich_down_data_.columns[6]], ascending=True).groupby(
					go_enrich_down_data_.columns[5], as_index=False).apply(get_sort)
				go_enrich_down_data = go_enrich_down_data__.values.tolist()
				self.chart_proteinsetcluster_gorichDenseBubble_tojs(
					this_group_name + "_down____" + this_group_name + '_go_enrich_bubble', go_enrich_down_data, False,
					'densebubble', "proteinsetgo_DenseBubble.json")
				self.chart_proteinsetcluster_gorichDenseBubble_tojs(
					this_group_name + "_down____" + this_group_name + '_go_enrich_bubble2', go_enrich_down_data, True,
					'scatterbubble', "proteinsetgo_DenseBubble.json")

				go_enrich_up_file_df = go_enrich_up_file_df[(go_enrich_up_file_df.iloc[:, 2] == 'e')]
				go_enrich_up_file_df['logUnpvalue'] = -np.log2(go_enrich_up_file_df.iloc[:, 6])
				go_enrich_up_data_ = go_enrich_up_file_df.sort_values(
					by=[go_enrich_up_file_df.columns[6], go_enrich_up_file_df.columns[1]], ascending=True).iloc[:20,
									 [12, 11, 3, 0, 8, 1, 6]]
				go_enrich_up_data__ = go_enrich_up_data_.sort_values(
					by=[go_enrich_up_data_.columns[5], go_enrich_up_data_.columns[6]], ascending=True).groupby(
					go_enrich_up_data_.columns[5], as_index=False).apply(get_sort)
				go_enrich_up_data = go_enrich_up_data__.values.tolist()
				self.chart_proteinsetcluster_gorichDenseBubble_tojs(
					this_group_name + "_up____" + this_group_name + '_go_enrich_bubble', go_enrich_up_data, False,
					'densebubble', "proteinsetgo_DenseBubble.json")
				self.chart_proteinsetcluster_gorichDenseBubble_tojs(
					this_group_name + "_up____" + this_group_name + '_go_enrich_bubble2', go_enrich_up_data, True,
					'scatterbubble', "proteinsetgo_DenseBubble.json")


	def chart_proteinsetcluster_gorich_web(self, go_enrich_all_file):
		# Bar pdf
		go_enrich_all_file_df = pd.read_table(go_enrich_all_file)
		go_enrich_all_file_df = go_enrich_all_file_df[(go_enrich_all_file_df.iloc[:,2] == 'e')]
		...
		go_enrich_all_data_.iloc[:,1] = go_enrich_all_data_.iloc[:,1]+':'+go_enrich_all_data_.iloc[:,0]
		go_enrich_all_data = go_enrich_all_data_.values.tolist()
		self.chart_proteinsetcluster_gorichBar_tojs('enrichgo', go_enrich_all_data, "proteinsetgo_richBar.json")
		...
		go_enrich_all_file_df['logUnpvalue'] = -np.log2(go_enrich_all_file_df.iloc[:,6])
		go_enrich_all_data_ = go_enrich_all_file_df.sort_values(by = [go_enrich_all_file_df.columns[6],go_enrich_all_file_df.columns[1]], ascending=True).iloc[:20,[12,11,3,0,8,1,6]]
		...
		go_enrich_all_data = go_enrich_all_data__.values.tolist()
		self.chart_proteinsetcluster_gorichDenseBubble_tojs('enrichgo', go_enrich_all_data, False,'densebubble', "proteinsetgo_DenseBubble.json")
		self.chart_proteinsetcluster_gorichDenseBubble_tojs('enrichgo', go_enrich_all_data, True,'scatterbubble', "proteinsetgo_DenseBubble.json")


	def chart_proteinsetcluster_gorichBar_tojs(self, name, data, json_mode):
		json_mode = self.mode_dir + "/" + json_mode
	...
	def chart_proteinsetcluster_gorichDenseBubble_tojs(self, name, data, scatter_or_not,bubble_type, json_mode):
		json_mode = self.mode_dir + "/" + json_mode
	...
	def chart_proteinsetcluster_keggrich(self, diff_group_list, kegg_enrich_all_files, kegg_enrich_down_files, kegg_enrich_up_files):
		...
		for i,this_group_name in enumerate(diff_group_list):
			...
			if os.path.exists(kegg_enrich_all_file):
				...
				self.chart_proteinsetcluster_keggrichBar_tojs(this_group_name+"_all____"+this_group_name+'_kegg_enrich_bar', kegg_enrich_all_data, "proteinsetkegg_richBar.json")
				...
				self.chart_proteinsetcluster_keggrichDenseBubble_tojs(this_group_name+"_all____"+this_group_name+'_kegg_enrich_bubble', kegg_enrich_all_data, False,'densebubble', "proteinsetkegg_DenseBubble.json")
				self.chart_proteinsetcluster_keggrichDenseBubble_tojs(this_group_name+"_all____"+this_group_name+'_kegg_enrich_bubble2', kegg_enrich_all_data, True,'scatterbubble', "proteinsetkegg_DenseBubble.json")
			...
			if os.path.exists(kegg_enrich_down_file):
				...
				self.chart_proteinsetcluster_keggrichBar_tojs(this_group_name+"_down____"+this_group_name+'_kegg_enrich_bar', kegg_enrich_down_data, "proteinsetkegg_richBar.json")
				...
				self.chart_proteinsetcluster_keggrichDenseBubble_tojs(this_group_name+"_down____"+this_group_name+'_kegg_enrich_bubble', kegg_enrich_down_data, False,'densebubble', "proteinsetkegg_DenseBubble.json")
				self.chart_proteinsetcluster_keggrichDenseBubble_tojs(this_group_name+"_down____"+this_group_name+'_kegg_enrich_bubble2', kegg_enrich_down_data, True, 'scatterbubble',"proteinsetkegg_DenseBubble.json")
			...
			if os.path.exists(kegg_enrich_up_file):
				...
				self.chart_proteinsetcluster_keggrichBar_tojs(this_group_name+"_up____"+this_group_name+'_kegg_enrich_bar', kegg_enrich_up_data, "proteinsetkegg_richBar.json")
				...
				self.chart_proteinsetcluster_keggrichDenseBubble_tojs(this_group_name+"_up____"+this_group_name+'_kegg_enrich_bubble', kegg_enrich_up_data, False,'densebubble', "proteinsetkegg_DenseBubble.json")
				self.chart_proteinsetcluster_keggrichDenseBubble_tojs(this_group_name+"_up____"+this_group_name+'_kegg_enrich_bubble2', kegg_enrich_up_data, True,'scatterbubble', "proteinsetkegg_DenseBubble.json")

			#########################################################################################################


	def chart_proteinsetcluster_keggrich_web(self, kegg_enrich_all_file):
		...
		if os.path.exists(kegg_enrich_all_file):
			...
			kegg_enrich_all_data_.iloc[:,1] = kegg_enrich_all_data_.iloc[:,1]+':'+kegg_enrich_all_data_.iloc[:,0]
			kegg_enrich_all_data = kegg_enrich_all_data_.values.tolist()
			self.chart_proteinsetcluster_keggrichBar_tojs('enrichkegg', kegg_enrich_all_data, "proteinsetkegg_richBar.json")
			...
			# kegg_enrich_all_data__ = kegg_enrich_all_data_.sort_values(by = [kegg_enrich_all_data_.columns[5],kegg_enrich_all_data_.columns[6]], ascending=True).groupby(kegg_enrich_all_data_.columns[5],as_index=False).apply(get_sort)
			# kegg_enrich_all_data = kegg_enrich_all_data__.values.tolist()
			self.chart_proteinsetcluster_keggrichDenseBubble_tojs('enrichkegg', kegg_enrich_all_data, False,'densebubble', "proteinsetkegg_DenseBubble.json")
			self.chart_proteinsetcluster_keggrichDenseBubble_tojs('enrichkegg', kegg_enrich_all_data, True,'scatterbubble', "proteinsetkegg_DenseBubble.json")


	def chart_proteinsetcluster_keggrichBar_tojs(self, name, data, json_mode):
		json_mode = self.mode_dir + "/" + json_mode
		with open(json_mode, 'r') as f, open(self.output_dir + name + ".shadowbar.js", 'w') as fo:
			a = json.loads(f.read())
			a["data"] = data
	...
	def chart_proteinsetcluster_keggrichDenseBubble_tojs(self, name, data, scatter_or_not,bubble_type, json_mode):
		json_mode = self.mode_dir + "/" + json_mode
		with open(json_mode, 'r') as f, open(self.output_dir + name + "."+bubble_type+".js", 'w') as fo:
			a = json.loads(f.read())
			a["data"] = data
			...
			for ite in [['all','go'],['all','kegg'],['down','go'],['down','kegg'],['up','go'],['up','kegg']]:
				...
				if os.path.exists(geneset_circ_choose) and os.path.exists(geneset_circ_zscore):
					...
					if ite[1] == 'go':
						...
						legendTitle = "KEGG Pathway"
					self.chart_proteinset_circ_tojs(this_group_name+'_'+ite[0]+"____"+this_group_name+'_'+ite[1]+'_chord', body, header,pdf_height_index, legendTitle,"proteinsetcirc.json")

	def chart_proteinset_circ_web(self, geneset_circ_choose_file, geneset_circ_zscore_file):
		for ite in ['go','kegg']:
			geneset_circ_choose = geneset_circ_choose_file.format(anno_type=ite)
			geneset_circ_zscore = geneset_circ_zscore_file
			if os.path.exists(geneset_circ_choose) and os.path.exists(geneset_circ_zscore):
				body = pd.read_table(geneset_circ_choose).values.tolist()
				pdf_height_index = pd.read_table(geneset_circ_choose).shape[0]
				header = pd.read_table(geneset_circ_zscore, header=None, index_col=0).values.tolist()
				if ite == 'go':
					...
					legendTitle = "KEGG Pathway"
				self.chart_proteinset_circ_tojs('chord', body, header,pdf_height_index, legendTitle,"proteinsetcirc.json")

	def chart_proteinset_circ_tojs(self, name, body, header,pdf_height_index, legendTitle, json_mode):
		json_mode = self.mode_dir + "/" + json_mode
		with open(json_mode, 'r') as f, open(self.output_dir + name + ".circ.js", 'w') as fo:
			...
			a["title"] = legendTitle
			a["legendTitle"] = legendTitle+':'
			fo.write("var options = ")
			fo.write(json.dumps(a, indent=4))
			# 41*2*k=838*3.14, k=32, so height=(rows*2*32)/3.14=rows*20.38~~rows*21
			self.js_list.append([self.output_dir + name + ".circ.js", {"model":"highchart","highchart_type":"circ","width":45*pdf_height_index,"height":21*pdf_height_index}])
	...
	def chart_cog(self,diff_group_list, cog_all_files, cog_up_down_files):
		for index,this_group_name in enumerate(diff_group_list):
			...
			if not cog_up_down_df.empty:
				...
				self.chart_cog_tojs(this_group_name + "____" + this_group_name + '_up_down_cog', categories, data,
									legend, "proteinsetcog_class.json")


	def chart_cog_web(self, cog_all_files):
		cog_all_df = pd.read_table(cog_all_files, index_col=False)
		categories = [i[1] for i in cog_all_df.iloc[:,1].values.tolist()]
		# if cog_all_df.shape[1] == 4:
		# 	data = [cog_all_df.iloc[:,2].values.tolist()]
		...
		data = [cog_all_df.iloc[:,i].values.tolist() for i in range(2,cog_all_df.shape[1],2)]
		legend = [list(cog_all_df.columns)[i][:-4] for i in range(2,cog_all_df.shape[1],2)]
	...
	def chart_sample_pca(self, sample_pca_sites_file, sample_pca_propor_file):
		this_pca_sites_df = pd.read_table(sample_pca_sites_file)
		shape = ["circle", "triangle-up", "diamond", "square", "del", "plus", "triangle-down", "circle", "triangle", "diamond", "square", "del", "plus", "triangle-down", "circle", "triangle","diamond", "square", "del", "plus", "triangle-down", "circle", "triangle", "diamond"]
		color = ["#388E3C", "#F44336", "#0288D1", "#FF9800", "#C0C0C0","#808080","#000000","#800000","#FFFF00","#808000","#00FF00","#00FFFF","#008080","#000080","#FF00FF","#800080","#E91E63","#F48FB1","#880E4F","#F50057","#C51162","#673AB7","#2196F3","#00BCD4"]
		shape_color_dic = {category:[shape[index],color[index]] for index,category in  enumerate(list([i[:-1] for i in this_pca_sites_df.iloc[:,0].values.tolist()]))}
		data = [{"name":i[0], "symbol_category_name":i[0][:-1], "color_category_name":i[0][:-1], "color":shape_color_dic[i[0][:-1]][1], "symbol":shape_color_dic[i[0][:-1]][0], "value":[i[1],i[2]]} for index,i in enumerate(this_pca_sites_df.iloc[:,[0,1,2]].values.tolist())]
	...
	def chart_proteinset_venn(self, diff_path_for_proteinset):
		...
		if len(diff_list_to_venn)>0:
			...
			for diff_file in [os.path.join(diff_path_for_proteinset,i) for i in diff_list_to_venn]:
				...
				if len(this_diff_df) == 0:
					venn_color = colors[:len(diff_list_to_venn)-1]
				else:
	...
	def chart_proteinset_venn_web(self, venndata):
		def randomcolor():
			colorArr = ['1','2','3','4','5','6','7','8','9','A','B','C','D','E','F']
			color = ""
			for i in range(6):
		...
		if len(venndata)>0:
			if len(venndata) <7:
				venn_color = colors[:len(venndata)]
			else:
				venn_color = []
				for i in range(len(venndata)):
					venn_color.append(randomcolor())

			all_data = [{"name":i[0], "data":i[1]} for i in venndata]
			self.chart_proteinset_venn_tojs("venn", all_data, venn_color, "proteinset_venn.json")
		else:
			print "当前项目的蛋白集为空，因此没有足够数据来绘制venn图。"
	...
	def chart_pt_goenrich(self, pt_goenrich_dir):
		...
		if gene_xls_file != pt_goenrich_dir and protein_xls_file != pt_goenrich_dir:
			...
			data_gene = []
			for i in protein_xls_data_:
				for ii in protein_xls_df.iloc[:,[6,11,3,8,0]].values.tolist():
					if i[0] == ii[4]:
						data_protein.append({'color':ii[0], 'x':ii[1], 'name':ii[2], 'size':ii[3]})
				for iii in gene_xls_df.iloc[:,[6,11,3,8,0]].values.tolist():
					if i[0] == iii[4]:
	...
	def chart_pt_goenrich_bubble_tojs(self, name, data, json_mode):
		...
		with open(json_mode, 'r') as f, open(self.output_dir + name + ".bubble_groups.js", 'w') as fo:
			...
			fo.write("var options = ")
			fo.write(json.dumps(a, indent=4))
			self.js_list.append([self.output_dir + name + ".bubble_groups.js", {"model":"highchart","highchart_type":"bubble_groups","width":1000,"height":650}])
	...
	def chart_pt_keggenrich(self, pt_keggenrich_dir):
		...
		if gene_xls_file != pt_keggenrich_dir and protein_xls_file != pt_keggenrich_dir:
			...
			data_gene = []
			for i in protein_xls_data_:
				for ii in protein_xls_df.iloc[:,[6,12,1,0,3]].values.tolist():
					if i[0][-5:] == ii[4][-5:]:
						if float(ii[0]) <= 0.5: 
							data_protein.append({'color':ii[0], 'x':ii[1], 'name':ii[2], 'size':ii[3]})
				for iii in gene_xls_df.iloc[:,[6,12,1,0,3]].values.tolist():
					if i[0][-5:] == iii[4][-5:]:
						if float(iii[0]) <= 0.5: 
							data_gene.append({'color':iii[0], 'x':iii[1], 'name':iii[2], 'size':iii[3]})
			data = [data_protein, data_gene]
	...
	def chart_wgcna_module_corr(self, module_corr, module_tree):
		...
		# print corr_heat
		# print newick_tree
		# print module2group_source
		self.chart_heat_tree("wgcna", ".mofule_corr", corr_heat, newick_tree, newick_tree, module2group_source, module2group_source, new_order, "wgcna.module_relation.heatmap_tree.json")

	def chart_heat_tree(self, name, out, corr_source, sample_tree, gene_tree, sample2group_source, gene2group_source, new_order, json_mode):
		json_mode = self.mode_dir + "/" + json_mode
		with open(json_mode, 'r') as f, open(self.output_dir + name + out + ".heat_corr.js", 'w') as fo:
			a = json.loads(f.read())
	...
	def generate_html_sh(self, para=True):
		...
		for js in self.js_list:
			...
			# elif "use_puppeteer" in para_dict and ('sanger' in str(Config().SOFTWARE_DIR) or 'isanger' in str(Config().SOFTWARE_DIR)):
	...
	def chart_json_batch(self, chart_json):
		...
		if "sample_corr_matrix" in chart_json and "sample_corr_tree" in chart_json:
			...
			self.chart_sample_corr(sample_corr_matrix_file, sample_corr_tree_file)

		if "sample_pca_sites" in chart_json and "sample_pca_propor" in chart_json:
			sample_pca_sites_file = chart_json["sample_pca_sites"]
			sample_pca_propor_file = chart_json["sample_pca_propor"]
			self.chart_sample_pca(sample_pca_sites_file, sample_pca_propor_file)
...
if __name__ == '__main__':
	...
	chart_json = {
		# "diff_group":["E_vs_C"],
		# "proteinsetcluster_group":["cluster_sample_5","cluster_sample_10","cluster_group_5","cluster_group_10"],
		# "protein_mw_distribution":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/qc/Protein_mw_distribution.xls",
		# "protein_seq_cover_distribution":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/qc/Protein_seq_cover_distribution.xls",
		# "protein_infomation":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/qc/Protein_infomation.xls",
		# "peptite_number_distribution":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/qc/Peptite_number_distribution.xls",
		# "peptite_length_distribution":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/qc/Peptite_length_distribution.xls",
		# "peptide_dmass":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/qc/dMass.xls",
		# "all_annotation_stat":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/ProteinAnnotation/output/anno_stat/all_annotation_statistics.xls",
		# "go12level_statistics":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/ProteinAnnotation/output/go/go12level_statistics.xls",
		# "go123level_statistics":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/ProteinAnnotation/output/go/go123level_statistics.xls",
		# "go1234level_statistics":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/ProteinAnnotation/output/go/go1234level_statistics.xls",
		# "kegg_layer":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/ProteinAnnotation/output/kegg/kegg_layer.xls",
		# "kegg_pathway":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/ProteinAnnotation/output/kegg/pathway_table.xls",
		# "cog_summary":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/ProteinAnnotation/output/cog/cog_summary.xls",
		# "pfam_domain":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/ProteinAnnotation/output/blast_xml/pfam_domain",
		# "subloc_stat":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/ProteinAnnotation/output/subloc/multiloc_stat.xls",
		# # "num_summary":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/Diff/num_summary.xls",
		# # "allsummary":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/Diff/allsummary.xls",
		# # "diff":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/Diff/output/{group_name}_diff.xls",
		# "num_summary":"/mnt/ilustre/users/sanger-dev/workspace/20210521/Diffexp_s5jn_s0ieqm5rf9dfelc1t88ajh_9938_9927/Diff/num_summary.xls",
		# "allsummary":"/mnt/ilustre/users/sanger-dev/workspace/20210521/Diffexp_s5jn_s0ieqm5rf9dfelc1t88ajh_9938_9927/Diff/allsummary.xls",
		# "diff":"/mnt/ilustre/users/sanger-dev/workspace/20210521/Diffexp_s5jn_s0ieqm5rf9dfelc1t88ajh_9938_9927/Diff/output/{group_name}_diff.xls",
		
		# "expression_matrix":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffCluster/output/{group_name}/expression_matrix.xls",
		# "seq_tree":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffCluster/output/{group_name}/seq.cluster_tree.txt",
		# "sample_tree":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffCluster/output/{group_name}/sample.cluster_tree.txt",
		
		# "proteinsetgo_class_all":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffGoClass/output/{group_name}/{group_name}_all_go_class_table.xls",
		# "proteinsetgo_class_updown":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffGoClass/output/{group_name}/{group_name}_up_down_go_class_table.xls",
		# #
		# "go_enrich_all":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffGoEnrich/output/{group_name}_all/go_enrich_{group_name}_all_protein.xls",
		# "go_enrich_down":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffGoEnrich/output/{group_name}_down/go_enrich_{group_name}_down_protein.xls",
		# "go_enrich_up":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffGoEnrich/output/{group_name}_up/go_enrich_{group_name}_up_protein.xls",
		# #
		# "kegg_class_level":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffKeggClass/protein_kegg_level_table.xls",
		# "kegg_class_stat_all":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffKeggClass/output/{group_name}_all/kegg_stat.xls",
		# "kegg_class_stat_updown":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffKeggClass/output/{group_name}_up_down/kegg_stat.xls",
		# #
		# "kegg_enrich_all":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffKeggEnrich/output/{group_name}_all/{group_name}_all_protein.list.DE.list.check.kegg_enrichment.xls",
		# "kegg_enrich_down":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffKeggEnrich/output/{group_name}_down/{group_name}_down_protein.list.DE.list.check.kegg_enrichment.xls",
		# "kegg_enrich_up":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffKeggEnrich/output/{group_name}_up/{group_name}_up_protein.list.DE.list.check.kegg_enrichment.xls",
		# # #
		# "geneset_circ_choose":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffCircle/output/{{anno_type}}_circle_{group_name}_{{all_up_down}}/{{anno_type}}_enrich_choose.table",
		# "geneset_circ_zscore":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffCircle/output/{{anno_type}}_circle_{group_name}_{{all_up_down}}/enrich_zscore",
		# #
		...
		# # #
		# "cog_all":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffCogClass/output/{group_name}/{group_name}_all_cog_class_table.xls",
		# "cog_up_down":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffCogClass/output/{group_name}/{group_name}_up_down_cog_class_table.xls",
		# #
		# "pfam_all":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffPfamStat/output/{group_name}_all/pfam_stat.xls",
		# "pfam_up_down":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffPfamStat/output/{group_name}_up_down/pfam_stat.xls",
		# #
		# "subloc_all":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffSublocStat/output/{group_name}_all/subloc_stat.xls",
		# "subloc_up_down":"/mnt/ilustre/users/sanger-dev/workspace/20210430/Itraqtmt_s5jn_s0ieqm5rf9dfelc1t88ajh/DiffSublocStat/output/{group_name}_up_down/subloc_stat.xls",
		# # # 下面画的两个图所在的页面 位于 样本比较分析（样本相关性热图，pca分析），这个分析模块是可选的
		# "sample_corr_tree":"/mnt/ilustre/users/sanger-dev/workspace/20210526/Itraqtmt_hpgg_nbl6mv5nhalvm4vih7im1p/ExpCorr/sample.cluster_tree.txt",
		# "sample_corr_matrix":"/mnt/ilustre/users/sanger-dev/workspace/20210526/Itraqtmt_hpgg_nbl6mv5nhalvm4vih7im1p/ExpCorr/sample_correlation.xls",
		# #
		# "sample_pca_sites":"/mnt/ilustre/users/sanger-dev/workspace/20210526/Itraqtmt_hpgg_nbl6mv5nhalvm4vih7im1p/ExpPcaMeta/output/pca_sites.xls",
		# "sample_pca_propor":"/mnt/ilustre/users/sanger-dev/workspace/20210526/Itraqtmt_hpgg_nbl6mv5nhalvm4vih7im1p/ExpPcaMeta/output/pca_importance.xls",
		# # # #
		# # # # 下面这个venn图是新增的，以前的页面上都是没有这个图的
		# "diff_path_for_proteinset_for_venn":"/mnt/ilustre/users/sanger-dev/workspace/20210526/Itraqtmt_hpgg_nbl6mv5nhalvm4vih7im1p/Diff",
		# above is pdf generated by main workflow
		#
		# protein_transcript analyse
		"protein_transcript_overview":"/mnt/ilustre/users/sanger-dev/workspace/20210527/ProteinTranscript_hpgg_nbl6mv5nhalvm4vih7im1p_1796_125/output/relationship_list",
		"protein_transcript_baseinfo":"/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210607/SampleProteinGene_hpgg_nbl6mv5nhalvm4vih7im1p_9981_814/output/relationship_list", 
		"protein_transcript_diff_allvenn":"/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210607/RelateDiff_hpgg_nbl6mv5nhalvm4vih7im1p_9021_2625/output/diff_relationship_list",
		"protein_transcript_diff_relatevenn":"/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210607/RelateDiff_hpgg_nbl6mv5nhalvm4vih7im1p_9021_2625/output/diff_venn.xls",
		"protein_transcript_diff_relatenine":"/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210607/RelateDiff_hpgg_nbl6mv5nhalvm4vih7im1p_9021_2625/output/nine_blocks.xls",
		"protein_transcript_diff_relatenine_relateparams":"2.0;2.0;1.2;0.83",#转录组的上下调差异倍数；蛋白组的上下调差异倍数；
		
		"pt_proteinsetcluster_group":["cluster_sample_5","cluster_sample_10","cluster_group_5","cluster_group_10"],
		"pt_expression_matrix":"/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210607/RelasetCluster_hpgg_nbl6mv5nhalvm4vih7im1p_6037_6479/ExpCluster/output/expression_matrix.xls",
		"pt_seq_tree":"/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210607/RelasetCluster_hpgg_nbl6mv5nhalvm4vih7im1p_6037_6479/ExpCluster/output/seq.cluster_tree.txt",
		
		"pt_corrScatter":"/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210607/RelasetCorr_hpgg_nbl6mv5nhalvm4vih7im1p_9581_5244/ProGeneCorr/output/protein_rna_density_matrix.xls",
		"pt_corrScatter_pvalue":"/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210607/RelasetCorr_hpgg_nbl6mv5nhalvm4vih7im1p_9581_5244/ProGeneCorr/output/corr_info",
		
		"pt_goclass":"/mnt/ilustre/users/sanger-dev/workspace/20210528/PrGoClass_hpgg_nbl6mv5nhalvm4vih7im1p_3730_7594/output/pro_gene_go_class.xls",
		
		"pt_goenrich":"/mnt/ilustre/users/sanger-dev/workspace/20210528/PrGoEnrich_hpgg_nbl6mv5nhalvm4vih7im1p_8574_8451/output",
		
		"pt_goenrich_cluster_matrix":"/mnt/ilustre/users/sanger-dev/workspace/20210528/PrGoEnrichCluster_hpgg_nbl6mv5nhalvm4vih7im1p_7910_959/EnrichCluster/output/cluster_matrix.xls",
		"pt_goenrich_cluster_tree":"/mnt/ilustre/users/sanger-dev/workspace/20210528/PrGoEnrichCluster_hpgg_nbl6mv5nhalvm4vih7im1p_7910_959/EnrichCluster/output/seq.cluster_tree.txt",
		#
		"pt_keggclass_level":"/mnt/ilustre/users/sanger-dev/workspace/20210528/PrKeggClass_hpgg_nbl6mv5nhalvm4vih7im1p_4986_9541/pr_kegg_level_table.xls",
		"pt_proteinset_file":"/mnt/ilustre/users/sanger-dev/workspace/20210528/PrKeggClass_hpgg_nbl6mv5nhalvm4vih7im1p_4986_9541/gene_protein.list",
		"pt_kegg_stat_file":"/mnt/ilustre/users/sanger-dev/workspace/20210528/PrKeggClass_hpgg_nbl6mv5nhalvm4vih7im1p_4986_9541/KeggClass/output/kegg_stat.xls",
		"pt_gene_kegg_level_table_xls":"/mnt/ilustre/users/sanger-dev/workspace/20210528/PrKeggClass_hpgg_nbl6mv5nhalvm4vih7im1p_4986_9541/pr_kegg_level_table.xls",
		
		"pt_keggenrich":"/mnt/ilustre/users/sanger-dev/workspace/20210528/PrKeggEnrich_hpgg_nbl6mv5nhalvm4vih7im1p_9359_9353/output",
		#
		"pt_keggenrich_cluster_matrix":"/mnt/ilustre/users/sanger-dev/workspace/20210528/PrKeggEnrichCluster_hpgg_nbl6mv5nhalvm4vih7im1p_7368_1775/EnrichCluster/output/cluster_matrix.xls",
		"pt_keggenrich_cluster_tree":"/mnt/ilustre/users/sanger-dev/workspace/20210528/PrKeggEnrichCluster_hpgg_nbl6mv5nhalvm4vih7im1p_7368_1775/EnrichCluster/output/seq.cluster_tree.txt",
		# #
		# ####
		# ####
		#### WGCNA
		# "sample_tree" : "/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210609/WgcnaPipeline_l5oe_jt0kprc0gkjqaij57g54h3_3042_6334/WgcnaPrepare/sample.cluster.dendrogram.txt",
		# "group_dict" : {"An10": ["An10_1", "An10_2", "An10_3"], "An5": ["An5_1", "An5_2", "An5_3"], "An_cPP": ["An_cPP_1", "An_cPP_2", "An_cPP_3"], "Con": ["Con_1", "Con_2", "Con_3"]},
		# "module_tree" : "/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210609/WgcnaPipeline_l5oe_jt0kprc0gkjqaij57g54h3_3042_6334/WgcnaModule/unmerged.module_corr.dendrogram.txt",
		# ##
		# "module_corr" : "/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210609/WgcnaPipeline_l5oe_jt0kprc0gkjqaij57g54h3_3042_6334/WgcnaModule/module_corr.matrix.xls",
		# "module_corr_tree" : "/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210609/WgcnaPipeline_l5oe_jt0kprc0gkjqaij57g54h3_3042_6334/WgcnaModule/module_corr.tree.txt",
		# ##
		# "module_stat" : "/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210609/WgcnaPipeline_l5oe_jt0kprc0gkjqaij57g54h3_3042_6334/WgcnaModule/module_size.stat.xls",
		# ##
		# "relation_corr" : "/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210609/WgcnaPipeline_l5oe_jt0kprc0gkjqaij57g54h3_3042_6334/WgcnaRelate/module_trait.correlation.xls",
		# "relation_corr_pvalue" : "/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210609/WgcnaPipeline_l5oe_jt0kprc0gkjqaij57g54h3_3042_6334/WgcnaRelate/module_trait.correlation_pvalues.xls",
		# ##
		# "gene_trait_corr" : "/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210609/WgcnaPipeline_l5oe_jt0kprc0gkjqaij57g54h3_3042_6334/WgcnaRelate/protein_trait.correlation.xls",
		# "seq_annot" : "/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210609/WgcnaPipeline_l5oe_jt0kprc0gkjqaij57g54h3_3042_6334/WgcnaModule/protein_module_detail.xls",
		# # write by xuxi ,above code is copy and modify from refrna
		# "wgcna_prepare_curve":"/mnt/ilustre/users/sanger-dev/wpm2/workspace/20210609/WgcnaPipeline_l5oe_jt0kprc0gkjqaij57g54h3_3042_6334/WgcnaPrepare/scale_free_analysis.xls"
