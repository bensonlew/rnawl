...
def prepare_hypergeom_data(class_gene_dict, gene_class_dict, deg_dict, total_gene_number, gene2ec_dict, gene2k_dict, path_annot_dict, metab=""):
    ...
    for each_class in considered_classes:
        ...
        if metab != "": # modify by ysh 20190329 for compund
            com_pathway = pd.read_table(metab, sep="\t", header=0)
            each_pathway_comp = com_pathway[com_pathway["pathway"]==each_class]
            pop_hitnumber = len(each_pathway_comp)
            c_in_path = set(each_pathway_comp["entry"])  # add by ghd @20191018
        else:
        ...
        if each_class2 in path_annot_dict:
            path_name, typeII, typeI = path_annot_dict[each_class2]
        else:
            print("%s not exist "%(each_class))
            continue

        associated_diff_info = '|'.join([x.split("|")[0] for x in associated_diff_info])
        study_hitnumber = len(associated_diff_genes)
        if metab != "":  # add by ghd @20191018
            yield study_hitnumber, pop_number, pop_hitnumber, c_in_path, each_class, associated_diff_info, link, path_name, typeII, typeI
        else:
...
def hypergeom_test(data, sort_fdr=True, correct_method=3, comp_metab=None, metab_comp=None, deg_list=None, compound_in_pathway=None):
    # add param compound_in_pathway by ghd @20191018
    p_value_list = []
    ratio_in_study_list = []
    ratio_in_pop_list = []
    classes = []
    ...
    for study_hitnumber, pop_number, pop_hitnumber, study_number, each_class, associated_diff_info, link, path_name, typeII, typeI in data:
        ...
        if comp_metab: # modify by ysh 20190329 for compund
            # 此条件下yield返回的study_number实际上不是数字，而是某一个通路下的代谢物集合
            associated_diff_info_list = associated_diff_info.split("|")
            all_compound = set()
            [all_compound.update(metab_comp[i]) for i in associated_diff_info_list]
            study_hitnumber = len(all_compound & study_number)  # modify by ghd @ 20191018
            metabset_compound = set()
            [metabset_compound.update(metab_comp[n]) for n in deg_list.keys()]
            study_number = len(compound_in_pathway & set(metabset_compound))  # modify by ghd @ 20191018

        # p_value = 1 - hypergeom.cdf(study_hitnumber-1,pop_number, pop_hitnumber, study_number)
        # modify by fwy @ 20191121 改为sf提高精度
...
if __name__ == '__main__':
    ...
    parser.add_argument('-metab', default= '', help='compound_pathway') # add 20190328 for metab
    ...
    for gene, ks in gene_k.items():
        for k in ks:
            for e in k_enzyme[k]:
                g2e_dict[gene].add(e)
    if args.metab != "":  # add by ghd @ 20191018
        compound_in_pathway = set(pd.read_table(args.metab)["entry"])

    deg_files = glob.glob(deg_file)
    # print(deg_files)
    for deg_file in deg_files:
        deg_list = read_diff_genes(deg_file)
        data = prepare_hypergeom_data(p_gene, gene_p, deg_list, gene_number, g2e_dict, gene_k, path_annot_dict, metab=args.metab)
        # modify by ysh 20190329
        if args.metab != "":
            result = hypergeom_test(data, sort_fdr=FDR, correct_method=correct_method, comp_metab=k_gene, metab_comp=gene_k, deg_list=deg_list, compound_in_pathway=compound_in_pathway)  # add compound_in_pathway by ghd @20191018
        else:
        ...
        if args.metab != "":
            f.write('#Study_num\tTerm\tDatabase\tID\tRatio_in_study\tRatio_in_pop\tP-Value\tCorrected P-Value\tMetab_ids\tHyperlink\ttypeII\ttypeI\n')
        else:
