...
class Terms(dict):
    ...
    def __init__(self, obo_fp="C:\\Users\\sheng.he.MAJORBIO\\Desktop\\goatools-master\\go-plus.obo", parse_altid=False):
        """初始化所有的terms"""
        self.obo_fp = obo_fp
        self.parse_altid = parse_altid
        self.parse_obo_file()
    ...
    def parse_obo_file(self):
        ...
        with open(self.obo_fp, 'r') as obo:
            ...
            for one in terms:
                ...
                if self.parse_altid:
                    if hasattr(t, 'alt_id'):
                        for a_alt_id in t.alt_id:
                            print "alt_id is {}".format(a_alt_id)
                            if isinstance(a_alt_id, str):
                                alt_id_n = a_alt_id
                            else:
                                alt_id_n = a_alt_id[1]
                            alt_t = copy.deepcopy(t)
                            alt_t.id = alt_id_n
                            self[alt_id_n] = alt_t
...
def draw_GO(GOs, out='GO_lineage', obo="/mnt/ilustre/users/sanger-dev/app/database/GO/go-plus.obo", parse_altid = False):
    """"""
    ...
    if isinstance(GOs, dict):
        ...
        GOs = GOs.keys()
    terms = Terms(obo_fp=obo, parse_altid=parse_altid)
    ...
    with open(out + ".tsv", 'w') as fo:
        for relation in go_relation:
            for edge in go_relation[relation]:
                node_dict[edge[0].id] = [edge[0].id, edge[0].name, edge[0].namespace]
                node_dict[edge[1].id] = [edge[1].id, edge[1].name, edge[1].namespace]
                edge_dict[edge[0].id + "|" + edge[1].id] = relation
                fo.write("\t".join([edge[0].id, edge[0].name, edge[0].namespace, edge[1].id, edge[1].name, edge[1].namespace, relation]) + "\n")
                G.add_edge(edge[1].id + r'\n' + r"\n".join(textwrap.wrap(edge[1].name, width=30, break_long_words=False)),
                           edge[0].id + r'\n' + r"\n".join(textwrap.wrap(edge[0].name, width=30, break_long_words=False)),
                           label=relation, color=relationship_dict[relation][0],
                           style=relationship_dict[relation][1],
                           minlen=1.5, arrowsize=1.3, penwidth=1.5)

    # print terms[GOs[0]].name
    # print r"\n".join(textwrap.wrap(terms[GOs[0]].name, width=5, break_long_words=False))
    G.add_nodes_from([terms[i].id + r'\n' + r"\n".join(textwrap.wrap(terms[i].name, width=30, break_long_words=False)) for i in GOs])
    ...
    for i in terms_colors:
        node = G.get_node(terms[i[0]].id + r'\n' + r"\n".join(textwrap.wrap(terms[i[0]].name, width=30, break_long_words=False)))
