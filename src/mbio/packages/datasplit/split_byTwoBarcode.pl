			...
			if($Rbarcode_mismatch > 0){
				...
				foreach my $linkPrimer(sort keys %{$sample{$Fbarcode}}){
					...
					if($primer_mismatch <= $linkPrimer_th){
						#my $temp_seq = substr($seq,length($Fbarcode) + length($linkPrimer),length($seq) - length($Fbarcode) - length($linkPrimer) - length($Rbarcode{$Fbarcode}));
						...
						#my $temp_quality = substr($quality,length($Fbarcode) + length($linkPrimer),length($quality) - length($Fbarcode) - length($linkPrimer) - length($Rbarcode{$Fbarcode}));
...
#				$Rbarcode_mismatch++ unless($seq_base =~ /$Rbarcode_base/i);
			...
			if($Rbarcode_mismatch > 0){
				...
				foreach my $linkPrimer(sort keys %{$sample{$Fbarcode}}){
					...
					if($primer_mismatch <= $linkPrimer_th){
						#my $temp_seq = substr($reverse_seq,length($Fbarcode) + length($linkPrimer),length($reverse_seq) - length($Fbarcode) - length($linkPrimer) - length($Rbarcode{$Fbarcode}));
						...
						#my $temp_quality = substr($reverse_quality,length($Fbarcode) + length($linkPrimer),length($reverse_quality) - length($Fbarcode) - length($linkPrimer) - length($Rbarcode{$Fbarcode}));
