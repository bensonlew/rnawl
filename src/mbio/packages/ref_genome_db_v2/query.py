...
class AllAnnoStat(object):
    ...
    def get_anno_stat(self, gene2trans, tran_outpath, gene_outpath, new_gtf_path, ref_gtf_path, length_path, gene_file,
                      cog_list=None, kegg_table=None, gos_list=None, blast_nr_table=None, blast_swissprot_table=None,
    ...
    def add_pathway_class(self, query_file):
        map2class = self.get_kegg_map2class()
        df = pd.read_table(query_file)
        pathways_class1 = list()
        pathways_class2 = list()
        for cell in df['paths']:
            if pd.isna(cell):
                class1 = str()
                class2 = str()
            else:
                paths = [pathway.split('(')[0] for pathway in cell.split('; ')]
                ...
                class1 = ';'.join(pathway_class1)
                class2 = ';'.join(pathway_class2)
            pathways_class1.append(class1)
            pathways_class2.append(class2)
        df['pathways_class1'] = pathways_class1
        df['pathways_class2'] = pathways_class2
...
if __name__ == '__main__':
    ...
    inst.add_pathway_class(sys.argv[2])
