...
class KeggHtml(object):
    ...
    def get_geneset_title(self, ko_title):
        ...
        for title in titles:
            K = ''
            if 'K' in title:
                K = title.strip().split(' ')[0].strip()
            genes_ = list()
            if K in self.k2g:
                genes_ = [x+'(gene)' for x in self.k2g[K]]
                # if genes_:
                #     print(genes_)
            if "accession: " in title:
                ...
                title_new = title.split("accession: ")[0] + "protein_gene: " + "|".join(genes_new)
                if genes_:
                    title_new += '|' + "|".join(genes_)
            elif ":" in title:
                # rna产品展示格式
                if title.split(":")[0].startswith("K"):
                    ...
                    head = ":".join(title.split(":")[0:2])
                # titles_ele = title.split(":")[1]
                titles_ele = titles_ele.rstrip(";")
                if 'K' in title:
                    genes = self.k2g.get(K, [])
                else:
                    genes = titles_ele.split(",")
                genes_new = []
                for gene in genes:
                    ...
                    if self.gene_dict.has_key(gene):
                        ...
                        genes_new.append(gene)
                if len(genes_new) > 0:
                    title_new = head + ":" + "protein_gene: " + "|".join(genes_new)
                else:
                    title_new = head
                # if 'K' in title:
                #     title_new += '|' + "|".join(genes_)
                ...
                title_new = title
                if genes_:
                    title_new = title + ' protein_gene:' + "|".join(genes_)
    ...
    def get_gene_set_html_mark(self, html_mark):
        ...
        with open(html_mark, 'r') as mark_in, open(self.out_file, 'w') as mark_out:
            for line in mark_in.readlines():
                ...
                if not kos:
                    kos = list()
                    for i in cols[5].split(','):
                        if '(' in i:
                            kos.append(i.split('(')[0].strip())
                    # print(kos)
                if cols[1] == 'rect':
    ...
    def run_gene_set_mark(self, map_file, out_file, gene_dict, html_mark, geneset_ko, k2g):
