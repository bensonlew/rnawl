...
class Ipath(object):
    def __init__(self, kegg_table, protein_list, prefix=''):
        ...
        self.db_dir = "/mnt/ilustre/centos7users/yitong.feng/script/ipath/IPATH3"
    ...
    def map_ko_to_svg(self, K, id_file):
        ...
        if self.KO:
            ...
            with open(id_file, 'r') as f:
                for line in f.readlines():
                    label, all_K = line.strip().split('\t')
                    all_ele = [x for x in all_K.split(';') if ':' in x and ' : ' not in x]
                    for ele in all_ele:
                        if K.has_key(ele):
                            if svg_label.has_key(label):
                                if K[ele]["color"] == "#0000ff":
    ...
    def set_svg_legend(self, element):
        ...
        if len(self.legend) == 2:
            ...
            if len(self.legend[0]) > 10:
                text2_x = text_x + int(text_font_size * 0.6 * len('up')) + 40
            ...
            if len(self.legend[0]) > 10:
                text3_x = text2_x + 150 + int(text_font_size * 0.6 * len('down')) + 40
    ...
    def map_file(self):
        ...
        for path_map in self.maps:
            ...
            if not self.KO:
                id_file = os.path.join(self.db_dir, path_map + ".new.map")
            ...
            for one_of_b in layer_b:
                ...
                for one_of_d in layer_d + layer_d2:
                    ...
                    if svg_lable.has_key(m_id) and one_of_d.attrib.has_key("stroke"):
                        ...
                        style_dict = {k.strip(): v.strip() for k, v in
                                      (item.strip().split(':') for item in style.strip(';').split(';'))}
    ...
    def convert(self):
        for file in os.listdir('.'):
            if file.endswith('.svg') and file.startswith(self.out):
                ...
                cmd = 'convert -density 500 %s %s'%(file, pdf)
                os.system(cmd)
