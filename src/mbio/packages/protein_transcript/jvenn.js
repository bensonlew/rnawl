...
*  The current version is: jvenn 1.9 (2016-11-25 15:05)
***************************************************************/
(function($) {
...
//            }],
        ...
        var maxNanmeLength = Math.max.apply(0, options.series.map(function (d) {
            return d.name.length;
        }));

        // default name length is 7
        maxNanmeLength = (maxNanmeLength - 7) > 0 ? (maxNanmeLength - 7) : 0;

		var opts = $.extend(defaults, options),
            __canvasWidth = 500 + maxNanmeLength * 7,
			// __canvasHeight = 415,
            __canvasHeight = 430,
			__context = null;
        ...
        function placeStat2(vennSize){
            ...
            var	data2 = countByNbClass(),
                ...
                xmargin = 60,
                // ymargin = 120 + 470;
                ymargin = opts.displayStat1 ? 590 : 470;
			var	maxwidth = 390 + xspacer,
				sum = 0;
			for (var i=0; i<vennSize; i++) {
				sum += data2[i];
		...
		function placeClassicVenn(vennSize) {
			...
			if (vennSize == 6) {
                ...
                $("#"+cpref+"-label2").css("left", 200).css("top",   28).css("color", opts.colors[1]);
				...
				$("#"+cpref+"-label1").css("left", 55).css("top", 15).css("color",    opts.colors[0]);
				$("#"+cpref+"-label2").css("left", 380).css("top", 10).css("color",   opts.colors[1]);
				$("#"+cpref+"-label3").css("left", 220).css("top", 396).css("color", opts.colors[2]);
...
//            		placeStat(vennSize);
//                }
                if (opts.displayStat1) {
            		placeStat1(vennSize);
                }
                if (opts.displayStat2) {
            		placeStat2(vennSize);
                }
		...
		function addSVGText(text, x, y, color, opacity, ffamily, fsize, fweight, isLabel) {
            ...
            return '<text fill="'+color+'" fill-opacity="'+opacity+'" stroke="none" font-family="'+ffamily+'" font-size="'+fsize+'"pt font-style="normal" font-weight="'+fweight+'" text-decoration="none" x="'+x+'"pt y="'+y+'"pt dominant-baseline="alphabetic">'+text+'</text>';
		}
		...
		function addExportModule(div, extraheight, type){
            ...
            var div_export = '<div id="'+cpref+'-module-export" style="position: relative; left:475px; top: -' + (__canvasHeight - 5) +'px; width: 25px; height: 20px;">';
            ...
            div_export += '<div id="'+cpref+'-format-print" style="padding:0px 10px;background: none repeat scroll 0% 0% transparent; color: rgb(48, 48, 48); font-size: 12px;">打印图表</div><hr />';
        	div_export += '<div id="'+cpref+'-format-png" style="padding: 0px 10px; background: none repeat scroll 0% 0% transparent; color: rgb(48, 48, 48); font-size: 12px;">下载PNG文件</div>';
        	div_export += '<div id="'+cpref+'-format-svg" style="padding: 0px 10px; background: none repeat scroll 0% 0% transparent; color: rgb(48, 48, 48); font-size: 12px;">下载SVG文件</div>';
        	if (type == "list") {
        		div_export += '<div id="'+cpref+'-format-csv" style="padding: 0px 10px; background: none repeat scroll 0% 0% transparent; color: rgb(48, 48, 48); font-size: 12px;">下载CSV文件</div>';
        	}
        	div_export += '</div>';
			div_export += '</div>';
			div_export += '</div>';
            ...
            */
            ...
            // exporting with printer
            $("#"+cpref+"-format-print").click(function(event){
                ...
                document.body.innerHTML = '<style>@page {margin: 15px;}</style>' + $expDiv.html();
	            window.print();
			...
			$("#"+cpref+"-format-png").click(function(event) {
				...
				$("[id^="+cpref+"-resultC]").each(function(){
					if($(this).css("display") != "none") {
						...
						console.log((parseInt($(this).css('top'))+20)+'px');
						$expDiv.children("svg").children("g").append(addSVGText(value,
							$(this).css("left"),  (parseInt($(this).css('top'))+20)+'px',
							$(this).css("color"), $(this).css("opacity"),
							$(this).css("font-family"), $(this).css("font-size"),
							$(this).css("font-weight"), false));
					}
				...
				img.onload = function() {
					...
					$('#'+cpref+'-data-export').attr("download", "Venn_chart.png");
			...
			$("#"+cpref+"-format-svg").click(function(event) {
				...
				$('#'+cpref+'-data-export').attr("download", "Venn_chart.svg");
				$('#'+cpref+'-data-export')[0].click();
...
//            }
            ...
            if(opts.displayStat1 || opts.displayStat2){
                extraheight = 280;
//                __canvasHeight += extraheight;
                if(opts.displayStat1){
                    __canvasHeight += 160;
            ...
            $('<style>.module-legend{border:1px solid lightgrey;border-radius:5px;position:relative;left:'+legleft+'px;top:-'+(legtop + __canvasHeight - 415)+'px;width:35px;padding-bottom:3px}</style>').appendTo('body');
...
//	                    }
        ...
        // re-adjust svg container size and export module left dimmension
        var svgWidth = $t.find("svg").attr("width"),
            svgHeight = $t.find("svg").attr("height");
        $t.css({width: +svgWidth + 5, height: +svgHeight + 5});
        $t.find("#" + cpref + "-module-export").css("left", +svgWidth - 25);
...
// 改数字位置 placeNumber函数   div.css("left", l).css("top", top + 5);
// 改文字位置 placeClassicVenn函数，根据vennSize 改对应的$("#"+cpref+"-label1") top left