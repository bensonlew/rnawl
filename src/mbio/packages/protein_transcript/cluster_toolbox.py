...
def hcluster(cluster_pd, transpose=False, n_clusters=10, method='average',
    ...
    :param cluster_pd: pandas DataFrame, cluster matrix
    :param transpose: if to transpose the cluster matrix
    ...
    """
    if transpose:
        cluster_pd = cluster_pd.transpose()
    if n_clusters > cluster_pd.shape[0]:
        print("n_clusters is bigger than sample number!!")
        n_clusters = cluster_pd.shape[0]
    try:
        z = hclust.linkage(cluster_pd, method=method, metric=metric)
        ...
        check = cluster_pd[cluster_pd.sum(axis=1) == 0]
        if check.shape[0] >= 1:
            ...
            cluster_pd = cluster_pd[cluster_pd.sum(axis=1) > 0]
        try:
            z = hclust.linkage(cluster_pd, method=method, metric=metric)
        except:
            print("enhen? fastcluster failed again, we will try scipy.cluster.hierarchy")
            z = sch.linkage(cluster_pd, method=method, metric=metric)
    except:
        print("fastcluster failed, we will try scipy.cluster.hierarchy")
        z = sch.linkage(cluster_pd, method=method, metric=metric)
    labels = cluster_pd.index
    tree = to_tree(z, labels)
    # tree = R_to_tree(cluster_pd, metric, method)
...
def r_hcluster(cluster_pd, transpose=False, n_clusters=10, method='average',
    ...
    """
        ...
        :param cluster_pd: pandas DataFrame, expression matrix
    ...
    def r_hclust_func(cluster_pd, method='complete', metric='euclidean', n_clusters=None):
        ...
        if metric in ("pearson", "kendall", "spearman"):
            r_dist = r['as.dist']
            corr_result = 1 - cluster_pd.transpose().corr(method=metric)
            corr_ds = pandas2ri.py2ri(corr_result)
            ds = r_dist(corr_ds)
        else:
            cluster_df = pandas2ri.py2ri(cluster_pd)
            ds = r_dist(cluster_df, method=metric)
    ...
    if transpose:
        raw_cluster = cluster_pd.transpose()
    else:
        raw_cluster = cluster_pd
    if do_scale:
        cluster_pd = cluster_pd.apply(preprocessing.scale, axis=0)
    if transpose:
        cluster_pd = cluster_pd.transpose()
    if n_clusters > cluster_pd.shape[0]:
        print("n_clusters is bigger than sample number!!")
        n_clusters = cluster_pd.shape[0]
    try:
        z = r_hclust_func(cluster_pd, method=method, metric=metric, n_clusters=n_clusters)
        ...
        if check.shape[0] >= 1:
            print('Actually, we detected that some genes have zero expression across all samples.')
            print('such as {} has zero clusterression across all sample'.format(check.index[0]))
            cluster_pd = cluster_pd[cluster_pd.sum(axis=1) > 0]
        z = r_hclust_func(cluster_pd, method=method, metric=metric, n_clusters=n_clusters)
...
def kmeans(cluster_pd, n_clusters=10, transpose=False, output=None, prefix=''):
    """
    ...
    if n_clusters > cluster_pd.shape[0]:
        print("n_clusters is bigger than sample number!!")
        n_clusters = cluster_pd.shape[0]
    cluster = KMeans(n_clusters=n_clusters, ).fit_predict(cluster_pd)
...
def corr(cluster_pd, method='pearson', output=None):
    """
    Correlation calculation with pandas.
    :param cluster_pd: DataFrame of expression matrix
    ...
    result = cluster_pd.corr(method=method)
    result.index.name = 'sample'
    result.to_csv(out_dir, sep='\t', header=True, index=True)
    return result


def pca(all_cluster_pd, output=None):
    """
    PCA analysis using sk-learn package of python
    :param all_cluster_pd: DataFrame of expression matrix
    :return: DataFrame with samples as rows and component values as columns.
    """
    pca = decomposition.PCA()
    data = all_cluster_pd.transpose()
    pca.fit(data)
    _ratio = list(enumerate(pca.clusterlained_variance_ratio_, start=1))
...
def cluster_corr_cluster(cluster_pd, method='pearson', n_clusters=10, s_method="complete",
                     s_metric="correlation", output=None):
    # for sample cluster
    s_cluster = hcluster(cluster_pd, transpose=True, n_clusters=n_clusters, method=s_method,
                         metric=s_metric, output=output, prefix="sample.")
    # for sample corr
    correlation = corr(cluster_pd, method=method, output=output)
    return correlation, s_cluster


def process_cluster_matrix(cluster_matrix, log_base=None, group_dict=None, group_method="mean"):
    if type(cluster_matrix) == str or type(cluster_matrix) == bytes:
        all_cluster_pd = pd.read_table(cluster_matrix, header=0, dtype={0:str})
        all_cluster_pd = all_cluster_pd.set_index(all_cluster_pd.columns[0])
    else:
        print('Input is Not a str ? then exp_matrix is assumed to be a pandas DataFrame Object')
        all_cluster_pd = cluster_matrix
    all_cluster_pd.index.name = args.type_
    #
    if group_dict is not None:
        group_cluster = list()
        for g in group_dict:
            if group_method == 'median':
                g_cluster = all_cluster_pd.loc[:, group_dict[g]].median(axis=1)
            else:
                g_cluster = all_cluster_pd.loc[:, group_dict[g]].mean(axis=1)
            g_cluster.name = g
            group_cluster.append(g_cluster)
        all_cluster_pd = pd.concat(group_cluster, axis=1)
    #
    if log_base:
        if log_base == math.e:
            all_cluster_pd = np.log(all_cluster_pd + 1)
        elif log_base == 2:
            all_cluster_pd = np.log2(all_cluster_pd + 1)
        elif log_base == 10:
            all_cluster_pd = np.log10(all_cluster_pd + 1)
        else:
            raise Exception('log base of {} is not supported'.format(log_base))
    # return
    return all_cluster_pd
...
def parse_group(group_file):
    ...
    return group_dict
def cluster_cluster(cluster_pd, n_clusters=10, s_cluster=True, g_cluster=True,
    ...
    if s_cluster:
        ...
        if s_cluster_type == 'hierarchy':
            s_cluster_result = hcluster(cluster_pd, transpose=True, n_clusters=n_clusters, method=s_method,
                                        metric=s_metric, output=output, prefix="sample.")
        else:
            s_cluster_result = kmeans(cluster_pd, n_clusters=n_clusters, transpose=True, output=output, prefix='sample.')
        result.append(s_cluster_result)

    if g_cluster:
        # for gene
        if g_cluster_type == 'hierarchy':
            g_cluster_result = hcluster(cluster_pd, transpose=False, n_clusters=n_clusters, method=g_method,
                                        metric=g_metric, output=output, prefix="seq.")
        else:
            g_cluster_result = kmeans(cluster_pd, n_clusters=n_clusters, transpose=False, output=output, prefix='seq.')
        result.append(g_cluster_result)
    # write out cluster matrix
    if output is not None:
        if not os.path.exists(output):
            os.mkdir(output)
    else:
        output = os.getcwd()
    out_dir = os.path.join(output, 'cluster_matrix.xls')
    cluster_pd.to_csv(out_dir, sep='\t', header=True, index=True)
    return result
...
if __name__ == '__main__':
    ...
    parser.add_argument('-cluster_file', type=str, metavar="cluster_matrix_file", required=True,
                        help="cluster matrix file")
    parser.add_argument('-log_base', type=int, metavar="log_base", default=10, help="log base for transformation cluster ")
    ...
    parser.add_argument('-type_', default='go', help="go, kegg")
    ...
    if args.group:
        ...
        group_dict = None
    #
    cluster_pd = process_cluster_matrix(args.cluster_file)
    #
    ...
    if args.ngc:
        g_cluster = False
    cluster_cluster(cluster_pd, n_clusters=args.n_clusters,
                ...
                output=args.out)
    if args.corr:
        corr(cluster_pd, args.corr_method, args.out)
    if args.pca:
        pca(cluster_pd, output=args.out)
