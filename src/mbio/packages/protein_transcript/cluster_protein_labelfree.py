...
class ClusterProtein(Controller):
    def __init__(self, exp, group_file, diff_path, clt, mr, ml, out):
        ...
        exp = pd.read_table(self.exp, sep='\t')
        exp = exp.fillna(0)
        exp = exp.replace('-', 0)
        exp = exp.replace('_', 0)
        exp.to_csv(self.exp+'.filted', sep='\t', index=False)
        self.exp = self.exp+'.filted'
    ...
    def run_sample_cluster(self):
        for cluster in self.sample_cluster:
            ...
            cmd = '/mnt/ilustre/users/ting.kuang/LABELFREE/bin/plot_heatmap_trendline.pl -i %s/sample.diffexp.txt -clt %s -n %s -o %s.Heatmap -mr %s -ml %s -scale T '%(self.out, self.clt, n, n, self.mr, self.ml)
            params = dict(
                cmd=cmd,
                node=1,
                memory=2
    ...
    def run_group_cluster(self):
        for cluster in self.group_cluster:
            n = cluster.name.split('_')[-1]
            cmd = '/mnt/ilustre/users/ting.kuang/LABELFREE/bin/plot_heatmap_trendline.pl -i %s/group.diffexp.txt -clt %s -n %s -o %s.Heatmap -mr %s -ml %s -scale F '%(self.out, self.clt, n, n, self.mr, self.ml)
            params = dict(
                cmd=cmd,
                node=1,
                memory=2
...
if __name__ == '__main__':
    ...
    diff = ClusterProtein(args.exp, args.group_file, args.diff_path, args.clt, args.mr, args.ml, args.out)
    diff.run()
