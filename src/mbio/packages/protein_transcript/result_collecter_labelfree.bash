...
cp ${diff_result}/{*.diff.exp.xls,*volcano.pdf,*.up.list,*.down.list,*.DE.list,*.10.xls,*.01.xls} 3.DiffExpAnalysis/3.1.Statistics/Volcano
cp ${diff_result}/*scatter.pdf 3.DiffExpAnalysis/3.1.Statistics/Scatter

cp -r ${venn_result}/{*list,*pdf,*xls} 3.DiffExpAnalysis/3.1.Statistics/Venn/
...
cp ${diff_result_}/*.DE.list 3.DiffExpAnalysis/3.5.Network
...
for i in `ls *.10.xls |awk -F '.10.xls' '{print $1}'`
do
    ${bin}/get_diff_table_detail.py -i $i.10.xls -d '${go_result}/GO.list;${kegg_result}/pathway.txt;${cog_result}/${cog}.list;${rawdata}/description.txt' -l 'GO;KEGG;${cog};Description' -o $i.10.detail.xls
done

for i in `ls *.01.xls |awk -F '.01.xls' '{print $1}'`
do
    ${bin}/get_diff_table_detail.py -i $i.01.xls -d '${go_result}/GO.list;${kegg_result}/pathway.txt;${cog_result}/${cog}.list;${rawdata}/description.txt' -l 'GO;KEGG;${cog};Description' -o $i.01.detail.xls
done
