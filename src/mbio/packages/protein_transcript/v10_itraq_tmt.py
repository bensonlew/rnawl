...
class V10ItraqTmt(Controller):
    def __init__(self, config):
        super(V10ItraqTmt, self).__init__()
        ...
        if not os.path.exists(self.config):
            cmd = 'python %s/creat_ITRAQ_v10_config.py %s %s' % (self.bin_v, self.config, os.getcwd())
            os.system(cmd)
            super(V10ItraqTmt, self).end(normal=1, out='您传入的配置文件不存在，可以用以下命令生成:%s\n而我已经贴心的为你生成了一个\n'%cmd)
        self.get_global()
        self.check_dbs()
    ...
    def get_global(self):
        ...
        if cfg.get("Global", "Confirm") == 'F':
            super(V10ItraqTmt, self).end(normal=1, out='请确认配置文件后,将Globle的Confirm参数值改为T,再提交运行！')
        # 各类参数的读入
        ...
        if not os.path.exists(self.output):
            try:
                ...
                super(V10ItraqTmt, self).end(normal=1, out='输出文件夹路径不对')
        ...
        if not os.path.exists(self.raw_data) or not os.path.isdir(self.raw_data):
            super(V10ItraqTmt, self).end(normal=1, out='请传入正确的原始数据路径，%s不存在或者不是目录'%self.raw_data)
    ...
    def check_dbs(self):
        ...
        if self.single == 'T' and not os.path.isfile('%s/%s.aa.KEGG.fasta' % (self.KEGG_dir, self.orgC)):
            super(V10ItraqTmt, self).end(normal=1, out=
    ...
    def end(self):
        super(V10ItraqTmt, self).end(out='your program finished')
        ...
        if self.single == 'F':
            ...
            try:
                ...
                if not os.path.exists(kegg_db):
                    super(V10ItraqTmt, self).end(normal=1, out='kegg的blast用的db文件不存在')
    ...
    def run_go_result(self):
        ...
        try:
            ...
            if not os.path.exists(nrgo):
                super(V10ItraqTmt, self).end(normal=1, out='GO报错，nr数据库mapping没有正常完成')
    ...
    def run_go_result_nr(self):
        ...
        try:
            ...
            if not os.path.exists(vsnr):
                super(V10ItraqTmt, self).end(normal=1, out='GO报错，diomand没有正常完成')
    ...
    def run_kegg_result(self):
        ...
        try:
            ...
            if not os.path.exists(vskegg):
                super(V10ItraqTmt, self).end(normal=1, out='kegg报错，diomand没有正常完成')
        if self.single == 'F':
    ...
    def run_cog_result(self):
        ...
        try:
            ...
            if not os.path.exists(vscog):
                super(V10ItraqTmt, self).end(normal=1, out='COG报错，diomand没有正常完成')
        if self.cog == 'COG':
...
"""
    ...
    def run_diff(self):
        cmd = 'python /mnt/ilustre/users/yitong.feng/scripts/diff/diff_itraq.py'
    ...
    def run_cluster(self):
        ...
        if not tmp_list:
            super(V10ItraqTmt, self).end(normal=1, out='diff运行出错，请检查')
        cmd = 'python /mnt/ilustre/users/yitong.feng/scripts/cluster/cluster_protein.py'
        cmd += ' -exp ' + os.path.join(self.raw_data, 'exp.txt')
        cmd += ' -group_file ' + os.path.join(self.raw_data, 'group.txt')
        cmd += ' -diff_path ' + os.path.join(self.diff.work_dir, 'diff_result')
        cmd += ' -clt ' + self.clt
        cmd += ' -mr ' + self.mb
        cmd += ' -ml ' + self.ml
...
'''
        cmd = Template(cmd)
        cmd = cmd.render(bin=self.bin,
                         diff=os.path.join(self.diff.work_dir, 'diff_result')
                         )
        params = dict(
            cmd=cmd,
            node=3,
...
'''
    ...
    def run_ppi(self):
        ...
        cmd += ' -blast_file ' + os.path.join(self.annot_diomand.work_dir, 'diomand_results',
                                              'exp.fasta_vs_string.xml')
        cmd += ' -diff_path ' + os.path.join(self.diff.work_dir, 'diff_result')
    ...
    def run_ipath(self):
        ...
        if not pathways:
            super(V10ItraqTmt, self).end(normal=1, out='ipath报错，diff_annot_enrich没有正常完成')
...
'''
    ...
    def run_results_collect(self):
        ...
        if self.single != 'T':
            MJ = 'MJ_PM_ITRAQ_autoreporter.py'
        else:
            MJ = 'MJ_PM_ITRAQ_nosingle_autoreporter.py'
        cmd = Template(filename='/mnt/ilustre/users/yitong.feng/scripts/itraq_v10/result_collecter.bash')
