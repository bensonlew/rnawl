...
if __name__ == '__main__':
    ...
    # 获取gene_id go对应关系
    go2gene = {}
    gene_go_file = open(out_dir + '/go_detail_des.tsv', 'w')
    gene_go_file.write("Gene ID\tGO ID\tGO Term\n")
    for linerecord in f:
        if len(linerecord) != 0:
            ...
            for go_term in gos:
                if go_term in GO_db:
                    names.append(GO_db[go_term].name)
                else:
                    names.append("unknown")
                if go_term in go2gene:
                    go2gene[go_term].add(gene_id)
                else:
                    go2gene[go_term] = set([gene_id])
            gene_go_file.write("{}\t{}\t{}\n".format(gene_id, ";".join(gos), ";".join(names)))
    go2gene_list = []
    for go_term, gene_set in go2gene.items():
        if go_term in GO_db:
            go_class = GO_db[go_term].namespace
            name = GO_db[go_term].name
            num = len(gene_set)
            pct = float(num)/float(len(allgene))
            go2gene_list.append([go_class,
                                 go_term,
                                 name,
                                 num,
                                 pct,
                                 ";".join(list(gene_set))])
    go2gene_list_s1 = sorted(go2gene_list, key=lambda x:x[3], reverse=True)
    go2gene_list_s2 = sorted(go2gene_list_s1, key=lambda x:x[0])

    go2gene_file = open(out_dir + '/go_detail_stat.tsv', 'w')
    go2gene_file.write("GO (Lev1)\tGO ID\tGO Term\tSeq Number\tPercent\tSeq List\n")
    for go_ele in go2gene_list_s2:
        go2gene_file.write("\t".join(map(str, go_ele)) + "\n")
