...
class DiffExpToolbox(PvalueCorrect):
    ...
    def __init__(self, count_matrix, group_info, cmp_info, batch_info=None, exp_matrix=None, exp_type='fpkm',
                 ...
                 normalized='tmm', is_duplicate=True, prob=0.9, no_filter=True, filter_method=None, analysis='split', project='ref_rna'):
        """
        initial inputs
        ...
        :param cmp_info: path of cmp info, file with only two columns. Header line starts with '#'.
               ...
               s5       s6
               -----------------
        :param batch_info: path of batch info, file with only two columns. Header line start with '#'.
               -----------------
               #sample  batch
               sample1  1
               sample2  1
        ...
        self.analysis = analysis  # split： 分开进行差异分析， total： 合在一起进行差异分析
        self.project = project    # 不同分析项目，结果文件处理不同
    ...
    def filter(self, count, count_cutoff=4, passed_number_cutoff=None, output=None):
        if output is None:
            output = os.getcwd()
        out_count = os.path.join(output, os.path.basename(count) + '_filtered')
        with open(count) as f, open(out_count, 'w') as f2:
            line = f.readline()
            f2.write(line)
            header = line.strip('\n').split('\t')
    ...
    def summary_stat(self, detail_dfs,out_stat):
        ...
        summary_ids_set = set()
        sig_id_dict = OrderedDict()   #  {'a1_vs_a2':set()}
        cmp_deg_sum = dict()          #  {'a1_vs_a2': len()}
        all_stat_dicts = dict()       #  {'a1_vs_a2': df}
        if self.project == 'denovo_rna':
            for cmp, detail_df in detail_dfs:
                  ctrl, case = cmp.split('_vs_')
                  sig_id_set = set(detail_df[detail_df['significant'] == 'yes'].index)
                  sig_id_set_all = set(detail_df.index)
                  sig_id_dict[cmp] = sig_id_set
                  summary_ids_set.update(sig_id_set_all)  #all significant=='yes'
                  cmp_deg_sum[cmp] = len(sig_id_set)
                  all_stat_dicts[cmp] = detail_df
        if self.project in ['medical_transcriptome', 'ref_rna', 'small_rna', 'lnc_rna', 'prok_rna']:
            for cmp, detail_df in detail_dfs:
                  ctrl, case = cmp.split('_vs_')
                  sig_id_set = set(detail_df[detail_df['significant'] == 'yes'].index)
                  sig_id_dict[cmp] = sig_id_set
                  summary_ids_set.update(sig_id_set)  #all significant=='yes'
                  cmp_deg_sum[cmp] = len(sig_id_set)
                  all_stat_dicts[cmp] = detail_df
        summary_data = list()
    ...
    def filter_tpm_filter_threshold(self, ctrl, test, output):
        """
        对表达量tpm值进行过滤，并按照过滤后的seq_id输出counts_table
        过滤方法为两个差异分组的所有样本的平均数大于self.tpm_filter_threshold
        """
        ...
        if self.filter_method is None or self.filter_method == 'None':
        ...
        elif self.filter_method == "average" or self.filter_method == "mean":
            ...
            exp_cmp_use = exp_cmp_total.loc[exp_cmp_total.median(axis=1) > self.tpm_filter_threshold]
    ...
    def DEGseq(self, sep='\t', method='MARS', threshold_kind=0, output=None):
        ...
        for ctrl, test in self.cmp_list:
            self.filter_tpm_filter_threshold(ctrl, test, output)
            counts_table = os.path.join(output, '{}_vs_{}.counts_filted'.format(ctrl, test))
            if self.no_filter is True:
                counts_table = counts_table
            else:
                counts_table = self.filter(counts_table)
    ...
    def edgeR(self, dispersion=0.1, output=None, sep='\t'):
        ...
        for ctrl,test in self.cmp_list:
            ...
            if self.batch is not None:
                cmp_batch = ctrl_batch + test_batch
                if len(set(cmp_batch)) == 1:
                    batch_err = True
                else:
                    # batch_err = False
                    pass
            if self.batch is not None:
                ...
                if all_batch:
                    ...
                    batch_err = True
            if self.batch is not None:
                tmp_group = ctrl_names + test_names
                tmp_batch = ctrl_batch + test_batch
                a = zip(tmp_group, tmp_batch)
                sample_batch_dict = dict()
                for i in a:
                    if i[0] not in sample_batch_dict:
                        sample_batch_dict[i[0]] = [i[1]]
                    else:
                        if i[1] not in sample_batch_dict[i[0]]:
                            sample_batch_dict[i[0]].append(i[1])
                        else:
                            pass
                n = 0
                for group, batch in sample_batch_dict.items():
                    n += len(batch)
                if n == len(sample_batch_dict):
                    batch_err = True
            ...
            if self.analysis == 'total':
                _group = 'all_group'
                _count = 'all_counts'
                _batch = 'all_batch'
            if self.analysis == 'split':
                _group = 'tmp_group'
                _count = 'tmp_counts'
                _batch = 'tmp_batch'
            f.write('y <- DGEList(counts={}, group={})\n'.format(_count, _group))
            ...
            if (',' not in ctrl_ind) and (',' not in test_ind):
                ...
                f.write('result <- exactTest(y, {}, dispersion={})\n'.format(_group, dispersion))
            else:
                if self.batch is not None and batch_err == False:
                    f.write('design <- model.matrix(~0+{}+{})\n'.format(_group, _batch))
                else:
                    f.write('design <- model.matrix(~0+{})\n'.format(_group))
                f.write('y <- estimateDisp(y, design, robust=F)\n')
                f.write('fit <- glmQLFit(y, design, robust=F)\n')
                f.write('con = makeContrasts({}{}-{}{}, levels=design)\n'.format(_group, test, _group, ctrl))
    ...
    def DESeq2(self, output=None, sep='\t', padjust_way=None):
        ...
        for ctrl,test in self.cmp_list:
            ...
            if self.analysis == 'split':
                ...
                _batch = 'tmp_batch'
            if self.batch is not None and batch_err is False:
                f.write('colData <- data.frame(row.names=colnames({}), group={}, batch={})\n'.format(_count, _group, _batch))
                f.write('dds <- DESeqDataSetFromMatrix(countData={}, colData=colData, '
                        'design= ~group+batch)\n'.format(_count))
            else:
                f.write('colData <- data.frame(row.names=colnames({}), group={})\n'.format(_count, _group))
                f.write('dds <- DESeqDataSetFromMatrix(countData={}, colData=colData, '
                        'design= ~group)\n'.format(_count))
            ...
            f.write('write.table(d,'
                    ' file="{}", sep="\t", quote=F, row.names=T, col.names=NA)'.format(raw_tmp_normalize))
            f.close()
        else:
            self.run(script_list)
    ...
    def detail_extract_DEGseq(self, tmp_path, threshold_kind, cmpname):
        ...
        if test in self.group_dict:
            ...
            test_samples = [test]
        detail_df['regulate'] = detail_df['log2fc'].fillna(0).apply(
            lambda x: 'up' if x > 0 else 'down' if x else 'no change')  # x > 0, up; x = 0, no change; x < 0, down
        detail_df['significant'] = ['yes' if i and j else 'no' for i, j in
                                    zip((abs(detail_df['log2fc']) >= log_fc_cutoff),
                                        detail_df[self.sig_type].fillna(1) <= self.stat_cutoff)]   # i is True and j is True, yes; else no
        detail_df["fc"] = detail_df["log2fc"].apply(lambda x: math.pow(2, x))
        select_columns = ["fc", "log2fc", "pvalue", "padjust", "significant", "regulate"]
        detail_df = detail_df[select_columns]
        cmp_samples = ctrl_samples + test_samples
    ...
    def detail_extract_NOIseq(self,tmp_path):
        ...
        cmp_stat_df.index.name = "seq_id"
        cmp_stat_df = cmp_stat_df.fillna(
            {'{}_mean'.format(ctrl): 0, '{}_mean'.format(test): 0, 'fc': 1, 'log2fc': 0, "theta": 0, "prob": 0,"D": 0, "significant": "no test", "regulate": "no test"})
    ...
    def limma(self, dispersion=0.1, output=None, sep='\t'):
        ...
        for ctrl,test in self.cmp_list:
            ...
            if (',' not in ctrl_ind) and (',' not in test_ind):
                ...
                f.write('result <- topTable(fit3, n = nrow({}))\n'.format(_count))
    ...
    def svaseq_limma(self, dispersion=0.1, output=None, sep='\t'):
        ...
        #for ctrl, test in self.cmp_list:
            ...
            f.write('all_counts <- counts[, c({})]\n'.format(all_sample_ind))
            f.write('all_counts <- floor(all_counts+0.5)\n')
            f.write('all_counts_filter <- all_counts[which(rowSums(all_counts)>0),]\n')
            f.write('all_counts_filter <- as.matrix(all_counts_filter)\n')
            f.write('tmp_group <- c({})\n'.format(','.join(ctrl_names + test_names)))
            f.write('all_group <- c({})\n'.format(','.join(all_sample_names)))
            if self.analysis == 'total':
                _group = 'all_group'
                _count = 'all_counts'
                _count_filter = 'all_counts_filter'
            if self.analysis == 'split':
                _group = 'tmp_group'
                _count = 'tmp_counts'
                _count_filter = 'tmp_counts_filter'
            f.write('mod <- model.matrix(~0+{})\n'.format(_group))
            f.write('mod0 <- cbind(rep(1,length(colnames({}))))\n'.format(_count))
            f.write('svobj <- svaseq({}, mod, mod0)\n'.format(_count_filter))
            f.write('sv <- svobj$sv\n')
            ...
            if (',' not in ctrl_ind) and (',' not in test_ind):
                ...
                f.write('con = makeContrasts({}{}-{}{}, levels=modSv)\n'.format(_group,
                    test, _group, ctrl))
                f.write('fit2 <- contrasts.fit(fit, con)\n')
                f.write('fit3 <- eBayes(fit2)\n')
            f.write('result <- topTable(fit3, n = nrow({}))\n'.format(_count))
    ...
    def NOIseq(self, dispersion=0.1, output=None, sep='\t'):
        ...
        for ctrl,test in self.cmp_list:
            ...
            if self.normalized == 'rpkm':
                f.write('normalized_count <- rpkm({}, long = mylength, lc = 1, k = 0.5)\n'.format(_count))
            if self.normalized == 'tmm':
                f.write('normalized_count <- tmm({}, long = 1000, lc = 0, k = 0.5)\n'.format(_count))
            if self.normalized == 'uqua':
                f.write('normalized_count <- uqua({}, long = 1000, lc = 0, k = 0.5)\n'.format(_count))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--no_filter', default=True, type=bool,
    ...
    parser.add_argument('--analysis', type=str, default='split')
    parser.add_argument('--filter_method', type=str, default=None)
    parser.add_argument('--project', type=str, default='ref_rna')
