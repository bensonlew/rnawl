...
class DiffExpToolbox(PvalueCorrect):
    ...
    def edgeR(self, dispersion=0.1, output=None, sep='\t'):
        ...
        for ctrl, test in self.cmp_list:
            ...
            f.write('write.table(t(colnames), file="{}", sep="\\t", quote=F, row.names=F, col.names=F)\n'.format(tmp_normFactor))
            f.write('write.table(y$samples, file="{}", sep = "\t", quote = F, row.names = T, col.names = F, append = T)\n'.format(tmp_normFactor))
            ## 输出normalized结果文件
            ...
            if (',' not in ctrl_ind) or (',' not in test_ind):
                ...
                f.write('tmp_group <- unique(tmp_group)\n')
    ...
    def DESeq2(self, output=None, sep='\t', padjust_way=None):
        ...
        for ctrl, test in self.cmp_list:
            ...
            f.write('write.table(t(colnames), file="{}", sep="\\t", quote=F, row.names=F, col.names=F)\n'.format(tmp_sizeFactor))
            f.write('write.table(sizefactor, file="{}", sep="\\t", quote=F, row.names=T, col.names=F, append=T)\n'.format(tmp_sizeFactor))
            ## 输出normalized结果文件
            ...
            f.write('write.table(t(colnames), file="{}", sep="\\t", quote=F, col.names=F, row.names=F)\n'.format(tmp_normalize))
            f.write('write.table(normalized_counts, file="{}", sep="\\t", quote=F, row.names=T, col.names=F, append=T)\n'.format(tmp_normalize))
