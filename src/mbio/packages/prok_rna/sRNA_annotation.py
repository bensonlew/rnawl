...
class srna_annot(object):
    def __init__(self, predict_fa, evalue):
    ...
    def run_blast(self):
        blast_cmd = \
"""
${blastn} -task blastn-short -strand plus  -num_threads 5 -db ${db_path}/Rfam  -query ${predict_fa} -outfmt 5 -evalue ${evalue} -num_alignments 1 -out ${predix}_vs_rfam.xml
...
"""
    ...
    def modify_rfam_description(self):
        ...
        rfam = self.predix + '_vs_rfam.xls'
        rfam_r = open(rfam, 'r')
        header = rfam_r.readline()
        rfam_info = rfam_r.readlines()
        rfam_r.close()
        with open(rfam, 'w') as rfam_w:
            rfam_w.write(header)
            for line in rfam_info:
    ...
    def merge_stat_rfam(self):
        ...
        with open(self.db_path + 'Rfam.seed', 'r') as rfam_seed:
            for line in rfam_seed.readlines():
                if line.startswith('#'):
        ...
        with open(self.predix + '_vs_rfam.xls', 'r') as rfam_r, \
            ...
            _ = rfam_r.readline()
            TP_list = list()
            merge_w.write('query_name\tTP\tDE\tAC\tHit\tHspLength\tEvalue\tIdentity\n')
            for line in rfam_r.readlines():
                line = line.strip().split('\t')
                if len(line) >= 10:
                    query_name = line[5]
                    hit = line[9]
    ...
    def stat_annot(self):
        ...
        with open(self.predix + '_vs_rfam.xls', 'r') as rfam_r:
            ...
            for line in rfam_r.readlines():
                line = line.strip().split('\t')
                if len(line) >= 9:
                    rfam_s2hit[line[5]].add(line[9].split(';')[0])
                    rfam_list.append(line[5])
        ...
        all_srna = set(rfam_list + SIPHI_list + sRNAMap_list + sRNATarBase_list)
    ...
    def run(self):
        self.run_blast()
        self.xml2table(self.predix + '_vs_rfam.xml', self.predix + '_vs_rfam.xls')
        ...
        self.get_list_file(self.predix + '_vs_rfam.xls')
        self.get_list_file(self.predix + '_vs_SIPHI.xls')
        self.get_list_file(self.predix + '_vs_sRNAMap.xls')
        self.get_list_file(self.predix + '_vs_sRNATarBase.xls')
...
if __name__ == '__main__':
    ...
    evalue = sys.argv[2]
    ANNO = srna_annot(predict_fa, evalue)
    ANNO.run()
