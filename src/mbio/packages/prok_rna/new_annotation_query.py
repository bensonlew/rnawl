...
class AllAnnoStat(object):
    ...
    def get_anno_stat(self, gene2trans, tran_outpath, gene_outpath, new_gtf_path, ref_gtf_path, length_path, gene_file, cog_list=None, cog_class=None, kegg_table=None, pathway_table=None, gos_list=None, gos_detail=None, blast_nr_table=None, blast_swissprot_table=None, pfam_domain=None, des=None, novel=None):
        """
        ...
        self.get_des_novel(novel=novel)
        ...
        if gos_detail != None and gos_detail != "None":
            self.get_go_lev2(gos_detail=gos_detail)
        ...
        with open(tran_outpath, 'wb') as w:
            head = 'gene_id\tgene_name\tdescription\tlocation\tstart\tend\tstrand\tcog\tcog_description\tcog_class1\tKO_id\tKO_name\tKO_des\tpaths\tpaths_class1\tpaths_class2\tpfam\tgo\tgo_lev2\tnr\tswissprot\ttype\n'
            w.write(head)
            for name in self.gene_ids:
                try:
                ...
                if gene2trans:
                    w.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(
                        ...
                        self.stat_info[name].chrom,
                        self.stat_info[name].start,
                        self.stat_info[name].end,
                        self.stat_info[name].strand,
                        self.stat_info[name].cog,
                        self.stat_info[name].cog_ids,
                        self.stat_info[name].cog_class1,
                        self.stat_info[name].ko_id,
                        self.stat_info[name].ko_name,
                        self.stat_info[name].ko_des,
                        self.stat_info[name].pathway,
                        self.stat_info[name].pathway_class1,
                        self.stat_info[name].pathway_class2,
                        '; '.join(self.stat_info[name].pfam),
                        self.stat_info[name].go,
                        self.stat_info[name].go_lev2,
                        self.stat_info[name].nr,
                        self.stat_info[name].swissprot,
                        self.stat_info[name].type
                    ))
        ...
        for class1 in classI:
            ...
            for class2 in class1['children']:
                ...
                for path in paths:
                    ...
                    map2class[path_name] = (class1_name, class2_name, str(path['name']).split("  ")[1])
        return map2class

    def get_ko_name(self, kegg_name):
        '''
        获取ko_name 去掉以大写字母开头的字符串 去掉以大写字母开头的字符串
        '''
        names = kegg_name.split(", ")
        name = ""
        for i in names:
            if i[0].isupper():
                pass
                        ...
                        if map_num not in self.gloabl:
                            ...
                            for index, i in enumerate(pids):
                                if i == pid:
                                    ...
                                    pathway_ids.append(map_id)
                                    pathway.append(item)
                    except:
                        pass
                        # print "{}在该物种中没有pathway".format(query_name)
                    '''
                    if map_id in map2class:
                        pathway_ids.append(map_id)
                        item = map_id + "(" + map2class[map_id][2] + ")"
                        pathway.append(item)
                pathway = "; ".join(pathway)
                if query_name in self.stat_info:
                    self.stat_info[query_name].ko_id = ko_id
                    self.stat_info[query_name].ko_name = self.get_ko_name(ko_name)
                    self.stat_info[query_name].pathway = pathway
                    self.stat_info[query_name].ko_des = ko_des

                    pathway_class1 = []
                    pathway_class2 = []
                    for path in pathway_ids:
                        if path in map2class:
                            pathway_class1.append(map2class[path][0])
                            pathway_class2.append(map2class[path][1])
                        else:
                            pathway_class1.append("-")
                            pathway_class2.append("-")
                    self.stat_info[query_name].pathway_class1 = ";".join(pathway_class1)
                    self.stat_info[query_name].pathway_class2 = ";".join(pathway_class2)
    ...
    def get_go_lev2(self, gos_detail):
        """找到转录本ID对应的goID及term、term_type"""
        with open(gos_detail, 'rb') as r:
            for line in r.readlines()[1:]:
                ...
                gos_level2 = ";".join(gos)
                if query_name in self.stat_info:
                    self.stat_info[query_name].go_lev2 = gos_level2
                ...
                for i,j in enumerate(cog_class_abr):
                    if query_name in self.stat_info:
                        if self.stat_info[query_name].cog:
                            ...
                            self.stat_info[query_name].cog_class1 += ";" +  cog_categories[i]
                        else:
                            self.stat_info[query_name].cog = cog + "({}:{})".format(cog_class_abr[i], cog_class[i].strip())
                            self.stat_info[query_name].cog_ids = cog + "({})".format(cog_des)
                            self.stat_info[query_name].cog_class1 = cog_categories[i]
    ...
    def get_des(self, des):
        ...
        with open(des, "rb") as f:
            for line in f.readlines():
                line = line.strip().split('\t')
                if self.gene2trans.has_key(line[0]):
                    self.gene2trans[line[6]].append(line[6])
                else:
                    self.gene2trans[line[6]] = [line[6]]

                self.stat_info[line[6]] = Transcript()

                self.stat_info[line[6]].length = line[3]
                self.stat_info[line[6]].name = line[5]
                self.stat_info[line[6]].gene_id = line[6]
                self.stat_info[line[6]].is_gene = True
                self.stat_info[line[6]].tran_id = line[6]
                self.stat_info[line[6]].des = line[9]
                self.stat_info[line[6]].gene_name = line[5]
                self.stat_info[line[6]].symbol = line[5]
                self.stat_info[line[6]].chrom = line[0]
                if line[2] == "+":
                    self.stat_info[line[6]].start = line[1].split("..")[0]
                    self.stat_info[line[6]].end = line[1].split("..")[1]
                else:
                    self.stat_info[line[6]].start = line[1].split("..")[1]
                    self.stat_info[line[6]].end = line[1].split("..")[0]
                self.stat_info[line[6]].strand = line[2]
                self.gene_ids.append(line[6])

    def get_des_novel(self, novel):
        """
        获取新预测蛋白描述信息
        """
        with open(novel, "rb") as f:
            f.readline()
            for line in f.readlines():
                line = line.strip().split('\t')
                if self.gene2trans.has_key(line[3]):
                    self.gene2trans[line[3]].append(line[3])
                else:
                    self.gene2trans[line[3]] = [line[3]]

                self.stat_info[line[3]] = Transcript()

                self.stat_info[line[3]].length = line[7]
                self.stat_info[line[3]].name = ""
                self.stat_info[line[3]].gene_id = line[3]
                self.stat_info[line[3]].is_gene = True
                self.stat_info[line[3]].tran_id = line[3]
                self.stat_info[line[3]].des = line[6]
                self.stat_info[line[3]].gene_name = ""
                self.stat_info[line[3]].symbol = ""
                self.stat_info[line[3]].type = "novel_gene"
                self.stat_info[line[3]].chrom = line[0]
                self.stat_info[line[3]].start = line[1]
                self.stat_info[line[3]].end = line[2]
                self.stat_info[line[3]].strand = line[5]
                self.gene_ids.append(line[3])
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 17:
        inst = AllAnnoStat(kegg_version=sys.argv[17])
    else:
        inst = AllAnnoStat()
    ...
    inst.get_anno_stat(gene2trans=sys.argv[1],
                                ...
                                gos_detail=sys.argv[11],
                                blast_nr_table=sys.argv[12],
                                blast_swissprot_table=sys.argv[13],
                                pfam_domain=sys.argv[14],
                                des=sys.argv[15],
                                novel=sys.argv[16],
                                )
