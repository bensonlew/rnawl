...
class string2cog(object):
    ...
    def __init__(self):
        self.sqlitedb_path = Config().SOFTWARE_DIR + \
            '/database/COG/cog_temporary_new_index_id.db'
    ...
    # 修改（sqlite3数据库换成mongo；2016.11.15；zengjing）
    def string2cog_by_mongo(self, string_xml, out_dir):
        ...
        with open(cog_list_path, 'wb') as listfile, open(cog_table_path, 'wb') as tablefile, open(cog_summary_path, 'wb') as summaryfile:
            ...
            for identation in identations.findall('Iteration'):
                ...
                query_length = identation.find(
                    'Iteration_query-len').text  # the query length
                ...
                if len(hits) > 0:  # check if such name has hits
                    ...
                    for hit in hits:
                        ...
                        if OG.count() >= 1:
                            ...
                            for og in OG:
                                # COG/NOG group
                                ortho_group = og['orthologous_group']
                                if len(og['function_categories']) > 1:
                                    func_cat = ','.split(
                                        og['function_categories'])
                                else:
                            ...
                            Hsp_start_of_query = hsp.find(
                                'Hsp_query-from').text
                            Hsp_end_of_query = hsp.find('Hsp_query-to').text
                            calcstrand = int(Hsp_end_of_query) - \
                                int(Hsp_start_of_query)
                            if calcstrand >= 0:
                                Hsp_strand_of_query = '+'
                            else:
                                Hsp_strand_of_query = '-'
                            ...
                            for desresult in DES:
                                group_description = desresult[
                                    'cog_description']
                            coverage = (abs(float(Hsp_end_of_hit) - float(Hsp_start_of_hit)) + 1) / (
                                abs(float(region_end) - float(region_start)) + 1)
                            ...
                            tablefile.write(queryname + '\t' + query_length + '\t' + Hsp_start_of_query + '\t' + Hsp_end_of_query + '\t' + Hsp_strand_of_query + '\t' + hit_id + '\t' + hit_desc + '\t' + hit_length + '\t' + Hsp_start_of_hit +
                                            '\t' + Hsp_end_of_hit + '\t' + ortho_group + '\t' + group_description + '\t' + ','.join(func_cat) + '\t' + region_start + '\t' + region_end + '\t' + coverage + '\t' + identity + '\t' + positives + '\n')
            summaryfile.write('#Total seqs with COG/NOG:' +
                              str(totalseq) + '\n')
...
"""
    def string2cog_by_sqlite(self, string_xml, out_dir):
        cog_list_path = out_dir + '/cog_list.xls'
        cog_table_path = out_dir + '/cog_table.xls'
        cog_summary_path = out_dir + '/cog_summary.xls'
        with open(cog_list_path, 'wb') as listfile, open(cog_table_path, 'wb') as tablefile, open(cog_summary_path, 'wb') as summaryfile:
            ...
            for identation in identations.findall('Iteration'):
                ...
                if len(hits) > 0:  # check if such name has hits
                    ...
                    for hit in hits:
                        hit_id = hit.find('Hit_id').text
                        hit_length = hit.find('Hit_len').text
                        OG = c.execute("SELECT * FROM COG_mapping WHERE _id = ?", (hit_id,)).fetchall()
                        if len(OG) >= 1:
                            if duplicate is False:
                                totalseq += 1
                                duplicate = True
                            ...
                            Hsp_end_of_hit = hsp.find('Hsp_hit-to').text
                            DES = c.execute(
                                "SELECT * FROM COG_description WHERE cog_id = ?", (ortho_group,)).fetchall()
                            for desresult in DES:
