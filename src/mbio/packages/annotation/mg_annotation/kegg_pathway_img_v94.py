...
from mbio.packages.metagenomic.kegg_html_v94 import KeggHtml
import pandas as pd
import lxml.html
...
class KeggPathwayImg(object):
    def __init__(self):
        ...
        self.png_coll = self.mongodb['kegg_v94.2_pathway_png'] ## fix by qingchen.zhang @20201020
        self.fs = gridfs.GridFS(self.mongodb)

    def run_img(self, pathway_file, ko_name, KO_list_name, outdir, png_file, html_db, enzyme_file,bac=None):
        if not os.path.exists(pathway_file):
            raise Exception("{}文件不存在！".format(pathway_file))
        if enzyme_file and not os.path.exists(enzyme_file):
            raise Exception("{}文件不存在! ".format(enzyme_file))
        ...
        with open(pathway_file, "r+") as f1:
            ...
            ko_index = head.index(ko_name)
            # print ko_index
            KO_list_index = head.index(KO_list_name)
            for line in f1:
                line = line.strip().split("\t")
                ko = line[ko_index]
                KOs = line[KO_list_index]
                path_KO = KOs.split(";")
                l = []
                # print ko, "\t:", KOs
                ...
                if result:
                    ...
                    png_base_name = ko
                    #kgml_path =KGML_parser.read(open('/mnt/ilustre/users/sanger-dev/app/database/KEGG/pathway_map/'+pid+'.kgml'))
                    #png_path='/mnt/ilustre/users/sanger-dev/app/database/KEGG/pathway_map/'+pid+'.png'
                    ...
                    for each in path_KO:
                        ...
                        for n in l:
                            for graphic in p_kgml.entries[n].graphics:
                                ...
                                # graphic.type= "roundrectangle"
                                # graphic.bgcolor = colors.Color(alpha=0.4)
                    ...
                    if png_file == 1 or png_file == "1":  # 微生物基因组
                        ...
                        map_html = KeggHtml()
                        if bac:
                            map_html.set_shape_list(shape_list=["rect", "circle", "poly"])  #细菌增加画线 20191112
                        map_html.color_bg[0] = "#FFFF00"
                        self.get_html(db_file, html_path, png_base_name)
                        # title_dict = dict(zip(path_KO, path_KO))
                        # map_html.run(db_file, html_path, png_base_name, title_dict)
                        ko_list = [set(path_KO + enzymes)]
                        # map_html.run_html_mark(html_path, html_path + ".mark", png_base_name, title_dict, ko_list)
    ...
    def get_pic(self, ko, ko_file, png, html_db):
        ...
        map_path = os.path.realpath(__file__).split("annotation")[0] + "metagenomic/pathway_pic.r"
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', metavar='[xml_file]', help='Input xml_file')
    # parser.add_argument('-g', metavar='[geneset file]', required=True, help='Input gene set table')
    ...
    parser.add_argument('-ko', metavar='[ko name]', required=True, help='Input ko head name in pathway_file')
    parser.add_argument('-KO', metavar='[KO_list name]', required=True, help='Input KO_list head name in pathway_file')
    ...
    parser.add_argument('-bac', metavar='[bool]', required=False, default=None, help='bac project need')
    args = parser.parse_args()
    pathway_file = args.p
    outdir = args.o
    ko_name = args.ko
    KO_list_name = args.KO
    png_file = args.png_file
    run_img = KeggPathwayImg()
    html_db = args.html
    enzyme_file = args.enzyme_file
    run_img.run_img(pathway_file, ko_name, KO_list_name, outdir, png_file, html_db, enzyme_file,args.bac)
