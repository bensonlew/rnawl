...
class meta_tcdb_anno(object):
    ...
    def tcdb_by_mongo(self, align_table, anno_table):
        with open(align_table, 'r') as infile, open(anno_table, 'wb') as outfile:
            outfile.write(
                'Gene ID\tTCDB ID\tTCDB Description\tTCDB Family\tTCDB Subclass\tTCDB Class\tIdentity(%)\tEvalue\tScore\tAlign_len\tCoverage\n')
            ...
            for line in infile:
                ...
                if not "Score" in line[0]:
                    ...
                    evalue = line[1]
                    score = line[0]
                    align_len = str(abs(int(line[7])-int(line[8])))            #line[11]
                    act_iden = line[3]
                    coverage = str(int(align_len)/float(line[6]))
                    try:
                    ...
                    if detail:
                        ...
                        outfile.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(query, id, des,
                                                                            ...
                                                                            evalue, score, align_len,coverage))
                    else:
                        print "wrong ID"
