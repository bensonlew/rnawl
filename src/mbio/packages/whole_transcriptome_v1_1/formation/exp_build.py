...
def main(args):
    ...
    main_dict = database['exp'].find_one({'task_id':json_dict['task_id'], 'level': json_dict['level'], 'is_rmbe': json_dict['is_rmbe'], 'status': 'end'})
    # main_dict = database['exp'].find_one({'task_id': json_dict['task_id'], 'level': json_dict['level']})
    if json_dict['is_rmbe'] == 'false':
        main_id = main_dict['main_id']
    if json_dict['is_rmbe'] == 'true':
        main_id = main_dict['batch_main_id']
    if 'library' in json_dict:
        ...
        filter_dict = {'exp_id': main_id, 'category': {'$in': rnas}}
    else:
        filter_dict = {'exp_id': main_id, 'category': json_dict['category']}
    ...
    if json_dict['is_rmbe'] == 'true':
        df = df.reindex(columns_batch, axis=1)
        df.rename(columns=dict(zip(columns_batch, columns)), inplace=True)
