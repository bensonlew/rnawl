...
class cox(object):
    def __init__(self, surv_table, exp_table, factor_list, gene_list, output):
    ...
    def cox(self, output=None):
        ...
        f.write('meta$group=ifelse( as.numeric(expr[g,]) > median(as.numeric(expr[g,])) ,"high" ,"low" )\n')
        ...
        f.write('coxe=rownames(cox[cox$p<0.5,])\n')
        ...
        f.write('ggsave(file="{}",print(p),device="pdf")\n'.format(self.output + '/cox.pdf'))
        #
        #
        #
        #
        ...
        # f.write('res <- t(as.data.frame(univ_results, check.names = FALSE))\n')
        # f.write('res <- as.data.frame(res)\n')
        # f.write('write.table(t(c("feature", "beta", "HR (95% CI for HR)", "wald.test", "p.value")), file="{}", sep="\t", quote=F, col.names=F,row.names=F)\n'.format(self.output+'/cox.txt'))
        # f.write('write.table(file="{}", res, sep="\t", quote=F, row.names=T, col.names=F, append=T)\n'.format(self.output+'/cox.txt'))
        #
        # f.write('formula <- as.formula(Surv(time, status) ~ {})\n'.format('+'.join(self.factor_list+self.genes)))
        # f.write('model <- coxph(formula, data = surv_cox )\n')
        # f.write('p <- ggforest(model,data=surv_cox,main = "Hazard ratio", cpositions = c(0.01, 0.22, 0.4),  fontsize = 1, refLabel = "reference", noDigits = 3)\n')
        # f.write('ggsave(file="{}",print(p),device="png")\n'.format(self.output + '/cox.png'))
        #
        #
        #
...
if __name__ == "__main__":
    ...
    parser.add_argument('-g', type=str, help='gene list')
