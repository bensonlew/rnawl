...
if __name__ == '__main__':
    ...
    do12file = open(out_file, 'w')
    do12file.write('DO1 (Lev1)\tDO Term (Lev2)\tDO ID (Lev2)\tSeq Number\tPercent\tSeq List\n')

    for do_lev1 in DO_db["DOID:4"].children:
        do_lev1_term = DO_db[do_lev1].name
        for do_lev2 in DO_db[do_lev1].children:
            if len(DO_db[do_lev2].genes) != 0:
                # pcent = float(len(DO_db[do_lev2].genes)) / float(len(allgene))
                pcent = "{}/{}".format(len(DO_db[do_lev2].genes), len(allgene))
                do12file.write( DO_db[do_lev1].name + '\t'
                                + DO_db[do_lev2].name + '\t' + do_lev2 + '\t'
                                + str(len(DO_db[do_lev2].genes)) + '\t' + str(pcent) + '\t' + ";".join(DO_db[do_lev2].genes) + '\n')
