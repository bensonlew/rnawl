...
class AllAnnoStat(object):
    def __init__(self, kegg_version="202007"):
        ...
        '''
        if kegg_version == "202003":
            print kegg_version
            self.kegg_json = Config().SOFTWARE_DIR + "/database/Annotation/other{}/kegg{}/br08901.json".format(kegg_version[0:4], kegg_version)
            self.kegg_des =  Config().SOFTWARE_DIR + "/database/Annotation/other{}/kegg{}/ko.des.txt".format(kegg_version[0:4], kegg_version)
        else:
    ...
    def get_anno_stat(self, gene2trans, tran_outpath, gene_outpath, new_gtf_path, ref_gtf_path, length_path, gene_file,
        ...
        with open(tran_outpath, 'wb') as w:
            head = 'gene_id\ttranscript_id\tis_gene\tgene_name\tlength\tdescription\tcog\tcog_description\tKO_id\tKO_name\tpaths\tpfam\tgo\tnr\tuniprot\tentrez\tbiotype\tkegg_genes\tspe_path\n'
            w.write(head)
        ...
        with open(gene2trans, 'r') as f:
            ...
            for line in lines:
                ...
                if line[2] == "yes":
                    ...
                    self.stat_info[line[1]].tran_id = self.gene2trans[line[1]]
                    if line[4] != "":
                        self.stat_info[line[1]].seq_type = "protein_predict"
                    else:
                        self.stat_info[line[1]].seq_type = "unknown"
        ...
        with open(blast_uniprot_table, 'rb') as f:
            ...
            for line in f:
                ...
                if flag == query_name:
                    ...
                    uniprot_hit_name = line[10].split("|")[1]
