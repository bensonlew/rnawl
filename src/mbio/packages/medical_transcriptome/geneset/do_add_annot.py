...
if __name__ == '__main__':
    ...
    def get_level1_name(do_id):
        for lev1,terms in level1_term.items():
            if do_id in terms:
                return DO_db[lev1].name
        else:
            return "unknown"
    ...
    def get_name(do_id):
        if do_id in DO_db:
            return DO_db[do_id].name
        else:
            return "unknown"

    enrich_df = pd.read_table(sys.argv[1], sep="\t")
    enrich_df['Term_type'] = enrich_df['DO_ID'].map(get_level1_name)
    enrich_df['DO name'] = enrich_df['DO_ID'].map(get_name)

    enrich_df.to_csv(sys.argv[2], sep="\t", index=False)
