...
es_method <- json.lst$es_method
output <- json.lst$output

geneset_list <- getGmt(gmx)
matrix <- read.table(matrix.path, row.names = 1, header = TRUE, sep='\t')
matrix <- as.matrix(matrix)
gsva_es <- gsva(matrix, geneset_list, method = method, min.sz = min_num, max.sz = max_num, mx.diff=es_method)
col = c('geneset' ,colnames(gsva_es$es.obs))
write.table(t(col), file = output, quote = FALSE, sep = "\t", row.names = FALSE, col.names = FALSE)
write.table(gsva_es$es.obs, file = output,append = TRUE, quote = FALSE, sep = "\t", col.names = FALSE, row.names = TRUE)
