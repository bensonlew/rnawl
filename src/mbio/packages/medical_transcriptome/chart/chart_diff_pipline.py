...
class ChartDiffPipline(Chart):
    def __init__(self):
        super(ChartDiffPipline, self).__init__()
    ...
    def chart_heat_tree(self, name, out, corr_source, sample_tree, gene_tree, sample2group_source, gene2group_source, json_mode,height=None):
        json_mode = self.mode_dir + "/" + json_mode
        with open(json_mode, 'r') as f, open(self.work_dir + name + out + ".heat_corr.js", 'w') as fo:
            a = json.loads(f.read())
            if height:
                a["chart"]["height"] = height
    ...
    def chart_geneset_gsva_diff(self, diff_file):
        ...
        self.chart_gsva_diff('geneset_gsva_diff.', name, up_dict, nosig_dict, down_dict, geneset_list, 'geneset.gsva.diff.json')
    ...
    def chart_geneset_class_kegg(self,geneset_name, kegg_class_table, geneset_list=None, top=20):
        ...
        for geneset in geneset_list:
            ...
            # i_choose = a_choose.sort_values(by=[geneset + "_num"], ascending=[False])[:top]
            ...
            self.chart_class_column("{}.kegg_annot".format(geneset),  ".gene_set", source, None, class_source,  None,  "geneset.annot_kegg_stat.bar.json")
        if len(geneset_list) >= 2:
            b = a
            col_names = a.columns[1:]
            b["sum"] = sum([b[x] for x in col_names if x.endswith("_num")])
            c = a.sort_values(by=["sum"], ascending=[False])[:top]
            categories = list(c["second_category"])
            for geneset in geneset_list:
                if geneset.split("_genes")[0].endswith("up"):
                    up_geneset = geneset.split("_genes")[0]
                if geneset.split("_genes")[0].endswith("down"):
                    down_geneset = geneset.split("_genes")[0]
            source = []
            source.append(
                {
                    "category": "up",
                    "value": list(c[up_geneset + "_genes_num"])
                }
            )
            source.append(
                {
                    "category": "down",
                    "value": list(c[down_geneset + "_genes_num"])
                }
            )
            categories = list(c["second_category"])
            self.chart_reactome_class_column("{}.kegg_annot".format(geneset_name), ".gene_set", source, categories,
                                             None, None, "../medical_transcriptome/geneset.annot_kegg_stat.stack.json")
    ...
    def chart_geneset_class_do(self, geneset,do_class_table, geneset_list=None, top=20):
        ...
        #该分析仅一个基因集
        self.chart_class_column("{}.do_annot".format(geneset), ".gene_set", source, categories, class_source, None, bac_json, reset_margin=False)

    def chart_diff_geneset_class_reactome(self,geneset_name, reactome_class_table, geneset_list=None, top=20):
        ...
        if len(geneset_list) == 1:
            # 单基因集
            geneset = geneset_list[0]
            d = d.sort_values(by=geneset + "_numbers", ascending=False)
            source = [
            ...
            class_source = None
            self.chart_class_column("{}.reactome_annot".format(geneset), ".gene_set", source, categories,
                                    class_source, None, "../medical_transcriptome/geneset.annot_reactome_stat2.bar.json")

        else:
            e = d.sort_values(by="sum", ascending=False)
            for geneset in geneset_list:
                if geneset.endswith("up"):
                    up_geneset = geneset
                if geneset.endswith("down"):
                    down_geneset = geneset
            source =[]
            source.append(
                {
                    "category" :"up",
                    "value" : list(e[up_geneset +"_numbers"])
                }
            )
            source.append(
                {
                    "category": "down",
                    "value": list(e[down_geneset + "_numbers"])
                }
            )
            categories = list(e["Category Function description"])
            self.chart_reactome_class_column("{}.reactome_annot".format(geneset_name), ".gene_set", source, categories,
                                    None, None, "../medical_transcriptome/geneset.annot_reactome_stat.stack.json")

            for geneset in geneset_list:
                e = b.sort_values(by=geneset+"_numbers", ascending=False)[:top]
                categories = list(e["Category Function description"])
                source = [
                    ["item"] + list(e["Category Function description"]),
                    ["series"] + list(e[geneset + "_numbers"]),
                    ["category"] + list(e["Category Function description"])
                ]
                class_source = None
                self.chart_class_column("{}.reactome_annot".format(geneset), ".gene_set", source, categories,
                                        class_source, None, "../medical_transcriptome/geneset.annot_reactome_stat2.bar.json")
        ...
        #
        # #常规这块只有一个基因集,因此只给一张图
        # self.chart_class_column("{}.reactome_annot".format(geneset_name), ".gene_set", source, categories, class_source, None, bac_json)

    # def chart_geneset_class_reactome(self,geneset,reactome_class_file):
    #     with open(reactome_class_file, "r") as f:
    #         f.readline()
    #         source = [["item"], ["series"], ["category"]]
    #         for line in f.readlines()[1:]:
    #             cols = line.strip().split("\t")
    #             source[0].append(cols[1])
    #             source[1].append(int(cols[-1]))
    #             source[2].append(cols[1])
    #     self.chart_column2("{}.reactome_annot", geneset, source, "geneset.class_reactome.column.json")

    def chart_reactome_class_column(self, name, out, source, categories, categories_source, title, json_mode):
        json_mode = self.mode_dir + "/" + json_mode
        with open(json_mode, 'r') as f, open(self.work_dir + name + out + ".column.js", 'w') as fo:
            ...
            fo.write("var options = ")
            fo.write(json.dumps(a, indent=4))
            self.js_list.append([self.work_dir + name + out + ".column.js", {"use_new":"yes","delay": 8000}])


    def chart_geneset_enrich_do(self, geneset,do_enrich_table, geneset_list=None, geneset_name="geneset", top=20, p_thre=0.5):
        ...
        if a.shape[0] >=1 :
            ...
            self.chart_bar_and_line("{}.do_enrich".format(geneset), ".gene_set", source_bar, source_line, categories_line, geneset_name,
            ...
            self.chart_bar("{}.do_enrich".format(geneset), ".gene_set", source_bar, geneset_name,
            ...
            self.chart_buble("{}.do_enrich".format(geneset), ".gene_set", source_buble, geneset_name,
            ...
            for atype, sub_class in a.groupby("Term_type"):
                ...
                source_buble_list.append(source)
            self.chart_buble2("{}.do_enrich".format(geneset), ".gene_set", source_buble_list, geneset_name,
                              "../medical_transcriptome/geneset.enrich_do.buble2.json")

    def chart_diff_geneset_enrich_reactome(self, geneset,reactome_enrich_table, geneset_list=None, geneset_name="geneset", top=20,
                                      p_thre=0.5):
        ...
        self.chart_bar_and_line("{}.reactome_enrich".format(geneset), ".gene_set", source_bar, source_line, categories_line, geneset_name,
                                "../medical_transcriptome/geneset.enrich_reactome.bar_line.json")

        # 不用管这段代码干嘛的,有问题去看有参原版,这版为了适应医学的zz需求弄的
        t = a[top - 1::-1]
        reactome_order = list(t.drop_duplicates("category")["category"])

        def Reverse(lst):
            return [ele for ele in reversed(lst)]

        reactome_order = Reverse(reactome_order)
        t['order'] = [reactome_order.index(x) for x in t['category']]
        b = t.sort_values(by=["order", "Padjust"], ascending=[True, False])
        source_bar = [
            ["item"] + list(b["Description"]),
            ["series"] + list(b["neg_log10p_corrected"]),
            ["category"] + list(b["category"])
        ]
        self.chart_bar("{}.reactome_enrich".format(geneset), ".gene_set", source_bar, geneset_name,
        ...
        self.chart_buble("{}.reactome_enrich".format(geneset), ".gene_set", source_buble, geneset_name,
        ...
        for atype, sub_class in a.groupby("category"):
            ...
            source_buble_list.append(source)
        self.chart_buble2("{}.reactome_enrich".format(geneset), ".gene_set", source_buble_list, geneset_name,
    ...
    # def chart_geneset_enrich_reactome(self, reactome_enrich_table, geneset_list=None, geneset_name="geneset", top=20,p_thre=0.5):
    #     a = pd.read_table(reactome_enrich_table, header=0)
    #     a = a.sort_values(by=["Pvalue"], ascending=[True])[:top]
    #     a = a[a["Padjust"] <= p_thre]
    #     # 判断最小 log10(p)
    #     if len([x for x in a["Padjust"] if x > 0]) > 0:
    #         pvalues_min = min([x for x in a["Padjust"] if x > 0])/10
    ...
    def chart_kegg_class(self,geneset_list,kegg_class_file_list,kegg_level_path):
        for geneset, kegg_class_file in zip(geneset_list, kegg_class_file_list):
            try:
                ...
                self.chart_geneset_class_kegg(geneset,norm_kegg_class_file)
            except:
                pass
    ...
    def chart_reactome_class(self,geneset_list,reactome_class_file_list):
        for geneset, reactome_class_file in zip(geneset_list, reactome_class_file_list):
            try:
                self.chart_diff_geneset_class_reactome(geneset,reactome_class_file)
            except:
                pass

    def chart_reactome_enrich(self,geneset_list,reactome_enrich_file_list):
        for geneset, reactome_class_file in zip(geneset_list, reactome_enrich_file_list):
            try:
                self.chart_diff_geneset_enrich_reactome(geneset,reactome_class_file,geneset_name =geneset )
            except:
                pass

    def chart_do_class(self,geneset_list,do_class_file_list):
        for geneset, do_class_file in zip(geneset_list, do_class_file_list):
            try:
                self.chart_geneset_class_do(geneset,do_class_file)
            except:
                pass

    def chart_do_enrich(self,geneset_list,do_enrich_file_list):
        for geneset, do_enrich_file in zip(geneset_list, do_enrich_file_list):
            try:
                self.chart_geneset_enrich_do(geneset,do_enrich_file,geneset_name = geneset )
            except:
                pass
