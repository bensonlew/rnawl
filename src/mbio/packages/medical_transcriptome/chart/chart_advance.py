...
class ChartAdvance(Chart):
    def __init__(self):
        super(ChartAdvance, self).__init__()
    ...
    def chart_tf_stat(self, column_stat, circ_gene, circ_trans):
        ...
        size_stat_pd_trans = size_stat_pd.sort_values('transcript_num', ascending=False)[0:10]
        source = [
            ["item"] + list(size_stat_pd_trans["family"]),
            ["series"] + list(size_stat_pd_trans["transcript_num"]),
            ["category"] + list(size_stat_pd_trans["family"])
        ]

        self.chart_column2("tf_stat", ".transcript", source, "tf.family_stat.column.json")
        size_stat_pd_gene = size_stat_pd.sort_values('gene_num', ascending=False)[0:10]
        source = [
            ["item"] + list(size_stat_pd_gene["family"]),
            ["series"] + list(size_stat_pd_gene["gene_num"]),
            ["category"] + list(size_stat_pd_gene["family"])
        ]

        self.chart_column2("tf_stat", ".gene", source, "tf.family_stat.column.json")

        fams = list(size_stat_pd_trans["family"])
        circ = pd.read_table(circ_trans, header=0)
        source_circ = list()
        for rec in circ.to_dict('records'):
            # try:
            #     log_e_value = -math.log(rec["e_value"], 10)
            # except:
            #     log_e_value = rec["e_value"]
            source_circ.append([rec["transcript_id"]] + [rec[term] for term in fams] + [rec["e_value"]])

        source_class = [[t, z] for t,z in zip(fams, list(size_stat_pd_trans["transcript_num"]))]
        title = "Statistics of TF family"

        self.chart_circ("tf_circ", ".transcript", source_circ, source_class, title, "tf.family_stat.circ.json")


        fams = list(size_stat_pd_gene["family"])
        ...
        source_class = [[t, z] for t,z in zip(fams, list(size_stat_pd_gene["gene_num"]))]
        title = "Statistics of TF family"
