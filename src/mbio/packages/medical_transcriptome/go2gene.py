...
if __name__ == '__main__':
    GO_db = Terms()
    go_annot_path = open(sys.argv[1]).read().split('\n')
    go_type = sys.argv[2]
    min_gene = int(sys.argv[3])
    max_gene = int(sys.argv[4])
    gos_sets = sys.argv[5].split(",")
    ...
    gene2go = {}
    # 获取gene_id go对应关系
    for linerecord in go_annot_path:
        if len(linerecord) != 0:
    ...
    for go, go_term in GO_db.items():
        # print go
        gene_list = go_term.genes
        go_des = go_term.name
