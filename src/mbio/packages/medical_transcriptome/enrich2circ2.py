...
class Enrich(object):
    ...
    def get_fc(self, fc_table):
        ...
        self.diff_fc_abs = {k: abs(v) for k,v in self.diff_fc.items()}
        ...
        with open(gene_detail_file, 'rb') as f:
            ...
            if len(terms) == 4:
                for line in f.readlines()[1:]:
                    if line.split("\t")[2] != "":
                        self.seq_id2name[line.split("\t")[0]] = line.split("\t")[2].strip()
                        self.seq_id2name[line.split("\t")[1]] = line.split("\t")[2].strip()
            elif len(terms) == 3:
                for line in f.readlines()[1:]:
                    if line.split("\t")[2] != "":
                        self.seq_id2name[line.split("\t")[0]] = line.split("\t")[1].strip()
    ...
    def filter_go_enrich(self, go_enrich_table):
        ...
        # 筛选基因
        seq_lists = list(enrich_data.seq_str)
        if len(self.gene_list) > 0:
            choose_seq_lists_genes = [set(x.split("|")).intersection(set(self.gene_list)) for x in seq_lists]
        else:
            choose_seq_lists_genes = list()
            choosed_gene_list = list()
            for x in seq_lists:
                choosed_gene_list.extend(x.split("|"))

            if len(set(choosed_gene_list)) <= 200:
                self.top_gene_num = 200
            for x in seq_lists:
                sorted_gene = sorted(x.split("|"), key=lambda x:self.diff_fc_abs[x], reverse=True)
                choose_seq_lists_genes.append(sorted_gene[:self.top_gene_num])
        enrich_data['choose_gene'] = pd.Series(choose_seq_lists_genes, index=enrich_data.index)
        choose_seq_lists_genes_num = [len(x) for x in choose_seq_lists_genes]
            ...
            for seq in choose_seqs:
                ...
                for line in range(0, len(enrich_data)):
                    ...
                    if seq in genes and seq in self.diff_fc:
                        has_gene.append('1')
                        gene_name_show = gene_name
                        if gene_name == "":
                            gene_name_show = "-"
                        f2.write("{}\t{}\t{}\t{}\t{}\n".format(seq, annot_id, des, self.diff_fc[seq], gene_name_show))
                    else:
                        has_gene.append('0')
                if gene_name == "":
                    gene_name = seq
    ...
    def filter_kegg_enrich(self, kegg_enrich_table):
        ...
        if len(enrich_data) == 0:
            raise Exception('筛选条件所得的富集结果为空请重新设置参数')
    ...
    def filter_do_enrich(self, do_enrich_table):
        '''
        过滤do富集分析结果, 并得到富集弦图
        '''
        enrich_data = pd.read_table(do_enrich_table, header=0)
        if self.anno_list:
            enrich_data = enrich_data[enrich_data.do_id.isin(self.anno_list)]
        else:
            enrich_data = enrich_data[enrich_data.pvalue < self.max_p_value]
            enrich_data = enrich_data[enrich_data.padjust < self.max_padj_value]
            enrich_data = enrich_data[:self.max_term_num]
        print("筛选参数 p {} padj {} term_num {} terms {}".format(self.max_p_value, self.max_padj_value, self.max_term_num,
                                                                 self.anno_list))
        print(len(enrich_data))
        ...
        with open("do_enrich_choose.table", 'w') as f, open("do_enrich_detail.table", 'w') as f2:
            ids = list(enrich_data.do_id)
            terms = list(enrich_data.do_name)
            f.write("{}\t{}\t{}\t{}\n".format("seq_id", "\t".join(ids), "log2FC", "Gene_name"))
            f2.write("{}\t{}\t{}\t{}\t{}\n".format("Gene", "DO_id", "DO_description", "log2FC", "Gene_name"))
            for seq in choose_seqs:
                gene_name = seq
                if self.seq_id2name.has_key(seq):
                    gene_name = self.seq_id2name[seq]
        ...
        with open("enrich_zscore", 'w') as f:
            ...
            for i in range(0, len(terms)):
                fc_list = [self.diff_fc[x] for x in list(seq_lists[i].split("|")) if x in self.diff_fc]
                fc_up = [self.diff_fc[x] for x in list(seq_lists[i].split("|")) if
                         x in self.diff_fc and self.diff_fc[x] > 0]
                fc_down = [self.diff_fc[x] for x in list(seq_lists[i].split("|")) if
                           x in self.diff_fc and self.diff_fc[x] < 0]
                z_score = (len(fc_up) - len(fc_down)) / (math.sqrt(len(fc_list)) + 0.01)
                f.write("{}\t{}\t{}\n".format(ids[i], terms[i], z_score))
    ...
    def filter_disgenet_enrich(self, disgenet_enrich_table):
        '''
        过滤disgenet富集分析结果, 并得到富集弦图
        '''
        enrich_data = pd.read_table(disgenet_enrich_table, header=0)
        if self.anno_list:
            enrich_data = enrich_data[enrich_data.disease_id.isin(self.anno_list)]
        else:
            enrich_data = enrich_data[enrich_data.pvalue < self.max_p_value]
            enrich_data = enrich_data[enrich_data.padjust < self.max_padj_value]
            enrich_data = enrich_data[:self.max_term_num]
        ...
        with open("disgenet_enrich_choose.table", 'w') as f, open("disgenet_enrich_detail.table", 'w') as f2:
            ids = list(enrich_data.disease_id)
            terms = list(enrich_data.disease_name)
            f.write("{}\t{}\t{}\t{}\n".format("seq_id", "\t".join(ids), "log2FC", "Gene_name"))
            f2.write("{}\t{}\t{}\t{}\t{}\n".format("Gene", "Disease_id", "Disease_description", "log2FC", "Gene_name"))
            for seq in choose_seqs:
                gene_name = seq
                if self.seq_id2name.has_key(seq):
                    gene_name = self.seq_id2name[seq]
    ...
    def filter_reactome_enrich(self, reactome_enrich_table):
        '''
        过滤reactome富集分析结果, 并得到富集弦图
        '''
        enrich_data = pd.read_table(reactome_enrich_table, header=0)
        if self.anno_list:
            enrich_data = enrich_data[enrich_data.pathway_id.isin(self.anno_list)]
        else:
            enrich_data = enrich_data[enrich_data.pvalue < self.max_p_value]
            enrich_data = enrich_data[enrich_data.padjust < self.max_padj_value]
            enrich_data = enrich_data[:self.max_term_num]
        ...
        with open("reactome_enrich_choose.table", 'w') as f, open("reactome_enrich_detail.table", 'w') as f2:
            ids = list(enrich_data.pathway_id)
            terms = list(enrich_data.description)
            f.write("{}\t{}\t{}\t{}\n".format("seq_id", "\t".join(ids), "log2FC", "Gene_name"))
            f2.write("{}\t{}\t{}\t{}\t{}\n".format("Gene", "Reactome_id", "Reactome_description", "log2FC", "Gene_name"))
            for seq in choose_seqs:
                gene_name = seq
                if self.seq_id2name.has_key(seq):
                    gene_name = self.seq_id2name[seq]
