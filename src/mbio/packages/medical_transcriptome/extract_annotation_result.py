...
class RefAnnotation():
    ...
    def change_annot_result(self, annot, gene_out, trans_out):
        ...
        annot_df['GO_id'] =annot_df['go'].map(lambda x: ";".join(
            [a.split("(")[0] for a in x.split("; ")]))
        annot_df['GO_term'] =annot_df['go'].map(lambda x: ";".join(
            [a.split("(")[1].split(":")[0]
             for a in x.split("; ") if len(a.split("(")) > 1]))
        annot_df['Go_description'] =annot_df['go'].map(lambda x: ";".join(
            [go_dict.get(a.split("(")[1].split(":")[0], 'UN') + ":" +
             "(".join(a.split("(")[1:]).split(":")[1].rstrip(")")
             for a in x.split("; ") if len(a.split("(")) > 1]))
        annot_df['COG_id'] = annot_df['cog'].map(lambda x: ";".join(
            [a.split("(")[0] for a in x.split("; ")]))
        annot_df['COG_Functional_Categories'] = annot_df['cog'].map(lambda x: ";".join(
            ["(".join(a.split("(")[1:]).rstrip(')')
             for a in x.split("; ") if len(a.split("(")) > 1]))
        annot_df['NR_hit-name'] = annot_df['nr'].map(lambda x: x.split("(")[0])
        annot_df['NR_description'] = annot_df['nr'].map(lambda x: "(".join(
            x.split("(")[1:]).rstrip(")") if len(x.split("(")) > 1 else "")
        annot_df['Swiss-Prot_hit-name'] = annot_df['swissprot'].map(lambda x: x.split("(")[0])
        annot_df['Swiss-Prot_description'] = annot_df['swissprot'].map(lambda x: "(".join(
            x.split("(")[1:]).rstrip(")") if len(x.split("(")) > 1 else "")

        annot_df['Pathway_id'] =annot_df['paths'].map(lambda x: ";".join([a.split("(")[0] for a in x.split("; ")]))
        annot_df['Pathway_definition'] =annot_df['paths'].map(lambda x: ";".join(
            ["(".join(a.split("(")[1:]).rstrip(")")
             for a in x.split("; ") if len(a.split("(")) > 1]))
        annot_df['Pfam_id'] =annot_df['pfam'].map(lambda x: ";".join(
            [a.split("(")[0] for a in x.split("; ")]))
        annot_df['Domain'] =annot_df['pfam'].map(lambda x: ";".join(
            [a.split("(")[1].split(":")[0]
             for a in x.split("; ") if len(a.split("(")) > 1]))
        annot_df['Domain_description'] =annot_df['pfam'].map(lambda x: ";".join(
            ["(".join(a.split("(")[1:]).split(":")[1].rstrip(")")
             for a in x.split("; ") if len(a.split("(")) > 1]))
        annot_df = annot_df.drop(columns=['pfam', 'go', 'length', 'cog', 'swissprot', 'entrez', 'nr', 'swissprot'])
        annot_df = annot_df.rename(index=str, columns={"gene_id": "Gene ID", "transcript_id": "Transcript ID", 'gene_name': "Gene Name", 'description': "Gene Description"})
        annot_df_gene = annot_df.loc[annot_df['is_gene'] == "yes"]
        annot_df = annot_df.reindex(["Transcript ID", "Gene ID", "Gene Name", "Gene Description", 'GO_id', "GO_term", 'Go_description',
        ...
        annot_df_gene = annot_df_gene.reindex(["Gene ID", "Gene Name", "Gene Description", 'GO_id', "GO_term", 'Go_description',
                                     "KO_id", "KO_name", "Pathway_id", "Pathway_definition",
                                     'COG_id', 'COG_Functional_Categories',
                                     'NR_hit-name',  'NR_description', 'Swiss-Prot_hit-name', 'Swiss-Prot_description',
                                     'Pfam_id', 'Domain', 'Domain_description'
            ...
            for line in lines[1:]:
                ...
                if database_venn.has_key(v3_db2db[line[0]]):
                    ...
                    if os.path.exists(tran_venn) and os.path.exists(gene_venn):
                        ...
                        data = [
                            str(len(gene_set)) + "({})".format(round(float(len(gene_set))/float(len(exp_g_set) if len(exp_g_set) else 1), 4)),
                            str(len(tran_set)) + "({})".format(round(float(len(tran_set))/float(len(exp_t_set) if len(exp_t_set) else 1), 4))
                        ] + data
                    else:
                        raise Exception("{}对应的venn.txt文件{} {}不存在".format(line[0], venn, gene_venn))
                elif v3_db2db[line[0]] == "Total_anno":
                    data = [
                        str(len(anno_gene_set)) + "({})".format(round(float(len(anno_gene_set))/float(len(exp_g_set) if len(exp_g_set) else 1), 4)),
                        str(len(anno_tran_set)) + "({})".format(round(float(len(anno_tran_set))/float(len(exp_t_set) if len(exp_t_set) else 1), 4))
                    ] + data
