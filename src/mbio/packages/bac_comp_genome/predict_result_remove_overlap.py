...
def get_type_info(infile,name_index,pos1_index,pos2_index,type,strand_pos =None,header=0,seq_index=None,seq_split=False, orf_index=None):
    ret = {}
    if seq_index == None:
        ret['-'] =[]
    with open(infile) as fr:
        ...
        for line in fr:
            ...
            if seq_index != None:
                if seq_split:
                    if orf_index:
                        k = sp[seq_index].split('_%s'%(orf_index))[0]
                    else:
                        k = sp[seq_index].split('_')[0]
                else:
                    k = sp[seq_index]
...
def main_(type_file_list,trna=None,rrna=None):
    ...
    gene_core = get_type_info(infile,c_index_info['name'],c_index_info['pos1'],c_index_info['pos2'],
                              type,c_index_info['strand'],header=1,seq_index=c_index_info['seq_id'],seq_split=True, orf_index='ORF')
    ...
    if len(current_types_sort)> 2:
        for t,f,i in current_types_sort[1:]:
            ...
            if t == 'prodigal':
                ...
                split_or = True
            other = get_type_info(f,i['name'],i['pos1'],i['pos2'],t, i['strand'],header=1,seq_index=i['seq_id'],seq_split=split_or)
            gene_core = add_element(gene_core,other,50)        

    if trna or rrna:
        if trna:
            trna_dic = get_type_info(trna[1],trna[2]['name'],trna[2]['pos1'],trna[2]['pos2'],trna[0],
                                      header=1,seq_index=trna[2]['seq_id'],seq_split=True, orf_index='tRNA')
        ...
        if rrna:
            rrna_dic = get_type_info(rrna[1],rrna[2]['name'],rrna[2]['pos1'],rrna[2]['pos2'],rrna[0],
                                      header=1,seq_index=rrna[2]['seq_id'],seq_split=True, orf_index='rRNA')
