...
for(i in 1:nrow(exp_11)){
  ...
  # o2[o2==0]=NA
  if(test == "student"){
    tt <- try(t.test(log_o1,log_o2,var.equal = TRUE,alternative = "${test_type}",conf.level = ${ci}))
  }else if(test == "welch"){
    tt <- t.test(log_o1,log_o2,var.equal = FALSE,alternative = "${test_type}",conf.level = ${ci})
  }else if(test == "signal"){
    tt <- wilcox.test(log_o1,log_o2,alternative = "${test_type}",exact = F,conf.level = ${ci},paired = T)
  }else{
    tt <- wilcox.test(log_o1,log_o2,alternative = "${test_type}",exact = F,conf.level = ${ci})
  }
  if(nrow(otu_data) == 1){
     o1 = o1/o1
     o2 = o2/o2
