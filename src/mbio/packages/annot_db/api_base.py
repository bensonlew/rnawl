...
class ApiBase(Base):
        ...
        if record_id is not None:
            if isinstance(record_id, types.StringTypes):
               ...
               record_id = record_id
                ...
                raise Exception('type of main id must be String or ObjectId')
        conn = self.db[table_name]
        if query_dict:
            if record_id is not None:
            ...
            if record_id is not None:
                ...
                raise Exception('query dict must be provided while record id is None')
        if insert_dict:
        ...
        for main_record in main_records:
            ...
            if detail_table:
                ...
                for table, table_key in zip(detail_table, detail_table_key):
                    if not table_key:
                        raise Exception('you should specify detail_table_key whose value is the _id of main table')
    ...
    def get_tables_by_main_record(self, main_table, **kwargs):
        """
        :根据maintable 属性获取表格记录
        :param main_table: 要删除的表的名称，如sg_exp
        :param kwargs 其它属性表示匹配需要删除主表的属性
        """
        conn = self.db[main_table]
        a = conn.find(kwargs, {'_id': 0}).sort("_id")
        return a
    ...
    def create_index(self, main_table, index_list, **kwargs):
        ...
        """
        conn = self.db[main_table]
        conn.create_indexes(index_list, **kwargs)

    def drop_collection(self, main_table):
        """
        创建索引
        """
        conn = self.db.drop_collection(main_table)
