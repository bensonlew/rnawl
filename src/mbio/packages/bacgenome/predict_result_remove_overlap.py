...
def get_type_info(infile,name_index,pos1_index,pos2_index,type,strand_pos =None,header=0,seq_index=None,seq_split=False):
    ret = {}
    if seq_index == None:
        ret['-'] =[]
...
if __name__ == "__main__":
    ...
    if a.prodigal:
        with open(a.prodigal) as f, open('prodigal_tmp.gff', 'w') as fw:
            fw.write("seqid\tsource\ttype\tstart\tend\tscore\tstrand\tphase\tattributes\n")
            for line in f:
                if line[0] == '#':
                    continue
                spline = line.split('\t')
