...
while(my $seq = $in1->next_seq()) {
 ...
 open(OUT, ">$id.cgview.cog") || die "could not open cgvew.cog \n";
print OUT "seqname\tsource\tfeature\tstart\tend\tscore\tstrand\tframe\n";
  ...
  while(<IN>)  {
    chomp;
    my @temp = split /\t+/;
    if($hash_chr{$id} eq $temp[5] || $hash_pla{$id} eq $temp[5] || $id eq $temp[5]){
      if($temp[10] ne '-'){
...
my $out = Bio::SeqIO->new(-file => ">$id.gbk", -format => "genbank") ;
