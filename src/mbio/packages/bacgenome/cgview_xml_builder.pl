...
my %my_cog_color ;
if(-e 'cog_color.list'){
    open(IN, "cog_color.list");
    while(<IN>){
        my @line = split;
        $my_cog_color{$line[0]}="rgb($line[2])";
    }
}
...
if(-e 'cog_color.list'){
    $settings{'cogColors'}= \%my_cog_color ;
};
...
sub _writeFeatureLegend {
	...
	if (defined($options->{'genes'})) {
		...
		$legend = $legend . "<legendItem text=\"" . $_ ." ". $legend_des{$_} . "\" drawSwatch=\"true\" swatchOpacity=\"" . $settings->{'featureOpacity'} . "\" swatchColor=\"" . $settings->{'cogColors'}->{$_} . "\" />\n";
	    }
	}
    }
