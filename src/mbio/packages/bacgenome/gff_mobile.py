...
"""
处理gff和预测出来的is文件，如果存在重复序列位置在基因上的，要去掉
"""
    ...
    with open(mobile, "r") as f, open(out, "w") as d:
        ...
        for line in lines:
            ...
            if "false" in list1:
                ...
                d.write("{}\n".format("\t".join(lin)))
    ## 对第一列的id进行排序,后面直接根据这个id进行导表
    # os.system("sort -k 1 {} > {}".format(out, "is_stat.xls"))
    os.system("sed -i '1iIS_ID\tLocation\tStart\tEnd\tStrand\tFamily\tCluster\tIRL\tIRR'" +" %s" %(out))
