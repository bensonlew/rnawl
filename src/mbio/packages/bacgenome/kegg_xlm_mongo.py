...
class kegg_xlm_mongo(object):
    ...
    def __init__(self):
        ...
        self.process_rerun1 = 0
        self.process_rerun2 = 0
    ...
    def kegg_by_mongo(self, kegg_table, out_dir):
        ...
        with open(kegg_table, 'r') as table, open(kegg_anno, 'wb') as anno, open(kegg_enzyme, 'wb') as e, \
            ...
            anno.write('#Query\tKegg_name_id\tGene\tKO\tDefinition\tPathway\tEnzyme\tModule\tHyperlink\tIdentity('
                       '%)\tAlign_len\tLevel1\tLevel2\tLevel3\tEnzyme_description\n')
                ...
                if line[0] != "Score":
                    ...
                    if query_name in query_list:
                        ...
                        try:
                            ...
                            if self.process_rerun1 < 5:
                                self.process_rerun1 += 1
                                # self.logger.info("检测到TimeoutError, 第{}次重运行方法".format(self.process_rerun))
                                time.sleep(5)
                                self.kegg_by_mongo(kegg_table, out_dir)
                            else:
                        ...
                        if koids:
                            ...
                            try:
                                ...
                                if self.process_rerun2 < 5:
                                    self.process_rerun2 += 1
                                    # self.logger.info("检测到TimeoutError, 第{}次重运行方法".format(self.process_rerun))
                                    time.sleep(5)
                                    self.kegg_by_mongo(kegg_table, out_dir)
                                else:
                            ...
                            if result:
                                ...
                                if ko_name.istitle():  # 按产品要求处理KO_name
                                    ko_name = "-"
                                else:
                                    te = re.subn("^[A-Z]\S{0,}, ", "",ko_name)[1]
                                    tes = re.subn("^[A-Z]\S{0,}", "",ko_name)[1]
                                    if te == 0  and tes == 0:
                                        ko_name = ko_name.split(", ")[0]
                                    elif te == 0 and tes != 0 :
                                        ko_name = "-"
                                    else:
                                        ko_name = re.subn(" [A-Z]\S{0,},", "",ko_name)[0]
                                        ko_name = ko_name.split(", ")[1]
                            ...
                            anno.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.
                                       format(query_name, line[10], ko_name, ko_id, des, path_, enzyme, module_, h, iden,
                                              align_len, l1, l2, l3, en_des))
                        else:
                            continue
                else:
                    continue
