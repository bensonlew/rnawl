...
def class_code_type(gtf_file, class_code_file, feature_type, gtf_type):
    ...
    with open(gtf_file,'r+') as f, open(class_code_file, 'w+') as w:
        ...
        for keys in class_code.keys():
            if "ref_gene_id" in class_code[keys].keys():
                w.write(str(keys)+"\t"+str(class_code[keys]["class_code"])+'\t'+str(class_code[keys]['ref_gene_id'])+"\n")
            else:
    ...
    """
"""
def convert(ref_gtf, old_gtf, new_gtf):
    
    对cufflinks生成的merged.gtf进行进一步的处理，oId → transcript_id(ensembl) → gene_id(ensembl)
    :param: ref_gtf参考基因组gtf文件
    :param: old_gtf, cufflinks 生成的merged.gtf
    :param: new_gtf, gene_id，transcript_id为ensembl id 的新merged.gtf文件
    
    if os.path.exists(old_gtf):
        pass
    else:
        raise Exception("{}文件不存在".format(old_gtf))
    with open(old_gtf, "r+") as f1, open(new_gtf, "w+") as f2, open(ref_gtf,'r+') as f3:
        transcript2gene={}  #读取所有的gene和转录本对应信息
        for k1 in f3:
            k2=k1.strip().split("\t")
            k3=k2[8]
            k4=k3.split(";")
            for lll in k4:
                k5=lll.replace(' ',"")
                if k5.startswith('gene_id'):
                    gene_id = k5.split('\"')[1]
                if k5.startswith('transcript_id'):
                    transcript_id = k5.split('\"')[1]
            if gene_id:
                if transcript_id:
        ...
        for lines in f1:
            ...
            for l in line2:
                d=l.replace(' ','')
                if d.startswith('transcript_id'):
                    old_transcript = d.split('\"')[1]
                if d.startswith('oId'):
                    new_transcript = d.split('\"')[1]
            new_line=[]  #存储基因和转录本信息
            new_line2 = [line2[1], line2[0]] #line2中基因和转录本顺序调换
            #print line2
            for ll in new_line2:
                if re.search('transcript_id', ll):
                    if old_transcript:
                        if new_transcript:
                            _line = ' transcript_id '+'\"'+new_transcript+"\""
                            if new_transcript in transcript2gene.keys():
                                _gene = ' gene_id '+'\"'+transcript2gene[new_transcript]+"\""
                                new_line.append(_gene)
                            else:
                                new_line.append(line2[0])
                            new_line.append(_line)
                        else:
                            new_line.extend(line2[:2])
                    else:
                        new_line.extend(line2[:2])
            new_line.extend(line2[2:])
            new_line1="; ".join(new_line)  #新的信息
            f2.write("\t".join(data[:8])+"\t"+new_line1+"\n")
"""
def convert(ref_gtf, old_gtf, new_transcript_gene_info):
    """
    对cufflinks生成的merged.gtf进行进一步的处理，oId → transcript_id(ensembl) → gene_id(ensembl)
    :param: ref_gtf参考基因组gtf文件
    :param: old_gtf, cufflinks 生成的merged.gtf
    ...
    a1=""" awk -F "\\t" "{print $9}" %s | awk -F ';' '{print $1$2}' | awk -F '"' '{print $2"\\t"$4}' """%(ref_gtf)
    print a1
    a2=subprocess.check_output(a1, shell=True)
    a3=[i.split("\t")for i in a2.split("\n")[:-1]]
    print '参考基因组gtf转换成功！'
    #b1=""" awk -F "\\t" "{print $9}" %s | awk -F ';' '{print $1$2$5}' | awk -F '"' '{print $2"\\t"$4"\\t"$6}' """%(old_gtf)
    b1=""" awk -F "\\t" "{print $9}" %s | awk -F ';' '{print $1$2$5$7}' | awk -F '"' '{print $2"\\t"$4"\\t"$6"\\t"$8}' """%(old_gtf)
    print b1
    b2=subprocess.check_output(b1, shell=True)
    b3=[j.split("\t")for j in b2.split("\n")[:-1]]
    print 'merged.gtf转换成功！'
    transcript2gene = {}
    for s in a3:
        transcript2gene[s[0]]=s[1]
    old2new={}
    print b3[0][0],b3[0][1],b3[0][2]
    for t in b3:
        try:
            old2new[t[1]] = {}
            old2new[t[1]]["gene_id"]=t[0]
            old2new[t[1]]['oId']=t[2]
            old2new[t[1]]['class_code']=t[3]
        except Exception:
            print "计算失败，打印出信息!"
            print t
    with open(new_transcript_gene_info, 'w+') as f1:
        f1.write("Transcript_ID_MergedGtf"+"\t"+'OId_MergedGtf'+'\t'+"Gene_ID_MergedGtf"+"\t"+"Gene_ID_RefGtf"+'\t'+'Class_code'+'\n')
        for keys, values in old2new.items():  #merged.gtf transcript_id
            if values['oId'] in transcript2gene.keys():
                f1.write(keys+"\t"+values['oId']+'\t'+values['gene_id']+"\t"+transcript2gene[values['oId']]+'\t'+values['class_code']+'\n')
            else:
                f1.write(keys+"\t"+values['oId']+'\t'+values['gene_id']+'\t'+values['class_code']+'\n')
    """
    #print transcript2gene
    #print old2new
    with open(old_gtf, 'r+') as f1, open(new_gtf,'w+') as f2:
        for lines in f1:
            data = lines.strip().split("\t")
            line=data[8]
            line2=line.split(";")
            ...
            for llm in new_line2:
                ...
                if re.search('transcript_id', ll):
                    ...
                    if transcript in  old2new.keys():
                        _line = ' transcript_id '+'\"'+old2new[transcript]+"\""
                        if old2new[transcript] in transcript2gene.keys():
                            _gene = ' gene_id '+'\"'+transcript2gene[old2new[transcript]]+"\""
                        else:
                            _gene = line2[0]
                        new_line.append(_gene)
                        new_line.append(_line)
...
def gff3(infile, outfile, out_path):
