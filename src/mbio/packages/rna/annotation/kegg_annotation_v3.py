...
class KeggAnnotation(object):
                    ...
                    for item in koids:
                        ...
                        if result:
                            ...
                            for index, i in enumerate(pids):
                                map_num = re.sub('ko', '', i)
                                i_spe = species_abr + map_num
                                # print "****" + i_spe
                                if self.species_path:
                                    if map_num in self.species_path:
                                        #map_id = re.sub("ko", "map", i)
                                        map_id = species_abr + map_num
    ...
    def pathTable(self, r_path, map_path, kegg_table, pathway_path, pidpath, link_bgcolor, png_bgcolor, pathwaydir, image_magick, species_abr,species_path_dir ):
        ...
        for key in path_table:
            if key:
                ...
                self.get_pic(r_path, map_path, key, kos_path, png_path, html_path, species_abr, species_path_dir)
                if image_magick:
    ...
    def get_pic(self, r_path, map_path, path, kos_path, png_path, html_path, species_abr, species_path_dir):
        """
        画通路图
        """
        fs = gridfs.GridFS(self.mongodb)
        species_path_id = path
        pid = re.sub(species_abr, "ko", path)
        map_id = re.sub(species_abr, "map", path)
        map_html = KeggHtml()
    ...
    def keggLayer(self, pathway_table, layerfile, species_abr):
        ...
        for record in islice(f, 1, None):
            ...
            pid = re.sub(species_abr, "ko", iterm[0])
            result = self.ko_coll.find_one({"pathway_id": {"$in": [pid]}})  # 找到对应的集合
            if result:
                pids = result["pathway_id"]  # 找到对应的pid列表
                layer = False
