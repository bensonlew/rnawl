...
def get_info_dict(paths_file,version):
    ...
    db = Config().get_mongo_client(mtype='whole_transcriptome',db_version =version)[Config().get_mongo_dbname('whole_transcriptome',db_version =version)]
    for main_id in dct:
        dct[main_id]['compare_plan'] = db['splicing_rmats'].find_one({'main_id': ObjectId(main_id)})['compare_plan']
        documents = list(db['splicing_rmats_detail'].find({'splicing_id': ObjectId(main_id)}))
        for d in documents:
