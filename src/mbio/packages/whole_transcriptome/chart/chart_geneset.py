...
class ChartGeneset(Chart):
    ...
    def chart_geneset_cluster(self, cluster_exp, cluster_tree, sample_tree, subcluster_list, group_dict=None, samples_order=None,level="T"):
    ...
    def chart_geneset_class_cog(self, cog_class_table, geneset_list=None,level="T"):
        ...
        self.chart_class_column("cog_annot", ".gene_set", source, categories, None, None, "geneset.annot_cog_stat.bar.json",y_title=y_title)
    ...
    def chart_geneset_class_go(self, go_class_table, geneset_list=None, top=20):
        ...
        if len(geneset_list) == 1:
            ...
            title = "GO annotations analysis_Level 2({})".format(geneset)
        else:
            for geneset in geneset_list:
                try:
        ...
        self.chart_class_column("go_annot", ".gene_set", source, categories,class_source,title, bac_json,reset_margin=False)
    ...
    def chart_geneset_class_kegg(self, kegg_class_table, geneset_list=None, top=20,level="T"):
        ...
        if len(geneset_list) == 2 :
            ...
            if geneset_name1 == geneset_name2:
                ...
                a[diff_all_geneset + "_num"] = a.apply(lambda  x :x[geneset_list[0]+"_num"] + x[geneset_list[1]+"_num"],axis=1)
        ...
        for geneset in geneset_list:
            ...
            self.chart_class_column("kegg_annot.",  geneset, source, None, class_source,  title,  "geneset.annot_kegg_stat.bar.json",y_title = y_title)
    ...
    def chart_geneset_enrich_go(self, go_enrich_table, geneset_list=None, geneset_name="geneset",top=20,p_thre=0.5):
        ...
        a = a.sort_values(by=["p_corrected"], ascending=[True])
        a = a[:top]
        # 反转
        a = a[top-1::-1]
        source_bar = [
        ...
        b = a.sort_values(by=["p_corrected"], ascending=[True])
        source_bar = [
    ...
    def chart_geneset_enrich_circ(self, circ_table, circ_zscore_input,level = "T"):
        ...
        for rec in circ.to_dict('records'):
            if level == "T":
                source_circ.append([rec["seq_id"]] + [rec[term] for term in term_ids] + [rec["log2fc"]])
            else:
