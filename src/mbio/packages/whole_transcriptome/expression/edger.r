...
for (i in seq(length(contrast.vec))) {
    ...
    if (fit.method == "exactTest") {
        ...
        qlf.obj <- glmLRT(fit.obj, contrast = makeContrasts(paste(case, ctrl, sep = "-"), levels = levels(o.y.obj$samples$group)))
    } else if (fit.method == "glmQLFit") {
        qlf.obj <- glmQLFTest(fit.obj, contrast = makeContrasts(paste(case, ctrl, sep = "-"), levels = levels(o.y.obj$samples$group)))
    }
