...
class Target2ce(object):
        ...
        for n,target in enumerate(target_file):
            with open(target, 'r') as f:
                ...
                for line in f:
                    ...
                    ce2type[cols[cn]] = cols[2]
                    ce2name[cols[cn]] = cols[3]
                    if self.gene_type == "G":
                        ce2gene[cols[1]] = cols[3]
                    else:
                        ce2gene[cols[1]] = cols[3]
    ...
    def run_ce(self, pvalue_cut):
        ...
        if self.retain_type in ['lnc2m', 'circ2m', 'lnccirc2m']:
            ce2_list  = [x for x in ce2type if ce2type[x] == "mRNA"]
            if self.retain_type == "lnc2m":
                ce1_list  = [x for x in ce2type if ce2type[x] == "lncRNA"]
            elif self.retain_type == "circ2m":
                ce1_list  = [x for x in ce2type if ce2type[x] == "circRNA"]
            elif self.retain_type == "lnccirc2m":
                ce1_list  = [x for x in ce2type if ce2type[x] in ["lncRNA", "circRNA"]]
            ...
            with open(self.output, 'w') as ce_f:
                ...
                for ce1 in ce1_list:
                    for ce2 in ce2_list:
                        ...
                        if p_value < pvalue_cut and int(inter_num)>=self.mi_min_num:
                            ce_f.write("\t".join([
...
if __name__ == "__main__":
    ...
    parser.add_argument('-pvalue_cutoff', type=float, default=1, help='pvalue cutoff. Default: 0.05')
    parser.add_argument('-anno', type=str, default=None, help='the dir of annotation')
    parser.add_argument('-mi_min_num', type=int, default=3, help='min support mirna num')
    parser.add_argument('-retain_type', type=str, default="all", help='retain ceRNA pair type')
    # ----------------------------------------------------------------------------------------------
    ...
    toolbox = Target2ce( mrna_target = args.mrna_target,
                         ...
                         mi_min_num = args.mi_min_num,
                         retain_type = args.retain_type
    )
