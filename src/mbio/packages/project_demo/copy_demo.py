...
class CopyDemoMongo(Base):
    def __init__(self, old_task_id, new_task_id, new_project_sn, new_member_id, project_type='ref_rna_v2', db_version=0):
        super(CopyDemoMongo, self).__init__()
        # self._config.DBVersion = db_version
            ...
            if params_str and params_str != "null" and len(params_str) > 1:
                if isinstance(params_str, dict):
                    ...
                    try:
                        ...
                        print("params：{}对应的字符串无法转换为字典，跳过对其修改".format(params_str))
                        # pass
        return old_doc_dict
    ...
    def run(self):
        ...
        print("Total {} collection to be copied".format(len(target_collections)))
        ...
        print("Total {} main tables copied".format(self.copied_doc_num))
