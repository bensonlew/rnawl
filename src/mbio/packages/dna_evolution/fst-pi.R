...
write.table(file=paste(opt$out,"fst_pi.detail.select",sep="."),row.names=FALSE,subset(pi,(pi$theta < quantile(pi$theta,probs=thre1) | pi$theta > quantile(pi$theta,probs=thre2)) & pi$fst > quantile(pi$fst,probs=thre2) ));
