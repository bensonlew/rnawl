...
a <- file.info(opt$input)$size  # qingmei
if (a>0){   # qingmei
    ...
    names(data)<-c("id","description","k","M","n","N","O");
    data<-na.omit(data)
    pvalue<-phyper(data$k,data$M,data$N-data$M,data$n,lower.tail=FALSE);
    qvalue<-p.adjust(pvalue,method="fdr")
    outd<-data.frame(id=data$id,des=data$description,eff=data$k,total=data$M,pvalue=1-pvalue,qvalue=1-qvalue,tra=data$O)
