...
if (a>0){
    ...
    print('$$$$$$')
    times<-Sys.time()
    data<-read.table(opt$input,sep="\t",comment.char="^",head=FALSE)
    # names(data)<-c("id","description","k","M","n","N");
    names(data)<-c("id","description","k","M","n","N","geneid");    # cui add
    data<-na.omit(data)
    ...
    outd<-data.frame(id=data$id,des=data$description,eff=data$k,total=data$n,pvalue=1-pvalue,qvalue=1-qvalue,geneid=data$geneid)
