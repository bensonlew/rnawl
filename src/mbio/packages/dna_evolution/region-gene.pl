...
while (<In>) {
	...
	foreach my $region (sort keys %{$region{$chr}}) {
		...
		if (($Pos1 > $pos3 && $Pos1 <$pos4) ||($Pos2 > $pos3 && $Pos2 < $pos4) || ($pos3 > $Pos1 && $pos3 < $Pos2) || ($pos4 > $Pos1 && $pos4 < $Pos2)) {
			...
			$stat{$chr}{$region}{totaleggnog}++ if($egid ne "--");
			...
			$stat{$chr}{$region}{effeggnog}++ if($egid ne "--" && $High+$Moderate > 0);
			$regioned=1;
		}
	}
	$astat{total}++;
	...
	for (my $i=0;$i<@koid;$i++) {
		next if ($koid[$i] eq "--");
		$enrich{$koid[$i]}{total}++;
		push @{$enrich{$koid[$i]}{geneid}},$Gene_id;
	...
	for (my $i=0;$i<@goid;$i++) {
		next if ($goid[$i] eq "--");
		$enrich{$goid[$i]}{total}++;
		push @{$enrich{$goid[$i]}{geneid}},$Gene_id;
	...
	for (my $i=0;$i<@eid;$i++) {
		next if ($eid[$i] eq "--");
		$enrich{$eid[$i]}{total}++;
		push @{$enrich{$eid[$i]}{geneid}},$Gene_id;
...
foreach my $koid (sort keys %kdetail) {
	...
	$astat{totalkegg}||=0;
	$astat{effkegg}||=0;
	print Out join("\t",$koid,$kdetail{$koid},$enrich{$koid}{enrich},$enrich{$koid}{total},$astat{eff},$astat{total},join("|",@{$enrich{$koid}{geneid}}),),"\n";
}
close Out;
...
foreach my $goid (sort keys %gdetail) {
	...
	$astat{totalgo}||=0;
	$astat{effgo}||=0;
	print Out join("\t",$goid,$gdetail{$goid},$enrich{$goid}{enrich},$enrich{$goid}{total},$astat{eff},$astat{total},join("|",@{$enrich{$goid}{geneid}})),"\n";
}
close Out;
...
foreach my $eggnog (sort keys %edetail) {
	...
	$astat{totaleggnog}||=0;
	$astat{effeggnog}||=0;
	print Out join("\t",$eggnog,$edetail{$eggnog},$enrich{$eggnog}{enrich},$enrich{$eggnog}{total},$astat{eff},$astat{total},join("|",@{$enrich{$eggnog}{geneid}})),"\n";
}
close Out;
