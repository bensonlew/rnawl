...
#    print CHRBSA "  {\"id\":\"$chr\",\"label\":\"$chr\",\"color\":\"#FFFFFF\",\"len\":$length},\n";
	my $chrbsa="{\"id\":\"$chr\",\"label\":\"$chr\",\"color\":\"#FFFFFF\",\"len\":$length}";   
	push @chrbsa,"$chrbsa";
}
print CHRBSA join(",\n  ",@chrbsa);
print CHRBSA "]\n";
...
foreach my $keys (sort {$a<=>$b}keys %hash_gff){
    ...
    foreach my $num (sort keys %{$hash_gff{$keys}}){
        ...
        print COBSA "$chr_type"."$keys,$start,$end,",$hash_gff{$keys}{$num},"\n"; ## gene.num.csv
        $band++;
    }
    my $bandnum=$keys;
