...
if __name__ == '__main__':
    ...
    plt.setp(ax, xticks=[x for x in range(1, len(data) + 1)], xticklabels=ratio_df.columns)
    for xtick in ax.get_xticklabels():
        xtick.set_rotation(90)
    fig = ax.get_figure()
    fig.tight_layout()
    fig.subplots_adjust(hspace=0)
    fig.savefig(os.path.join(output_dir, 'box.png'))
    fig.savefig(os.path.join(output_dir, 'box.pdf'))
