...
sub dat_to_gff3 {
	...
	while (<IN>) {
		...
		if ($_ =~ /^\@Chromosome(\S{0,})\s+/){
			$chr = "Chr".$1;
			$dat_out .= "\@$chr\n";
		}elsif ($_ =~ /^\@Plasmid(\S{0,})\s+/){
			$chr = "p".$1;
			$dat_out .= "\@$chr\n";
			if(grep /^$chr$/, @plas){
				$chr = "p".$1;
		...
		my $score = $t[7];
		my $strand = "+";
		$output .= "$chr\tTRF\tTandemRepeat\t$start\t$end\t$len\t$score\t$strand\t.\tID=$id;PeriodSize=$t[2];CopyNumber=$t[3];PercentMatches=$t[5];PercentIndels=$t[6];Consensus=$t[13];\n";
		$mark++;
		$dat_out .= "$seq";
    ...
    print OUT "Sequence Name\tSource\tType\tStart\tEnd\tLength\tScore\tStrand\tPhase\tAttributes\n$output";
	close OUT;	
	open OUT1,">$file.new" || die "fail creat $file";
    print OUT1 "$dat_out";
	close OUT1;
}
...
sub out_to_gff3 {
	...
	open OUT,">$file.gff" || die "fail creat $file";
	#print OUT "##gff-version 3\n$output";
    print OUT "Sequence Name\tSource\tType\tBegin\tEnd\tScore\tStrand\tPhase\tAttributes\n$output";
	close OUT;	
