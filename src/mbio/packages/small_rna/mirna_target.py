...
class TagertPrediction():
    def __init__(self, query_fa, target_fa,seq_num=100, method='psRobot',
                 ...
                 miranda=None, pita=None, rnahybrid=None, targetscan=None, target_species=None, pool=10):
        self.method = method
    ...
    def RNAhybrid_list(self, b=1, e=-20, pvalue=0.01,distribution="2,0.2",max_target=10000):
        cmd_list = []
        for split in self.split_fasta:
    ...
    def merge_generate(self):
        if self.method.lower() == 'miranda' and self.spiece.lower() == 'animal':
            ...
            with open(self.targetscan, 'r') as f1, open('targetscan_merge_out', 'w') as f2:
                f1.readline()
                f2.write("#small_rna\ttarget\tUTR start\tUTR end\tMSA start\tMSA end\tSeed match\n")
                for line in f1:
                            ...
                            if mi in query_dict:
                                if self.target_species == "9606":
                                    if cols[3].split(".")[0] in target_list:
                                        f2.write("\t".join([query_dict[mi], cols[3].split(".")[0], cols[5], cols[6], cols[7], cols[8], cols[9]]) + "\n")
                                else:
                                    gene = cols[1].split(".")[0]
                                    if gene in hsa2id:
                                        for transcript in hsa2id[gene]:
                                            if transcript in target_list:
                                                f2.write("\t".join([query_dict[mi], transcript, cols[5], cols[6], cols[7], cols[8], cols[9]]) + "\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-rnahybrid_b', type=str, metavar="rnahybrid_b",
                        default="1"),
    parser.add_argument('-rnahybrid_e', type=str, metavar="rnahybrid_energy",
                        default="-20"),
    parser.add_argument('-rnahybrid_p', type=str, metavar="rnahybrid_p",
                        default="0.01"),
    ...
    parser.add_argument('-psrobot_ts', type=str, metavar="psrobot_ts",
                        default="2.5")
    ...
    parser.add_argument('-targetfinder_c', type=str, metavar="targetfinder_c",
                        default="4")
