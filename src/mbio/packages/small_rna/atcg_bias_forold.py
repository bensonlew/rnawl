...
def change_to_per(file):
    with open(file,'r') as xls_r, open(file.split('.xls')[0] + '_per.xls','w') as per:
...
with open(args.fasta,'r') as fasta:
    ...
    for mirna in mirnas:
        ...
        if not first_atcg[len(seq)][seq[0]]:
            first_atcg[len(seq)][seq[0]] = 0
        first_atcg[len(seq)][seq[0]] += int(time)
        if len(seq) >= 18:
            for i in range(len(seq)):
                if not loc_atcg[i+1][seq[i]]:
                    loc_atcg[i + 1][seq[i]] = 0
                loc_atcg[i+1][seq[i]] += int(time)

with open('all_loc_bias.xls', 'w') as loc_w, open('all_first_bias.xls', 'w') as length_w:
    ...
    for i in range(1, int(args.max)+1):
        ...
        print(line)
        loc_w.write(line)
change_to_per('all_loc_bias.xls')
change_to_per('all_first_bias.xls')
