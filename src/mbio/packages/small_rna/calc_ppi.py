...
nodes_table = pd.read_table(nodeFile, header=0)
nodes_table.sort_values(['degree'],  ascending = [False], inplace=True)
nodes_table['node_id'] = range(0, len(nodes_table))
id2acc =dict(zip(nodes_table['node_id'],nodes_table['node']))
acc2id =dict(zip(nodes_table['node'],nodes_table['node_id']))
id2acc2 =dict(zip(nodes_table['node_id'],nodes_table['accession_id']))
...
#     for i in G.edges():
#         tmp_file.write(id2acc[i[0]]+"\t"+id2acc[i[1]]+"\t"+str(G[i[0]][i[1]]["weight"])+"\n")
with open(os.path.join(args["output"], "protein_interaction_network_node_degree.txt"), "w") as tmp_file:
    ...
    for i in id2acc.keys():
        tmp_file.write(str(i)+"\t"+id2acc[i]+"\t")
        tmp_file.write(str(G.degree(i)) + "\t" + id2acc2[i] + "\n")
