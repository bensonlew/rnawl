...
def process_exp_matrix(exp_matrix, log_base=None, group_dict=None, group_method="mean"):
    if type(exp_matrix) == str or type(exp_matrix) == bytes:
        ...
        all_exp_pd.dropna(how='all', inplace=True)
    else:
...
def exp_cluster(exp_pd, n_clusters=10, s_cluster=True, g_cluster=True,
                ...
                s_metric="euclidean", g_metric="euclidean",
...
if __name__ == '__main__':
    ...
    parser.add_argument('-scd', metavar='sample-cluster-distance', default='euclidean', help="hclust distance metric")
    ...
    if args.group:
        ...
        group_dict = None
    #
    exp_pd = process_exp_matrix(args.exp, log_base=args.log_base, group_dict=group_dict, group_method=args.group_method)
    exp_pd.dropna(how='all', inplace=True)
    exp_pd.fillna(value=0, inplace=True)
    #
