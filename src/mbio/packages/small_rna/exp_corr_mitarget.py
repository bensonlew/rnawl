...
class Expresscor(object):
    ...
    def calculate_corr(self, pvalue_cutoff=None, qvalue_cutoff=None):
        ...
        if self.cor_cutoff:
            df3_filter = df3[(df3['corr']>self.cor_cutoff) | (df3['corr']< - self.cor_cutoff)]
        else:
            df3_filter = df3
        ...
        df_filter = df3_filter[['query', 'target', 'gene_id', 'gene_name', 'gene_description', 'mirtarbase', 'corr', 'pvalue', 'qvalue']]
