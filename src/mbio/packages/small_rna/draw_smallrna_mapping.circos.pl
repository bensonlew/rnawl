...
my $BEGIN_TIME=time();
use Getopt::Long;
my ($pos, $neg,$chrlist,$gff,$outdir,$windows);
use Data::Dumper;
...
&USAGE unless ($pos and $neg and $gff and $chrlist);
########
########work dir
########
$outdir||="./";
...
foreach my $keys (sort {$hash_chr_num{$a}<=>$hash_chr_num{$b}}keys %hash_chr_num){
    ...
    foreach my $num (sort{$a<=>$b} keys %{$hash_gff{$hash_chr_num{$keys}}}){
