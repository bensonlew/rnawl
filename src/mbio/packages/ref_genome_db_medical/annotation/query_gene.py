...
class AllAnnoStat(object):
    ...
    def get_anno_stat(self, gene2trans, tran_outpath, gene_outpath, new_gtf_path, ref_gtf_path, length_path, gene_file,
                      ...
                      pfam_domain=None, des=None, subloc=None, enterz=None, dos_list=None, reactomes_list=None, disgenets_list=None):
        """
        ...
        if dos_list != None and dos_list != "None":
            self.get_do(dos_list=dos_list)
        if reactomes_list != None and reactomes_list != "None":
            self.get_reactome(reactomes_list=reactomes_list)
        if disgenets_list != None and disgenets_list != "None":
            self.get_disgenet(disgenets_list=disgenets_list)

        with open(tran_outpath, 'wb') as w:
            head = 'gene_id\ttranscript_id\tis_gene\tgene_name\tlength\tdescription\tcog\tcog_description\tKO_id\tKO_name\tpaths\tpfam\tgo\tnr\tuniprot\tentrez\tbio_type\tkegg_genes\tspe_path\tdo\treactome_id\treactome_name\tpathway_id\tpathway_description\treactome_link\tdisgenet_name\tdisgenet_id\tdisgenet_des\n'
            w.write(head)
            for name in self.stat_info:
                ...
                if gene2trans:
                    w.write('\t'.join([
                        ...
                        ";".join(self.stat_info[name].tran_id),
                        ...
                        self.stat_info[name].do,
                        self.stat_info[name].reactome_id,
                        self.stat_info[name].reactome_name,
                        self.stat_info[name].pathway_id,
                        self.stat_info[name].pathway_description,
                        self.stat_info[name].reactome_link,
                        self.stat_info[name].disgenet_name,
                        self.stat_info[name].disgenet_id,
                        self.stat_info[name].disgenet_des,
                    ]) + "\n")
    ...
    def get_unigene(self, gene2trans):
        ...
        with open(gene2trans, 'r') as f:
            ...
            for line in lines:
                ...
                if line[1] not in self.stat_info:
                    self.stat_info[line[1]] = Transcript()
                    self.stat_info[line[1]].length = line[3]
                    self.stat_info[line[1]].name = line[0]
                    self.stat_info[line[1]].gene_id = line[1]
                    self.stat_info[line[1]].is_gene = line[2]
                    self.stat_info[line[1]].tran_id = [line[0]]
                else:
                    self.stat_info[line[1]].tran_id.append(line[0])
    ...
    def get_enterz(self, enterz):
        ...
        with open(enterz, 'r') as f:
            for id_dict in csv.DictReader(f, delimiter='\t'):
                if id_dict['gene_id'] in self.stat_info:
                    self.stat_info[id_dict['gene_id']].enterz = id_dict['entrezgene_id']
                    if id_dict['entrezgene_id'] == "\\N":
                        self.stat_info[id_dict['gene_id']].enterz = ""
                    self.stat_info[id_dict['gene_id']].seq_type = id_dict['gene_biotype']
    ...
    def get_do(self, dos_list):
        for line in open(dos_list):
            eles = line.strip('\n').split('\t')
            transcript_id = eles[0]
            if transcript_id in self.stat_info:
                self.stat_info[transcript_id].do = eles[1]
        logging.debug('succeed in calling {}'.format(sys._getframe().f_code.co_name))

    def get_reactome(self, reactomes_list):
        for line in open(reactomes_list):
            eles = line.strip('\n').split('\t')
            transcript_id = eles[0]
            if transcript_id in self.stat_info:
                self.stat_info[transcript_id].reactome_id = eles[1]
                self.stat_info[transcript_id].reactome_name = eles[2]
                self.stat_info[transcript_id].pathway_id = eles[3]
                self.stat_info[transcript_id].pathway_description = eles[4]
                self.stat_info[transcript_id].reactome_link = eles[5]
        logging.debug('succeed in calling {}'.format(sys._getframe().f_code.co_name))

    def get_disgenet(self, disgenets_list):
        for line in open(disgenets_list):
            eles = line.strip('\n').split('\t')
            transcript_id = eles[0]
            if transcript_id in self.stat_info:
                self.stat_info[transcript_id].disgenet_name = eles[1]
                self.stat_info[transcript_id].disgenet_id = eles[2]
                self.stat_info[transcript_id].disgenet_des = eles[3]
        logging.debug('succeed in calling {}'.format(sys._getframe().f_code.co_name))
    ...
    '''
    def get_uniprot(self, blast_uniprot_table):
        """找到转录本ID对应uniprot库的最佳hit_name及描述"""
        with open(blast_uniprot_table, 'rb') as f:
            f.readline()
            flag = None
            for line in f:
                ...
                if flag == query_name:
                    ...
                    uniprot_hit_name = line[10]
                    uniprot_description = line[-1]
                    uniprot = uniprot_hit_name + "(" + uniprot_description + ")"
                    flag = query_name
                    if query_name in self.stat_info:
                        self.stat_info[query_name].uniprot = uniprot
    ...
    def get_des(self, des, des_type="type1"):
        """
        获取蛋白名或描述信息
        """
        with open(des, "rb") as f:
            ...
            for line in f.readlines():
                ...
                if self.stat_info.has_key(gene_id):
                    self.stat_info[gene_id].des = des
                    self.stat_info[gene_id].gene_name = symbol
                    self.stat_info[gene_id].symbol = symbol
...
if __name__ == '__main__':
    ...
    AllAnnoStat().get_anno_stat(gene2trans=sys.argv[1],
                                ...
                                subloc=sys.argv[15],
                                enterz=sys.argv[16],
                                dos_list=sys.argv[17],
                                reactomes_list=sys.argv[18],
                                disgenets_list=sys.argv[19]
                                )
