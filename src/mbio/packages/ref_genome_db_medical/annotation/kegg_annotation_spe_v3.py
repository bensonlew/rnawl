...
class KeggAnnotation(object):
    ...
    def merge_known(self, known_ko_annot, kegg_table):
        ...
        def map_kos2paths(kos):
            # print("kos is {}".format(kos))
            paths = list()
            for ko in kos.split(";"):
                if ko in self.ko2path:
                    for path in self.ko2path[ko]:
                        # print "path is {}".format(path)
                        if path.replace(self.species_abr, "map")  in self.map2class and path.replace(self.species_abr, "map") not in self.gloabl:
                            paths.append(path)
                        # if path.replace("map", "ko") in self.ko_list:
                        #     paths.append(path)
                    # paths.extend(list(self.ko2path[ko]))
        
            # print("paths is {}".format())
        ...
        for key in sorted_paths:
            if key:
                ...
                if key in self.map2class:
                    definition = self.map2class[key][2]
                else:
                    definition = ""
    ...
    def get_species_path(self, species_abr, species_path_dir):
        ...
        if os.path.exists(path_file):
            ...
            self.species_path = list(set(spe_paths))
        # self.gloabl = [p.replace("map", species_abr) for p in self.gloabl]
