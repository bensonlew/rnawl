...
class Tree(object):
    ...
    def __init__(self, name, taxid, level_num, level_id, all_reads, lvl_reads, children=None, parent=None):
        ...
        self.children = []
        self.parent =  parent
        self.linkage =  {self.level_id: self.name}
        self.get_linkage(self)
    ...
    def get_linkage(self, node=None):
        parent = node.parent
        if isinstance(node.parent, Tree):
            self.linkage[parent.level_id] = parent.name
            if parent.level_id != 'U':
                 self.get_linkage(parent)
...
def main():
    ...
    for line in i_file:
        ...
        if taxid == '1':
            ...
            linkages[taxid] = root_node.linkage
            prev_node = root_node
            continue 
        #Save leaf nodes
        ...
        linkages[taxid] = curr_node.linkage
    ...
    o_file.write('name\t' + 'taxonomy_id\t' + 'taxonomy_lvl\t' + 'kraken_assigned_reads\t' + 'added_reads\t' + 'new_est_reads\t' + 'fraction_total_reads\tlinkage\n')
    for taxid in lvl_taxids:
        [name, all_reads, lvl_reads, added_reads] = lvl_taxids[taxid]
        ...
        o_file.write("%0.5f\t" % (float(new_all_reads)/float(sum_all_reads)))
        o_file.write("{}\n".format(linkages[taxid]))
    o_file.close()
