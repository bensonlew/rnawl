...
class Ipath(object):
    ...
    def set_svg_legend(self, element):
        ...
        text.attrib['x'] = str(int(text.attrib['x']) - 5)
        legend_length = len(self.legend[0])

        # 根据字符长度自适应字体大小
        text_font_size = 50
        if len(self.legend) == 2:
            legend_length = legend_length + len(self.legend[1])
        if legend_length > 30:
            text_font_size = 1800/legend_length
            text.attrib['style'] = 'opacity: 1'
            text.attrib['lengthAdjust'] = 'spacingAndGlyphs'
            text.attrib['textLength'] = str(text_font_size * 0.6 * len(self.legend[0]))
        ...
        if len(self.legend) == 2:
            ...
            text2_x = text_x  + int(text_font_size * 0.6 * len(self.legend[0])) + 30
            text2.attrib['lengthAdjust'] = 'spacingAndGlyphs'
            text2.attrib['textLength'] = str(text_font_size * 0.6 * len(self.legend[1]))
            keg_col2.attrib['d'] = 'M{},30L{},30'.format(text2_x , text2_x+100)
            text2.attrib['x'] = str(text2_x + 145)
            text2.attrib['fill'] = "#00ff00"
            keg_col2.attrib['stroke'] = "#00ff00"

            text3_x = text2_x + 150 + int(text_font_size * 0.6 * len(self.legend[1])) + 30
            keg_col3.attrib['d'] = 'M{},30L{},30'.format(text3_x , text3_x+100)
            text3.attrib['x'] = str(text3_x + 145)
            text3.attrib['lengthAdjust'] = 'spacingAndGlyphs'
            text3.attrib['textLength'] = str(text_font_size * 0.6 * 4)
