...
def gtf_to_trans2gene(gtf, trans2gene):
    transcript_id_map_gene_id = dict()
    pt = re.compile(r'transcript_id "(\S+)";')
    pg = re.compile(r'gene_id "(\S+)";')
    for line in open(gtf):
        mt, mg = re.search(pt, line), re.search(pg, line)
        if mt and mg:
            transcript_id = mt.group(1)
            gene_id = mg.group(1)
            transcript_id_map_gene_id[transcript_id] = gene_id
    with open(trans2gene, 'w') as handle:
        for transcript_id, gene_id in transcript_id_map_gene_id.items():
