...
fpkm_density<-function(fpkm_data, logValue){ #add functions by khl 20170403
    ...
    if(logValue != "None"){
        minfpkm = as.integer(log(1e-02)/log(logValue))-1
        maxfpkm = as.integer(log(1e+06)/log(logValue))+1
    }else{
        minfpkm = as.integer(min(fpkm_data))-1
        maxfpkm = as.integer(max(fpkm_data))+1
    }

    for(i in 1:length(samples)){
       a=fpkm_data[i]
       den=density(a[,1],n = 601,from = minfpkm, to = maxfpkm)
...
write.table(fpkmlog2, file=paste("${outputfile}","/log2",filename,"_distribution.xls", sep=""), sep='\t', quote=F, row.names=F, col.names=T)
write.table(fpkmlog10, file=paste("${outputfile}","/log10",filename, "_distribution.xls", sep=""), sep='\t', quote=F, row.names=F, col.names=T)
write.table(fpkm, file=paste("${outputfile}","/",filename,"_distribution.xls", sep=""), sep='\t', quote=F, row.names=F, col.names=T)
#dens_new<-cbind(den$x,dens)
