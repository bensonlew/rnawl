...
def filter_ref_gene_or_transcript(input_file, output_path, gene_list=True):
    """
    ...
    if gene_list:
        list_file = open(output_path + "/ref.{}.list".format(filename), 'w+')
    with open(input_file, 'r+') as f1, open(output_path + "/ref.{}".format(filename), 'w+') as f2:
        f1.readline()
        id_list = []
        for lines in f1:
            line = lines.strip().split("\t")
            if re.search(r'MSTRG', line[0]):
                continue
            elif re.search(r'XLOC', line[0]):
                continue
            elif re.search(r'TCONS', line[0]):
                continue
            else:
                f2.write(lines)
            if gene_list:
