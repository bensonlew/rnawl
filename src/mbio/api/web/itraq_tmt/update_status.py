...
class UpdateStatus(Us):
    ...
    def __init__(self, data):
        # self.submit_type = self.update_info.pop("submit_type") if 'submit_type' in self.update_info else 0
    ...
    def update_status(self):
        ...
        # submit_type = self.update_info.pop("submit_type") if 'submit_type' in self.update_info else 0
        for obj_id, collection_name in self.update_info.items():
            ...
            if status == "start":
                if not batch_id:
                    ...
                    control_id = ""
                    proteinset_id = ""
                    if temp_find and "submit_type" in temp_find.keys():
                        submit_type = int(temp_find['submit_type'])
                    else:
                        submit_type = 0
                    if temp_find and "params" in temp_find.keys():
                        ...
                        try:
                            ...
                            if isinstance(temp_json, dict) and "proteinset_id" in temp_json.keys():
                                proteinset_id = temp_json['proteinset_id']
                    insert_data = {
                        "table_id": obj_id,
