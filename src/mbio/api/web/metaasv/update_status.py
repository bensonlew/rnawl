...
class UpdateStatus(Log):
        ...
        for obj_id, collection_name in self.update_info.items():
            ...
            if status == "start":
                if not batch_id:
                    ...
                    add_item = ['group_id', 'env_id', 'env_labs', "asv_id", "set_id", "alpha_diversity_id"]
                    ...
                    if collection_name in ["asv_set"]:
                        insert_data['set_id'] = ObjectId(str(obj_id))
                    if collection_name not in ["sample_check"]:
                ...
                if not batch_id:
                    ...
                    if collection_name not in ["sample_check"]:
                        sg_status_col.update_one({"table_id": obj_id, "type_name": collection_name},{'$set': insert_data}, upsert=True)
                ...
                if not batch_id:
                    ...
                    if collection_name not in ["sample_check"]:
                        sg_status_col.update_one({"table_id": obj_id, "type_name": collection_name},{'$set': insert_data},upsert=True)
                        sg_status_col.update_one({"table_id": obj_id, "type_name": collection_name},{"$set": {"task_id": main_task_id}},upsert=True)
    ...
    def pipe_update(self, batch_id, collection, _id, status, desc):
        ...
        if str(collection) == "asv" and str(status) == "failed" and self.db['asv'].find_one({"_id": ObjectId(_id)}):
