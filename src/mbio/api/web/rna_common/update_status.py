...
# last_modified_date = 20171109
# last modified by hd@20200825
import urllib
import json
import datetime
import re
...
class UpdateStatus(Log):
    ...
    def __init__(self, data):
        ...
        self._project_db = self.config.get_mongo_client(mtype="project",
                                                        db_version=self._dbversion)[self.config.get_mongo_dbname(mtype="project", db_version=self._dbversion)]
    ...
    #     if "region" in self.data['sync_task_log'].keys():
    #         content['region'] = self.data['sync_task_log']['region']
    #     data['sync_task_log'] = json.dumps(content, cls=CJsonEncoder)
    #     yield urllib.urlencode(data)
        ...
        if status == "failed" or status == "terminated" or status == "finish":
            ...
            for obj_id, collection_name in self.update_info.items():
                ...
                data = {
                    ...
                    # 'task_id': int(re.sub(r'^[\w\-]+_', '', main_task_id)),
                    "task_id": re.sub(r'^[\w\-]+_', '', main_task_id),
                    'out_task_id': main_task_id,
                    'params': params,
                    'submit_location': submit_location,
                    'desc': desc,
                    'cmd_id': int(task_info["cmd_id"]) if "cmd_id" in task_info.keys() else 0,
                    'member_id': int(re.sub(r"^\w+_", "", task_info["member_id"])) if "member_id" in task_info.keys() else 0,
        ...
        for obj_id, collection_name in self.update_info.items():
            ...
            if status == "start":
                if not batch_id:
                    ...
                    # by houshuang 20191010 >>>
                    tmp_json = json.loads(params)
                    add_item = ['group_id', 'env_id', 'env_lab', 'env_labs', 'otuset_id']
                    for each_i in add_item:
                        if each_i in tmp_json.keys():
                            insert_data[each_i] = tmp_json[each_i]
                    if 'group_detail' in tmp_json.keys():
                        insert_data['group_lab'] = ','.join(tmp_json['group_detail'].keys())
                    if 'filter_json' in tmp_json:
                        otuset_id = []
                        for condition in tmp_json['filter_json']:
                            if 'otuset' in condition:
                                otuset_id.append(condition['otuset'])
                        insert_data['otuset_id'] = ','.join(set(otuset_id))
                    if submit_location == 'otuset':
                        insert_data['otuset_id'] = str(obj_id)
                    # <<<
                    # sg_status_col.insert_one(insert_data)
                    sg_status_col.update_one(insert_data)
