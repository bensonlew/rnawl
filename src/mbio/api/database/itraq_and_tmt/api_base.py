    ...
    def update_db_record(self, table_name, record_id, **kwargs):
        ...
        if isinstance(record_id, types.StringTypes):
           ...
           record_id = record_id
            ...
            self.bind_object.set_error("main_id参数必须为字符串或者ObjectId类型!", code="52500201")
        conn = self.db[table_name]
    ...
    def update_db_record_2(self, table_name, record_id, query_dict=None, **kwargs):
        if isinstance(record_id, types.StringTypes):
           ...
           record_id = record_id
            ...
            self.bind_object.set_error("main_id参数必须为字符串或者ObjectId类型!", code="52500202")
        conn = self.db[table_name]
        if query_dict:
            kwargs.update(query_dict)
        conn.update({"_id": record_id}, {"$set": kwargs}, upsert=True)
    ...
    def update_sgtask_record(self, task_id=None, table_name="sg_task", **kwargs):
        ...
        if task_id is None:
            if self.bind_object:
                ...
                self.bind_object.set_error('No valid task_id', code="52500203")
        conn = self.db[table_name]
    ...
    def get_project_sn(self, task_id=None):
        if task_id is None:
            if self.bind_object:
                ...
                self.bind_object.set_error('No valid task_id', code="52500204")
        ...
        for each in a:
            ...
            if detail_table:
                ...
                for table, table_key in zip(detail_table, detail_table_key):
                    if not table_key:
                        self.bind_object.set_error('you should specify detail_table_key whose value is main table "_id"', code="52500205")
    ...
    def remove_table_by_main_record(self, main_table, detail_table=None, detail_table_key=None, **kwargs):
        ...
        for each in a:
            ...
            if detail_table:
                ...
                for table, table_key in zip(detail_table, detail_table_key):
                    if not table_key:
                        self.bind_object.set_error('you should specify detail_table_key whose value is main table "_id"', code="52500206")
                    self.remove_db_record(table, query_dict={table_key: each['_id']})
