...
from mbio.api.database.itraq_and_tmt.proteinset import Proteinset

class AllExp(Proteinset):
    def __init__(self, bind_object):
        super(AllExp, self).__init__(bind_object)

    def add_t2g_info(self, t2g_file, project_sn='itraq_tmt', task_id='itraq_tmt'):
        """
    ...
    def process_exp_matrix(exp_matrix, log_base=None, group_dict=None):
        ...
        if log_base:
            if log_base == math.e:
                ...
                self.bind_object.set_error('log base of %s is not supported', variables=(log_base), code="52500101")
        # return
        return all_exp_pd

    def add_exp(self, exp_matrix, quant_method='RSEM', exp_level='T', group_dict=None, main_id=None, group_id=None,
                add_distribution=True, exp_type='tpm', project_sn='itraq_tmt', task_id='itraq_and_tmt',
    ...
    def add_distribution(self, exp_matrix, group_dict, params=None, exp_level='T',
                         quant_method='RSEM', project_sn='itraq_tmt', main_id=None,
                         task_id='itraq_tmt', ):
        """
    ...
    def add_rsem_mapping(self, align_rate_file, project_sn='itraq_tmt', task_id='itraq_tmt'):
        "该函数暂时无用"
    ...
    def add_exp_pca(self, exp_matrix, quant_method='RSEM', exp_id=None, exp_level='T', params=None,
                    project_sn='itraq_tmt', task_id='itraq_tmt', ):
        """
        dump exp data into database
        :param exp_matrix: expression matrix path or an express matrix in pandas DataFrame format.
        :param exp_level: str, transcript or gene
    ...
    def add_exp_pca2(self, exp_output_dir, exp_id=None, params=None,
                    project_sn='itraq_tmt', task_id='itraq_tmt', main_id=None):
        if not main_id:
            # prepare main table info
            name = "ExpPCA" + '_'
            time_now = datetime.datetime.now()
        ...
        self.update_db_record('sg_express_pca', main_id, status="end", main_id=main_id, version='V2')
        return main_id
    ...
    def add_exp_corr(self, exp_matrix, exp_level='T', quant_method='RSEM', sample_order=None,
                     ...
                     project_sn='itraq_tmt', task_id='itraq_tmt', params=None):
        """
        dump exp data into database
        :param exp_matrix: expression matrix path or an express matrix in pandas DataFrame format.
        :param exp_level: str, transcript or gene
    ...
    def add_exp_corr3(self, corr_output_dir, params=None, main_id=None,
        ...
        if type(main_id) == str or type(main_id) == unicode:
            main_id = ObjectId(main_id)
        self.update_db_record('sg_express_corr', main_id, clust_tree=sample_tree, samples=samples,)
        tag_dict = dict(corr_id=main_id)
    ...
    def add_diffexp(self, diff_output, exp_id=None, group_dict=None, group_id=None,
        ...
        if not diff_files:
            self.bind_object.set_error('No target file found in %s', variables=(diff_output), code="52500102")
        cmp_list = list()
    ...
    def add_geneset_cluster(self, cluster_output_dir, main_id=None, project_sn='itraq_and_tmt', task_id='itraq_and_tmt',
                            params=None):
        update_status = False
        if not main_id:
            update_status = True
