...
class ProteinAnnotation(ApiBase):
        ...
        for key, value in self.result_file.items():
            if key == "png_path":
                if os.path.exists(value) or os.path.exists(value + ".tar.gz"):
                    ...
                    self.bind_object.set_error('%s对应的结果文件%s 不存在，请检查', variables=(key, value), code="52501001")
            else:
                if os.path.exists(value):
                    pass
                else:
                    self.bind_object.set_error('%s对应的结果文件%s 不存在，请检查', variables=(key, value), code="52501002")
    ...
    def check_id(self, object_id):
        if not isinstance(object_id, ObjectId):
            if isinstance(object_id, types.StringTypes):
                ...
                self.bind_object.set_error('assemble_id必须为ObjectId对象或其对应的字符串！', code="52501003")
        return object_id
    ...
    def get_trans2gene(self, trans2gene):
        if os.path.exists(trans2gene):
            ...
            self.bind_object.set_error('转录本基因对应的结果文件%s不存在，请检查', variables=(trans2gene), code="52501004")
    ...
    def add_annotation_kegg_pic(self, kegg_id, seq_type, anno_type, level_path, png_dir):
        ...
        if not isinstance(kegg_id, ObjectId):
            if isinstance(kegg_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_id必须为ObjectId对象或其对应的字符串！', code="52501005")
        ...
        if data_list:
            try:
                ...
                self.bind_object.set_error("导入kegg注释图片信息：%s、%s出错!" , variables=(level_path, png_dir), code="52501006")
            else:
    ...
    def run_protein_webroot(self, result_dir, params_dict, task_id, stat_id, last_id):
        ...
        self.update_db_record('sg_annotation_kegg', kegg_id, status="end", main_id=kegg_id)
        if kegg_old_id:
            self.remove_table_by_main_record(main_table='sg_annotation_kegg', _id=kegg_old_id['_id'],
                                             detail_table=['sg_annotation_kegg_categories', 'sg_annotation_kegg_level',
                                                           'sg_annotation_kegg_table', 'sg_annotation_kegg_pic'],
                                             detail_table_key='kegg_id')
        else:
    ...
    def add_annotation(self, name=None, params=None, ref_anno_path=None, new_anno_path=None, pfam_path=None, merge_tran_output=None, merge_gene_output=None):
        ...
        if os.path.exists(blast_path):
            ...
            self.bind_object.set_error("没有blast的结果文件夹", code="52501007")
        ...
        if os.path.exists(nr_path):
            ...
            self.bind_object.set_error("新序列NR注释结果文件不存在", code="52501008")
        ...
        if os.path.exists(swissprot_path):
            ...
            self.bind_object.set_error("新序列Swiss-Prot注释结果文件不存在", code="52501009")
        ...
        if os.path.exists(pfam_path) and os.path.exists(gene_pfam_path):
            ...
            self.bind_object.set_error("pfam注释结果文件不存在", code="52501010")
        ...
        if os.path.exists(ref_stat_path) and os.path.exists(ref_venn_path):
            ...
            self.bind_object.set_error("已知序列注释统计文件和venn图文件夹不存在", code="52501011")
        ...
        def add_go(go_id, go_path, gene_go_path, seq_type):
            if os.path.exists(go_path) and os.path.exists(gene_go_path):
                ...
                self.bind_object.set_error("GO注释的结果文件不存在", code="52501012")
        ...
        def add_kegg(kegg_id, kegg_path, gene_kegg_path, seq_type):
            if os.path.exists(kegg_path) and os.path.exists(gene_kegg_path):
                ...
                self.bind_object.set_error("KEGG注释文件不存在", code="52501013")
    ...
    def add_annotation_stat_detail(self, stat_id, stat_path, venn_path):
        ...
        if not isinstance(stat_id, ObjectId):
            if isinstance(stat_id, types.StringTypes):
                ...
                self.bind_object.set_error('stat_id必须为ObjectId对象或其对应的字符串！', code="52501014")
        if not os.path.exists(stat_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(stat_path), code="52501015")
        if not os.path.exists(venn_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(venn_path), code="52501016")
        data_list = []
        database_venn = {
            'NR': 'nr',
            ...
            for line in lines[1:]:
                ...
                if database_venn.has_key(line[0]) and database_venn[line[0]] in database:
                    ...
                    if os.path.exists(venn):
                        ...
                        self.bind_object.set_error("%s对应的venn.txt文件不存在", variables=(line[0]), code="52501017")
                data = SON(data)
        ...
        try:
            ...
            self.bind_object.set_error("导入注释统计信息失败:%s" , variables=( stat_path), code="52501018")
        else:
    ...
    def add_stat_detail(self, old_stat_id, stat_id, nr_evalue, gene_nr_evalue, sw_evalue, gene_sw_evalue):
        ...
        if not isinstance(old_stat_id, ObjectId):
            if isinstance(old_stat_id, types.StringTypes):
                ...
                self.bind_object.set_error('old_stat_id必须为ObjectId对象或其对应的字符串！', code="52501019")
        if not isinstance(stat_id, ObjectId):
            if isinstance(stat_id, types.StringTypes):
                stat_id = ObjectId(stat_id)
            else:
                self.bind_object.set_error('stat_id必须为ObjectId对象或其对应的字符串！', code="52501020")
        collection = self.db["sg_annotation_stat_detail"]
        ...
        try:
            ...
            self.bind_object.set_error("导入注释统计信息出错", code="52501021")
        else:
    ...
    def add_annotation_blast(self, name=None, params=None, stat_id=None, result_dir=None):
        ...
        if not isinstance(stat_id, ObjectId):
            if isinstance(stat_id, types.StringTypes):
                ...
                self.bind_object.set_error('stat_id必须为ObjectId对象或其对应的字符串！', code="52501022")
        insert_data = {
            'project_sn': project_sn,
            'task_id': task_id,
    ...
    def add_annotation_blast_detail(self, blast_id, seq_type, anno_type, database, blast_path):
        if not isinstance(blast_id, ObjectId):
            if isinstance(blast_id, types.StringTypes):
                ...
                self.bind_object.set_error('blast_id必须为ObjectId对象或其对应的字符串！', code="52501023")
        if not os.path.exists(blast_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(blast_path), code="52501024")
        data_list = []
    ...
    def add_annotation_blast_nr_detail(self, blast_id, seq_type, anno_type, database, blast_path):
        if not isinstance(blast_id, ObjectId):
            if isinstance(blast_id, types.StringTypes):
                ...
                self.bind_object.set_error('blast_id必须为ObjectId对象或其对应的字符串！', code="52501025")
        if not os.path.exists(blast_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(blast_path), code="52501026")
        data_list = []
        with open(blast_path, 'r') as f:
            lines = f.readlines()
            flag = None
    ...
    def add_annotation_blast_swissprot_detail(self, blast_id, seq_type, anno_type, database, blast_path):
        if not isinstance(blast_id, ObjectId):
            if isinstance(blast_id, types.StringTypes):
                ...
                self.bind_object.set_error('blast_id必须为ObjectId对象或其对应的字符串！', code="52501027")
        if not os.path.exists(blast_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(blast_path), code="52501028")
        data_list = []
        with open(blast_path, 'r') as f:
            lines = f.readlines()
            flag = None
    ...
    def add_annotation_nr(self, name=None, params=None, stat_id=None, result_dir=None):
        ...
        if not isinstance(stat_id, ObjectId):
            if isinstance(stat_id, types.StringTypes):
                ...
                self.bind_object.set_error('stat_id必须为ObjectId对象或其对应的字符串！', code="52501029")
        insert_data = {
            'project_sn': project_sn,
            'task_id': task_id,
    ...
    def add_annotation_nr_pie(self, nr_id, species_path, evalue_path, similar_path, seq_type, anno_type):
        if not isinstance(nr_id, ObjectId):
            if isinstance(nr_id, types.StringTypes):
                ...
                self.bind_object.set_error('nr_id必须为ObjectId对象或其对应的字符串！', code="52501030")
        if not os.path.exists(evalue_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(evalue_path), code="52501031")
        if not os.path.exists(similar_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(similar_path), code="52501032")
        ...
        if data_list:
            try:
                ...
                self.bind_object.set_error("导入nr库注释作图信息evalue,similar：%s、%s出错!" , variables=(evalue_path, similar_path), code="52501033")
            else:
    ...
    def add_annotation_swissprot(self, name=None, params=None, stat_id=None, result_dir=None):
        ...
        if not isinstance(stat_id, ObjectId):
            if isinstance(stat_id, types.StringTypes):
                ...
                self.bind_object.set_error('stat_id必须为ObjectId对象或其对应的字符串！', code="52501034")
        insert_data = {
            'project_sn': project_sn,
            'task_id': task_id,
    ...
    def add_annotation_swissprot_pie(self, swissprot_id, evalue_path, similar_path, seq_type, anno_type):
        ...
        if not isinstance(swissprot_id, ObjectId):
            if isinstance(swissprot_id, types.StringTypes):
                ...
                self.bind_object.set_error('swissprot_id必须为ObjectId对象或其对应的字符串！', code="52501035")
        if not os.path.exists(evalue_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(evalue_path), code="52501036")
        if not os.path.exists(similar_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(similar_path), code="52501037")
        ...
        if data_list:
            try:
                ...
                self.bind_object.set_error("导入swissprot库注释作图信息evalue,similar：%s、%s出错!" , variables=(evalue_path, similar_path), code="52501038")
            else:
    ...
    def add_annotation_pfam(self, name=None, params=None, stat_id=None, result_dir=None):
        ...
        if not isinstance(stat_id, ObjectId):
            if isinstance(stat_id, types.StringTypes):
                ...
                self.bind_object.set_error('stat_id必须为ObjectId对象或其对应的字符串！', code="52501039")
        insert_data = {
            'project_sn': project_sn,
            'task_id': task_id,
    ...
    def add_annotation_pfam_detail(self, pfam_id, pfam_path, seq_type, anno_type):
        ...
        if not isinstance(pfam_id, ObjectId):
            if isinstance(pfam_id, types.StringTypes):
                ...
                self.bind_object.set_error('pfam_id必须为ObjectId对象或其对应的字符串！', code="52501040")
        if not os.path.exists(pfam_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(pfam_path), code="52501041")
        data_list = []
        ...
        if data_list:
            try:
                ...
                self.bind_object.set_error("导入pfam注释信息:%s失败！" , variables=( pfam_path), code="52501042")
            else:
    ...
    def add_annotation_pfam_bar(self, pfam_id, pfam_path, seq_type, anno_type):
        ...
        if not isinstance(pfam_id, ObjectId):
            if isinstance(pfam_id, types.StringTypes):
                ...
                self.bind_object.set_error('pfam_id必须为ObjectId对象或其对应的字符串！', code="52501043")
        if not os.path.exists(pfam_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(pfam_path), code="52501044")
        data_list = []
        for i in domain:
            data = [
                ('pfam_id', pfam_id),
        ...
        if data_list:
            try:
                ...
                self.bind_object.set_error("导入pfam注释信息:%s失败！" , variables=( pfam_path), code="52501045")
            else:
                self.bind_object.logger.info("导入pfam注释信息:%s成功！" % pfam_path)
    ...
    def add_annotation_cog_detail(self, cog_id, cog_path, seq_type, anno_type):
        ...
        if not isinstance(cog_id, ObjectId):
            if isinstance(cog_id, types.StringTypes):
                ...
                self.bind_object.set_error('cog_id必须为ObjectId对象或其对应的字符串！', code="52501053")
        if not os.path.exists(cog_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(cog_path), code="52501054")
        data_list = list()
        ...
        if data_list:
            try:
                ...
                self.bind_object.set_error("导入cog注释信息：%s出错!" , variables=(cog_path), code="52501055")
            else:
    ...
    def add_annotation_cog_detail_all(self, cog_id, r_cog_path, n_cog_path, seq_type, anno_type):
        ...
        if not isinstance(cog_id, ObjectId):
            if isinstance(cog_id, types.StringTypes):
                ...
                self.bind_object.set_error('cog_id必须为ObjectId对象或其对应的字符串！', code="52501056")
        if not os.path.exists(r_cog_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(r_cog_path), code="52501057")
        if not os.path.exists(n_cog_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(n_cog_path), code="52501058")
        ...
        if data_list:
            try:
                ...
                self.bind_object.set_error("导入cog注释all出错：%s， %s" , variables=(r_cog_path, n_cog_path), code="52501059")
            else:
    ...
    def add_annotation_cog_table(self, cog_id, table_path, seq_type, anno_type):
        ...
        if not isinstance(cog_id, ObjectId):
            if isinstance(cog_id, types.StringTypes):
                ...
                self.bind_object.set_error('cog_id必须为ObjectId对象或其对应的字符串！', code="52501060")
        if not os.path.exists(table_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(table_path), code="52501061")
        data_list = list()
        ...
        try:
            ...
            self.bind_object.set_error("导入cog注释table信息：%s出错!" , variables=(table_path), code="52501062")
        else:
    ...
    def add_annotation_go_detail(self, go_id, seq_type, anno_type, level, go_path):
        ...
        if not isinstance(go_id, ObjectId):
            if isinstance(go_id, types.StringTypes):
                ...
                self.bind_object.set_error('go_id必须为ObjectId对象或其对应的字符串！', code="52501063")
        if not os.path.exists(go_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(go_path), code="52501064")
        data_list = list()
        with open(go_path, 'r') as f:
            ...
            if data_list:
                try:
                    ...
                    self.bind_object.set_error("导入go注释信息：%s出错!" , variables=(go_path), code="52501065")
                else:
    ...
    def add_annotation_go_graph(self, go_id, seq_type, anno_type, level, go_path):
        ...
        if not isinstance(go_id, ObjectId):
            if isinstance(go_id, types.StringTypes):
                ...
                self.bind_object.set_error('go_id必须为ObjectId对象或其对应的字符串！', code="52501066")
        if not os.path.exists(go_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(go_path), code="52501067")
        data_list = list()
        with open(go_path, 'r') as f:
            lines = f.readlines()
            term = {}
        ...
        if data_list:
            try:
                ...
                self.bind_object.set_error("导入go注释画图信息出错：%s" , variables=(go_path), code="52501068")
            else:
    ...
    def add_annotation_go_level(self, go_id, seq_type, anno_type, level, level_path):
        ...
        if not isinstance(go_id, ObjectId):
            if isinstance(go_id, types.StringTypes):
                ...
                self.bind_object.set_error('go_id必须为ObjectId对象或其对应的字符串！', code="52501069")
        if not os.path.exists(level_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(level_path), code="52501070")
        data_list = list()
        ...
        if data_list:
            try:
                ...
                self.bind_object.set_error("导入go注释第二层级信息：%s出错!" , variables=(level_path), code="52501071")
            else:
    ...
    def add_annotation_go_list(self, go_id, seq_type, anno_type, gos_path):
        ...
        if not isinstance(go_id, ObjectId):
            if isinstance(go_id, types.StringTypes):
                ...
                self.bind_object.set_error('go_id须为ObjectId对象或其他对应的字符串！', code="52501072")
        if not os.path.exists(gos_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(gos_path), code="52501073")
        data_list = []
        with open(gos_path, 'r') as f:
            ...
            if data_list:
                try:
                    ...
                    self.bind_object.set_error("导入gos_list注释信息：%s出错:%s" , variables=(gos_path), code="52501074")
                else:
    ...
    def add_annotation_go_all(self, go_id, seq_type, anno_type, level, r_go_path, n_go_path):
        ...
        if not isinstance(go_id, ObjectId):
            if isinstance(go_id, types.StringTypes):
                ...
                self.bind_object.set_error('go_id必须为ObjectId对象或其对应的字符串！', code="52501075")
        if not os.path.exists(r_go_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(r_go_path), code="52501076")
        if not os.path.exists(n_go_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(n_go_path), code="52501077")
        ...
        if data_list2:
            try:
                ...
                self.bind_object.set_error("导入go注释all出错：%s、%s" , variables=(r_go_path, n_go_path), code="52501078")
            else:
    ...
    def add_annotation_kegg_categories(self, kegg_id, seq_type, anno_type, categories_path):
        ...
        if not isinstance(kegg_id, ObjectId):
            if isinstance(kegg_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_id必须为ObjectId对象或其对应的字符串！', code="52501079")
        if not os.path.exists(categories_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(categories_path), code="52501080")
        data_list = list()
        first_type = []
        ...
        try:
            ...
            self.bind_object.set_error("导入kegg注释分类信息：%s出错!" , variables=(categories_path), code="52501081")
        else:
    ...
    def add_annotation_kegg_level(self, kegg_id, seq_type, anno_type, level_path, png_dir):
        ...
        if not isinstance(kegg_id, ObjectId):
            if isinstance(kegg_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_id必须为ObjectId对象或其对应的字符串！', code="52501082")
        if not os.path.exists(level_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(level_path), code="52501083")
        if not os.path.exists(png_dir):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(png_dir), code="52501084")
        data_list = []
        with open(level_path, 'rb') as r:
            r.readline()
            for line in r:
        ...
        if data_list:
            try:
                ...
                self.bind_object.set_error("导入kegg注释层级信息：%s、%s出错!" , variables=(level_path, png_dir), code="52501085")
            else:
    ...
    def add_annotation_kegg_important(self, kegg_id, seq_type, anno_type, level_path, png_dir):
        ...
        if not isinstance(kegg_id, ObjectId):
            if isinstance(kegg_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_id必须为ObjectId对象或其对应的字符串！', code="52501086")
        if not os.path.exists(level_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(level_path), code="52501087")
        if not os.path.exists(png_dir):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(png_dir), code="52501088")
        data_list = []
        ...
        try:
            ...
            self.bind_object.set_error("导入kegg注释层级信息：%s、%s出错!" , variables=(level_path, png_dir), code="52501089")
        else:
            self.bind_object.logger.info("导入kegg注释层级信息：%s、%s 成功!" % (level_path, png_dir))
    ...
    def add_annotation_kegg_table(self, kegg_id, seq_type, anno_type, table_path):
        if not isinstance(kegg_id, ObjectId):
            if isinstance(kegg_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_id必须为ObjectId对象或其对应的字符串！', code="52501090")
        if not os.path.exists(table_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(table_path), code="52501091")
        ...
        try:
            ...
            self.bind_object.set_error("导入kegg注释table信息：%s出错!" , variables=(table_path), code="52501092")
        else:
    ...
    def add_annotation_kegg_categories_all(self, kegg_id, seq_type, anno_type, r_cate_path, n_cate_path):
        ...
        if not isinstance(kegg_id, ObjectId):
            if isinstance(kegg_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_id必须为ObjectId对象或其对应的字符串！', code="52501093")
        if not os.path.exists(r_cate_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(r_cate_path), code="52501094")
        if not os.path.exists(n_cate_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(n_cate_path), code="52501095")
        data_list = list()
        cate = {}
        ...
        try:
            ...
            self.bind_object.set_error("导入kegg注释分类alls出错：%s, %s" , variables=(r_cate_path, n_cate_path), code="52501096")
        else:
    ...
    def add_annotation_kegg_level_all(self, kegg_id, seq_type, anno_type, r_level_path, n_level_path):
        ...
        if not isinstance(kegg_id, ObjectId):
            if isinstance(kegg_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_id必须为ObjectId对象或其对应的字符串！', code="52501097")
        if not os.path.exists(r_level_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(r_level_path), code="52501098")
        if not os.path.exists(n_level_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(n_level_path), code="52501099")
        data_list = []
        path_def = {}
        r_path_list = {}
        n_path_list = {}
        ...
        try:
            ...
            self.bind_object.set_error("导入kegg注释层级all信息出错：%s、%s" , variables=(r_level_path, n_level_path), code="525010100")
        else:
    ...
    def add_annotation_query(self, name=None, params=None, stat_id=None, result_dir=None):
        ...
        if not isinstance(stat_id, ObjectId):
            ...
            if isinstance(stat_id, types.StringTypes):
                ...
                self.bind_object.set_error('stat_id必须为ObjectId对象或其对应的字符串！', code="525010101")
        insert_data = {
            'project_sn': project_sn,
            'task_id': task_id,
    ...
    def add_annotation_query_denovo_detail(self, query_id, query_path, anno_type):
        if not isinstance(query_id, ObjectId):
            if isinstance(query_id, types.StringTypes):
                ...
                self.bind_object.set_error('query_id必须为ObjectId对象或其对应的字符串！', code="525010102")
        if not os.path.exists(query_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(query_path), code="525010103")
        data_list = []
        with open(query_path, 'r') as f:
            ...
            for line in lines[1:]:
                ...
                try:
                    data.append(('description', line[1]))
                except:
        ...
        try:
            ...
            self.bind_object.set_error("导入转录本注释统计信息：%s出错!" , variables=(query_path), code="525010104")
        else:
    ...
    def add_annotation_query_detail(self, query_id, query_path, anno_type):
        if not isinstance(query_id, ObjectId):
            if isinstance(query_id, types.StringTypes):
                ...
                self.bind_object.set_error('query_id必须为ObjectId对象或其对应的字符串！', code="525010105")
        if not os.path.exists(query_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(query_path), code="525010106")
        data_list = []
        with open(query_path, 'r') as f:
            lines = f.readlines()
            for line in lines[1:]:
        ...
        try:
            ...
            self.bind_object.set_error("导入转录本注释统计信息：%s出错!" , variables=(query_path), code="525010107")
        else:
            self.bind_object.logger.info("导入转录本注释统计信息：%s成功!" % (query_path))
    ...
    def add_annotation_gene_query_detail(self, query_id, query_path, anno_type):
        if not isinstance(query_id, ObjectId):
            if isinstance(query_id, types.StringTypes):
                ...
                self.bind_object.set_error('query_id必须为ObjectId对象或其对应的字符串！', code="525010108")
        if not os.path.exists(query_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(query_path), code="525010109")
        data_list = []
        with open(query_path, 'r') as f:
            lines = f.readlines()
        ...
        try:
            ...
            self.bind_object.set_error("导入基因注释查询信息：%s出错!" , variables=(query_path), code="525010110")
        else:
    ...
    def add_annotation_gene_query_denovo_detail(self, query_id, query_path, anno_type):
        if not isinstance(query_id, ObjectId):
            if isinstance(query_id, types.StringTypes):
                ...
                self.bind_object.set_error('query_id必须为ObjectId对象或其对应的字符串！', code="525010111")
        if not os.path.exists(query_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(query_path), code="525010112")
        data_list = []
        with open(query_path, 'r') as f:
            lines = f.readlines()
            for j in range(1, len(lines)):
        ...
        try:
            ...
            self.bind_object.set_error("导入基因注释查询信息：%s出错!" , variables=(query_path), code="525010113")
        else:
            self.bind_object.logger.info("导入基因注释统计信息：%s成功!" % (query_path))
...
class TestFunction(unittest.TestCase):
    ...
    def test_mongo(test):
        ...
        wf.test_api = wf.api.api("itraq_and_tmt.protein_annotation")
        params = {
            "go_evalue": 1e-3,
            "cog_evalue": 1e-3,
            "kegg_evalue": 1e-3,
