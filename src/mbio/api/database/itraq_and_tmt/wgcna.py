...
class Wgcna(ApiBase):
    ...
    def add_prepare_detail(self, prepare_result_dir, main_id):
        ...
        self.update_db_record('sg_wgcna_prepare', main_id, status="end", main_id=ObjectId(main_id))
    ...
    def add_module_detail(self, result_dir, protein_id2protein_name, exp_matrix, main_id):
        """
        模块识别结果入库
        :param result_dir:
        :param main_id:
        :return:
        """
        # protein_id2protein_name
        g2n = pd.read_table(protein_id2protein_name, header=0, index_col=0)
        ...
        self.bind_object.logger.info(size_stat_dict)
        self.bind_object.logger.info(self._project_type)
        ...
        membership_pd.to_csv(result_dir+"/protein_module_detail.xls", header=True, index=True, sep='\t')
        ...
        # sg_wgcna_module_eigenproteins_detail
        eigenproteins = glob.glob(result_dir + '/eigenproteins.txt')[0]
        eigenproteins_pd = pd.read_table(eigenproteins, header=0, index_col=0)
        module_list = list(eigenproteins_pd.index)
        sample_list = list(eigenproteins_pd.columns)
        eigenproteins_pd.index.name = 'module'
        eigenproteins_pd.reset_index(inplace=True)
        eigenproteins_dict_list = eigenproteins_pd.to_dict("records")
        eigenproteins_dict_list = self.order_row_dict_list(eigenproteins_dict_list, list(eigenproteins_pd.columns) + ['module'])
        self.create_db_table('sg_wgcna_module_eigenproteins_detail', eigenproteins_dict_list, tag_dict=dict(module_id=ObjectId(main_id)))

        # update main table
        self.update_db_record('sg_wgcna_module', main_id, status="end", main_id=ObjectId(main_id),
                              modules=module_list, samples=sample_list, block_num=len(blocks))
    ...
    def add_relate_detail(self, result_dir, seq_annot, main_id):
        ...
        # protein_trait.correlation.xls == protein significant detail
        protein_trait_corr = glob.glob(result_dir + '/protein_trait.correlation.xls')[0]
        corr_pd = pd.read_table(protein_trait_corr,index_col=0,header=0)
        ...
        self.create_db_table('sg_wgcna_relate_protein_detail',corr_all_dict_list)

        # get protein significant stat
        ...
        # update main table
        self.update_db_record('sg_wgcna_relate', main_id, status="end", modules=module_list,
                              traits=trait_list, main_id=ObjectId(main_id),)
    ...
    def add_network_detail(self, result_dir, main_id):
        ...
        # update main table
        self.update_db_record('sg_wgcna_network', main_id, status="end", main_id=ObjectId(main_id),)
