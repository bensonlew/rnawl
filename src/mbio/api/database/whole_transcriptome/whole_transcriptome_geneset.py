...
class WholeTranscriptomeGeneset(ApiBase):
    def __init__(self, bind_object):
        super(WholeTranscriptomeGeneset, self).__init__(bind_object)
    ...
    def add_go_enrich_detail(self, go_enrich_id, go_enrich_dir):
        ...
        if data_list:
            ...
            try:
                collection = self.db['geneset_go_enrich_detail']
                collection.insert_many(data_list)
                coll = self.db['geneset_go_enrich']
                go_type = list(set(go_type))
                coll.update({'_id': go_enrich_id}, {'$set': {'categories': go_type}})
                # main_collection = self.db['geneset_go_enrich']
    ...
    def update_directed_graph(self, go_enrich_id, go_graph_png, go_graph_pdf):
        collection = self.db['geneset_go_enrich']
    ...
    def add_kegg_regulate_pathway(self, pathway_dir, regulate_id):
        ...
        for f in files:
            ...
            data_list.append(insert_data)
        try:
            collection = self.db['geneset_kegg_class_pathway']
            collection.insert_many(data_list)
        except Exception as e:
    ...
    def add_kegg_regulate_detail(self, regulate_id, kegg_regulate_table):
        ...
        if not os.path.exists(kegg_regulate_table):
            self.bind_object.set_error('kegg_regulate_table所指定的路径:%s不存在，请检查！', variables=(kegg_regulate_table),
                                       code="53702352")
        data_list = []
        with open(kegg_regulate_table, 'rb') as r:
            ...
            for line in r:
                ...
                data_list.append(insert_data)
            try:
                collection = self.db['geneset_kegg_class_detail']
    ...
    def add_kegg_regulate_new(self, main_table_id, geneset_id, kegg_stat_xls, gene_kegg_level_table_xls, work_dir,
                              geneset_type, geneset_dir, source=None):
        ...
        # 20191202 为了区分特殊情况(基因集来源于差异但只有up/down的情况)
        totalset = 1
        if is_diff is False and genesets_num == 1:
            totalset = 1
        if is_diff is True:
            with open(geneset_dir) as gd:
                ts = gd.readlines()
                if len(ts[0].strip().split("\t")) != len(ts[1].strip().split("\t")):
                    totalset = 1
                else:
                    totalset = 2
        if genesets_num == 2:
        ...
        if totalset == 1:
            ...
            if is_diff is True:
                ...
                new_data_rkaa[total_geneset_name+"_geneko"] = new_data_rkaa.apply(lambda x:x[raw_geneset_name+"_up_geneko"]+x[raw_geneset_name+"_down_geneko"],axis=1)
    ...
    def add_kegg_enrich_stat(self, main_table_id, geneset_id, kegg_stat_xls, gene_kegg_level_table_xls, work_dir,
        ...
        if not os.path.exists(gene_kegg_level_table_xls):
            self.bind_object.set_error('gene_kegg_level_table_xls所指定的路径:%s不存在，请检查！',
                                       variables=(gene_kegg_level_table_xls), code="53702360")
    ...
    def add_genesetkegg_enrich_stat(self, main_id, work_dir):
        ...
        self.create_db_table('geneset_kegg_enrich_stat_detail', statis_new,
                             tag_dict={'kegg_enrich_stat_id': ObjectId(main_id)})
        self.bind_object.logger.info("导入kegg富集统计信息成功")
        self.update_db_record('geneset_kegg_enrich_stat', main_id, status="end", main_id=ObjectId(main_id))
        ...
        df_short = pd.merge(df_b, df_control, on="first_category")
        categories = list(df_short['categories'])
        main_collection = self.db['geneset_kegg_enrich_stat']
    ...
    # kegg_stat_xls是kegg_class这个tool产生的，也是通过这个文件来更新geneset_kegg_class这个主表的table_columns字段
    def add_ipath_detail(self, main_table_id, ipath_input, geneset):
        ...
        main_collection = self.db['geneset_ipath']
        ...
        try:
            self.create_db_table('geneset_ipath_detail', row_dict_list)
    ...
    def add_circ_detail(self, main_table_id, circ_input, enrich_type):
        ...
        main_collection = self.db['geneset_circ']

        try:
            self.create_db_table('geneset_circ_detail', row_dict_list)
        except Exception as e:
    ...
    def add_circ_graph(self, main_table_id, circ_input, enrich_type):
        ...
        try:
            self.create_db_table('geneset_circ_graph', row_dict_list)
        except Exception as e:
    ...
    def run1(self):
        self.add_circ_graph("5dd1fd7e17b2bf71838dc820",
                            "/mnt/ilustre/users/sanger-dev/workspace/20191118/GenesetCirc_tsg_36174_7031_786/Enrich2circ/output/go_enrich_choose.table",
                            "GO")
        self.add_circ_detail("5dd1fd7e17b2bf71838dc820",
                             "/mnt/ilustre/users/sanger-dev/workspace/20191118/GenesetCirc_tsg_36174_7031_786/Enrich2circ/output/go_enrich_detail.table",
                             "GO")
        self.update_circ_main("5dd1fd7e17b2bf71838dc820",
                              "/mnt/ilustre/users/sanger-dev/workspace/20191118/GenesetCirc_tsg_36174_7031_786/Enrich2circ/output/enrich_zscore",
                              "GO")
...
class TestFunction(unittest.TestCase):
    ...
    def test_mongo(test):
        ...
        wf.test_api = wf.api.api("whole_transcriptome.whole_transcriptome_geneset")
        wf.test_api.run1()
