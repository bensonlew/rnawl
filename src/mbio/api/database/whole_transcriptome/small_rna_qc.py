...
class SmallRnaQc(ApiBase):
    ...
    def add_before_qc(self, qc_stat, fq_type='se', about_qc='before'):
        """
        :param qc_stat: 统计结果文件夹，即module.output_dir
        """
        self.add_samples_info(qc_stat, fq_type=fq_type, about_qc=about_qc)
        quality_stat_before = qc_stat + "/qualityStat"
        self.add_gragh_info(quality_stat_before, "before")   # 导表时会自动导入add_graph_info新息。


    def add_after_qc(self, qc_stat, qc_dir=None,  fq_type='se', about_qc='after'):
        self.add_samples_info(qc_stat, fq_type=fq_type, qc_dir=qc_dir, about_qc=about_qc)
        quality_stat_after = qc_stat + "/qualityStat"
        self.add_gragh_info(quality_stat_after, "after")
        qc_id = self.add_qc()
        self.add_qc_detail(qc_id, qc_stat, qc_dir)
        self.update_db_record('qc', qc_id, status="end", main_id=qc_id)

    @report_check
    def add_samples_info(self, qc_stat, qc_adapt=None, qc_dir=None, fq_type='se', about_qc='before'):
        """
        :param qc_stat: 统计结果文件夹，即module.output_dir
        ...
        :return:
        """
        stat_file = qc_stat + "/fastq_stat.xls"   # 生成fastq_stat的路径
        '''
        ...
        '''
        adapter = False
        if not os.path.exists(stat_file):  # 如果不存在该fastq_stat的路径
            raise Exception("%s文件不存在" % stat_file)   # 报错
        if qc_adapt is not None:
            adapt_rate = {}
            adapter = True
            with open(qc_adapt, "r") as a:
                for line in a:
                    line = line.split()  # 默认为制表符
        ...
        for i in result.inserted_ids:
            self.update_db_record('specimen', i, status="end", main_id=i)

        sample_ids = [str(i) for i in result.inserted_ids]
        return sorted(sample_ids)

    @report_check #判断在何时导表的修饰符
    def add_qc(self, name=None):  #质控长度统计
        task_id = self.bind_object.sheet.id
        project_sn = self.bind_object.sheet.project_sn
        ...
        insert_data = {
            ...
            'task_id': task_id,
            "library": "long",
            'name': name if name else 'qcgthStat_' + str(datetime.datetime.now().strftime("%Y%m%d_%H%M%S")),
            'params': "",
        ...
        self.bind_object.logger.info("add qc")
        return qc_id

    @report_check   #判断在何时导表的修饰符
    def add_qc_detail(self, qc_id, qc_stat, qc_dir):
        if not isinstance(qc_id, ObjectId):
            if isinstance(qc_id, types.StringTypes):
                qc_id = ObjectId(qc_id)
            else:
                raise Exception('qc_id必须为ObjectId对象或其对应的字符串！')
    ...
    def add_gragh_info(self, quality_stat, about_qc="before"):
        ...
        :return:
        """
        stat_files = glob.glob("{}/*".format(quality_stat))  # 获得指定路径的文件
        data_list = []
    ...
    def get_spname_spid(self):
        if not self.sample_ids:
            raise Exception("样本id列表为空，请先调用add_samples_info产生speciem的id")
        collection = self.db["specimen"]
        spname_spid = {}
    ...
    def add_control_group(self,file, group_id):
        ...
        col = self.db["specimen_group_compare"]
        try:
...
@report_check  # 将bam 文件路径出入specimen
def add_bam_path(self, dir_path):
    """
    将bam文件的路径插入specimen表中，供可变剪切使用
...
class TestFunction(unittest.TestCase):
    ...
    def test_mongo(test):
        ...
        wf.api_qc = wf.api.api("whole_transcriptome.small_rna_qc")

        # test_dir = "/mnt/ilustre/users/sanger-dev/sg-users/liubinxu/test_small_RNA/data_hsa/"
        #
        # after_qc_stat = test_dir + "before_qc"    #  hiseq_reads_stats(use) 的结果文件夹
        # before_qc_stat = test_dir + "before_qc"  #  hiseq_reads_stats(raw) 的结果文件夹
        # quality_stat_after = after_qc_stat + "/qualityStat"     # after 文件
        # quality_stat_before = before_qc_stat + "/qualityStat"  # before 文件
        # qc_dir = after_qc_stat + "qc"
        qc_dir = "/mnt/ilustre/users/sanger-dev/workspace/20190926/Single_whole_transcriptome.smallrna.mirna_qc15-22-40/MirnaQc/output/clean_data"
        # qc_dir： 为质控结果文件路径
        fq_type = "SE"  # 一般为单端测序
        before_qc_stat = "/mnt/ilustre/users/sanger-dev/workspace/20190928/Single_HiseqReadsStat_smallrna_raw6066/HiseqReadsStat/output" # raw数据统计结果
        after_qc_stat = "/mnt/ilustre/users/sanger-dev/workspace/20190928/Single_hiseq_reads_stat_smallrna_2151_9340/HiseqReadsStat/output" # use(qc)数据统计结果
        ...
        wf.group_id, wf.group_detail, wf.group_category = wf.api_qc.add_specimen_group("/mnt/ilustre/users/sanger-dev/sg-users/shicaiping/whole_transcriptome/demo_human/group.txt")
        wf.logger.info("group_detail为：" + str(wf.group_detail))
        wf.logger.info("group_id为：" + str(wf.group_id))
        # wf.control_id, compare_detail = wf.api_qc.add_control_group(wf.option("control_file").prop["path"], wf.group_id)
        wf.control_id, compare_detail = wf.api_qc.add_control_group("/mnt/ilustre/users/sanger-dev/sg-users/shicaiping/whole_transcriptome/demo_human/control.txt", wf.group_id)
        wf.compare_detail = compare_detail
        wf.logger.info("compare_detail为：" + str(wf.compare_detail))
...
class TestFunction(unittest.TestCase):
    ...
    def test_mongo2(test):
        ...
        wf.api_qc.add_after_qc(after_qc_stat, qc_dir=qc_dir, fq_type=fq_type, about_qc="after")
        # wf.group_id, wf.group_detail, wf.group_category = wf.api_qc.add_specimen_group(wf.option("group_table").prop["path"])
        wf.group_id, wf.group_detail, wf.group_category = wf.api_qc.add_specimen_group(
            "/mnt/ilustre/users/sanger-dev/sg-users/shicaiping/whole_transcriptome/demo_human/group.txt")
        wf.logger.info("group_detail为：" + str(wf.group_detail))
        wf.logger.info("group_id为：" + str(wf.group_id))
        # wf.control_id, compare_detail = wf.api_qc.add_control_group(wf.option("control_file").prop["path"], wf.group_id)
        wf.control_id, compare_detail = wf.api_qc.add_control_group(
            "/mnt/ilustre/users/sanger-dev/sg-users/shicaiping/whole_transcriptome/demo_human/control.txt", wf.group_id)
        wf.compare_detail = compare_detail
        wf.logger.info("compare_detail为：" + str(wf.compare_detail))
        # wf.api_qc.add_bam_path(wf.workflow_output)
...
if __name__ == '__main__':
    ...
    suite.addTests([TestFunction('test_mongo2')])
