...
class MirnaEdit(ApiBase):
    ...
    @report_check
    def add_mirna_edit(self, result_dir, task_id, project_sn, params=None, is_novel=False):
        ...
        # insert one document to mirna_edit, return _id as main_id
        main_info = {
            # essential keys
            'task_id': task_id,
        ...
        main_id = self.create_db_table('mirna_edit', [main_info])
        self.update_db_record('mirna_edit', main_id, main_id=main_id)
        self.bind_object.logger.info('succeed in creating document in mirna_edit')
        ...
        # create a new document in mirna_edit_detail
        ...
        self.create_db_table('mirna_edit_detail', detail_dict_list, tag_dict={'edit_id': main_id})

        # create a new document in mirna_edit_type
        type_pd = type_pd.reset_index()
        type_dict_list = type_pd.to_dict('records')
        self.create_db_table('mirna_edit_type', type_dict_list, tag_dict={'edit_id': main_id})

        # update mirna_edit
        self.update_db_record('mirna_edit', record_id=main_id, status='end')
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        wf.test_api = wf.api.api('whole_transcriptome.mirna_edit')
        result_dir = '/mnt/ilustre/users/sanger-dev/workspace/20191016/Single_mirna_edit_8709_6071/MirnaEdit/output'
