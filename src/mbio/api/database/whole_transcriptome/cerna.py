...
class Cerna(ApiBase):
    ...
    def import_net(self, result_dir=None, params=None, mi2mrna=None, mi2lncrna=None, ce_corr=None, nodes=None, edges=None):
        ...
        self.remove_table_by_main_record(main_table='cerna', task_id=task_id, detail_table=['cerna_mi2mrna', 'cerna_mi2lncrna', 'cerna_cerna', 'cerna_nodes', 'cerna_edges'], detail_table_key='cerna_id')
        cerna_id = self.add_cerna_main(result_dir, params)
        self.add_cerna_mi2ce(cerna_id, mi2mrna)
        self.add_cerna(cerna_id, ce_corr)
        self.add_cerna_nodes(cerna_id, nodes)
        self.add_cerna_edges(cerna_id, edges)
        self.db['cerna'].update({"_id": cerna_id},
    ...
    def import_cerna_sankey(self, main_id=None, sankey_file=None):
        task_id = self.bind_object.sheet.id
        self.bind_object.logger.info("开始导入cerna sankey 图")
        cerna_sankey_id = ObjectId(main_id)
        self.add_cerna_sankey(cerna_sankey_id, sankey_file)

        self.db['cerna_sankey'].update({"_id": cerna_sankey_id},
                                       {"$set": {"status": "end", "main_id": cerna_sankey_id}})

    @report_check
    def add_cerna_sankey(self, sankey_id, sankey_file):
        if not isinstance(sankey_id, ObjectId):
            if isinstance(sankey_id, StringTypes):
                sankey_id = ObjectId(sankey_id)
            else:
                raise Exception("sankey_id必须为ObjectId对象或者其对应的字符串！")
        data_list = []
        with open(sankey_file, "rb") as sankey_f:
            head_line = sankey_f.readline()
            header = head_line.strip("\n").split("\t")
            for line in sankey_f:
                cols = line.strip("\n").split("\t")
                data = zip(header, cols)
                data.append(('sankey_id', sankey_id))
                data = SON(data)
                data_list.append(data)

        try:
            self.create_db_table('cerna_sankey_detail', data_list)
        except Exception as e:
            self.bind_object.set_error("导入cerna_sankey信息出错!")
        else:
            self.bind_object.logger.info("导入cerna_sankey信息成功!")
    ...
    @report_check
    def add_cerna_mi2ce(self, cerna_id, mi2mrna):
        '''
        导入miRNA与靶基因相关性信息
        '''
        ...
        try:
            self.create_db_table('cerna_mi2mrna', data_list)
        except Exception as e:
    ...
    def add_cerna(self, cerna_id, cerna):
        ...
        try:
            self.create_db_table('cerna_cerna', data_list)
        except Exception as e:
    ...
    def add_cerna_nodes(self, cerna_id, nodes):
        ...
        try:
            self.create_db_table('cerna_nodes', data_list)
        except Exception as e:
    ...
    def add_cerna_edges(self, cerna_id, edges):
        ...
        with open(edges, "rb") as edges_f:
            ...
            for line in edges_f:
                cols = line.strip("\n").split("\t")
                cols[9] = int(cols[9])
                cols[10] = cols[10].split(",")
                cols[11] = float(cols[11])
                cols[13] = abs(float(cols[13]))
                cols[14] = abs(float(cols[14]))
                cols[15] = abs(float(cols[15]))
                cols[16] = cols[16]
                cols[17] = abs(float(cols[17]))
                cols[18] = cols[18]
        ...
        try:
            self.create_db_table('cerna_edges', data_list)
        except Exception as e:
