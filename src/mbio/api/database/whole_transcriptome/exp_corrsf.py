...
class ExpCorrsf(ApiBase):
    ...
    def add_ExpCorrsf(self, upload_dir, work_dir, main_id=None, corr_way=None,
        ...
        if main_id is None:
            ...
            main_id = self.create_db_table('exp_corrsf', [main_info])
        ...
        if df_exp.shape[0] == 0:
            self.update_db_record('exp_corrsf',main_id,in_null="yes")
        df_exp = df_exp.round(6)
        df_exp['expcorr_id'] = main_id
        df_exp_dict = df_exp.to_dict("records")
        self.create_db_table('exp_corrsf_detail', df_exp_dict)
        self.update_db_record('exp_corrsf', main_id, status="end",
