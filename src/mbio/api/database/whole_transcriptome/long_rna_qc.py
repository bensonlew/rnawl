...
class LongRnaQc(ApiBase):
    def __init__(self, bind_object):
        super(LongRnaQc, self).__init__(bind_object)
    ...
    def get_spname_spid(self):
        if not self.sample_ids:
            self.bind_object.set_error("样本id列表为空，请先调用add_samples_info产生speciem的id", code="53702006")
        collection = self.db["specimen"]
        spname_spid = {}
    ...
    def add_mapping_stat(self, stat_file, type="genome"):
        ...
        with open(stat_file, "r") as f:
            ...
            for line in f:
                ...
                data_list.append(data)
        try:
            collection = self.db["specimen_mapping"]
            result = collection.insert_many(data_list)
            for i in result.inserted_ids:
                self.update_db_record('specimen_mapping', i, status="end", main_id=i)
        except Exception, e:
    ...
    def add_rpkm_curve(self, rpkm_file, rpkm_id=None):
        ...
        try:
            collection = self.db["assessment_saturation_curve"]
    ...
    @report_check
    def add_chrom_distribution_detail(self, chorm_stat, chorm_stat_id):
    ...
    def run3(self):
        self.add_samples_info("/mnt/ilustre/users/sanger-dev/workspace/20190926/Single_HiseqReadsStat_raw8242/HiseqReadsStat/output",
            fq_type="PE", about_qc="before")
        self.add_gragh_info(
            "/mnt/ilustre/users/sanger-dev/workspace/20190926/Single_HiseqReadsStat_raw8242/HiseqReadsStat/output/qualityStat",
            about_qc="before")
        self.add_samples_info(
            "/mnt/ilustre/users/sanger-dev/workspace/20190926/Single_hiseq_reads_stat_7717_3415/HiseqReadsStat/output",
            fq_type="PE", about_qc="after")
        self.add_gragh_info(
            "/mnt/ilustre/users/sanger-dev/workspace/20190926/Single_hiseq_reads_stat_7717_3415/HiseqReadsStat/output/qualityStat",
            about_qc="after")
        ...
        self.add_control_group(
            "/mnt/ilustre/users/sanger-dev/sg-users/shicaiping/whole_transcriptome/demo_human/control.txt", self.group_id)
        #self.add_hisat2_mapping_stat(
...
class TestFunction(unittest.TestCase):
    ...
    def test_mongo(test):
      ...
      wf.test_api = wf.api.api("whole_transcriptome.long_rna_qc")
      wf.test_api.run3()
