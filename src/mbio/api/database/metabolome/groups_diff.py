...
class GroupsDiff(Base):
    ...
    def add_groups_diff(self, name=None, params=None, metab_table_id=None):
        ...
        collection = self.db["groups_diff"]
        inserted_id = collection.insert_one(insert_data).inserted_id
        collection.update_one({'_id': inserted_id}, {'$set': {'main_id': inserted_id}})
        return inserted_id

    @report_check
    def add_groups_diff_detail(self, result_path, main_id, table_type=None):
        ...
        try:
            collection = self.db["groups_diff_detail"]
            collection.insert_many(data_list)
            main_collection = self.db["groups_diff"]
            ...
            self.bind_object.logger.info("导入groups_diff_detail数据出错:%s" % e)
        else:
            self.bind_object.logger.info("导入groups_diff_detail数据成功")
        ...
        try:
            collection = self.db["groups_diff_relation"]
            collection.insert_many(insert_pvalue)
        except Exception as e:
            self.bind_object.logger.info("导入groups_diff_relation数据出错:%s" % e)
        else:
            self.bind_object.logger.info("导入groups_diff_relation数据成功")