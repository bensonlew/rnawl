...
class RelationProcrustes(Base):
    def __init__(self, bind_object):
        super(RelationProcrustes, self).__init__(bind_object)
        self._project_type = "metabolome"
    
    def add_procrustes(self, main_id=None, name=None, project_sn='metabolome', task_id='metabolome', params=None):
        if main_id is None:
            name = "relation_procrustes" + "_"
            ...
            main_info = dict(
                ...
                task_id=task_id,
                version="v1",
                name=name if name else 'RelationProcrustes',
                created_ts=time_now.strftime('%Y-%m-%d %H:%M:%S'),
                desc='Relation_procrustes',
                params=params,
                status="start")
            collection = self.db['relation_procrustes']
            main_id = collection.insert_one(main_info).inserted_id
            self.update_table("main_id", main_id, main_id)
        else:
            main_id = ObjectId(main_id)
            try:
                self.update_db_record('relation_procrustes', main_id)
            except Exception as e:
                self.bind_object.logger.error("导入relation_procrustes数据出错:%s" % e)
            else:
                self.bind_object.logger.info("导入relation_procrustes数据成功")
        return main_id    
    ...
    def add_procrustes_detail(self, summaryfile, main_id):
        ...
        try:
            collection = self.db["relation_procrustes_detail"]
    ...
    def add_procrustes_graph(self, ref, query, main_id):
        ...
        try:
            collection = self.db["relation_procrustes_graph"]
    ...
    def update_table(self, str, name, main_table_id):
        try:
            self.db['relation_procrustes'].update_one({'_id': ObjectId(main_table_id)}, {'$set': {str: name}})
        except Exception as e:
            self.bind_object.set_error('relation_procrustes%s字段出错:%s', variables=(str,e), code="54700109")
