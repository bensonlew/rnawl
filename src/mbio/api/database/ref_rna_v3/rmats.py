...
class Rmats(ApiBase):
    ...
    def add_sg_splicing_rmats(self, params, outpath):
        ...
        result_dir = os.path.join(self.bind_object.sheet.output, '09AS', os.path.basename(outpath))
        rmats_output = os.path.join(self.bind_object.sheet.output.replace('/workflow_results', '/intermediate_results'),
                                    'AS', os.path.basename(outpath))
        insert_data = {
            'task_id': task_id,
        ...
        self.add_sg_splicing_rmats_detail(splicing_id=splicing_id, outpath=outpath)
