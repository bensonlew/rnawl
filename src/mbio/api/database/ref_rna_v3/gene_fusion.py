...
class GeneFusion(ApiBase):
    ...
    def add_fusion_main(self, fusion_result=None,pos_file= None,chr_length=None, circos = None ,project_sn=None, task_id=None, main_id=None,params=None, method_type=None, new_output=None):
        """
        导入gene_fusion主表信息
        :return:
        """
        ...
        if fusion_result:
            if circos == 1:
                ...
                sample_list = self.add_fusion_detail(fusion_result=fusion_result, fusion_id = fusion_id, pos_file =pos_file, circos = circos,chr_length=chr_length)
                self.update_db_record('sg_gene_fusion', fusion_id, status="end",sample =sample_list,
                                      main_id=fusion_id, ciros="true",)
            else:
                sample_list = self.add_fusion_detail(fusion_result=fusion_result,fusion_id = fusion_id, circos = circos)
                self.update_db_record('sg_gene_fusion', fusion_id, status="end", sample=sample_list,
                                      main_id=fusion_id)

        return fusion_id

    def add_fusion_detail(self,fusion_result=None,fusion_id=None,pos_file=None,circos=None,chr_length=None):
        """
            导入Gene_fusion详情表的函数
        ...
        for i in result_dir:
            if i == "fusion_stat.txt":
                ...
                self.add_fusion_stat(fusion_stat, fusion_id)
            elif i == "star_fusion":
    ...
    def add_circos_detail(self,sample_name,select_df,pos_file,fusion_id,chr_length=None):
        ...
        for index,item in circos_select_df.iterrows():
            ...
            left_area_loc = int(math.ceil(left_gene_loc / chr_dict[left_gene_chr]["chr_length"] * 10))
            # left_detail_info=[(left_area_loc-1)*chr_dict[left_gene_chr]["chr_length"]/100,left_gene_loc,left_gene_name]
            left_detail_info = [(left_area_loc - 1) * chr_dict[left_gene_chr]["chr_length"] / 10, left_gene_loc,
                                left_gene_name]
            # tree_dict[left_gene_chr+"_"+str(left_area_loc)].append(left_detail_info)
            if left_detail_info not in tree_dict[left_gene_chr+"_"+str(left_area_loc)]:
                tree_dict[left_gene_chr + "_" + str(left_area_loc)].append(left_detail_info)

            right_gene_name = item["fusionname"].split("-")[-1]
            right_gene_chr = item["right_chr"]
            right_gene_loc = item["right_local"]
            right_area_loc = int(math.ceil(right_gene_loc / chr_dict[right_gene_chr]["chr_length"] * 10))
            right_detail_info = [(right_area_loc - 1) * chr_dict[right_gene_chr]["chr_length"] / 10, right_gene_loc, right_gene_name]
            # right_area_loc = int(math.ceil(right_gene_loc / chr_dict[right_gene_chr]["chr_length"] * 100))
            # right_detail_info = [(right_area_loc - 1) * chr_dict[right_gene_chr]["chr_length"] / 100, right_gene_loc,
            #                      right_gene_name]
            # tree_dict[right_gene_chr + "_"+str(right_area_loc)].append(right_detail_info)

            if right_detail_info not in tree_dict[right_gene_chr + "_"+str(right_area_loc)]:
                tree_dict[right_gene_chr + "_" + str(right_area_loc)].append(right_detail_info)
    ...
    def add_fusion_stat(self,fusion_stat,fusion_id):
    ...
    def run(self):
        fusion_result="/mnt/ilustre/users/sanger-dev/workspace/20200805/GeneFusion_tsg_38132_6455_5036/GeneFusion/output"
        pos_file = "/mnt/ilustre/users/sanger-dev/workspace/20200805/GeneFusion_tsg_38132_6455_5036/gene_pos"
        chr_length = "/mnt/ilustre/users/sanger-dev/workspace/20200805/GeneFusion_tsg_38132_6455_5036/chr_length"
        ...
        self.add_fusion_main(fusion_result=fusion_result, pos_file=pos_file,chr_length=chr_length,
                          task_id="test10", project_sn="test10",circos=1)


    def add_gene_fusion_venn(self, venn_graph, venn_table=None, project_sn='ref_rna_v2', main_id=None,
                      task_id='ref_rna_v2', params=None):
        """
    ...
    def run2(self):
        fusion_venn_resut="/mnt/ilustre/users/sanger-dev/workspace/20200615/FusionVenn_tsg_37575_6440_5440/fusion_venn"
        params = dict(
            task_id="medical_transcriptome",
            submit_location="genefusionvenn",
            task_type=int(2),
            use_group="yes",
            group_dict={
            "A" : [
            ...
            fusion_id="5f3dd4fa17b2bf012d63160e",
            group_id="5f3703a617b2bf7a2bafdcb2"
        )
...
class TestFunction(unittest.TestCase):
    ...
    def test_mongo(test):
      ...
      wf.test_api = wf.api.api("ref_rna_v3.gene_fusion")
      wf.test_api.run()
