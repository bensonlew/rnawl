...
class RefSnp(ApiBase):
    ...
    def add_snp_main(self, snp_anno=None, group=None, project_sn=None, task_id=None, main_id=None,params=None, method_type=None, new_output=None):
        """
        导入SNP主表信息
    ...
    def add_snp_detail(self, snp_anno, group, snp_id, new_output=None):
        ...
        with open(os.path.join(snp_anno, "snp_annotation_statistics.xls"),"r")as all_info,open(os.path.join(new_output,"indel_anno.xls"),"w") as ide,open(os.path.join(new_output,"snp_anno.xls"),"w") as sp:
    ...
    def run(self):
        new_snp_rewrite = "/mnt/ilustre/users/sanger-dev/i-sanger_workspace/20191118/Refrna_majorbio_220283/CallSnpIndel/output/predeal"
        self.add_snp_main(snp_anno=new_snp_rewrite, method_type="sentieon",
                          task_id="test_1119", project_sn="test_1119",new_output="/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/test/snp/")
...
class TestFunction(unittest.TestCase):
    ...
    def test_mongo(test):
      ...
      wf.test_api = wf.api.api("ref_rna_v3.ref_snp")
      wf.test_api.run()
