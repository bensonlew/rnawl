...
class Multiple(Base):
    def __init__(self, bind_object):
        super(Multiple, self).__init__(bind_object)
        ...
        self.boxfile_path = self.output_dir + '/' + self.bind_object._task.option("test") + '_boxfile.xls'
        self.bar_path = self.bind_object._task.multiple.work_dir + '/' + self.bind_object._task.option(
    ...
    def table_in(self):
		...
		导入表格相关信息
		"""
        sample_list = self.insert_table(self.stat_path, self.cifiles, '组间差异显著性结果表',
                                        '多组间差异性检验', posthoc=self.bind_object._task.option("methor"))
        return sample_list
    ...
    def insert_table(self, statfile, cifiles, name, desc, posthoc=None, group=None):
        ...
        table_id = self.db['table'].insert_one(SON(
            ...
            graphic='bar,box,difference_bar'
        )).inserted_id

        self.db['multi_bar'].insert_one(SON(
            _id=table_id,
            project_sn=self.bind_object.sheet.project_sn,
            task_id=self.bind_object.id,
            name=name,
        ...
        for name in sort_list:
            ...
            for i in stat_info[name].keys():
                if i in ['pvalue', 'qvalue', 'corrected_pvalue'] and stat_info[name][i] == 'NA':
                    ...
                    if stat_info[name][i] == 'NA':         #guanqing.zou 20181017
                        data.append((i, stat_info[name][i]))
                        data_bar.append((i, stat_info[name][i]))
                    else:
        ...
        try:
            ...
            self.db['multi_bar_detail'].insert_many(data_list_bar)
        except Exception, e:
        ...
        self.db['multi_bar'].update({"_id": table_id}, {'$set': {"attrs": attrs, 'status': 'end'}})
        return table_id
    ...
    def box_in(self, file_path):
        ...
        with open(file_path, 'rb') as r:
            ...
            while True:
                ...
                for name in group_list:
                    ...
                    data = [("box_id", box_plot_id), ("sample_name", species_name)]
                    data.append(("category_name", name))
                    data.append(("box_data", [float(line_data[i]), float(line_data[i + 1]), float(line_data[i + 2]),
                                              float(line_data[i + 3]), float(line_data[i + 4])]))
                    i += 5
                    data_son = SON(data)
                    data_list.append(data_son)
        try:
        ...
        self.db['box_plot'].update_one({'_id': box_plot_id},
