...
class Env(Base):
    ...
    @report_check
    def add_env_table(self, file_path, name_to_id, task_id=None, name=None, env_true={},names=None):
        if isinstance(name_to_id, types.DictType):
            pass
        else:
            self.bind_object.set_error('错误的name_to_id类型，或者没有提供', code="51002701")
        if not task_id:
        ...
        if env_true:
            main_insert_data['env_true_name'] = [env_true[n] for n in names]
        ...
        #main_collection.update({"_id": main_env_id}, {"$set": {"main_id": main_env_id}})
        ...
        return str(main_env_id),','.join(names)
        ...
        with open(file_path, 'rb') as f:
            ...
            for line in lines[1:]:
                ...
                if values[0] not in name_to_id:
                    self.bind_object.logger.info('分组文件或环境因子表中的样本：%s没有在fastq文件中找到' % values[0])
                    self.bind_object.set_error("fastq文件中没有找到样本", code="51002702")
