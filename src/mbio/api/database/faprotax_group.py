...
class FaprotaxGroup(Base):
    def __init__(self, bind_object):
        super(FaprotaxGroup, self).__init__(bind_object)
    ...
    @report_check
    def add_species_difference_check_detail(self, statfile, cifiles, check_type=None, params=None, category_name=None, table_id=None, group_id=None, from_otu_table=None, major=False, posthoc=None, correlation_key=None, coll_name='multiple_group_detail', main_coll='multiple_group',bar_name="faprotax_group_bar",task_type=None):
        if major:
        ...
        for name in sort_list:
            ...
            for i in stat_info[name].keys():
                if i in ['pvalue', 'qvalue', 'corrected_pvalue', 'odds_ratio'] and stat_info[name][i] == 'NA':
            ...
            if task_type in ["multiple_group"]:
                ...
                main_collection.update({"_id": table_id},{"$set": {"main_id": table_id,
                                                          ...
                                                          "table_data":table_data_json,
                                                           ...
                                                           "all_function":",".join(all_function)}})
    ...
    def add_species_difference_check_boxplot(self, file_path, table_id, correlation_key=None,
                                             coll_name='multiple_group_box', main_coll='multiple_group',task_type=None):
        if not isinstance(table_id, ObjectId):
            if isinstance(table_id, StringTypes):
                table_id = ObjectId(table_id)
            else:
