...
class Relaset(ApiBase):
    ...
    def add_relaset_cluster(self, cluster_output_dir, main_id=None, project_sn='itraq_tmt', task_id='itraq_tmt',
                            params=None):
    ...
    def add_relaset_corr(self, corr_output_dir, main_id=None, workflow_out=None):
        ...
        # if 'corr_result.xls' in results:
        #     corr_file = os.path.join(corr_output_dir, 'corr_result.xls')
        else:
            self.bind_object.set_error('找不到密度文件')
        # for file in results:
        #     if file == 'protein_rna_density_matrix.xls':
        #         density_file = os.path.join(corr_output_dir, file)
        #     elif file == 'corr_result.xls':
        #         corr_file = os.path.join(corr_output_dir, file)
        #     else:
        #         self.bind_object.set_error('找不到密度文件')
...
'protein_list', 'gene_num', 'gene_percent', 'gene_list']
                    ...
                    with open(png_dir + '/' + line[0] + '.html.mark', 'r') as mark_f:
                        for line_mark in mark_f.readlines():
                            ...
                            if len(line_mark.strip("\n").split("\t")) == 8:
                                ...
                                continue
                            # 由于html和pdf的版本不一样，经常会有不对应的问题，没有办法，只能强行一下了
                            if '(gene)' in title and '(protein)' in title:
                                bg_color = '#FFA500'
                                if len(bg_color) < 2:
                                    bg_color = "#0000CD,#FF0000"
                            elif '(gene)' in title:
                                if bg_color != '#FFA500':
                                    bg_color = '#FFC0CB'
    ...
    def add_enrich_cluster(self, cluster_output_dir, main_id=None, project_sn='itraq_tmt', task_id='itraq_tmt',
