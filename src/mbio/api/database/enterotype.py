...
class Enterotype(Base):
    ...
    def add_enterotype(self, main_id=None, main=False, task_id=None, params=None,
        ...
        _main_collection = self.db['sg_enterotype']
        if main:
            insert_mongo_json = {
            ...
            if not main_id:
                self.bind_object.set_error('不写入主表时，需要提供主表ID', code="51002501")
            if not isinstance(main_id, ObjectId):
                main_id = ObjectId(main_id)
        result = _main_collection.find_one({'_id': main_id})
        if not result:
            self.bind_object.set_error('找不到主表id对应的表', code="51002502")
    ...
    @report_check
    def add_enterotype_detail(self, file_path, table_type, update_id, coll_name='sg_enterotype_detail',
                              main_coll='sg_enterotype', update_column=True, db=None, all_cluster=None):
        """
        """
        self._tables.append(table_type)
        if not update_id:
            self.bind_object.set_error('需要提供主表ID',code="51002503")
        ...
        with open(file_path, 'rb') as f:
            ...
            if data_temp:
                ...
                return None
            if table_type in ['BCA_circle', 'pcoa_circle']:
                if all_cluster:
                    columns = all_cluster
        if update_column:
            ...
            if table_type in default_column:
                ...
                self.bind_object.set_error('错误的表格类型：%s不能在主表中插入相应表头', variables=(table_type), code="51002504")

    @report_check
    def add_enterotype_detail_cluster(self, id=None, file_path=None, name=None, update_column=True):
        insert_data = list()
        main_collection = self.db['sg_enterotype']
        ...
        with open(file_path, 'rb') as r:
            ...
            for line in all_lines[1:101]:
                ...
                otu_detail = dict()
                otu_detail['tax'] = line[0].split("; ")[-1]  # 保留最低层级的物种名称
        ...
        try:
            collection = self.db['sg_enterotype_detail_cluster']
            collection.insert_many(insert_data)
        except Exception as e:
            self.bind_object.logger.error("导入sg_enterotype_detail_cluster表格信息出错:{}".format(e))
        else:
            self.bind_object.logger.info("导入sg_enterotype_detail_cluster表格成功")
        if update_column:
