...
class Randomforest(Base):
    ...
    def add_randomforest_error(self, file_path, table_id=None, group_id=None, from_otu_table=None, level_id=None,
        ...
        if major:
            ...
            if not isinstance(table_id, ObjectId):
                ...
                self.bind_object.set_error("table_id必须为ObjectId对象或者其对应的字符串！", code="51005401")
        data_list = []
        data_list1 = []
        b1 = len(file_path)
        b2 = len(file_path[0])
    ...
    def add_randomforest_dim(self, file_path, table_id=None, group_id=None, from_otu_table=None, level_id=None,
        ...
        if major:
            ...
            if table_id is None:
                self.bind_object.set_error("major为False时需提供table_id!", code="51005403")
            if not isinstance(table_id, ObjectId):
                if isinstance(table_id, StringTypes):
                    table_id = ObjectId(table_id)
            else:
                self.bind_object.set_error("table_id必须为ObjectId对象或其对应的字符串!", code="51005401")
        data_list = []
        with open(file_path, 'rb') as r:
            i = 0
            for line in r:
                if i == 0:
                    ...
                    line = line.strip('\n')
                    line_data = line.split('\t')
                    data = [("randomforest_id", table_id), ("specimen_name", line_data[0]), ("x", line_data[1]),
                            ("y", line_data[2])]
                    data_son = SON(data)
                    data_list.append(data_son)
        try:
        ...
        with open(file_path, 'rb') as r:
            ...
            for line in r:
                if i == 0:
                    ...
                    data = [("randomforest_id", table_id), ("species_name", line_data[0]),
    ...
    def add_randomforest_evaluate(self, file_path, table_id=None, group_id=None, from_otu_table=None, level_id=None,
        ...
        with open(file_path, 'rb') as r:
            ...
            if len(head) == 2:
                ...
                data_list.append({"randomforest_id": table_id, "table_type": "plot", "data": data})
            else:
                prob = head[2:]
                for line in r:
                    lines = line.strip().split("\t")
        ...
        try:
            collection = self.db["sg_randomforest_evaluate"]
            collection.insert_many(data_list)
            if prob != "":
                main_collection = self.db["sg_randomforest"]
    ...
    def create_randomforest(self, params, group_id=0, from_otu_table=0, name=None, level_id=0):
        if from_otu_table != 0 and not isinstance(from_otu_table, ObjectId):
            if isinstance(from_otu_table, StringTypes):
                ...
                self.bind_object.set_error("from_otu_table必须为ObjectId对象或其对应的字符串!", code="51005404")
        if group_id != 0 and not isinstance(group_id, ObjectId):
            if isinstance(group_id, StringTypes):
                group_id = ObjectId(group_id)
            else:
                self.bind_object.set_error("group_detail必须为ObjectId对象或其对应的字符串!", code="51005405")
        if level_id not in range(1, 10):
            self.bind_object.logger.error("level参数%s为不在允许范围内!" % level_id)
            self.bind_object.set_error("level不在允许范围内", code="51005406")
