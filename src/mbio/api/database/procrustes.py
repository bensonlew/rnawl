...
class Procrustes(Base):
    ...
    def add_procrustes_detail(self, summaryfile, main_id):
        ...
        try:
            collection = self.db["sg_procrustes_detail"]
            collection.insert_many(data_list)
            main_collection = self.db["sg_procrustes"]
            ...
            self.bind_object.set_error("Error: failed to import procrusts_detail data: %s" , variables=( e), code="51008401")
        else:
    ...
    def add_procrustes_graph(self, ref, query, main_id):
        ...
        for sample in site_ref.keys():
            ...
            if len(site_ref[sample]) != len(site_query[sample]):
                ...
                self.bind_object.set_error("ERROR: site length of ref and query must be equal!", code="51008402")
        ...
        try:
            collection = self.db["sg_procrustes_graph"]
            ...
            self.bind_object.set_error("Error: failed to import procrusts_graph data: %s" , variables=( e), code="51008403")
        else:
