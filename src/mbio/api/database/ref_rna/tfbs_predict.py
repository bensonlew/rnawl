    ...
    def add_tfbs_predict_detail(self, output_dir, main_id):
        ...
        for each in result_file:
            ...
            if each.endswith("_predict.xls"):
                self.create_db_table('sg_tfbs_predict_detail', result_dict_list)
            elif each.endswith("tf_target_number.xls"):
                self.create_db_table('sg_tfbs_predict_stat_detail', result_dict_list)
        self.update_db_record('sg_tfbs_predict', main_id, status="end", )
