...
class GenePredict(Base):
    ...
    @report_check
    def add_gene_predict(self, file_path1, file_path2, file_path3,software, params=None):
    ...
    def add_gene_predict_detail(self, inserted_id, specimen_id, predict_gff, anno_pipeline=False):
        data_list = []
        suffix_nu = {}
        with open(predict_gff, "r") as f:
            ...
            for line in lines[1:]:
                ...
                if anno_pipeline:
                    data["location"] = '_'.join(line[1].split("_")[:-1])
                else:
                    data['cds_id'] = line[1].split("_")[1]
                    data["location"] = line[1].split("_")[0]
