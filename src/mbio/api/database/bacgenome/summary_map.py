...
class SummaryMap(Base):
    ...
    def add_map(self, task_id=None):
        ...
        for i in kegg_collection.find({"kegg_id": kegg_id},no_cursor_timeout=True).batch_size(5):
            gene_id = i['gene_id']
            ...
            summary_collection.update_one({'summary_id': sumamary_id, 'gene_id': gene_id, 'specimen_id': specimen_id},
                                          {'$set': {'map_id': map}},
                                          upsert=False)
        for i in cazy_collection.find({"cazy_id": cazy_id},no_cursor_timeout=True).batch_size(5):
            gene_id = i['gene_id']
            specimen_id = i['specimen_id']
            class_name = i['class']
            summary_collection.update_one({'summary_id': sumamary_id, 'gene_id': gene_id, 'specimen_id': specimen_id},
