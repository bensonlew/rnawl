...
class AnnoSignalp(Base):
    ...
    def add_anno_signalp(self, anno_signalp_path, main=False, task_id=None, project_sn=None, main_id=None,
                         params=None, name=None, specimen_id=None, type=None):
        if main:
            if task_id is None:
        ...
        if type == "bac":
            self.add_anno_signalp_detail(anno_signalp_path + '/' + specimen_id + '_Gram-_SignalP.txt',"Gram-", main_id)
        else:
            self.add_anno_signalp_detail(anno_signalp_path + '/' + specimen_id + '_Gram+_SignalP.txt',"Gram+", main_id)
        return main_id

    def add_anno_signalp_detail(self, detail_file, mytype, main_id=None):
        if not isinstance(main_id, ObjectId):
            main_id = ObjectId(main_id)
        data_list = []
        ...
        with open(detail_file, 'r') as f:
            lines = f.readlines()
            for line in lines[1:]:
                data = [("anno_signalp_id", main_id),("type", mytype)]
                ...
                if len(line) > 11:
                    data.extend([("d_score",line[11])])
                data_son = SON(data)
                data_list.append(data_son)
        try:
            collection_detail.insert_many(data_list)
            s_collection = self.db['anno_signalp']
            ...
            self.bind_object.set_error("导入anno_signalp_detail%s信息出错:%s" , variables=(detail_file, e), code="51401102")
        else:
