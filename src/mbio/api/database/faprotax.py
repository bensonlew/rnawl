...
class Faprotax(Base):
    ...
    def add_Faprotax(self, prediction_file,group_file,main_id=None,project_sn='faprotax',task_id='faprotax', params=None):
        ...
        table_name = [col for col in data1.columns if col not in ['OTU ID']]
        ...
        try:
            collection1 = self.db["sg_faprotax_detail"]
            collection1.insert_many(insert_data1)
            collection2 = self.db["sg_faprotax_detail"]
            collection2.insert_many(insert_data2)
            main = self.db['sg_faprotax']
            ...
            self.bind_object.logger.info("导入sg_faprotax详情数据出错:%s" % e)
        else:
            self.bind_object.logger.info("导入sg_faprotax详情数据成功")
