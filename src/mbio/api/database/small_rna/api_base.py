...
class ApiBase(Base):
    ...
    def create_db_table(self, table_name, content_dict_list, tag_dict=None):
        ...
        :param content_dict_list: a list with dict as elements
        ...
        try:
            ...
            if record_num >= 2:
                self.bind_object.logger.info('success in inserting records into table {}'.format(table_name))
            elif record_num == 1:
                self.bind_object.logger.info('success in inserting record into table {}'.format(table_name))
            return table_id
