...
class GenesetPpi(ApiBase):
    ...
    def add_network_attributes(self, file2_path, table_id=None, major=False):
        ...
        with open(file2_path, "rb") as w:
            data2 = w.readlines()
            for line2 in data2:
                temp2 = line2.rstrip().split(b"\t")
                data_list1.append(temp2[1])
            data = [("ppi_id", table_id),
                    ("num_of_nodes", eval(data_list1[0])),
                    ("num_of_edges", eval(data_list1[1])),
                    ("average_node_degree", "None" if data_list1[2] == "NA" else eval(data_list1[2])),
                    ("average_path_length", "None" if data_list1[4] == "NA" else eval(data_list1[3])),
                        ...
                        for line1 in data1:
                            ...
                            if temp[1] == line_data[1]:
                                ...
                                data = [("ppi_id", table_id),
                                        ...
                                        ("accession_id", temp[3]),
                                        ("node_description", self.node_desc[int(node_id)])]
                                data_son = SON(data)
                                data_list.append(data_son)
        try:
    ...
    def add_node_table(self, file_path, table_id=None, major=False):
        ...
        print("self.node_desc = dict(zip(nodes_table['node_id'], nodes_table['STRING_description']))")
        self.node_desc = dict(zip(nodes_table['node_id'], nodes_table['STRING_description']))
