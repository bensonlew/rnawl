...
class ApiBase(Base):
    ...
    def create_db_table(self, table_name, content_dict_list, tag_dict=None):
        ...
        try:
            ...
            if record_num >= 2:
                self.bind_object.set_error('fail to insert records into table {} -> ({})'.format(table_name, e))
            else:
    ...
    def find_table_by_query_record(self, table_name, query_dict):
        conn = self.db[table_name]
    ...
    def update_db_record_new(self, table_name, record_id=None, query_dict=None, insert_dict=None, **kwargs):
        ...
        if insert_dict:
            kwargs.update(insert_dict)
        conn.update(query_dict, {'$set': kwargs}, upsert=False)

    def delete_db_record_some_key(self, table_name, record_id=None, query_dict=None, insert_dict=None, **kwargs):
        if record_id is not None:
            if isinstance(record_id, types.StringTypes):
                record_id = ObjectId(record_id)
            elif isinstance(record_id, ObjectId):
        ...
        if insert_dict:
            kwargs.update(insert_dict)
        conn.update(query_dict, {'$unset': kwargs})
    ...
    def update_sgtask_record(self, task_id=None, table_name="sg_task", **kwargs):
        ...
        if task_id is None:
            if self.bind_object:
                ...
                self.bind_object.set_error('No valid task_id', code="53701109")
        conn = self.db[table_name]
    ...
    def get_project_sn(self, task_id=None):
        if task_id is None:
            if self.bind_object:
                ...
                self.bind_object.set_error('No valid task_id', code="53701110")
        ...
        for main_record in main_records:
            ...
            if detail_table:
                ...
                for table, table_key in zip(detail_table, detail_table_key):
                    if not table_key:
                        self.bind_object.set_error('you should specify detail_table_key whose value is the _id of main table', code="53701111")
                    self.remove_db_record(table, query_dict={table_key: main_record['_id']})
        print 'succeed in removing tables in {} and {} by {}'.format(main_table, detail_table, task_id)
    ...
    def remove_table_by_main_record(self, main_table, detail_table=None, detail_table_key=None, **kwargs):
        ...
        for main_record in main_records:
            ...
            if detail_table:
                ...
                for table, table_key in zip(detail_table, detail_table_key):
                    if not table_key:
                        self.bind_object.set_error('you should specify detail_table_key whose value is the _id of main table', code="53701112")
                    self.remove_db_record(table, query_dict={table_key: main_record['_id']})
        else:
            print 'succeed in removing tables in {} and {} by {}'.format(main_table, detail_table, kwargs)
    ...
    # ----Note----
    # 如果你要在这里添加的函数并不通用或不会被大量重复使用，不要在这里添加，你可以考虑通过self.db[table_name].find()等函数解决你的问题。
