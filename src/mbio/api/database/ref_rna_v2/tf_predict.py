...
class TfPredict(ApiBase):
    ...
    def to_dict_record_dropna(self, data):
        ...
        for i in range(len(data)):
            ...
            if 'dbd' in record_dict:
                try:
                    record_dict['dbd'] = float(record_dict['dbd'])
                except:
                    pass
    ...
    def add_tf_predict_detail(self, output_dir, main_id):
        ...
        if find_file:
            ...
            if result_pd.shape[0] == 0:
                self.bind_object.set_error("没有鉴定出转录因子")
            if 'gene_name' in result_pd.columns:
                result_pd.drop(columns=['gene_name'])
            if 'description' in result_pd.columns:
                result_pd.drop(columns=['description'])
            # result_pd.drop(columns=['gene_name', 'description'])
            families = list(set(result_pd['family'].dropna()))
