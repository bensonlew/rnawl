...
class AllExp(ApiBase):
    ...
    def add_exp(self, exp_matrix, quant_method='RSEM', exp_level='T', group_dict=None, main_id=None, group_id=None,
        ...
        try:
            ...
            self.bind_object.set_error("导入表格sg_exp_detail信息出错:{}".format(e))
        # add graph info
        if add_distribution:
            params = dict(
                task_id=task_id,
    ...
    def add_diffexp_v3(self, diff_output, exp_id=None, group_dict=None, group_id=None,
    ...
    def add_diffexp(self, diff_output, exp_id=None, group_dict=None, group_id=None,
                    exp_level='T', quant_method='RSEM', diff_method='DESeq2', main_id=None,
                    project_sn='denovo_rna_v2', task_id='denovo_rna_v2', params=None,
                    pvalue_padjust='padjust', create_geneset=True, nosig_info=None,s3_output_dir = None):
        """
        add differential analysis result to database
        :param diff_output: diffexp result dir
        :param exp_id: exp table id from POST, will be used for getting expression value
        ...
        for each in diff_files:
            ...
            # get_s3_file_path
            final_file_name = os.path.basename(each).split('.xls')[0] + '.annot.xls'
            result_dir = os.path.join(s3_output_dir, final_file_name)
            diff_file_dict[cmp_combine] = result_dir
            ...
            bool_ind = volcano_pd['log10pvalue'] > stat_cutoff
            # volcano_pd.loc[bool_ind, 'log10pvalue'] = stat_cutoff
            # degseq 再上述处理后volcano_pd基因数量减少
            vol_cmp_pd = pd.DataFrame([cmp_combine] * volcano_pd.shape[0], columns=['compare'])
            volcano_pd = pd.concat([volcano_pd, vol_cmp_pd], axis=1)
            volcano_pd_nosig = volcano_pd[volcano_pd['significant'] == 'no']
            # random select 10000 not sig diff genes for plotting
            if volcano_pd_nosig.shape[0] > 8000:
                volcano_pd_sig = volcano_pd[volcano_pd['significant'] == 'yes']
            ...
            if diff_method =="DEGseq":
                ...
                scatter_pd = scatter_pd[scatter_pd["regulate"] != "no test"]
                ...
                if diff_method=="DESeq2":
                    ...
                    # final_scatter_pd = final_scatter_pd.drop(final_scatter_pd[final_scatter_pd["significant"] == "no"][
                    #                                              abs(final_scatter_pd["group1"] - final_scatter_pd[
                    #                                                  "group2"]) >= abs(np.log10(2))].index)

                    # final_scatter_pd = final_scatter_pd.drop(final_scatter_pd[final_scatter_pd["significant"]=="no"][(final_scatter_pd["group1"]/final_scatter_pd["group2"]>2) & (final_scatter_pd["group1"]/final_scatter_pd["group2"]<0.5)].index)
                    # final_scatter_pd = final_scatter_pd[(final_scatter_pd["group1"]/final_scatter_pd["group2"]>2)
                    # | (final_scatter_pd["group1"]/final_scatter_pd["group2"] < 0.5 ) if final_scatter_pd["significant"] == "no" ]
    ...
    def add_geneset_cluster(self, cluster_output_dir, gene_detail_file, main_id=None, project_sn='denovo_rna_v2',
        ...
        if "seq.cluster_tree.txt" in results:
            ...
            seq_id = exp_matrix['seq_id'].tolist()
            for i in seq_id:
                if ':' in i:
    ...
    def add_diffexp_noiseq(self, diff_output, exp_id=None, group_dict=None, group_id=None,
                    exp_level='T', quant_method='RSEM', diff_method='DESeq2', main_id=None,
                    project_sn='denovo_rna_v2', task_id='denovo_rna_v2', params=None,
                           create_geneset=True, nosig_info=None,s3_output_dir = None):
        """
        add differential analysis result to database
        :param diff_output: diffexp result dir
        :param exp_id: exp table id from POST, will be used for getting expression value
...
class TestFunction(unittest.TestCase):
    ...
    def test_diff(self):
        ...
        wf.test_api = wf.api.api("ref_rna_v2.all_exp")
        wf.test_api.run1()
