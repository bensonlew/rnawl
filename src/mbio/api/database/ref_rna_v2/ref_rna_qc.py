...
class RefRnaQc(ApiBase):
    ...
    @report_check
    def add_samples_info(self, qc_stat, group, qc_adapt=None, fq_type='se', about_qc='before'):
        """
    ...
    def add_chrom_distribution_table(self, distribution=None, group=None, name=None, params='RSeQC-2.6.3'):
        ...
        inserted_id = collection.insert_one(insert_data).inserted_id
        if distribution:
            self.add_chorm_distribution_detail(distribution, group, inserted_id)
            self.update_db_record('sg_assessment_chrom_distribution', inserted_id, status="end", main_id=inserted_id)
        return inserted_id

    @report_check
    def add_chorm_distribution_detail(self, chorm_stat, group, chorm_stat_id):
        sample_list = list()
        with open(group, 'r') as g:
            for line in g.readlines():
                if line.startswith('#'):
        ...
        try:
            ...
            main_collection.update({"_id": ObjectId(chorm_stat_id)}, {"$set": {"distribution": list(distribution), "specimen_name": sample_list}})
        except Exception, e:
    ...
    @report_check
    def add_tophat_mapping_stat(self, stat_file, group):
    ...
    @report_check
    def add_hisat_mapping_stat(self, stat_file, group):
        files = sorted(glob.glob("{}/*".format(stat_file)))
        sample_list = list()
        with open(group, 'r') as g:
            for line in g.readlines():
    ...
    @report_check
    def add_star_mapping_stat(self, stat_file, group):
        files = sorted(glob.glob("{}/*".format(stat_file)))
        sample_list = list()
        with open(group, 'r') as g:
            for line in g.readlines():
