...
class EnvVif(Base):
    ...
    def add_env_vif_detail(self, file_path, table_type, vif_id):
        ...
        try:
            collection = self.db["sg_env_vif_detail"]
            collection.insert_many(data_list)
            self.bind_object.logger.info("开始刷新主表写入")
            main_collection = self.db["sg_env_vif"]
            #main_collection.update({"_id": ObjectId(vif_id)}, {"$set": {"main_id": ObjectId(vif_id)}})
            if table_type == "before_vif":
            ...
            self.bind_object.logger.error("导入sg_env_vif结果数据出错:%s" % e)
            self.bind_object.set_error("导入sg_env_vif数据出错", code="51002801")
        else:
            self.bind_object.logger.info("导入sg_env_vif结果数据成功")
