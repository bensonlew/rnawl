...
class Proteinset(ApiBase):
    ...
    @report_check
    def add_sg_status(self, desc=None, submit_location=None, params=None, table_id=None, table_name=None, type_name=None):
    ...
    def add_proteinset_cog_detail(self, proteinset_cog_table, proteinset_cog_id):
        ...
        try:
            ...
            if update_status:
                self.add_sg_status(submit_location='proteinsetcog', params=params, table_id=ObjectId(proteinset_cog_id), table_name=name, type_name='sg_proteinset_cog_class')
            self.bind_object.logger.info(proteinset_name)
    ...
    def add_proteinset_venn(self, task_id=None, project_sn=None):
        ...
        all_proteinsets_id__of_all = [str(proteinset["main_id"]) for proteinset in proteinset_result if proteinset['name'][-4:] == "_all"]
    ...
    def add_go_regulate_detail(self, go_regulate_dir, go_regulate_id):
        ...
        try:
            ...
            if update_status:
                self.add_sg_status(submit_location='proteinsetgo', params=params, table_id=ObjectId(go_regulate_id), table_name=name, type_name='sg_proteinset_go_class')
            self.bind_object.logger.info(proteinset_name)
    ...
    def add_go_regulate_detail2(self, go_regulate_dir, go_regulate_id):
        ...
        try:
            ...
            if update_status:
                self.add_sg_status(submit_location='proteinsetgo_two', params=params, table_id=ObjectId(go_regulate_id), table_name=name, type_name='sg_proteinset_go_class2')
            self.bind_object.logger.info(proteinset_name)
            self.bind_object.logger.info(ObjectId(go_regulate_id))
        except Exception as e:
            self.bind_object.logger.info("导入go调控信息：%s出错:%s" % (go_regulate_dir, e))
    ...
    def add_ipath_detail(self, main_table_id, ipath_input, proteinset):
        ...
        try:
            ...
            main_collection.update({"_id": main_table_id},
                                   {"$set": {"table_columns": proteinset_name, "status": "end"}})
            if update_status:
                self.add_sg_status(submit_location='proteinsetipath', params=params, table_id=ObjectId(main_table_id),
                                   table_name=name, type_name='sg_proteinset_ipath')
                cmp = os.path.dirname(ipath_input).split('/')[-1]
                result_dir = os.path.join(self.bind_object.workflow_output,
                                          '5_Proteinset/06_PsetIpath', cmp)
                self.update_db_record('sg_proteinset_ipath', main_table_id, result_dir=result_dir, pathways=['Metabolism.svg', 'Secondary_metabolites.svg', 'Antibiotics.svg', 'Microbial_metabolism.svg'])
    ...
    def add_circ_graph(self, main_table_id, circ_input, enrich_type):
        ...
        if not main_table_id:
            ...
            name = 'Diff%sCircle_'%enrich_type.capitalize() + datetime.datetime.now().strftime("%Y%m%d_%H%M%S%f")[:-3] + pset_name_
        ...
        try:
            ...
            if update_status:
                self.add_sg_status(submit_location='proteinsetcirc', params=params, table_id=ObjectId(main_table_id),
    ...
    def add_kegg_regulate_new2(self, main_table_id, proteinset_file, kegg_stat_file, gene_kegg_level_table_xls):
        ...
        try:
            ...
            if update_status:
                self.add_sg_status(submit_location='proteinsetkegg', params=params, table_id=ObjectId(main_table_id), table_name=name, type_name='sg_proteinset_kegg_class')
                cmp = os.path.dirname(kegg_stat_file).split('/')[-1]
                result_dir = os.path.join(self.bind_object.workflow_output, '5_Proteinset/03_PsetAnno/02_PsetKEGG', cmp)
        ...
        if not main_table_id:
            ...
            if not species:
                try:
                    ...
                    pass
            if self.bind_object.option('useblast') is False:
                useblast = 'no'
            else:
                useblast = 'yes'
    ...
    def add_proteinset_self(self, proteinset_output_dir, name=None,
        ...
        if protein_length < 10:
            ...
            try:
                collection = self.db['sg_status']
                collection.update({"table_id": main_id}, {"$set": {'status': 'failed', 'submit_location': 'proteinset_upload'}}, upsert=False)
            except:
                try:
                    collection = self.db['sg_status']
                    collection.update({"table_id": main_id, "status": 'start'},
                                      {"$set": {'status': 'failed', 'desc': '可以创建的蛋白集中蛋白数量少于10，不予创建',
                                                'submit_location': 'proteinset_upload'}}, upsert=True)
                # {"$set": {'submit_location': 'proteinset_upload'}}
                except:
                    pass
            # raise Exception("可以创建的蛋白集中蛋白数量少于10，不予创建")
            self.bind_object.set_error("可以创建的蛋白集中蛋白数量少于10，不予创建")
        # self.db['sg_proteinset'].update({'_id': main_id}, {'$inc': {'protein_length': protein_length}})
        else:
            self.update_db_record('sg_proteinset', main_id, status="end", main_id=main_id, proteinset_length=protein_length, submit_location='proteinset_upload')
            collection = self.db['sg_status']
            try:
                collection.update({"table_id": main_id}, {"$set": {'status': 'end'}}, upsert=True)
            except:
                pass
