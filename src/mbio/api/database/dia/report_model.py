...
class ReportModel(ApiBase):
    ...
    def get_result_table_id(self, task_id, p_dict):
        ...
        if "main_dict" in p_dict:
            ...
            if main_records.count() == 0:
                ...
                # name_list = ["_".join(i['name'].split('_')[3:]) for i in main_records]
                # record_choose = main_records[name_list.index(sorted(name_list)[0])]
                result_table_names_ids_dic = {}
                for i in main_records:
                    result_table_names_ids_dic["_".join(i['name'].split('_')[3:])] = [i['name'],i["main_id"]]
                result_table_name_ = sorted(list(result_table_names_ids_dic.keys()))[0]
                result_table_name = result_table_names_ids_dic[result_table_name_][0]
                result_table_id = result_table_names_ids_dic[result_table_name_][1]
                # record_choose = self.choose_one_from_record(p_dict["main_dict"], main_records)
                # print "xx6.2.3 : record_choose is"+str(record_choose)
            ...
            if not main_record:
                ...
                result_table_name = main_record.get("name", "")
    ...
    def add_report_image(self, task_id, report_config, img_path, member_id=""):
        ...
        for p_id, p_dict in report_dict.items():
            ...
            if p_id == '110203':    # sg_protein_info, lack of detail table
                result_table_id = task_id
                result_table_name = ''
            else:
