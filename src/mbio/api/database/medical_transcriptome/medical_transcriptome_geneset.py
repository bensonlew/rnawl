...
class MedicalTranscriptomeGeneset(ApiBase):
    def __init__(self, bind_object):
        super(MedicalTranscriptomeGeneset, self).__init__(bind_object)
    ...
    def add_geneset_cog_detail(self, geneset_cog_table, geneset_cog_id):
        ...
        with open(geneset_cog_table, 'r') as f:
            ...
            self.bind_object.set_error(geneset_name)
            for line in f:
    ...
    def add_go_regulate_detail(self, go_regulate_dir, go_regulate_id):
        ...
        try:
            ...
            if len(data_list) != 0:
                collection.insert_many(data_list)
            if len(data_list) == 0:
                self.bind_object.logger.info("nononononononono")
    ...
    def add_kegg_regulate_new(self, main_table_id, geneset_id, kegg_stat_xls, gene_kegg_level_table_xls, work_dir,
        ...
        if is_diff is False and genesets_num == 1:
            ...
            #change _genes->_list,_numbers->_num
            # rename_dic_list = [{i:"{}_list".format(i.split('_genes')[0])} for i in new_data_rkaa.columns if i.endswith('_genes')]
            # rename_dic_list.extend([{i:"{}_num".format(i.split('_numbers')[0])} for i in new_data_rkaa.columns if i.endswith('_numbers')])
            rename_dic_list = [{i:"{}_list".format(i[:-6])} for i in new_data_rkaa.columns if i.endswith('_genes')]
            rename_dic_list.extend([{i:"{}_num".format(i[:-8])} for i in new_data_rkaa.columns if i.endswith('_numbers')])
            rename_dic = {}
            for i in rename_dic_list:
                rename_dic.update(i)
            new_data_rkaa.rename(columns=rename_dic,inplace=True)
            #
            ...
            try:
                ...
                #change field _genes_num->_num
                # rename_dic_list = [{i:"{}_num".format(i.split('_genes_num')[0])} for i in df_a.columns if i.endswith('_genes_num')]
                rename_dic_list = [{i:"{}_num".format(i[:-10])} for i in df_a.columns if i.endswith('_genes_num')]
                rename_dic = {}
                for i in rename_dic_list:
                    rename_dic.update(i)
                df_a.rename(columns=rename_dic,inplace=True)
                #
            ...
            with open(work_dir + "/" + "kegg_analysis_of_anotate") as r_kaa, open(work_dir + "/" + "new_data_rkaa",
                ...
                for line in r_kaa:
                    ...
                    wkaa.write("\t".join(line) + "\n")
            new_data_rkaa = pd.read_table(work_dir + "/" + "new_data_rkaa", header=0, sep="\t")
            #change field _genes->_list,_numbers->_num
            # rename_dic_list = [{i:"{}_list".format(i.split('_genes')[0])} for i in new_data_rkaa.columns if i.endswith('_genes')]
            rename_dic_list = [{i:"{}_list".format(i[:-6])} for i in new_data_rkaa.columns if i.endswith('_genes')]
            # rename_dic_list.extend([{i:"{}_num".format(i.split('_numbers')[0])} for i in new_data_rkaa.columns if i.endswith('_numbers')])
            rename_dic_list.extend([{i:"{}_num".format(i[:-8])} for i in new_data_rkaa.columns if i.endswith('_numbers')])
...
#------------------蛋白相关复制过来的函数---------------------------
    ...
    def add_circ_detail(self, main_table_id, circ_input, enrich_type):
        ...
        try:
            ...
            self.bind_object.set_error("导入circ_detail: %s出错!" , variables=(circ_input), code="53702366")
        else:
    ...
    def add_circ_graph(self, main_table_id, circ_input, enrich_type):
        ...
        try:
            ...
            self.bind_object.logger.info("导入circ_graph：%s成功!" % (circ_input))
    ...
    def update_circ_main(self, main_table_id, circ_zscore_input, enrich_type):
        ...
        try:
            ...
            self.bind_object.logger.info("更新circ主表：%s成功!" % (main_table_id))
