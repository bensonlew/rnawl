...
class DiffGenesetPipline(ApiBase):
    def __init__(self, bind_object):
        super(DiffGenesetPipline, self).__init__(bind_object)
    ...
    @report_check
    def add_diff_genest_pipline_table(self,diff_geneset_pipline_result,diff_geneset_pipline_id = None,
                                      diff_id = None, task_id=None , analysis_names = None,kegg_level_path=None,inter_path= None):
        """
        差异基因一键化数据挖掘导表函数
        ...
        if not isinstance(diff_geneset_pipline_id, ObjectId):
            if isinstance(diff_geneset_pipline_id, types.StringTypes):
                diff_geneset_pipline_id = ObjectId(diff_geneset_pipline_id)
            else:
                self.bind_object.set_error('diff_geneset_pipline_id{}必须为ObjectId对象或其对应的字符串！'.format(diff_geneset_pipline_id))
        ...
        self.pipline_id = diff_geneset_pipline_id
        ...
        analysis_results =os.listdir(diff_geneset_pipline_result)
    ...
    def update_geneset_status(self):
        for geneset in self.genesets:
            ...
            try:
                self.update_db_record('sg_geneset', geneset_main_id, status="end", is_use=1, main_id=geneset_main_id)
                self.bind_object.logger.info('基因集状态更新为end,基因集导表成功}')
    ...
    def updata_diff_main_id(self,diff_id):
        ...
        try:
            ...
            self.bind_object.logger.info('差异主表更新成功,导表结束}')
        except Exception as e:
    ...
    def add_single_genset_analysis_result(self,single_genset_analysis_dir,diff_id):
        ...
        try:
            geneset_id = self.create_geneset(geneset_info,diff_id)
        except Exception as e:
            self.bind_object.logger.error("导入基因集{}信息出错:{}".format(geneset_info["geneset_name"], e))
            self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
            self.bind_object.set_error('基因集{}创建失败'.format(geneset_info["geneset_name"]))
    ...
    def add_geneset_analysis_table(self, geneset_info):
        ...
        if "go" in self.total_analysis:
            ...
            if has_go_class_result:
                ...
                try:
                    self.update_db_record('sg_diff_geneset_go_class', go_class_main_id, status="end",
                                          main_id=go_class_main_id)
                except Exception as e:
                    self.bind_object.logger.error("更新基因集{}sg_diff_geneset_go_class 状态失败:{}".format(geneset_info["geneset_name"], e))
                    self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
                    self.bind_object.set_error("fail to update status sg_diff_geneset_go_class for genset:{}".format(geneset_info["geneset_name"]))
            else:
                self.update_db_record('sg_diff_geneset_go_class', go_class_main_id, has_results=False, status="end")
            ...
            go_enrich_s3_dir = os.path.join(self.s3_output,geneset_info["geneset_name"],"diff_go_enrich")
            ...
            if has_go_enrich_result:
                ...
                self.update_db_record('sg_diff_geneset_go_enrich', go_enrich_main_id, has_results=False,status="end")
        ...
        if "kegg" in self.total_analysis:
            ...
            if has_kegg_class_result :
                ...
                self.add_kegg_regulate_pic(kegg_class_main_id, self.kegg_level_path, pathway_file,
                                                  source="diff_exp")
                try:
                    conn=self.db["sg_diff_geneset_kegg_class"]
                    #更新结果文件目录到主表(为了前端取html和png文件
                    # graph_dir = os.path.join(self.s3_output,geneset_info["geneset_name"],"diff_kegg_class","pathways")
                    graph_dir = os.path.join(self.s3_output, '03DiffExpress_Geneset_Annotion','02KEGG',geneset_info["geneset_name"],"pathways")
                    conn.update({"_id": kegg_class_main_id}, {"$set": {'graph_dir': graph_dir,"status":"end"}}, upsert=True)
                except Exception as e:
                    self.bind_object.logger.error("更新sg_diff_geneset_kegg_class文件graph_dir失败")
                    self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
                    self.bind_object.set_error("更新sg_diff_geneset_kegg_class文件graph_dir失败")
            else:
                self.update_db_record('sg_diff_geneset_kegg_class', kegg_class_main_id, has_results=False,status="end")

            # kegg富集
            kegg_enrich_main_id = self.add_kegg_enrich_main_table(geneset_info)
            try:
                self.insert_geneset_info(geneset_info["geneset_id"],"sg_diff_geneset_kegg_enrich",kegg_enrich_main_id)
            except Exception as e:
                self.bind_object.logger.error("更新基因集:{}sg_geneset_info状态失败".format(geneset_info["geneset_name"]))
                self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
                self.bind_object.set_error("更新基因集:{}sg_geneset_info状态失败".format(geneset_info["geneset_name"]))
            ...
            # s3_workflow_output = os.path.join(self.s3_output,geneset_info["geneset_name"],"diff_kegg_enrich","enrich")
            s3_workflow_output = os.path.join(self.s3_output, '04DiffExpress_Geneset_Enrich',
                                              '02KEGG',
            ...
            if has_kegg_enrich_result:
                ...
                self.update_db_record('sg_diff_geneset_kegg_enrich', kegg_enrich_main_id,
                                             result_dir=workflow_output)
                self.add_kegg_enrich_pic(kegg_enrich_main_id, output_file[0],
                                                output_dir2 + '/pathways', source="diff_exp")
                try:
                    # graph_dir = os.path.join(self.s3_output,geneset_info["geneset_name"],"diff_kegg_enrich","class",'pathways')
                    graph_dir = os.path.join(self.s3_output, '04DiffExpress_Geneset_Enrich', '02KEGG',
                                             geneset_info["geneset_name"] , "kegg_enrich_pathways")
                    self.update_db_record('sg_diff_geneset_kegg_enrich', kegg_enrich_main_id, graph_dir=graph_dir,status="end")
                except Exception as e:
                ...
                self.update_db_record('sg_diff_geneset_kegg_enrich', kegg_enrich_main_id, has_results=False,
                                      status="end")
        ...
        if "reactome" in self.total_analysis:
            ...
            try:
                self.insert_geneset_info(geneset_info["geneset_id"], "sg_diff_geneset_reactome_class",
                                         reactome_class_main_id)
            except Exception as e:
                self.bind_object.logger.error("更新基因集:{}sg_geneset_info状态失败".format(geneset_info["geneset_name"]))
                self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
                self.bind_object.set_error("更新基因集:{}sg_geneset_info状态失败".format(geneset_info["geneset_name"]))
            reactome_class_dir = geneset_info["geneset_results"]["diff_reactome_class"]
            pathway_file = os.path.join(reactome_class_dir , 'reactome_path.xls')
            class_file = os.path.join(reactome_class_dir, 'reactome_class.xls')
            svg_path = os.path.join(reactome_class_dir , 'svg')
            # modify by fwy 20210812
            result_num = len(open(pathway_file, 'r').readlines())
            has_reactome_class_result = True if result_num > 1 else False
            if has_reactome_class_result:
                self.add_geneset_reactome_detail(reactome_class_main_id,
                                                        geneset_info["geneset_name"],
                                                        pathway_file,
                                                        class_file,
                                                        svg_path,
                                                        regulate=geneset_info["regulate"])
                try:
                    # graph_dir = os.path.join(self.s3_output,geneset_info["geneset_name"],"diff_reactome_class","svg")
                    graph_dir = os.path.join(self.s3_output, '03DiffExpress_Geneset_Annotion', '03Reactome',
                                             geneset_info["geneset_name"] ,"Reactome_pathways")
                    self.update_db_record('sg_diff_geneset_reactome_class', reactome_class_main_id, graph_dir=graph_dir,status="end")
                except Exception as e:
                    self.bind_object.logger.error("更新sg_diff_geneset_reactome_class文件graph_dir失败")
                    self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
                    self.bind_object.set_error("更新sg_diff_geneset_reactome_class文件graph_dir失败")
            else:
                self.update_db_record('sg_diff_geneset_reactome_class', reactome_class_main_id, has_results=False,
                                      status="end")
            ...
            try:
                self.insert_geneset_info(geneset_info["geneset_id"], "sg_diff_geneset_reactome_enrich",
                                         reactome_enrich_main_id)
            except Exception as e:
                self.bind_object.logger.error("更新基因集:{}sg_geneset_info状态失败".format(geneset_info["geneset_name"]))
                self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
                self.bind_object.set_error("更新基因集:{}sg_geneset_info状态失败".format(geneset_info["geneset_name"]))
            reactome_enrich_dir = geneset_info["geneset_results"]["diff_reactome_enrich"]
            enrich_result = glob.glob("{}/*.xls".format(reactome_enrich_dir))[0]
            svg_path = os.path.join(reactome_enrich_dir,"svg")
            # modify by fwy 20210812
            result_num = len(open(enrich_result, 'r').readlines())
            has_reactome_enrich_result = True if result_num > 1 else False
            if has_reactome_enrich_result:
                self.add_geneset_reactome_enrich(reactome_enrich_main_id,
                                                 geneset_info["geneset_name"],
                                                 enrich_result,
                                                 svg_path)
                try:
                    # graph_dir = os.path.join(self.s3_output,geneset_info["geneset_name"],"diff_reactome_enrich","svg")
                    graph_dir = os.path.join(self.s3_output,  '04DiffExpress_Geneset_Enrich', '03Reactome',
                                             geneset_info["geneset_name"] ,
                                             "Reactome_enrich_pathways")
                    self.update_db_record('sg_diff_geneset_reactome_enrich', reactome_enrich_main_id, graph_dir=graph_dir,status="end")
                except Exception as e:
                    self.bind_object.logger.error("更新sg_diff_geneset_reactome_enrich文件graph_dir失败")
                    self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
                    self.bind_object.set_error("更新sg_diff_geneset_reactome_enrich文件graph_dir失败")
            else:
                self.update_db_record('sg_diff_geneset_reactome_enrich', reactome_enrich_main_id, has_results=False,
                                      status="end")
        ...
        if "do" in self.total_analysis:
            ...
            try:
                ...
                self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
                self.bind_object.set_error("更新基因集:{}sg_geneset_info状态失败".format(geneset_info["geneset_name"]))
            do_class_output = geneset_info["geneset_results"]["diff_do_class"]
            do_result_file  = os.path.join(do_class_output, "do_level2_class.xls")
            # modify by fwy 20210812
            result_num = len(open(do_result_file, 'r').readlines())
            has_do_class_result = True if result_num > 1 else False
            if has_do_class_result:
                self.add_diff_geneset_do_detail(do_result_file,do_class_main_id, geneset_names=geneset_info["geneset_name"])
            else:
                self.update_db_record('sg_diff_geneset_do_class', do_class_main_id, has_results=False,
                                      status="end")
            # do富集
            ...
            try:
                ...
                self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
                self.bind_object.set_error("更新基因集:{}sg_geneset_info状态失败".format(geneset_info["geneset_name"]))
            do_enrich_output = geneset_info["geneset_results"]["diff_do_enrich"]
            do_enrich_result_file = os.path.join(do_enrich_output, "do_enrichment.xls")
            # modify by fwy 20210812
            result_num = len(open(do_enrich_result_file, 'r').readlines())
            has_do_enrich_result = True if result_num > 1 else False
            if has_do_enrich_result:
                self.add_geneset_do_enrich(do_enrich_result_file, do_enrich_main_id)
            else:
                self.update_db_record('sg_diff_geneset_do_enrich', do_enrich_main_id, has_results=False,
                                      status="end")
    ...
    def add_kegg_enrich_main_table(self,geneset_info):
        ...
        try:
            main_table_id = self.insert_main_table("sg_diff_geneset_kegg_enrich", mongo_data)
        except Exception as e :
            self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
            self.bind_object.set_error("sg_diff_geneset_kegg_enrich主表创建失败")
        return main_table_id
    ...
    def add_kegg_class_main_table(self, geneset_info):
        ...
        try:
            main_table_id = self.insert_main_table("sg_diff_geneset_kegg_class", mongo_data)
        except Exception as e:
            self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
            self.bind_object.set_error("sg_diff_geneset_kegg_class主表创建失败")
        return main_table_id
    ...
    def add_go_enrich_main_table(self, geneset_info):
        ...
        try:
            main_table_id = self.insert_main_table("sg_diff_geneset_go_enrich", mongo_data)
        except Exception as e:
            self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
            self.bind_object.set_error("sg_diff_geneset_go_enrich主表创建失败")
        return main_table_id
    ...
    def add_go_class_main_table(self,geneset_info):
        ...
        try:
            main_table_id = self.insert_main_table("sg_diff_geneset_go_class", mongo_data)
        except Exception as e:
            self.bind_object.logger.error("导入sg_diff_geneset_go_class{}主表:{}".format(geneset_info["geneset_name"], e))
            self.bind_object.logger.error('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
            self.bind_object.set_error("sg_diff_geneset_go_class主表创建失败")
        return main_table_id
    ...
    def create_geneset(self,geneset_info,diff_id):
        ...
        level =geneset_info["level"]
        diff_conn = self.db["sg_diff"]
        diff_main_table = diff_conn.find_one({"main_id":ObjectId(str(diff_id))})
        diff_table_name = diff_main_table["name"]
        if geneset_info["gene_num"] != 0:
            ...
            geneset_main_info = dict(
                ...
                regulate =geneset_info["regulate"] ,
                level=level,
                desc='differential create genesets of table {}'.format(diff_table_name),
    ...
    def add_go_regulate_detail(self, go_regulate_dir, go_regulate_id):
        ...
        with open(go_regulate_dir, 'r') as f:
            ...
            for line in f:
                ...
                data = {
                    'diff_go_regulate_id': go_regulate_id,
                    'go_type': line[0],
                    'go': line[1],
                    'go_id': line[2]
                }
                for n, dk in enumerate(doc_keys):
                    ...
                    try:
                        ...
                        data["{}_list".format(dk)] = line[5 + n * 3].split(";")
                        if data["{}_list".format(dk)] == ["none"]:
                            data["{}_list".format(dk)] = list()
        ...
        try:
            collection = self.db['sg_diff_geneset_go_class_detail']
            main_collection = self.db['sg_diff_geneset_go_class']
            if len(data_list) != 0:
            ...
            self.bind_object.logger.error('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
            self.bind_object.set_error("sg_diff_geneset_go_class_detail failed主表创建失败")
        else:
    ...
    def add_go_enrich(self, result, main_id, s3_output):
        ...
        df['diff_go_enrich_id'] = main_id
        self.create_db_table('sg_diff_geneset_go_enrich_detail', df.to_dict('r'))
        self.update_db_record('sg_diff_geneset_go_enrich', main_id, categories=categories,
    ...
    def add_kegg_regulate_new(self, main_table_id, geneset_id, kegg_stat_xls, gene_kegg_level_table_xls, work_dir,
        ...
        # if not regulate :
            ...
            new_data_rkaa['diff_kegg_class_id'] = ObjectId(main_table_id)
            ...
            self.create_db_table('sg_diff_geneset_kegg_class_detail', new_data_rkaa_list)
            # self.create_db_table('sg_geneset_kegg_class_detail', kaa_list)
            self.update_db_record('sg_diff_geneset_kegg_class', ObjectId(main_table_id), main_id=ObjectId(main_table_id))
            ...
            try:
                ...
                appended_data_new_1["diff_kegg_class_id"] = ObjectId(main_table_id)
                ...
                self.create_db_table('sg_diff_geneset_kegg_class_statistic', data_new)
                ...
                main_collection = self.db['sg_diff_geneset_kegg_class']
            ...
            self.create_db_table('sg_diff_geneset_kegg_class_detail', new_data_rkaa_list)
            self.bind_object.logger.info("sg_diff_geneset_kegg_class_detail 导表成功：总数 [%s]" % len(new_data_rkaa_list))
            self.bind_object.logger.info("sg_diff_geneset_kegg_class_detail 导表成功：head [%s]" % str(new_data_rkaa_list[0]))
            # self.create_db_table('sg_geneset_kegg_class_detail', kaa_list)
            self.update_db_record('sg_diff_geneset_kegg_class', ObjectId(main_table_id), main_id=ObjectId(main_table_id))
            ...
            appended_data_new_2["diff_kegg_class_id"] = ObjectId(main_table_id)
            ...
            self.bind_object.logger.info("成功更新kegg主表的一级分类信息缩写")
            # except Exception as e:
            #     self.bind_object.logger.info("导入kegseq_strg统计信息出错")
            # else:
            self.bind_object.logger.info("导入kegg统计信息成功")
    ...
    def add_kegg_regulate_pic(self, main_table_id, level_path, png_dir, source):
        ...
        if data_list:
            try:
                collection = self.db['sg_diff_geneset_kegg_class_pic']
                collection.insert_many(data_list)
            except Exception, e:
    ...
    def add_kegg_enrich_detail(self, enrich_id, kegg_enrich_table, geneset_list_path, all_list_path):
        ...
        with open(kegg_enrich_table, 'rb') as r:
            ...
            if data_list:
                ...
                try:
                    collection = self.db['sg_diff_geneset_kegg_enrich_detail']
                    collection.insert_many(data_list)
                    coll = self.db['sg_diff_geneset_kegg_enrich']
    ...
    def add_kegg_enrich_pic(self, kegg_id, level_path, png_dir, source):
        ...
        if data_list:
            try:
                collection = self.db['sg_diff_geneset_kegg_enrich_pic']
                collection.insert_many(data_list)
            except Exception, e:
    ...
    def add_geneset_reactome_detail(self, main_id, geneset_names, geneset_reactome_pathway, geneset_reactome_stat,
        ...
        try:
            ...
            self.bind_object.logger.info("导入%s成功!" % ("sg_diff_geneset_reactome_class"))
...
#------------------蛋白相关复制过来的函数---------------------------
    ...
    def add_geneset_cluster(self, cluster_output_dir, gene_detail_file, main_id=None, project_sn='medical_transcriptome',
        ...
        if main_id is None:
            ...
            main_id = self.create_db_table('sg_diff_geneset_cluster', [main_info])
        else:
            if isinstance(main_id, types.StringTypes):
                main_id = ObjectId(main_id)
        # update main table
        self.update_db_record('sg_diff_geneset_cluster', main_id,
        ...
        self.create_db_table('sg_diff_geneset_cluster_tree', [tree_info])
        self.create_db_table('sg_diff_geneset_cluster_detail', detail_info, tag_dict=dict(cluster_id=main_id))
        self.update_db_record('sg_diff_geneset_cluster', main_id, status="end", main_id=main_id, )
        return main_id
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            "output":"s3://common/files/m_188/188_5d01dede4f911/medical_transcriptome/interaction_results/Diff_geneset_pipline_G_20200918_110445",
            'options': {}
        }
        wheet = Sheet(data=data)
        wf = MedicalTranscriptomeTestApiWorkflow(wheet)
        wf.sheet.id = '8ju59of6lkdojsbreb482jveeb_9266_9786'
        wf.sheet.project_sn = '188_5d12fb95c7db8'
        wf.IMPORT_REPORT_DATA = True
        wf.IMPORT_REPORT_AFTER_DATA = False
        wf.test_api = wf.api.api('medical_transcriptome.diff_geneset_pipline')
        output_dir = "/mnt/ilustre/users/sanger-dev/workspace/20201119/DiffGenesetPipline_8ju59of6lkdojsbreb482jveeb_9266_9786/output"
        diff_geneset_pipline_id = "5fb5ccb117b2bf532d80e9d7"
        diff_id = "5facb1fd17b2bf65566b8880"
        task_id = "8ju59of6lkdojsbreb482jveeb"
        analysis_names =["go","kegg","do","reactome"]
        kegg_level_path = "/mnt/ilustre/users/sanger-dev/workspace/20201119/DiffGenesetPipline_8ju59of6lkdojsbreb482jveeb_9266_9786/DiffGenesetPrepare/AnnotPrepare/output/gene_kegg_level_table.xls"

        main_id = wf.test_api.add_diff_genest_pipline_table(output_dir,diff_geneset_pipline_id = diff_geneset_pipline_id,
                                                          diff_id=diff_id,task_id = task_id ,
                                                          analysis_names = analysis_names,kegg_level_path =kegg_level_path,
                                                            inter_path = "/mnt/ilustre/users/sanger-dev/workspace/20201119/DiffGenesetPipline_8ju59of6lkdojsbreb482jveeb_9266_9786/test")
