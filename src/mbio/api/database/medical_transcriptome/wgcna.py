...
class Wgcna(ApiBase):
    ...
    def add_module_detail(self, result_dir, gene_id2gene_name, exp_matrix, main_id):
        ...
        size_stat_pd.rename(columns={'seqs': 'seqs_str'}, inplace=True)
        size_stat_dict = size_stat_pd.to_dict("records")
        size_stat_dict_new = list()
        for i in size_stat_dict:
            if i['module'] == 'grey':
                size_stat_dict_new.insert(0, i)
            else:
                size_stat_dict_new.append(i)
        self.create_db_table('sg_wgcna_module_stat_detail', size_stat_dict_new)
