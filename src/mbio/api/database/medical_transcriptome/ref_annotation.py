...
class RefAnnotation(ApiBase):
    ...
    def set_result_dir(self, merge_annot_path):
        ...
        # for db in ["nr", "uniprot", "string", "kegg"]:
    ...
    def run(self, annotation_mudule_dir, trans2gene, trans2gene_ref, params_dict, taxonomy='Animals', exp_level='transcript', version = "v2", gene_exp = None, trans_exp = None):
        ...
        stat_id = self.add_annotation_stat(name=None, params=params, seq_type="new", database="nr,uniprot,pfam,cog,go,kegg", taxon=taxonomy, result_dir=self.result_dir, exp_level=exp_level)

        #
        # self.add_annotation_stat_detail(stat_id=stat_id, stat_path=self.result_file['ref_stat_path'], venn_path=self.result_file['ref_venn_path'], seq_type = "ref", exp_level=exp_level, gene_exp = gene_exp, trans_exp = trans_exp)
        # if self.has_new:
        #     self.add_annotation_stat_detail(stat_id=stat_id, stat_path=self.result_file['new_stat_path'], venn_path=self.result_file['new_venn_path'], seq_type = "new", exp_level=exp_level, gene_exp = gene_exp, trans_exp = trans_exp)
        #     self.add_annotation_stat_detail(stat_id=stat_id, stat_path=self.result_file['all_stat_path'], venn_path=self.result_file['all_venn_path'], seq_type = "all", exp_level=exp_level, gene_exp = gene_exp, trans_exp = trans_exp)
        ...
        self.remove_table_by_main_record(main_table='sg_annotation_uniprot', task_id=task_id, type=self.anno_type,  detail_table=['sg_annotation_uniprot_detail', 'sg_annotation_uniprot_pie'], detail_table_key='uniprot_id')
        params_select_uniprot = dict([(k,params_dict.get(k,None)) for k in ('uniprot_evalue', 'uniprot_similarity', 'uniprot_identity')])
        params_select_uniprot = json.dumps(params_select_uniprot, sort_keys=True, separators=(',', ':'))
        uniprot_id = self.add_annotation_uniprot(name=None, params=params_select_uniprot, stat_id=stat_id, result_dir=self.result_dir)
        if self.has_new:
            self.add_annotation_blast_uniprot_detail(blast_id=uniprot_id, seq_type="new", anno_type="T", database='uniprot', blast_path=self.result_file["uniprot" + "trans_blast_path"], exp_level=exp_level)
        self.add_annotation_blast_uniprot_detail(blast_id=uniprot_id, seq_type="ref", anno_type="T", database='uniprot', blast_path=self.result_file["uniprot" + "trans_blast_path" + "ref"], exp_level=exp_level)
        self.update_db_record('sg_annotation_uniprot', uniprot_id, status="end", main_id=uniprot_id)
        ...
        if exp_level.lower() == "transcript":
            ...
            # self.add_annotation_kegg_table(kegg_id=kegg_id, seq_type=seq_type, anno_type="T", table_path=self.result_file['table_path_all'])
        ...
        if self.has_new:
            ...
            if exp_level.lower() == "transcript":
                ...
                self.add_annotation_kegg_table(kegg_id=kegg_id, seq_type="all", anno_type="T", table_path=self.result_file['table_path_all'])
            ...
            self.add_annotation_kegg_table(kegg_id=kegg_id, seq_type="all", anno_type="G", table_path=self.result_file['gene_table_path_all'])
    ...
    def run_webroot(self, result_dir, trans2gene, trans2gene_ref, params_dict, task_id, stat_id, last_id, taxonomy, exp_level):
        ...
        uniprot_old_id = self.get_table_by_main_record(main_table='sg_annotation_uniprot', task_id=task_id, type=self.anno_type)
        self.bind_object.logger.info("查找表格成{}".format(uniprot_old_id))
        params_select_uniprot = dict([(k,params_dict.get(k,None)) for k in ('uniprot_evalue', 'uniprot_similarity', 'uniprot_identity')])
        params_select_uniprot = json.dumps(params_select_uniprot, sort_keys=True, separators=(',', ':'))
        uniprot_id = self.add_annotation_uniprot(name=None, params=params_select_uniprot, stat_id=stat_id, result_dir=self.result_dir)
        if self.has_new:
        ...
        self.update_db_record('sg_annotation_uniprot', uniprot_id, status="end", main_id=uniprot_id)
        if uniprot_old_id:
            self.remove_table_by_main_record(main_table='sg_annotation_uniprot', _id=uniprot_old_id['_id'],  detail_table=['sg_annotation_uniprot_detail', 'sg_annotation_uniprot_pie'], detail_table_key='uniprot_id')
        else:
            self.bind_object.logger.info("未找到旧表不做删除")
    ...
    def add_annotation(self, name=None, params=None, ref_anno_path=None, new_anno_path=None, pfam_path=None, merge_tran_output=None, merge_gene_output=None):
        ...
        stat_id = self.add_annotation_stat(name=None, params=params, seq_type="new", database="nr,uniprot,pfam,cog,go,kegg")
        ...
        uniprot_id = self.add_annotation_uniprot(name=None, params=params, stat_id=stat_id)
        uniprot_path = new_anno_path + "/anno_stat/blast_uniprot_statistics"
        if os.path.exists(uniprot_path):
            evalue_path = uniprot_path + "/uniprot_evalue.xls"
            similar_path = uniprot_path + "/uniprot_similar.xls"
            gene_evalue_path = uniprot_path + "/gene_uniprot_evalue.xls"
            gene_similar_path = uniprot_path + "/gene_uniprot_similar.xls"
            self.add_annotation_uniprot_pie(uniprot_id=uniprot_id, evalue_path=evalue_path, similar_path=similar_path, seq_type="new", anno_type="transcript")
            self.add_annotation_uniprot_pie(uniprot_id=uniprot_id, evalue_path=gene_evalue_path, similar_path=gene_similar_path, seq_type="new", anno_type="gene")
        else:
            self.bind_object.set_error("新序列Uniprot注释结果文件不存在", code="537018116")
    ...
    @report_check
    def add_annotation_stat_detail(self, stat_id, stat_path, venn_path, seq_type, exp_level, gene_exp=None, trans_exp=None):
        """
        database: 进行统计的数据库
        ...
        database_venn = {
            ...
            'UniProt': 'uniprot/uniprot_venn_{}.txt'.format(tail),
            'Uniprot': 'uniprot/uniprot_venn_{}.txt'.format(tail),
        ...
        with open(stat_path, 'r') as f:
            ...
            for line in lines[1:]:
                ...
                database = ["nr", "uniprot", "pfam", "kegg", "go", "string", "cog"]
        ...
        print "trans_dict key is {}".format(trans_dict.keys())
        if self.version == "v3":
            anno_gene_list = set()
            anno_trans_list = set()
            for db in ['GO', 'KEGG', 'COG', 'NR', 'Uniprot', 'Pfam']:
    ...
    @report_check
    def add_annotation_blast_uniprot_detail(self, blast_id, seq_type, anno_type, database, blast_path, exp_level):
        if not isinstance(blast_id, ObjectId):
            if isinstance(blast_id, types.StringTypes):
                blast_id = ObjectId(blast_id)
            else:
            ...
            for line in lines[1:]:
                ...
                if flag == query_name:
                    ...
                    if exp_level.lower() == "gene" and self.trans_isgene[line[5]] == False:
                        ...
                        data_list.append(SON(data))
        collection = self.db['sg_annotation_uniprot_detail']
        if data_list:
            try:
                collection.insert_many(data_list)
                self.bind_object.logger.info("导入uniprot blast信息：%s成功!" % (blast_path))
            except Exception, e:
                self.bind_object.set_error("导入uniprot注释统计信息失败:%s" , variables=( blast_path), code="537018137")
    ...
    @report_check
    def add_annotation_uniprot(self, name=None, params=None, stat_id=None, result_dir=None):
        task_id = self.task_id
        project_sn = self.bind_object.sheet.project_sn
        if not isinstance(stat_id, ObjectId):
            if isinstance(stat_id, types.StringTypes):
        ...
        insert_data = {
            ...
            'project_sn': project_sn,
            'task_id': task_id,
            'name': name if name else 'AnnotationUniprot_' + self.anno_type + '_' + str(datetime.datetime.now().strftime("%Y%m%d_%H%M%S")),
            'type': self.anno_type,
            'params': params,
            'result_dir': result_dir,
            'status': 'start',
        ...
        collection = self.db['sg_annotation_uniprot']
        uniprot_id = collection.insert_one(insert_data).inserted_id
        self.bind_object.logger.info("add sg_annotation_uniprot!")
        return uniprot_id

    @report_check
    def add_annotation_uniprot_pie(self, uniprot_id, evalue_path, similar_path, seq_type, anno_type):
        """
        """
        if not isinstance(uniprot_id, ObjectId):
            if isinstance(uniprot_id, types.StringTypes):
                uniprot_id = ObjectId(uniprot_id)
            else:
                self.bind_object.set_error('uniprot_id必须为ObjectId对象或其对应的字符串！', code="537018144")
        if not os.path.exists(evalue_path):
        ...
        if data_list:
            try:
                collection = self.db['sg_annotation_uniprot_pie']
                collection.insert_many(data_list)
            except Exception, e:
                self.bind_object.set_error("导入uniprot库注释作图信息evalue,similar：%s、%s出错!" , variables=(evalue_path, similar_path), code="537018147")
            else:
                self.bind_object.logger.info("导入uniprot库注释作图信息evalue,similar：%s、%s成功!" % (evalue_path, similar_path))
    ...
    def add_annotation_go_all(self, go_id, seq_type, anno_type, level, r_go_path):
        ...
        if data_list1:
            try:
                ...
                self.bind_object.logger.error("导入go注释画图all信息出错：%s" % (r_go_path))
    ...
    def add_annotation_kegg_table(self, kegg_id, seq_type, anno_type, table_path):
        ...
        with open(table_path, 'rb') as r:
            ...
            for line in r:
                ...
                if len(line) >= 6:
                    insert_data['kegg_gene_id'] = line[5]
                data_list.append(insert_data)
        if data_list:
            try:
        ...
        with open(query_path, 'r') as f:
            ...
            for line in lines[1:]:
                ...
                try:
                    ...
                    data.append(('nr', None))
                try:
                    data.append(('uniprot', line[14]))
                except:
                    data.append(('uniprot', None))
                try:
...
class TestFunction(unittest.TestCase):
    ...
    """
    def test_mongo(test):
        from mbio.workflows.medical_transcriptome.medical_transcriptome_test_api import \
            MedicalTranscriptomeTestApiWorkflow
        ...
        # gene2trans = "/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/AnnotMerge/output/newannot_class/all_tran2gene.txt"
        # gene2trans_ref = "/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/AnnotMerge/output/refannot_class/all_tran2gene.txt"
        # merge_annot = "/mnt/ilustre/users/sanger-dev/sg-users/liubinxu/test_ref_rna_v2/annotation_result2/merge_annot"

        # merge_annot_v3 = "/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/output/annot_merge"
        # gene_exp = "/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/Quant/output/gene.tpm.matrix"
        # tran_exp = "/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/Quant/output/transcript.tpm.matrix"

        result_dir = "/mnt/ilustre/users/sanger-dev/sg-users/liubinxu/medical_transcriptome/annot_merge"

        new_annotation = result_dir + "/newannot_class"
        ref_annotation = result_dir + "/refannot_class"

        gene2trans = new_annotation + "/all_tran2gene.txt"
        gene2trans_ref = ref_annotation + "/all_tran2gene.txt"
        merge_annot_v3 = result_dir
        gene_exp = "/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/Quant/output/gene.tpm.matrix"
        tran_exp = "/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/Quant/output/transcript.tpm.matrix"
        ...
        wf.test_api = wf.api.api("medical_transcriptome.ref_annotation")
        params = {
            "nr_evalue": 1e-3,
            "nr_similarity": 0,
            "nr_identity": 0,
            "uniprot_evalue":1e-3,
            "uniprot_similarity": 0,
            "uniprot_identity": 0,
            "cog_evalue": 1e-3,
            "cog_similarity": 0,
            "cog_identity": 0,
        ...
        wf.test_api.species_name = "Human"
        wf.test_api.run(merge_annot_v3, gene2trans, gene2trans_ref, params, version="v1", gene_exp = gene_exp, trans_exp=tran_exp)
