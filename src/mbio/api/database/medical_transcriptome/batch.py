...
class Batch(ApiBase):
    ...
    def add_exp_batch(self, count_batch, exp_all, exp_level, main_exp_id, exp_id, sg_exp_batch_id, batch_version, task_id,params_json, s3_path):
        ...
        self.create_db_table('sg_exp_detail', detail)
        self.remove_db_record('sg_exp_detail', exp_id=exp_id, batch_version=batch_version)
        ...
        # record.remove()
        try:
            table_name = self.db['sg_exp'].find_one({'batch_main_id': exp_id, 'task_id': task_id, 'status': 'end', 'is_rmbe': True})['name']
            sg_status = self.db['sg_status']
            self.remove_db_record('sg_exp', is_rmbe=True, batch_main_id=exp_id, status='end')
            sg_status.update({'table_name': table_name, 'task_id': task_id}, {'$set': {'status': 'deleted'}}, upsert=True)
        except:
            pass
        self.update_db_record('sg_exp', exp_id, batch_version=batch_version_new)
        try:
            is_demo = self.db['sg_task'].find_one({'task_id': task_id})['is_demo']
            if is_demo == 1:
                _id = self.db['sg_exp'].find_one({'main_id': exp_id, 'task_id': task_id, 'level': exp_level})['_id']
                self.update_db_record('sg_exp', _id, batch_version=batch_version_new)
        except:
            pass

        self.update_db_record('sg_exp', main_exp_id, status='end', batch_version=batch_version_new)
        # self.update_db_record('sg_exp', main_exp_id, main_id=exp_id)
        self.update_db_record('sg_exp', main_exp_id, batch_main_id=exp_id, exp_file=s3_path)
        # self.update_db_record('sg_exp_batch', sg_exp_batch_id, insert_dict={'main_id': sg_exp_batch_id})
        self.update_db_record('sg_exp_batch', sg_exp_batch_id, status='end')

        # self.create_db_table('sg_exp_detail', exp_matrix.to_dict('r'))
        # self.update_db_record('sg_exp_batch', sg_exp_batch_id, insert_dict={'main_id': sg_exp_batch_id})


    def add_exp_batch_corr(self, corr_output_dir, batch_version, task_id, level, main_id=None, record_id=None):
        """
        ...
        self.update_db_record('sg_exp_batch', main_id, batch_version=batch_version_new)

        db = Config().get_mongo_client(mtype=self._project_type)[Config().get_mongo_dbname(self._project_type)]
        connect = db['sg_exp_batch']
        record = connect.find_one({'task_id': task_id, 'batch_version': batch_version, 'level': level})
        if record:
            corr_id = record['main_id']
            self.remove_db_record('sg_exp_batch',main_id=corr_id)
            self.remove_db_record('sg_exp_batch_corr_detail', batch_id=corr_id)
        else:
            pass
    ...
    def add_exp_batch_pca(self, exp_output_dir, batch_version, task_id, level, main_id, record_id):
        batch_version_new = batch_version+1
        ...
        if record:
            ...
            self.remove_db_record('sg_exp_batch_pca_detail', batch_id=pca_id)
        else:
            pass
        self.update_db_record('sg_exp_batch', main_id, status="end")
        return main_id

    def insert_ellipse_table(self, infile, batch_version, task_id, level, main_id, record_id):
        if not isinstance(main_id, ObjectId):
            main_id = ObjectId(main_id)
        if type(record_id) == str or type(record_id) == bytes or type(record_id) == unicode:
            record_id = ObjectId(record_id)
        ...
        try:
            ...
            if record:
                ...
                self.remove_db_record('sg_exp_batch_corr_detail', batch_id=exp_pca_ellipse_id)
            else:
                pass
