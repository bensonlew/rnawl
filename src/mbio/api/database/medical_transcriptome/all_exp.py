...
class AllExp(ApiBase):
    ...
    def add_exp(self, exp_matrix, annotation_matrix, quant_method='RSEM', level='T', group_dict=None, main_id=None, group_id=None,
                add_distribution=True, exp_type='tpm', project_sn='tsg_zjx', task_id='tsg_zjx',
        ...
        # add main table info
        if main_id is None:
            name = "Exp" + '_' + str(level) + '_' + str(quant_method) + '_' + str(exp_type.upper()) + '_'
            time_now = datetime.datetime.now()
            name += time_now.strftime("%Y%m%d_%H%M%S")
            if type(params) == dict:
            ...
            main_info = dict(
                ...
                method=quant_method,
                desc='{} exp main table'.format(level),
                created_ts=time_now.strftime('%Y-%m-%d %H:%M:%S'),
                params=params,
                libtype=lib_type,
                version="v1",
        ...
        self.update_db_record('sg_exp', query_dict=record_dict, count_file=count_path, main_id=main_id, is_rmbe=False)

        # -- add detail table info
        # 添加group_tpm(平均数)
        all_exp_pd_dicts = all_exp_pd.to_dict('index')
        group_tpm = OrderedDict()
        for seq_id in sorted(all_exp_pd_dicts):
            tmp_exp_dict = all_exp_pd_dicts[seq_id]
            for group in group_dict:
                if seq_id not in group_tpm:
                    group_tpm[seq_id] = dict()
                group_tpm[seq_id].update(
                    {group: round(sum([tmp_exp_dict[x] for x in group_dict[group]]) / len(group_dict[group]), 4)})
        group_pd = (pd.DataFrame(data=group_tpm, columns=group_tpm.keys())).T
        all_exp_pd = pd.concat([all_exp_pd, group_pd], axis=1)

        #添加注释信息
        if level == 'T':
            annot = pd.read_table(annotation_matrix, sep='\t', header=0, index_col=1)
            # all_exp_pd = pd.concat([all_exp_pd, annot], axis=1, join='inner')
            # all_exp_pd = pd.concat([all_exp_pd, annot], axis=1, join='inner')
            all_exp_pd = all_exp_pd.join(annot)
            # all_exp_pd.rename(columns={'index':'transcript_id'}, inplace=True)
        # if exp_level == 'G':
        #     annot = pd.read_table(annotation_matrix, sep='\t', header=0, index_col=0)
        #     gene_annot_pd = annot[annot['is_gene'] == 'yes'].drop(columns=['transcript_id'])
        #     all_exp_pd = pd.concat([all_exp_pd, gene_annot_pd], axis=1)
            # all_exp_pd.rename(columns={'index': 'gene_id'}, inplace=True)
        if level == 'G':
            annot = pd.read_table(annotation_matrix, sep='\t', header=0, index_col=0)
            gene_annot_pd = annot.drop(columns=['transcript_id'])
            # all_exp_pd = pd.concat([all_exp_pd, gene_annot_pd], axis=1, join='inner')
            all_exp_pd = all_exp_pd.join(gene_annot_pd)
        all_exp_pd['exp_id'] = main_id
        all_exp_pd['kind'] = ['new' if x.startswith(('MSTRG', 'TCON', 'XLOC')) else 'ref' for x in all_exp_pd.index]
        all_exp_pd = all_exp_pd.drop(columns=['is_gene'])
        all_exp_pd.reset_index(level=0, inplace=True)
        all_exp_pd = all_exp_pd.fillna("")
        if level == 'T':
            all_exp_pd.rename(columns={'index': 'transcript_id'}, inplace=True)
        if level == 'G':
            all_exp_pd.rename(columns={'index': 'gene_id'}, inplace=True)
            all_exp_pd.rename(columns={'seq_id': 'gene_id'}, inplace=True)
        all_exp_pd['batch_version'] = 0
        ...
        if add_distribution:
            ...
            self.add_distribution(exp_matrix, group_dict, params=params, level=level,
    ...
    def add_distribution(self, exp_matrix, group_dict, params=None, level='T',
                         quant_method='RSEM', project_sn='medical_transcriptome', main_id=None,
    ...
    def add_exp_pca2(self, exp_output_dir, quant_method='RSEM', exp_id=None, exp_level='T', params=None,
                     project_sn='medical_transcriptome', task_id='medical_transcriptome', main_id=None):
        if main_id is None:
            # prepare main table info
            name = "ExpPCA" + '_' + exp_level + '_' + quant_method + '_'
            time_now = datetime.datetime.now()
    ...
    def add_exp_corr2(self, corr_output_dir, exp_level='T', quant_method='RSEM', params=None, main_id=None,
                      project_sn='medical_transcriptome', task_id='medical_transcriptome'):
        """
    ...
    def add_exp_venn(self, venn_graph, venn_table=None, project_sn='medical_transcriptome', exp_level='T', main_id=None,
                     quant_method='RSEM', task_id='medical_transcriptome', params=None):
        """
    ...
    def add_diffexp(self, diff_output, exp_id=None, group_dict=None, group_id=None,
        ...
        if main_id is None:
            # add main table info
            name = "DiffExpress" + '_' + str(exp_level) + '_' + str(quant_method) + '_' + str(diff_method) + '_'
            time_now = datetime.datetime.now()
            name += time_now.strftime("%Y%m%d_%H%M%S")
            last_record  = self.db["sg_diff"].find_one({"task_id": task_id,'status':"end"}, sort=[('_id', -1)])
            if last_record:
        ...
        summary = glob.glob(os.path.join(diff_output, '*diff_summary*.xls'))
        for each in summary:
        ...
        sig_status =OrderedDict()
        scatter_dict_list = list()
        ma_dict_list = list()
        geneset_infos = dict()
        for each in sorted(diff_files):
            if each.endswith('.annot.xls'):
                continue
            ...
            if create_geneset:
                # name = ctrl + '_vs_' + test +'_All_' +str(current_flag)
                name = ctrl + '_vs_' + test + '_all_' + str(current_flag)
                ...
                if len(sig_seqs) >= 1:
                    ...
                    geneset_id = self.add_set(geneset_main_info, genet_detail_info)
                    geneset_infos[name] = str(geneset_id)
        else:
        ...
        with open(summary, "r") as f:
            ...
            if len(lines) >= 3:
                ...
                # summary_pd['gene_name'] llll= [id2gene_name[x] for x in summary_pd['seq_id']]
                # summary_pd['description'] = [id2desc[x] for x in summary_pd['seq_id']]
                # add by fwy 20200825 将yes/no和up/down分开导入
                ...
                # add scatter detail
                for each in ma_dict_list:
                    each['gene_name'] = id2gene_name[each['seq_id']]
                self.create_db_table('sg_diff_ma', ma_dict_list, tag_dict={'diff_id': main_id})
            else:
                pass
    ...
    def add_geneset_cluster(self, cluster_output_dir, gene_detail_file, main_id=None, project_sn='medical_transcriptome',
        ...
        if main_id is None:
            ...
            main_info = dict(
                ...
                status="start",
                params=params,
                type='T' if "level" not in params_dict else params_dict["level"],
            )
    ...
    def get_gene_name_dict(self, main_id, main_table_name):
        ...
        exp_level = diff_main['level']
        annot_table = self.db['sg_exp']
        # try:
        #     annot_main = annot_table.find_one({"task_id": task_id, "level": "T" , 'status': 'end'})
        # except:
        #     annot_main = annot_table.find_one({"task_id": task_id, "level": "G", 'status': 'end'})
        ...
        annot_detail = self.db['sg_exp_detail']
    ...
    def add_diffexp_noiseq(self, diff_output, exp_id=None, group_dict=None, group_id=None,
                    exp_level='T', quant_method='RSEM', diff_method='DESeq2', main_id=None,
                    project_sn='denovo_rna_v2', task_id='denovo_rna_v2', params=None,
                           create_geneset=True, nosig_info=None):
        """
        add differential analysis result to database
        :param diff_output: diffexp result dir
        :param exp_id: exp table id from POST, will be used for getting expression value
        ...
        if isinstance(main_id, types.StringTypes):
            main_id = ObjectId(main_id)
        summarys = glob.glob(os.path.join(diff_output, '*diff_summary*.xls'))
        ...
        for each in diff_files:
            ...
            # get significant diff gene set and add to database
            if create_geneset:
                # name = ctrl + '_vs_' + test + '_' + exp_level+'_'+str(current_flag)
                #modify_by_fwy 20210727
                name = ctrl + '_vs_' + test + '_all_' + str(current_flag)
                # name += '_' + quant_method + '_' + diff_method
                # time_now = datetime.datetime.now()
                # name += '_' + time_now.strftime("%Y%m%d_%H%M%S")
                ...
                if len(sig_seqs) >= 1:
                    ...
                    genet_detail_info = [{"seq_list": sig_seqs, "regulate_list": sig_regulate}]
                    genest_id = self.add_set(geneset_main_info, genet_detail_info)
                    geneset_infos[name] = genest_id
    ...
    def run1(self):
        diff_dir = "/mnt/ilustre/users/sanger-dev/workspace/20200508/DiffexpBatch_tsg_36994_7063_8355/DiffexpBatch/output"
        # diff_dir="/mnt/ilustre/users/sanger-dev/workspace/20200508/DiffexpBatch_tsg_36994_7063_8355/DiffexpBatch/output"

        # group_dict = r'{"TR1":["TR1_1", "TR1_2", "TR1_3"],"TR2": [ "TR2_1", "TR2_2", "TR2_3"], "WT": [ "WT_1", "WT_2", "WT_3"]}'.replace(
        #     '"', '\\"'),
        group_dict = r'{"TR1":["TR1_1", "TR1_2", "TR1_3"],"TR2": [ "TR2_1", "TR2_2", "TR2_3"], "WT": [ "WT_1", "WT_2", "WT_3"]}'
        diff_method = "DESeq2",
        create_geneset = False,
        pvalue_padjust = "padjust"
        params={"control_id": "5e5f02b017b2bf64fce6292f", "correct_method": "BH", "diff_method": "DESeq2",
         "exp_id": "5e5f039817b2bf64fcee6671", "exp_level": "T", "fc": "1.5",
         "group_dict": {"TR1": ["TR1_1", "TR1_2", "TR1_3"], "TR2": ["TR2_1", "TR2_2", "TR2_3"],
                        "WT": ["WT_1", "WT_2", "WT_3"]}, "group_id": "5e5f02b017b2bf64fce6292e", "is_batch": "False",
         "stat_cutoff": "0.05", "stat_type": "padjust", "submit_location": "diffexp", "task_id": "tsg_36994",
         "task_type": 2, "tpm_filter_threshold": "0.0", "type": "all"}
        self.add_diffexp(diff_dir,group_dict=group_dict,diff_method = "DESeq2",pvalue_padjust = "padjust",
                         params=params,project_sn = "188_5e5c9ecdc912d",create_geneset = False,task_id="tsg_36994",
                         exp_id=ObjectId("5e5f039817b2bf64fcee6671"))
...
class TestFunction(unittest.TestCase):
    ...
    def test_diffexp(self):
        ...
        wf.test_api = wf.api.api("medical_transcriptome.all_exp")
        # transcript_matrix = '/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/Quant/output/transcript.tpm.matrix'
        # gene_matrix = '/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/Quant/output/gene.tpm.matrix'
        # annot_matrix = '/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/output/annot_merge/allannot_class/all_annot.xls'
        diff_output = "/mnt/ilustre/users/sanger-dev/workspace/20200825/Single_Diffexp_medical_transcriptome_837/DiffexpBatch/output"
        exp_id = "5f3cd2db17b2bf367bf4bd34"
        group_id = "5f33947d17b2bf70a4af2eaf"
        group_dict = {'H1': ['H1581_1', 'H1581_2', 'H1581_3'],'H2': ['H1581_4', 'H1581_5', 'H1581_6'],'H3': ['H1581_7', 'H1581_8', 'H1581_9'],'S1': ['SNU16_1', 'SNU16_2', 'SNU16_3'],'S2': ['SNU16_4', 'SNU16_5', 'SNU16_6'],'S3': ['SNU16_7', 'SNU16_8', 'SNU16_9']}
        quant_method = "RSEM"
        diff_method = "DESeq2"
        project_sn = "test"
        task_id = "medical_transcriptome"
        params = []
        pvalue_padjust = "padjust"
        wf.test_api.add_diffexp(diff_output, exp_id=str(exp_id), group_dict=group_dict, group_id=group_id, exp_level='G',
                            quant_method=quant_method, diff_method=diff_method, project_sn=project_sn, task_id=task_id,
                            params=params, pvalue_padjust= pvalue_padjust)

    def test_exp_medical(self):
        from mbio.workflows.medical_transcriptome.medical_transcriptome_test_api import MedicalTranscriptomeTestApiWorkflow
        from biocluster.wsheet import Sheet
        import random
        ...
        wf.test_api = wf.api.api("medical_transcriptome.all_exp")
        group_dict = {'H1': ['H1581_1', 'H1581_2', 'H1581_3'],'H2': ['H1581_4', 'H1581_5', 'H1581_6'],'H3': ['H1581_7', 'H1581_8', 'H1581_9'],'S1': ['SNU16_1', 'SNU16_2', 'SNU16_3'],'S2': ['SNU16_4', 'SNU16_5', 'SNU16_6'],'S3': ['SNU16_7', 'SNU16_8', 'SNU16_9']}
        transcript_matrix = '/mnt/ilustre/users/sanger-dev/sg-users/zoujiaxun/medical_transcriptome/transcript.tpm.matrix.xls'
        gene_matrix = '/mnt/ilustre/users/sanger-dev/sg-users/zoujiaxun/medical_transcriptome/gene.tpm.matrix.xls'
        # annot_matrix = '/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/output/annot_merge/allannot_class/all_annot.xls'
        annot_matrix_gene = '/mnt/ilustre/users/sanger-dev/sg-users/liubinxu/medical_transcriptome/annot_merge/allannot_class/all_annot_gene.xls'
        annot_matrix_tran = '/mnt/ilustre/users/sanger-dev/sg-users/liubinxu/medical_transcriptome/annot_merge/allannot_class/all_annot_tran.xls'
        wf.test_api.add_exp(gene_matrix, annot_matrix_gene, group_dict=group_dict, level='G',task_id='medical_transcriptome')
        wf.test_api.add_exp(transcript_matrix, annot_matrix_tran, group_dict=group_dict, level='T', task_id='medical_transcriptome')
...
if __name__ == '__main__':
    ...
    suite.addTest(TestFunction('test_exp_medical'))
