...
class DiffGeneset(ApiBase):
    def __init__(self, bind_object):
        super(DiffGeneset, self).__init__(bind_object)
    ...
    def add_geneset_cog_detail(self, geneset_cog_table, geneset_cog_id):
        ...
        with open(geneset_cog_table, 'r') as f:
            ...
            for line in f:
                ...
                data = {
                    'diff_geneset_cog_id': ObjectId(geneset_cog_id),
                    'type': line[0],
        ...
        try:
            collection = self.db['sg_diff_geneset_cog_class_detail']
            main_collection = self.db['sg_diff_geneset_cog_class']
            if len(data_list) != 0:
    ...
    def add_go_enrich_detail(self, go_enrich_id, go_enrich_dir):
        ...
        if not isinstance(go_enrich_id, ObjectId):
            if isinstance(go_enrich_id, types.StringTypes):
                ...
                self.bind_object.set_error('go_enrich_id必须为ObjectId对象或其对应的字符串！', code="53702337")
        if not os.path.exists(go_enrich_dir):
            self.bind_object.set_error('%s所指定的路径不存在。请检查！', variables=(go_enrich_dir), code="53702338")
        data_list = []
        go_type = []
        ...
        if data_list:
            ...
            self.bind_object.set_error('GO富集没有结果', code="53702339")
    ...
    def add_kegg_enrich_detail(self, enrich_id, kegg_enrich_table, geneset_list_path, all_list_path):
        ...
        if not os.path.exists(kegg_enrich_table):
            self.bind_object.set_error('kegg_enrich_table所指定的路径:%s不存在，请检查！', variables=(kegg_enrich_table), code="53702345")
        data_list = []
    ...
    def add_kegg_regulate_pathway(self, pathway_dir, regulate_id):
        ...
        if not isinstance(regulate_id, ObjectId):
            if isinstance(regulate_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_regulate_id必须为ObjectId对象或其对应的字符串!', code="53702349")
        if not os.path.exists(pathway_dir):
            self.bind_object.set_error('pathway_dir所指定的路径:%s不存在，请检查！', variables=(pathway_dir), code="53702350")
        data_list = []
    ...
    def add_kegg_regulate_detail(self, regulate_id, kegg_regulate_table):
        ...
        if not isinstance(regulate_id, ObjectId):
            if isinstance(regulate_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_regulate_id必须为ObjectId对象或其对应的字符串!', code="53702351")
        if not os.path.exists(kegg_regulate_table):
            self.bind_object.set_error('kegg_regulate_table所指定的路径:%s不存在，请检查！', variables=(kegg_regulate_table), code="53702352")
        data_list = []
    ...
    def add_kegg_enrich_stat(self, main_table_id, geneset_id, kegg_stat_xls, gene_kegg_level_table_xls, work_dir,
                             geneset_type, source=None):
        # 通过判断传入的geneset_id的个数来确认取数据的位置，确认是一个还是两个基因集，然后现在分情况讨论
        # 以后mongo出现NaN的时候，通过fillna更改的时候，尽量靠近插入mongo库那一步，测试发现二者之间如果还进行读写操作，会导致
        # NaN改不过来的情况
        ...
        if not isinstance(main_table_id, ObjectId):
            if isinstance(main_table_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_main_table_id必须为ObjectId对象或其对应的字符串!', code="53702359")
        if not os.path.exists(gene_kegg_level_table_xls):
            self.bind_object.set_error('gene_kegg_level_table_xls所指定的路径:%s不存在，请检查！', variables=(gene_kegg_level_table_xls), code="53702360")
        ...
        if is_diff is False and genesets_num == 1:
            ...
            appended_data_new_2.to_csv(work_dir + "/" + "kegg_statistic", sep='\t', index=False)
            data_new = appended_data_new_2.to_dict('records')
            # data_new = a.to_dict('records')
            self.create_db_table('sg_geneset_kegg_class_statistic', data_new)
            self.bind_object.logger.info("完成class分类导表")
