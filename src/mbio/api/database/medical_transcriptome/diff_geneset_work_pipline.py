...
class DiffGenesetWorkPipline(ApiBase):
    ...
    def add_geneset_analysis_table(self, geneset_info):
        ...
        if "reactome" in self.total_analysis:
            ...
            if has_reactome_class_result:
                ...
                try:
                    graph_dir = os.path.join(self.s3_output,'01Diff_Express', '03DiffExpress_Geneset_Annotion', '03Reactome',
                                             geneset_info["geneset_name"] ,
                                             "Reactome_pathways")
                    ...
                    self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
                    self.bind_object.set_error("更新sg_diff_geneset_reactome_class件graph_dir失败")
            else:
            ...
            if has_reactome_enrich_result:
                ...
                try:
                    graph_dir = os.path.join(self.s3_output,'01Diff_Express', '04DiffExpress_Geneset_Enrich', '03Reactome',
                                             geneset_info["geneset_name"] ,
    ...
    def add_geneset_reactome_enrich(self, main_id, geneset_names, geneset_reactome_table, svg_path):
        ...
        try:
            ...
            self.bind_object.logger.info("导入reactome enrich ：%s成功!" % ("reactome enrich"))
...
#------------------蛋白相关复制过来的函数---------------------------
    ...
    def add_ipath_detail(self, main_table_id, ipath_input, geneset):
        ...
        try:
            ...
            self.bind_object.logger.info("导入ipath：%s成功!" % (ipath_input))
    ...
    def add_circ_detail(self, main_table_id, circ_input, enrich_type):
        ...
        try:
            ...
            self.bind_object.logger.info("导入circ_detail：%s成功!" % (circ_input))
    ...
    def add_circ_graph(self, main_table_id, circ_input, enrich_type):
        ...
        try:
            ...
            self.bind_object.logger.info("导入circ：%s成功!" % (circ_input))
    ...
    def update_circ_main(self, main_table_id, circ_zscore_input, enrich_type):
        ...
        try:
            ...
            self.bind_object.logger.info("导入ipath：%s成功!" % (main_table_id))
    ...
    def add_single_geneset_cluster(self,cluster_analysis_result):
        ...
        try:
            self.insert_geneset_info(geneset_id, "sg_diff_geneset_cluster",cluster_id)
        except Exception as e:
            self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
            self.bind_object.set_error("sg_diff_geneset_cluster加入sg_geneset_infos创建失败")
    ...
    def add_geneset_cluset_main_table(self,geneset_id,level):
        params_json = {
            "task_id": self.task_info["task_id"],
            "submit_location": "diffgenecluster",
            "exp_id" : self.exp_id,
            "task_type": 2,
            "group_id": json.loads(self.diff_table_info["params"])["group_id"],
            "level": level,
        ...
        try:
            main_table_id = self.insert_main_table("sg_diff_geneset_cluster", mongo_data)
        except Exception as e:
            self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
            self.bind_object.set_error("sg_diff_geneset_cluster主表创建失败")
        return main_table_id
    ...
    def add_diff_geneset_venn(self):
        if len(self.genesets_ids) >1:
            params_json = {
                "task_id": self.task_info["task_id"],
                "submit_location": "diffgenevenn",
                "task_type": 1,
                "level": "G",
                # "use_group": "no",
                # "geneset_ids": self.genesets_ids,
                "geneset_ids": ','.join(self.genesets_ids)
            }
            ...
            try:
                main_table_id = self.insert_main_table("sg_diff_geneset_venn", mongo_data)
            except Exception as e:
                self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
                self.bind_object.set_error("sg_diff_geneset_cluster主表创建失败")
            conn = self.db['sg_diff_geneset_venn']
            ...
            for geneset_id in self.genesets_ids:
                try:
                    self.insert_geneset_info(geneset_id, "sg_diff_geneset_venn", main_table_id)
                except Exception as e:
                    self.bind_object.logger.error("sg_diff_geneset_venn 的 {} 插入sg_diff_geneset_venn失败".format(geneset_id))
                    self.bind_object.logger.info('error:\t {} \n {}'.format(repr(e), str(traceback.format_exc())))
                    self.bind_object.set_error("sg_diff_geneset_venn 的 {} 插入sg_diff_geneset_venn失败".format(geneset_id))
            return main_table_id
