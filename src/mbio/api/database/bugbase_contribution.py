...
class BugbaseContribution(Base):
    ...
    def add_contribute(self, geneset_id, fun_tax_file, tax_species_old, main=True, main_table_id=None, name=None,
        ...
        if main:
            ...
            try:
                collection = self.db['sg_bugbase_contribution ']
                contribute_id = collection.insert_one(insert_data).inserted_id
            except Exception, e:
                self.bind_object.logger.error('导入sg_bugbase_contribution 主表异常:{}'.format(e))
                self.bind_object.set_error("导入sg_bugbase_contribution 主表异常")
            print contribute_id
    ...
    def add_contribute_detail(self, contribute_id, fun_tax_file,tax_species_old, project_sn='bugbase',task_id='bugbase',params='bugbase',update_main=True):
            ...
            for i in data[1:]:
                ...
                if total != 0:
                    insert_data = {
                        ...
                        'tax_abundance': float(tax_file[i.strip().split("\t")[1]]) if i.strip().split("\t")[1] in tax_file else 0
                    }
                    for x in range(0, len(data[0].strip().split("\t")) - 3):
        ...
        try:
            collection1 = self.db['sg_bugbase_contribution_detail']
            ...
            self.bind_object.logger.error("导入sg_bugbase_contribution_detail表格信息出错:%s" % (e))
            self.bind_object.set_error("导入sg_bugbase_contribution_detail表信息出错", code="52800506")
        else:
            self.bind_object.logger.info("导入sg_bugbase_contribution_detail表格信息成功!")
