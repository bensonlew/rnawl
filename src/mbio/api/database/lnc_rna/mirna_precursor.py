...
class MirnaPrecursor(ApiBase):
    ...
    def add_mirna_precursor(self, tabular, params=None):
        ...
        desc = 'miRNA precursor main table'
        if not params:
            params = json.dumps({
                'task_id': task_id,
        ...
        main_id = self.create_db_table('sg_mirna_precursor', [main_info])
        df = pd.read_table(tabular)
        df['mir_pre_id'] = main_id
        self.create_db_table('sg_mirna_precursor_detail', df.to_dict('r'))
        self.update_db_record('sg_mirna_precursor', main_id, main_id=main_id, status='end')
