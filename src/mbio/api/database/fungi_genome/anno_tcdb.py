...
class AnnoTcdb(Base):
    ...
    def add_anno_tcdb(self, anno_tcdb_path, main=False, task_id=None, project_sn=None, main_id=None,
        ...
        if main:
            ...
            if main_id is None:
                ...
                self.bind_object.set_error("main为False时需提供main_id!", code="52101401")
            if not isinstance(main_id, ObjectId):
                main_id = ObjectId(main_id)
        if specimen_id + '_tcdb_anno.xls' in os.listdir(anno_tcdb_path):
            anno_file = anno_tcdb_path + '/' + specimen_id + '_tcdb_anno.xls'
        elif specimen_id + '_whole_genome_tcdb_anno.xls' in os.listdir(anno_tcdb_path):
            anno_file = anno_tcdb_path + '/' + specimen_id + '_whole_genome_tcdb_anno.xls'
        else:
            #raise Exception("找不到TCDB注释文件")
            self.bind_object.set_error("找不到TCDB注释文件", code="52101402")
    ...
    def add_anno_tcdb_detail(self, detail_file, main_id=None, update_id=None):
            ...
            for line in lines[1:]:
                ...
                data.extend(
                    [("gene_id", line[0]), ("location", line[1]), ("specimen_id", self.specimen_id), ("tcdb", line[3]),
                     ...
                     ("identity", float(line[8])), ("evalue", float(line[9])), ("score", float(line[10]))])
                data_son = SON(data)
        ...
        try:
            ...
            self.bind_object.set_error("导入anno_tcdb_detail%s信息出错:%s" , variables=(detail_file, e), code="52101403")
        else:
        ...
        try:
            ...
            main_collection = self.db['anno_tcdb']
            ...
            self.bind_object.set_error("导入anno_tcdb_stat%s信息出错:%s" , variables=(detail_file, e), code="52101404")
        else:
