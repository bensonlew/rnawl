...
class AnnoPhi(Base):
    ...
    def add_anno_phi(self, anno_phi_path, main=False, task_id=None, project_sn=None, main_id=None,
        ...
        if main:
            ...
            if main_id is None:
                ...
                self.bind_object.set_error("main为False时需提供main_id!", code="52101001")
    ...
    def add_anno_phi_type(self, type_file, main_id=None):
        ...
        try:
            ...
            self.bind_object.set_error("导入anno_phi_type%s信息出错:%s" , variables=(type_file, e), code="52101002")
        else:
    ...
    def add_anno_phi_pie(self, pie_file, main_id=None):
        ...
        try:
            ...
            self.bind_object.set_error("导入anno_phi_type%s信息出错:%s" , variables=(pie_file, e), code="52101003")
        else:
    ...
    def add_anno_phi_detail(self, detail_file, main_id=None, update_id=None):
            ...
            for line in lines[1:]:
                ...
                data.extend(
                    [("gene_id", line[0]), ("location", line[1]), ("specimen_id", self.specimen_id), ("phi", line[3]),
                     ...
                     ("type", line[8]),
                     ("host_des", line[9]), ("host_id", line[10]), ("host_species", line[11]), ("function", line[12]),
                     ("identity", float(line[13])), ("evalue", float(line[14]))])
                data_son = SON(data)
                data_list.append(data_son)
                if update_id:
        ...
        try:
            ...
            main_collection = self.db['anno_phi']
            ...
            self.bind_object.set_error("导入anno_phi_detail%s信息出错:%s" , variables=(detail_file, e), code="52101004")
        else:
