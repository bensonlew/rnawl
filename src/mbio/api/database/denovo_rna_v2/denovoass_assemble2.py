...
class DenovoassAssemble2(ApiBase):
    def __init__(self, bind_object):
        super(DenovoassAssemble2, self).__init__(bind_object)
    ...
    def set_result_dir(self, result_dir, result_unigene_dir, evolution_dir, evolution_unigene_dir):
        ...
        for key, value in self.result_file.items():
            if os.path.exists(value):
                print "{} 文件为 {}".format(key, value)
                pass
            else:
                self.bind_object.set_error('结果文件%s 不存在，请检查', variables=(value), code="52001901")
    ...
    def check_id(self, object_id):
        if not isinstance(object_id, ObjectId):
            if isinstance(object_id, types.StringTypes):
                ...
                self.bind_object.set_error('assemble_id必须为ObjectId对象或其对应的字符串！', code="52001902")
        return object_id
            ...
            for line in lines[1:]:
                ...
                if line[0] == "BUSCO score":
                    ...
                    if line[1].startswith("C:"):
                        stat_match = re.search(r'C:(.*)\[S:(.*),D:(.*)\],F:(.*),M:(.*),n:(.*)', line[1])
                        stat_c = stat_match.group(1)
                        stat_d = stat_match.group(3)
...
class TestFunction(unittest.TestCase):
    ...
    def test_mongo(self):
        ...
        test_dir = '/mnt/ilustre/users/sanger-dev/workspace/20190618/DenovoAssemble_denovo_ass6741/output/assemble'
        evolution_dir = '/mnt/ilustre/users/sanger-dev/workspace/20190618/DenovoAssemble_denovo_ass6741/output/filter_evaluation'
        wf.IMPORT_REPORT_DATA = True
        wf.IMPORT_REPORT_AFTER_END = False
        wf.test_api = wf.api.api("denovo_rna_v2.denovoass_assemble")
...
if __name__ == '__main__':
    ...
    test_dir = '/mnt/ilustre/users/sanger-dev/workspace/20190618/DenovoAssemble_denovo_ass6741/output/assemble'
    evolution_dir = '/mnt/ilustre/users/sanger-dev/workspace/20190618/DenovoAssemble_denovo_ass6741/output/filter_evaluation'
    test_api = DenovoAssemble(None)
    test_api.run(test_dir)
    '''
