...
class SeqDetail(ApiBase):
    ...
    def build_seq_database(self, db_path, cds_file, pep_file, transcript, trans2unigene, task_id='denovo_rna_v2'):
        ...
        for cds_desc, cds_seq in cds_parser:
            ...
            t_id = cds_desc.split()[-1].split(":")[0]
            _type = cds_desc.split()[2].split(":")[1]
            cds_pos = cds_desc.split()[-1].split(":")[1]
            g_id = t2g_dict[t_id]
...
class TestFunction(unittest.TestCase):
    ...
    """
    test_dir = "/mnt/ilustre/users/sanger-dev/workspace/20190829/Denovorna_tsg_35429/AnnotOrfpfam/output/"
    test_dir2="/mnt/ilustre/users/sanger-dev/workspace/20190829/Denovorna_tsg_35429/DenovoAssemble2Filter/output"
    test_dir3="/mnt/ilustre/users/sanger-dev/workspace/20190416/Denovorna_tsg_33857/CdsTf/output"
    toolbox = SeqDetail(None)

    def test_build_seq_databse(self):
        cds = os.path.join(self.test_dir, 'all_predicted.cds.fa')
        pep = os.path.join(self.test_dir, 'all_predicted.pep.fa')
        # cds=os.path.join(self.test_dir3,"Trinity.filter.fasta.transdecoder.cds")
        # pep=os.path.join(self.test_dir3,"Trinity.filter.fasta.transdecoder.pep")
        fasta = os.path.join(self.test_dir2, 'Trinity.filter.fasta')
        trans2unigene = os.path.join(self.test_dir2, 'Trinity.filter.gene_trans_map')
        seq_db = '/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/denovo_rna_v2/test/sq_db/seqs.db'
        self.toolbox.build_seq_database(seq_db, cds, pep, fasta, trans2unigene,task_id="denovo_test_0902")
