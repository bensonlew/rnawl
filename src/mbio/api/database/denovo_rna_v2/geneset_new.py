    ...
    def add_kegg_enrich_detail(self, enrich_id, kegg_enrich_table):
        ...
        if not isinstance(enrich_id, ObjectId):
            if isinstance(enrich_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_enrich_id必须为ObjectId对象或其对应的字符串!', code="52001801")
        if not os.path.exists(kegg_enrich_table):
            self.bind_object.set_error('kegg_enrich_table所指定的路径:%s不存在，请检查！', variables=(kegg_enrich_table), code="52001802")
        data_list = []
        ...
        with open(kegg_enrich_table, 'rb') as r:
            ...
            if data_list:
                ...
                self.bind_object.set_error("kegg富集统计表没结果", code="52001803")
    ...
    def add_kegg_enrich_pic(self, main_table_id, level_path, png_dir,source):
        ...
        if not isinstance(main_table_id, ObjectId):
            if isinstance(main_table_id, types.StringTypes):
                ...
                self.bind_object.set_error('main_table_id必须为ObjectId对象或其对应的字符串！', code="52001804")
        if not os.path.exists(level_path):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(level_path), code="52001805")
        if not os.path.exists(png_dir):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(png_dir), code="52001806")
        data_list = []
        ...
        if data_list:
            try:
                ...
                self.bind_object.set_error("导入kegg注释图片信息：%s、%s出错!" , variables=(level_path, png_dir), code="52001807")
            else:
