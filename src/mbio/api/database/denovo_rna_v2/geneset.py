...
class Geneset(ApiBase):
    ...
    def add_go_enrich_detail(self, go_enrich_id, go_enrich_dir):
        ...
        if not isinstance(go_enrich_id, ObjectId):
            if isinstance(go_enrich_id, types.StringTypes):
                ...
                self.bind_object.set_error('go_enrich_id必须为ObjectId对象或其对应的字符串！', code="52002301")
        if not os.path.exists(go_enrich_dir):
            self.bind_object.set_error('%s所指定的路径不存在。请检查！', variables=(go_enrich_dir), code="52002302")
        data_list = []
        df = pd.read_table(go_enrich_dir)
        ...
        if data_list:
            ...
            self.bind_object.set_error('GO富集没有结果', code="52002303")
        self.update_db_record('sg_geneset_go_enrich', go_enrich_id,categories=categories,
                              status='end')
    ...
    def add_kegg_enrich_detail(self, enrich_id, kegg_enrich_table):
        ...
        if not isinstance(enrich_id, ObjectId):
            if isinstance(enrich_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_enrich_id必须为ObjectId对象或其对应的字符串!', code="52002304")
        if not os.path.exists(kegg_enrich_table):
            self.bind_object.set_error('kegg_enrich_table所指定的路径:%s不存在，请检查！', variables=(kegg_enrich_table), code="52002305")
        data_list = []
        ...
        with open(kegg_enrich_table, 'rb') as r:
            ...
            if data_list:
                ...
                self.bind_object.set_error("kegg富集统计表没结果", code="52002306")
    ...
    def add_go_regulate_detail(self, go_regulate_dir, go_regulate_id):
        ...
        if not isinstance(go_regulate_id, ObjectId):
            if isinstance(go_regulate_id, types.StringTypes):
                ...
                self.bind_object.set_error('go_enrich_id必须为ObjectId对象或其对应的字符串！', code="52002307")
        if not os.path.exists(go_regulate_dir):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(go_regulate_dir), code="52002308")
    ...
    def add_kegg_regulate_pathway(self, pathway_dir, regulate_id):
        ...
        if not isinstance(regulate_id, ObjectId):
            if isinstance(regulate_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_regulate_id必须为ObjectId对象或其对应的字符串!', code="52002309")
        if not os.path.exists(pathway_dir):
            self.bind_object.set_error('pathway_dir所指定的路径:%s不存在，请检查！', variables=(pathway_dir), code="52002310")
        data_list = []
    ...
    def update_db_record(self, table_name, record_id, **kwargs):
        ...
        if isinstance(record_id, types.StringTypes):
           ...
           record_id = record_id
            ...
            self.bind_object.set_error("main_id参数必须为字符串或者ObjectId类型!", code="52002311")
        conn = self.db[table_name]
    ...
    def add_kegg_regulate_new(self, main_table_id, geneset_id, kegg_stat_xls, gene_kegg_level_table_xls, work_dir, geneset_type):
        ...
        # 按照kegg官网进行一级分类的排序
        list_custom = ['Metabolism', 'Genetic Information Processing', 'Environmental Information Processing', 'Cellular Processes', 'Organismal Systems', 'Human Diseases',
        'Drug Development']
        appended_data = []
        ...
        if not isinstance(main_table_id, ObjectId):
            if isinstance(main_table_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_main_table_id必须为ObjectId对象或其对应的字符串!', code="52002312")
        if not os.path.exists(gene_kegg_level_table_xls):
            self.bind_object.set_error('gene_kegg_level_table_xls所指定的路径:%s不存在，请检查！', variables=(gene_kegg_level_table_xls), code="52002313")
                ...
                for geneset in genesets:
                    ...
                    for g in group_detail[geneset]:
                        ...
                        if not pd.isnull(g):
                            ...
                            #modified by shicaiping
                            #list remove redundancy
    ...
    def add_kegg_regulate_detail(self, regulate_id, kegg_regulate_table):
        ...
        if not isinstance(regulate_id, ObjectId):
            if isinstance(regulate_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_regulate_id必须为ObjectId对象或其对应的字符串!', code="52002314")
        if not os.path.exists(kegg_regulate_table):
            self.bind_object.set_error('kegg_regulate_table所指定的路径:%s不存在，请检查！', variables=(kegg_regulate_table), code="52002315")
    ...
    def add_ipath_detail(self, main_table_id, ipath_input, geneset, t2g=None):
        if not isinstance(main_table_id, ObjectId):
            if isinstance(main_table_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_id必须为ObjectId对象或其对应的字符串！', code="52002316")
        if not os.path.exists(ipath_input):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(ipath_input), code="52002317")
        ...
        try:
            ...
            self.bind_object.set_error("导入ipath：%s出错!" , variables=(ipath_input), code="52002318")
        else:
    ...
    def add_circ_graph(self, main_table_id, circ_input, enrich_type):
        if not isinstance(main_table_id, ObjectId):
            if isinstance(main_table_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_id必须为ObjectId对象或其对应的字符串！', code="52002319")
        if not os.path.exists(circ_input):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(circ_input), code="52002320")
        ...
        try:
            ...
            self.bind_object.set_error("导入circ_graph：%s出错!" , variables=(circ_input), code="52002321")
        else:
    ...
    def add_circ_detail(self, main_table_id, circ_input, enrich_type):
        if not isinstance(main_table_id, ObjectId):
            if isinstance(main_table_id, types.StringTypes):
                ...
                self.bind_object.set_error('main_id必须为ObjectId对象或其对应的字符串！', code="52002322")
        if not os.path.exists(circ_input):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(circ_input), code="52002323")

        circ = pd.read_table(circ_input, header=0)
        circ.drop(columns=['Gene_name'],inplace=True)
        circ.columns = ['seq_id', 'id', 'term', 'log2fc']
        circ['circ_id'] = main_table_id
        row_dict_list = circ.to_dict('records')
        main_collection = self.db['sg_geneset_circ']
        ...
        try:
            ...
            self.bind_object.set_error("导入main: %s出错!" , variables=(circ_input), code="52002324")
        else:
    ...
    def update_circ_main(self, main_table_id, circ_zscore_input, enrich_type):
        if not isinstance(main_table_id, ObjectId):
            if isinstance(main_table_id, types.StringTypes):
                ...
                self.bind_object.set_error('kegg_id必须为ObjectId对象或其对应的字符串！', code="52002325")
        if not os.path.exists(circ_zscore_input):
            self.bind_object.set_error('%s所指定的路径不存在，请检查！', variables=(circ_zscore_input), code="52002326")
        ...
        try:
            ...
            self.bind_object.set_error("更新circ主表：%s出错!" , variables=(circ_zscore_input), code="52002327")
        else:
