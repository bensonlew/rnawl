...
class DenovoassAssemble(ApiBase):
    def __init__(self, bind_object):
        super(DenovoassAssemble, self).__init__(bind_object)
        self.result_file = {}
        self.result_dir = ''
        self.filtered = 'yes'
    ...
    def set_result_dir(self, result_dir, result_unigene_dir, evolution_dir, evolution_unigene_dir):
        ...
        for key, value in self.result_file.items():
            if os.path.exists(value):
                ...
                self.bind_object.set_error('结果文件%s 不存在，请检查', variables=(value), code="52002001")
    ...
    def check_id(self, object_id):
        if not isinstance(object_id, ObjectId):
            if isinstance(object_id, types.StringTypes):
                ...
                self.bind_object.set_error('assemble_id必须为ObjectId对象或其对应的字符串！', code="52002002")
        return object_id
