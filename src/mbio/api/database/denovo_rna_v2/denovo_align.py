...
class DenovoAlign(ApiBase):
    def __init__(self, bind_object):
        super(DenovoAlign, self).__init__(bind_object)
    ...
    def set_result_dir(self, align_mudule_dir):
        ...
        for key, value in self.result_file.items():
            if os.path.exists(value):
                ...
                self.bind_object.set_error('结果文件%s 不存在，请检查', variables=(value), code="52000401")
    ...
    def check_id(self, object_id):
        if not isinstance(object_id, ObjectId):
            if isinstance(object_id, types.StringTypes):
                ...
                self.bind_object.set_error('onject_id必须为ObjectId对象或其对应的字符串！', code="52000402")
        return object_id
    ...
    def add_align(self):
        ...
        insert_data = {
            ...
            'task_id': task_id,
            'name': 'Align_RSEM_' + datetime.datetime.now().strftime('%Y%m%d_%H%M%S'),
            'desc': 'denovo比对主表',
    ...
    @report_check
    def add_align_stat(self, align_id, group=None):
        '''
        插入比对详情表
        '''
...
class TestFunction(unittest.TestCase):
    ...
    def test_mongo(self):
        ...
        test_dir = '/mnt/ilustre/users/sanger-dev/workspace/20190416/Denovorna_tsg_33857/Quant/output/'
        wf.IMPORT_REPORT_DATA = True
        wf.IMPORT_REPORT_AFTER_END = False
        print("TEST fUNCTION")
        wf.test_api = wf.api.api("denovo_rna_v2.denovo_align")
