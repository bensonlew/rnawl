    ...
    def add_network_attributes(self, file2_path, table_id=None, major=False):
        if not isinstance(table_id, ObjectId):
            if isinstance(table_id, StringTypes):
                ...
                self.bind_object.set_error("table_id必须为ObjectId对象或者其对应的字符串！", code="52000801")
        data_list = []
        data_list1 = []
    ...
    def add_network_cluster_degree(self, file1_path, file2_path, file3_path, table_id=None, major=False):
        if not isinstance(table_id, ObjectId):
            if isinstance(table_id, StringTypes):
                ...
                self.bind_object.set_error("table_id必须为ObjectId对象或其对应的字符串!", code="52000802")
        data_list = []
    ...
    def add_network_centrality(self, file_path, file1_path, table_id=None, major=False):
        if not isinstance(table_id, ObjectId):
            if isinstance(table_id, StringTypes):
                ...
                self.bind_object.set_error("table_id必须为ObjectId对象或其对应的字符串!", code="52000803")
        data_list = []
    ...
    def add_node_table(self, file_path, table_id=None, major=False):
        if not isinstance(table_id, ObjectId):
            if isinstance(table_id, StringTypes):
                ...
                self.bind_object.set_error("table_id必须为ObjectId对象或其对应的字符串!", code="52000804")
        ...
        self.update_db_record('sg_geneset_ppi', ObjectId(table_id), main_id=ObjectId(table_id))
    ...
    def add_edge_table(self, file_path, table_id=None, major=False):
        if not isinstance(table_id, ObjectId):
            if isinstance(table_id, StringTypes):
                ...
                self.bind_object.set_error("table_id必须为ObjectId对象或其对应的字符串!", code="52000805")
    ...
    def add_degree_distribution(self, file_path, table_id=None, major=False):
        if not isinstance(table_id, ObjectId):
            if isinstance(table_id, StringTypes):
                ...
                self.bind_object.set_error("table_id必须为ObjectId对象或其对应的字符串!", code="52000806")
        data_list = []
        with open(file_path, 'rb') as r:
...
class TestFunction(unittest.TestCase):
    ...
    """
    def test_mongo(test):
        from mbio.workflows.denovo_rna_v2.itraq_test_api import ItraqApiWorkflow
        ...
        data = {
            ...
            #+ str(random.randint(1,10000)),
            "type": "workflow",
            "name": "denovo_rna_v2.gene_test_api",
            "options": {
            },
        }
        ...
        wf.test_api = wf.api.api("denovo_rna_v2.geneset_ppi")

        test_dir = '/mnt/ilustre/users/sanger-dev/workspace/20180409/GenesetPpi_denovo_rna_v2_4321_9222/PpinetworkAnalysis/output'
        ...
        if not os.path.isfile(all_nodes_path):
            self.bind_object.set_error("找不到报告文件:%s", variables=(all_nodes_path), code="52000807")
        if not os.path.isfile(interaction_path):
            self.bind_object.set_error("找不到报告文件:%s", variables=(interaction_path), code="52000808")
        if not os.path.isfile(network_stats_path):
            self.bind_object.set_error("找不到报告文件:%s", variables=(network_stats_path), code="52000809")
        if not os.path.isfile(network_clustering_path):
            self.bind_object.set_error("找不到报告文件:%s", variables=(network_clustering_path), code="52000810")
        if not os.path.isfile(network_centrality_path):
            self.bind_object.set_error("找不到报告文件:%s", variables=(network_centrality_path), code="52000811")
        if not os.path.isfile(network_transitivity_path):
            self.bind_object.set_error("找不到报告文件:%s", variables=(network_transitivity_path), code="52000812")
        if not os.path.isfile(degree_distribution_path):
            self.bind_object.set_error("找不到报告文件:%s", variables=(degree_distribution_path), code="52000813")
        if not os.path.isfile(network_node_degree_path):
            self.bind_object.set_error("找不到报告文件:%s", variables=(network_node_degree_path), code="52000814")
