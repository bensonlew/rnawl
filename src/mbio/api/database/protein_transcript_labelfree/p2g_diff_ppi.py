...
import unittest
import os
from mbio.api.database.protein_transcript_labelfree.api_base import ApiBase

class P2gDiffPpi(ApiBase):
    def __init__(self, bind_object):
        super(P2gDiffPpi, self).__init__(bind_object)
        self.node2id = dict()
        self.node2acc = dict()
        self.acc2id = dict()
    ...
    def add_ppi_main_id(self, geneset_id, combine_score, gene_type, species):
        ...
        data = {
            ...
            "status": "end",
            "name": 'P2gDiffPpi_' + datetime.datetime.now().strftime("%Y%m%d_%H%M%S%f")[:-3],
    ...
    def add_network_attributes(self, file2_path, table_id=None, major=False):
        ...
        with open(file2_path, "rb") as w:
            ...
            data = [("p2g_diff_id", table_id), ("num_of_nodes", eval(data_list1[0])), ("num_of_edges", eval(data_list1[1])),
        ...
        try:
            collection = self.db["sg_p2g_diff_ppi_stat"]
                ...
                for line1 in data1:
                    ...
                    for line3 in data3:
                        ...
                        if temp1[1] == temp2[1] and temp1[1] == temp3[0]:
                            ...
                            data = [("p2g_diff_id", table_id), ("node_id", node_id), ("node_name", temp1[1]),
                                    ("degree", eval(temp1[2])), ("clustering", eval(temp2[2])), ("accession_id", temp3[2])]
                            data_son = SON(data)
                            data_list.append(data_son)
        try:
            collection = self.db["sg_p2g_diff_ppi_nodes_cluster"]
            collection.insert_many(data_list)
        except Exception as e:
                    ...
                    with open(file1_path, 'rb') as m:
                        ...
                        for line1 in data1:
                            ...
                            if temp[1] == line_data[1]:
                                node_id = line_data[0]
                                data = [("p2g_diff_id", table_id), ("node_id", node_id),
                                ...
                                data_list.append(data_son)
        try:
            collection = self.db["sg_p2g_diff_ppi_nodes_centrality"]
            collection.insert_many(data_list)
        except Exception as e:
    ...
    def add_node_table(self, file_path, table_id=None, major=False):
        ...
        nodes_table['p2g_diff_id'] = table_id
        ...
        self.create_db_table('sg_p2g_diff_ppi_node', nodes_table_list)
    ...
    def add_edge_table(self, file_path, table_id=None, major=False):
        ...
        edge_table['p2g_diff_id'] = table_id
        edge_table_list = edge_table.to_dict('records')
        self.create_db_table('sg_p2g_diff_ppi_edge', edge_table_list)
    ...
    def add_degree_distribution(self, file_path, table_id=None, major=False):
        ...
        with open(file_path, 'rb') as r:
            ...
            for line in data_line:
                ...
                data = [("p2g_diff_id", table_id), ("degree", eval(line_data[0])), ("node_num", eval(line_data[1]))]
                data_son = SON(data)
                data_list.append(data_son)
        try:
            collection = self.db["sg_p2g_diff_ppi_nodes_degreestat"]
            collection.insert_many(data_list)
        except Exception as e:
            self.bind_object.set_error("导入%s信息出错:%s" % (file_path, e))
        else:
...
class TestFunction(unittest.TestCase):
    ...
    """
    def test_mongo(test):
        from mbio.workflows.labelfree.itraq_test_api import ItraqApiWorkflow
        ...
        wf.test_api = wf.api.api("labelfree.proteinset_ppi")

        test_dir = '/mnt/ilustre/users/sanger-dev/workspace/20180409/P2gDiffPpi_labelfree_4321_9222/PpinetworkAnalysis/output'
