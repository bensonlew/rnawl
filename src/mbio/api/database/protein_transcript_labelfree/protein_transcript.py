...
class ProteinTranscript(ApiBase):
    ...
    #@report_check
    def add_relation(self, main_id = None, params=None, project_sn='protein_transcript_labelfree',
                    task_id='protein_transcript_labelfree', relation_file=None, table = 'sg_p2g_relationship', detail_id = 'rela_id'):
        insert_data = dict()
        rela_data = dict()
        ...
        insert_data = {
            ...
            'status': 'start',
            'desc': 'protein_transcript_labelfree_relationship',
    ...
    def update_task_relation(self, main_id):
        ...
        data = ({
            ...
            'created_ts': datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            'created_id': 'proteni_transcript_labelfree_controller',
            'is_delete': 0})
