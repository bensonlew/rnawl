...
class NetworkCorfd(Base):
    ...
    def add_network_corfd(self, attributes_file, geneset_id=None, main=True, main_table_id=None, params=None,
        ...
        if main:
            ...
            try:
                self.db['network_corfd'].update_one({'_id': ObjectId(main_table_id)}, {
    ...
    def add_network_corfd_link(self, network_corfd_id, network_corfd_dir, anno_type="none"):
        if not isinstance(network_corfd_id, ObjectId):  # 检查传入的NetworkCor_id是否符合ObjectId类型
            if isinstance(network_corfd_id, types.StringTypes):  # 如果是string类型，则转化为ObjectId
                ...
                self.bind_object.set_error('network_corfd_id必须为ObjectId对象或其对应的字符串！', code="52802608")
        if not os.path.exists(network_corfd_dir):  # 检查要上传的数据表路径是否存在
            self.bind_object.set_error('network_corfd_dir所指定的路径不存在，请检查！', code="52802609")
        data_list = []
        result = self.db['network_corfd'].find_one({'_id': network_corfd_id})
        if not result:
        ...
        with open(network_corfd_dir + "/corr_network_by_cut.txt", 'rb') as f:
            ...
            if "Node1_Name" in head:
                ...
                self.bind_object.set_error('corr_network_by_cut.txt文件错误！', code="52802612")
            for line in f:
                line = line.strip().split('\t')
                source1 = line[0]
                target1 = line[1]
            ...
            try:
                collection = self.db['network_corfd_link']
                collection.insert_many(data_list)
            except Exception as e:
                self.bind_object.logger.error("导入link表格%s信息出错:%s" % (network_corfd_dir, e))
                self.bind_object.set_error("导入network_corfd_dir信息出错", code="52802613")
            else:
                self.bind_object.logger.info("导入link表格%s信息成功!" % network_corfd_dir)

    def add_network_corfd_node(self, network_corfd_id, network_corfd_dir, profile1, profile2,level1=None, level2=None,fac2_second_level=None):
        ...
        with open(network_corfd_dir + "/corr_network_node_degree.txt", 'rb') as f:
            ...
            for line in f:
                ...
                if len(node_name_ori.split("|")) > 1:
                    color_level = node_name_ori.split("|")[1].strip()
                degree = line[2]
                ...
                if name_abu1.has_key(node_name):
                    ...
                    if color_level:
                        ...
                        if fac2_second_level:
                            level = fac2_second_level
                        else:
                            level = level2
                else:
                    self.bind_object.logger.info("-------------------")
                ...
                data_list.append(insert_data)
            try:
                collection = self.db['network_corfd_node']
                collection.insert_many(data_list)
            except Exception as e:
                self.bind_object.logger.error("导入node表格%s信息出错:%s" % (network_corfd_dir, e))
                self.bind_object.set_error("导入network_corfd_dir信息出错", code="52802615")
            else:
    ...
    def add_network_corfd_degree(self, network_corfd_id, network_corfd_dir):
        ...
        result = self.db['network_corfd'].find_one({'_id': network_corfd_id})
        if not result:
            self.bind_object.set_error('找不到network_corfd_nog对应的主表id', code="52802616")
        else:
            task_id = result['task_id']
        ...
        with open(network_corfd_dir + "/corr_network_degree_distribution.txt", 'rb') as f:
            ...
            for line in f:
                ...
                data_list.append(insert_data)
            try:
                collection = self.db['network_corfd_degree']
                collection.insert_many(data_list)
            except Exception as e:
                self.bind_object.logger.error("导入degree表格%s信息出错:%s" % (network_corfd_dir, e))
                self.bind_object.set_error("导入network_corfd_dir信息出错", code="52802618")
            else:
