...
class Permanova(Base):
    ...
    def add_permanova(self, file_path, main=False, task_id=None, main_id=None,
        ...
        if main:
            ...
            if main_id is None:
                self.bind_object.set_error("main为False时需提供main_id!", code="52802701")
        ...
        with open(file_path, 'r') as f:
            lines = f.readlines()
            collection_detail = self.db["permanova_detail"]
            for line in lines[1:]:
                data = [("permanova_id", main_id)]
                line = line.strip().split('\t')
                data.extend([("characteristcs", line[0]), ("sumsqs", line[1]), ("meanseq", line[2]), ("f", line[3]),
        ...
        try:
            ...
            self.bind_object.set_error("导入permanova出错", code="52802702")
        else:
