...
from mbio.packages.metagenomic.colors import rotate
...
class DiffKegg(Base):
    def __init__(self, bind_object):
        ...
        self.have_export_pic = False
        self.profile = {}
        self.query = []
        self.base_color = "#7dff24"
        self.max_profile = "0"

    @report_check
    def add_diff_kegg(self, name, task_id=None, group_name=None, graph=None, params=None):
        """
        没有实际使用，只在交互分析里面做
        :param name:
        :param task_id:
    ...
    @report_check
    def add_diff_kegg_detail(self, main_id, path):
        ...
        if not os.path.isfile(path):
            self.bind_object.set_error('所指定的文件不存在，请检查: %s' , variables=( path), code="52804702")
        data_list = []
        with open(path, 'rb') as f:
            ...
            for line in lines:
                ...
                groups = self.profile.keys()
                if line[6]:
                    for group in groups:
                        profiles = float(self.profile[group].get(line[6], 0))
                        ...
                        profile_color = rotate(self.base_color, -profiles / max_scale * 100)
                        data.append((group, profile_color))
                data = SON(data)
                data_list.append(data)
                self.query.append(line[7])
        try:
