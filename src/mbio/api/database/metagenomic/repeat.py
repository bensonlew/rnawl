...
class Repeat(Base):
        ...
        if result:
            for each in result:
                ...
                try:
                    ...
                    self.bind_object.set_error("删除主表%s信息出错:%s" , variables=(collect_name, e), code="52803501")
                else:
    ...
    def rm_each(self, collect_name, name=None, name_detail=None):
        ...
        if name:
            if not name_detail:
                self.bind_object.set_error('if name exists, name_detial must exists!', code="52803502")
        ...
        if result:
            for each in result:
                ...
                try:
                    ...
                    self.bind_object.set_error("删除主表%s信息出错:%s" , variables=(collect_name, e), code="52803503")
                else:
                    self.bind_object.logger.info("删除主表%s信息成功!" % collect_name)
