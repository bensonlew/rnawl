...
class Mlst(ApiBase):
	...
	def add_mlst(self, main_id=None, project_sn='tool_lab', task_id='tool_lab', params=None):
		if main_id is None:
			...
			main_info = dict(
				...
				params=params,
				status="start")
			main_id = self.create_db_table('mlst', [main_info])
		else:
			main_id = ObjectId(main_id)

			try:
				self.update_db_record('mlst', main_id, )
			except Exception as e:
				self.bind_object.logger.error("导入mlst数据出错:%s" % e)
			else:
				self.bind_object.logger.info("导入mlst数据成功")
		return main_id
	...
	def add_mlst_detail(self, main_id, file1, file2, sample):
		with open(file1, "r") as f:
			...
			self.create_db_table('mlst_st', data_list)
		with open (file2, "r") as g:
			...
			self.create_db_table('mlst_detail', data_list1)
		table2_dict = {
			"column": [
				{"field": "sample_name", "filter": "false", "sort": "false", "title": "sample name",
				 "type": "string"},
				...
				{"field": "ali_len", "filter": "false", "sort": "false", "title": "Alignment Length(bp)",
				 "type": "int"},
				{"field": "all_len", "filter": "false", "sort": "false", "title": "Allele Length(bp)",
				 "type": "int"},
		...
		try:
			self.update_db_record('mlst', main_id, main_id=main_id, detail_table=table2_info, stat_table=table1_info, status="end")
		except Exception as e:
			self.bind_object.logger.error("导入mlst_detail数据出错:%s" % e)
		else:
			self.bind_object.logger.info("导入mlst_detail数据成功")