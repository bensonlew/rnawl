...
class ChiSquare(ApiBase):
    def __init__(self, bind_object):
        super(ChiSquare, self).__init__(bind_object)
    ...
    def add_two_sample_detail(self, result_path, main_id,sample1,sample2):
        ...
        try:
            collection = self.db["chi_square_detail"]
            collection.insert_many(data_list)
            self.db['chi_square_sig'].insert_many(sig_list)
            self.db['chi_square_sig'].insert_many(pvalue_text_list)
            ...
            #text_data =  json.dumps({"name":"p_value"})
            ...
            self.db["chi_square"].update(
                {"_id": main_id},
                {"$set": {"main_id":main_id,
                          "table_data":json.dumps(table_data),
                          "column_data":column_data,
                          "significant_data":significant_data,
                          "pvalue_text_data":text_data}})

        except Exception as e:
            self.bind_object.logger.info("导入chi_square_detail数据出错:%s" % e)
        else:
            self.bind_object.logger.info("导入chi_square_detail数据成功")
