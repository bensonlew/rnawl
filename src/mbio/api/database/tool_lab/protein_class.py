...
class ProteinClass(ApiBase):
    def __init__(self, bind_object):
        super(ProteinClass, self).__init__(bind_object)

    @report_check
    def add_class_detail(self, class_id, class_detail):
        class_pd = pd.read_table(class_detail, header=0)
        class_pd['class_id'] = ObjectId(class_id)
        result_dict_list = class_pd.to_dict("records")
        self.create_db_table('protein_class_detail', result_dict_list)
