...
class GenomedbSearch(ApiBase):
    def __init__(self, bind_object):
        super(GenomedbSearch, self).__init__(bind_object)

    @report_check
    def add_genome_detail(self, main_id, genome_list):
        # print genome_list
        result_dict_list = list()
        for gd in genome_list:
            gd_new = gd.copy()
            gd_new.update({"genome_id": ObjectId(main_id)})
            result_dict_list.append(gd_new)
        # result_dict_list = [gd.update({"genome_id": ObjectId(main_id)}) for gd in genome_list]
        # print result_dict_list
        self.create_db_table('genomedb_search_detail', result_dict_list)
