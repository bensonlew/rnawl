...
class GenesetPpi(ApiBase):
    def __init__(self, bind_object):
        super(GenesetPpi, self).__init__(bind_object)
    ...
    def add_ppi_main_id(self, geneset_id, combine_score, gene_type, species):
        ...
        data = {
            ...
            "status": "end",
            "name": 'genesetPpi_' + datetime.datetime.now().strftime("%Y%m%d_%H%M%S%f")[:-3],
    ...
    def add_network_attributes(self, file2_path, table_id=None, major=False):
        ...
        try:
            collection = self.db["sg_geneset_ppi_stat"]
                ...
                for line1 in data1:
                    ...
                    for line3 in data3:
                        ...
                        if temp1[1] == temp2[1] and temp1[1] == temp3[0]:
                            ...
                            data_list.append(data_son)
        try:
            collection = self.db["sg_geneset_ppi_nodes_cluster"]
            collection.insert_many(data_list)
        except Exception as e:
            self.bind_object.logger.error("导入%s信息出错:%s" % (file1_path, e))
        else:
                    ...
                    with open(file1_path, 'rb') as m:
                        ...
                        for line1 in data1:
                            ...
                            if temp[1] == line_data[1]:
                                ...
                                data_list.append(data_son)
        try:
            collection = self.db["sg_geneset_ppi_nodes_centrality"]
            collection.insert_many(data_list)
        except Exception as e:
    ...
    def add_node_table(self, file_path, table_id=None, major=False):
        ...
        dict_a = {"name": "node_name", 'group': 'method', 'id': 'accession_id', "condition": {"type": "node"}}
        dict_b = json.dumps(dict_a, sort_keys=True, separators=(',', ':'))
        nodes_table_list = nodes_table.to_dict('records')
        self.create_db_table('sg_geneset_ppi_node', nodes_table_list)
        self.update_db_record('sg_geneset_ppi', ObjectId(table_id), main_id=ObjectId(table_id), node_data = dict_b)
    ...
    def add_edge_table(self, file_path, table_id=None, major=False):
        ...
        edge_table['source'] = edge_table['from'].map(lambda x:self.node2id[x])
        edge_table['target'] = edge_table['to'].map(lambda x:self.node2id[x])
        transfer_list = []
        ...
        edge_table['name'] = edge_table['source'].map(str) + '_' + edge_table['target'].map(str)
        edge_table['type'] = 'link'
        edge_table_list = edge_table.to_dict('records')
        self.create_db_table('sg_geneset_ppi_edge', edge_table_list)
        dict_a = {"name": "name", "value": 'combined_score', "condition": {"type": "link"}}
        dict_b = json.dumps(dict_a, sort_keys=True, separators=(',', ':'))
        self.update_db_record('sg_geneset_ppi', ObjectId(table_id), main_id=ObjectId(table_id), link_data = dict_b)
        columns_list = list()
        columns_list.append({'field': 'from_name', 'filter': False, 'sort': False, 'title': 'Node1', 'type': 'string'})
        columns_list.append({'field': 'to_name', 'filter': False, 'sort': False, 'title': 'Node2', 'type': 'string'})
        columns_list.append({'field': 'source', 'filter': False, 'sort': False, 'title': 'Node1_accession_id', 'type': 'string'})
        columns_list.append({'field': 'target', 'filter': False, 'sort': False, 'title': 'Node2_accession_id', 'type': 'string'})
        columns_list.append({'field': 'combined_score', 'filter': False, 'sort': False, 'title': 'Combined_score', 'type': 'float'})
        columns_list.append({'field': 'neighborhood', 'filter': False, 'sort': False, 'title': 'Neighborhood on chromosome', 'type': 'float'})
        columns_list.append({'field': 'fusion', 'filter': False, 'sort': False, 'title': 'Gene fusion', 'type': 'float'})
        columns_list.append({'field': 'cooccurence', 'filter': False, 'sort': False, 'title': 'Phylogenetic cooccurrence', 'type': 'float'})
        columns_list.append({'field': 'homology', 'filter': False, 'sort': False, 'title': 'Homology', 'type': 'float'})
        columns_list.append({'field': 'coexpression', 'filter': False, 'sort': False, 'title': 'Coexpression', 'type': 'float'})
        columns_list.append({'field': 'experiments', 'filter': False, 'sort': False, 'title': 'Experimentally determined interaction', 'type': 'float'})
        columns_list.append({'field': 'database', 'filter': False, 'sort': False, 'title': 'Database annotated', 'type': 'float'})
        columns_list.append({'field': 'textmining', 'filter': False, 'sort': False, 'title': 'Automated textmining', 'type': 'float'})
        data_columns = {'column': columns_list, 'condition': {'type': 'table'}}
        columns_data = json.dumps(data_columns)
        self.update_db_record('sg_geneset_ppi', ObjectId(table_id), main_id=ObjectId(table_id), data_detail=columns_data)
    @report_check
    def add_degree_distribution(self, file_path, table_id=None, major=False):
        ...
        with open(file_path, 'rb') as r:
            ...
            for line in data_line:
                ...
                data_list.append(data_son)
        try:
            collection = self.db["sg_geneset_ppi_nodes_degreestat"]
            collection.insert_many(data_list)
        except Exception as e:
            self.bind_object.logger.error("导入%s信息出错:%s" % (file_path, e))
        else:
...
class TestFunction(unittest.TestCase):
    ...
    """
    def test_mongo(test):
        from mbio.workflows.ref_rna_v2.itraq_test_api import ItraqApiWorkflow
        ...
        wf.test_api = wf.api.api("ref_rna_v2.geneset_ppi")

        test_dir = '/mnt/ilustre/users/sanger-dev/workspace/20180409/GenesetPpi_ref_rna_v2_4321_9222/PpinetworkAnalysis/output'
        ...
        network_centrality_path = test_dir + '/ppinetwork_topology/gene_interaction_network_centrality.txt'
        network_clustering_path = test_dir + '/ppinetwork_topology/gene_interaction_network_clustering.txt'
        network_transitivity_path = test_dir + '/ppinetwork_topology/gene_interaction_network_transitivity.txt'
        degree_distribution_path = test_dir + '/ppinetwork_topology/gene_interaction_network_degree_distribution.txt'
        network_node_degree_path = test_dir + '/ppinetwork_topology/gene_interaction_network_node_degree.txt'
