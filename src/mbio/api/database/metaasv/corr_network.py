...
class CorrNetwork(Base):
    ...
    def add_network_corfd(self, attributes_file, geneset_id=None, main=True, main_table_id=None, params=None,
        ...
        if main:
            ...
            try:
                collection = self.db['corr_network']
            ...
            try:
                collection = self.db['corr_network_node']
                collection.insert_many(data_list)
                network_corfd_id = main_table_id
            except Exception as e:
                self.bind_object.logger.error('更新corr_network_node主表attributes_file出错:{}'.format(e))
    ...
    def add_network_corfd_link(self, network_corfd_id, network_corfd_dir, anno_type="none"):
        ...
        data_list = []
        result = self.db['corr_network'].find_one({'_id': network_corfd_id})
        if not result:
        ...
        with open(network_corfd_dir + "/corr_network_by_cut.txt", 'rb') as f:
            ...
            for line in f:
                ...
                data_list.append(insert_data)
            try:
                collection = self.db['corr_network_link']
                collection.insert_many(data_list)
            except Exception as e:
    ...
    def add_network_corfd_node(self, network_corfd_id, network_corfd_dir, profile1,color_level_id):
        ...
        origin_level_name = {}
        origin_table = self.bind_object.option("otutable").prop["path"]
        with open(origin_table, "r") as otu_table:
            otu_lines = otu_table.readlines()
            for otu_line in otu_lines[1:]:
                otu_line = otu_line.strip().split("\t")
                full_otu_name = otu_line[0]
                level_otu_name = full_otu_name.split("; ")[-1]
                if level_otu_name not in origin_level_name:
                    origin_level_name[level_otu_name] = full_otu_name
        ...
        with open(network_corfd_dir + "/corr_network_node_degree.txt", 'rb') as f:
            ...
            for line in f:
                ...
                level_name = origin_level_name[node_name_ori]
                sp_names = level_name.split("; ")
                if len(sp_names) > 1:
            ...
            try:
                collection = self.db['corr_network_node']
                collection.insert_many(data_list)
                main_collection = self.db['corr_network']
    ...
    def add_network_corfd_degree(self, network_corfd_id, network_corfd_dir):
        ...
        with open(network_corfd_dir + "/corr_network_degree_distribution.txt", 'rb') as f:
            ...
            for line in f:
                ...
                data_list.append(insert_data)
            try:
                collection = self.db['corr_network_degree']
                collection.insert_many(data_list)
