...
def export_network_diff(data, option_name, dir_path, bind_obj=None):
    ...
    stat_type_value = float(bind_obj.sheet.option('stat_type_value'))
    fc_log2fc = bind_obj.sheet.option('log2fc')
    log2fc_operation = bind_obj.sheet.option('log2fc_operation')
    log2fc_value = float(bind_obj.sheet.option('log2fc_value'))
    ...
    if stat_type_value != "" and not None:
        find_params[pvalue_padjust] = {stat_type_operation: stat_type_value}
    if log2fc_value != "" and not None:
        find_params[fc_log2fc] = {log2fc_operation: log2fc_value}
...
def export_gene_type(data, option_name, dir_path, bind_obj=None):
    gene_type_path = os.path.join(dir_path, "gene_type.txt")
    bind_obj.logger.debug("正在导出基因类型表")
    collection = db['sg_exp_detail']
    main_collection = db['sg_exp']
    my_result = main_collection.find_one({"task_id": data})
    if not my_result:
        bind_obj.set_error("意外错误，未找到定量结果表！")
    exp_id = my_result["main_id"]
    results = collection.find({"exp_id": ObjectId(exp_id)})
    with open(gene_type_path, "wb") as f:
        for result in results:
            gene_id = result["seq_id"]
            gene_type = result["type"]
            f.write(gene_id + "\t" + gene_type + "\n")
    return gene_type_path
...
def export_kegg_level_table(data, option_name, dir_path, bind_obj=None):
    ...
    with open(kegg__level_path, 'wb') as w:
        w.write('Pathway_id\tgraph_id\tnumber_of_seqs\tpathway_definition\tfirst_category\thyperlink\tseq_list\tgraph_png_id\tsecond_category\n')
        for i in my_result:
            kegg_id = i["main_id"]
            bind_obj.logger.debug(kegg_id)
            if not kegg_id:
...
def export_cog_class(data, option_name, dir_path, bind_obj=None):
    ...
    genesets, table_title, task_id = get_geneset_detail(data, bind_obj)
    cog_collection = db["sg_annotation_cog"]
    cog_detail_collection = db["sg_annotation_cog_summary"]
    cog_id = cog_collection.find_one({"task_id": bind_obj.sheet.option('task_id'), 'type': bind_obj.sheet.option('type')})["main_id"]
    print("cog_id:", cog_id)
    if cog_detail_collection.find_one({'cog_id': cog_id}):
        cog_results = cog_detail_collection.find({'cog_id': cog_id})
        # cog_sum = cog_stat_collection.find({'cog_id': cog_id})
    else:
        bind_obj.set_error("意外错误，task_id:{}未找到相关COG注释信息！".bind_obj.sheet.option('task_id'))
    new_table_title = []
...
def get_geneset_detail(data, bind_obj):
    ...
    for geneset_id in data.split(","):
        ...
        genesets[geneset_name] = ['all_is_gene']
        collection = db['sg_geneset_detail']
...
def export_go_class(data, option_name, dir_path, bind_obj=None):
    ...
    genesets, names, task_id = get_geneset_detail(data, bind_obj)
    ...
    try:
        if bind_obj.sheet.option('version') >= "v3":
            one_record = go_level_collection.find_one({'go_id': go_id})
        else:
            one_record = go_level_collection.find_one({'go_id': go_id, "level": 2})
    except:
        one_record = ''
    if not one_record:
        bind_obj.set_error("意外错误:未找到go_id为{}的基因集信息 {}".format(go_id, bind_obj.sheet.option('version')))
    new_table_title = []
    for gt in genesets:
    ...
    with open(go_path, "wb") as w:
        ...
        for item in term_list:
            ...
            for gr in go_results:
                if gr["term_type"] == item:
                    seq_list = set(gr["seq_list"])
                    write_line = {}
                    for gt in genesets:
                    ...
                    if len(write_line):
                        w.write("{}\t{}\t{}\t".format(gr["term_type"], gr["go_term"], gr["go_id2"]))
                        for tt in genesets:
...
def export_go_detail(data, option_name, dir_path, bind_obj=None):
    go_path = os.path.join(dir_path, 'go_detail_table.xls')
    bind_obj.logger.debug("正在导出{}".format(go_path))
    genesets, names, task_id = get_geneset_detail(data, bind_obj)
    go_collection = db["sg_annotation_go"]
    go_detail_collection = db["sg_annotation_go_list"]
    go_id = go_collection.find_one({"task_id": bind_obj.sheet.option('task_id'), "type": bind_obj.sheet.option('type')})["main_id"]
    try:
        one_record = go_detail_collection.find_one({'go_id': go_id})
    except:
        one_record = ''
    if not one_record:
    ...
    genesets_all = []
    for gt in genesets:
        genesets_all.extend(genesets[gt][1])

    with open(go_path, "wb") as w:
        w.write("gene_id\tgos_list\tgo_names\n")

        go_results = go_detail_collection.find({'go_id': go_id})
        for gr in go_results:
            if gr["gene_id"] in genesets_all:
                w.write("{}\t{}\t{}\n".format(gr["gene_id"], gr["gos_list"], gr["gos_name"]))
    return go_path
...
def export_add_info(data, option_name,dir_path, bind_obj=None):
    ...
    result = col.find_one({"task_id":data, "type": bind_obj.sheet.option('type')})
...
def export_multi_gene_list(data, option_name, dir_path, bind_obj=None):
    ...
    if len(geneset_id) == 1 and source == 'diff_exp':
        geneset_id = geneset_id[0]
        ...
        seq_list = [str(i) for i in results["seq_list"]]
        ...
        f.write(geneset_name + '_down\t' + ','.join(down_list) + '\n')
        f.write(geneset_name + '_up\t' + ','.join(up_list) + '\n')
    else:
        for n, gi in enumerate(geneset_id):
            ...
            f.write(",".join([str(i) for i in results["seq_list"]]) + "\n")
    return multi_geneset_path

def export_multi_gene_list_new(data, option_name, dir_path, bind_obj=None):
    geneset_id = data.split(",")
    multi_geneset_path = dir_path + "/multi_geneset_list"
    collection = db['sg_geneset_detail']
    main_collection = db['sg_geneset']
...
def export_exp_matrix_prok(data, option_name, dir_path, bind_obj=None):
    ...
    level_seq_list = list()
    if "+" in exp_level:
        exp_level = exp_level.split("+")
        for i in exp_level:
            level_seq_list.append(unicodedata.normalize('NFKD', i).encode('ascii', 'ignore'))
    elif ";" in exp_level:
        exp_level = exp_level.split(";")
        for i in exp_level:
            level_seq_list.append(unicodedata.normalize('NFKD', i).encode('ascii', 'ignore'))
    else:
        level_seq_list = [unicodedata.normalize('NFKD', exp_level).encode('ascii', 'ignore')]
    group_dict = json.loads(group_dict, object_pairs_hook=OrderedDict)
    samples = list()
    for each in group_dict:
        samples += group_dict[each]
    ...
    if not isinstance(data, ObjectId):
        data = ObjectId(data)
    exp_records = conn.find({"exp_id": data, "type": {"$in": level_seq_list}},  target_cols)
    exp_matrix = pd.DataFrame(list(exp_records))
    print(exp_matrix.head(3))
    print(level_seq_list)
    exp_matrix = exp_matrix.set_index('seq_id')
    output = os.path.join(dir_path, option_name)
    exp_matrix.to_csv(output, sep='\t', header=True, index=True)
    print('success to export expression matrix')
...
def export_geneset_exp_matrix(data, option_name, dir_path, bind_obj=None):
    ...
    if geneset:
        ...
        bind_obj.set_error('The number of geneset can not be 0')
    output = os.path.join(dir_path, option_name)
    exp_matrix.to_csv(output, sep='\t', header=True, index=True)
    print('success to export expression matrix')
    return output
...
def export_geneset_from_query(data, option_name, dir_path, bind_obj=None):
    ...
    output = os.path.join(dir_path, "all_genes.txt")
...
def export_go_enrich_matrix(data, option_name, dir_path, bind_obj=None):
    ...
    target_cols = OrderedDict(go_id=1, go_type=1, description=1, p_corrected=1, p_uncorrected=1, seq_list=1, depth=1, _id=0)
...
def get_all_pep_seq(data, option_name, dir_path, bind_obj=None):
    ...
    if re.match(r'^\w+://\S+/.+$', pep_db_path):
        download_s3_file(pep_db_path, os.path.join(bind_obj.work_dir, "pep.db"))
        pep_db_path = os.path.join(bind_obj.work_dir, "pep.db")
    import sqlite3
...
def get_gene_detail(data, option_name, dir_path, bind_obj=None):
    ...
    result = annot_detail.find(query_dict, result_dict)
    result_pd = pd.DataFrame(list(result))
    result_pd['transcript_id'] = result_pd['gene_id']
...
# 赵-测试
def export_diff_genes(data, option_name, dir_path, bind_obj=None):  #此函数待定 不一定对
    """
    取gene——list参数检测:{'diff_id': ObjectId('5ba33849a4e1af2f8db9a82c'), '$or': [{'type': u'WT|rcsBKO'}]}

    :param data: {'main_table_name': 'sg_diff', 'task_id': u'tsg_32038'}, main_id： sg_diff, task_id: tsg_32038
    :param option_name:
    :param dir_path:
    :param bind_obj:
    :return:
    """
    file_path = os.path.join(dir_path, '{}.txt'.format(option_name))
    main_table = data['main_table_name']
    task_id = data['task_id']
    bind_obj.logger.debug("data参数:%s, main_id： %s, task_id: %s" % (data, main_table, task_id))
    bind_obj.logger.debug("正在导出计数矩阵:%s" % file_path)
    collection = db[main_table]
    result = collection.find_one({'task_id':'tsg_32038'}, {'main_id': 1, 'cmp_list': 1, '_id': 0})
    if not result:
        bind_obj.set_error("意外错误，task_id:{}在{}中未找到！".format(task_id, main_table))

    filter_dict = {'diff_id': result['main_id'], '$or': [{'compare': t} for t in result['cmp_list']]}
    bind_obj.logger.debug("取gene——list参数检测:%s" % filter_dict)

    detail_res = db[main_table + '_detail'].find(filter_dict, {'seq_id': 1,'_id': 0})
    with open(file_path, 'w') as outfile:
        for dic in detail_res:
            gene = dic['seq_id'].strip()
            if gene:
                outfile.write(gene + '\n')
    return file_path

def get_anno_file_path(data, option_name, dir_path, bind_obj=None):  #此函数待定 不一定对
    collection = db['sg_annotation_stat']
    result = collection.find_one(data)
    if not result:
        bind_obj.set_error("意外错误，{}在sg_annotation_stat中未找到！".format(ObjectId(data)))
    res_dir = result['result_dir']
    if not os.path.isdir(res_dir):
        bind_obj.set_error("路径异常，{}不存在！".format(ObjectId(data)))
    file_path = os.path.join(res_dir, 'anno_stat', 'all_anno_detail.xls')
    return file_path
...
def export_exp_other_matrix(data, option_name, dir_path, bind_obj):
    exp_id = data
    collection = db['sg_exp_detail']
    other_cols = OrderedDict(seq_id=1, _id=0, type=1, gene_name=1)
