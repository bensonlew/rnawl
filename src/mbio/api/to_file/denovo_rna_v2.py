...
def export_diff_genesets(data, option_name, dir_path, bind_obj=None):
    '''
    导出差异分析genesets
    '''
    diff_genesets = dict()
    ...
    with open(output, "w") as w:
        w.write("\t".join(["compare_group", "seq_list", "regulate_list"]) + "\n")
        for compare_group in diff_genesets:
            w.write("\t".join([compare_group.replace("|", "_vs_"), ",".join(diff_genesets[compare_group]["seq_list"]),
...
def export_go_list(data, option_name, dir_path, bind_obj=None):
    ...
    my_result = db["sg_annotation_go"].find_one(
...
def export_kegg_level_table(data, option_name, dir_path, bind_obj=None):
    ...
    with open(kegg__level_path, 'wb') as w:
        ...
        for i in my_result:
            ...
            if not kegg_id:
                bind_obj.set_error("意外错误，annotation_kegg_id:{}在sg_annotation_kegg中未找到！".format(kegg_id))
            results = db["sg_annotation_kegg_level"].find({"kegg_id": kegg_id, "anno_type": bind_obj.sheet.option("geneset_type")})
            one_record = db['sg_annotation_kegg_level'].find_one({'kegg_id': kegg_id, 'anno_type': bind_obj.sheet.option("geneset_type")})
            if not one_record:
            ...
            for result in results:
                if 'hyperlink' not in result:
                    ...
                    result['hyperlink'] = 'None'
                w.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(result['pathway_id'], result['graph_id'], result['number_of_seqs'], result['pathway_definition'],
                result['first_category'], result['anno_type'], result['hyperlink'], result['seq_list'], result['graph_png_id'], result['second_category']))
...
def export_cog_class(data, option_name, dir_path, bind_obj=None):
    ...
    bind_obj.logger.debug("bind_obj: {}".format(bind_obj))
    bind_obj.logger.debug("genesets: {}".format(genesets))
    bind_obj.logger.debug("table_title: {}".format(table_title))
    bind_obj.logger.debug("task_id: {}".format(task_id))
    bind_obj.logger.debug("geneset_type: {}".format(geneset_type))
    cog_id = \
    cog_collection.find_one({"task_id": bind_obj.sheet.option("task_id"), 'type': bind_obj.sheet.option('type')})[
        "main_id"]
    print("cog_id:", cog_id)
        ...
        for cr in cog_results:
            ...
            for gt in genesets:
                ...
                if not len(nog_count) + len(cog_count) == 0:
                    write_line[gt] = [str(len(cog_count)), str(len(nog_count)), ";".join(cog_count),
                                      ";".join(nog_count)]
            print('here:', nog_list)
...
def get_geneset_detail(data, bind_obj):
    ...
    for geneset_id in data.split(","):
        ...
        if not results:
            bind_obj.set_error("基因集详情在sg_geneset_detail中未找到！请核实基因集来源并联系技术!")
...
def export_go_class(data, option_name, dir_path, bind_obj=None):
    ...
    go_id = \
    go_collection.find_one({"task_id": bind_obj.sheet.option("task_id"), "type": bind_obj.sheet.option('type')})[
        "main_id"]
...
# -----added for wgcna------
def export_geneset_exp_matrix2(data, option_name, dir_path, bind_obj=None):
    """ gdq
...
def export_wgcna_exp_matrix(data, option_name, dir_path, bind_obj=None):
    ...
    exp_matrix_s3 = prepare_main['exp_matrix']
    # exp_matrix (e.g.) ==>
    # "s3://denovornav2/files/m_188/188_5b3c7b570311f/tsg_32458/interaction_results/WgcnaPrepare_T_RSEM_TPM_20181024_101703/exp_matrix_after_filtering.txt"
    exp_matrix = download_s3_file(exp_matrix_s3, os.path.join(bind_obj.work_dir, "exp_matrix_after_filtering.txt"))
...
def export_go_enrich_matrix(data, option_name, dir_path, bind_obj=None):
    ...
    target_cols = OrderedDict(go_id=1, go_type=1, discription=1, p_corrected=1, p_uncorrected=1, gene_list=1, depth=1,
                              _id=0)
    ...
    seq_list2 = [i.replace(';', '|') for i in go_enrich_matrix['gene_list']]
    go_enrich_matrix['gene_list'] = seq_list2
    # exp_matrix = exp_matrix.loc[geneset, :]
    output = os.path.join(dir_path, option_name)
...
def export_go_enrich_matrix2(data, option_name, dir_path, bind_obj=None):
    go_enrich_id = str(data)
...
def export_kegg_enrich_info_ids(data, option_name, dir_path, bind_obj=None):
    ...
    if len(enrich_records) == 0:
        bind_obj.set_error("filted_records is empty")
...
def export_kegg_enrich_info_filter(data, option_name, dir_path, bind_obj=None):
    ...
    if bind_obj.sheet.option("stat_threshold_value") and bind_obj.sheet.option("stat_numbers_value"):
        print("stat_threshold_value is {},stat_numbers_value is {}").format(
            bind_obj.sheet.option("stat_threshold_value"), bind_obj.sheet.option("stat_numbers_value"))
    ...
    if len(enrich_records) < v2:
        bind_obj.set_error("filted_records less than {} ".format(bind_obj.sheet.option("stat_numbers_value")))
    enrich_matrix = pd.DataFrame(list(enrich_records))
    enrich_matrix = enrich_matrix.set_index('id')
