...
def export_go_list(data, option_name, dir_path, bind_obj=None):
    ...
    if not one_record:
        bind_obj.set_error("生成gos_list出错：annotation_id:%s在sg_annotation_gos_list中未找到！",
                                   variables=(ObjectId(go_id)), code="51008134")
    with open(go_list_path, "wb") as w:
        for result in results:
            ...
            if go_anno_type == geneset_type:
                #modify by fwy 20211201 为了适应部分go注释有多余的符号
                go_list = go_list.lstrip(";")
                go_list = go_list.rstrip(";")
...
def export_kegg_table(data, option_name, dir_path, bind_obj=None):
    ...
    with open(kegg_path, 'wb') as w:
        ...
        for main_table in my_result:
            ...
            if not my_result:
                bind_obj.set_error("意外错误，annotation_kegg_id:%s在sg_annotation_kegg中未找到！", variables=(kegg_id),
                                           code="51008135")
            ...
            if not one_record:
                bind_obj.set_error("生成kegg_table出错：kegg_id:%s在sg_annotation_kegg_table中未找到！",
                                           variables=(ObjectId(kegg_id)), code="51008136")
            for result in results:
        ...
        for i in my_result:
            ...
            if not one_record:
                ...
                one_record = db['sg_annotation_kegg_level'].find_one(
                    {'kegg_id': kegg_id, "seq_type": "ref", 'anno_type': bind_obj.sheet.option("geneset_type")})
            if not one_record:
                bind_obj.set_error("生成kegg_table出错：kegg_id:%s在sg_annotation_kegg_level中未找到！",
                                           variables=(ObjectId(kegg_id)), code="51008138")
            for result in results:
                if 'hyperlink' not in result:
...
def get_geneset_detail(data, bind_obj):
    ...
    for geneset_id in data.split(","):
        ...
        if results["seq_list"] is None:
            bind_obj.set_error("基因集为空，请重新选择基因集进行分析")
        elif len(results["seq_list"]) == 0:
            bind_obj.set_error("基因集为空，请重新选择基因集进行分析")
        ...
        for item in term_list:
            if go_level_collection.find_one({'go_id': go_id, "seq_type": "all", "level": 2, "anno_type": seq_type}):
                go_results = go_level_collection.find(
                    {'go_id': go_id, "seq_type": "all", "level": 2, "anno_type": seq_type})
            else:
                go_results = go_level_collection.find(
...
def export_group_table_by_detail(data, option_name, dir_path, bind_obj=None):
    ...
    if not group_schema:
        bind_obj.set_error("无法根据传入的group_id:%s在sg_specimen_group_compare表里找到相应的记录", variables=(data),
                                   code="51008144")
    ...
    with open(file_path, "ab") as f:
        for k in table_dict:
            for sp_id in table_dict[k]:
                ...
                if not sp:
                    bind_obj.set_error("group_detal中的样本_id:%s在样本表%s中未找到", variables=(sp_id, sample_table_name),
                                               code="51008145")
                else:
...
def export_control_file(data, option_name, dir_path, bind_obj=None):  # 此函数待定 不一定对
    ...
    if not result:
        bind_obj.set_error("意外错误，control_id:%s在sg_specimen_group_compare中未找到！", variables=(ObjectId(data)),
                                   code="51008148")
    ...
    if group_id not in ['all', 'All', 'ALL']:
        ...
        if not g_result:
            bind_obj.set_error("意外错误，control_file的group_id:%s在sg_specimen_group中未找到！", variables=(group_id),
                                       code="51008149")
...
def export_exp_matrix(data, option_name, dir_path, bind_obj=None):
    ...
    if not isinstance(group_dict, dict):
        bind_obj.set_error("传入的group_dict不是字典形式")
    ...
    if 'type' in bind_obj.sheet.options():
        ...
        if bind_obj.sheet.option('type') == "ref":
            ...
            for record in exp_records:
                if 'seq_id' not in record:
                    bind_obj.set_error("表中查找不到seq_id字段")
...
def export_compare(data, option_name, dir_path, bind_obj=None):
    ...
    if not result:
        bind_obj.set_error("control_id: %s is not found in sg_specimen_group_compare",
                                   variables=(ObjectId(data)), code="51008153")
...
def export_kegg_enrich_info_filter(data, option_name, dir_path, bind_obj=None):
    ...
    if len(enrich_records) < v2:
        bind_obj.set_error("filted_records less than %s ",
                                   variables=(bind_obj.sheet.option("stat_numbers_value")), code="51008157")
    enrich_matrix = pd.DataFrame(list(enrich_records))
    enrich_matrix = enrich_matrix.set_index('id')
...
def export_geneset_from_query(data, option_name, dir_path, bind_obj=None):
    ...
    if not my_result:
        bind_obj.set_error("意外错误，task_id:%s在sg_annotation_query中未找到！", variables=(data), code="51008158")
