...
def export_exp_matrix_labelfree(data, option_name, dir_path, bind_obj=None):
    ...
    express_id = con_main.find_one({"task_id": data, "name": "ProteinTable_Origin"})["main_id"]
...
def export_diff_rna(data, option_name, dir_path, bind_obj=None):
    ...
    db = selecet_db(project_type, table_name, "diff_id",bind_obj.sheet.option('gene_diff_id'), bind_obj=bind_obj)
    # db = Config().get_mongo_client(mtype=project_type)[Config().get_mongo_dbname(project_type)]
