...
def export_searchdb(data, option_name, dir_path, bind_obj=None):
    searchdb_path = os.path.join(dir_path, "searchdb.txt")
    main_collection = db["sg_search_db"]
    detail_collection = db["sg_search_db_detail"]
    main_id = main_collection.find_one({"task_id": data})["main_id"]
    if not main_id:
        bind_obj.set_error("意外错误，task_id:%s 基因集在sg_search_db中未找到！", variables=(data,), )
    target_cols = OrderedDict(accession_id=1, _id=0, description=1)
    cursor = detail_collection.find({'searchdb_id': main_id, }, target_cols)
    df = pd.DataFrame(list(cursor))
    df.rename(columns={"accession_id": "Accession", "description": "Description"}, inplace=True)
    searchdb_df = df.set_index('Accession')
    searchdb_df.to_csv(searchdb_path, sep='\t', header=True, index=True)
    return searchdb_path
...
def export_exp_matrix_new(data, option_name, dir_path, bind_obj=None):
    ...
    con_main = db['sg_express']
    # express_id = con_main.find_one({"task_id": data, "type": "ratio"})["main_id"]
    exp_records = conn.find({"express_id": ObjectId(data)}, target_cols)
    exp_matrix = pd.DataFrame(list(exp_records))
    exp_matrix = exp_matrix.set_index('accession_id')
    if length == 2:
    ...
    exp_matrix.to_csv(output, sep='\t', header=True, index=True)
    # print('success to export expression matrix')
    bind_obj.logger.info('success to export expression matrix')
    return output


def export_exp_matrix_all(data, option_name, dir_path, bind_obj=None):
    main_db = db['sg_express']
    if type(data) == str or type(data) == bytes or type(data) == unicode:
        data = ObjectId(data)
    exp_main = main_db.find_one({'_id': data})
    samples = exp_main['samples']
    target_cols = OrderedDict(accession_id=1, _id=0)
    for each in samples:
        target_cols[each] = 1
    conn = db['sg_express_detail']
    # express_id = con_main.find_one({"task_id": data, "type": "ratio"})["main_id"]
    exp_records = conn.find({"express_id": data}, target_cols)
    exp_matrix = pd.DataFrame(list(exp_records))
    exp_matrix = exp_matrix.set_index('accession_id')
    output = os.path.join(dir_path, option_name)
    exp_matrix.to_csv(output, sep='\t', header=True, index=True)
...
def export_group_origin(data, option_name, dir_path, bind_obj=None):
    group_db = db['sg_specimen_group']
    group_record = group_db.find_one({"task_id": data})
    category_names = group_record['category_names']
    specimen_names = group_record['specimen_names']
    group_dict = dict(zip(category_names, specimen_names))
    group_out = os.path.join(dir_path, option_name)
    with open(group_out, 'w') as f:
        f.write('#sample\tgroup\n')
        for key in group_dict:
...
def export_proteinset_exp_matrix_new(data, option_name, dir_path, bind_obj=None):
    exp_id, proteinset_id = data.split(";")
    group_dict = bind_obj.sheet.option('group_dict')
    group_dict = json.loads(group_dict, object_pairs_hook=OrderedDict)
    samples = list()
