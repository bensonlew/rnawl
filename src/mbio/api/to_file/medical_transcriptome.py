...
def export_gene_list(data, option_name, dir_path, bind_obj=None):
    ...
    if not my_result:
        ...
        # self.bind_object.set_error("意外错误，geneset_id:%s在sg_geneset中未找到！", variables=(ObjectId(data)), code="51008132")
...
def export_go_list(data, option_name, dir_path, bind_obj=None):
    ...
    if not one_record:
        bind_obj.set_error("生成gos_list出错：annotation_id:{}在sg_annotation_gos_list中未找到！".format(go_id))
...
def export_kegg_table(data, option_name, dir_path, bind_obj=None):
    ...
    with open(kegg_path, 'wb') as w:
        ...
        for main_table in my_result:
            ...
            if not one_record:
                bind_obj.set_error("生成kegg_table出错：kegg_id:{}在sg_annotation_kegg_table中未找到！".format(kegg_id))
            for result in results:
...
def export_kegg_table2(data, option_name, dir_path, bind_obj=None):
    ...
    with open(kegg_path, 'wb') as w:
        ...
        for main_table in my_result:
            ...
            bind_obj.logger.debug(kegg_id)
            if not my_result:
                bind_obj.set_error("意外错误，annotation_kegg_id:{}在annotation_kegg中未找到！".format(kegg_id))
            results = db['sg_annotation_kegg_table'].find({'kegg_id': kegg_id, 'anno_type': geneset_type})
            one_record = db['sg_annotation_kegg_table'].find_one({'kegg_id': kegg_id, 'anno_type': geneset_type})
            if not one_record:
                bind_obj.set_error("生成kegg_table出错：kegg_id:{}在annotation_kegg_table中未找到！".format(kegg_id))
            for result in results:
                if 'hyperlink' not in result:
                    bind_obj.logger.debug(result['ko_id'] + result['transcript_id'] + '-> no hyperlink')
                    result['hyperlink'] = 'None'
        ...
        for i in my_result:
            ...
            if not one_record:
                ...
                one_record = db['sg_annotation_kegg_level'].find_one(
                    {'kegg_id': kegg_id, "seq_type": "ref", 'anno_type': bind_obj.sheet.option("geneset_type")})
            if not one_record:
                bind_obj.set_error("生成kegg_table出错：kegg_id:{}在sg_annotation_kegg_level中未找到！".format(kegg_id))
            for result in results:
                if 'hyperlink' not in result:
...
def _get_gene_id(geneset, geneset_detail, _id, bind_obj=None):
    try:
        ...
        bind_obj.set_error("{}在sg_geneset_detail表中没有找到!".format(_id))
    try:
        my_result = geneset.find_one({"main_id":ObjectId(_id)})
        _name = my_result['name']
    except Exception:
        bind_obj.set_error("{}在sg_geneset表中没有找到!".format(_id))
    return seq_id, _name
...
def export_multi_gene_list_search(data, option_name, dir_path, bind_obj=None):
    geneset_id = data.split(",")
    multi_geneset_path = dir_path + "/multi_geneset_list"
    collection = db['sg_geneset_detail']
    main_collection = db['sg_geneset']
...
def get_gene_detail_whole(data, option_name, dir_path, bind_obj=None):
    ...
    # if not annot_main:
    #     self.bind_object.set_error("Not Found in annotation_query by query %s", variables=(task_id), code="51008161")
...
def export_exp_matrix_new(data, option_name, dir_path, bind_obj=None):
    ...
    for each in samples:
        target_cols[each] = 1
    if 'type' not in bind_obj.sheet.options() or bind_obj.sheet.option('type') =='all':
        exp_records = conn.find({"exp_id": ObjectId(exp_id)}, target_cols)
    else:
        if bind_obj.sheet.option('type') in ["ref", 'new']:
            exp_records = conn.find({"exp_id": ObjectId(exp_id), 'kind': bind_obj.sheet.option('type')}, target_cols)
    ...
    exp_matrix = exp_matrix[~exp_matrix.index.duplicated(keep='first')]
    output = os.path.join(dir_path, option_name)
    exp_matrix.to_csv(output, sep='\t', header=True, index=True)
    print('success to export expression matrix')
    return output
...
def export_group_gsva(data, option_name, dir_path, bind_obj=None):
...
def export_geneset_exp_matrix_new(data, option_name, dir_path, bind_obj=None):
    """ gdq
    该函数根据提供的geneset_id和exp_id和group_dict提取表达信息。
    当geneset_id为‘all’时，则导出所有的exp_id对应的详情表。
    当geneset_id为‘refall’时，则导出所有的exp_id对应的详情表时还需限制'is_new'字段为False(false)。
    ...
    该函数仅返回表达矩阵文件的路径信息
    """
    exp_id, geneset_id, exp_level, is_rmbe = data.split(";")
    group_dict = bind_obj.sheet.option('group_dict')
    group_dict = json.loads(group_dict, object_pairs_hook=OrderedDict)
    samples = list()
    samples_raw = list()
    ...
    # get all exp matrix
    conn = db['sg_exp_detail']
    if "ref" in geneset_id.lower():
        exp_records = conn.find({"exp_id": ObjectId(exp_id), 'kind': 'ref'}, target_cols)
    else:
        exp_records = conn.find({"exp_id": ObjectId(exp_id)}, target_cols)
    exp_matrix = pd.DataFrame(list(exp_records))
    if exp_matrix.shape[0] == 0:
    ...
    if is_rmbe == 'true':
        exp_matrix.rename(columns=samples_dict, inplace=True)
    exp_matrix = exp_matrix.set_index('seq_id')
    if geneset:
        try:
            exp_matrix = exp_matrix.loc[geneset, :]
...
def export_geneset_genes_name(data, option_name, dir_path, bind_obj=None):
    ...
    results = col.find({"exp_id": ObjectId(exp_id)}, select_dict)
...
def export_wgcna_exp_matrix(data, option_name, dir_path, bind_obj=None):
    ...
    try:
        if level.upper() == 'T':
            annot_main = annot_table.find_one({"task_id": task_id, "level": "T", 'is_rmbe': False})
        else:
            annot_main = annot_table.find_one({"task_id": task_id, "level": "G", 'is_rmbe': False})
...
def get_gene_detail_new(data, option_name, dir_path, bind_obj=None):
    ...
    annot_table = db['sg_exp']
    annot_main = annot_table.find_one({"task_id": task_id, "exp_level": "T", 'status': 'end', 'is_rmbe': False})
    if not annot_main:
...
def export_masigpro_geneset_medical(data, option_name, dir_path, bind_obj):
    geneset_id, exp_id, level = data.strip().split(",")
    # show_data(
    ...
    if geneset_id in ['All', 'RefAll']:
        collection = db['sg_exp_detail']
        cursor = collection.find({'exp_id': ObjectId(exp_id)})
        if geneset_id == 'All':
            if level == 'G':
                dct = {
                    'seq_list': list({document['gene_id'] for document in cursor })}
            else:
                dct = {'seq_list': list(
                    {document['transcript_id'] for document in cursor })}
        ...
        open(output, 'w').writelines('{}\n'.format(seq_id) for seq_id in dct['seq_list'])
    return output
