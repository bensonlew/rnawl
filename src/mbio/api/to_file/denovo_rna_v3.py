...
def export_kegg_level_table(data, option_name, dir_path, bind_obj=None):
    ...
    with open(kegg__level_path, 'wb') as w:
        w.write(
            'Pathway_id\tnumber_of_seqs\tpathway_definition\tfirst_category\tanno_type\thyperlink\tseq_list\tsecond_category\n')
        for i in my_result:
            kegg_id = i["main_id"]
            bind_obj.logger.debug(kegg_id)
            if not kegg_id:
            ...
            results = db["sg_annotation_kegg_level"].find(
                {"kegg_id": kegg_id, "anno_type": bind_obj.sheet.option("geneset_type")})
...
def export_kegg_enrich_matrix(data, option_name, dir_path, bind_obj=None):
    ...
    target_cols = OrderedDict(_id=0, id=1, term=1, pvalue=1, corrected_pvalue=1, gene_lists=1, kegg_type=1)
    ...
    seq_list2 = [i.replace(';', '|') for i in kegg_enrich_matrix['gene_lists']]
    kegg_enrich_matrix['gene_lists'] = seq_list2
    kegg_enrich_matrix.rename(columns={'gene_lists': 'seq_list'}, inplace=True)
    # exp_matrix = exp_matrix.loc[geneset, :]
    output = os.path.join(dir_path, option_name)
