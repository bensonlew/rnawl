...
import pandas as pd
from mbio.packages.medical_transcriptome.annotation.go_annotation2 import Terms
from itertools import chain

# project_type = 'itraq_tmt'
# db = Config().get_mongo_client(mtype=project_type)[Config().get_mongo_dbname(project_type)]
# if project_type == 'itraq_tmt' or project_type == 'denovo_rna_v2':
...
def export_exp_matrix_itraq(data, option_name, dir_path, bind_obj=None):
    project_type = 'itraq_tmt'
    db = Config().get_mongo_client(mtype=project_type)[Config().get_mongo_dbname(project_type)]
    ...
    exp_matrix_new = exp_matrix.set_index('accession_id')
    output = os.path.join(dir_path, option_name)
    exp_matrix_new.to_csv(output, sep='\t', header=True, index=True)
    return output
...
def export_relation_tab(data, option_name, dir_path, bind_obj=None):
    ...
    try:
        main_id = main_conn.find_one({"task_id": data, 'status': 'end'})['main_id']
    except:
...
def export_diff_itraq(data, option_name, dir_path, bind_obj=None):
    project_type = 'itraq_tmt'
    db = Config().get_mongo_client(mtype=project_type)[Config().get_mongo_dbname(project_type)]
...
def export_go_rna(data, option_name, dir_path, bind_obj=None):
    ...
    if project_type == 'prok_rna':
        ...
        for go_lev1 in ['GO:0003674', 'GO:0005575', 'GO:0008150']:
            go_level2_list.extend(GO_db[go_lev1].children)
        for gr in go_results:
            # seq_list = gr["seq_list"].split(";")
            ...
            try:
                ...
                if gr["go_id2"] in go_level2_list:
                    level = '2'
                else:
                    level = None
            if in_list:
...
def export_go_list_rna(data, option_name, dir_path, bind_obj=None):
    ...
    with open(go_list_path, "wb") as w:
        for result in results:
            ...
            go_list_re = result["gos_list"].split(";")
            while '' in go_list_re:
                go_list_re.remove('')
            go_list = ";".join(go_list_re)
...
def selecet_db(project_type,table_name,id_name,table_id,bind_obj=None):
    ...
    try:
        ...
        db = old_config.get_mongo_client(mtype=project_type, db_version=0)[ old_config.get_mongo_dbname(project_type, db_version=0)]
        ...
        # Config().DBVersion = 1
        db = new_config.get_mongo_client(mtype=project_type, db_version=1)[new_config.get_mongo_dbname(project_type, db_version=1)]
        target_collection = db[table_name].find_one({id_name: main_id})
        target_collection["_id"]
        bind_obj.config.DBVersion = raw_workflow_db_version
        return db
