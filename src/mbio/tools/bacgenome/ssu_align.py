...
class SsuAlignAgent(Agent):
    """
    16s序列进行比对对齐和掩盖低置信度的位置处理
    """

    def __init__(self, parent):
        super(SsuAlignAgent, self).__init__(parent)
        options = [
            {"name": "16s_fa", "type": "infile", "format": "sequence.fasta"},  # 所有16s的序列
            {"name": "out", "type": "outfile", "format": "sequence.fasta"}, #比对对齐的序列文件
        ]
        self.add_option(options)

    def check_options(self):
        if not self.option("16s_fa"):
    ...
    def end(self):
        super(SsuAlignAgent, self).end()


class SsuAlignTool(Tool):
    def __init__(self, config):
        super(SsuAlignTool, self).__init__(config)
        self._version = "1.0"
        ...
        self.s16_fa = self.option("16s_fa").prop["path"]
    ...
    def set_output(self):
        if os.path.exists(self.output_dir + "/all.16s.align.fasta"):
            os.remove(self.output_dir + "/all.16s.align.fasta")
        os.link(self.work_dir + "/temp/temp.bacteria.mask.afa", self.output_dir + "/all.16s.align.fasta")
        self.option("out", self.output_dir + "/all.16s.align.fasta")