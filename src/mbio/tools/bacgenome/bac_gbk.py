...
class BacGbkAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "antismash_gbk", "type": "outfile", "format": "gene_structure.gbk"},  # 输出文件
            {"name": "antismash_gbk_co", "type": "outfile", "format": "paternity_test.data_dir"}
        ]
...
class BacGbkTool(Tool):
    ...
    def set_output(self):
        ...
        if self.option('analysis') in ['uncomplete']:
            if not os.path.exists(self.output_dir + '/seq_gbk'):
                os.mkdir(self.output_dir + '/seq_gbk')
            if not os.path.exists(self.output_dir + '/analysis_gbk'):
                os.mkdir(self.output_dir + '/analysis_gbk')
            if not os.path.exists(self.output_dir + '/gbk'):
                os.mkdir(self.output_dir + '/gbk')
            if not os.path.exists(self.output_dir + '/gbk/Scaffold'):
            ...
            self.option('gbk_dir', self.output_dir + '/seq_gbk')
            ...
            self.option('all_gbk', self.output_dir + '/analysis_gbk/' + 'all.antismash.gbk')
            ...
            #self.option('antismash_gbk').set_path(self.output_dir + '/gbk/Scaffold/' + self.option('sample_name') + '.gbk')
            self.option('antismash_gbk', self.output_dir + '/gbk/Scaffold/' + self.option('sample_name') + '.gbk')
            ...
            self.option('gbk_dir', self.output_dir + '/analysis_gbk')
            #self.option('antismash_gbk_co').set_path(self.output_dir + '/seq_gbk')
            self.option('antismash_gbk_co', self.output_dir + '/seq_gbk')
    ...
    def run_gff_ptt(self):  # zouguanqing 20190409
        ...
        cmd = "{} {}{} {} {} {} {}".format(self.perl_path, self.perl_script, gff_src,
                                                            self.genegff, self.trnagff, self.rrnagff,self.anno)
        self.logger.info(cmd)
        ...
        cmd1 = "{} {}{} {} {}".format(self.perl_path, self.perl_script, ptt_src, self.genegff, self.anno)
