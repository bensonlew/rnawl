...
class GeneTidyTool(Tool):
    ...
    def run_tidy(self):
        cmd = '{} {}/predict_result_remove_overlap.py'.format(self.python_path,self.config.PACKAGE_DIR +'/bacgenome')
    ...
    def run_fna2ffn(self):
        ...
        if command.return_code == 0:
            ...
            if os.path.getsize(self.work_dir + "/" + self.sample_name + ".fnn"):
                if self.option('genemark').is_set and not self.option('prodigal').is_set and not self.option('glimmer').is_set:
                    self.run_transeq2()
                else:
                    self.run_transeq()
            else:
    ...
    def run_transeq2(self):
        nul_seq = self.work_dir + "/" + self.sample_name + ".fnn"
        port_seq = self.work_dir + "/" + self.sample_name + ".faa"
        cmd = '{} {} {} {} {} {}'.format(self.python_path, self.trans_py, self.trans_table, self.option('trans_code'), nul_seq, port_seq)
        command = self.add_command("transeq2", cmd).run()
        self.wait(command)
        if command.return_code == 0:
            self.logger.info("翻译蛋白序列运行完成")
            self.run_get_information()
