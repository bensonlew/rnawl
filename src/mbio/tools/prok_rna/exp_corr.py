...
class ExpCorrAgent(Agent):
    ...
    def check_options(self):
        if self.option('exp').prop['sample_number'] <= 1:
            raise OptionError("Please input at least 2 samples", code = "35002801")
    ...
    def run_cluster_toolbox(self):
        ...
        if command.return_code == 0:
            ...
            if command.return_code is 0:
                ...
                self.set_error("%s Failed. >>> %s", variables = (cmd_name, cmd), code = "35002802")
        else:
            self.set_error("%s Failed. >>> %s", variables = (cmd_name, cmd), code = "35002803")
