...
class GatkAgent(Agent):
    ...
    def check_options(self):
        if self.option("ref_genome") == "customer_mode" and not self.option("ref_fa").is_set:
            raise OptionError("自定义参考基因组文件未提供", code = "35003301")
        if not self.option("input_bam").is_set:
            raise OptionError("用于分析的bam文件未提供", code = "35003302")
...
The reference sequence can be gzipped (both .fasta and .fasta.gz are supported).
        ...
        if command.return_code == 0:
            ...
            self.set_error("构建dict过程error", code = "35003303")
        ...
        if command.return_code == 0:
            ...
            if command.return_code == None:
                ...
                if command.return_code == 0:
                    ...
                    self.set_error("samtools建索引出错", code = "35003304")
    ...
    def realignment(self, ref_fasta):
        ...
        if command.return_code == 0:
            ...
            self.set_error("indel比对出错", code = "35003305")
    ...
    def indel_realn(self, ref_fasta):
        ...
        if command.return_code == 0:
            ...
            self.set_error("indel_realn出错", code = "35003306")
    ...
    def base_recalibrator(self, ref_fasta):
        ...
        if command.return_code == 0:
            ...
            self.set_error("base_recalibrator出错", code = "35003307")
    ...
    def print_reads(self, ref_fasta):
        ...
        if command.return_code == 0:
            ...
            self.set_error("print_reads出错", code = "35003308")
    ...
    def gatk_split(self, ref_fasta, bam):
        ...
        if command.return_code == 0:
            ...
            self.set_error("split NCigar reads出错", code = "35003309")
    ...
    def gatk_vc(self, ref_fasta, split_bam):
        ...
        if command.return_code == 0:
            ...
            self.set_error("variant calling出错", code = "35003310")
    ...
    def gatk_vf(self, ref_fasta, variantfile):
        ...
        try:
            subprocess.check_output(cmd, shell=True)  # 由于这一步用的是subprocess,所以并没有打印在log里面， 所以添加一个self.logger.info(cmd)
            self.logger.info('variant filtering成功')
        except subprocess.CalledProcessError:
            self.logger.info('variant filtering失败, 重运行')
            try:
                subprocess.check_output(cmd, shell=True)
                self.logger.info('variant filtering成功')
            except:
                self.set_error("无法生成vcf文件", code = "35003311")
