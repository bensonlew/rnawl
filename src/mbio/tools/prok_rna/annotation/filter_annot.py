...
class FilterAnnotAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {'type': 'infile', 'name': 'xml', 'format': 'prok_rna.blast_xml'},
            {'type': 'infile', 'name': 'hmm', 'format': 'prok_rna.common'},
            {'type': 'infile', 'name': 'hmm', 'format': 'prok_rna.common'},
    ...
    def check_options(self):
        if self.option("types") == "xml":
            if not self.option("xml").is_set:
                raise OptionError("必须设置参数query", code = "35000601")
        elif self.option("types") == "hmm":
            if not self.option("hmm").is_set:
                raise OptionError("必须设置参数hmm", code = "35000602")
        elif self.option("types") == "go":
            if not self.option("blast2go_annot").is_set:
                raise OptionError("必须设置参数hmm", code = "35000603")
        else:
            raise OptionError("method：%s必须为xml或hmm", variables = (self.option('types')), code = "35000604")

        if not (self.option('evalue') >= 0 and self.option('evalue') <= 0.001) :
            raise OptionError("E-value值设定必须为[0-0.001]之间：%s", variables = (self.option('evalue')), code = "35000605")
        if not (self.option('identity') >= 0 and self.option('identity') <= 100) :
            raise OptionError("Identity值设定必须为[0-100]之间：%s", variables = (self.option('identity')), code = "35000606")
        if not (self.option('similarity') >= 0 and self.option('similarity') <= 100) :
            raise OptionError("similarity值设定必须为[0-100]之间：%s", variables = (self.option('similarity')), code = "35000607")
        return True
...
class FilterAnnotTool(Tool):
    ...
    def run_filter_hmm(self):
        ...
        pfam_table = pd.read_table(self.option('hmm').prop['path'], header=0, dtype={'Seq_id': str, 'Protein_id': str})
