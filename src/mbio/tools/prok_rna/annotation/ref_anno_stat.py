...
from mbio.packages.prok_rna.gene2trans import gene2trans
from mbio.packages.prok_rna.nr_stat import nr_stat
...
class RefAnnoStatAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {"name": "nr_xml", "type": "infile", "format": "prok_rna.blast_xml"},  # blast比对到nr库的xml结果文件
            {"name": "ncbi_taxon", "type": "infile", "format": "prok_rna.nr_taxon"},
            {"name": "blast2go_annot", "type": "infile", "format": "prok_rna.blast2go_annot"},
            {"name": "gos_list", "type": "infile", "format": "prok_rna.go_list"},  # go注释tool的结果文件
            {"name": "gos_list_upload", "type": "infile", "format": "prok_rna.anno_upload"},   # 客户上传go注释文件,用blast2go_annot、gos_list或gos_list_upload进行go注统计
            {"name": "kegg_xml", "type": "infile", "format": "prok_rna.blast_xml"},
            {"name": "kos_list_upload", "type": "infile", "format": "prok_rna.anno_upload"},  # 客户上传kegg注释文件,kegg_xml或kos_list_upload进行kegg注释统计
            {"name": "string_xml", "type": "infile", "format": "prok_rna.blast_xml"},
            {"name": "string_table", "type": "infile", "format": "prok_rna.blast_table"},
            {"name": "cog_summary", "type": "infile", "format": "prok_rna.cog_summary"},
            ...
            {"name": "swissprot_xml", "type": "infile", "format": "prok_rna.blast_xml"},
            {"name": "ref_genome_gtf", "type": "infile", "format": "prok_rna.gtf"},  # 参考基因组gtf文件/新转录本gtf文件
            ...
            {"name": "gene_nr_table", "type": "outfile", "format": "prok_rna.blast_table"},
            {"name": "gene_string_table", "type": "outfile", "format": "prok_rna.blast_table"},
            {"name": "gene_kegg_table", "type": "outfile", "format": "prok_rna.blast_table"},
            {"name": "gene_swissprot_table", "type": "outfile", "format": "prok_rna.blast_table"},
            {"name": "gene_go_level_2", "type": "outfile", "format": "prok_rna.level2"},
            {"name": "gene_go_list", "type": "outfile", "format": "prok_rna.go_list"},
            {"name": "gene_kegg_anno_table", "type": "outfile", "format": "prok_rna.kegg_table"},
            {"name": "kegg_anno_table", "type": "outfile", "format": "prok_rna.kegg_table"},
            {"name": "gene_pfam_domain", "type": "outfile", "format": "prok_rna.kegg_list"},
            {"name": "eggnog_cog_table", "type": "infile", "format": "prok_rna.eggnog_cog_table"},
            {"name": "kegg_version", "type": "string", "default": ""},
            {"name": "gtf", "type": "infile", "format": "prok_rna.gtf"},
        ]
        self.add_option(options)
        self.step.add_steps("ref_anno_stat")
        self.on('start', self.stepstart)
        self.on('end', self.stepfinish)
        self.queue = 'BLAST2GO'  # 投递到指定的队列BLAST2GO

    def stepstart(self):
        self.step.ref_anno_stat.start()
        self.step.update()

    def stepfinish(self):
        self.step.ref_anno_stat.finish()
        self.step.update()
    ...
    def check_options(self):
        ...
        if len(self.anno_database) < 1:
            raise OptionError("至少选择一种注释库", code = "35001701")
        if self.option("gene2trans").is_set:
            pass
        elif not self.option("des").is_set:
            raise OptionError("缺失参考基因组des文件", code = "35001702")
        else:
            pass

        for i in self.anno_database:
            if i not in ['nr', 'go', 'cog', 'pfam', 'kegg', 'swissprot']:
                raise OptionError("需要注释的数据库（%s）不在支持范围内[nr, go, cog, pfam, kegg, swissprot]", variables = (i), code = "35001703")
            if i == 'go':
                if self.option('gos_list').is_set and self.option('blast2go_annot').is_set:
                    pass
                elif self.option('gos_list_upload').is_set:
                    pass
                elif self.option('eggnog_cog_table').is_set:
                    pass
                else:
                    raise OptionError("缺少go注释的输入文件", code = "35001704")
            if i == 'cog':
                if self.option('string_xml').is_set:
                    ...
                    raise OptionError("进行cog注释统计必须输入blast到string数据库的xml文件或table文件或eggnog", code = "35001705")
                # if not self.option('cog_summary').is_set:
                #     raise OptionError('缺少cog注释的输入文件:cog_summary')
            if i == 'nr' and not self.option('nr_xml').is_set:
                raise OptionError("缺少nr注释的输入文件", code = "35001706")
            if i == "pfam" and not self.option('pfam_domain').is_set:
                raise OptionError("缺少pfam注释的输入文件", code = "35001707")
            if i == 'kegg':
                if self.option('kegg_xml').is_set:
                    ...
                    raise OptionError("缺少kegg注释的输入文件", code = "35001708")
            if i == 'swissprot' and not self.option('swissprot_xml').is_set:
                raise OptionError("缺少swissprot注释的输入文件", code = "35001709")
        if self.option('des').is_set:
            pass
        elif not self.option('gene_file').is_set:
            raise OptionError("缺少gene输入文件", code = "35001710")
        else:
            pass
...
class RefAnnoStatTool(Tool):
    ...
    def __init__(self, config):
        ...
        self.kegg_path = self.config.PACKAGE_DIR + "/prok_rna/kegg_annotation_v2.py"
        ...
        self.cog_xml = self.config.PACKAGE_DIR + "/prok_rna/String2Cog.pl"
        ...
        elif self.option("des").is_set:
            with open(self.option("des").prop['path'], 'r') as tran_list:
                self.tran_list = [line.strip().split("\t")[0] for line in tran_list.readlines()]
        else:
            pass
            #tran_gene = gene2trans().get_gene_transcript(gtf_path=self.option("ref_genome_gtf").prop["path"])
        self.tran_gene = {}
    ...
    def run_nr_stat(self):
        ...
        try:
            nr_stat().nr_stat_info(
                tran_taxonfile = self.option("ncbi_taxon").prop['path'],
                gene_list = self.gene_list,
                outpath = self.work_dir + '/blast_nr_statistics/gene_nr_species.xls'
            )
            self.logger.info("nr taxomy End ")
        except Exception as e:
            self.set_error("运行nr taxomy出错:{}".format(e))
    ...
    def run_cog_stat(self):
        ...
        if self.option("eggnog_cog_table").is_set:
            cat_sum,fun_sum,gene_num = self.option("eggnog_cog_table").cog_class("cog_summary.xls")
            with open('cog_stat.xls', 'w') as cogstat_f:
                cogstat_f.write("cat_num\t{}\ntype_num\t{}\ncog_num\t{}\npercent\t{}\n".format(cat_sum,fun_sum,gene_num,float(gene_num)/float(len(self.tran_list))))
    ...
    def run_kegg_stat(self):
        ...
        if self.option("kegg_xml").is_set:
            ...
            cmd = "{} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {}".format(self.python_path, self.kegg_path, self.r_path, self.map_path, self.gene_kegg_xml, None, kegg_table, pidpath, gene_pathway, pathway_table, layerfile, taxonomy, self.option("link_bgcolor"), self.option("png_bgcolor"), self.image_magick, self.html_path, None, self.option("kegg_version"))
        else:
        ...
        if command.return_code == 0:
            ...
            self.set_error("运行kegg注释脚本出错", code = "35001711")
    ...
    def run_go_stat(self):
        ...
        if go_annot_cmd.return_code == 0:
            ...
            self.set_error("go_annot_cmd运行出错", code = "35001712")
        ...
        try:
            ...
            for db in self.database:
                if db == 'cog':
                    ...
                    # cog_venn = self.list_num(self.option("eggnog_cog_table").prop["path"])
                    # cog_gene_venn = self.list_num(self.option("eggnog_cog_table").prop["path"])
            ...
            self.set_error("设置注释统计分析结果目录失败%s", variables = (e), code = "35001713")
    ...
    def movedir2output(self, olddir, newname, mode='link'):
        ...
        if not os.path.isdir(olddir):
            self.set_error("需要移动到output目录的文件夹不存在", code = "35001714")
    ...
    def get_all_anno_stat(self):
        ...
        with open(all_anno_stat, 'wb') as w:
            ...
            w.write('type\ttranscripts\ttranscripts_percent\n')
            tmp = []
            # tmp_gene = []
            tmp_annot = []
            # tmp_annot_gene = []
