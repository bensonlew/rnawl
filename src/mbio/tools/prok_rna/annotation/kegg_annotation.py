...
class KeggAnnotationAgent(Agent):
    ...
    def check_options(self):
        if not self.option("blastout").is_set:
            raise OptionError("必须提供BLAST结果文件", code = "35001001")
        if self.option("taxonomy") not in ["Animals", "Plants", "Fungi", "Protists", "Archaea", "Bacteria", "None", None, "All"]:
            raise OptionError("物种类别必须为Animals/Plants/Fungi/Protists/Archaea/Bacteria/None", code = "35001002")
...
class KeggAnnotationTool(Tool):
    ...
    def __init__(self, config):
        ...
        self.kegg_path3 = self.config.PACKAGE_DIR + "/rna/annotation/kegg_annotation_v3.py"
        self.kegg_path2 = self.config.PACKAGE_DIR + "/rna/annotation/kegg_annotation_v2.py"
    ...
    def kegg_annotation(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("运行kegg注释脚本出错", code = "35001003")
