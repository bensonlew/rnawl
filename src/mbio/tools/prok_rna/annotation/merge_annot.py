...
import subprocess
import json
from mbio.packages.prok_rna.merge_kegg_pathway import MergeKeggPathway
from mbio.packages.prok_rna.merge import Merge
...
class MergeAnnotAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "annot_class", "type": "infile", "format": "prok_rna.common_dir"},
            {"name": "annot_class_ref", "type": "infile", "format": "prok_rna.common_dir"},
            {"name": "annot_db", "type": "infile", "format": "prok_rna.common_dir"},
    ...
    def check_options(self):
        ...
        if self.option("is_assemble") == True:
            if not self.option("annot_class").is_set:
                raise OptionError("需要指定新转录本注释路径")
            if not self.option("annot_db").is_set:
                raise OptionError("需要指定新转录本数据库比对路径")
...
class MergeAnnotTool(Tool):
    def __init__(self, config):
        ...
        self.map_path = self.config.PACKAGE_DIR + "/prok_rna/map4.r"
        ...
        self.merge_kegg_pathway = self.config.PACKAGE_DIR + "/prok_rna/merge_kegg_pathway.py"
    ...
    def creat_output(self):
        ...
        if os.path.exists(self.output_dir + "/newannot_mapdb"):
            os.rmdir(self.output_dir + "/newannot_mapdb")
        if self.option("is_assemble") == True:
        ...
        for annot_class in ["newannot_class", "refannot_class", "allannot_class"]:
            if os.path.exists(self.output_dir + "/" + annot_class):
                os.rmdir(self.output_dir + "/" + annot_class)
    ...
    def run_kegg_anno(self, kegg_table, r_level_path, n_level_path, seq_type, path_out, merged_gene_kegg):
        # 合并kegg注释
        cmd = "{} {} {} {} {} {} {} {} {} {} {}".format(self.python, self.merge_kegg_pathway, self.map_path, self.r_path, self.image_magick, r_level_path, n_level_path,  kegg_table, path_out, merged_gene_kegg, self.html_path)
        self.logger.info("开始画图")
        self.logger.info(cmd)
        cmd1_obj = self.add_command("merge_png" + seq_type, cmd).run()
        self.wait(cmd1_obj)
    ...
    def run_go_anno(self, go_list, seq_type):
        ...
        cmd1_obj = self.add_command("cmd1" + seq_type, cmd1).run()
        self.wait(cmd1_obj)
        if cmd1_obj.return_code == 0:
        ...
        cmd2_obj = self.add_command("cmd2" + seq_type, cmd2).run()
        self.wait(cmd2_obj)
