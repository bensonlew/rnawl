...
class BlastAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "reference", "type": "infile", "format": "prok_rna.fasta"},  # 参考序列  选择customer时启用
            ...
            {"name": "outxml", "type": "outfile", "format": "prok_rna.blast_xml"},  # 输出格式为5时输出
            {"name": "outtable", "type": "outfile", "format": "prok_rna.blast_table"},  # 输出格式为6时输出
    ...
    def check_options(self):
        if not self.option("query").is_set:
            raise OptionError("必须设置参数query", code = "35000101")
        if self.option('query_type') not in ['nucl', 'prot']:
            ...
            if self._fasta_type[self.option('query').prop['seq_type']] != self.option('query_type'):
                raise OptionError("文件检查发现查询序列为：%s，而说明的文件类型为：%s",
                                  variables = (self._fasta_type[self.option('query').prop['seq_type']], self.option('query_type')),
                                  code = "32000103")
        if self.option("database") == 'customer_mode':
            if not self.option("reference").is_set:
                raise OptionError("使用自定义数据库模式时必须设置reference", code = "35000104")
            if self.option('reference_type') not in ['nucl', 'prot']:
                ...
                if self._fasta_type[self.option('reference').prop['seq_type']] != self.option('reference_type'):
                    raise OptionError("文件检查发现参考序列为：%s, 而说明的文件类型为：%s",
                                      variables = (self._fasta_type[self.option('reference').prop['seq_type']], self.option('reference_type')),
                                      code = "32000106")
            ...
            raise OptionError("数据库%s不被支持", variables = (self.option("database")), code = "35000107")
        else:
            self.option('reference_type', self._database_type[self.option("database").lower()])
        if not 15 > self.option('outfmt') > -1:
            raise OptionError("outfmt遵循blast+输出规则，必须为0-14之间：%s", variables = (self.option('outfmt')), code = "35000108")
        ...
        if self.option('blast') not in self._blast_type[self.option('query_type')][self.option('reference_type')]:
            raise OptionError("程序不适用于提供的查询序列和库的类型，或者没有提供blast参数，请仔细检查，核酸比对核酸库只能使用blastn或者tblastn，核酸比对蛋白库只能使用blastp， 蛋白比对蛋白库只能使用blastp")
        return True
...
class BlastTool(Tool):
    ...
    def run_makedb_and_blast(self):
        ...
        if makedb_obj.return_code == 0:
            ...
            self.set_error("makeblastdb运行出错", code = "35000112")
