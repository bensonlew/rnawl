...
class RefAnnoQueryAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "gos_detail", "type": "string", "default": None},
            ...
            {"name": "des", "type": "string", "default": None},  # 基因描述文件
            {"name": "des_type", "type": "string", "default": "type3"},  # 基因描述文件类型
            {"name": "enterz", "type": "string", "default": None},  # 基因ncbi entrez对应关系文件
    ...
    def check_options(self):
        if not self.option("gos_list"):
                raise OptionError("缺少输入文件：gos_list", code = "35001601")
        if not self.option("kegg_table"):
            raise OptionError("缺少输入文件：kegg_table", code = "35001602")
...
class RefAnnoQueryTool(Tool):
    def __init__(self, config):
        ...
        self.query_path = self.config.PACKAGE_DIR + "/prok_rna/new_annotation_query.py"


    def run_query(self):
        tran_outpath = self.output_dir + "/all_anno_detail.xls"
        ...
        gos_detail = self.option("gos_detail")
        ...
        try:
            ...
            self.set_error("运行注释查询脚本失败", code = "35001603")
        # self.set_output()
        self.end()
