...
from mbio.packages.prok_rna.go_graph import draw_GO
import subprocess
import unittest
import pandas as pd
...
class GoEnrichAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            dict(name="go_version", type="string", default="20200628"),
...
class GoEnrichTool(Tool):
    ...
    def run_enrich(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("goatools计算错误", code = "35003403")
    ...
    def run_draw_go_graph(self):
        try:
            ...
            self.set_error("绘图发生错误：%s", variables = (traceback.format_exc()), code = "35003404")
    ...
    def get_go_pvalue_dict(self):
        ...
        padjust = columns_list[8]
        df_10 = df.sort_values(by=['p_uncorrected', padjust], ascending=True)[0:10]
        for i in df_10.index:
            go2pvalue[i] = df_10.loc[i]['p_uncorrected']
            go2padjust[i] = df_10.loc[i][padjust]
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            "type": "tool",
            "name": "prok_rna.geneset.go_enrich",
            "instant": False,
            "options": dict(
