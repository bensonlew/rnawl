...
class KeggRichAgent(Agent):
    ...
    def check_options(self):
        ...
        if not self.option('kegg_table').is_set:
            raise OptionError("必须设置kegg的pathway输入文件", code = "35003601")
        if self.option('correct').lower() not in ['by', 'bh', 'bonferroni', 'holm']:
            raise OptionError("多重检验校正的方法不在提供的范围内", code = "35003602")
        if not self.option("diff_list").is_set:
            raise OptionError("必须设置输入文件diff_list", code = "35003603")
        return True
...
class KeggRichTool(Tool):
    ...
    def run_kegg_rich(self):
        ...
        try:
            ...
            self.set_error("kegg富集第一步运行出错：%s", code = "35003604")
    ...
    def run_identify(self):
        ...
        if command_2.return_code == 0:
            ...
            self.set_error("kegg富集分析运行出错", code = "35003605")
    ...
    def set_output(self, linkfile):
        ...
        try:
            ...
            self.set_error("设置kegg富集分析结果目录失败：%s", variables = (e), code = "35003606")
