...
class SnpAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {"name": "trinity_fa", "type": "infile", "format":"sequence.fasta"},  # 这是fa文件的格式检查
            {"name": "bamlist", "type": "infile", "format": "prok_rna.bamlist"},
            {"name": "call_vcf", "type": "outfile", "format": "prok_rna.vcf"},
    ...
    def check_options(self):
        ...
        if not self.option("trinity_fa").is_set:
            raise OptionError("必须设置输入文件：组装完成的unigene.fa文件", code = "35004601")

        if not self.option("bamlist"):
            raise OptionError("必须设置输入文件：rsem比对得到的bamlist文件", code = "35004602")
...
class SnpTool(Tool):
    ...
    def mpileup(self):
        ...
        if cmd2_obj.return_code == 0:
            ...
            if cmd2_obj.return_code == 0:
                ...
                self.set_error("运行mpileup出错", code = "35004603")
    ...
    def bcfcall(self):
        ...
        if cmd3_obj.return_code == 0:
            ...
            if cmd3_obj.return_code == 0:
                ...
                self.set_error("运行bcfcall出错", code = "35004604")
    ...
    def run(self):
        ...
        self.download_bam()
        if not os.path.exists(self.option("trinity_fa").prop["path"] + ".fai"):
            cmd = "{} faidx {}".format(self.samtools_path, self.option("trinity_fa").prop["path"])
