...
class BlastToRfamAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "query_fq", "type": "infile", "format": "prok_rna.common"},  # 输入fq序列文件
...
class BlastToRfamTool(Tool):
    ...
    def blast_rfam(self):
        ...
        if self.option("query_fq").is_set:
            fq = self.option("query_fq").prop['path']
            input_file = os.path.join(self.work_dir, '{}.fa'.format(self.option('sample_name')))
            self.fastq_convert_fasta(fq, input_file)
        ...
        if command.return_code == 0:
            ...
            self.set_error("运行rfam库比对出错", code = "35002402")
    ...
    def run_rfam_stat(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("Rfam统计运行失败", code = "35002403")
        with open(detail_out, "r") as f, open(stat_out, "w") as w:
            w.write("Sample\trRNA(%)\n")
            if self.option("query_fq").is_set:
                fa = os.path.join(self.work_dir, '{}.fa'.format(self.option('sample_name')))
            else:
                fa = self.option('query').prop['path']
