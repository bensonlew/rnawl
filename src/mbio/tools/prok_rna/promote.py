...
class PromoteAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {"name": "sequence", "type": "infile", "format": "prok_rna.fasta"},  # 基因序列
            {"name": "assemble", "type": "infile", "format": "prok_rna.fasta"},  # 拼接序列
            {"name": "window_size", "type": "int", "default": 100},  # 窗口大小
            {"name": "gtf", "type": "infile", "format": "prok_rna.gtf"},
            {"name": "rock_index", "type": "infile", "format": "prok_rna.common"},
    ...
    def check_options(self):
        if not self.option("sequence").is_set:
            raise OptionError("必须设置输入序列文件", code = "35003801")
        if self.option('seq_type') not in [0, 1, 2]:
            raise OptionError("错误的序列类型，只能为[0, 1, 2]", code = "35003802")
        return True
...
class PromoteTool(Tool):
    def __init__(self, config):
        ...
        self.pre_path = self.config.PACKAGE_DIR + '/prok_rna/pro_prepare.pl'
    ...
    def predict_prom(self):
        ...
        '''
        linkfile = self.work_dir + '/' + os.path.basename(self.option('gtf').prop['path'])
        if os.path.exists(linkfile):
            os.remove(linkfile)
        os.link(self.option('gtf').prop['path'], linkfile)
        '''
        pre_fasta = self.work_dir + '/pre.fasta'
        if self.option("rock_index").is_set:
            stru_type = "rock_index"
            stru_file = self.option("rock_index").prop['path']
        else:
            stru_file = linkfile
            stru_type = "gtf"

        cmd = '{} {} {} {} {} {} {}'.format(self.perl_path, self.pre_path, self.option('assemble').prop['path'], stru_file,
                                            self.option('cut'), pre_fasta, stru_type)
        ...
        if command2.return_code == 0:
            ...
            self.set_error("启动子统计失败", code = "35003805")
