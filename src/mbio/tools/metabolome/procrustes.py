...
class ProcrustesAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {'name': 'coord_ref', 'type': 'infile', 'default': 'None', 'format': 'metabolome.coord_matrices'}, # 只能有一个
            {'name': 'coord_query', 'type': 'infile', 'default': 'None', 'format': 'metabolome.coord_matrices'}, # 如有多个可逗号隔开，每个均会与ref进行计算
            {'name': 'random', 'type': 'int', 'default': '1000', 'format': 'None'},
            {'name': 'out', 'type': 'string', 'default': 'None', 'format': 'None'}
        ]
...
class ProcrustesTool(Tool):
    ...
    def __init__(self, config):
        ...
        self.python_path = software_dir + '/program/Python/bin/python'
    ...
    def run_procrustes(self):
        ...
        cmd += '-i {},{} '.format(self.option("coord_ref"), self.option("coord_query"))
