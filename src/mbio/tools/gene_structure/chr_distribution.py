...
class ChrDistributionAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "strand", "type": "bool", "default": False}  # 是否统计正负链信息
        ]
...
class ChrDistributionTool(Tool):
    ...
    def chr_distribution1(self):
        bam_path = self.option("bam").prop["path"]
        self.bam_name = bam_path.split("/")[-1]
        awk = """ awk '{if ($0 ~ /XS:A:+/) {print $3"\\tforward"} else if ($0 ~ /XS:A:-/) {print $3"\\treverse"} else {print $3"\\tunknown"}}' """
        cmd = "{}samtools view {} | {} | sort | uniq -c > {}".format(self.samtools_path, bam_path, awk, self.bam_name + "_chr_stat.xls")
        ...
        with open(self.bam_name + "_chr_stat.xls", 'r') as f:
            for line in f:
                ...
                if line[1] != "*":
                    ...
                    chrom = line[1]
                    position = line[2]
                    if chrom not in distribution:
                        distribution[chrom] = dict()
                    distribution[chrom][position] = num
        with open(out_put, "w") as w:
            header = ["#chr", "read_num_forward", "read_num_reverse", "read_num_unknown"]
            w.write("\t".join(header) + "\n")
            for key1 in sorted(distribution):
                if "forward" not in distribution[key1]:
                    distribution[key1]["forward"] = 0
                if "reverse" not in distribution[key1]:
                    distribution[key1]["reverse"] = 0
                if "unknown" not in distribution[key1]:
                    distribution[key1]["unknown"] = 0
                line = [key1, distribution[key1]["forward"], distribution[key1]["reverse"], distribution[key1]["unknown"]]
    ...
    def run(self):
        ...
        if self.option("strand") == True:
            self.chr_distribution1()
            self.set_output1()
        else:
...
class TestFunction(unittest.TestCase):
    def test(self):
        ...
        data = {
            ...
            'instant': False,
            'options': dict(
                bam='/mnt/ilustre/users/sanger-dev/workspace/20191008/LncRna_tsg_35703/RnaseqMapping/output/bam/A1.bam',
                strand=True
            )
        }
