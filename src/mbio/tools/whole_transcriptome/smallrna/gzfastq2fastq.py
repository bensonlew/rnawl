        ...
        with open(self.option("fastq_path") + '/list.txt', 'r') as list_r, open('ungz.bash', 'w') as ungz_w:
            ...
            for line in list_info.split('\n'):
                ...
                if line[0].split(".")[-1] in ["gz"]:
                    ...
                    list_info_new += self.option("fastq_path") + "/" + gzed + '\t' + '\t'.join(line[1:]) + '\n'
                    ...
                    if line[0] != "":
                        ...
                        elif os.path.exists(self.option("fastq_path") + "/" + line[0]):
                            list_info_new += self.option("fastq_path") + "/" + '\t'.join(line) + '\n'
                        else:
                            pass
        if os.path.exists(self.option("fastq_path") + '/list_re.txt'):
            os.remove(self.option("fastq_path") + '/list_re.txt')
        with open(self.option("fastq_path") + '/list_re.txt', 'w') as list_w:
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "whole_transcriptome_gzfastq2fastq_gff" + datetime.datetime.now().strftime('%H-%M-%S'),
            "type": "tool",
            "name": "whole_transcriptome.smallrna.gzfastq2fastq",
            "instant": False,
            "options": dict(
                fastq_path = '/mnt/ilustre/users/sanger-dev/sg-users/shicaiping/whole_transcriptome/demo_human/smallrna/rawdata'
            )
           }
