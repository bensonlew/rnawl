...
class FastaUniqTool(Tool):
    ...
    def run_uniqueFastas(self):
        ...
        elif command.return_code in [-9]:  # 加入return_code检测，在sanger超出内存的返回值为-9
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        import datetime
        test_dir='/mnt/ilustre/users/sanger-dev/workspace/20190926/Single_whole_transcriptome.smallrna.mirna_qc15-22-40/MirnaQc'
        data = {
            "id": "whole_transcriptome_smallrna_FastaUniq" + datetime.datetime.now().strftime('%H-%M-%S'),
            "type": "tool",
            "name": "whole_transcriptome.smallrna.fasta_uniq",
            "instant": False,
            "options": dict(
