...
import pandas as pd
from mbio.packages.labelfree.metastat import two_group_test, two_sample_test
import math
import glob
import subprocess
...
class DiffTool(Tool):
    ...
    def diff_treat(self, fc_up, fc_down):
        ...
        for file in files:
            ...
            fc = df['fc']
            log2fc = df['log2fc']
            df.drop(labels=['fc', 'log2fc'], axis=1, inplace=True)
            df['fc'] = fc
            df['log2fc'] = log2fc
    ...
    def diff_summary_labelfree(self):
        ...
        for file in files:
            ...
            df_new.index.name = "accession_id"
            # df = df[(df.fc >= self.option("fc_up")) | (df.fc ==self.option("fc_down"))]
            df_new.to_csv(self.work_dir + "/diff_labelfree_summary" + str(i) + ".csv",
                          sep = '\t', index=False)
    
    def merge_labelfree(self):
        files = glob.glob(r"*diff_labelfree_summary*")
        i = 0
        list_df = list()
        for file in files:
            i += 1
        ...
        # df_merged = reduce(lambda  left,right: pd.merge(left,right, on=['accession_id'], how='outer'), list_df)
        df_merged.index.name = "accession_id"
        df_merged.to_csv(self.work_dir + "/all_labelfree_summary" + ".xls", sep = '\t')
    ...
    def set_output(self):
        ...
        diff_summary_lf = self.work_dir + "/" + "all_labelfree_summary.xls"
        os.link(diff_summary_lf, os.path.join(self.output_dir, "diff_protein_summary_labelfree.xls"))
