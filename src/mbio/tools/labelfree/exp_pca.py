...
class ExpPcaAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {'name': 'exp', 'type': 'infile', 'format': 'labelfree.express_matrix'},
    ...
    def check_options(self):
        if self.option('exp').prop['sample_number'] <= 1:
            raise OptionError('Please input at least 2 samples', code = "33704801")
...
class ExpPcaTool(Tool):
    ...
    def __init__(self, config):
        ...
        self.cluster_toolbox = self.config.PACKAGE_DIR + '/labelfree/cluster_toolbox.py'
    ...
    def run_cluster_toolbox(self):
        ...
        if command.return_code == 0:
            ...
            if command.return_code is 0:
                ...
                self.set_error("%s Failed. >>>%s", variables = (cmd_name, cmd), code = "33704802")
        else:
            self.set_error("%s Failed. >>>%s", variables = (cmd_name, cmd), code = "33704803")
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        test_dir='/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/test/lnc_rna/pca/test_data'
        data = {
            "id": "ExpPca" + str(random.randint(1, 10000)),
            "type": "tool",
            "name": "lnc_rna.exp_pca",
            "instant": False,
            "options": dict(
                exp=test_dir + "/" + "gene.tpm.matrix",
                # scm="complete",
                # scd="correlation",
                # corr_method='pearson',
                output=None,
