...
import json
import unittest
from mbio.packages.labelfree.ipath import Ipath
...
class IpathTool(Tool):
    ...
    def generate_ipath_file(self):
        ...
        '''
        protein_set_file = self.option("proteinset_kegg")
        kegg_file = self.option("kegg_table").prop['path']
        # 读入基因集文件
        f = open(protein_set_file, 'r')
        lines = f.readlines()
        f.close()
        protein_set1 = []
        protein_set2 = []
        #self.logger.info("pset lines{}".format("*".join(lines)))
        protein_set1 = lines[0].strip().split("\t")[1].split(",")
        self.sets.append(lines[0].strip().split("\t")[0])
        if len(lines) == 2:
            protein_set2 = lines[1].strip().split("\t")[1].split(",")
            self.sets.append(lines[1].strip().split("\t")[0])
        protein_set1_ko = []
        protein_set2_ko = []

        ko_gene = dict()
        #self.logger.info("pset ko1{}".format(protein_set1))
        #self.logger.info("pset ko1{}".format(protein_set1_ko))
        KO_type = dict()
        # 读入每个基因集的KO
            ...
            for line in lines[1:]:
                ...
                for ko in KO_ids:
                    if ko_gene.has_key(ko):
                        if acc_id in protein_set1 or acc_id in protein_set2:
                ...
                if acc_id in protein_set1:
                    protein_set1_ko.extend(KO_ids)
                else:
                    pass
                if acc_id in protein_set2:
                    protein_set2_ko.extend(KO_ids)
                else:
                    pass
        # self.logger.info("pset ko1{}".format(protein_set1_ko))
        # 根据交集差集些输出文件
        with open(self.ipath_input, 'wb') as w, open(self.gene_ipath_input, 'wb') as w2:
            for i in list(set(protein_set1_ko).intersection(set(protein_set2_ko))):
                w.write("{}\t#0000ff\tW15\n".format(i))
                w2.write("{}\t{}\t#0000ff\tW15\n".format(";".join(set(ko_gene[i])), i))
            for i in list(set(protein_set1_ko).difference(set(protein_set2_ko))):
                w.write("{}\t#ff0000\tW15\n".format(i))
                w2.write("{}\t{}\t#ff0000\tW15\n".format(";".join(set(ko_gene[i])), i))
            for i in list(set(protein_set2_ko).difference(set(protein_set1_ko))):
                w.write("{}\t#00ff00\tW15\n".format(i))
                w2.write("{}\t{}\t#00ff00\tW15\n".format(";".join(set(ko_gene[i])), i))
        return
    ...
    def set_output(self):
        ...
        all_files = ['gene_ipath_input.xls'] + glob.glob('*.svg')
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        test_dir = '/mnt/ilustre/users/sanger-dev/workspace/20180312/ProteinsetKegg_labelfree_7534_948'
        data = {
            "id": "ipath" + str(random.randint(1, 10000)),
            "type": "tool",
            "name": "labelfree.proteinset.ipath",
            "instant": True,
            "options": dict(
