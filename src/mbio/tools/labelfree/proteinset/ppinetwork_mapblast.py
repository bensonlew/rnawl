...
class PpinetworkMapblastAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {"name": "fa", "type": "infile", "format": "labelfree.fasta"},  # 输入文件
            ...
            {"name": "outtable", "type": "outfile", "format": "labelfree.common"},  # 输出格式为6时输出
            ...
            if self._fasta_type[self.option('fa').prop['seq_type']] != self.option('query_type'):
                raise OptionError(
                    '文件检查发现查询序列为:{}, 而说明的文件类型为:{}'.format(
                        self._fasta_type[self.option('fa').prop['seq_type'], self.option('query_type')]))
...
class PpinetworkMapblastTool(Tool):
    ...
    def choose_unmapped_db(self):
        ...
        """
        import glob
        if not os.path.exists(self.option("unmapped_db")):
            unmapp = glob.glob(self.option("unmapped_db").replace('PpinetworkMap','*'))
            self.logger.info(unmapp)
            if unmapp:
                self.option("unmapped_db", unmapp[0])
    ...
    def get_blast_best(self):
        ...
        with open(output_xls , 'w') as f:
            ...
            if os.path.getsize(blastfile) == 0:
                ...
                blast=pd.read_table(blastfile, header = None, dtype={0:object, 1:object})
                all_hit=list(set(blast[1]))
                for hit in all_hit:
                    best_hit=blast[blast[1]==hit].sort_values(10, ascending=True).iloc[0]
                    if str(best_hit[1]).strip() in gene_list2:
                        pass
                    else:
                        gene_list2.append(str(best_hit[1]).strip())
                        f.write(str(best_hit[0]).strip() + "\t" + str(best_hit[1]).strip() + "\tby_sequence\n")
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            "type": "tool",
            "name": "labelfree.proteinset.ppinetwork_mapblast",
            "instant": True,
            "options": dict(
