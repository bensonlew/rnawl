...
class PcaAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {"name": "otutable", "type": "infile","format": "labelfree.ratio_exp"},
            ...
            {"name": "group_table", "type": "infile", "format": "labelfree.group_table"},  # modify by zhouxuan 20170823
...
class PcaTool(Tool):  # PCA需要第一行开头没有'#'的丰度表，filter_otu_sample函数生成的表头没有'#'
    ...
    def get_otu_table(self):
        ...
        if len(otu_file.readlines()) < 3:
            raise OptionError('数据表特征数量必须大于等于2: {}'.format(len(otu_file.readlines())))
        return otu_path
        # if self.option('envtable').is_set:
