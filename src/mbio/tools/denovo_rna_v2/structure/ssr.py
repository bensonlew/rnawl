...
from mbio.packages.denovo_rna_v2.functions import runcmd
...
class SsrTool(Tool):
    def __init__(self, config):
        ...
        self.program = {
            ...
            'primer3_core': 'bioinfo/denovo_rna_v2/SSR/misa/primer3_core'
        }
        self.script = {
            'misa2': os.path.join(self.config.SOFTWARE_DIR, 'bioinfo/denovo_rna_v2/SSR/misa/misa2.pl'),
            'p3_in': os.path.join(self.config.SOFTWARE_DIR, 'bioinfo/denovo_rna_v2/SSR/misa/p3_in.pl'),
            'p3_out': os.path.join(self.config.SOFTWARE_DIR, 'bioinfo/denovo_rna_v2/SSR/misa/p3_out.pl'),
        }
        self.file = {
            'seqs_fasta': os.path.join(self.work_dir, 'seqs.fasta'),
            'misa_ini': os.path.join(self.work_dir, 'misa.ini'),
            'misa_out': os.path.join(self.work_dir, 'seqs.fasta.misa'),
            'p3in': os.path.join(self.work_dir, 'seqs.fasta.p3in'),
            'p3out': os.path.join(self.work_dir, 'seqs.fasta.p3out'),
        }
    ...
    def prepare(self):
        if os.path.isfile(self.file['seqs_fasta']):
            os.remove(self.file['seqs_fasta'])
        os.link(self.option('unigene_fa').path, self.file['seqs_fasta'])
        head = 'definition(unit_size,min_repeats): 1-{mr1} 2-{mr2} 3-{mr3} 4-{mr4} 5-{mr5} 6-{mr6}'.format(
            **{'mr{}'.format(i): self.option('rept_{}'.format(i)) for i in range(1, 7)})
        tail = 'interruptions(max_difference_between_2_SSRs): {}'.format(self.option('ssr_distance'))
        open(self.file['misa_ini'], 'w').write('{}\n{}\n'.format(head, tail))

    def run_misa2(self):
        cmd = '{} {} {} {}'.format(self.program['perl'], self.script['misa2'], self.file['seqs_fasta'], self.file['misa_ini'])
        runcmd(self, 'run_misa2', cmd)

    def run_p3_in(self):
        cmd = '{} {} {}'.format(self.program['perl'], self.script['p3_in'], self.file['misa_out'])
        runcmd(self, 'run_p3_in', cmd)

    def run_primer3_core(self):
        cmd = '{} < {} > {}'.format(self.program['primer3_core'], self.file['p3in'], self.file['p3out'])
        runcmd(self, 'run_primer3_core', cmd, shell=True)

    def run_p3_out(self):
        cmd = '{} {} {} {}'.format(self.program['perl'], self.script['p3_out'], self.file['p3out'], self.file['misa_out'])
        runcmd(self, 'run_p3_out', cmd)
    ...
    def unigene_fa2regular_fa(self):
        ...
        cmd1 = '{} {} {} {}'.format(self.perl_path, self.misa_pl, self.work_dir + '/testt.fasta',
                                    self.work_dir + '/misa.ini')
        ...
        cmd3 = '{} {} {} {} {}'.format('/sh', self.query_path, self.primer3_core, self.work_dir + '/testt.fasta.p3in',
                                       self.work_dir + '/testt.fasta.p3in.p3out')
        ...
        cmd4 = '{}  {}  {}'.format(self.p3_out, self.work_dir + '/testt.fasta.p3in.p3out',
                                   self.work_dir + '/testt.fasta.misa')
        ...
        with open(self.work_dir + '/testt.fasta.statistics', 'r') as f2, open(self.work_dir + '/testt.fasta.txt',
                                                                              'w') as w2:
        ...
        with open(self.work_dir + '/testt.fasta.txt') as f3, open(self.work_dir + '/unigene.fa.misa.txt') as f4, \
            ...
            for line in f3:
                ...
                class1, class2, class3, class4 = target_sum_list[0], target_sum_list[1], target_sum_list[2], \
                                                 target_sum_list[3]
                w4.write(
            ...
            w3.write('ssr_information' + '\t' + 'number' + '\t' + 'percent' + '\t' + '\n' \
                ...
                num_Mono_nucleotide_repeats) + '\t' + str(Mono_percent) + '\t' + '\n' \
                                                                                 'di_rep' + '\t' + str(
                num_Di_nucleotide_repeats) + '\t' + str(Di_percent) + '\t' + '\n' \
                                                                             'tri_rep' + '\t' + str(
