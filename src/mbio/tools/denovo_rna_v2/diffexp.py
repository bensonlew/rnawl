...
class DiffexpAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            dict(name="count", type="infile", format="denovo_rna_v2.express_matrix"),
            dict(name="exp", type="infile", format="denovo_rna_v2.express_matrix"),
            dict(name="exp_type", type="string", default="tpm"),

            # group为样本分组信息文件，要求至少两列，第一列为样本名，其他列为组名。
            # 没有重复实验时，可以用样本名作为组名。
            dict(name="group", type="infile", format="denovo_rna_v2.group_table"),
            ...
            # tpm_filter_threshold 20190708修改
    ...
    def check_options(self):
        if self.option("method").lower() not in ["degseq", "edger", "deseq2"]:
            raise OptionError("Method is incorrect", code = "32002701")
        pvalue, fc = float(self.option("pvalue")), float(self.option("fc"))
        dispersion = float(self.option("dispersion"))
        if not (0 <= pvalue <= 1):
            raise OptionError("pvalue cutoff must be in range [0-1]", code = "32002702")
        if not (0 <= dispersion <= 1):
            raise OptionError("Dispersion argument of edgeR must be in range [0-1]", code = "32002703")
        if not fc >= 0:
            raise OptionError("fold change cutoff must be positive", code = "32002704")
        ...
        if diff_groups:
            raise OptionError('Groups:%s in Compare is not in Group info', variables = (diff_groups), code = "32002705")
...
class DiffexpTool(Tool):
    ...
    def __init__(self, config):
        ...
        # self.diff_toolbox = self.config.PACKAGE_DIR + "/denovo_rna_v2/diff_toolbox_new.py"
    ...
    def diffexp(self):
        ...
        cmd += '--exp_type {} '.format(self.option("exp_type"))
        cmd += '-method {} '.format(self.option("method"))
        if self.option("method").lower() == "edger":
            cmd += '--edger_method {} '.format(self.option("edger_method"))
        elif self.option("method").lower() == "degseq":
            cmd += '--degseq_method {} '.format(self.option("degseq_method"))
        elif self.option("method").lower() == "deseq2":
            cmd += '--deseq2_method {} '.format(self.option("deseq2_method"))
    ...
    def set_output(self):
        ...
        for diff_file_stat in diff_files_stat:
            ...
            df_t.rename(columns={"log2fc": "{}_log2fc({}/{})".format(fname,test, ctrl), "fc": "{}_fc({}/{})".format(fname,test, ctrl),"pvalue":"{}_pvalue".format(fname),"padjust":"{}_padjust".format(fname),"significant":"{}_significant".format(fname),"regulate":"{}_regulate".format(fname)},
                          inplace=True)
            df_core = pd.DataFrame(df_t, columns=["{}_log2fc({}/{})".format(fname,test, ctrl),"{}_fc({}/{})".format(fname,test, ctrl),"{}_pvalue".format(fname), "{}_padjust".format(fname), "{}_significant".format(fname), "{}_regulate".format(fname)])
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "Diffexp_pandas" + str(random.randint(1, 10000)),
            "type": "tool",
            "name": "denovo_rna_v2.diffexp",
            "instant": False,
            "options": dict(
                # count="/mnt/ilustre/users/sanger-dev/workspace/20190416/Denovorna_tsg_33857/Quant/output/gene.count.matrix",
                # exp="/mnt/ilustre/users/sanger-dev/workspace/20190416/Denovorna_tsg_33857/Quant/output/gene.tpm.matrix",
                # method="edgeR",
                # group="/mnt/ilustre/users/sanger-dev/workspace/20190416/Denovorna_tsg_33857/remote_input/group/group.txt",
                # cmp= "/mnt/ilustre/users/sanger-dev/workspace/20190416/Denovorna_tsg_33857/remote_input/control/control.txt",
                ...
                pvalue=0.05,
                fc=1,
                #filter_method="min",#average,max,NA
                # tpm_filter_threshold="10",
            )
        }
