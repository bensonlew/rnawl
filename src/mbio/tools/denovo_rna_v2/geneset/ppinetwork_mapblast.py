...
class PpinetworkMapblastAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {"name": "fa", "type": "infile", "format": "denovo_rna_v2.fasta"},  # 输入文件
    ...
    def check_options(self):
        if not self.option("fa").is_set:
            raise OptionError("必须设置参数fa", code = "32004201")
        if self.option('query_type') not in ['nucl', 'prot']:
            raise OptionError('query_type查询序列的类型为nucl(核酸)或者prot(蛋白):%s', variables = (self.option('query_type')), code = "32004202")
        else:
            if self._fasta_type[self.option('fa').prop['seq_type']] != self.option('query_type'):
                raise OptionError(
                    '文件检查发现查询序列为:%s, 而说明的文件类型为:%s', variables = (
                        self._fasta_type[self.option('fa').prop['seq_type'], self.option('query_type')]), code = "32004203")
        if self.option("database") == 'customer_mode':
            ...
            raise OptionError("数据库%s不被支持", variables = (self.option("database")), code = "32004204")
        else:
            self.option('reference_type', self._database_type[self.option("database").lower()])
        if not 15 > self.option('outfmt') > -1:
            raise OptionError('outfmt遵循blast+输出规则，必须为0-14之间：%s', variables = (str(self.option('outfmt'))), code = "32004205")
        if not 1 > self.option('evalue') >= 0:
            raise OptionError('E-value值设定必须为[0-1)之间：%s', variables = (str(self.option('evalue'))), code = "32004206")
        if not 0 < self.option('num_alignment') < 1001:
            raise OptionError('序列比对保留数必须设置在1-1000之间：%s', variables = (str(self.option('num_alignment'))), code = "32004207")
...
class PpinetworkMapblastTool(Tool):
    def __init__(self, config):
        ...
        # self.species_path = self.config.SOFTWARE_DIR + '/database/stringDB_10.5/split/' + str(self.option("species")) + ".fa"
    ...
    def run_makedb_and_blast(self):
        ...
        if makedb_obj.return_code == 0:
            ...
            self.set_error("makeblastdb运行出错!", code = "32004208")
    ...
    def run_blast(self):
        ...
        if blast_command.return_code == 0:
            ...
            self.set_error("blast运行出错!", code = "32004209")
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        test_dir = '/mnt/ilustre/users/sanger-dev/sg-users/liubinxu/test_denovo_rna_v2/data4'
        data = {
            "id": "mapblast" + str(random.randint(1, 10000)),
            "type": "tool",
            "name": "denovo_rna_v2.geneset.ppinetwork_mapblast",
            "instant": True,
            "options": dict(
