...
class PpinetworkPredictAgent(Agent):
    ...
    def check_options(self):
        ...
        if not self.option('diff_exp_mapped'):
            raise OptionError("必须输入含有STRINGid的差异基因表", code = "32004301")
        line = open(self.option('diff_exp_mapped'), "r").readlines()[1:]
        if not line:
            raise OptionError("基因集中的基因不能匹配到string数据库中id，请您确认基因id为Ensemble或者Entrez GeneID！", code = "32004302")
        if not isinstance(self.option('combine_score'), int) or self.option('combine_score') < 0:
            raise OptionError("combined_score值必须是大于0的整数！", code = "32004303")
...
class PpinetworkPredictTool(Tool):
    ...
    def run_PPI(self):
        ...
        if cmd.return_code == 0:
            ...
            self.set_error("运行one_cmd出错", code = "32004304")
