...
import time
from mbio.packages.denovo_rna_v2.kegg_regulate import KeggRegulate
...
class KeggClassTool(Tool):
    ...
    def get_kegg_pics(self):
        ...
        if not result:
            self.set_error('geneset with main_id: %s was not found', variables = (geneset_id), code = "32004801")
    ...
    def download_s3_file(self, path, to_path):
        ...
        if os.path.exists(to_path):
            ...
            try:
                ...
                self.set_error('file can not find %s', variables=(path), code="32004803")
        return to_path
    ...
    def get_kegg_png(self):
        ...
        if geneset_type == 'G':
            kegg_pathways = os.path.join(kegg_annot_output, 'AnnoStat', 'KEGG', 'unigene_pathways')
        else:
            kegg_pathways = os.path.join(kegg_annot_output, 'AnnoStat', 'KEGG', 'transcript_pathways')
    ...
    def get_dicts(self):
        ...
        # file kegg_table中获取该对应关系方法错误（对应多个K时）
        self.get_path_ko(ko_genes.keys(), path_ko.keys())
        self.path_ko = self.path2ko
        self.geneset_gene = regulate_gene  # 基因集与基因的对应关系

    def get_path_ko(self, kos, paths):
        '''
        获取通路ko 与 大 K的对应关系
        '''
        for ko in kos:
                ...
                for index, i in enumerate(pids):
                    ...
                    if map_id in paths:
                        if map_id not in self.gloabl:
                            if map_id in self.path2ko:
                                if ko in self.path2ko[map_id]:
                                    pass
                                else:
                                    self.path2ko[map_id].append(ko)
                            else:
                                self.path2ko[map_id] = [ko]

        '''
        使用分级分类注释是错的也会出错
        path2ko = dict()
        with open(self.option('kegg_table_2').prop['path'], 'r') as kegg2_f:
            kegg2_f.readline()
            for line in kegg2_f.readlines():
                cols = line.split('\t')
                map_id = cols[0]
                hyperlink = cols[6]
                KOs = hyperlink.split(map_id)
                if len(KOs) > 1:
                    for KO in KOs[1].split("/"):
                        if KO.startswith("K"):
                            ko = KO.split('%')[0]
                            if map_id in path2ko:
                                path2ko[map_id].append(ko)
                            else:
                                path2ko[map_id] = [ko]
                        else:
                            pass
                    ...
                    pass
            self.path_ko = path2ko
        '''

    def get_kegg_stat_tmp(self):
        self.logger.info("生成kegg_stat_tmp.xls文件")
        self.logger.info("ko_genes {} path_ko{}".format(self.ko_genes, self.path_ko))
