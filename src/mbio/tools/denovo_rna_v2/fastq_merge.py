...
class FastqMergeAgent(Agent):
    """
    Merge fastq file
    """
    def __init__(self, parent):
        super(FastqMergeAgent, self).__init__(parent)
        options = [
    ...
    def check_options(self):
        if not self.option('sample_fq_list'):
            raise OptionError("样本列表文件需提供依次为sample_id\tleft.fq\tright.fq", code = "32003201")
        return True
    ...
    def end(self):
        ...
        """
        super(FastqMergeAgent, self).end()


class FastqMergeTool(Tool):
    """
    Merge fastq file
    """
    def __init__(self, config):
        super(FastqMergeTool, self).__init__(config)
        self.filter_samples = []
        self.right_fq = []
        self.left_fq = []
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "FastqMerge" + str(random.randint(1, 10000)),
            "type": "tool",
