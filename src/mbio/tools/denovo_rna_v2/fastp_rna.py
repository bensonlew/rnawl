...
class FastpRnaAgent(Agent):
    ...
    def __init__(self, parent=None):
        ...
        options = [
            ...
            {"name": "low_complexity_filter", "type": "bool", "default": True},  # 是否过滤掉低复杂度的序列
            {"name": "trim_poly_x", "type": "bool", "default": True},  # 是否对双端数据进行校正
    ...
    def check_options(self):
        ...
        if not self.option('fq1'):
            raise OptionError('必须输入1端序列', code="32007301")
        if not self.option('fq2'):
            raise OptionError('必须输入2端序列', code="32007302")
        return True
...
class FastpRnaTool(Tool):
    ...
    def run_fastp(self):
        ...
        if self.option("low_complexity_filter"):
            cmd += " -y"
        if self.option("trim_poly_x"):
            cmd += " -x"
        ...
        if command.return_code == 0:
            ...
            self.set_error("fastp运行出错!", code="32007301")
    ...
    def set_output(self):
        ...
        try:
            ...
            self.set_error("设置fastp分析结果目录失败%s", variables=(e), code="32007302")
