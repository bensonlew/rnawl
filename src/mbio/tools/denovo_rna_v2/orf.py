...
class OrfAgent(Agent):
    ...
    def check_options(self):
        ...
        if not self.option("fasta").is_set:
            raise OptionError("请传入fasta序列文件", code="32005101")
...
class OrfTool(Tool):
    ...
    def td_longorfs(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("提取orf过程出错", code="32005101")
    ...
    def td_predict(self, hmm_out=None):
        ...
        if command.return_code == 0:
            ...
            self.set_error("预测过程过程出错", code="32005102")
    # hmmer3.0的页面默认参数没这么复杂，并且选择的参数是--cut_ga，我们还最好加一个-E参数，这个可以控制E_value的输出值，最后的pep事transdecoder
    # 得出的那个transcripts.fasta.transdecoder.pep
    # 这个pep参数输入绝对路径和相对路径测试都能通过
    def hmmscan(self, pep):
        cmd = "{}hmmscan -E {} --cpu 20 --noali --acc --notextw --domtblout {} --tblout {} {} {}".format(
            self.hmmscan_path, self.option("e_value"), "pfam.domtblout", "pfam.tblout", self.pfam_db, pep)
        print(cmd)
        ...
        if command.return_code == 0:
            ...
            self.set_error("运行hmmscan出错", code="32005103")
    ...
    def run(self):
        ...
        if self.option("search_pfam") is True:
            ...
            with open(self.work_dir + "/" + self.fasta_name + ".transdecoder.pep", "r") as cds_seq, open(self.work_dir + "/total_len_cds_list_only_unigene.txt","w") as total_cds_unigene, open(self.work_dir + "/total_len_cds_list_only_transcript.txt", "w") as total_cds_transcript:
                ...
                # isoform_unigene.columns = ["query_name", "unigene", "yes_no"]
                cds_only_transcript = pd.merge(total_cds_len_list, isoform_unigene, on="query_name")
                # 按照区间范围统计cds的长度范围,文本的读入都是字符串，所以前面需要类型转换下，或者df[数字列].astype(int, inplace=True)
            ...
            with open(self.work_dir + "/pfam.tblout") as tbout, open(self.work_dir + "/best_1_domain.tblout", "w") as best_1_domain:
