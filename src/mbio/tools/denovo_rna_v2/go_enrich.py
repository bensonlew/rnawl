...
from mbio.packages.denovo_rna_v2.go_graph import draw_GO
import subprocess
import unittest
import pandas as pd
...
class GoEnrichAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            dict(name="diff_list", type="infile", format="denovo_rna_v2.gene_list"),
            # dict(name="all_list", type="infile", format="denovo_rna_v2.gene_list"),
            dict(name="go_list", type="infile", format="denovo_rna_v2.go_list"),
...
class GoEnrichTool(Tool):
    ...
    def __init__(self, config):
        ...
        self.go_enrich_stats_path=os.path.join(self.config.PACKAGE_DIR, 'ref_rna_v2/geneset/go_enrich_stats.py')
        self.outfile=os.path.join(self.work_dir, 'outfile.txt')
        self.result_file= os.path.join(self.output_dir, 'go_enrich_geneset_list_gene.xls')
    ...
    def run_enrich(self):
        ...
        cmd = cmd + ' --pval ' + self.option('pval') + ' --indent' + ' --method ' + self.option('method') + ' --outfile ' + self.outfile
        ...
        if command.return_code == 0:
            ...
            self.set_error('goatools计算错误', code = "32004603")
    ...
    def run_draw_go_graph(self):
        try:
            ...
            self.set_error('绘图发生错误:\n%s', variables = (traceback.format_exc()), code = "32004604")

    def run_go_enrich_stats(self):
        cmd = '{} {}'.format(self.python_path, self.go_enrich_stats_path)
        cmd += ' -i {}'.format(self.outfile)
        cmd += ' -o {}'.format(self.result_file)
        ...
        if command.return_code == 0:
            ...
            self.set_error('go_enrich_stats计算错误')
    ...
    def get_go_pvalue_dict(self):
        ...
        #
        # tar_adjust = sorted(go2padjust.items(), key=lambda e:go2pvalue[e[0]], reverse=True)
        ...
        # new_go2padjust = dict(tar_adjust[-10:])
        # new_go2pvalue = dict(tar[-10:])
        df = pd.read_table(self.outfile, sep='\t', index_col=0)
        df = df[df['enrichment'] != 'p']
        method = self.option('method').lower()
        p_select = "p_fdr_bh"
        if method == 'sm_bonferroni':
            p_select = "p_sm_bonferroni"
        elif method == 'fdr_bh':
            p_select = "p_fdr_bh"
        elif method == 'fdr_by':
            p_select = "p_fdr_by"
            ...
            p_select = "p_sm_holm"
        df_10 = df.sort_values(by=['p_uncorrected', p_select], ascending=True)[0:10]
        for i in df_10.index:
            go2pvalue[i] = df_10.loc[i]['p_uncorrected']
            go2padjust[i] = df_10.loc[i][p_select]
