...
class RefAnnoQueryAgent(Agent):
    ...
    def check_options(self):
        if not self.option("gos_list"):
            raise OptionError('缺少输入文件:gos_list', code = "32001501")
        if not self.option("kegg_table"):
            raise OptionError('缺少输入文件:kegg_table', code = "32001502")
        if self.option("ref_gtf_path"):
            if not self.option("length_path"):
                raise OptionError('缺少输入文件:length_path', code = "32001503")
            if not self.option("gene_file"):
                raise OptionError('缺少输入文件：gene_file', code = "32001504")
        if self.option("new_gtf_path") or self.option("gene2trans") :
            if not self.option("blast_nr_table"):
                raise OptionError("缺少nr注释文件", code = "32001505")
            if not self.option("blast_swissprot_table"):
                raise OptionError("缺少swissprot注释文件", code = "32001506")
...
class RefAnnoQueryTool(Tool):
    def __init__(self, config):
        ...
        if self.option("gene2trans"):
            self.query_path = self.config.PACKAGE_DIR + "/denovo_rna_v2/new_annotation_query.py"
        else:
    ...
    def run_query(self):
        ...
        try:
            ...
            self.set_error("运行注释查询脚本失败", code = "32001507")
        # self.set_output()
        self.end()
