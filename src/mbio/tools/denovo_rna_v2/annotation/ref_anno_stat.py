...
class RefAnnoStatAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "eggnog_cog_table", "type": "infile", "format": "ref_rna_v2.eggnog_cog_table"},
    ...
    def check_options(self):
        ...
        if len(self.anno_database) < 1:
            raise OptionError('至少选择一种注释库', code = "32001601")
        if self.option("gene2trans").is_set:
            pass
        elif not self.option("ref_genome_gtf").is_set:
            raise OptionError("缺失参考基因组gtf文件", code = "32001602")
        else:
            pass

        for i in self.anno_database:
            if i not in ['nr', 'go', 'cog', 'pfam', 'kegg', 'swissprot']:
                raise OptionError('需要注释的数据库不在支持范围内[nr, go, cog, pfam, kegg, swissprot]:%s', variables = (i), code = "32001603")
            if i == 'go':
                if self.option('gos_list').is_set and self.option('blast2go_annot').is_set:
                    ...
                    raise OptionError('缺少go注释的输入文件', code = "32001604")
            if i == 'cog':
                if self.option('string_xml').is_set:
                    ...
                    raise OptionError('进行cog注释统计必须输入blast到string数据库的xml文件或table文件', code = "32001605")
                # if not self.option('cog_summary').is_set:
                #    raise OptionError('缺少cog注释的输入文件:cog_summary', code = "32001606")
            if i == 'nr' and not self.option('nr_xml').is_set:
                raise OptionError('缺少nr注释的输入文件', code = "32001607")
            if i == "pfam" and not self.option('pfam_domain').is_set:
                raise OptionError('缺少pfam注释的输入文件', code = "32001608")
            if i == 'kegg':
                if self.option('kegg_xml').is_set:
                    ...
                    raise OptionError('缺少kegg注释的输入文件', code = "32001609")
            if i == 'swissprot' and not self.option('swissprot_xml').is_set:
                raise OptionError('缺少swissprot注释的输入文件', code = "32001610")
        if self.option('gene2trans').is_set:
            pass
        elif not self.option('gene_file').is_set:
            raise OptionError('缺少gene输入文件', code = "32001611")
        else:
            pass
...
class RefAnnoStatTool(Tool):
    ...
    def run_nr_stat(self):
        ...
        gene2trans().get_gene_blast_xml_from_trans(tran_list=self.gene_list, tran_gene=self.tran_gene, xml_path=self.option('nr_xml').prop["path"], gene_xml_path=self.gene_nr_xml)
    ...
    def run_cog_stat(self):
        ...
        if self.option("eggnog_cog_table").is_set:
            ...
            gene2trans().get_gene_blast_xml_from_trans(tran_list=self.gene_list, tran_gene=self.tran_gene, xml_path=self.option('string_xml').prop['path'], gene_xml_path=self.gene_string_xml)
            ...
            if cog_cmd.return_code == 0:
                ...
                self.set_error("cog_cmd运行出错!", code = "32001614")
        else:
    ...
    def run_kegg_stat(self):
        ...
        if self.option("kegg_xml").is_set:
            ...
            gene2trans().get_gene_blast_xml_from_trans(tran_list=self.gene_list, tran_gene=self.tran_gene, xml_path=self.option('kegg_xml').prop['path'], gene_xml_path=self.gene_kegg_xml)
        ...
        if self.option("kegg_xml").is_set:
            ...
            cmd = "{} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} ".format(self.python_path, self.kegg_path, self.r_path, self.map_path, self.gene_kegg_xml, None, kegg_table, pidpath, gene_pathway, pathway_table, layerfile, taxonomy, self.option("link_bgcolor"), self.option("png_bgcolor"), self.image_magick,self.html_path)
        else:
            ...
            cmd = "{} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} ".format(self.python_path, self.kegg_path, self.r_path, self.map_path, None, kegg_ids, kegg_table, pidpath, gene_pathway, pathway_table, layerfile, taxonomy, self.option("link_bgcolor"), self.option("png_bgcolor"), self.image_magick,self.html_path)
        ...
        if command.return_code == 0:
            ...
            self.set_error("运行kegg注释脚本出错", code = "32001615")
    ...
    def run_go_stat(self):
        ...
        if go_annot_cmd.return_code == 0:
            ...
            self.set_error("go_annot_cmd运行出错!", code = "32001616")
    ...
    def run_swissprot_stat(self):
        ...
        gene2trans().get_gene_blast_xml_from_trans(tran_list=self.gene_list, tran_gene=self.tran_gene, xml_path=self.option('swissprot_xml').prop['path'], gene_xml_path=self.gene_swissprot_xml)
        ...
        try:
            ...
            self.set_error("运行swissprot evalue,similar for gene swissprot blast table出错:%s", variables = (e), code = "32001617")
        ...
        try:
            ...
            for db in self.database:
                if db == 'cog':
                    ...
                    if self.option("cog_summary").is_set:
            ...
            self.set_error("设置注释统计分析结果目录失败 %s", variables = (e), code = "32001618")
    ...
    def movedir2output(self, olddir, newname, mode='link'):
        ...
        if not os.path.isdir(olddir):
            self.set_error('需要移动到output目录的文件夹不存在。', code = "32001619")
    ...
    def get_all_anno_stat(self):
        ...
        with open(all_anno_stat, 'wb') as w:
            ...
            for db in ['nr', 'swissprot', 'pfam', 'cog', 'go', 'kegg']:
