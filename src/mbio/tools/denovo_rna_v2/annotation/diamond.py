...
class DiamondAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "outxml", "type": "outfile", "format": "denovo_rna_v2.blast_xml"},
    ...
    def check_options(self):
        if not self.option("query").is_set:
            raise OptionError("必须设置参数query", code = "32000601")
        if self.option('query_type') not in ['nucl', 'prot']:
            raise OptionError('query_type查询序列的类型为nucl(核酸)或者prot(蛋白):%s', variables = (str(self.option('query_type'))), code = "32000602")
        if not 1 > self.option('evalue') >= 0:
            raise OptionError('E-value值设定必须为[0-1)之间：%s', variables = (str(self.option('evalue'))), code = "32000603")
        if not 0 <= self.option("sensitive") <= 2:
            raise OptionError('敏感度设定必须为[0-2]之间：%s', variables = (str(self.option('sensitive'))), code = "32000604")
        return True
...
class DiamondTool(Tool):
    def __init__(self, config):
        ...
        if self.option("database").startswith("kegg"):
            self.db_path = AnnotConfig().get_file_path(
                file=self.option("database"),
                version=self.option("version"),
                soft="diamond",
    ...
    def run_makedb_and_diamond(self):
        ...
        if makedb_obj.return_code == 0:
            ...
            self.set_error("创建diamond数据库出错!", code = "32000605")
                ...
                if line.lstrip().startswith("<Hit_id>"):
                    ...
                    if m:
                        ...
                        try:
                            ...
                            print line
        # 删除new文件防止重运行读写同一个文件出错
        if os.path.exists(path + "_new"):
            os.remove(path + "_new")
    ...
    def run_code(self, cmd_name, cmd, shell=False, memory_err_code=[1]):
        if shell:
