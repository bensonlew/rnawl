...
class SwissprotAnnotTool(Tool):
    ...
    def run_swissprot_stat(self):
        try:
            blastout_statistics(blast_table=self.txpt_tsv, evalue_path=self.output_dir + '/trans_swissprot_evalue.xls', similarity_path=self.output_dir + '/trans_swissprot_similar.xls')
            blastout_statistics(blast_table=self.gene_tsv, evalue_path=self.output_dir + '/gene_swissprot_evalue.xls', similarity_path=self.output_dir + '/gene_swissprot_similar.xls')
            self.logger.info("End: evalue,similar for gene swissprot blast table ")
        except Exception as e:
            self.set_error("运行swissprot evalue,similar for gene swissprot blast table出错:%s", variables = (e), code = "32001612")
