...
class SingleQuantAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            dict(name="transcriptome", type="infile", format="denovo_rna_v2.fasta"),
    ...
    def check_options(self):
        if self.option("method").lower() not in ["rsem", "salmon", "kallisto"]:
            raise OptionError("Method is incorrect", code = "32005401")
        if not self.option("transcriptome").is_set:
            raise OptionError("transcriptome not exist", code = "32005402")
        if self.option("map_tool").lower() not in ["bowtie", "bowtie2", "star"]:
            raise OptionError("map_tool/aligner is incorrect", code = "32005403")
        if self.option("libtype") is not None:
            if self.option("libtype").lower() not in ["fr", "rf"]:
                raise OptionError("libtype argument is not in [None, 'fr', 'rf']", code = "32005404")
...
class SingleQuantTool(Tool):
    ...
    def __init__(self, config):
        ...
        self.set_environ(PATH=self.samtools_path)
    ...
    def quant(self):
        ...
        if command.return_code == 0:
            ...
            if command.return_code is 0:
                ...
                self.set_error("运行>>>%s出错", variables = (cmd), code = "32005401")
        else:
            self.set_error("运行>>>%s出错", variables = (cmd), code = "32005402")
...
class TestFunction(unittest.TestCase):
    ...
    data = {
        ...
        'type': 'tool',
        'name': 'denovo_rna_v2.single_quant',
        'instant': False,
        'options': dict(
            transcriptome="/mnt/ilustre/users/sanger-dev/workspace/20190920/Denovorna_tsg_35592/DenovoAssemble2Filter/output/Trinity.filter.fasta",
            fastq="HGL4;/mnt/ilustre/users/sanger-dev/workspace/20190920/Denovorna_tsg_35592/FastpRna/output/fastq/HGL4.clean.1.fastq;/mnt/ilustre/users/sanger-dev/workspace/20190920/Denovorna_tsg_35592/FastpRna/output/fastq/HGL4.clean.2.fastq",
            method="rsem",
            t2g="/mnt/ilustre/users/sanger-dev/workspace/20190920/Denovorna_tsg_35592/DenovoAssemble2Filter/output/Trinity.filter.gene_trans_map",
            pool=1,
            thread=6,
            output=None,
            read_len=149,
...
if __name__ == '__main__':
    ...
    suite.addTests([TestFunction('test_rsem')])
