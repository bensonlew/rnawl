...
class FastxClipperAgent(Agent):
    ...
    def check_options(self):
        ...
        if not self.option("fastq_dir").is_set and not self.option("fastq_s").is_set:
            raise OptionError("请传入SE序列文件或者文件夹", code="32007901")
...
class FastxClipperTool(Tool):
    ...
    def fastxclipper(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("运行fastx_clipper运行出错!", code="32007901")
            return False
        ...
        if self.option("fastq_dir").is_set:
            ...
            for cmd in commands:
                if cmd.return_code == 0:
                    ...
                    self.set_error("运行%s运行出错!", variables=(cmd.name), code="32007902")
                    return False
            self.set_output()
            # self.write_list()
        else:
