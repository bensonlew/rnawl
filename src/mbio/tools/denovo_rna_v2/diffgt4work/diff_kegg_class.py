...
class DiffKeggClassTool(Tool):
    ...
    def get_dicts(self):
        ...
        if self.species_abr == 'map':
            ko_genes, path_ko, _ = self.option('kegg_table').get_pathway_koid2( kegg_version=self.option('kegg_version'))
        else:
        ...
        with open('gene2set',"w") as t:
            t.write(str(self.gene2set))
        with open('ko2set',"w") as t:
            t.write(str(geneset2_ko))
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            "type": "tool",
            "name": "medical_transcriptome.diffgt4work.diff_kegg_class",
            "instant": False,
            "options": dict(
                geneset_kegg="/mnt/ilustre/users/sanger-dev/workspace/20201105/MedicalTranscriptome_qbavpmefnfs8qae5l8290vaq2i/DiffGenesetAllPipline/DiffGenesetAll/DiffGenesetPrepare/output/kegg_class/multi_geneset_list",
                #group_dict=r'{"A":["A1", "A2", "A3"],"B": [ "B1", "B2", "B3"], "C": [ "C1", "C2", "C3"]}'.replace('"', '\\"'),
                level="G",
                kegg_table="/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/test/test20201110/gene_kegg_table.xls",
                kegg_table2="/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/test/test20201110/gene_kegg_level_table.xls",
                background_links="/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/test/test20201110/add_info.txt",
                source="diff_exp",
