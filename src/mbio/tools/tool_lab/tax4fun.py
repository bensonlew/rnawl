...
class Tax4funAgent(Agent):
    def __init__(self, parent):
        ...
        options = [
            {"name": "database", "type": "string", "default":"silva132/16s_bacteria"},
    ...
    def check_options(self):
        ...
        if not os.path.isfile(self.option("in_otu_table").prop['path']):
            raise OptionError("otu表不存在!", code="32707201")
...
class Tax4funTool(Tool):
    ...
    def __init__(self, config):
        ...
        if self.option('database') in ['silva119/16s_bacteria', 'silva119/16s_archaea', 'silva119/16s']:
            ...
            self.database = self.config.SOFTWARE_DIR + '/database/meta/tax4fun/SILVA123'
        self.modified_taxon = self.config.SOFTWARE_DIR + '/database/taxon_db/origi/uniq_name/' + database_dict[str(self.option('database'))][0]
        self.origi_taxon = self.config.SOFTWARE_DIR + '/database/taxon_db/origi/uniq_name/' + database_dict[str(self.option('database'))][1]
    ...
    def restore_spe_name(self):
        ...
        for line in lines[1:]:
            ...
            split = "\t".join(split[1:])
            out.write("OTU" + str(count) + "\t" + split + "\t" + temp + ";\n")
        out.close()
        record.close()
