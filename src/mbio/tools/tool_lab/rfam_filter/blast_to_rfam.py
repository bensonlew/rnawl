...
class BlastToRfamAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "fq_type", "type": "string", "default": "PE"},  # 评估软件 ["PE","SE"]
            {"name": "query", "type": "infile", "format": "ref_rna_v2.common"},  # 输入序列文件
            {"name": "query1", "type": "infile", "format": "ref_rna_v2.common"},  # 输入序列文件
...
class BlastToRfamTool(Tool):
    def __init__(self, config):
        ...
        self.query_name, self.blast_xml, self.table_out,self.detail_out = str(), str(), str() ,str()
        self.target_seqs = []
    ...
    def fastq_to_fasta(self, fq_fp, fa_fp):
        ...
        cmd1 = "{} seq -a {} > {}".format(self.seqtk,fq_fp,fa_fp)
    ...
    def blast_rfam(self):
        ...
        if self.option('seq_type') == "fastq":
            self.input_file = self.fastq_to_fasta(input_file_fastq, input_file)
        else:
            self.input_file = input_file_fastq
            input_file = input_file_fastq
    ...
    def run_rfam_stat(self):
        ...
        self.detail_out = self.work_dir + "/" + os.path.basename(rfam_table)[:-3] + "detail.xls"
        stat_out = self.output_dir + "/" + os.path.basename(rfam_table)[:-3] + "stat.xls"
        cmd = "{} {} -i {} -db {} -o {}".format(self.python, self.rfam_stat, rfam_table, self.rfam_seed, self.detail_out)
        ...
        with open(self.detail_out, "r") as f, open(stat_out, "w") as w:
            w.write("Sample\trRNA(%)\n")
            ...
            for line in f:
                ...
                if items[6] == "rRNA":
                    rRNA_num += 1
                    self.target_seqs.append(items[0])
            self.logger.info("rRNA_num: %s, total_num: %s" % (rRNA_num, total_num))
            w.write(self.query_name + "\t" + str(float(rRNA_num) / float(total_num) * 100) + "\n")

    def filter_seq(self):
        if self.option('seq_type') == "fastq":
            target_seqs = os.path.join(self.work_dir,"target_seqs")
            with open(target_seqs,"w") as t:
                t.write("\n".join(self.target_seqs))
            if self.option("fq_type") == "PE":
                self.extract_pe_seqs(self.target_seqs)
            else:
                self.extract_se_seqs(self.target_seqs)
        else:
            self.extract_fasta_seqs(self.target_seqs)


    def extract_pe_seqs(self,target_seqs):
        filter_left_fq = os.path.join(self.output_dir,self.option("sample_name") + "_input_rmrRNA.1.fastq")
        filter_right_fq = os.path.join(self.output_dir, self.option("sample_name") + "_input_rmrRNA.2.fastq")
        l_sequences = list()
        r_sequences = list()
        for l, r in zip(SeqIO.parse(self.option("query").prop["path"], 'fastq'),SeqIO.parse(self.option("query1").prop["path"], 'fastq')):
            if not l.id in  target_seqs:
                l_sequences.append(l)
                r_sequences.append(r)
        SeqIO.write(l_sequences, filter_left_fq, 'fastq')
        SeqIO.write(r_sequences, filter_right_fq, 'fastq')

    def extract_se_seqs(self, target_seqs):
        filter_s_fq = os.path.join(self.output_dir, self.option("sample_name") + "_input_rmrRNA.fastq")
        sequences = list()
        for i, r in enumerate(SeqIO.parse(self.option("query").prop["path"], 'fastq')):
            if not r.id in target_seqs:
                sequences.append(r)
        SeqIO.write(sequences, filter_s_fq, 'fastq')

    def extract_fasta_seqs(self, target_seqs):
        filter_fa = os.path.join(self.output_dir, self.option("sample_name") + "_input_rmrRNA.fasta")
        sequences = list()
        for i, r in enumerate(SeqIO.parse(self.option("query").prop["path"], 'fasta')):
            if not r.id in target_seqs:
                sequences.append(r)
        SeqIO.write(sequences, filter_fa, 'fasta')
...
class TestFunction(unittest.TestCase):
    def test(self):
        ...
        data = {
            "id": "BlastRfamfilter" + str(random.randint(1, 10000)),
            "type": "tool",
            "name": "tool_lab.rfam_filter.blast_to_rfam",
            "instant": False,
            "options": dict(
                # query="/mnt/ilustre/users/sanger-dev/workspace/20210224/Single_BlastRfam2031/BlastToRfam/test.fa",
                query="/mnt/ilustre/users/sanger-dev/workspace/20210203/Refrna_tsg_249609/remote_input/fastq_dir/rawdata/CL1.1.fq",
                query1 = "/mnt/ilustre/users/sanger-dev/workspace/20210203/Refrna_tsg_249609/remote_input/fastq_dir/rawdata/CL1.2.fq",
                seq_type = "fastq",
                sample_name ="test"
            )
        }
