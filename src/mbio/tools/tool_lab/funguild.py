...
class FunguildTool(Tool):
    ...
    def change_format_fun(self):
        ...
        data['taxonomy'] = data['taxonomy'].apply(lambda x : ';'.join(str(x).split(';')[:-1]))
    ...
    def run_stat(self):
        ...
        sub_data['Guild'] = sub_data['Guild'].apply(lambda x: 'unknown' if  str(x) == '-' else x)
        ...
        # FUNGuild_Trophic_mode.txt
        head_list3 = data.columns.tolist()
        sample_list3 = head_list3[1:head_list3.index('Trophic Mode')]
        sub_data3 = data[sample_list + ['Trophic Mode']]
        # sub_data['Guild'] = sub_data['Guild'].apply(lambda x: str(x).split('-')[0] if  str(x) != '-' else x)
        sub_data3['Trophic Mode'] = sub_data3['Trophic Mode'].apply(lambda x: 'unknown' if str(x) == '-' else x)
        data3 = sub_data3.groupby('Trophic Mode').sum()

        self.stat_out3 = 'FUNGuild_Trophic_mode.txt'
        data3.to_csv(self.stat_out3, sep='\t')
    ...
    def set_output(self):
        ...
        Trophic_mode_data = os.path.join(self.output_dir, self.stat_out3)
        for i in detail_data,linksum_data,Trophic_mode_data:
            if os.path.exists(i):
               os.remove(i)
        os.link(self.work_dir + '/'+ self.guild_file, detail_data)
        os.link(self.work_dir + '/'+self.stat_out, linksum_data)
        os.link(self.work_dir + '/' + self.stat_out3, Trophic_mode_data)
        self.end()
