...
class FastaLenSplitAgent(Agent):
    '''
    last_modify: 2019.04.09
    '''
    def __init__(self, parent):
        super(FastaLenSplitAgent, self).__init__(parent)
        options = [
    ...
    def end(self):
        super(FastaLenSplitAgent, self).end()

class FastaLenSplitTool(Tool):
    def __init__(self, config):
        super(FastaLenSplitTool, self).__init__(config)
        # self.cmd_path = "bioinfo/align/ncbi-blast-2.3.0+/bin"

    def run(self):
        super(FastaLenSplitTool, self).run()
        self.len_split()
        self.set_output()
        self.end()
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            'id': 'len_split_{}_{}'.format(random.randint(1000, 10000), random.randint(1000, 10000)),
            'type': 'tool',
