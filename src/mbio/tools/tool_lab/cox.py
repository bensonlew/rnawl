...
class CoxTool(Tool):
    def __init__(self, config):
        ...
        self.script = {
            'cox': os.path.join(self.config.PACKAGE_DIR, 'tool_lab/cox.py'),
        }
        self.file = {
            'cox_table': os.path.join(self.output_dir, 'cox.txt'),
            'cox_graph': os.path.join(self.output_dir, 'cox.pdf')
        }
    ...
    def cox(self):
        ...
        self.wait()
        cox_failed = False
        with open(os.path.join(self.work_dir, 'run_cox.o'), 'r') as c:
            for line in c.readlines():
                if 'Ran out of iterations and did not converge' in line:
                    cox_failed = True
                    break
        if command.return_code == 0:
            if cox_failed is False:
                self.logger.info("{} Finished successfully".format(cmd_name))
            else:
                self.set_error("Ran out of iterations and did not converge, 样本数过少或者表达量表和生存统计表中0值和缺失值过多，导致无法计算，请重新选择参数运行")
