...
class PanGenomeTool(Tool):
    def __init__(self, config):
        ...
        self.r_path = self.config.SOFTWARE_DIR + "/program/miniconda3/envs/R_v4/bin:"
        self._LD_LIBRARY_PATH = self.config.SOFTWARE_DIR + "/program/miniconda3/envs/R_v4/lib:"
        self.set_environ(PATH=self.r_path, R_HOME=self.r_home, LD_LIBRARY_PATH=self._LD_LIBRARY_PATH, C_INCLUDE_PATH=self.c_include)
        self.set_environ(PATH=self.path, PERL5LIB=self.perl5path)
        self.core_plot = self.config.SOFTWARE_DIR + "/bioinfo/compare_genome/software/get_homologues-master/plot_pancore_matrix.pl"

        # 画静态图脚本 20210416
        self.newgene_bar_r = self.config.PACKAGE_DIR + '/tool_lab/pang_newgene_plot.py'
        self.r_script_path = self.config.SOFTWARE_DIR + '/program/miniconda3/envs/R_v4/bin/Rscript '
    ...
    def calculate_newgene(self):
        ...
        cmd2 = "{} {} {} {} {} {}".format(self.python, self.newgene_bar_r, infile_path, infile_path + '.log',
                                          'out_result/new_gene_cluster.xls_newgene.png', self.r_script_path)
        self.logger.info(cmd2)
        command2 = self.add_command('newgene_plot', cmd2).run()
        self.wait(command2)

        if command2.return_code == 0:
            self.logger.info('运行R脚本完成')
        else:
            self.logger.info('运行R脚本失败')
    ...
    def plot_core(self, method):
        ...
        elif method in ["core_Tettelin", "core_Willenbrock", "core_both"]:
