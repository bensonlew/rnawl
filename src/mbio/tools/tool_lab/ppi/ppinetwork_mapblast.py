...
class PpinetworkMapblastAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {"name": "fa", "type": "infile", "format": "ref_rna_v2.fasta"},  # 输入文件
            ...
            {"name": "outtable", "type": "outfile", "format": "ref_rna_v2.common"},  # 输出格式为6时输出
    ...
    def check_options(self):
        if not self.option("fa").is_set:
            raise OptionError("必须设置参数fa", code = "33706801")
        if self.option('query_type') not in ['nucl', 'prot']:
            raise OptionError('query_type查询序列的类型为nucl(核酸)或者prot(蛋白):%s', variables = (self.option('query_type')), code = "33706802")
        else:
            if self._fasta_type[self.option('fa').prop['seq_type']] != self.option('query_type'):
                raise OptionError(
                    '文件检查发现查询序列为:%s, 而说明的文件类型为:%s', variables = (
                        self._fasta_type[self.option('fa').prop['seq_type'], self.option('query_type')]), code = "33706803")
        if self.option("database") == 'customer_mode':
            ...
            raise OptionError("数据库%s不被支持", variables = (self.option("database")), code = "33706804")
        else:
            self.option('reference_type', self._database_type[self.option("database").lower()])
        if not 15 > self.option('outfmt') > -1:
            raise OptionError('outfmt遵循blast+输出规则，必须为0-14之间：%s', variables = (self.option('outfmt')), code = "33706805")
        if not 1 > self.option('evalue') >= 0:
            raise OptionError('E-value值设定必须为[0-1)之间：%s', variables = (self.option('evalue')), code = "33706806")
        if not 0 < self.option('num_alignment') < 1001:
            raise OptionError('序列比对保留数必须设置在1-1000之间:%s', variables = (self.option('num_alignment')), code = "33706807")
...
class PpinetworkMapblastTool(Tool):
    ...
    def run_makedb_and_blast(self):
        ...
        cmd += " -dbtype %s -in %s -out %s " % (self.option('reference_type'), 'unmapped_db.fa', 'unmapped_db')
        ...
        if makedb_obj.return_code == 0:
            ...
            self.set_error("makeblastdb运行出错!", code = "33706808")
    ...
    def run_blast(self):
        ...
        if blast_command.return_code == 0:
            ...
            self.set_error("blast运行出错!", code = "33706809")
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            "type": "tool",
            "name": "ref_rna_v2.geneset.ppinetwork_mapblast",
            "instant": True,
            "options": dict(
