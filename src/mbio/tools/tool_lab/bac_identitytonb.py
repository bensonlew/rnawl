...
class BacIdentitytonbAgent(Agent):
    def __init__(self, parent):
        super(BacIdentitytonbAgent, self).__init__(parent)
        options = [
            ...
            {"name": "picture_path","type":"infile","format":"denovo_rna_v2.common_dir"}
        ]
    ...
    def check_option(self):
        ...
        if not self.option("fasta_dir"):
            raise OptionError("没有找到fasta_dir")
        if not self.option("picture_path"):
            raise OptionError("没有找到picture_path")
        return True
    ...
    def end(self):
        super(BacIdentitytonbAgent, self).end()


class BacIdentitytonbTool(Tool):
    def __init__(self, config):
        super(BacIdentitytonbTool, self).__init__(config)
        ...
        self.Rscript = "program/miniconda3/envs/R_v4/bin/Rscript" #更新R包的安装地址
        ...
        self.set_environ(
            ...
            BLASTDB=self.config.SOFTWARE_DIR + '/database/align/ncbi/db/nt/nt_v20211009/')
        self.set_environ(
            CONSED_HOME=self.config.SOFTWARE_DIR + '/bioinfo/ysource/pairAss_bak/')
        self.script = self.config.PACKAGE_DIR + '/tool_lab/bac_identity/sangerSeq2_new.R'
        self.assemble_sh = 'bioinfo/tool_lab/assemble.sh.newnb'
        self.blastn = "bioinfo/align/ncbi-blast-2.12.0+/bin/blastn"
        # self.perl = 'program/perl-5.24.0/bin/perl'
        # 20211102 更新数据库，改用taxonkit搜索物种名
        self.python_script = self.config.PACKAGE_DIR + "/tool_lab/bac_identity/getNtTaxontonb.py"
        self.python = "miniconda2/bin/python"
        # self.nt = os.path.join(self.software_dir, "database/Tool_lab/bac_identity/NT_20211102/nt")
        self.nt = os.path.join(self.software_dir, "database/align/ncbi/db/nt/nt_v20211009/nt")
        ...
        # self.pictrue_dir = self.option("picture_path").prop["path"]
        self.taxonkit = self.config.SOFTWARE_DIR+'/bioinfo/tool_lab/taxonkit'
        self.datadir = self.config.SOFTWARE_DIR+'/bioinfo/tool_lab/data_dir/'
        self.pairAss_bak = self.config.SOFTWARE_DIR+'/bioinfo/ysource/pairAss_bak/'
    ...
    def run(self):
        ...
        运行
        """
        super(BacIdentitytonbTool, self).run()
    ...
    def run_sangerSeq2_R(self):
        ...
        install.packages("./phangorn_2.7.0.tar.gz") #需要去下2.7版本的phangorn
        #需要去下2.7版本的phangorn: wget https://cran.r-project.org/src/contrib/Archive/phangorn/phangorn_2.7.0.tar.gz
    ...
    def run_assemble(self):
        ...
        cmd1 = '{} {} {}'.format(self.assemble_sh, self.fasta_dir, self.pairAss_bak)
        command1 = self.add_command("assemble", cmd1).run()
        self.wait(command1)
        if command1.return_code == 0:
    ...
    def run_nt_blastn(self):
        ...
        cmd1 += " -i {} -o {} -t {} -d {}".format(out,out_tax,self.taxonkit,self.datadir)
    ...
    def set_output(self):
        ...
        try:
            ...
            cmd_picture_path = os.system('cd {} && cp *.jpg {}'.format(self.option("picture_path").prop["path"], ot_dir))
            self.wait()
            if cmd_picture_path == 0:
                self.logger.info("胶图文件存入结果文件夹成功")
            else:
                self.logger.info("胶图文件存入结果文件夹失败")
            ...
            os.link(self.option("sample_info").prop["path"], os.path.join(rstdir,"sample.detail.xls"))
