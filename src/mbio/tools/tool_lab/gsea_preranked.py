...
class GseaPrerankedTool(Tool):
    def __init__(self, config):
        ...
        self.script = {
            'id2id': os.path.join(self.config.PACKAGE_DIR, 'tool_lab/id2id.r'),
        }
        self.file = {
            'ensembl2symbol': os.path.join(self.work_dir, 'ensembl2symbol.txt'),
            'matrix2gsea': os.path.join(self.work_dir, '{}'.format('GSEA.txt')),
            'biomart_mus': os.path.join(self.config.SOFTWARE_DIR, 'database/Genome_DB_finish/vertebrates/Mus_musculus/GRCm38_Ensembl_96/biomart/biomart.txt'),
            'biomart_hsa': os.path.join(self.config.SOFTWARE_DIR, 'database/Genome_DB_finish/vertebrates/Homo_sapiens/GRCh38_Ensembl_96/biomart/biomart.txt'),
            'biomart_rno': os.path.join(self.config.SOFTWARE_DIR,'database/Genome_DB_finish/vertebrates/Rattus_norvegicus/Rnor_6.0_Ensembl_96/biomart/biomart.txt'),            # 'output': os.path.join(self)
            'json': os.path.join(self.work_dir, 'input.json'),
            # 'matrix': os.path.join(self.work_dir, 'matrix')
        }
    ...
    def pre_gsea(self):
        exp = pd.read_table(self.option('matrix').path, header=0, index_col=0)
        id_list = exp.index.tolist()
        # json.dump({
        #     'ensembl_id': id_list,
        #     'output': self.file['ensembl2symbol']
    ...
    def matrix2gsea(self):
        ...
        with open(self.file['ensembl2symbol'], 'r') as e2s:
        ...
        # ensemble_symbol_dict = dict(zip(symbol['gene_id'],[symbol['gene_name'],symbol['description']]))
        a = list()
        c = list()
    ...
    def biomart_dict(self):
        gene_id_name_description_dict = dict()
        if self.option('species') == 'Mus musculus':
            biomart = self.file['biomart_mus']
        if self.option('species') == 'Homo sapiens':
            biomart = self.file['biomart_hsa']
        if self.option('species') == 'Rattus norvegicu':
            biomart = self.file['biomart_rno']
        with open(biomart, 'r') as bio:
            for line in bio.readlines():
                gene_id = line.strip().split('\t')[0]
                trans_id = line.strip().split('\t')[1]
                gene_name = line.strip().split('\t')[2]
                description = line.strip().split('\t')[7]
                if description == '':
                    description = 'not description'
                if gene_id not in gene_id_name_description_dict:
                    gene_id_name_description_dict[gene_id] = [gene_name, description]
                else:
                    continue
        id_list = self.pre_gsea()
        with open(self.file['ensembl2symbol'], 'w') as e2s:
            e2s.write('gene_id' + '\t' + 'gene_name' + '\t' + 'description' + '\n')
            for i in id_list:
                if i in gene_id_name_description_dict:
                    e2s.write(i + '\t' + gene_id_name_description_dict[i][0].upper() + '\t' + gene_id_name_description_dict[i][1] + '\n')
                else:
                    continue
    ...
    def gene_detail(self):
        ...
        # exp_pd = pd.read_table(self.file['matrix2gsea'], header=0, index_col=0,sep='\t')
        # del exp_pd['description']
        # exp_pd = np.log(exp_pd + 1)
        gene_list = list(exp_pd.index)
        tmp_dic = {}
        with open(self.file['ensembl2symbol'], 'r') as handle:
            for dic in csv.DictReader(handle, delimiter='\t'):
    ...
    def gsea_analysis(self):
        ...
        os.link(self.option('rnk').path, os.path.join(self.work_dir, "genes.rnk"))
        # copyfile(self.option('rnk').path, os.path.join(self.work_dir, "genes.rnk"))
        cmd_list = [
    ...
    class TestFunction(unittest.TestCase):
        ...
        def test(self):
            ...
            data = {
                ...
                "type": "tool",
                "name": "tool_lab.gsea_preranked",
                "instant": False,
                "options": dict(
                    rnk='/mnt/ilustre/users/sanger-dev/workspace/20200618/GenesetGsea_tsg_37719_7924_3476/Gsea__1/tmp_output/my_analysis.Gsea.1592451316549/edb/GSEA.rnk',
                    gmx='/mnt/ilustre/users/sanger-dev/workspace/20200618/GenesetGsea_tsg_37719_7924_3476/Gsea__1/tmp_output/my_analysis.Gsea.1592451316549/edb/gene_sets.gmt',
                    # gene_detail='/mnt/ilustre/users/sanger-dev/sg-users/zhaozhipeng/temp/out/GSEA.chip',
                    matrix='/mnt/ilustre/users/sanger-dev/workspace/20200618/GenesetGsea_tsg_37719_7924_3476/gsea.txt',
