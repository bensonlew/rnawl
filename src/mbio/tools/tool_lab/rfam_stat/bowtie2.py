...
class Bowtie2Tool(Tool):
    ...
    def run_prepare_fastq(self):
        if self.option("seq_method") == "PE":
            left_reads_raw = self.option("left_reads").prop['path']
            left_reads_final = os.path.join(self.work_dir,"left.fastq")
            self.left_reads = self.extract_fastq(left_reads_raw,left_reads_final, n_lines =self.option("seq_num"))
            right_reads_raw = self.option("right_reads").prop['path']
            right_reads_final = os.path.join(self.work_dir, "right.fastq")
            self.right_reads = self.extract_fastq_r(right_reads_raw,right_reads_final, n_lines =self.option("seq_num"))
        else:
            single_reads_raw = self.option("single_end_reads").prop['path']
            single_reads_final =  os.path.join(self.work_dir,"single.fastq")
            self.single_reads = self.extract_fastq(single_reads_raw,single_reads_final, n_lines =self.option("seq_num"))
    ...
    def run_tophat(self, index_ref):
        ...
        if self.option("seq_method") == "PE" and self.option("strand_specific"):
            cmd = '{}/bowtie2 --fr -x {} -1 {} -2 {}  -p 10 -S {}.sam > {}.stat\n'.format(self.bowtie_path, self.ref_path, self.left_reads, self.right_reads, self.work_dir + '/' + name, self.work_dir + '/' + name)
        elif self.option("seq_method") == "PE" and not self.option("strand_specific"):
            cmd = '{}/bowtie2 -x {} -1 {} -2 {}  -p 10 -S {}.sam > {}.stat\n'.format(self.bowtie_path, self.ref_path, self.left_reads, self.right_reads, self.work_dir + '/' + name, self.work_dir + '/' + name)
        else:
            cmd = '{}/bowtie2 -x {} {}  -p 10 -S {}.sam > {}.stat\n'.format(self.bowtie_path, self.ref_path, self.single_reads, self.work_dir + '/' + name, self.work_dir + '/' + name)
...
class TestFunction(unittest.TestCase):
    def test(self):
        ...
        data = {
            "id": "Botie2Rfam" + str(random.randint(1, 10000)),
            "type": "tool",
            "name": "tool_lab.rfam_stat.bowtie2",
            "instant": False,
            "options": dict(
                ref_genome ="/mnt/ilustre/users/sanger-dev/app/database/Annotation/other2019/rfam14.1/Rfam_bowtie2_index/Rfam",
                left_reads="/mnt/ilustre/users/sanger-dev/workspace/20210207/Refrna_l46s_j2e96sbdlkp048auflfubl/remote_input/fastq_dir/data/9P.R1.fastq",
                right_reads = "/mnt/ilustre/users/sanger-dev/workspace/20210207/Refrna_l46s_j2e96sbdlkp048auflfubl/remote_input/fastq_dir/data/9P.R2.fastq",
                seq_method = "PE",
                sample ="test"
            )
        }
