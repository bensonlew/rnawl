...
class WgcnaModuleAgent(Agent):
    ...
    def check_options(self):
        with open(self.option('datExpr').prop['path'], 'r') as f:
            gene_num = len(f.readlines()) - 1
        if gene_num < self.option('minModuleSize'):
            raise OptionError("预处理基因数只有 {} 个, 无法分析请调整 预处理参数".format(gene_num))
        pass
...
class WgcnaModuleTool(Tool):
    ...
    def __init__(self, config):
        ...
        self.wgcna_module = self.config.PACKAGE_DIR + '/tool_lab/wgcna/wgcna_module.py'
    ...
    def run_wgcna_module(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("无法划分基因到模块， 建议降低 mergeCutHeight")
            ...
            if command.return_code is 0:
                ...
                self.set_error("%s Failed. >>>%s", variables = (cmd_name, cmd), code = "33708603")
        else:
            self.set_error("the only color is grey ，建议调整参数")
    ...
    def set_output(self):
        ...
        f4 = glob.glob(self.work_dir + '/eigengenes.txt')
        ...
        f8 = glob.glob(self.work_dir + '/../seq_id2gene_name.txt')
        f9 = glob.glob(self.work_dir + '/block_*_dendrogram.*png')
        f10 = glob.glob(self.work_dir + '/*Network-heatmap*')
        all_files = f1 + f2 + f3 + f4 + f5 + f6 + f7 + f8 + f9 + f10
