...
class WgcnaPrepareTool(Tool):
    ...
    def __init__(self, config):
        ...
        self.wgcna_prepare = self.config.PACKAGE_DIR + '/tool_lab/wgcna/wgcna_prepare.py'
    ...
    def run_wgcna_prepare(self):
        ...
        if command.return_code == 0:
            ...
            with open(os.path.join(self.work_dir, 'wgcna_prepare.o'), 'r') as f:
                f.readline()
                info = f.readline().strip()
            if info == 'pandas failed':
                self.set_error('输入表达谱文件解析失败，请检查。')
            else:
                self.set_error("所有的基因都被过滤了， 请调整预处理参数")
            ...
            self.set_error("预处理基因数过少， 请调整预处理参数")
    ...
    def set_output(self):
        ...
        f6 = glob.glob(self.work_dir + '/pick_power.pdf')
        f7 = glob.glob(self.work_dir + '/sampleClustering.pdf')
        all_files = f1 + f3 + f4 + f5 + f6 + f7
