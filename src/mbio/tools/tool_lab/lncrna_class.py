...
class LncrnaClassAgent(Agent):
    """
    lnc rna target prediction
    """

    def __init__(self, parent):
        super(LncrnaClassAgent, self).__init__(parent)
        options = [
...
class LncrnaClassTool(Tool):
    ...
    def __init__(self, config):
        super(LncrnaClassTool, self).__init__(config)
    ...
    def gtf2bed(self):
        ...
        data = list()
        if self.option('lncrna_gtf').is_set:
            data.append((self.option('lncrna_gtf').prop['path'], lncrna_bed, 'lncrna_gtf2bed'))
        else:
            lncrna_bed = self.option('lncrna_bed').prop['path']
        if self.option('mrna_gtf').is_set:
            data.append((self.option('mrna_gtf').prop['path'], mrna_bed, 'mrna_gtf2bed'))
        else:
    ...
    def run(self):
        super(LncrnaClassTool, self).run()
