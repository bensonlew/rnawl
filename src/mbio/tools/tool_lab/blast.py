...
class BlastAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "reference", "type": "string", "default": None},
            # 参考序列  选择customer时启用
            {"name": "reference_type", "type": "string"},
            # 参考序列(库)的类型  为nucl或者prot
            ...
            # 输出格式为5时输出
            {"name": "outtable", "type": "outfile", "format": "ref_rna_v2.blast_table"},
            # 输出格式为6时输出
    ...
    def check_options(self):
        self.logger.info("option input is {}".format({o: str(self.option(o)) for o in self._options}))
        if not self.option("query").is_set:
            raise OptionError("必须设置参数query", code = "33700101")
        if self.option('query_type') not in ['nucl', 'prot']:
            raise OptionError('query_type查询序列的类型为nucl(核酸)或者prot(蛋白):%s', variables = (self.option('query_type')), code = "33700102")
        else:
            if self._fasta_type[self.option('query').prop['seq_type']] != self.option('query_type'):
                raise OptionError(
                    '文件检查发现查询序列为:%s, 而说明的文件类型为:%s', variables = (
                        self._fasta_type[self.option('query').prop['seq_type'], self.option('query_type')]), code = "33700103")
        if self.option("database") == 'customer_mode':
            if not self.option("reference"):
                raise OptionError("使用自定义数据库模式时必须设置reference", code = "33700104")
            if self.option('reference_type') not in ['nucl', 'prot']:
                raise OptionError('reference_type参考序列的类型为nucl(核酸)或者prot(蛋白):%s', variables = (self.option('query_type')), code = "33700105")
            else:
                pass

        elif self.option("database").lower() not in ["nt", "nr", "string", 'kegg', 'swissprot', 'archaea', 'viruses', 'fungi', 'viridiplantae', 'eukaryota_other', 'eukaryota', 'craniata', 'bacteria', 'eggnog']:
            raise OptionError("数据库%s不被支持",  variables = (self.option("database")), code = "33700107")

        if not 15 > self.option('outfmt') > -1:
            raise OptionError('outfmt遵循blast+输出规则，必须为0-14之间：%s',  variables = (self.option('outfmt')), code = "33700108")
        if not 10 >= self.option('evalue') >= 0:
            raise OptionError('E-value值设定必须为[0-1)之间：%s',  variables = (self.option('evalue')), code = "33700109")
        if not 0 < self.option('num_alignment') < 1001:
            raise OptionError('序列比对保留数必须设置在1-1000之间:%s',  variables = (self.option('num_alignment')), code = "33700110")

        if self.option('blast') not in self._blast_type[self.option('query_type')][self.option('reference_type')]:
            print self.option('blast')
            print self.option('query_type')
            print self.option('reference_type')
            print self._blast_type[self.option('query_type')][self.option('reference_type')]

            raise OptionError(
                '程序不试用于提供的查询序列和库的类型，请仔细检查，核酸比对核酸库只能使用blastn或者tblastx，\
                 核酸比对蛋白库只能使用blastx， 蛋白比对蛋白库只能使用blastp, 蛋白比对核算库只能使用tblastn')
        return True
...
class BlastTool(Tool):
    ...
    def run_makedb_and_blast(self):
        ...
        :return:
        """
        db_name = os.path.splitext(self.option("reference"))[0].strip()
        is_make_db = False

        if self.option("reference_type") == "nucl":
            if os.path.exists(db_name + ".nsq") or os.path.exists(db_name + ".00.nsq"):
                is_make_db = True

        elif  self.option("reference_type") == "prot":
            if os.path.exists(db_name + ".psq") or os.path.exists(db_name + ".00.psq"):
                is_make_db = True
        ...
        if not is_make_db:
            ...
            if makedb_obj.return_code == 0:
                ...
                self.set_error("makeblastdb运行出错!", code = "33700112")
    ...
    def run_blast(self, db_name):
        ...
        outputfile = self.work_dir + "/blast_result"
        ...
        if blast_command.return_code == 0:
            ...
            if os.path.exists(self.output_dir + '/' + os.path.basename(outputfile[:-3] + 'xls')):
                os.remove(self.output_dir + '/' + os.path.basename(outputfile[:-3] + 'xls'))
            os.link(outputfile[:-3] + 'xls', self.output_dir + '/' + os.path.basename(outputfile[:-3] + 'xls'))
            self.end()
        else:
            self.set_error("blast运行出错!", code = "33700113")
