...
class DnafungiPredictTidyAgent(Agent):
    """
    ...
    def check_options(self):
        if not self.option("gene_gff").is_set:
            raise OptionError("必须设置参数gene_gff", code="32101601")
        if not self.option("trna_gff").is_set:
            raise OptionError("必须设置参数trna_gff", code="32101602")
        if not self.option("rrna_gff").is_set:
            raise OptionError("必须设置参数rrna_gff", code="32101603")
        if not self.option("input_genome").is_set:
            raise OptionError("必须设置参数input_genome", code="32101604")
...
class PredictTidyTool(Tool):
    ...
    def run_get_fa_bygff(self, genome, gff):
        ...
        command = self.add_command(name.lower(), cmd).run()
        self.wait(command)
        if command.return_code == 0:
            self.logger.info("提取fnn和整理GFF文件运行完成")
        else:
            self.set_error("提取fnn和整理GFF文件运行出错!", code="32101601")
    ...
    def set_output(self):
        os.link(self.work_dir + "/predict.gff", self.output_dir + "/" + self.sample_name + "_CDS.gff")
