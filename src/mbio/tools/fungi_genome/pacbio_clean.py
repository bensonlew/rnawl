...
class PacbioCleanAgent(Agent):
    ...
    def check_options(self):
        if not self.option("input_fq").is_set:
            raise OptionError("必须添加样品的input_fq的三代fq文件！", code="32101401")
...
class PacbioCleanTool(Tool):
    ...
    def run_len_graphic(self):
        cmd = '{} {}pacbio_length.pl {} 100 1000 pacbio {}'.format(self.perl_path,self.perl_script,self.fq,self.option("sample_name"))
