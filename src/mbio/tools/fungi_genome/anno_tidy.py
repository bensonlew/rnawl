...
class AnnoTidyTool(Tool):
    ...
    def run_get_nr_des(self):
        ...
        if self.option("anno_go").is_set:
            ...
            go = go.ix[:,["GO ID (Lev2)", "Seq List"]]
            go.columns = ["GO", "Seq List"]
            ...
            go_anno = pd.DataFrame(columns = ["Seq_id", "GO"])
            ...
            if os.path.exists(self.output_dir + "/GO/" + self.option("sample") + "_go_level2_statistics.xls"):
                os.remove(self.output_dir + "/GO/" + self.option("sample") + "_go_level2_statistics.xls")
            os.link(self.option("anno_go").prop["path"], self.output_dir + "/GO/" + self.option("sample") + "_go_level2_statistics.xls")
    ...
    def run_summary(self):
        ...
        gene_li = sorted(list(set(ann_gene['Gene ID'])))
        pfam_id = {}
        for ge in gene_li:
            pfam_g = ";".join(pfam[pfam["Gene ID"] == ge]["Pfam_id"])
            pfam_id[ge] = pfam_g
        ann_gene["Pfam_id"] = ann_gene["Gene ID"].map(pfam_id)
        ann_gene.index = ann_gene["Gene ID"]
        Pfam = ann_gene.ix[:, ["Gene ID", "Pfam_id"]]
        Pfam.columns = ["Gene ID", "Pfam_id"]
        cog = pd.read_table(self.output_dir + "/COG/" + self.option("sample") + "_cog_anno.xls", sep='\t', header=0)
        cog = cog.ix[:, ["Gene ID", "COG ID", "COG Type"]]
        kegg = pd.read_table(self.output_dir + "/KEGG/" + self.option("sample") + "_kegg_anno.xls", sep='\t', header=0)
        kegg = kegg.ix[:, ["Gene ID", "Gene", "KO"]]
        kegg.columns = ["Gene ID", "Gene Name", "KO ID"]
        go = pd.read_table(self.output_dir + "/GO/" + self.option("sample") + "_go_anno.xls", sep='\t', header=0)
        go = go.ix[:, ["Gene ID", "GO"]]
        go.columns = ["Gene ID", "GO ID"]
    ...
    def file_split(self,anno,type):
        ...
        if re.match("^[Ss]caffold[0-9]+$", list(f1["Location"])[1]):
            ...
            for loc in list(set(f1['Location'])):
                ...
                location = re.subn("^p", "plasmid", re.subn("^Chr", "chromosome", loc)[0])[0]
