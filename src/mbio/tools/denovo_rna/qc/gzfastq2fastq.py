...
class Gzfastq2fastqTool(Tool):
    ...
    def gzfastq2fastq(self):
        ...
        with open(self.option("fastq_path") + '/list.txt', 'w') as list_w:
            list_info = list_info.replace(u'.gz', '')
        ...
        if command.return_code == 0:
            ...
            if command.return_code is 0:
                ...
                self.set_error("%s Failed. >>>%s", variables = (cmd_name, cmd), code = "33709001")
        else:
            self.set_error("%s Failed. >>>%s", variables = (cmd_name, cmd), code = "33709002")
