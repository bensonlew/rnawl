...
class StatGenomeTool(Tool):
    ...
    def run_n50_stat(self):
        cmd = '{} '.format(self.trinity_stat)
        cmd += '{} '.format(self.option("genome").prop['path'])
        cmd_name = 'genomestat'
    ...
    def set_output(self):
        ...
        os.link(os.path.join(self.work_dir, "Trinity.stat.txt"), os.path.join(self.output_dir, "Trinity.stat.txt"))
...
class TestFunction(unittest.TestCase):
    ...
    def test_ncbi(self):
        ...
        data = {
            'id': 'stat_genome_{}_{}'.format(random.randint(1000, 10000), random.randint(1000, 10000)),
            'type': 'tool',
            'name': 'ref_genome_db_v2.stat_genome',
            'instant': False,
            'options': {
