...
class NcbiTaxonAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {"name": "blastout", "type": "infile", "format": "ref_rna_v2.blast_table,ref_rna_v2.blast_xml"},  # 输入文件
            {"name": "taxon_out", "type": "outfile", "format": "ref_rna_v2.nr_taxon"},  # 输出结果文件
    ...
    def check_options(self):
        if not self.option("blastout").is_set:
            raise OptionError("必须设置输入文件", code = "33702101")
        if self.option('blastdb') == 'None':
            raise OptionError("必须设置输入文件的blast比对类型", code = "33702102")
        else:
            if self.option('blastdb') not in ['nr', 'nt']:
                raise OptionError('blastdb必须为nr或者nt:%s', variables = (self.option('blastdb')), code = "33702103")
        return True
...
class NcbiTaxonTool(Tool):
    ...
    def run(self):
        ...
        # if run_gitaxon.result:
            ...
            self.set_error('注释查询出错！', code = "33702104")
        ...
        for i in openfp:
            ...
            if line_sp[5] in query_gi:
                ...
                if len(hitname) < 3 or hitname[0] != 'gi' or (not re.match(r'^\d+$', hitname[1])):
                    self.set_error('输入文件中不是nr库比对结果(20160622)不含有gi信息:%s', variables = (line_sp[10]), code = "33702105")
