...
class CatBlastoutAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {"name": "blastout", "type": "infile", "format": "ref_rna_v2.blast_xml_dir"},
            {"name": "blast_result", "type": "outfile", "format": "ref_rna_v2.blast_xml"},
        ]
    ...
    def check_options(self):
        ...
        if not self.option("blastout").is_set:
            raise OptionError("请传入blast输出结果文件", code = "33700501")
...
class CatBlastoutTool(Tool):
    ...
    def cat_files(self):
        """
        """
        if self.option('blastout').format == 'ref_rna_v2.blast_xml_dir':
