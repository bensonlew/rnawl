...
class RefAnnoQueryAgent(Agent):
    ...
    def check_options(self):
        if not self.option("gos_list"):
            raise OptionError('缺少输入文件:gos_list', code = "33702501")
        if not self.option("kegg_table"):
            raise OptionError('缺少输入文件:kegg_table', code = "33702502")
...
class RefAnnoQueryTool(Tool):
    def __init__(self, config):
        ...
        self.query_path = self.config.PACKAGE_DIR + "/ref_rna_v2/new_annotation_query.py"
    ...
    def run_query(self):
        ...
        try:
            ...
            self.set_error("运行注释查询脚本失败", code = "33702503")
        # self.set_output()
        self.end()
