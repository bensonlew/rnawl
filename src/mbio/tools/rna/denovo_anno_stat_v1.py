...
class DenovoAnnoStatV1Agent(Agent):
    """
    统计无参转录组注释模块NR、GO、COG、KEGG、SwissProt的相关信息
    """
    def __init__(self, parent):
        super(DenovoAnnoStatV1Agent, self).__init__(parent)
        options = [
            ...
            {"name": "database", "type": "string", "default": "nr,go,cog,kegg,swissprot"},
    ...
    def check_options(self):
        ...
        for i in self.anno_database:
            if i not in ['nr', 'go', 'cog', 'kegg', 'swissprot']:
                raise OptionError('需要注释的数据库不在支持范围内[nr, go, cog, kegg, swissprot]:{}'.format(i))
    ...
    def end(self):
        ...
        relpath = [
            ...
            ["/blast/gene_swissprot.xls", "xls", "基因序列blast比对swissprot注释结果table"],
            ["/blast/gene_string.xml", "xml", "基因序列blast比对string注释结果xml"],
            ["/blast/gene_kegg.xml", "xml", "基因序列blast比对kegg注释结果xml"],
            ["/blast/gene_string.xml", "xml", "基因序列blast比对string注释结果xml"],
            ["/blast/gene_swissprot.xml", "xml", "基因序列blast比对swissprot注释结果xml"],
            ...
            ["/blast_nr_statistics/gene_nr_evalue.xls", "xls", "基因序列blast到nr结果E-value统计"],
            ["/blast_nr_statistics/gene_nr_similar.xls", "xls", "基因序列blast到nr结果similarity统计"],
            ["/blast_swissprot_statistics/gene_swissprot_evalue.xls", "xls", "基因序列blast到swissprot结果E-value统计"],
            ["/blast_swissprot_statistics/gene_swissprot_similar.xls", "xls", "基因序列blast到swissprot结果similarity统计"],
        ...
        super(DenovoAnnoStatV1Agent, self).end()


class DenovoAnnoStatV1Tool(Tool):
    """
    表达量差异检测tool
    """
    def __init__(self, config):
        super(DenovoAnnoStatV1Tool, self).__init__(config)
        self._version = '1.0.1'
        ...
        dir_list = ['/blast/', '/ncbi_taxonomy/', '/cog_stat/', '/go_stat/', '/kegg_stat/', '/blast_nr_statistics/', '/blast_swissprot_statistics/']
        for i in dir_list:
    ...
    def run_nr_stat(self):
        ...
        try:
            ...
            self.logger.info("End: stat nr taxon")
        except Exception as e:
            self.set_error("运行nr物种分类出错:{}".format(e))
            self.logger.info("Error: stat nr taxon")
    ...
    def run_cog_stat(self):
        ...
        # cog stat
        # cog = cog_stat()
        # self.logger.info(self.option('cog_list').prop['path'])
        # try:
        ...
        # except Exception as e:
        #     self.set_error("运行cog stat evalue出错:{}".format(e))
    ...
    def run(self):
        super(DenovoAnnoStatV1Tool, self).run()
        for db in self.database:
            if db == 'cog':
                self.run_cog_stat()
            if db == 'nr':
