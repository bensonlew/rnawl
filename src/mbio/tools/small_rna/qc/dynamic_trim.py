...
class DynamicTrimAgent(Agent):
    def __init__(self, parent=None):
        ...
        options = [
            ...
            {"name": "quality", "type": "string", "default": "sanger"},  # fastq质量体系
            {"name": "list_file", "type": "infile", "format": "sequence.file_sample"},  # fastq文件夹对应的list.txt文件
            {"name": "out_fastq", "type": "outfile", "format": "sequence.fastq"}  # 输出结果
        ]
...
class DynamicTrimTool(Tool):
    def __init__(self, config):
        ...
        self.dynamic_trim = self.config.SOFTWARE_DIR + "/datasplit/bin/DynamicTrim.pl"

    def run_single_dynamic_trim(self):
        """DynamicTrim.pl, 去低值"""
        tmp_file = glob.glob(self.work_dir + "/*.trimmed")
