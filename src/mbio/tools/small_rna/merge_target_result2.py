...
class MergeTargetResult2Agent(Agent):
    """
    fasta files remove duplication
    """

    def __init__(self, parent):
        super(MergeTargetResult2Agent, self).__init__(parent)
        options = [
    ...
    def check_options(self):
        ...
        if not self.option('anno_detail').is_set:
            raise OptionError("不能没有注释文件")
    ...
    def end(self):
        ...
        super(MergeTargetResult2Agent, self).end()


class MergeTargetResult2Tool(Tool):
    """
    fasta files remove duplication
    """

    def __init__(self, config):
        super(MergeTargetResult2Tool, self).__init__(config)
    ...
    def get_targetscan(self):
        ...
        with open(self.option('targetscan'), 'r') as f_r:
            ...
            for line in f_r.readlines():
                line = line.strip("\n").split('\t')
                if len(line) > 4:
                    couple2info['%s|%s' % (line[0], line[1])] = '\t'.join(
    ...
    def get_anno_info(self):
        ...
        with open(self.option('anno_detail').path, 'r') as f_r:
            annot_type = "ref"
            header = f_r.readline()
            ...
            for line in f_r.readlines():
                ...
                if len(line) > 5:
                    ...
                    g2gn[line[0]].add(line[2].lower())
        return anno_info, g2gn

    def run(self):
        super(MergeTargetResult2Tool, self).run()
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "MergeTargetResult2" + datetime.datetime.now().strftime('%H-%M-%S'),
            "type": "tool",
