...
class SrnaStatAgent(Agent):
    ...
    def check_options(self):
        ...
        if not os.path.exists(self.option("known_mirna_count")):
            raise OptionError("known_mirna_count文件不存在")
...
class SrnaStatTool(Tool):
    ...
    def run_srna_stat(self):
        ...
        with open(self.option("list"), "r") as f:
            for line in f:
                if line.strip().split("\t")[1] not in self.samples:
                    self.samples.append(line.strip().split("\t")[1])
        ...
        for sample in self.samples:
            ...
            if os.path.exists(sample_stat):
                ...
                if sample not in dict:
                    ...
                    dict[sample]["total"] = 0
            print sample + "\t" + dict[sample]["total"] + "\n"
        '''
        ...
        with open(os.path.join(self.output_dir, "ncrna_stat.xls"), "w") as w1:
            ...
            #         dict[sample]["total"] = sum(map(int,dict[sample].values()))
            #     else:
            #         dict[sample] = {}
            #         dict[sample]["total"] = 0
            for sample in self.samples[0:-1]:
                if sample in dict:
                    if "rRNA" in dict[sample]:
            ...
            if self.samples[-1] in dict:
                if "rRNA" in dict[self.samples[-1]]:
            ...
            for sample in self.samples[0:-1]:
                if sample in dict:
                    ...
                    w1.write("0(0.0%)" + "\t")
            if self.samples[-1] in dict:
                if "tRNA" in dict[self.samples[-1]]:
            ...
            for sample in self.samples[0:-1]:
                if sample in dict:
                    ...
                    w1.write("0(0.0%)" + "\t")
            if self.samples[-1] in dict:
                if "snoRNA" in dict[self.samples[-1]]:
            ...
            for sample in self.samples[0:-1]:
                if sample in dict:
                    ...
                    w1.write("0(0.0%)" + "\t")
            if self.samples[-1] in dict:
                if "snRNA" in dict[self.samples[-1]]:
            ...
            for sample in self.samples[0:-1]:
                if sample in dict:
                    ...
                    w1.write("0(0.0%)" + "\t")
            if self.samples[-1] in dict:
                if "repbase" in dict[self.samples[-1]]:
            ...
            for sample in self.samples[0:-1]:
                if sample in dict:
                    ...
                    w1.write("0(0.0%)" + "\t")
            if self.samples[-1] in dict:
                if "exon" in dict[self.samples[-1]]:
            ...
            for sample in self.samples[0:-1]:
                if sample in dict:
                    ...
                    w1.write("0(0.0%)" + "\t")
            if self.samples[-1] in dict:
                if "intron" in dict[self.samples[-1]]:
        ...
        with open(os.path.join(self.output_dir, "srna_stat.xls"), "w") as w1:
            ...
            for sample in self.samples[0:-1]:
                if sample in dict:
                    ...
                    w1.write("0(0.0%)" + "\t")
            if self.samples[-1] in dict:
                if "known_mirna" in dict[self.samples[-1]]:
            ...
            for sample in self.samples[0:-1]:
                if sample in dict:
                    ...
                    w1.write("0(0.0%)" + "\t")
            if self.samples[-1] in dict:
                if "novel_mirna" in dict[self.samples[-1]]:
    ...
    def test(self):
        ...
        data = {
            ...
            "options": dict(
                ...
                novel_mirna_count="/mnt/ilustre/users/sanger-dev/workspace/20181107/ath_srna18-09-12/Srna/output/novel_mirna/novel_mirna_count.xls",
                rfam_summary="/mnt/ilustre/users/sanger-dev/workspace/20181107/ath_srna18-09-12/Srna/output/rfam_stat/rfam_summary.xls",
                repeat_summary="/mnt/ilustre/users/sanger-dev/workspace/20181107/ath_srna18-09-12/Srna/output/repeat_stat/repeat_summary.xls",
                exon_summary="/mnt/ilustre/users/sanger-dev/workspace/20181107/ath_srna18-09-12/Srna/output/exon_stat/exon_summary.xls",
                intron_summary="/mnt/ilustre/users/sanger-dev/workspace/20181107/ath_srna18-09-12/Srna/output/intron_stat/intron_summary.xls",
                filter_fa="/mnt/ilustre/users/sanger-dev/workspace/20181107/ath_srna18-09-12/Srna/output/intron_stat/filtered.fa",
                config="/mnt/ilustre/users/sanger-dev/sg-users/shicaiping/miRNA/test_rawdata/Arabidopsis_thaliana_24samples/infile/Uniq.cfg.ini",
            )
        }
