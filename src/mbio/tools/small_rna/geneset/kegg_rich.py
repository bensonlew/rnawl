...
class KeggRichAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "diff_list", "type": "infile", "format": "small_rna.gene_list"},
            # gene名字文件
...
class KeggRichTool(Tool):
    def __init__(self, config):
        ...
        if self.option('kegg_version') in ["201909", "202003"]:
            self.k2e = os.path.join(self.config.SOFTWARE_DIR, 'database/Annotation/other{}/kegg{}/K2enzyme.tab'.format(self.option('kegg_version')[0:4],
                                                                                                          self.option('kegg_version')))
            self.brite = os.path.join(self.config.SOFTWARE_DIR, 'database/Annotation/other{}/kegg{}/br08901.keg'.format(self.option('kegg_version')[0:4],
                                                                                                            self.option('kegg_version')))
