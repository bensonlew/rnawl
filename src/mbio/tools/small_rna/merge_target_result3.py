...
import os
import unittest
__author__ = 'fengyitong, liubinxu'



class MergeTargetResult3Agent(Agent):
    """
    fasta files remove duplication
    v3  添加circRNA映射host基因的功能
    """
    def __init__(self, parent):
        super(MergeTargetResult3Agent, self).__init__(parent)
        options = [
    ...
    def end(self):
        ...
        super(MergeTargetResult3Agent, self).end()

class MergeTargetResult3Tool(Tool):
    """
    fasta files remove duplication
    """
    def __init__(self, config):
        super(MergeTargetResult3Tool, self).__init__(config)
        self.result_summary = defaultdict(dict)
        self.mirtarbase = self.config.SOFTWARE_DIR + '/database/miRTarBase/miRTarBase_MTI.txt'
        self.encori = self.config.SOFTWARE_DIR + '/database/ENCORI/miRNA-Target'
    ...
    def get_targetscan(self):
        ...
        with open(self.option('targetscan'), 'r') as f_r:
            ...
            for line in f_r.readlines():
                line = line.strip("\n").split('\t')
                if len(line) > 4:
                    couple2info['%s|%s' % (line[0], line[1])] = '\t'.join([line[2], line[3], line[4], line[5], line[6], line[7], line[8]])
        return couple2info
    ...
    def get_rnahybrid(self):
        ...
        with open(self.option('rnahybrid'), 'r') as f_r:
            ...
            for line in f_r.readlines():
                line = line.strip().split('\t')
                if len(line) > 4:
                    couple2info['%s|%s'%(line[1], line[0])] = '\t'.join([line[2], line[3], line[4], line[5]])
            return couple2info
    ...
    def get_psRobot(self):
        ...
        with open(self.option('psrobot'), 'r') as f_r:
            ...
            for line in f_r.readlines():
                line = line.strip().split('\t')
                if len(line) > 6:
                    couple2info['%s|%s'%(line[0], line[2])] = '\t'.join([line[5], line[6], line[1]])
            return couple2info
    ...
    def get_encori_set(self):
        mir_set = set()
        encori_files = glob.glob(self.encori + "/*_all.txt")
        for f in encori_files:
            with open(self.mirtarbase, 'r') as base_r:
                for line in base_r.readlines()[4:]:
                    line = line.strip().split('\t')
                    if len(line) > 4:
                        mir_set.add('%s|%s'%(line[1], line[3]))
        return mir_set
    ...
    def get_anno_info(self):
        ...
        with open(self.option('anno_detail'), 'r') as f_r:
            ...
            for line in f_r.readlines():
                line = line.strip("\n").split('\t')
                if annot_type == "lnc":
                    anno_info[line[0]] = '\t'.join([line[1], line[2], line[3]])
                    g2gn[line[0]].add("")
                else:
                    anno_info[line[1]] = '\t'.join([line[0], line[3], line[5]])
                    g2gn[line[1]].add(line[3].lower())
                    anno_gene_info[line[0]] = [line[1], line[3], line[5]]
        return anno_info, g2gn, anno_gene_info

    def get_circ2gene(self):
        c2g = dict()
        with open(self.option('circ_detail'), 'r') as f_r:
            header = f_r.readline()
            for line in f_r:
                cols = line.strip("\n").split("\t")
                c2g[cols[0]] = cols[1]
    ...
    def run(self):
        super(MergeTargetResult3Tool, self).run()
        mir_set = self.get_mirtarbase_set()
        encori_set = self.get_encori_set()
        anno_info, g2gn , anno_gene_info = self.get_anno_info()
        if self.option('circ_detail'):
            c2g = self.get_circ2gene()
        header = 'query\ttarget\tgene_id\tgene_name\tgene_description\t'
        if self.option('miranda'):
            self.result_summary['miranda'] = self.get_miranda()
            header += 'start_miranda\tend_miranda\tscore_miranda\tenergy_miranda\t'
        if self.option('targetscan'):
            self.result_summary['targetscan'] = self.get_targetscan()
            header += 'utr_start_targetscan\tutr_end_targetscan\tmsa_start_targetscan\tmsa_end_targetscan\tseed_match_targetscan\tpct\tcontext_score\t'
        if self.option('psrobot'):
            self.result_summary['psrobot'] = self.get_psRobot()
            header += 'start_psrobot\tend_psrobot\tscore_psrobot\t'
        ...
        if self.option('rnahybrid'):
            ...
            header += 'start_rnahybrid\tend_rnahybrid\tenergy_rnahybrid\tpvalue_rnahybrid\t'
        header = header + 'mirtarbase\tENCORI\n'
        all_matched = set()
        ...
        with open(os.path.join(self.output_dir, 'all_merged.xls'), 'w') as all_w:
            ...
            for match in all_matched:
                ...
                elif self.option('circ_detail') :
                    if match.split('|')[1] in c2g and c2g[match.split('|')[1]] != "None":
                        line += '{}\t{}\t{}\t'.format(c2g[match.split('|')[1]],
                                                      ",".join([anno_gene_info[g][1] for g in c2g[match.split('|')[1]].split(",") if g in anno_gene_info ]),
                                                      ",".join([anno_gene_info[g][2] for g in c2g[match.split('|')[1]].split(",") if g in anno_gene_info ])
                        )
                    else:
                        line += '_\t_\t_\t'
                else:
                ...
                if match.split('|')[1] in g2gn:
                    for gn in g2gn[match.split('|')[1]]:
                        judge = False
                        if '%s|%s'%(match.split('|')[0], gn) in encori_set:
                            print('%s|%s'%(match.split('|')[0], gn))
                            line += 'yes\n'
                            judge = True
                            break
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "MergeTargetResult3" + datetime.datetime.now().strftime('%H-%M-%S'),
            "type": "tool",
            "name": "small_rna.merge_target_result2",
            "instant": False,
            "options": dict(
