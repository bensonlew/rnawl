...
class AnnotPrepareTool(Tool):
    ...
    def export_all_list(self):
        ...
        exp_df = pd.read_table(self.option("transcript_exp_file"),sep="\t",index_col=0)
        all_transcripts = exp_df.index.tolist()
        with open(all_list, "wb") as f:
            for gene_id in all_transcripts:
                f.write(gene_id + "\n")
        return all_list
    ...
    def export_go_list(self):
        ...
        raw_go_list_path = os.path.join(self.option("annot_result"),"allannot_class","go","go_list_tran.xls")
    ...
    def export_kegg_table(self):
        ...
        raw_kegg_path = os.path.join(self.option("annot_result"), "allannot_class", "kegg", "kegg_gene_tran.xls")
    ...
    def export_kegg_level_table(self):
        ...
        raw_kegg_level_path = os.path.join(self.option("annot_result"), "allannot_class", "kegg", "kegg_pathway_tran.xls")
        raw_kegg_level_df = pd.read_table(raw_kegg_level_path)
        raw_kegg_level_df["graph_id"] = ""
        raw_kegg_level_df["anno_type"] = "T"
