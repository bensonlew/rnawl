...
class DiffGenesetPrepareTool(Tool):
    ...
    def get_annotation_cog_detail_all(self,ref_path, new_path, anno_type, seq_type):
        first = [
    ...
    def prepare_cog_class(self):
        ...
        ref_cog_path = os.path.join(self.option("annot_result"), "refannot_class", "cog", "summary.T.tsv")
        new_cog_path = os.path.join(self.option("annot_result"), "newannot_class", "cog", "summary.T.tsv")
        cog_df = self.get_annotation_cog_detail_all(ref_cog_path,new_cog_path,"all","T")
        new_table_title = []
            ...
            for cr in cog_infos:
                ...
                if len(write_line) > 0:
                    ...
                    for geneset_name in  genesets:
                        line = "\t".join(write_line[geneset_name]) + "\t" if geneset_name in write_line else "0\tnone\t"
                        all_line += line
    ...
    def prepare_go_class(self):
        ...
        genesets = self.get_geneset_detail()
        geneset_name = self.option("geneset_name")
        go_level2_path = os.path.join(self.option("annot_result"), "allannot_class", "go", "go_lev2_tran.stat.xls")
        new_table_title = []
