...
class AddStatAgent(Agent):
    ...
    def __init__(self, parent):
        super(AddStatAgent, self).__init__(parent)
        options = [
            {"name": "contig_dir", "type": "string", "default": "false"},  # 输入文件，对组装后的序列路径
            {"name": "choose_kmer", "type": "string", "default": "false"},  # 是否进行kmer筛选，默认不筛选
            #{"name": "min_contig", "type": "string"},  #是否只统计序列长度高于此值的结果
...
class AddStatTool(Tool):
    ...
    def run(self):
        ...
        :return:
        """
        super(AddStatTool, self).run()
        self.logger.info("start run")
        self.logger.info("this tool is instant: %s" % self.instant)
        self.add_state("memory_limit", "test memory_limit can rerun tool?")
    ...
    def get_assemble_stat(self):
        ...
        if self.option('choose_kmer') == "false":
            ...
            cmd = self.Python_path + self.stat_path + ' -contig_dir %s -final_stat %s -min_contig 300'\
                                                  % (self.option('contig_dir').prop['path'], final_stat)
        else:
