...
class UnicyclerAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "min_len", "type": "int", "default": 100},  # 拼接最小长度100bp
...
class UnicyclerTool(Tool):
    def __init__(self, config):
        ...
        self.path = self.config.SOFTWARE_DIR + '/bioinfo/align/bowtie2-2.3.4.3-linux-x86_64:' + self.config.SOFTWARE_DIR + '/program/Python35/bin:' + self.config.SOFTWARE_DIR + '/gcc/7.2.0/bin:' + self.config.SOFTWARE_DIR + '/bioinfo/Genomic/Sofware/racon/build/bin:' + self.config.SOFTWARE_DIR + '/bioinfo/align/bcftools-1.6:' + self.config.SOFTWARE_DIR + '/bioinfo/metaGenomic/SPAdes-3.11.0-Linux/bin:' + self.config.SOFTWARE_DIR + '/bioinfo/align/ncbi-blast-2.3.0+/bin:'  + self.config.SOFTWARE_DIR + '/bioinfo/Genomic/Sofware/Pilon:' + self.config.SOFTWARE_DIR + '/bioinfo/align/samtools-1.3.1'
    ...
    def set_output(self):
        ...
        os.link(self.work_dir + "/result/assembly.fasta", self.output_dir + "/" + self.option("sample_name") + ".scaffold.fna")
