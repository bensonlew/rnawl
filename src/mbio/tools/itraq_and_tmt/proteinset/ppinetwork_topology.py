...
class PpinetworkTopologyAgent(Agent):
    ...
    def check_options(self):
        ...
        if not self.option('ppitable'):
            raise OptionError('必须提供PPI网络表', code = "32504001")
        if self.option('combine_score') > 1000 or self.option('combine_score') < 0:
            raise OptionError("combine_score值超出范围", code = "32504002")
        if not os.path.exists(self.option('ppitable')):
            self.logger.info('PPI网络表路径错误')

            time.sleep(3)
            return True #为了让他不报错，能包到流程里面
            # super(PpinetworkTopologyAgent, self).end()
            # raise OptionError('PPI网络表路径错误', code = "32504003")
        ppi_list = open(self.option('ppitable'), "r").readline().strip().split("\t")
        if "combined_score" not in ppi_list:
            raise OptionError("PPI网络表缺少结合分数", code = "32504004")
        if ("from" not in ppi_list) or ("to" not in ppi_list):
            raise OptionError("PPI网络缺少相互作用蛋白信息", code = "32504005")
        return True
...
class PpinetworkTopologyTool(Tool):
    def __init__(self, config):
        ...
        self._version = "1.0.1"
        self.cmd_path = self.config.PACKAGE_DIR + "/itraq_and_tmt/calc_ppi.py"
        self.ppi_table = self.option('ppitable')
        if not os.path.exists(self.ppi_table):
            time.sleep(20)
        ...
        self.out_files = ['protein_interaction_network_centrality.txt',
                          'protein_interaction_network_degree_distribution.txt',
                          'protein_interaction_network_node_degree.txt']
    ...
    def run_ppi_network_py(self):
        ...
        try:
            ...
            self.set_error("运行calc_ppi.py失败", code = "32504006")
    ...
    def get_filesname(self):
        ...
        for i in range(len(self.out_files)):
            if not files_status[i]:
                self.set_error('未知原因，结果文件生成出错或丢失', code = "32504007")
        return files_status
