...
class ClusterTool(Tool):
            ...
            with open(self.option("diff_fpkm").prop['path'],'r+') as f1,open(tmp,'w+') as f2:
                ...
                for lines in f1:
                    ...
                    if line[0] in gene_list:
                        ...
                        continue
            if not sequence:
                self.set_error("%s对应的genelist不在%s中出现！", variables=(diff_list_file,self.option('diff_fpkm').prop['path']), code="32503101")
            else:
                return tmp
        else:
            self.set_error("过滤基因或转录本id时请设置is_genelist为true！", code="32503102")
    ...
    def run_cluster(self,fpkm_path):
        ...
        if cmd.return_code == 0:
            ...
            self.set_error("运行clust_cmd出错", code="32503103")
