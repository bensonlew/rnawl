...
class ExpCorrAgent(Agent):
    ...
    def check_options(self):
        if self.option('exp').prop['sample_number'] <= 1:
            raise OptionError('Please input at least 2 samples', code = "32502701")
...
class ExpCorrTool(Tool):
    ...
    def run_cluster_toolbox(self):
        ...
        #cmd += '-log_base 2 ' # 蛋白数据本来就很小，暂时用2归一化
        ...
        if command.return_code == 0:
            ...
            if command.return_code is 0:
                ...
                self.set_error("%s Failed. >>>%s", variables = (cmd_name, cmd), code = "32502702")
        else:
            self.set_error("%s Failed. >>>%s", variables = (cmd_name, cmd), code = "32502703")
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        test_dir='/mnt/ilustre/users/sanger-dev/sg-users/litangjian' \
                 '/protein_dev/ratio.exp.txt'
        data = {
