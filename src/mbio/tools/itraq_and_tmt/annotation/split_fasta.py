...
class SplitFastaAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {"name": "fasta", "type": "infile", "format": "itraq_and_tmt.common"},
    ...
    def check_options(self):
        ...
        if not self.option("fasta").is_set:
            raise OptionError("请传入fasta序列文件", code = "32502001")
        if not isinstance(self.option('lines'), int):
            raise OptionError("行数必须为整数", code = "32502002")
        # if self.option('lines') % 2:
        #     raise OptionError("行数必须为整除2")
        if self.option('lines') <= 0:
            raise OptionError("行数小于等于0，请重设！", code = "32502003")
