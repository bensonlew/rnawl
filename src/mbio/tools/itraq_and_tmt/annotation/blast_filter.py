    ...
    def check_options(self):
        if self.option("method") == "blast":
            if not self.option("blast").is_set:
                raise OptionError("必须设置参数query", code="32500201")
        elif self.option("method") == "hmm":
            if not self.option("hmm").is_set:
                raise OptionError("必须设置参数hmm", code="32500202")
        else:
            raise OptionError('method %s必须为blast或hmm', variables=(self.option('method')), code="32500203")

        if not (self.option('evalue') >= 0 and self.option('evalue') <= 0.001) :
            raise OptionError('E-value值设定必须为[0-0.001]之间：%s', variables=(self.option('evalue')), code="32500204")
        if not (self.option('identity') >= 0 and self.option('identity') <= 100) :
            raise OptionError('Identity值设定必须为[0-100]之间：%s', variables=(self.option('identity')), code="32500205")
        if not (self.option('similarity') >= 0 and self.option('similarity') <= 100) :
            raise OptionError('similarity值设定必须为[0-100]之间：%s', variables=(self.option('similarity')), code="32500206")
