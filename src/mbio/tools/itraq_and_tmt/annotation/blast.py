...
class BlastAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {"name": "query", "type": "infile", "format": "itraq_and_tmt.common"},  # 输入文件
    ...
    def check_options(self):
        if not self.option("query").is_set:
            raise OptionError("必须设置参数query", code="32500101")
        if self.option('query_type') not in ['nucl', 'prot']:
            raise OptionError('query_type查询序列的类型为nucl(核酸)或者prot(蛋白):%s', variables=(self.option('query_type')), code="32500102")
        else:
        ...
        if self.option("database") == 'customer_mode':
            if not self.option("reference").is_set:
                raise OptionError("使用自定义数据库模式时必须设置reference", code="32500104")
            if self.option('reference_type') not in ['nucl', 'prot']:
                raise OptionError('reference_type参考序列的类型为nucl(核酸)或者prot(蛋白):%s', variables=(self.option('query_type')), code="32500105")
            else:
            ...
            raise OptionError("数据库%s不被支持" , variables=( self.option("database")), code="32500107")
        else:
            self.option('reference_type', self._database_type[self.option("database").lower()])
        if not 15 > self.option('outfmt') > -1:
            raise OptionError('outfmt遵循blast+输出规则，必须为0-14之间：%s', variables=(self.option('outfmt')), code="32500108")
        if not 1 > self.option('evalue') >= 0:
            raise OptionError('E-value值设定必须为[0-1)之间：%s', variables=(self.option('evalue')), code="32500109")
        if not 0 < self.option('num_alignment') < 1001:
            raise OptionError('序列比对保留数必须设置在1-1000之间:%s', variables=(self.option('num_alignment')), code="32500110")
...
class BlastTool(Tool):
    ...
    def run_makedb_and_blast(self):
        ...
        if makedb_obj.return_code == 0:
            ...
            self.set_error("makeblastdb运行出错!", code="32500101")
    ...
    def run_blast(self, db_name):
        ...
        if blast_command.return_code == 0:
            ...
            self.set_error("blast运行出错!", code="32500102")
