...
class EggnogAnnotAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {'name': 'blast_xml', 'type': 'infile', 'format': 'align.blast.blast_xml'},
            {'name': 'longest_t2g', 'type': 'infile', 'format': 'itraq_and_tmt.common'},
    ...
    def set_resource(self):
        ...
        self._memory = '{}G'.format(int(float(infile_size) / 1024 ** 3 * 2 + 10))
...
class EggnogAnnotTool(Tool):
    def __init__(self, config):
        ...
        self.get_venn_list_py = os.path.join(self.config.PACKAGE_DIR, 'itraq_and_tmt/annotation/get_venn_list.py')
        ...
        self.cog_tsv = os.path.join(self.output_dir, 'cog_table.xls')
        self.txpt_summary_tsv = os.path.join(self.output_dir, 'cog_summary.xls')
        self.txpt_list = os.path.join(self.output_dir, 'cog_list.xls')
    ...
    def run_get_venn_list_t(self):
        ...
        cmd += ' --input {}'.format(self.txpt_summary_tsv + "tmp")
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            'type': 'tool',
            'name': 'itraq_tmt.annotation.eggnog_annot',
            'instant': False,
            'options': {
