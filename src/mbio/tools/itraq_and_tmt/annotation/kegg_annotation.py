...
class KeggAnnotationAgent(Agent):
    ...
    def check_options(self):
        if not self.option("blastout").is_set:
            raise OptionError("必须提供BLAST结果文件", code = "32501101")
        if self.option("taxonomy") not in ["Animals", "Plants", "Fungi", "Protists", "Archaea", "Bacteria", "None", None]:
            raise OptionError("物种类别必须为Animals/Plants/Fungi/Protists/Archaea/Bacteria/None", code = "32501102")
...
class KeggAnnotationTool(Tool):
    ...
    def __init__(self, config):
        ...
        self.kegg_path3 = self.config.PACKAGE_DIR + "/itraq_and_tmt/kegg_annotation_v3.py"
        self.kegg_path2 = self.config.PACKAGE_DIR + "/itraq_and_tmt/kegg_annotation_v2.py"
    ...
    def kegg_annotation(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("运行kegg注释脚本出错", code = "32501103")
