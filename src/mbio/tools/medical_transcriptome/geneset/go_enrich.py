...
from mbio.packages.medical_transcriptome.functions import toolfuncdeco, runcmd
...
class GoEnrichTool(Tool):
    def __init__(self, config):
        ...
        self.script = {
            ...
            'go_enrich_stats': os.path.join(self.config.PACKAGE_DIR, 'medical_transcriptome/geneset/go_enrich_stats.py')
        }
        self.file = {
    ...
    def run(self):
        ...
        tool_run = self.pre_find_enrichment()
        if tool_run:
            self.run_find_enrichment()
            self.run_go_enrich_stats()
            self.set_output()
        else:
            self.logger.info('本次分析基因集无go注释结果不予分析')
            pass
        self.end()
    ...
    def pre_find_enrichment(self):
        ...
        count = len(open(self.file['study'], 'rU').readlines())
        if count > 0:
            return True
        else:
            return False
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            'type': 'tool',
            'name': 'medical_transcriptome.geneset.go_enrich',
            'instant': False,
            'options': {
