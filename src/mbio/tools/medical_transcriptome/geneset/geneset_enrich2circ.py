...
import json
import unittest
from mbio.packages.medical_transcriptome.enrich2circ2 import Enrich
from biocluster.config import Config
import subprocess


class GenesetEnrich2circAgent(Agent):
    """
    基因集ENRICH2CIRC 分析
    last_modify: 2018.3.13
    """
    def __init__(self, parent):
        super(GenesetEnrich2circAgent, self).__init__(parent)
        options = [
            {"name": "enrich_type", "type": "string", "default": "GO"},
            {"name": "enrich_table", "type": "infile", "format": "medical_transcriptome.common"},
            {"name": "fc_table", "type": "infile", "format": "medical_transcriptome.common"},
    ...
    def check_options(self):
        ...
        :return:
        """
        if self.option("enrich_type").upper() not in ['GO', 'KEGG', 'DISGENET', 'REACTOME', 'DO']:
    ...
    def end(self):
        super(GenesetEnrich2circAgent, self).end()
...
class GenesetEnrich2circTool(Tool):
    ...
    def run(self):
        ...
        if self.option("enrich_type") == "GO":
            ...
            result = Enrich2circ.filter_disgenet_enrich(self.option("enrich_table").prop['path'])
        elif self.option("enrich_type").upper() == "DO":
            result = Enrich2circ.filter_do_enrich(self.option("enrich_table").prop['path'])
        elif self.option("enrich_type").upper() == "REACTOME":
            result = Enrich2circ.filter_reactome_enrich(self.option("enrich_table").prop['path'])
        else:
            pass
    ...
    def set_output(self):
        if self.option("enrich_type") == "GO":
            ...
            all_files = ['disgenet_enrich_choose.table', 'disgenet_enrich_detail.table', 'enrich_zscore']
        elif self.option("enrich_type").upper() == "DO":
            all_files = ['do_enrich_choose.table', 'do_enrich_detail.table', 'enrich_zscore']
        elif self.option("enrich_type").upper() == "REACTOME":
            all_files = ['reactome_enrich_choose.table', 'reactome_enrich_detail.table', 'enrich_zscore']
        else:
            pass
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            "type": "tool",
            "name": "medical_transcriptome.proteinset.enrich2circ",
            "instant": True,
            "options": dict(
                enrich_type = "GO",
