...
import subprocess
import shutil
from mbio.packages.medical_transcriptome.snp_anno import snp_anno
import json
import glob
import os
import pandas as pd
...
class AnnovarTool(Tool):
    ...
    def filter_vcf(self):
        '''过滤掉合并vcf后没有AD信息的位点'''
        raw_vcf = "Combine_Variants.vcf"
        new_vcf = "filter_Combine_Variants"
        with open(raw_vcf,"r") as r,open(new_vcf,"w") as n:
    ...
    def run(self):
        ...
        if self.option('combine_vcf'):
            ...
            self.combine_vcf()
            self.filter_vcf()
            self.vcf_path = "filter_Combine_Variants"
