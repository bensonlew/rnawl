...
class ExpCorrAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {'name': 'exp', 'type': 'infile', 'format': 'ref_rna_v2.express_matrix'},
    ...
    def check_options(self):
        if self.option('exp').prop['sample_number'] <= 1:
            raise OptionError('Please input at least 2 samples', code = "33704601")
    ...
    def run_cluster_toolbox(self):
        ...
        if command.return_code == 0:
            ...
            if command.return_code is 0:
                ...
                self.set_error("%s Failed. >>>%s", variables = (cmd_name, cmd), code = "33704602")
        else:
            self.set_error("%s Failed. >>>%s", variables = (cmd_name, cmd), code = "33704603")
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            "type": "tool",
            "name": "medical_transcriptome.exp_corr",
            "instant": False,
            "options": dict(
                exp="/mnt/ilustre/users/sanger-dev/workspace/20200819/ExpDistribution_tsg_zjx_9406_6478/exp_matrix",
                scm="complete",
                scd="euclidean",
                corr_method='pearson',
                output=None,
