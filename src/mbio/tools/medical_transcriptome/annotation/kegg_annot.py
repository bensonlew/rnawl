...
class KeggAnnotAgent(Agent):
    ...
    def set_resource(self):
        ...
        self._memory = '{}G'.format(int(float(infile_size) / 1024 ** 3 * 8 + 20))
...
class KeggAnnotTool(Tool):
    ...
    def run(self):
        ...
        if os.path.exists(self.work_dir + "/run_txml2gxml.finished"):
            pass
        else:
            self.run_txml2gxml()
        if os.path.exists(self.work_dir + "/run_kegg_annotation_t.finished"):
            pass
        else:
            self.run_kegg_annotation_t()
        if os.path.exists(self.work_dir + "/run_kegg_annotation_g.finished"):
            pass
        else:
            self.run_kegg_annotation_g()
        if self.option("kegg_species"):
            if os.path.exists(self.work_dir + "/run_kegg_annotation_t_spe.finished"):
                pass
            else:
                self.run_kegg_annotation_t_spe()
            if os.path.exists(self.work_dir + "/run_kegg_annotation_g_spe.finished"):
                pass
            else:
    ...
    def run_code(self, cmd_name, cmd, shell=False, block=True):
        ...
        if block:
            self.wait()
            os.system("touch {}".format(self.work_dir + "/" + cmd_name + ".finished"))
