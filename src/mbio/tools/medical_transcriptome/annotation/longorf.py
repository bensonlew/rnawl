...
class LongorfAgent(Agent):
    ...
    def check_options(self):
        ...
        if not self.option("fasta").is_set:
            raise OptionError("请传入fasta序列文件", code = "33701801")
...
class LongorfTool(Tool):
    ...
    def td_longorfs(self):
        ...
        command = self.add_command("transdecoder_longorfs", cmd, ignore_error=True)
        command.run()
        self.wait()
        if command.return_code == 0:
            ...
            self.set_error("提取orf过程出错", code = "33701802")
