...
class DiamondAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "db", "type": "string", "default": "nr"},
            # 比对数据库 plant, nr, etc.
...
class DiamondTool(Tool):
    ...
    def get_db_path(self):
        sub_dict = dict()
        sub_version = "{}_version".format(self.option("db"))
        sub_dict.update({sub_version: self.option(sub_version)})

        # for sub_version in ['kegg_version', 'nr_version', 'swissprot_version', 'uniprot_version', 'eggnog_version', 'string_version']:
        #     sub_dict.update({sub_version: self.option(sub_version)})
    ...
    def get_nrxml_gi_description(self, xml_path):
        ...
        elif self.option("database") == "uniprot":
            nracc2des = os.path.dirname(self.db_path) + "/uniprot_acc2des.db"
            if os.path.exists(nracc2des):
                pass
            else:
                nracc2des = AnnotConfig().get_file_path(
