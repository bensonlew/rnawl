...
class GoAnnotation2Agent(Agent):
    """
    ...
    def check_options(self):
        if self.option("blastout").is_set:
            ...
            raise OptionError("必须提供BLAST结果文件", code = "33709902")
    ...
    def end(self):
        ...
        super(GoAnnotation2Agent, self).end()


class GoAnnotation2Tool(Tool):

    def __init__(self, config):
        super(GoAnnotation2Tool, self).__init__(config)
    ...
    def run(self):
        super(GoAnnotation2Tool, self).run()
        self.run_gomerge()
    ...
    def run_gomerge(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("运行merge_go出错", code="33709903")
    ...
    def run_annotation(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("运行annot_go出错", code="33709904")
        self.run_go_stat()
        self.set_out()
    ...
    def run_go_stat(self):
        ...
        go_class = pd.read_table(self.work_dir + '/go12level_statistics.xls')
    ...
    def set_out(self):
        outfiles = ['go12level_statistics.xls', 'go123level_statistics.xls', 'go1234level_statistics.xls', 'GO.list', 'go_detail.xls', 'GO.stat.xls']
