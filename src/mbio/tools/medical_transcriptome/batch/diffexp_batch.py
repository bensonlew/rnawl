...
class DiffexpBatchAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            # tpm_filter_method 20200921修改 by fwy ["num","average","min","max","median"]
            dict(name="method", type="string", default=None),
            ...
            dict(name='filter_method', type='string', default=None) # [None, sum, average, max, min, median]
    ...
    def set_resource(self):
        ...
        self._memory = "{}G".format(self.option("pool")*15)
...
class DiffexpBatchTool(Tool):
    ...
    def __init__(self, config):
        ...
        #self.diff_toolbox = self.config.PACKAGE_DIR + "/medical_transcriptome/batch/diff_toolbox_batch_new.py"
        self.diff_toolbox = self.config.PACKAGE_DIR + "/medical_transcriptome/batch/diff_toolbox_zjx.py"
        ...
        self.r_path = software_dir + "/miniconda3/bin"
        # self._r_home = software_dir + "/bioinfo/miniconda2/lib64/R/"
        # self._LD_LIBRARY_PATH = software_dir + "/bioinfo/miniconda2/lib64/R/lib:$LD_LIBRARY_PATH"
    ...
    def diffexp(self):
        ...
        cmd += '--analysis {} '.format('split')
        cmd += '--filter_method {} '.format(self.option('filter_method'))
        cmd += '--project {} '.format('medical_transcriptome')
    ...
    def check_diff_results(self):
        ...
        if self.is_duplicate:
            default_order = [("DESeq2", "padjust", 2.0, 0.05), ("DESeq2", "pvalue", 2.0, 0.05),
                             ("edgeR", "padjust", 2.0, 0.05),
                             ("edgeR", "pvalue", 2.0, 0.05), ("Limma", "padjust", 2.0, 0.05),
                             ("Limma", "pvalue", 2.0, 0.05),
                             ("DESeq2", "padjust", 1.5, 0.05), ("DESeq2", "pvalue", 1.5, 0.05),
                             ("edgeR", "padjust", 1.5, 0.05),
                             ("edgeR", "pvalue", 1.5, 0.05), ("Limma", "padjust", 1.5, 0.05),
                             ("Limma", "pvalue", 1.5, 0.05),
                             ("NOIseq", 0.8, 2.0)]
        else:
            default_order = [("DEGseq", "padjust", 2.0, 0.001), ("DEGseq", "pvalue", 2.0, 0.001),
                             ("edgeR", "padjust", 2.0, 0.05),
                             ("edgeR", "pvalue", 2.0, 0.05), ("Limma", "padjust", 2.0, 0.05),
                             ("Limma", "pvalue", 2.0, 0.05),
                             ("deseq2", "padjust", 1.5, 0.05), ("deseq2", "pvalue", 1.5, 0.05),
    ...
    def rerun_diffexp(self):
        all_deleted_inter_files = glob.glob(self.option("output") + '/*_vs_*.*') + glob.glob(
            self.option("output") + '/*summary*.xls')
    ...
    def modify_result_file(self):
        if self.option("method").lower() in ["degseq", "edger", "deseq2", 'limma', 'svaseqlimma']:
            ...
            diff_files_stat = glob.glob(
                os.path.join(self.output_dir, '*_vs_*.{}.xls'.format(self.option("method").lower())))
            df1 = pd.DataFrame()
            # f_head = "seq_id\t \t \t"
            for diff_file_stat in diff_files_stat:
                ctrl, test = \
                    os.path.basename(diff_file_stat).split('.{}.xls'.format(self.option("method").lower()))[0].split(
                        '_vs_')
            ...
            for diff_file_stat in diff_files_stat:
                ctrl, test = \
                    os.path.basename(diff_file_stat).split('.{}.xls'.format(self.option("method").lower()))[
                        0].split('_vs_')
                fname = os.path.basename(diff_file_stat).split(".")[0]
                # f_head = f_head + fname + "\t*\t*\t*\t*\t*\t*\t"
    ...
    def chart(self):
        ...
        diff_exp_summary = glob.glob(self.output_dir + '/*summary*.xls')
        for each in diff_exp_summary:
            if each.endswith('.annot.xls'):
                continue
            else:
                summary = each
                break
        chart.chart_diffexp_stat(summary)

        for cmps in self.option("cmp").prop['cmp_list']:
            diff_exps = glob.glob(self.output_dir + '/*{}_vs_{}*.xls'.format(cmps[0], cmps[1]))
            for each in diff_exps:
                if each.endswith('.annot.xls'):
                    continue
                else:
                    chart.chart_diffexp_scatter(each, "{}_vs_{}".format(cmps[0], cmps[1]), soft=self.option("method"),pvalue_pajust = self.option("pvalue_padjust"))
        chart.to_pdf()

        # move pdf to result dir
        pdf_file = glob.glob(self.work_dir + "/*.pdf")
        for p in pdf_file:
            if os.path.exists(os.path.join(self.output_dir,os.path.basename(p))):
                os.remove(os.path.join(self.output_dir,os.path.basename(p)))
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "Diffexp_medical_transcriptome_noiseq" + str(random.randint(1, 10000)),
            "type": "tool",
            "name": "medical_transcriptome.batch.diffexp_batch",
            "instant": False,
            "options": dict(
                count = '/mnt/ilustre/users/sanger-dev/workspace/20200922/MedicalTranscriptome_medical_transcriptome_workflow_20200922_140020529/Quant/ref.gene.count.matrix',
                exp = '/mnt/ilustre/users/sanger-dev/workspace/20200922/MedicalTranscriptome_medical_transcriptome_workflow_20200922_140020529/Quant/ref.gene.tpm.matrix',
                method = "DESeq2",
                group = "/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/remote_input/group_table/example_group_1528169151.txt",
                cmp = "/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/remote_input/control_file/example_control_1528169151.txt",
                pool=3,
                output=None,
                count_cutoff=2,
                dispersion=0.05,
                ...
                # is_batch=True,
                # has_batch=True,
                # batch_matrix='/mnt/ilustre/users/sanger-dev/sg-users/zoujiaxun/batch/test/test_batch_new',
                no_filter=True,
                analysis='split'
            )
        }
