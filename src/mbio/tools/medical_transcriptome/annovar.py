...
import subprocess
import shutil
from mbio.packages.ref_rna_v2.snp_anno import snp_anno
import json
import glob
import os
import pandas as pd
...
class AnnovarAgent(Agent):
    ...
    def check_options(self):
        ...
        if not self.option("input_file").is_set:
            raise OptionError("请输入VCF格式文件", code = "33703201")
...
class AnnovarTool(Tool):
    ...
    def dict(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("构建dict过程error！", code = "33703202")
    ...
    def combine_vcf(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("运行combine_variants出错", code = "33703203")
    ...
    def gtf_to_genepred(self):
        ...
        if ret:
            ...
            self.set_error('find format error in %s, abord', variables=(self.ref_gtf), code="33703208")
        ...
        if command.return_code == 0:
            ...
            self.set_error("运行gtfToGenePred出错！", code = "33703204")
    ...
    def retrieve_seq_from_fasta(self):
        ...
        if command.return_code == 0 or None:
            ...
            self.set_error("运行retrieve_seq_from_fasta出错！", code = "33703205")


    def convert2annovar(self):
        cmd = "{}perl {}convert2annovar.pl -format vcf4 -allallele {} > clean.snp.avinput"\
    ...
    def annotate_variation(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("运行annotate_variation出错！", code = "33703206")
                    ...
                    for i in gene_list:
                        for j in i:
                            if j in dict_id_symbol.keys():
                                ...
                                desc_list_str = ";;".join(desc_list)
                    if len(gene_list) == 0:
                        print "gene_list is empty\t".join(whole[0:8])
    ...
    def test(self):
        ...
        data = {
            ...
            "options": dict(
                ...
                input_file="/mnt/ilustre/users/sanger-dev/workspace/20190701/Single_snp_test_gatk477-xxx/SamRna/VcfFilterSamtools/output/final.vcf",
