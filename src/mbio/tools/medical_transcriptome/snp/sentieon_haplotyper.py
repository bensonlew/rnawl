...
class SentieonHaplotyperTool(Tool):
    ...
    def run_LocusCollector(self):
        ...
        command = self.add_command("locuscollector", cmd,ignore_error=True).run()
        self.wait()
    ...
    def run_dnascope(self):
        """
        dnascope

        """
        if self.option("file_format").lower() == "bam":
            cmd = "{} driver -t 8 -i {}.splice.bam -r {} --algo DNAscope --trim_soft_clip --call_conf 20 --emit_conf 20 --emit_mode GVCF {}"\
                .format(self.sentieon, self.option("name"), self.option("fa_file").prop["path"],
                        self.output_dir + "/" + self.option("name")+".g.vcf")
        else:
            cmd = "{} driver -t 8 -i {}.splice.cram -r {} --algo DNAscope --trim_soft_clip --call_conf 20 --emit_conf 20 --emit_mode GVCF {}" \
                .format(self.sentieon, self.option("name"), self.option("fa_file").prop["path"],
                        self.output_dir + "/" + self.option("name") + ".g.vcf")
        command = self.add_command("haplotyper_v2", cmd).run()
        self.wait()
    ...
    def run(self):
        ...
        if self.option("algorithm").lower() == "haplotypecaller":
            self.run_haplotyper_v2()
        elif self.option("algorithm").lower() == "dnascope":
            self.run_dnascope()
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "sentieon_2020" + str(random.randint(1, 10000))+"yyyy",
            "type": "tool",
            "name": "medical_transcriptome.snp.sentieon_haplotyper",
            "instant": False,
            "options": dict(
                bam_file="/mnt/ilustre/users/sanger-dev/workspace/20201127/MedicalTranscriptome_10geotnj4elqrt9kqtb36obrid/CallSnpIndelSentieon/Pre4sentieon/BamSort/output/shNS_3.bam",
