...
class SickleAgent(Agent):
    ...
    def check_options(self):
        ...
        if not self.option("fastq_dir").is_set and not self.option("fastq_r").is_set and not self.option("fastq_s").is_set:
            raise OptionError("请传入fastq序列文件或者文件夹", code = "34002701")
        if self.option('fq_type') not in ['PE', 'SE']:
            raise OptionError("请说明序列类型，PE or SE?", code = "34002702")
        if not self.option("fastq_dir").is_set and self.option('fq_type') in ["PE"]:
            if not self.option("fastq_r").is_set:
                raise OptionError("请传入PE右端序列文件", code = "34002703")
            if not self.option("fastq_l").is_set:
                raise OptionError("请传入PE左端序列文件", code = "34002704")
        if not self.option("fastq_dir").is_set:
            if self.option('fq_type') in ["SE"] and not self.option("fastq_s").is_set:
                raise OptionError("请传入SE序列文件", code = "34002705")
...
class SickleTool(Tool):
    ...
    def sickle(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("运行sickle运行出错!", code = "34002706")
            return False
        ...
        if self.option("fastq_dir").is_set:
            ...
            for cmd in commands:
                if cmd.return_code == 0:
                    ...
                    self.set_error("运行%s运行出错!", variables = (cmd.name), code = "34002707")
                    return False
        else:
            self.sickle()
        self.set_output()
