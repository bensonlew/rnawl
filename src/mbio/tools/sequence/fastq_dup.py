...
class FastqDupTool(Tool):
    ...
    def multi_dup(self):
        ...
        for sample in self.samples:
            ...
            command = self.add_command("{}_dup".format(sample.lower()), cmd,ignore_error=True)
            command.run()
            cmds.append(command)
        return cmds
    ...
    def single_dup(self):
        ...
        command = self.add_command("fastq_dup", cmd,ignore_error=True)
        command.run()
        self.wait()
        if command.return_code == 0:
            ...
            self.set_error("运行fastq_dup.py运行出错!", code = "34000906")
            return False
    ...
    def run(self):
        ...
        if self.option("fastq_dir").is_set:
            ...
            for cmd in cmds:
                ...
                elif cmd.return_code == 1:
                    self.logger.info("return code: {}".format(cmd.return_code))
                    self.add_state('memory_limit', 'memory is low!')
                else:
                    self.set_error("运行%s出错!", variables = (cmd.name), code = "34000907")
                    return False
        else:
            self.single_dup()
        self.set_output()
