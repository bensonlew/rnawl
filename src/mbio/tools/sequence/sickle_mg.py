...
class SickleMgAgent(Agent):
    ...
    def check_options(self):
        ...
        if not self.option("fastq_dir").is_set and not self.option("fastq_r").is_set and not self.option(
                "fastq_s").is_set:
            raise OptionError("请传入fastq序列文件或者文件夹", code="34002801")
        if self.option('fq_type') not in ['PE', 'SE', 'PSE']:
            raise OptionError("请说明序列类型，PE or SE or 'PSE'?", code="34002802")
        if not self.option("fastq_dir").is_set and self.option('fq_type') in ["PE", "PSE"]:
            if not self.option("fastq_r").is_set:
                raise OptionError("请传入PE右端序列文件", code="34002803")
            if not self.option("fastq_l").is_set:
                raise OptionError("请传入PE左端序列文件", code="34002804")
        if not self.option("fastq_dir").is_set:
            ...
            if self.option('fq_type') in ["SE"] and not self.option(
                ...
                raise OptionError("请传入SE序列文件", code="34002805")
...
class SickleMgTool(Tool):
    ...
    def sickle(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("运行sickle运行出错!", code="34002801")
            return False
        ...
        if self.option("fastq_dir").is_set:
            ...
            for cmd in commands:
                if cmd.return_code == 0:
                    ...
                    self.set_error("运行%s运行出错!", variables=(cmd.name), code="34002802")
                    return False
        else:
            self.sickle()
        self.set_output()
