...
class NoiseqAgent(Agent):
    '''
    last_modify: 2019.11.15
    '''

    def __init__(self, parent):
        super(NoiseqAgent, self).__init__(parent)
        options = [
    ...
    def end(self):
        super(NoiseqAgent, self).end()


class NoiseqTool(Tool):
    def __init__(self, config):
        super(NoiseqTool, self).__init__(config)
        ...
        self.script = {
            'noiseq': os.path.join(self.config.PACKAGE_DIR, 'whole_transcriptome_v1_1/expression/noiseq.r')
        }
        self.file = {
    ...
    def run_noiseq(self):
        cmd = '{} {} -i {}'.format(self.program['rscript'], self.script['noiseq'], self.file['json'])
        runcmd(self, 'run_noiseq', cmd)
