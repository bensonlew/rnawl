...
class MetastatAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {"name": "cpu", "type": "int", "default": 2},  # cpu数
            ...
            {"name": "est_correction", "type": "string", "default": "fdr"},  # add by qingchen.zhang @20200423
            {"name": "est_type", "type": "string", "default": "two.side"},  # add by qingchen.zhang @20200423
            {"name": "signal_pair_file","type":"infile", "format":"meta.otu.otu_table"}, # guaniqng
            {"name": "need_percents", "type": "string", "default": "true"}, ## 结果是否需要相对丰度  # add by qingchen.zhang @20200423
            {"name": "percent_abund", "type": "string", "default": "true"},  # 是否用相对丰度 add by qingchen.zhang @20200423
            {"name": "meta_group_name", "type": "string","default": ""}, # 结果目录层级
        ]
    ...
    def check_sample(self, group_sample, otu_sample):
        for name in group_sample:
            if name not in otu_sample:
                raise OptionError('分组样本不在otu表所拥有的样本内，请检查分组方案', code="34100801")
    ...
    def check_group(self, groupfile):
        with open(groupfile, 'rb') as g:
            ...
            if 1 in sam_num:
                raise OptionError('一个分组类别下面至少有两个样本，请重新分组', code="34100802")
            if len(sample) != len(set(sample)):
                raise OptionError('不同的分组类别下有相同的样本，此分析不支持该分组方案，请重新分组', code="34100803")
        ...
        for i in self.option('test').split(','):
            if i not in ["chi", "fisher", "kru_H", "mann", "signal", "anova", "student", "welch", "estimator"]:
                ...
                if self.option('fisher_sample1') not in otu_sample or self.option('fisher_sample2') not in otu_sample:
                    raise OptionError('输入的样本不在otu表所拥有的样本内，请检查费舍尔检验样本名', code="34100819")
            elif i == "kru_H":
                if not self.option("kru_H_input").is_set:
                    raise OptionError('必须设置Kruskal-Wallis秩和检验输入的otutable文件', code="34100820")
                if not self.option("kru_H_group").is_set:
                    raise OptionError('必须设置Kruskal-Wallis秩和检验输入的分组文件', code="34100821")
                if not self.option("kru_H_gname"):
                    raise OptionError("Kruskal-Wallis秩和检验分组方案选择参数为必须参数，请设置", code="34100822")
                if len(self.option("kru_H_gname").split(',')) != 1:
                    raise OptionError("组间差异的分组方案只能为1个", code="34100823")
                if self.option("kru_H_correction") not in ["holm", "hochberg", "hommel", "bonferroni", "BH", "BY",
                                                           "fdr", "none"]:
                    raise OptionError('该多重检验校正的方法不被支持', code="34100824")
                ...
                if gnum < 3:
                    raise OptionError("Kruskal-Wallis秩和检验的分组方案的分组类别必须大于等于3", code="34100825")
                if self.option("kru_H_coverage") not in [0.90, 0.95, 0.98, 0.99, 0.999]:
                    raise OptionError('Kruskal-Wallis秩和检验的posthoc的置信度不在范围值内', code="34100826")
                if self.option("kru_H_methor") not in ["scheffe", "welchuncorrected", "tukeykramer", "gameshowell",
                                                       "dunn", "nemenyi", "conover-iman", "steel_dwass"]:
                    raise OptionError('Kruskal-Wallis秩和检验的posthoc检验方法不在范围值内', code="34100827")
                ...
                if not self.option("anova_input").is_set:
                    raise OptionError('必须设置Kruskal-Wallis秩和检验输入的otutable文件', code="34100828")
                if not self.option("anova_group").is_set:
                    raise OptionError('必须设置Kruskal-Wallis秩和检验输入的分组文件', code="34100829")
                if not self.option("anova_gname"):
                    raise OptionError("单因素方差分析分组方案选择参数为必须参数，请设置", code="34100830")
                if len(self.option("anova_gname").split(',')) != 1:
                    raise OptionError("组间差异的分组方案只能为1个", code="34100831")
                if self.option("anova_correction") not in ["holm", "hochberg", "hommel", "bonferroni", "BH", "BY",
                                                           "fdr", "none"]:
                    raise OptionError('该多重检验校正的方法不被支持', code="34100832")
                gnum = self.option('anova_group').group_num(self.option('anova_gname'))
                if gnum < 3:
                    raise OptionError("单因素方差分析的分组方案的分组类别必须大于等于3", code="34100833")
                if self.option("anova_coverage") not in [0.90, 0.95, 0.98, 0.99, 0.999]:
                    raise OptionError('单因素方差分析的posthoc的置信度不在范围值内', code="34100834")
                if self.option("anova_methor") not in ["scheffe", "welchuncorrected", "tukeykramer", "gameshowell"]:
                    raise OptionError('单因素方差分析的posthoc检验方法不在范围值内', code="34100835")
                ...
                if not self.option("mann_input").is_set:
                    raise OptionError('必须设置wilcox秩和检验输入的otutable文件', code="34100836")
                if not self.option("mann_group").is_set:
                    raise OptionError('必须设置wilcox秩和检验输入的分组文件', code="34100837")
                if self.option("mann_correction") not in ["holm", "hochberg", "hommel", "bonferroni", "BH", "BY", "fdr",
                                                          "none"]:
                    raise OptionError('该多重检验校正的方法不被支持', code="34100838")
                if self.option("mann_ci") <= 0 or self.option("mann_ci") >= 1:
                    raise OptionError('所输入的显著水平不在范围值内', code="34100839")
                if self.option("mann_type") not in ["two.side", "greater", "less"]:
                    raise OptionError('所输入的类型不在范围值内', code="34100840")
                if not self.option("mann_gname"):
                    raise OptionError("wilcox秩和检验分组方案选择参数为必须参数，请设置", code="34100841")
                if len(self.option("mann_gname").split(',')) != 1:
                    raise OptionError("组间差异的分组方案只能为1个", code="34100842")
                gnum = self.option('mann_group').group_num(self.option('mann_gname'))
                if gnum != 2:
                    raise OptionError("wilcox秩和检验的分组方案的分组类别必须等于2", code="34100843")
                if self.option("mann_coverage") not in [0.90, 0.95, 0.98, 0.99, 0.999]:
                    raise OptionError('wilcox秩和检验的置信区间的置信度不在范围值内', code="34100844")
                ...
                if not self.option("signal_input").is_set:
                    raise OptionError('必须设置wilcox秩和检验输入的otutable文件', code="34100845")
                if not self.option("signal_group").is_set:
                    raise OptionError('必须设置wilcox秩和检验输入的分组文件', code="34100846")
                if self.option("signal_correction") not in ["holm", "hochberg", "hommel", "bonferroni", "BH", "BY", "fdr",
                                                          "none"]:
                    raise OptionError('该多重检验校正的方法不被支持', code="34100847")
                if self.option("signal_ci") <= 0 or self.option("signal_ci") >= 1:
                    raise OptionError('所输入的显著水平不在范围值内', code="34100848")
                if self.option("signal_type") not in ["two.side", "greater", "less"]:
                    raise OptionError('所输入的类型不在范围值内', code="34100849")
                if not self.option("signal_gname"):
                    raise OptionError("wilcox秩和检验分组方案选择参数为必须参数，请设置", code="34100850")
                if len(self.option("signal_gname").split(',')) != 1:
                    raise OptionError("组间差异的分组方案只能为1个", code="34100851")
                gnum = self.option('signal_group').group_num(self.option('signal_gname'))
                if gnum != 2:
                    raise OptionError("wilcox秩和检验的分组方案的分组类别必须等于2", code="34100852")
                if self.option("signal_coverage") not in [0.90, 0.95, 0.98, 0.99, 0.999]:
                    raise OptionError('wilcox秩和检验的置信区间的置信度不在范围值内', code="34100853")
                ...
                if not self.option("student_input").is_set:
                    raise OptionError('必须设置student_T检验输入的otutable文件', code="34100854")
                if not self.option("student_group").is_set:
                    raise OptionError('必须设置student_T检验输入的分组文件', code="34100855")
                if self.option("student_correction") not in ["holm", "hochberg", "hommel", "bonferroni", "BH", "BY",
                                                             "fdr", "none"]:
                    raise OptionError('该多重检验校正的方法不被支持', code="34100856")
                if self.option("student_ci") <= 0 or self.option("student_ci") >= 1:
                    raise OptionError('所输入的显著水平不在范围值内', code="34100857")
                if self.option("student_type") not in ["two.side", "greater", "less"]:
                    raise OptionError('所输入的类型不在范围值内', code="34100858")
                if not self.option("student_gname"):
                    raise OptionError("student_T检验分组方案选择参数为必须参数，请设置", code="34100859")
                if len(self.option("student_gname").split(',')) != 1:
                    raise OptionError("组间差异的分组方案只能为1个", code="34100860")
                gnum = self.option('student_group').group_num(self.option('student_gname'))
                if gnum != 2:
                    raise OptionError("student_T检验的分组方案的分组类别必须等于2", code="34100861")
                if self.option("student_coverage") not in [0.90, 0.95, 0.98, 0.99, 0.999]:
                    raise OptionError('student_T检验的置信区间的置信度不在范围值内', code="34100862")
                ...
                if not self.option("welch_input").is_set:
                    raise OptionError('必须设置welch_T检验输入的otutable文件', code="34100863")
                if not self.option("welch_group").is_set:
                    raise OptionError('必须设置welch_T检验输入的分组文件', code="34100864")
                if self.option("welch_correction") not in ["holm", "hochberg", "hommel", "bonferroni", "BH", "BY",
                                                           "fdr", "none"]:
                    raise OptionError('该多重检验校正的方法不被支持', code="34100865")
                if self.option("welch_ci") <= 0 or self.option("welch_ci") >= 1:
                    raise OptionError('所输入的显著水平不在范围值内', code="34100866")
                if self.option("welch_type") not in ["two.side", "greater", "less"]:
                    raise OptionError('所输入的类型不在范围值内', code="34100867")
                if not self.option("welch_gname"):
                    raise OptionError("welch_T检验分组方案选择参数为必须参数，请设置", code="34100868")
                if len(self.option("welch_gname").split(',')) != 1:
                    raise OptionError("组间差异的分组方案只能为1个", code="34100869")
                gnum = self.option('welch_group').group_num(self.option('welch_gname'))
                if gnum != 2:
                    raise OptionError("welch_T检验的分组方案的分组类别必须等于2", code="34100870")
                if self.option("welch_coverage") not in [0.90, 0.95, 0.98, 0.99, 0.999]:
                    raise OptionError('welch_T检验的置信区间的置信度不在范围值内', code="34100871")
                ...
                if not self.option("est_input").is_set:
                    raise OptionError('必须设置est_T检验输入的多样性指数文件', code="34100872")
                if not self.option("est_group").is_set:
                    raise OptionError('必须设置est_T检验输入的分组文件夹', code="34100873")
...
class MetastatTool(Tool):
    ...
    def remove_zero_and_mask_taxon(self, profile, group):
        if type(group) == str:
            ...
            raise OptionError('函数中group类型问题，type is %s', variables=(type(group)), code="34100874")
        ...
        if len(self.name_to_name) < 2:
            self.set_error("所含非零物种/功能过少，更改其他分类水平分析。非零物种/功能数量为：%s", variables=(len(self.name_to_name)), code="34100801")
    ...
    def run_est(self):    # modify by qindanhua 20170105 add option test method
        ...
        for group in gfilelist:
            ...
            # by houshuang 20191011 替换底层脚本，与两组比较一致
            two_group_test(self.option('est_input').prop['path'], os.path.join(self.option("est_group").prop['path'], group),
                           self.work_dir + '/est_result%s.xls' % i, self.work_dir + '/est_barplot_result%s.xls' % i,
                           self.option("est_test_method"), test_type=self.option("est_type"),mul_test=self.option("est_correction"), norm="FF")
            r_file = "run_%s_test.r" % self.option("est_test_method")
            cmd = self.r_path + " " + r_file
            # cmd = self.r_path + " run_est_ttest.r"
            self.logger.info("开始运行est_T检验")
            command = self.add_command("est_cmd{}".format(i), cmd, ignore_error=True).run()
            i += 1
            self.wait(command)
            if command.return_code == 0 or command.return_code is None:
                self.logger.info("est_ttest运行完成")
            else:
                self.set_error("est_ttest运行出错!", code="34100802")
    ...
    def run_chi(self):
        ...
        two_sample_test(self.work_dir + "/tmp_mask_otu.xls", self.work_dir + '/chi_result.xls', "chi",
                        self.option('chi_sample1'), self.option('chi_sample2'), mul_test=self.option('chi_correction'), cpu=self.option("cpu")-1)
        ...
        command = self.add_command("chi_cmd", cmd, ignore_error=True).run()
        self.wait(command)
        def success():
        ...
        def fail():
        # else:
            self.set_error("chi_cmd运行出错!", code="34100803")
    ...
    def run_fisher(self):
        ...
        two_sample_test(self.work_dir + '/tmp_mask_otu.xls', self.work_dir + '/fisher_result.xls', "fisher",
                        ...
                        mul_test=self.option('fisher_correction'),cpu=self.option("cpu")-1)
        ...
        command = self.add_command("fisher_cmd", cmd, ignore_error=True).run()
        self.wait(command)
        # if command.return_code == 0:
        def success():
        ...
        # else:
        def fail():
            self.set_error("fisher_cmd运行出错!", code="34100804")
    ...
    def run_student(self):
        ...
        command = self.add_command("student_cmd", cmd, ignore_error=True).run()
        self.wait(command)
        # if command.return_code == 0:
        def success():
            ...
            if bar_cmd.return_code == 0:
                ...
                self.set_error("bar_cmd运行出错!", code="34100805")
        # else:
        def fail():
            self.set_error("student_cmd运行出错!", code="34100806")
        check_command(self, command, [0], [-9], success, fail)
    ...
    def run_welch(self):
        ...
        command = self.add_command("welch_cmd", cmd, ignore_error=True).run()
        self.wait(command)
        # if command.return_code == 0:
        def success():
            ...
            # else:
            def child_fail():
                self.set_error("bar_cmd运行出错!", code="34100807")
            check_command(self, bar_cmd, [0], [-9], child_success, child_fail)
        # else:
        def fail():
            self.set_error("welch_cmd运行出错!", code="34100808")
        check_command(self, command, [0], [-9], success, fail)
    ...
    def run_mann(self):
        ...
        def success():
            ...
            # else:
            def child_fail():
                self.set_error("bar_cmd运行出错!", code="34100809")
            check_command(self, bar_cmd, [0], [-9], child_success, child_fail)
        # else:
        def fail():
            self.set_error("mann_cmd运行出错!", code="34100810")
        check_command(self, command, [0], [-9], success, fail)

    def produce_new_signal_pair(self):
        ##guanqing >>>
        ## 按配对方案调准行的位置  20191014
        map_pair = {}
        with open(self.option("signal_pair_file").path) as f:
            lines = f.readlines()
            hit_g = lines[0].split("\t")[0]
            pair_g = lines[0].strip().split("\t")[1]
            for line in  lines[1:]:
                spline = line.strip().split("\t")
                map_pair[spline[0]] = spline[1]
        new_pair_file = 'signal_group_checked'

        with open('./signal_group') as fr, open(new_pair_file, 'w') as fw:
            lines = fr.readlines()
            hit_g_list = []
            pair_list = []
            for line  in lines[1:]:
                spline = line.strip().split("\t")
                if spline[1] not in [hit_g]:
                    pair_list.append(spline[0])
                else:
                    hit_g_list.append(spline[0])

            # 输出配对校正后的文件
            fw.write(lines[0])
            pick_hits = []
            for s in hit_g_list:
                if s not in map_pair:
                    continue
                if map_pair[s] not in pair_list:
                    continue
                pick_hits.append(s)
                fw.write("%s\t%s\n"%(s, hit_g))
    ...
    def run_signal(self):
        ...
        if self.option('signal_pair_file').is_set:
            pair_file = self.produce_new_signal_pair()
        else:
            pair_file = './signal_group'

        two_group_test(self.work_dir + '/tmp_mask_otu.xls', pair_file,
        ...
        command = self.add_command("signal_cmd", cmd, ignore_error=True).run()
        self.wait(command)
        # if command.return_code == 0:
        def success():
            ...
            # else:
            def child_fail():
                self.set_error("bar_cmd运行出错!", code="34100811")
            check_command(self, bar_cmd, [0], [-9], child_success, child_fail)
        # else:
        def fail():
            self.set_error("signal_cmd运行出错!", code="34100812")
        check_command(self, command, [0], [-9], success, fail)
    ...
    def run_kru(self):
        ...
        mul_group_test(self.work_dir + "/tmp_mask_otu.xls", self.work_dir + '/kru_H_result.xls',
                       ...
                       self.option('kru_H_correction'), need_percents=self.option("need_percents"), percent_abund=self.option("percent_abund"))
        cmd = self.r_path + " run_kru_H_test.r"
        self.logger.info("开始运行kru_H检验")
        command = self.add_command("kru_cmd", cmd, ignore_error=True).run()
        self.wait(command)
        # if command.return_code == 0:
        def success():
            self.logger.info("kru_cmd运行完成，开始运行post-hoc检验")
            in_file = self.work_dir + '/kru_H_result.xls'
            if self.option("kru_H_methor") in ["dunn", "nemenyi", "conover-iman", "steel_dwass"]:
                in_file = self.work_dir + "/tmp_mask_otu.xls"
            self.posthoc(self.option("kru_H_methor"), in_file, './kru_H_group',
            ...
            # else:
            def child_fail():
                self.set_error("bar_cmd运行出错!", code="34100813")
            check_command(self, bar_cmd, [0], [-9], child_success, child_fail)
        # else:
        def fail():
            self.set_error("kru_cmd运行出错!", code="34100814")
        check_command(self, command, [0], [-9], success, fail)
    ...
    def run_anova(self):
        ...
        mul_group_test(self.work_dir + "/tmp_mask_otu.xls", self.work_dir + '/anova_result.xls',
                       ...
                       self.option('anova_correction'), need_percents=self.option("need_percents"), percent_abund=self.option("percent_abund"))
        cmd = self.r_path + " run_anova_test.r"
        self.logger.info("开始运行anova检验")
        command = self.add_command("anova_cmd", cmd, ignore_error=True).run()
        self.wait(command)
        # if command.return_code == 0:
        def success():
            ...
            # else:
            def child_fail():
                self.set_error("bar_cmd运行出错!", code="34100815")
            check_command(self, bar_cmd, [0], [-9], child_success, child_fail)
        # else:
        def fail():
            self.set_error("anova_cmd运行出错!", code="34100816")
        check_command(self, command, [0], [-9], success, fail)
    ...
    def posthoc(self, methor, statfile, groupfile, coverage, outfile):
        if methor == 'tukeykramer':
            ...
            self.logger.info("进行kru独有的 posthoc 计算")
            r_path = "/bioinfo/metaGenomic/metaphlan3/bin/Rscript"
            r_posthoc = Config().PACKAGE_DIR + '/statistical/posthoc.r'
            ...
            if t == 'chi':
                ...
                try:
                    if self.option('meta_group_name'):
                        ...
                        self.result_dir = self.output_dir + "/" + self.option('meta_group_name')
                        os.link(self.work_dir + '/student_plot_group_bar.xls', self.result_dir + '/student_plot_group_bar.xls')
                    else:
                        self.result_dir = self.output_dir
                    self.add_taxon(self.work_dir + '/student_result.xls', self.work_dir + '/student_result_new.xls')
                    os.link(self.work_dir + '/student_result_new.xls', self.result_dir + '/student_result.xls')
                    self.add_taxon(self.work_dir + '/student_boxfile.xls', self.work_dir + '/student_boxfile_new.xls')
                    os.link(self.work_dir + '/student_boxfile_new.xls', self.result_dir + '/student_boxfile.xls')
                    self.add_taxon(self.work_dir + '/student_CI.xls', self.work_dir + '/student_CI_new.xls')
                    os.link(self.work_dir + '/student_CI_new.xls', self.result_dir + '/student_CI.xls')
                ...
                try:
                    if self.option('meta_group_name'):
                        if not os.path.exists(self.output_dir + "/" + self.option('meta_group_name')):
                            os.makedirs(self.output_dir + "/" + self.option('meta_group_name'))
                        self.result_dir = self.output_dir + "/" + self.option('meta_group_name')
                        os.link(self.work_dir + '/welch_plot_group_bar.xls', self.result_dir + '/welch_plot_group_bar.xls')
                    else:
                        self.result_dir = self.output_dir
                    self.add_taxon(self.work_dir + '/welch_result.xls', self.work_dir + '/welch_result_new.xls')
                    os.link(self.work_dir + '/welch_result_new.xls', self.result_dir + '/welch_result.xls')
                    self.add_taxon(self.work_dir + '/welch_boxfile.xls', self.work_dir + '/welch_boxfile_new.xls')
                    os.link(self.work_dir + '/welch_boxfile_new.xls', self.result_dir + '/welch_boxfile.xls')
                    self.add_taxon(self.work_dir + '/welch_CI.xls', self.work_dir + '/welch_CI_new.xls')
                    os.link(self.work_dir + '/welch_CI_new.xls', self.result_dir + '/welch_CI.xls')
                ...
                try:
                    if self.option('meta_group_name'):
                        if not os.path.exists(self.output_dir + "/" + self.option('meta_group_name')):
                            os.makedirs(self.output_dir + "/" + self.option('meta_group_name'))
                        self.result_dir = self.output_dir + "/" + self.option('meta_group_name')
                        os.link(self.work_dir + '/mann_plot_group_bar.xls', self.result_dir + '/mann_plot_group_bar.xls')
                    else:
                        self.result_dir = self.output_dir
                    self.add_taxon(self.work_dir + '/mann_result.xls', self.work_dir + '/mann_result_new.xls')
                    os.link(self.work_dir + '/mann_result_new.xls', self.result_dir + '/mann_result.xls')
                    self.add_taxon(self.work_dir + '/mann_boxfile.xls', self.work_dir + '/mann_boxfile_new.xls')
                    os.link(self.work_dir + '/mann_boxfile_new.xls', self.result_dir + '/mann_boxfile.xls')
                    ...
                    os.link(self.work_dir + '/bootstrap_CI.xls', self.result_dir + '/mann_CI.xls')
                ...
                try:
                    if self.option('meta_group_name'):
                        if not os.path.exists(self.output_dir + "/" + self.option('meta_group_name')):
                            os.makedirs(self.output_dir + "/" + self.option('meta_group_name'))
                        self.result_dir = self.output_dir + "/" + self.option('meta_group_name')
                        os.link(self.work_dir + '/kru_H_plot_group_bar.xls', self.result_dir + '/kru_H_plot_group_bar.xls')
                    else:
                        self.result_dir = self.output_dir
                    ...
                    os.link(self.work_dir + '/kru_H_result_new.xls', self.result_dir + '/kru_H_result.xls')
                    os.link(self.work_dir + '/kru_H_boxfile_new.xls', self.result_dir + '/kru_H_boxfile.xls')
                    ...
                    for i in filelist:
                        if re.match(r'^kru_H_%s' % self.option("kru_H_methor"), i):
                            self.add_taxon(self.work_dir + '/' + i, self.work_dir + '/new_' + i)
                            os.link(self.work_dir + '/new_' + i, self.result_dir + '/' + i)
                            # os.link(self.work_dir + '/' + i, self.output_dir + '/' + i)
                ...
                try:
                    ...
                    for i in filelist:
                        ...
                        if re.match(r'^est_barplot_result', i):
                            os.link(self.work_dir + '/' + i, self.output_dir + '/' + i)
                        # <<<
