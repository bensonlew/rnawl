...
class RegressionAgent(Agent):
    ...
    def check_options(self):
        ...
        if not self.option('taxon_table').is_set:
            raise OptionError('必须提供物种层次的表', code="34101001")
        if not self.option('func_table').is_set:
            raise OptionError('必须提供功能层次的表', code="34101002")
...
class RegressionTool(Tool):
    def __init__(self, config):
        ...
        self.script1_path = self.config.PACKAGE_DIR + "/statistical/regression_analysis.pl"      #guanqing 20180521 宏基因分析的脚本
        #self.script1_path = self.config.PACKAGE_DIR + "/statistical/regression_analysis_meta.pl"    # guanqing 20180521 多样性分析的脚本
        self.R_path = 'program/R-3.3.1/bin/Rscript'

    def run_regression(self):
        cmd = self.perl_path +' %s -i1 %s -i2 %s -c T -o %s ' % (                  # guanqing 20180521 
        self.script1_path, self.option('taxon_table').prop['path'],self.option('func_table').prop['path'], self.work_dir + '/Regression')
        ...
        if command.return_code == 0:
            ...
            self.set_error("Regression的r文件生成失败", code="34101001")
        ...
        if command1.return_code == 0:
            ...
            self.set_error("R程序计算Regression失败", code="34101002")
