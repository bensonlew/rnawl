...
class SwissprotAnnotAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {'name': 'swissprot_ids', 'type': 'infile', 'format': 'ref_rna_v2.common'},
...
class SwissprotAnnotTool(Tool):
    ...
    def run_idst2g(self):
        ...
        cmd += ' --tids {}'.format(self.option('swissprot_ids').path)
        cmd += ' --output_t {}'.format(self.txpt_ids)
        cmd += ' --output_g {}'.format(self.gene_ids)
        cmd += ' --output_format {}'.format("{swissprot}({description})")
        cmd_name = 'run_ids2g'
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            'type': 'tool',
            'name': 'ref_genome_db_medical.annotation.swissprot_annot',
            'instant': False,
            'options': {
                'swissprot_ids': '/mnt/ilustre/users/sanger-dev/sg-users/liubinxu/genome_annotation/swissprot_annot.tsv',
