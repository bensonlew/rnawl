...
class EggnogAnnotAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {'name': 'eggnog_ids', 'type': 'infile', 'format': 'ref_rna_v2.common'},
...
class EggnogAnnotTool(Tool):
    def __init__(self, config):
        ...
        self.cog_summary_py = os.path.join(self.config.PACKAGE_DIR, 'ref_genome_db_medical/annotation/cog_summary.py')
        self.get_venn_list_py = os.path.join(self.config.PACKAGE_DIR, 'ref_genome_db_medical/annotation/get_venn_list.py')
        self.prefix = "cog_annot"
        ...
        self.cog_tsv = os.path.join(self.output_dir, '{}.T.tsv'.format(self.option('database')))
    ...
    def run_xml2table(self):
        shutil.copy(self.option('eggnog_ids').path, self.txpt_xml)
    ...
    def run_cog_annotation(self):
        ...
        cmd += ' -i {}'.format(self.option('eggnog_ids').path)
    ...
    def set_output(self):
        ...
        self.option('cog_gene_table').set_path(self.cog_gene_tsv)
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            'type': 'tool',
            'name': 'ref_genome_db_medical.annotation.eggnog_annot',
            'instant': False,
            'options': {
                'eggnog_ids': '/mnt/ilustre/users/sanger-dev/sg-users/liubinxu/genome_annotation/eggnog_annot.tsv',
