...
class CatTableAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {"name": "tabledir", "type": "infile", "format": "ref_rna_v2.common_dir"},
            {"name": "tableout", "type": "outfile", "format": "ref_rna_v2.common"},
    ...
    def check_options(self):
        ...
        if not self.option("tabledir").is_set:
            raise OptionError("请传入表格输出结果文件 %s不存在", variables = (self.option("tabledir")), code="33700601")
        if not self.option("known"):
            ...
            if fill_num != fill_num:
                raise OptionError("known_format 和 known_fill数量不一致", code="33700603")
        else:
            pass
    ...
    def cat_files(self):
            ...
            if self.option("known_format") == "{}" and self.option("known_fill") == "all":
                ...
                with open(self.option('known'), 'rb') as r:
                    ...
                    for line in lines:
                        ...
                        if ";;" in cols[2]:
                            col_list = cols[2].split(";")
                            col_list_new = [col for col in col_list if col != ""]
                            cols[2] = ";".join(col_list_new)
                        seq_known_list.append(cols[0])
                        if (self.option('choose_known') and cols[1] in self.known_list) or (not self.option('choose_known')):
        ...
        if self.option("merge_type") == "all":
            for f in files:
                ...
                first_file = False
        if self.option("merge_type") == "partial":
            for f in files:
                with open(os.path.join(self.option('tabledir').prop['path'], f), 'rb') as r:
                    lines = r.readlines()
                    ...
                    if has_head:
                        ...
                        w.writelines([line for line in lines if line.split("\t")[0] not in seq_known_list])

                first_file = False

        elif self.option("merge_type") == "refonly":
            pass
