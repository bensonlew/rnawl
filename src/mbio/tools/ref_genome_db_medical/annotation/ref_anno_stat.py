...
from mbio.packages.ref_rna_v2.gene2trans import gene2trans
from mbio.packages.ref_rna_v2.nr_stat import nr_stat
...
class RefAnnoStatAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {"name": "nr_xml", "type": "infile", "format": "ref_rna_v2.blast_xml"},  # blast比对到nr库的xml结果文件
            {"name": "ncbi_taxon", "type": "infile", "format": "ref_rna_v2.nr_taxon"},
            {"name": "blast2go_annot", "type": "infile", "format": "ref_rna_v2.blast2go_annot"},
            {"name": "gos_list", "type": "infile", "format": "ref_rna_v2.go_list"},  # go注释tool的结果文件
            {"name": "gos_list_upload", "type": "infile", "format": "ref_rna_v2.anno_upload"},   # 客户上传go注释文件,用blast2go_annot、gos_list或gos_list_upload进行go注统计
            {"name": "kegg_xml", "type": "infile", "format": "ref_rna_v2.blast_xml"},
            {"name": "kos_list_upload", "type": "infile", "format": "ref_rna_v2.anno_upload"},  # 客户上传kegg注释文件,kegg_xml或kos_list_upload进行kegg注释统计
            ...
            {"name": "swissprot_xml", "type": "infile", "format": "ref_rna_v2.blast_xml"},
            {"name": "ref_genome_gtf", "type": "infile", "format": "ref_rna_v2.gtf"},  # 参考基因组gtf文件/新转录本gtf文件
            ...
            {"name": "kegg_anno_table", "type": "outfile", "format": "ref_rna_v2.kegg_table"},
    ...
    def check_options(self):
        ...
        if len(self.anno_database) < 1:
            raise OptionError('至少选择一种注释库', code = "33702601")
        if self.option("gene2trans").is_set:
            pass
        elif not self.option("ref_genome_gtf").is_set:
            raise OptionError("缺失参考基因组gtf文件", code = "33702602")
        else:
            pass

        for i in self.anno_database:
            if i not in ['nr', 'go', 'cog', 'pfam', 'kegg', 'swissprot']:
                raise OptionError('需要注释的数据库不在支持范围内[nr, go, cog, pfam, kegg, swissprot]:%s', variables = (i), code = "33702603")
            if i == 'go':
                if self.option('gos_list').is_set and self.option('blast2go_annot').is_set:
                    ...
                    raise OptionError('缺少go注释的输入文件', code = "33702604")
            if i == 'cog':
                if self.option('string_xml').is_set:
                    ...
                    raise OptionError('进行cog注释统计必须输入blast到string数据库的xml文件或table文件 或eggnog', code = "33702605")
                # if not self.option('cog_summary').is_set:
                #     raise OptionError('缺少cog注释的输入文件:cog_summary')
            if i == 'nr' and not self.option('nr_xml').is_set:
                raise OptionError('缺少nr注释的输入文件', code = "33702606")
            if i == "pfam" and not self.option('pfam_domain').is_set:
                raise OptionError('缺少pfam注释的输入文件', code = "33702607")
            if i == 'kegg':
                if self.option('kegg_xml').is_set:
                    ...
                    raise OptionError('缺少kegg注释的输入文件', code = "33702608")
            if i == 'swissprot' and not self.option('swissprot_xml').is_set:
                raise OptionError('缺少swissprot注释的输入文件', code = "33702609")
        if self.option('gene2trans').is_set:
            pass
        elif not self.option('gene_file').is_set:
            raise OptionError('缺少gene输入文件', code = "33702610")
        else:
            pass
...
class RefAnnoStatTool(Tool):
    ...
    def run_kegg_stat(self):
        ...
        if self.option("taxonomy"):
            ...
            taxonomy = None
        if self.option("kegg_xml").is_set:
            if self.option("known_ko") and os.path.isfile(self.option("known_ko")):
                known_ko_gene = self.get_known_ko_gene(self.option("known_ko"))
            else:
                known_ko_gene = "None"
            ...
            cmd = "{} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {}".format(self.python_path, self.kegg_path, self.r_path, self.map_path, self.gene_kegg_xml, None, kegg_table, pidpath, gene_pathway, pathway_table, layerfile, taxonomy, self.option("link_bgcolor"), self.option("png_bgcolor"), self.image_magick, self.html_path, known_ko_gene)
        else:
    ...
    def set_output(self):
        ...
        try:
            ...
            self.set_error("设置注释统计分析结果目录失败%s", variables = (e), code = "33702614")
