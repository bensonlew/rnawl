...
class UniprotAnnotAgent(Agent):
    '''
    last_modify: 2020.09.14
    '''
    def __init__(self, parent):
        super(UniprotAnnotAgent, self).__init__(parent)
        options = [
            {'name': 'uniprot_ids', 'type': 'infile', 'format': 'ref_rna_v2.common'},
            {'name': 'longest_t2g', 'type': 'infile', 'format': 'ref_rna_v2.common'},
            {'name': 'database', 'type': 'string', 'default': 'uniprot'},
    ...
    def step_start(self):
        self.step.uniprot_annot.start()
        self.step.update()

    def step_end(self):
        self.step.uniprot_annot.finish()
        self.step.update()
    ...
    def end(self):
        super(UniprotAnnotAgent, self).end()
...
class UniprotAnnotTool(Tool):
    ...
    def run(self):
        super(UniprotAnnotTool, self).run()
        self.run_idst2g()
    ...
    def run_idst2g(self):
        ...
        cmd += ' --tids {}'.format(self.option('uniprot_ids').path)
        cmd += ' --output_t {}'.format(self.txpt_ids)
        cmd += ' --output_g {}'.format(self.gene_ids)
        cmd += ' --output_format {}'.format("{uniprot}({description})")
        cmd_name = 'run_ids2g'
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            'id': 'uniprot_annot_{}_{}'.format(random.randint(1000, 10000), random.randint(1000, 10000)),
            'type': 'tool',
            'name': 'ref_genome_db_medical.annotation.uniprot_annot',
            'instant': False,
            'options': {
                'uniprot_ids': '/mnt/ilustre/users/sanger-dev/sg-users/liubinxu/genome_annotation/uniprot_annot.tsv',
