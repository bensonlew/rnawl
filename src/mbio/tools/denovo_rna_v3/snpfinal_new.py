...
class SnpfinalNewAgent(Agent):
    """
    对无参转录组序列进行SSR的预测，以及引物设计,序列位置分布预测。
    检查tool的详细的错误可以去tool的结果里面一个err的文件查看
    """
    def __init__(self, parent):
        super(SnpfinalNewAgent, self).__init__(parent)
        """
        ...
        options = [
            ...
            {"name": "snp_rewrite", "type": "infile", "format": "denovo_rna_v2.common"},
            {"name": "indel_rewrite", "type": "infile", "format": "denovo_rna_v2.common"},
    ...
    def end(self):
        ...
        super(SnpfinalNewAgent, self).end()


class SnpfinalNewTool(Tool):
    def __init__(self, config):
        super(SnpfinalNewTool, self).__init__(config)
                ...
                for line in r:
                    ...
                    for sample in line.split("\t")[9:]:
                        ...
                        if gt.split("/") == ['0', '0']:
                            ...
                            w.write("\t".join([",".join([alt_infos[2], alt_infos[2]]), dp, ad, "Homo"]) + "\t")
                        else:
                            w.write("\t".join(["-", "-", "-" ,"-"]) + "\t")
    ...
    def snpdepth(self, s4):
        self.pd_snp = pd.read_table(s4, header=0, sep="\t")
        self.logger.info("wandanle")
        with open("sb","w") as a:
            a.write("duwanle")
        snp_col_len = self.pd_snp.shape[1]
        select_col = range(5, snp_col_len, 4)
        dp_selected = self.pd_snp.iloc[:, select_col].replace([r'-'], -1)
        ...
        snp_selected = self.pd_snp.iloc[:, snp_select_col]
        self.samples = list(snp_selected.columns)
        detail_dfs = []
        for i in list(snp_selected.columns):
            sample_df = self.pd_snp[["REF",i,i + "_depth"]]
            sample_depth_df = dask.delayed(self.extract_depth)(sample_df,i)
            detail_dfs.append(sample_depth_df)
        else:
            detail_dfs = dask.compute(*detail_dfs)
            outstat = os.path.join(self.output_dir,"depth")
            stat_dfs = [df] + list(detail_dfs)
            stat_info = pd.concat(stat_dfs, axis = 1)
            stat_info.to_csv(outstat,sep="\t")
        #
        # for i in list(snp_selected.columns):
        ...
        #     pd.concat([df, df[i]])
            stat_info.to_csv(self.work_dir + "/depth",sep = "\t")
            tt_per = stat_info.iloc[:,1:]/stat_info.iloc[:,1:].sum()
            tt_per.columns = [x + "_per" for x in tt_per.columns]
            tt_new_per = pd.concat([stat_info, tt_per], axis = 1)
            tt_new_per = tt_new_per.round(6)
            tt_new_per.to_csv(self.work_dir + "/depth_new_per",sep = "\t", index=False)


    def extract_depth(self,pd_snp,sample_name):
        self.logger.info("lalalalalala")
        self.logger.info("lalalalalala{}".format(sample_name))
        # sample_info = pd_snp[["REF",sample_name,sample_name+"_depth"]]
        sample_info = pd_snp
        sample_info[sample_name + "_anno"] = sample_info.apply(lambda x: "snp" if
            x[sample_name] != "-" and x[sample_name] != x.REF else "no", axis=1)
        num30 = sample_info[(sample_info[sample_name + "_anno"] == "snp") & (
            sample_info[sample_name+"_depth"].replace([r'-'], -1).astype('int64').between(0, 30, inclusive=True))].shape[0]
        num31_100 = sample_info[(sample_info[sample_name + "_anno"] == "snp") & (
            sample_info[sample_name+"_depth"].replace([r'-'], -1).astype('int64').between(31, 100, inclusive=True))].shape[0]
        num101_200 = sample_info[(sample_info[sample_name + "_anno"] == "snp") & (
            sample_info[sample_name+"_depth"].replace([r'-'], -1).astype('int64').between(101, 200, inclusive=True))].shape[0]
        num201_300 = sample_info[(sample_info[sample_name + "_anno"] == "snp") & (
            sample_info[sample_name+"_depth"].replace([r'-'], -1).astype('int64').between(201, 300, inclusive=True))].shape[0]
        num301_400 = sample_info[(sample_info[sample_name + "_anno"] == "snp") & (
            sample_info[sample_name+"_depth"].replace([r'-'], -1).astype('int64').between(301, 400, inclusive=True))].shape[0]
        num401_500 = sample_info[(sample_info[sample_name + "_anno"] == "snp") & (
            sample_info[sample_name+"_depth"].replace([r'-'], -1).astype('int64').between(401, 500, inclusive=True))].shape[0]
        num501 = sample_info[(sample_info[sample_name + "_anno"] == "snp") & (
                    (sample_info[sample_name+"_depth"].replace([r'-'], -1).astype('int64')) >= 501)].shape[0]
        df = pd.DataFrame()
        df[sample_name+"_depth"] = [num30, num31_100, num101_200, num201_300, num301_400, num401_500, num501]
        self.logger.info("xixi")
        return df
    ...
    def run(self):
        super(SnpfinalNewTool, self).run()
        self.snpdepth(self.option("snp_rewrite").prop["path"])
        # self.bcffilter()
        '''
        self.splitvcf()
        self.bedpre()
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "SnpfinalNew" + str(random.randint(1, 10000)),
            "type": "tool",
            "name": "denovo_rna_v3.snpfinal_new",
            "instant": False,
            "options": dict(
                # method="samtools",
                # call_vcf="/mnt/ilustre/users/sanger-dev/workspace/20190929/Snp_tsg_35544_6195_513/Snp/VcfFilterSamtools/output/final.vcf",
                # # call_vcf="/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/test/denovo_snp/final_snp/SnpfinalNew_new/output/filter_vcf",
                ...
                # bamlist=9,
                # # anno="/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/test/denovo_snp/final_snp/SnpfinalNew_new/gene_anno_detail.xls"
                ...
                snp_rewrite ="/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/test/test1016/Snpfinal__1/snp_rewrite",
                indel_rewrite ="/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/test/test1016/Snpfinal__1/indel_rewrite"
            )
        }
