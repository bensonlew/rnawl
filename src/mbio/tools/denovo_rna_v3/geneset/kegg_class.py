...
class KeggClassTool(Tool):
    ...
    def get_kegg_pics(self):
        ...
        self.mongo_db = Config().get_mongo_client(mtype="denovo_rna_v2")[Config().get_mongo_dbname("denovo_rna_v2")]
    ...
    def get_kegg_png(self):
        ...
        result = geneset_collection.find_one({"main_id": ObjectId(geneset_id)})
        if not result:
            self.set_error('geneset with main_id: %s was not found', variables=(geneset_id), code="32004802")
        task_id = result["task_id"]
        ...
        if geneset_type == 'G':
            kegg_pathways = os.path.join(kegg_annot_output,'kegg','kegg_pathway_gene_dir')
        else:
            kegg_pathways = os.path.join(kegg_annot_output, 'kegg', 'kegg_pathway_tran_dir')
        if not kegg_pathways.endswith('/'):
            kegg_pathways += '/'
        anno_path = os.path.join(self.work_dir, "png/")
        self.logger.info("download from {} to {}".format(kegg_pathways,  anno_path))
        self.download_s3_file(kegg_pathways, anno_path)

    def get_dicts(self):
        gene_kegg_gene = dict()
        if self.species_abr == 'map':
            ko_genes, path_ko, _ = self.option('kegg_table').get_pathway_koid2(self.option('kegg_table2'), kegg_version=self.option('kegg_version'))
        else:
            #区分比对到整个ko与单个物种的ko时有区别(有参无参没有用过）
    ...
    def generate_new_pics(self):
        ...
        for path in path_list:
            ...
            if os.path.exists(ko_path):
                ...
                map_html.color_bg = ["#00CD00", "#FFFF00"]
                try:
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "Kegg_class" + str(random.randint(1, 10000)),
            "type": "tool",
            "name": "denovo_rna_v3.geneset.kegg_class",
            "instant": False,
            "options": dict(
                ...
                background_links="/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/test/geneset_kegg/add_info.txt",
                type="origin",
                source="diff_exp"
            )
           }
