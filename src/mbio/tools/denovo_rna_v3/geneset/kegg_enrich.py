...
class KeggEnrichAgent(Agent):
    ...
    def __init__(self, parent):
        super(KeggEnrichAgent, self).__init__(parent)
        options = [
            {"name": "kegg_table", "type": "infile", "format": "denovo_rna_v3.kegg_table"},
    ...
    def check_options(self):
        ...
        if not self.option('kegg_table').is_set:
            raise OptionError('必须设置kegg的pathway输入文件', code = "32004901")
        if self.option('correct').lower() not in ['by', 'bh', 'bonferroni', 'holm']:
            raise OptionError('多重检验校正的方法不在提供的范围内', code = "32004902")
        if not self.option("diff_list").is_set:
            raise OptionError("必须设置输入文件diff_list", code = "32004903")
        return True
    ...
    def end(self):
        ...
        super(KeggEnrichAgent, self).end()


class KeggEnrichTool(Tool):
    def __init__(self, config):
        super(KeggEnrichTool, self).__init__(config)
        ...
        self.script_path = self.config.PACKAGE_DIR + "/ref_rna_v2/"
    ...
    def run_kegg_rich(self):
        ...
        try:
            ...
            self.option("kegg_table").get_gene2path2(self.work_dir,self.option("kegg_table2"))
            ...
            self.set_error("kegg富集第一步运行出错:%s", variables = (e), code = "32004904")
    ...
    def run_identify(self):
        ...
        if command_2.return_code == 0:
            ...
            self.set_error("kegg富集分析运行出错!", code = "32004905")
    ...
    def set_output(self, linkfile):
        ...
        try:
            ...
            self.set_error("设置kegg富集分析结果目录失败%s", variables = (e), code = "32004906")
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "Kegg_enrich" + str(random.randint(1, 10000)),
            "type": "tool",
            "name": "denovo_rna_v3.geneset.kegg_enrich",
            "instant": False,
            "options": dict(
                # geneset_kegg='/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/test/geneset_kegg/multi_geneset_list',
                correct="BH",
                # scm="complete",
                # scd="correlation",
                # corr_method='pearson',
                # task_id="denovo_rna_v2_upgrade",
                kegg_table="/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/test/geneset_kegg/gene_kegg_table.xls",
                kegg_table2="/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/test/geneset_kegg/gene_kegg_level_table.xls",
                #geneset_id="5d54d55ac6598d28168b4567",
                add_info="/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/test/geneset_kegg/add_info.txt",
                diff_list="/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/test/geneset_kegg/gene.list",
                #type="origin",
                # source="diff_exp"
            )
           }
