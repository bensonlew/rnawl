...
class BamAddrgTool(Tool):
    ...
    def addorreplacereadgroups_picard(self,bamfile):
        self.sample_name = os.path.basename(self.option("in_bam").prop["path"])[:-4]
        self.logger.info(self.sample_name)
        cmd = "program/sun_jdk1.8.0/bin/java -Djava.io.tmpdir={} -jar {}picard.jar AddOrReplaceReadGroups I={} O={} MAX_RECORDS_IN_RAM=1000000 SO=coordinate LB=HG19 PL=illumina PU=HG19 VALIDATION_STRINGENCY=SILENT SM={}".format(self.tmp_path, self.picard_path, bamfile, "add.bam", self.sample_name)
