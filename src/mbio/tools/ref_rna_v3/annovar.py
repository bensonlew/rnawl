...
class AnnovarTool(Tool):
    ...
    def __init__(self, config):
        ...
        self.gtfToGenePred_path = "/bioinfo/align/ucsc_tools/"
    ...
    def gtf_to_genepred(self):
        ...
        if ret:
            cmd = "{}gtfToGenePred -genePredExt -allErrors {} {}.genes_refGene.tmp.txt".format(
                self.gtfToGenePred_path, self.ref_gtf, self.option("ref_genome")
            )
        else:
            cmd = "{}gtfToGenePred -allErrors {} {}.genes_refGene.tmp.txt".format(
                self.gtfToGenePred_path, self.ref_gtf, self.option("ref_genome")
            )
    ...
    def convert2annovar(self):
        ...
        cmd = "{}perl {}convert2annovar.pl -allallele -format vcf4old {} > clean.snp.avinput" \
            .format(self.perl_full_path, self.annovar_path, self.vcf_path)
    ...
    def set_output(self):
        ...
        snp_df = pd.read_table(self.work_dir + "/snp_anno.xls")
        snp_df1 = snp_df[(snp_df["ANNO"] != ".") & (snp_df["GENE(in or nearby)"] != ".")  &  (snp_df["type"] == "snp") ]
        indel_df = snp_df[snp_df["type"] == "indel"]
        final_df = pd.concat([snp_df1, indel_df])
        final_df.to_csv(self.work_dir + "/snp_anno.xls",sep="\t",index=False)
    ...
    def run(self):
        ...
        self.add_name_des(self.output_dir + "/snp_anno.xls", self.work_dir + "/id_name_desc.txt")
    ...
    def test(self):
        ...
        data = {
            ...
            "options": dict(
                ...
                ref_fasta = "/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/CallSnpIndelSentieon/Homo_sapiens.GRCh38.dna.toplevel.fa",
                ref_gtf ="/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/RefrnaAssemble/output/NewTranscripts/ref_and_new.gtf",
                ref_genome="customer_mode",
                combine_vcf=False,
                ...
                des_type="type1",
                input_file="/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/CallSnpIndelSentieon/VcfFilterGatk/output/final.vcf"
            )
           }
