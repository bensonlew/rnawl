...
class Pigzfastq2fastqAgent(Agent):
    def __init__(self, parent):
        ...
        options = [
            ...
            {'name': 'fastq_info', 'type': 'string', 'default': ''}
        ]
...
class Pigzfastq2fastqTool(Tool):
    ...
    def gzfastq2fastq(self):
        with open(self.option("fastq_info"), 'r') as list_r, open('ungz.bash', 'w') as ungz_w:
            ...
            for line in list_info.split('\n'):
                ...
                if line[0].split(".")[-1] in ["gz"]:
                    ...
                    ungz_w.write('pigz -c -p 8 -k -d ' + self.option('fastq_path') + '/' + line[0] + ' > ' + self.option('fastq_path') + '/' + gzed + '\n')
                else:
                    if line:
        ...
        with open(self.option("fastq_path") + '/list.txt', 'r') as list_r, open('ungz.bash', 'w') as ungz_w:
            ...
            for line in list_info.split('\n'):
                ...
                if line[0].split(".")[-1] in ["gz"]:
                    gzed = ".".join(line[0].split(".")[:-2]) + ".fastq"
                    list_info_new += gzed + '\t' + '\t'.join(line[1:]) + '\n'
                    ungz_w.write('{}pigz -c -p 8 -k -d '.format(self.config.SOFTWARE_DIR + self.pigz) + self.option(
                        'fastq_path') + '/' + line[0] + ' > ' + self.option('fastq_path') + '/' + gzed + '\n')
