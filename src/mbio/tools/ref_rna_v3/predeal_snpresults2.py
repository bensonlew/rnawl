# -*- coding: utf-8 -*-
# __author__ = 'XuXi ，improve code from HONGDONG'
# last modify 20201112
...
class PredealSnpresults2Agent(Agent):
    ...
    def set_resource(self):
        ...
        """
        self._cpu = 20
        memory = int(float(self.infile_size) / 1024 ** 3 * 48 + 16)
        if memory > 500:
            memory = 500
...
class PredealSnpresults2Tool(Tool):
        ...
        if self.option('method') == 'samtools':
            with open(snp_detail,'r') as f:
                ...
                if "bam" in column_line:
                    cmd = "sed -i \'1s/bam\///\' {}".format(snp_detail)
                    os.system(cmd)
    ...
    # def rearrange(snp_detail,new_output + "/data_anno_pre.xls"):
        ...
        # 读取dataframe时，指定数据类型，从而处理大文件会节省内存
        column_types = {}
        ...
        snp_detail_df = pd.read_table(snp_detail,sep='\t',dtype=column_types)
        ...
        with open(snp_anno,'r') as f:
            column_line_y = f.readline().strip().split('\t')
        column_types_y = {}
        ...
        df_anno_pre2 = pd.read_table(snp_anno, header=0, sep="\t", dtype=column_types_y, usecols=column_line_y[:13])
    ...
    def run(self):
        ...
        self.deal_snp_detail(self.option('snp_detail'), self.option('snp_anno'), new_output=self.output_dir)
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            "type": "tool",
            "name": "ref_rna_v3.predeal_snpresults2",
            "instant": False,
            "options": dict(
                ...
                snp_detail="/mnt/ilustre/users/isanger/workspace/20210315/Refrna_n34u_49qekvdhgok1ed33c1m5hf/CallSnpIndelSentieon/snp_anno_re.xls",
                snp_anno="/mnt/ilustre/users/isanger/workspace/20210315/Refrna_n34u_49qekvdhgok1ed33c1m5hf/CallSnpIndelSentieon/snp_annotation_re.xls",
                method="gatk"
            )
           }
