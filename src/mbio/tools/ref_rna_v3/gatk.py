...
class GatkAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "sample_bam", "type": "infile", "format": "align.bwa.bam"},  # bam文件类型，输入
...
The reference sequence can be gzipped (both .fasta and .fasta.gz are supported).
    ...
    def gatk_split(self, ref_fasta, bam):
        ...
        command = self.add_command("splitncigarreads", cmd,ignore_error=True)
        command.run()
        self.wait()
        if command.return_code == 0:
    ...
    def gatk_vc(self, ref_fasta, split_bam):
        ...
        cmd = "{}java -jar {}GenomeAnalysisTK.jar -T HaplotypeCaller -R {} " \
              "-I {} -o {}  -nct 8 --genotyping_mode DISCOVERY --emitRefConfidence GVCF ".format(self.java_path, self.gatk_path, ref_fasta, split_bam, "{}.g.vcf".format(self.option("sample_id")))
    ...
    def run(self):
        ...
        if self.option("ref_genome") == "customer_mode" and self.option("ref_fa").is_set:
            ...
            self.gatk_split(ref_current, self.option("sample_bam").prop["path"])
            ...
            with open(ref_genome_json, "r") as f:
                ...
                ref_current = ref
            self.gatk_split(ref, self.option("sample_bam").prop["path"])
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "gatk_test" + str(random.randint(1, 10000))+"yyyyy",
            "type": "tool",
            "name": "ref_rna_v3.gatk",
            "instant": False,
            "options": dict(
                ...
                sample_bam="/mnt/ilustre/users/sanger-dev/workspace/20190712/Snp_tsg_34825_8321_5445/bam_folder/A1_1.bam",
                sample_id="A1_1",
