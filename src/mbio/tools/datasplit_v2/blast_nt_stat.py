...
class BlastNtStatAgent(Agent):
    ...
    def end(self):
        super(BlastNtStatAgent, self).end()
...
class BlastNtStatTool(Tool):
    ...
    def species_stat(self):
        ...
        stat_path = os.path.join(self.output_dir, "nt_species_stat.xls")
        if os.path.exists(stat_path):
            os.remove(stat_path)
        os.link(os.path.join(self.work_dir, "nt_species_stat.xls"), stat_path)

    def run(self):
        super(BlastNtStatTool, self).run()
        self.xml_to_table()
        self.species_stat()
        self.end()
