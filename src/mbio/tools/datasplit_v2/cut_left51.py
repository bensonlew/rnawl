...
class CutLeft51Agent(Agent):
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "cut_three", "type": "bool", "default": True},  # 是否删除前三个碱基
            {"name": "length", "type": "string", "default": "75"},  # 保留碱基数
...
class CutLeft51Tool(Tool):
    ...
    def run_single_cut_left51(self):
        ...
        start = 0
        if self.option("cut_three"):
            start = 3
        cmd = "{} {} -i {} -o {} -s {} -l {}".format(self.python, self.cut_left, fastq_file, outfile, start, self.option("length"))
    ...
    def run_pariend_cut_left51(self):
        ...
        cmd = "{} {} -i {} -o {} -s {} -l {}".format(self.python, self.cut_left, fastq_file, outfile_l, start, self.option("length"))
        ...
        cmd = "{} {} -i {} -o {} -s {} -l {}".format(self.python, self.cut_left, fastq_file, outfile_r, start, self.option("length"))
    ...
    def file_gz(self):
        ...
        for f in os.listdir(self.output_dir):
            old = os.path.join(self.output_dir, f)
            new = os.path.join(self.output_dir, f+".gz")
