...
class ExtractDenovoAgent(Agent):
    def __init__(self, parent):
        ...
        options = [
            {"name": "pep", "type": "infile", "format": "labelfree.common"},  # 输入文件
            {"name": "protein_faa", "type": "infile", "format": "labelfree.common"},  # 输入文件
            {"name": "outlist", "type": "outfile", "format": "labelfree.common"},  # 输出的有关联关系的list文件
            ]
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        import datetime
        test_dir='/mnt/ilustre/users/sanger-dev/sg-users/fengyitong/protein_transcript_labelfree'
        data = {
            "id": "Extract_gff_" + datetime.datetime.now().strftime('%H-%M-%S'),
            "type": "tool",
            "name": "protein_transcript_labelfree.extract_denovo",
            "instant": False,
            "options": dict(
