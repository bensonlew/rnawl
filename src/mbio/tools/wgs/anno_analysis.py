...
class AnnoAnalysisAgent(Agent):
    ...
    def check_options(self):
        ...
        if not self.option('anno_stat'):
            raise OptionError('必须输入各注释统计表', code="34500101")
        if not self.option('anno_type'):
            raise OptionError('必须输入注释类型', code="34500102")
        if self.option('anno_type') not in ["kegg", "go", "eggnog"]:
            raise OptionError('注释类型只能是"kegg,go,eggnog"中的一种', code="34500103")
        return True
...
class AnnoAnalysisTool(Tool):
    ...
    def run_anno_analysis(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("anno_analysis运行出错!", code="34500101")
    ...
    def get_picture(self):
        ...
        cmd_list = []
        if not os.path.isdir(self.output_dir + '/pathway_dir'):
        ...
        with open(self.option('anno_stat').prop['path'])as fr:
            ...
            for line in data:
                ...
                ko_png = tmp[0].strip("\"") + '.png'
                if ko_png in all_ko:
                    ko_path = os.path.join(self.ko_path, ko_png)
                    if not os.path.exists(self.output_dir + '/pathway_dir/' + ko_png):
                    ...
                    self.logger.info("ko{} png文件在本地数据库{}中没有，请核实!".format(tmp[0],self.ko_path))
        ...
        if cmd_obj.return_code == 0:
            ...
            self.set_error("cmd list运行出错!", code="34500102")
    ...
    def set_output(self):
        ...
        try:
            base_name = os.path.basename(self.option('anno_stat').prop['path']).split('.stat')[0]
            if os.path.exists(self.output_dir + '/' + base_name + '.final.stat.detail'):
                os.remove(self.output_dir + '/' + base_name + '.final.stat.detail')
            os.link(self.work_dir + '/' + base_name + '.final.stat.detail',
                    self.output_dir + '/' + base_name + '.final.stat.detail')
            self.logger.info("设置anno_analysis分析结果目录成功")

        except Exception as e:
            self.logger.info("设置anno_analysis分析结果目录失败{}".format(e))
            self.set_error("设置anno_analysis分析结果目录失败%s", variables=(e), code="34500103")
    ...
    def direct_output(self, anno_type):   # add by wzy 20180329
        ...
        try:
            ...
            self.logger.info("设置anno_analysis分析结果目录失败{}".format(e))
            self.set_error("设置anno_analysis分析结果目录失败%s", variables=(e), code="34500104" )