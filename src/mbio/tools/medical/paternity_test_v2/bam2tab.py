...
class Bam2tabTool(Tool):
    ...
    def bam2vcf(self):
        ...
        full_path = self.script_absolute_path + '/' + sample_id + '_bam2vcf_pt.sh' + now_time
        path = self.script_path + '/' + sample_id + '_bam2vcf_pt.sh' + now_time
        cmd_pool = list()
        with open(full_path, 'w') as out:
            header_line = '#!/bin/bash'

            cmd1 = 'samtools mpileup -uvf {} -l {} {} -o {}.bcf'.format(ref, bed, bam_path, sample_id)
    ...
    def deal_with_vcf(self):
        ...
        if not os.path.exists(self.script_absolute_path):
            os.mkdir(self.script_absolute_path)
        now_time = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
        full_path = self.script_absolute_path + '/' + sample_id + '_vcf2tab_pt.sh' + now_time
        path = self.script_path + '/' + sample_id + '_vcf2tab_pt.sh' + now_time
        cmd_pool = list()
        with open(full_path, 'w') as out:
            header_line = '#!/bin/bash'
    ...
    def qc(self):
        ...
        cmd = '{}/pt_qc.sh {} {} {} {}'.format(self.cmd_path, cpu, sample_id, mem_sort_bam, mem_sort_hit_bam)
    ...
    def set_output(self):
        ...
        if os.path.getsize(tab_path) > 0:
            ...
            table_name = 'sg_sample_tab' if float(qc['dp']) > 10 else 'sg_sample_tab_problem'
            if db.check_existence(table_name, {'sample_id': sample_id}):
                ...
                self.logger.info("tab表中已有该样品：{}".format(sample_id))

            line_count = db.add_tab(tab_path, table_name)

            if table_name == 'sg_sample_tab':
                db.update_sample_tab(sample_id, self.option('board_batch'), self.option('batch_id'), 'pt')

            if db.check_existence('sg_sample_qc', {'sample_id': sample_id}):
                # self.set_error('可能样本{}重名，请检查！'.format(sample_id))
                # raise Exception('可能样本{}重名，请检查！'.format(sample_id))
                self.logger.info('可能样本{}重名，请检查！'.format(sample_id))
            db.pt_qc(qc_path, sample_id, line_count, self.option('board_batch'))
        else:
            db.pt_qc(qc_path, sample_id, line_count, self.option('board_batch'), False)
        db.update_analysis_status(self.option('batch_id'), 'snp', self.option("is_update"))
