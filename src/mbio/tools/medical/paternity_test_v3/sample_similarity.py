...
class SampleSimilarityTool(Tool):
    ...
    def get_new_sample(self):
        ...
        ref_set = set(self.api_pt.get_ref_list(month_back=10))                                     # 4个月内的所有样品
        tab_exists = set([i.split(".")[0] for i in os.listdir(self.library_path)])    # 库中存在的所有样品
        valid_set = ref_set & tab_exists
        merge_list = [i for i in valid_set if "-F" in i or "-M" in i]
        merge_list = [i for i in merge_list if "_" not in i]
