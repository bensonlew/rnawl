...
class FamilyMergeHomohybridAgent(Agent):
    """
    tab2family
    将已知的tab文件合并转化为整个家系在一起的表格--该tool筛选出来的结果包含了胎儿的纯合与杂合所有位点
    包含family_joined_update.R
    version v1.0
    author: hongdong
    last_modify: 20191108
    """
    def __init__(self, parent):
        super(FamilyMergeHomohybridAgent, self).__init__(parent)
        options = [
    ...
    def end(self):
        ...
        super(FamilyMergeHomohybridAgent, self).end()


class FamilyMergeHomohybridTool(Tool):
    def __init__(self, config):
        super(FamilyMergeHomohybridTool, self).__init__(config)
        self._version = '1.0.1'
    ...
    def run_tf(self):
        tab2family_cmd = "{}Rscript {}family_joined_update.R {} {} {} {} {} {}".\
    ...
    def set_output(self):
        ...
        for f in results:
            if re.search(r'.*family_joined_tab_update\.Rdata$', f):
                os.link(self.work_dir + '/' + f, self.output_dir + '/' + f)
            if re.search(r'.*family_joined_tab_update\.txt$', f):
                os.link(self.work_dir + '/' + f, self.output_dir + '/' + f)
        self.logger.info('设置文件夹路径成功')

    def run(self):
        super(FamilyMergeHomohybridTool, self).run()
        self.run_tf()
        self.set_output()
        self.end()
