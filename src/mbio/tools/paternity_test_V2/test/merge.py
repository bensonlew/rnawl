...
"""
这个程序是为了合并两个脚本而写的
python -u xx -d xx -p xx -t 20171017
...
dbconfigure={'host':'172.16.101.202','name':'mjlimsbak','passwd':'Q6k9pU9ZN5FTow','database':'mjlims'}
libraryType = {"NIPT文库构建实验流程":"WS", 
							"WQ gDNA建库实验流程_多重":"WQ", 
...
def main(Fileup,Filedown,Filedate,Filebatch):
	test_init = MergeMatrix(Fileup,Filedown,Filedate,Filebatch)
	test_init.mergeupdown()
	test_init.Checkbarcode()
	test_init.NamePattern()
	#pdb.set_trace()
	#test_init.CheckSqlLocal()
	test_init.Checkemergency()
	test_init.GenerateSplitTable()
	test_init.CheckSqlLIMS()
...
class MergeMatrix:
	def __init__(self,Fileup,Filedown,Filedate,Filebatch):
		self.up = Fileup
		self.down = Filedown
		self.date = Filedate
		self.batch = Filebatch
...
if __name__ == "__main__":
	...
	parser.add_option("-u","--up_matrix", default = "empty",action="store",type="string",dest="up",help="matrix from LIMS")
	parser.add_option("-d","--down_matrix",default="empty",action="store",type="string",dest="down",help="matrix from mail") 
	parser.add_option("-t","--time_date",default="empty",action="store",type="string",dest="date",help="date,use for name all files")
	parser.add_option("-p","--experiment_batch",default="empty",action="store",type="string",dest="batch",help="batch table, use for emergency")
	(options, args) = parser.parse_args()

	if options.up == "empty":
		parser.error("-u is not right.there must be a matrix from LIMS")

	if options.batch == "empty":
		parser.error("-p is not set. there must be a batch table")

	if options.date == "empty":
		print("-t is not set.date is use for naming all files")

	if options.up and options.down != "empty":
		print("-u and -d is set: now there are two matrix input")

	if options.up and options.down == "empty":
		print("-u is set but -d is not set, now use matrix from LIMS only")

	DicConvertUTF8(libraryType)
	DicConvertUTF8(sampletype)
	DicConvertUTF8(analysistype)

	main(options.up,options.down,options.date,options.batch)
