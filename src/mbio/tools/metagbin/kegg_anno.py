...
class KeggAnnoAgent(Agent):
    ...
    mkdify by qingchen.zhang 兼容binning的注释不同版本的数据库
    """
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "database", "type": "string", 'default': "kegg_v94.2"}, ## 兼容不同的版本
        ]
...
class KeggAnnoTool(Tool):
    def __init__(self, config):
        ...
        if self.option("database") in ['kegg_v94.2']:
            ...
            self.python_script = self.config.PACKAGE_DIR + '/metagbin/kegg_xlm_mongo.py'
            self.python_script2 = self.config.PACKAGE_DIR + '/annotation/mg_annotation/kegg_pathway_img.py'
    ...
    def run_pathway_img(self):
        ...
        if self.option('produce_mark'):
            if self.option("database") in ['kegg_v94.2']:
                ...
                cmd += ' -png_file True -html {}/database/KEGG/map_html/'.format(self.config.SOFTWARE_DIR)
        self.logger.info(cmd)
