...
class PanPgapTool(Tool):
    ...
    def run_pagp(self):
        ...
        os.mkdir(self.out)
        cmd = "{} {} --strains {} --cluster --input {} --output {} --cluster --identity {}  --thread 16 --coverage {}".format(self.perl_path, self.perl_script, self.strains, self.input_dir, self.out, self.option("identity"), self.option("coverage"))
