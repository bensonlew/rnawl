...
class AlignTrimlAgent(Agent):
    """
    序列进行比对对齐和掩盖低置信度的位置处理
    """

    def __init__(self, parent):
        super(AlignTrimlAgent, self).__init__(parent)
        options = [
    ...
    def end(self):
        super(AlignTrimlAgent, self).end()


class AlignTrimlTool(Tool):
    def __init__(self, config):
        super(AlignTrimlTool, self).__init__(config)
        self._version = "1.0"
    ...
    def set_output(self):
        ...
        self.option("out", self.output_dir + "/all.align_last.fna")
