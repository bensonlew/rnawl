...
class DiffDoClassAgent(Agent):
    '''
    last_modify: 2019.04.16
    '''
    def __init__(self, parent):
        super(DiffDoClassAgent, self).__init__(parent)
        options = [
    ...
    def end(self):
        super(DiffDoClassAgent, self).end()
...
class DiffDoClassTool(Tool):
    ...
    def run(self):
        super(DiffDoClassTool, self).run()
    ...
    def set_output(self):
        ...
        if len(self.option("geneset_names").split(",")) == 1:
            ...
            table1.to_csv(self.output_dir + "/do_level2_class.xls", sep="\t", index=False)


        elif len(self.option("geneset_names").split(",")) == 2:
            ## merge 2 class table
            [name1,name2] = self.option("geneset_names").split(",")
            ...
            a = pd.merge(table1, table2, how='outer')
            a[name1 + " numbers"]=a[name1 + " numbers"].fillna(0).map(int)
            a[name2 + " numbers"]=a[name2 + " numbers"].fillna(0).map(int)
            a[name1 + " percent"] =  a[name1 + " percent"].fillna(0)
            a[name2 + " percent"] =  a[name2 + " percent"].fillna(0)
            a[name2 + " seqs"] =  a[name2 + " seqs"].fillna("")
            a[name1 + " seqs"] =  a[name1 + " seqs"].fillna("")
            b = a.sort_values(["Term Type", "DO Name"])
