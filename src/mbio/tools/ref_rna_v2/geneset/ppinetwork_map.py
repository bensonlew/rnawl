...
class PpinetworkMapAgent(Agent):
    ...
    def check_options(self):
        ...
        if not self.option("diff_exp_gene").is_set:
            raise OptionError("必须输入含有gene_id的差异基因列表！", code = "33706701")
...
class PpinetworkMapTool(Tool):
    ...
    def __init__(self, config):
        ...
        self.script_path = os.path.join(Config().PACKAGE_DIR, "itraq_and_tmt/new_map.r")
    ...
    def set_output(self):
        ...
        #add by fwy 20211115  部分项目id为小写,经过分析后会变成大写,导致一系列报错问题
        try:
            self.logger.info('开始进行id回溯')
            all_genes = [i.strip() for i in open(self.option("diff_exp_gene").prop["path"]).readlines()[1:]]
            gene_upper2raw_dict = dict(zip([i.upper() for i in all_genes], all_genes))
            mapped_result = os.path.join(self.work_dir,"PPI_result","seq_mapped.txt")
            a = pd.read_table(mapped_result)
            # all_inter_genes = set(all_genes) & set(a["accession_id"])
            # self.logger.info('一共{}id需要回溯'.format(str(len(all_inter_genes))))
            a["accession_id"] = a["accession_id"].apply(
                lambda x: gene_upper2raw_dict[x] if x in gene_upper2raw_dict else x)
            a.to_csv(os.path.join(self.output_dir,"seq_mapped.txt"),sep="\t",index=False)
            self.logger.info('id回溯成功')
        except:
            self.logger.info('id回溯失败')
            pass
