...
class PpinetworkPredictAgent(Agent):
    ...
    def check_options(self):
        ...
        if not self.option('diff_exp_mapped'):
            raise OptionError("必须输入含有STRINGid的差异基因表", code = "33706901")
        line = open(self.option('diff_exp_mapped'), "r").readlines()[1:]
        if not line:
            raise OptionError("基因集中的基因不能匹配到string数据库中id，请您确认基因id为Ensemble或者Entrez GeneID！", code = "33706902")
        if not isinstance(self.option('combine_score'), int) or self.option('combine_score') < 0:
            raise OptionError("combined_score值必须是大于0的整数！", code = "33706903")
    ...
    def set_resource(self):
        ...
        self._memory = '40G'
        # 特殊物种需要内存高
        if self.option('species') in [4565]:
            self._memory = '120G'
...
class PpinetworkPredictTool(Tool):
    ...
    def __init__(self, config):
        ...
        self.script_path =  self.config.PACKAGE_DIR + "/ref_rna_v2/new_PPInetwork_predict.r"
    ...
    def run_PPI(self):
        one_cmd = self.r_path + " %s %s %s %s %s %s %s" % (
            self.script_path, self.option('diff_exp_mapped'), self.option('species'),
            'PPI_result', self.option('combine_score'), self.database, self.option('score'))
        ...
        if cmd.return_code == 0:
            ...
            self.set_error("运行one_cmd出错", code = "33706904")
