...
class KeggClassTool(Tool):
    def __init__(self, config):
        ...
        if self.option("kegg_version"):
            print(self.option("kegg_version"))
    ...
    def get_kegg_pics(self):
        ...
        self.mongo_db = Config().get_mongo_client(mtype="ref_rna_v2")[Config().get_mongo_dbname("ref_rna_v2")]
    ...
    def download_s3_file(self, path, to_path):
        ...
        if os.path.exists(path):
            ...
            try:
                self.logger.info("from {} to {}".format(path, to_path))
                transfer = MultiFileTransfer()
                transfer.add_download(path, to_path)
                transfer.perform()
            except Exception as e:
                self.logger.info("文件下载异常：{}".format(e))
                return False
    ...
    def get_kegg_png(self):
        ...
        if kegg_collection.find_one({"task_id": self.option('task_id'), "type": self.option('type'), 'status': "end"})['has_new'] == False:
        ...
        if os.path.exists(self.work_dir + "/png/"):
            shutil.rmtree(self.work_dir + "/png/")
    ...
    def get_kegg_stat_tmp(self):
        ...
        kegg_stat_tmp = pd.read_table(os.path.join(self.work_dir, 'kegg_stat_tmp.xls'), header=None, sep='\t')
        if kegg_stat_tmp.shape[0] == 1:
            raise Exception("所选基因集没有KEGG注释结果，请重新选择基因集")
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        import random
        from mbio.workflows.ref_rna_v2.report.exp_pca import KeggClassWorkflow
        from biocluster.wsheet import Sheet
        data = {
            "id": "test_keggcalssref" + str(random.randint(1, 10000))+"yyyy",
            "type": "workflow",
            "name": "ref_rna_v2.kegg_class",
            "instant": False,
            "options": dict(
                geneset_kegg="/mnt/ilustre/users/sanger-dev/workspace/20190725/GenesetEnrich_tsg_34905_8931_876/multi_geneset_list",
                #group_dict=r'{"A":["A1", "A2", "A3"],"B": [ "B1", "B2", "B3"], "C": [ "C1", "C2", "C3"]}'.replace('"', '\\"'),
                task_id="tsg_34905",
                kegg_table="/mnt/ilustre/users/sanger-dev/workspace/20190725/GenesetEnrich_tsg_34905_8931_876/gene_kegg_table.xls",
                kegg_table2="/mnt/ilustre/users/sanger-dev/workspace/20190725/GenesetEnrich_tsg_34905_8931_876/gene_kegg_level_table.xls",
                geneset_id="5d31f85d17b2bf752cb3a299",
                background_links="/mnt/ilustre/users/sanger-dev/workspace/20190725/GenesetEnrich_tsg_34905_8931_876/add_info.txt",
                type="origin",
