...
class PredictTool(Tool):
    ...
    def td_predict(self, hmm_out=None):
        ...
        if not os.path.exists(pre_dir):
            os.mkdir(pre_dir)
            for file in os.listdir(self.work_dir + "/" + os.path.basename(self.option("fasta").prop["path"]) + ".transdecoder_dir"):
                os.link(self.work_dir + "/" + os.path.basename(self.option("fasta").prop["path"]) + ".transdecoder_dir/" + file, pre_dir + "/" + file)
        if hmm_out is None:
            hmm_out = ""
        else:
        ...
        if command.return_code == 0:
            ...
            ## 尝试旧版软件
            cmd = "{}TransDecoder.Predict -t {} -T {} {}".format(
                self.transdecoder_old_path, self.option("fasta").prop["path"], self.option("Markov_length"), hmm_out)
            command = self.add_command("transdecoder_old_predict", cmd, ignore_error=True)
            command.run()
            self.wait()
            if command.return_code != 0:
    ...
    def set_output(self):
        ...
        if self.option("search_pfam"):
            if os.path.exists(self.work_dir + "/pfam_domain"):
    ...
    def run(self):
        ...
        if self.option("search_pfam") is True:
            ...
            # 统计cds的长度区间
            if not os.path.exists(self.work_dir + "/" + self.fasta_name + ".transdecoder.pep"):
                cds_seq = self.work_dir + "/../Predict/" + self.fasta_name + ".transdecoder.pep"
            else:
                cds_seq = self.work_dir + "/" + self.fasta_name + ".transdecoder.pep"
