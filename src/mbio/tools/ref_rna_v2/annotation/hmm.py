...
from mbio.packages.ref_rna_v2.pfam_domtblout import pfam_out
...
# self.step.xx.{finish, start}和self.step.add_steps("xx),里面的xx需要保持一致
    ...
    def check_options(self):
        ...
        if not self.option("pep").is_set:
            raise OptionError("请传入fasta序列文件", code = "33701301")
...
class HmmTool(Tool):
    ...
    def __init__(self, config):
        ...
        self.hmmscan_path = "miniconda3/bin/"
    ...
    def hmmscan(self, pep):
        ...
        if command.return_code == 0:
            ...
            self.set_error("运行hmmscan出错", code = "33701302")
