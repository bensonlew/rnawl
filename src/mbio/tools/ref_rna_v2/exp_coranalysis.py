...
class ExpCoranalysisAgent(Agent):
    """
    Expression clustering analysis
    """
    def __init__(self, parent):
        super(ExpCoranalysisAgent, self).__init__(parent)
        options = [
    ...
    def check_options(self):
        if self.option('exp').prop['gene_number'] >= 3001:
            raise OptionError('Please input at most 3000 genes')
    ...
    def end(self):
        ...
        super(ExpCoranalysisAgent, self).end()


class ExpCoranalysisTool(Tool):
    """
    Expression clustering analysis
    """
    def __init__(self, config):
        super(ExpCoranalysisTool, self).__init__(config)
        software_dir = self.config.SOFTWARE_DIR
        self.python_path = 'miniconda2/bin/python'
        self.expcoranalysis_toolbox = self.config.PACKAGE_DIR + '/ref_rna_v2/express_cor.py'
    ...
    def run_expcoranalysis_toolbox(self):
        cmd = '{} {} '.format(self.python_path, self.expcoranalysis_toolbox)
    ...
    def run(self):
        super(ExpCoranalysisTool, self).run()
        self.run_expcoranalysis_toolbox()
        self.set_output()
        self.end()
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "ExpCoranalysis" + str(random.randint(1, 10000)),
            "type": "tool",
