...
class GatkIndelAgent(Agent):
    ...
    def check_options(self):
        if not self.option("ref_fasta").is_set:
            raise OptionError("缺少ref_fasta参数", code="33711903")
        if not self.option("pop_var_vcf"):
            raise OptionError("缺少pop_var_vcf参数", code="33711904")
...
class GatkIndelTool(Tool):
    ...
    def gatk_indel_run(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("gatk_indel出错！", code="33711908")
            self.set_error("gatk_indel出错！", code="33711909")
    ...
    def gatk_indel_filter_run(self):
        ...
        if code == 0:
            ...
            self.set_error("修改%s为可执行文件失败！",variables=(file_path), code="33711910")
        ...
        if command1.return_code == 0:
            ...
            self.set_error("indel_filter出错！", code="33711911")
            self.set_error("indel_filter出错！", code="33711912")
    ...
    def gatk_indel_filter_recode_run(self):
        ...
        if command2.return_code == 0:
            ...
            self.set_error("filter_recode出错！", code="33711913")
            self.set_error("filter_recode出错！", code="33711914")
