...
class CuffmergeAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "min_isoform_fraction", "type": "float", "default": 0.1},  # Discard isoforms with abundance below this
        ]
    ...
    def check_options(self):
        ...
        if not self.option('assembly_GTF_list.txt'):
            raise OptionError('必须输入所有样本gtf路径文件为txt格式', code = "33703501")
        if not self.option('ref_fa'):
            raise OptionError('必须输入参考序列ref.fa', code = "33703502")
        if not self.option('ref_gtf'):
            raise OptionError('必须输入参考序列ref.gtf', code = "33703503")
        return True
...
class CuffmergeTool(Tool):
    ...
    def run_cuffmerge(self):
        ...
        """
        cmd = self.cuffmerge_path + ('cuffmerge --min-isoform-fraction {} -p {} -g {} -s {}  -o {}merge_out'.format(
            self.option("min_isoform_fraction"), self.option('cpu'), self.option('ref_gtf').prop['path'], self.option('ref_fa').prop['path'],
        ...
        if command.return_code == 0:
            ...
            self.set_error("cuffmerge运行出错!", code = "33703504")
    ...
    def run_gtf_to_fa(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("gtf_to_fasta运行出错!", code = "33703505")
    ...
    def set_output(self):
        ...
        try:
            ...
            self.set_error("设置拼接合并分析结果目录失败%s", variables = (e), code = "33703506")
