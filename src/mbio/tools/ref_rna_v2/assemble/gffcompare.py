...
class GffcompareTool(Tool):
    ...
    def run(self):
        ...
        self.run_filter_gtf()
        try:
            self.run_filter_tmap()
        except Exception as e:
            self.logger.info("tmap filter errors {}".format(e))
        self.set_output()
        self.end()
        ...
        with open(merge_raw_gtf, "r") as merge_raw_file, open(merge_gtf, "w") as merge_file:
            for line in merge_raw_file:
                ...
                if content_m:
                    ...
                    merge_file.write(line + "\n")
        self.known_tran2gene = known_tran2gene

    def run_filter_tmap(self):
        known_gtf = self.option('merged_gtf')
        old_tmap = os.path.join(os.path.dirname(known_gtf), "cuffcmp.merged.gtf.tmap")
        new_tmap = "filter_tmap.tmp"
        with open(old_tmap, "r") as f_in, open(new_tmap, "w") as f_out:
            for line in f_in:
                cols = line.split("\t")
                if cols[4] in self.known_tran2gene and cols[2] == "u":
                    cols[2] = "="
                    cols[0] = self.known_tran2gene[cols[4]]
                    cols[1] = cols[4]

                    line = "\t".join(cols)
                f_out.write(line)

        if os.path.exists(old_tmap):
            os.remove(old_tmap)
        os.link(new_tmap, old_tmap)
