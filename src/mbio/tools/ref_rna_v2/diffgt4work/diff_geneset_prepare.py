...
class DiffGenesetPrepareTool(Tool):
    ...
    def prepare_go_class(self):
        ...
        go_level2_path = os.path.join(self.option("annot_result"), "allannot_class", "go", "go_lev2_gene.stat.xls")
        new_table_title = []
    ...
    def prepare_cog_class(self):
        ...
        genesets = self.get_geneset_detail()
        geneset_name = self.option("geneset_name")
        cog_summary_ref = os.path.join(self.option("annot_result"), "refannot_class", "cog", "summary.G.tsv")
        ...
        if os.path.exists(cog_summary_new):
            ...
            cog_df_r = pd.read_table(cog_summary_ref, sep="\t", header=0)
            cog_df_n = pd.read_table(cog_summary_new, sep="\t", header=0)
            ...
            cog_df = pd.read_table(cog_summary_ref, sep="\t", header=0)
