...
class SeqPrepAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "quality_score_system", "type": "string", "default": "phred+33"},# 接头序列 add by 20200401 fwy
            {"name": "adapter_a", "type": "string", "default": "AGATCGGAAGAGCACACGTC"},# 接头序列 add by 20200401 fwy
            {"name": "adapter_b", "type": "string", "default": "AGATCGGAAGAGCGTCGTGT"},# 接头序列 add by 20200401 fwy
    ...
    def check_options(self):
        ...
        if not self.option("fastq_dir").is_set and not self.option("fastq_r").is_set:
            raise OptionError("请传入PE序列文件或者文件夹", code = "33707801")
        if self.option("fastq_r").is_set and not self.option("fastq_l").is_set:
            raise OptionError("缺少PE序列左端文件", code = "33707802")
        if self.option("fastq_l").is_set and not self.option("fastq_r").is_set:
            raise OptionError("缺少PE序列右端文件", code = "33707803")
        ...
        if self.option("fastq_dir").is_set:
            ...
            for cmd in commands:
                ...
                if cmd.return_code == 0:
                    ...
                    self.set_error("运行%s运行出错!", variables = (cmd.name), code = "33707804")
                    return False
            self.set_output()
        else:
            self.seqprep()
