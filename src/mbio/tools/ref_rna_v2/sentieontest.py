...
class SentieontestAgent(Agent):
    """
    基因比对接口
    """

    def __init__(self, parent):
        super(SentieontestAgent, self).__init__(parent)
        options = [
    ...
    def end(self):
        super(SentieontestAgent, self).end()


class SentieontestTool(Tool):
    def __init__(self, config):
        super(SentieontestTool, self).__init__(config)
        self.set_environ(
    ...
    def run_step1(self):
        ...
        command = self.add_command("sentieon_step1", cmd, ignore_error=True).run()
        self.wait()
        if command.return_code == 0:
            self.logger.info("step1运行完成")
        elif command.return_code in [1,-7,-9,255,-11]:
    ...
    def run_step2(self):
        ...
        command = self.add_command("sentieon_step2", cmd,ignore_error=True).run()
        self.wait()
        if command.return_code == 0:
            self.logger.info("step2运行完成")
        elif command.return_code in [1,-7,-9,255]:
            self.logger.info("return code: %s" % command.return_code)
            self.add_state('memory_limit', 'memory is low!')
        else:
    ...
    def run_step3(self):
        ...
        command = self.add_command("sentieon_step3", cmd,ignore_error=True).run()
        self.wait()
        if command.return_code == 0:
    ...
    def run_haplotyper_v2(self):
        ...
        elif command.return_code in [1,-7,-9,255,-6]:
            self.logger.info("return code: %s" % command.return_code)
            self.add_state('memory_limit', 'memory is low!')
        else:
    ...
    def run(self):
        super(SentieontestTool, self).run()
        self.run_step1()
        self.run_step2()
        self.run_step3()
    ...
    def test(self):
        ...
        data = {
            ...
            "options": dict(
                ...
                bam_file="/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/test/20201209/LA1.bam",
                fa_file="/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/test/20201209/Trinity.filter.unigene.fasta",
                name="LA1"
               #mkdup_method="picard"
            )
           }
