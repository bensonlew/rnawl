...
class GatkSnpAgent(Agent):
    ...
    def check_options(self):
        if not self.option("ref_fasta").is_set:
            raise OptionError("缺少ref_fasta参数", code="33711703")
        if not self.option("pop_var_vcf"):
            raise OptionError("缺少pop_var_vcf参数", code="33711704")
...
class GatkSnpTool(Tool):
    ...
    def gatk_snp_run(self):
        ...
        if command.return_code == 0:
            ...
            self.set_error("gatk_snp出错！", code="33711708")
            self.set_error("gatk_snp出错！", code="33711709")
    ...
    def gatk_snp_filter_run(self):
        ...
        if code == 0:
            ...
            self.set_error("修改%s为可执行文件失败！", variables=(file_path), code="33711710")
        ...
        if command1.return_code == 0:
            ...
            self.set_error("snp_filter出错！", code="33711711")
            self.set_error("snp_filter出错！", code="33711712")
    ...
    def gatk_snp_filter_recode_run(self):
        ...
        if command2.return_code == 0:
            ...
            self.set_error("filter_recode出错！", code="33711713")
            self.set_error("filter_recode出错！", code="33711714")
