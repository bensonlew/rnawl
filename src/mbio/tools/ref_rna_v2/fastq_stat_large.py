...
class FastqStatLargeAgent(Agent):
    """
    用于做fastq序列基本信息统计的工具
    version 1.0
    author: qindanhua
    ...
    def __init__(self, parent):
        super(FastqStatLargeAgent, self).__init__(parent)
        options = [
    ...
    def end(self):
        super(FastqStatLargeAgent, self).end()


class FastqStatLargeTool(Tool):
    """
    version 1.0
    """
    def __init__(self, config):
        super(FastqStatLargeTool, self).__init__(config)
    ...
    def fastq_stat(self):
        self.get_list_file()
        cmd = "{}java -jar {} -i {} -t {} -Q {}".format(self.java_path, self.FastqStat_path, "fq_list_for_FastqStat", 10, self.option("quality"))
    ...
    def run(self):
        super(FastqStatLargeTool, self).run()
        self.fastq_stat()
