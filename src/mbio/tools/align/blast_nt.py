...
class BlastNtAgent(Agent):
    """
    ncbi blast+ 2.12.0  注意：在outfmt为6时不按照ncbi格式生成table，而是按照特殊的表头类型，参见packages.align.blast.xml2table
    version 1.0
    author: shenghe
    last_modify: 2021.10.25
    """

    def __init__(self, parent):
        super(BlastNtAgent, self).__init__(parent)
        ...
        self._database_type = {'nt': 'nucl', 'nr': 'prot', 'kegg': 'prot', 'swissprot': 'prot', 'rfam': 'nucl',
                               ...
                               'craniata': 'prot', 'bacteria': 'prot', 'cgc': 'nucl', 'nt_20180827': 'nucl', '16s': 'nucl', 'nt_bac': 'nucl','nt_v20200604': 'nucl','swissprot_v20200617': 'prot', 'nt_v20211009': 'nucl', 'rfam_v14.6': 'nucl'}
        options = [
        ...
        self._memory_increase_step = 30  # 每次重运行增加内存30G
    ...
    def check_options(self):
        ...
        if self.option("database") == 'customer_mode':
        ...
        elif self.option("database").lower() not in ["nt","nt_v20200604", "nr", "string", 'kegg', 'swissprot', 'rfam', 'archaea',
                                                     'viruses', 'fungi', 'viridiplantae', 'eukaryota_other','eukaryota', 'craniata',
                                                     'bacteria', 'cgc', 'nt_20180827', '16s','nt_bac', "swissprot_v20200617", "nt_v20211009", "rfam_v14.6"] and self.option("database") not in ['fgr/amoA_archaea_202012',
                                                     'fgr/amoA_bacteria_202012', 'nt', "nt_v20200604", "nt_v20211009", 'fgr/amoA_AOB_like_202012',
            ...
            if self.option("database").lower() in ["nt", "nr", "string", 'kegg', 'swissprot', "nt_v20200604", "nt_v20211009", "swissprot_v20200617"]:
        ...
        if self.option("database").lower() in ["nt","nt_v20200604", "nt_v20211009", "nr", "string", 'kegg', 'swissprot', 'rfam',
                                               'archaea','viruses', 'fungi', 'viridiplantae', 'eukaryota_other',
                                               'eukaryota', 'craniata', 'bacteria', 'cgc', 'nt_20180827', '16s',
                                               'nt_bac', "swissprot_v20200617", "rfam_v14.6"]:
    ...
    def set_resource(self):
        self._cpu = "10"
        memory = 140
        fq = self.option('query').prop['path']
        fq_size = os.path.getsize(fq)
        if fq_size > 1000000000:
            memory += (fq_size - 1000000000)/1000000000*5  #超过1G ，每增加1G ，内存加5G
    ...
    def end(self):
        ...
        super(BlastNtAgent, self).end()


class BlastNtTool(Tool):
    def __init__(self, config):
        super(BlastNtTool, self).__init__(config)
        self._version = "2.3.0"
        ...
        elif self.option("database") == "nt_v20211009":
            self.db_path = os.path.join(self.config.SOFTWARE_DIR, "database/align/ncbi/db/nt/nt_v20211009")
        elif self.option("database") == "rfam_v14.6":
            self.db_path = os.path.join(self.config.SOFTWARE_DIR, "database/align/ncbi/db/rfam_v14.6")
        ...
        if self.option("database") == "rfam_v14.6":
            self.cmd_path = "bioinfo/align/ncbi-blast-2.12.0+/bin"
            self.set_environ(PATH=os.path.join(self.config.SOFTWARE_DIR, "bioinfo/align/ncbi-blast-2.12.0+/bin"))
        if self.option("database") == "nt_v20211009":
            self.cmd_path = "bioinfo/align/ncbi-blast-2.12.0+/bin"
            self.set_environ(PATH=os.path.join(self.config.SOFTWARE_DIR, "bioinfo/align/ncbi-blast-2.12.0+/bin"))
    ...
    def run_blast(self, db_name):
        ...
        if blast_command.return_code == 0:
            ...
            self.end()
        elif blast_command.return_code == 2:
            self.logger.info("return code: %s" % command.return_code)
            self.add_state('memory_limit', 'memory is low!')
        elif blast_command.return_code == 255:
            self.logger.info("return code: %s" % command.return_code)
            self.add_state('memory_limit', 'memory is low!')
        else:
