...
import pandas as pd
from mbio.packages.dia_v3.metastat import two_group_test, two_sample_test
import math
import glob
import subprocess
...
class DiffAgent(Agent):
    def __init__(self, parent):
        ...
        options = [
            {"name":"group", "type":"infile","format":"labelfree.group_table"}, # 分组文件
            {"name": "ratio_exp", "type": "infile", "format":"labelfree.ratio_exp"},  # fisher,卡方，t检验的输入文件
            {"name":"cmp", "type":"infile","format":"labelfree.compare_table"}, # 比较分组文件
            ...
            {"name": "padjust_method", "type": "string", "default": "fdr"},
        ]
...
class DiffTool(Tool):
    ...
    def split_group(self, group, cmp):
        ...
        for con in con_other:
            ...
            if not df_selected_control.empty and not df_selected_other.empty:
        ...
        for i in sample2group:
            group2sample[i[1]].append(i[0])
        self.g2cutoff = dict()
        cutoffs = self.option('cutoffs').split(',')
        groups = df_group['group']
        try:
            self.g2cutoff = {x[1]: float(x[2]) for x in zip(groups, cutoffs)}
        except:
            self.logger.info('采用默认方法确定有效蛋白,即有一半及一半以上的样本有这个蛋白，则判定此分组里存在这个蛋白')
            self.g2cutoff = {g: float(len(s)) / 2 for g, s in group2sample.items()}
        for g, s in group2sample.items():
    ...
    def diff(self):
        ...
        for cmp in files:
            i += 1
            cmp_df = pd.read_csv(cmp, sep='\t')
            groups_ = cmp_df['group'].tolist()
            group = list(set(groups_))
            group.sort(key=groups_.index)
            control, other = group
            two_group_test(self.option("ratio_exp").prop['path'], cmp, self.work_dir + '/result'+ str(i) + '.xls',
                           log_trans=self.option('log'),
                           choose_test=self.option('method_type'), test_type=self.option("correct_method"),
                           cutoff_a=str(self.g2cutoff[control]), cutoff_b=str(self.g2cutoff[other]),
                           mul_test=self.option('padjust_method'))
            os.rename(self.work_dir + "/run_%s_test.r"%self.option('method_type'), self.work_dir + "/run_%s_test"%self.option('method_type') + str(i) + ".r")
            cmd = self.r_path + " run_%s_test"%self.option('method_type') + str(i) + ".r" #这里有一个空格，作为运行命令
    ...
    def run_fisher(self):
        ...
        for cmp in files:
            ...
            fisher_sample1, fisher_sample2 = df["#sample"].tolist()[:2]
            two_sample_test(self.option("ratio_exp").prop['path'], self.work_dir + '/result'+ str(i) + '.xls', "fisher",
                            fisher_sample1, fisher_sample2, log_trans=self.option('log'),
                            test_type=self.option('correct_method'), mul_test=self.option('padjust_method'))
    ...
    def run_chi(self):
        ...
        for cmp in files:
            i += 1
            df = pd.read_table(cmp, header=0, sep="\t")
            chi_sample1, chi_sample2 = df["#sample"].tolist()[:2]
            two_sample_test(self.option("ratio_exp").prop['path'], self.work_dir + '/result'+ str(i) + '.xls',  "chi",
                            chi_sample1, chi_sample2, log_trans=self.option('log'), mul_test=self.option('padjust_method'))
    ...
    def diff_treat(self, fc_up, fc_down):
        ...
        def signicent(row, p1=self.option("pvalue")):
            if p1 == 1 and row[4] == 0 and row[3] == 1 and row[2] == 0 and row[1] == 0:
                p1 = 0.999
            fc = row[4]
        ...
        for file in files:
            ...
            fc = df['fc']
            log2fc = df['log2fc']
            padjust = df['corrected_pvalue']
            df.drop(labels=['fc', 'log2fc', 'corrected_pvalue'], axis=1, inplace=True)
            df['fc'] = fc
            df['log2fc'] = log2fc
            # df.drop(df.columns[[2, 4, 6]], axis=1, inplace=True)
            df.drop(df.columns[[2, 4]], axis=1, inplace=True)   # retain padjust
            ...
            df['padjust'] = padjust
            df['regulate'] = df.apply(up_down, axis=1)
            df['significant'] = df.apply(signicent, axis=1)
            str_1 = list(df.columns)[2] + "|" + list(df.columns)[1]
            df['log10pvalue'] = df.apply(pvaluelog, axis=1)
            df['log10padjust'] = df['padjust'].apply(lambda x: math.log(x + 0.00001, 10))
            df['compare'] = str_1
    ...
    def diff_treat_fisher_chi(self, fc_up, fc_down):
        ...
        for file in files:
            ...
            # df.drop(df.columns[[4]], axis=1, inplace=True)    # retain adjust
            df['padjust'] = padjust
            df['regulate'] = df.apply(up_down, axis=1)
            df['significant'] = df.apply(signicent, axis=1)
            str_1 = list(df.columns)[2] + "|" + list(df.columns)[1]
    ...
    def diff_summary(self):
        def my_test(row):
            # if row[7] == "yes" and (row[6] == "up" or row[6] == "down"):
        ...
        def my_test1(row):
            # if row[6] == "up" and row[7] == "yes":
            if row[7] == "up" and row[8] == "yes":
                return "yes"
            # if row[6] == "up" and row[7] == "no":
            if row[7] == "up" and row[8] == "no":
                return "no"

        def my_test2(row):
            # if row[6] == "down" and row[7] == "yes":
            if row[7] == "down" and row[8] == "yes":
                return "yes"
            # if row[6] == "down" and row[7] == "no":
    ...
    def diff_summary_labelfree(self):
        def diff_lf(row):
            # if row[7] == "yes" and (row[6] == "up" or row[6] == "down"):
            # if row[8] == "yes" and (row[7] == "up" or row[6] == "down"):
            if row[8] == "yes" and (row[7] == "up" or row[7] == "down"):
                return "yes"
            else:
                return "no"
        ...
        def down_lf(row):
            ...
            if row[7] == "down" and row[8] == "yes":
                ...
                return "no"
        def only_A(row):
            if row[4] == maxfc and row[3] == 0:
                return "yes"
            else:
                return "no"
        def only_B(row):
            if row[4] == 0.00001 and row[3] == 0:
                return "yes"
            else:
                return "no"
        def neither(row):
            if row[4] == 0 and row[3] == 1 and row[2] == 0 and row[1] == 0:
                return "yes"
            else:
                return "no"
        def both(row):
        ...
        for file in files:
            ...
            self.logger.info("双击8888888")
            str1 = columns[1] + "_" + "vs" + "_" + columns[0]
            str1_both = str1 + "_both"
            str1_diff = str1 + "_diff"
            str1_up = str1 + "_up"
            str1_down = str1 + "_down"
            df[str1_both] = df.apply(both, axis=1)
            df[str1_diff] = df.apply(diff_lf, axis=1)
            df[str1_up] = df.apply(up_lf, axis=1)
            df[str1_down] = df.apply(down_lf, axis=1)
            df_new = df.loc[:, ["accession_id", str1_both, str1_diff, str1_up, str1_down, ]]
            if self.option("fill_type").lower() != 'all':
                str1_onlyA = str1 + "_only_" + columns[1]
                str1_onlyB = str1 + "_only_" + columns[0]
                str1_neither = str1 + "_neither"
                df[str1_onlyA] = df.apply(only_A, axis=1)
                df[str1_onlyB] = df.apply(only_B, axis=1)
                df[str1_neither] = df.apply(neither, axis=1)
                df_new = df.loc[:, ["accession_id", str1_both,str1_diff, str1_up, str1_down, str1_onlyA, str1_onlyB, str1_neither]]
            df_new.fillna("no", inplace=True)
            df_new.index.name = "accession_id"
            # df = df[(df.fc >= self.option("fc_up")) | (df.fc ==self.option("fc_down"))]
            df_new.to_csv(self.work_dir + "/diff_summary" + str(i) + ".csv",
                          sep = '\t', index=False)
    
    def merge_labelfree(self):
        files = glob.glob(r"*diff_summary*")
        i = 0
        list_df = list()
        for file in files:
            i += 1
        ...
        # df_merged = reduce(lambda  left,right: pd.merge(left,right, on=['accession_id'], how='outer'), list_df)
        df_merged.index.name = "accession_id"
        df_merged.to_csv(self.work_dir + "/all_summary" + ".xls", sep = '\t')
    ...
    def set_output(self):
        ...
        diff_summary_lf = self.work_dir + "/" + "all_summary.xls"
        os.link(diff_summary_lf, os.path.join(self.output_dir, "diff_protein_summary.xls"))
    ...
    def run(self):
        ...
        self.diff_summary()
        self.merge()
        self.diff_summary_labelfree()
        self.merge_labelfree()
...
class TestFunction(unittest.TestCase):
    def test(self):
        ...
        data = {
            "id": "diff" + str(random.randint(1, 10000)),
            "type": "tool",
            "name": "labelfree.diff",
            "instant": False,
            "options": dict(
                group="/mnt/ilustre/users/sanger-dev/sg-users/fengyitong/labelfree_dev/group.txt",
                cmp="/mnt/ilustre/users/sanger-dev/sg-users/fengyitong/labelfree_dev/control.txt",
                ratio_exp="/mnt/ilustre/users/sanger-dev/sg-users/fengyitong/labelfree_dev/exp1.txt",
                fc_up=1.2,
                fc_down=0.83,
                pvalue=0.05,
        ...
        wf = SingleWorkflow(wsheet)  # 各类参数的手动指定，测试用的经典做法
        wf.run()
