...
class SortSamplesToolappsAgent(Agent):
    """
    传入一个group表，以及是否进行样本合并的参数生成一张丰度表并对并依照group表OTU表进行筛选合并
    计算相对表时，利用全表的计算方式，然后再选择top
    """
    def __init__(self, parent):
        super(SortSamplesToolappsAgent, self).__init__(parent)
        options = [
    ...
    def end(self):
        ...
        super(SortSamplesToolappsAgent, self).end()
...
class SortSamplesToolappsTool(Tool):
    def __init__(self, config):
        super(SortSamplesToolappsTool, self).__init__(config)
    ...
    def cat_samples_percent(self):
        ...
        if self.option("top") != "":
            ...
            abund_table2 = abund_table.head(top)
        else:
            abund_table2 = abund_table
        abund_table2.to_csv(self.abund_table_path, sep="\t", encoding="utf-8")
        ...
        if self.option("others") != "" or self.option("top") != "" or self.option("sample_del_warn") != "F" or self.option("get_only_percent") == 'T':
            ...
            if self.option("top") != "":
                top = int(self.option("top"))
                abund_table3 = abund_table_percent.head(top)
            else:
                abund_table3 = abund_table_percent
            abund_table3.to_csv(self.abund_table_percent_path, sep="\t", encoding="utf-8")
    ...
    def run(self):
        super(SortSamplesToolappsTool, self).run()
