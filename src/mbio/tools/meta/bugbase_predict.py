...
class BugbasePredictTool(Tool):
    ...
    def picrust_predict(self):
        ...
        if command.return_code in [0]:
            ...
            if command.return_code in [1]:
                self.set_error("序列未能与greengene数据库比对上！")
            else:
    ...
    def set_output(self):
        ...
        out_normalize_table_path = os.path.join(self.output_dir, "16s_normalized_otus.txt")
        if os.path.exists(out_normalize_table_path):
            ...
            os.link(normalize_table_path, out_normalize_table_path)
        new_out_normalize_table_path = os.path.join(self.output_dir, "16s_normalized_otus.txt_new")
        ...
        contributing_file_new = os.path.join(self.output_dir, "contributing_otus.txt")
        if os.path.exists(contributing_file_new):
            os.remove(contributing_file_new)
            os.link(contributing_file, contributing_file_new)
        else:
            os.link(contributing_file, contributing_file_new)
        contributing_file_new1 = os.path.join(self.output_dir, "contributing_otus.txt_new")
