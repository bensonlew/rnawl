...
class OtuTaxonStatAgent(Agent):
    ...
    version 1.0
    author: xuting
    last_modify: 2018.4.20 by zhengyuan
    """
    ...
    def check_options(self):
        ...
        if self.option("sub_otu_table").is_set:
            ...
            if not self.option("in_otu_table").is_set:
                raise OptionError("输入的OTU文件不能为空", code="32705001")
            if not self.option("taxon_file").is_set:
                raise OptionError("输入的taxon文件不能为空", code="32705002")
            self.option("in_otu_table").get_info()
            if self.option("in_otu_table").prop['metadata'] == "taxonomy":
                raise OptionError("otu表不应该有taxonomy信息", code="32705003")
        return True
    ...
    def end(self):
        ...
        result_dir.add_relpath_rules([
            ...
            [r"otu_summary.xls", "meta.otu.otu_table", "基于 OTU 数量的统计"]
        ])
...
class OtuTaxonStatTool(Tool):
    ...
    def get_biom_otu(self):
        ...
        with open(tax_file, 'r') as r1:
            for line in r1:
                ...
                tmp = re.split(';', re.sub(r'\s+', '_', line[1])) #add by zouxuan 20180115 ,nt注释结果只有两列，中间可能存在空格
                new_line = '; '.join(tmp)
                otu_tax[line[0]] = new_line
        taxon_otu = os.path.join(self.work_dir, "output", "otu_taxon.xls")
        otu_stat = os.path.join(self.work_dir, "output", "otu_summary.xls")
        ...
        if create_taxon_biom.return_code == 0:
            ...
            self.set_error("taxon_biom生成失败", code="32705001")
    ...
    def get_diff_level(self, biom):
        ...
        if create_tax_summary_.return_code == 0:
            ...
            self.set_error("tax_summary文件夹生成失败", code="32705002")
        ...
        if create_tax_summary.return_code == 0:
            ...
            self.set_error("文件夹生成失败", code="32705003")
        ...
        for my_otu_table in list_:
            if re.search(r"txt", my_otu_table):
                ...
                try:
                    ...
                    self.set_error("运行sum_tax.pl出错", code="32705004")
        ...
        for my_otu_table in list__:
            if re.search(r"txt", my_otu_table):
                ...
                try:
                    ...
                    self.set_error("运行sum_tax.pl出错", code="32705005")
                    raise Exception("运行sum_tax.pl出错")
    ...
    def rename(self):
        ...
        if create_taxon_biom_otu_.return_code == 0:
            ...
            self.set_error("taxon_biom_otu生成失败", code="32705006")
        ...
        if create_taxon_biom_otu.return_code == 0:
            ...
            self.set_error("taxon_biom_otu生成失败", code="32705007")
