...
class PanCoreOtuAgent(Agent):
    ...
    def check_options(self):
        ...
        if not self.option("in_otu_table").is_set:
            raise OptionError("参数otu_table不能为空", code="32705201")
        if self.option("level") not in ['otu', 'domain', 'kindom', 'phylum', 'class',
                                        'order', 'family', 'genus', 'species']:
            raise OptionError("请选择正确的分类水平", code="32705202")
        return True
...
class PanCoreOtuTool(Tool):
    ...
    def _create_pan_core(self):
        ...
        if num_lines < 5:
            self.set_error("Otu表里的OTU数目小于3个！请更换OTU表或者选择更低级别的分类水平！", code="32705101")
        ...
        try:
            ...
            self.set_error("表格生成失败", code="32705202")
        ...
        if self.option('meta_group_name'):
            if not os.path.exists(os.path.join(self.work_dir, "output", self.option('meta_group_name'), "PanCoreOTU")):
                os.makedirs(os.path.join(self.work_dir, "output", self.option('meta_group_name'), "PanCoreOTU"))
            pan_dir = os.path.join(self.work_dir, "output", self.option('meta_group_name'), "PanCoreOTU/pan.richness.xls")
            core_dir = os.path.join(self.work_dir, "output", self.option('meta_group_name'), "PanCoreOTU/core.richness.xls")
        else:
