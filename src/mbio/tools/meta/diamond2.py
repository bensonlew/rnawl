...
class Diamond2Agent(Agent):
    """
    diamond version: 2.0.13
    version 1.0
    author:
    last_modify: 2022.01.10
    """
    def __init__(self, parent):
        super(Diamond2Agent, self).__init__(parent)
        options = [
            {"name": "query", "type": "infile", "format": "sequence.fasta"},  # 输入文件
            {"name": "query_type", "type": "string", "default": "nucl"},  # 输入的查询序列的格式，为nucl或者prot
            {"name": "database", "type": "string", "default": "plant"},# 比对数据库 plant, nr, etc.
    ...
    def end(self):
        super(Diamond2Agent, self).end()


class Diamond2Tool(Tool):
    def __init__(self, config):
        super(Diamond2Tool, self).__init__(config)
    ...
    def run_diamond(self, db_name):
        ...
        outfmt = self.option('outfmt')       # if self.option('outfmt') == 5:
        outputfile += '.xml'       # outfmt默认为5#outfmt = 5  #guanqing 20180525
        ...
        with open(path,"r") as file:
            for line in file:
                ...
                if line.lstrip().startswith("<Hit_id>"):
                    ...
                    if m:
                        self.repl.append(m.group(1))
                        line = file.next()
                        n = re.match("<Hit_def>(.+)</Hit_def>", line.lstrip())
                        try:
                            self.ori.append(n.group(1))
                        except:
                            print line
