...
class PcaAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "group_table", "type": "infile", "format": "toolapps.group_table, meta.otu.group_table"},  # modify by zhouxuan 20170823
    ...
    def check_options(self):
        ...
        if self.option('scale') not in ['T', 'F']:
            raise OptionError('scale只能为T或者F', code="32702901")
        if self.option('ellipse') not in ['T', 'F']:
            raise OptionError('ellipse只能为T或者F', code="32702902")
            ...
            if self.option('envlabs'):
                ...
                for lab in labs:
                    if lab not in self.option('envtable').prop['group_scheme']:
                        raise OptionError('提供的envlabs中有不在环境因子表中存在的因子：%s', variables=(lab), code="32702905")
            else:
                pass
            if self.option('envtable').prop['sample_number'] < 3:
                raise OptionError('环境因子表的样本数目少于3，不可进行beta多元分析', code="32702906")
        ...
        if self.option('envtable').is_set:
            ...
            if len(common_samples) < 3:
                raise OptionError("环境因子表和丰度表的共有样本数必需大于等于3个：%s", variables=(len(common_samples)), code="32702907")
            ...
            if self.option('eigenvalue') == 'row':
                for i in self.option('group_table').prop['sample_name']:
                    if i not in self.option('otutable').prop['col_sample']:
                        raise OptionError('分组文件中的样本不存在于表格中，查看是否是数据取值选择错误', code="32702909")
            else:
                for i in self.option('group_table').prop['sample_name']:
                    if i not in self.option('otutable').prop['row_sample']:
                        raise OptionError('分组文件中的样本不存在于表格中，查看是否是数据取值选择错误', code="32702910")
        return True
...
class PcaTool(Tool):  # PCA需要第一行开头没有'#'的丰度表，filter_otu_sample函数生成的表头没有'#'
    def __init__(self, config):
        ...
        if self.option("group_table"):
            if self.option("group_table").format == "meta.otu.group_table" and not self.option("group_table").prop['is_empty'] and self.option("ellipse")=='T':
                self.gexist = True
            else:
                self.gexist = False
    ...
    def get_otu_table(self):
        ...
        if len(otu_file.readlines()) < 3:
            raise OptionError('数据表特征数量必须大于等于2: %s', variables=(len(otu_file.readlines())), code="32702911")
        return otu_path
        # if self.option('envtable').is_set:
    ...
    def run_ordination(self, group=None, group_table=None, cmd1='cmd', cmd2='pca'):
        ...
        if self.option('envtable').is_set:  # 小工具不用判断
            ...
            if not self.create_otu_and_env_common(old_otu_table, old_env_table, self.otu_table, self.env_table):
                self.set_error('环境因子表中的样本与丰度表中的样本共有数量少于2个', code="32702901")
        else:
        ...
        if not self.gexist:
            ...
            cmd += ' %s -type pca -community %s -outdir %s -scale %s -group %s' % (
                self.cmd_path, real_otu_path, self.work_dir, self.option('scale'), group_run)
        if self.option('envtable').is_set:
        ...
        def get_range(path):
            with open(path) as sites:
                ...
                for line in sites:
                    ...
                    if len(split_line) < 3:
                        self.set_error('未知原因，坐标文件少于3列', code="32702905")
            ...
            if self.option('envtable').is_set:
                if pca_factor_score_file:
                    if not pca_factor_file:
                        self.set_error('未知原因，环境因子相关结果丢失或者未生成,factor文件不存在', code="32702906")
                else:
                    if pca_factor_file:
                        self.set_error('未知原因，环境因子相关结果丢失或者未生成,factor_scores文件不存在', code="32702907")
                if pca_vector_score_file:
                    if not pca_vector_file:
                        self.set_error('未知原因，环境因子相关结果丢失或者未生成,vector文件不存在', code="32702908")
                else:
                    if pca_vector_file:
                        self.set_error('未知原因，环境因子相关结果丢失或者未生成,vector_scores文件不存在', code="32702909")
                    elif not pca_factor_score_file:
                        self.set_error('未知原因，环境因子相关结果全部丢失或者未生成', code="32702910")
