...
class EstimatorsAgent(Agent):
    ...
    def set_resource(self):
        ...
        self._cpu = 12
        table_size = 4
        if self.option('otu_table').format == "meta.otu.otu_table":
            table_size = int(os.path.getsize(self.option('otu_table').prop["path"]) / float(1024 * 1024))
            if table_size < 4 :
                table_size = 4
            elif table_size > 100:
                table_size = 100
        self._memory = '{}G'.format(table_size)
