...
class PrephageAgent(Agent):
    ...
    def check_options(self):
        if not self.option("prot_seq").is_set:
            raise OptionError("必须添加基因的蛋白的序列文件！", code="31402701")
        if not self.option("scaf_seq").is_set:
            raise OptionError("必须添加基因组scaffold的序列文件", code="31402702")
        if not self.option("gene_gff").is_set:
            raise OptionError("必须添加基因组的gff3的文件！", code="31402703")
        if not self.option("anno").is_set:
            raise OptionError("必须添加基因组的基因注释的总览表的文件！", code="31402704")
...
class PrephageTool(Tool):
    ...
    def set_output(self):
        ...
        if num > 1 :
            ...
            os.link(self.work_dir + '/' + self.sample_name + '.summary.xls',path + '/' + self.sample_name + '_prephage_summary.xls')
            if os.path.exists(path + '/' + self.sample_name + '_prephage_detail.xls'):
                os.remove(path + '/' + self.sample_name + '_prephage_detail.xls')
            os.link(self.work_dir + '/' + self.sample_name + '.detail.xls',path + '/' + self.sample_name + '_prephage_detail.xls')
            if os.path.exists(path + '/' + self.sample_name + '.stat.xls'):
                os.remove(path + '/' + self.sample_name + '.stat.xls')
            os.link(self.work_dir + '/' + self.sample_name + '.stat.xls',path + '/' + self.sample_name + '.stat.xls')
