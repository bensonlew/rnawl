...
class TcdbAnnoAgent(Agent):
    ...
    def __init__(self, parent):
        super(TcdbAnnoAgent, self).__init__(parent)
        options = [
    ...
    def check_options(self):
        if not self.option("tcdb_xml_dir").is_set:
            raise OptionError("必须设置输入文件", code="31203501")
        return True
    ...
    def end(self):
        self.option('tcdb_anno_result', os.path.join(self.output_dir, "gene_tcdb_anno.xls"))
        result_dir = self.add_upload_dir(self.output_dir)
        result_dir.add_relpath_rules([
            [".", "", "结果输出目录"],
            ['gene_tcdb_anno.xls', 'xls', '序列详细注释文件']
        ])
        super(TcdbAnnoAgent, self).end()


class TcdbAnnoTool(Tool):
    def __init__(self, config):
        ...
        self.result_name = ''
        self.logger.info(">>>__init__ is end")
    ...
    def merge_table(self):
        ...
        for i in xml_file:
            ...
            if command.return_code == 0:
                ...
                self.set_error("cat %s error", variables=(i), code="31203501")
                self.set_error("cat %s error", variables=(i), code="31203502")
    ...
    def run_tcdb_anno(self):
        ...
        if self.option("has_coverage"):#add by qingchen.zhang20190717
            cmd = '{} {} -i {} -o {}'.format(self.python_path, self.python_script2, new_result,
        ...
        try:
            ...
            self.set_error('运行tcdb_anno出错', code="31203503")
