...
class CypsAddInfoAgent(Agent):
    """
    author: guanqing.zou
    last_modify: 20180528
    """
    def __init__(self, parent):
        super(CypsAddInfoAgent, self).__init__(parent)
        options = [
    ...
    def check_options(self):
        ...
        if not self.option("bsnout"):
            raise OptionError("必须设置参数bsnout", code="31201201")
        return True
    ...
    def end(self):
        super(CypsAddInfoAgent, self).end()


class CypsAddInfoTool(Tool):
    ...
    def run_addinfo(self):
        ...
        cmd_path = os.path.join(self.config.PACKAGE_DIR, "annotation/p450_merge_info.py")
        ...
        if command.return_code == 0:
                ...
                self.set_error("%s 运行失败", variables=(cmd_path), code="31201201")
    ...
    def run(self):
        ...
        :return:
        """
        super(CypsAddInfoTool, self).run()
        self.run_addinfo()
        self.end()
