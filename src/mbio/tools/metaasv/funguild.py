...
class FunguildTool(Tool):
    ...
    def change_format_fun(self):
        ...
        data['taxonomy'] = data['ASV ID'].apply(lambda x : ';'.join(str(x).split(';')[:-1]))
        data['ASV ID'] = data['ASV ID'].apply(lambda x : str(x).split(';')[-1].strip())
