...
class CorrNetworkCalcAgent(Agent):
    ...
    def check_options(self):
        ...
        if not self.option("corr_table"):
            raise OptionError("必须输入物种相似性网络文件")
        if self.option('coefficient') < 0 or self.option('coefficient') > 1:
            raise OptionError('coefficient值必须是在0~1范围之内的值')
...
class CorrNetworkCalcTool(Tool):
    ...
    def __init__(self, config):
        ...
        self.script_path = os.path.join(self.config.PACKAGE_DIR, "metaasv/corr_net_calc.py")
