...
class LncannotAgent(Agent):
    ...
    def __init__(self, parent):
        print "init annot init1"
        super(LncannotAgent, self).__init__(parent)
        self._fasta_type = {'Protein': 'prot', 'DNA': 'nucl'}
        self._blast_type = {'nucl': {'nucl': ['blastn', 'tblastn'],
                                     'prot': ['blastx']},
                            'prot': {'nucl': [],
                                     'prot': ['blastp']}}
        print "init annot init"
    ...
    def end(self):
        ...
        result_dir.add_regexp_rules([
            ...
            [r".+_vs_.+\.txt_\d+\.xml", "xml", "Lncannot比对输出多xml结果，输出格式为14的单个比对结果文件,主结果文件在txt文件中"],
            [r".+_vs_.+\.txt_\d+\.json", "json", "Lncannot比输出对多json结果，输出格式为13的单个比对结果文件,主结果文件在txt文件中"],
            ])
        # print self.get_upload_files()
        super(LncannotAgent, self).end()


class LncannotTool(Tool):
    def __init__(self, config):
        super(LncannotTool, self).__init__(config)
        self._version = "2.3.0"
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            "type": "tool",
            "name": "lnc_rna.lncrna_annot",
            "instant": False,
            "options": dict(
                reference="/mnt/ilustre/users/sanger-dev/app/database/Genome_DB_finish/vertebrates/Test_ref/Ensemble_release_89/lncrna_old/lncrna.fa",
                query="/mnt/ilustre/users/sanger-dev/workspace/20190722/LncRna_tsg_34914/NewLncrnaPredict/output/novel_lncrna.fa",
            )
        }
