...
class ExpVennAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            dict(name="express_matrix", type="infile", format="lnc_rna.express_matrix"),
            # 没有重复实验时，可以用样本名作为组名。
            dict(name="group_table", type="infile", format="lnc_rna.group_table"),
    ...
    def check_options(self):
        try:
            ...
            raise OptionError('无法将threshold的转换为数值，请检查输入')
        else:
            if threshold <= 0:
                raise OptionError('threshold必须大于0')
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        test_dir='/mnt/ilustre/users/sanger-dev/workspace/20190306/Single_Quant+3482/Quant/output'
        data = {
            "id": "ExpVenn" + str(random.randint(1, 10000)),
            "type": "tool",
            "name": "lnc_rna.exp_venn",
            "instant": True,
            "options": {
                "express_matrix": test_dir + '/' + 'transcript.tpm.matrix',
                "group_table":'/mnt/ilustre/users/sanger-dev/sg-users/fuwenyao/lncRNA/test_data_new/new_test_rawdata1/Human_10smaples/group.txt',
                "threshold": 1,
            }
        }
