...
class CnciPredictorTool(Tool):
    ...
    def lncrna_mark(self, predict_file):
        ...
        out_fields = ['transcript_id', 'gene_id', 'transcript_length', 'orf_length', 'score', 'label']
        ...
        with open(predict_file) as in_handler, open(self.cnci_out_file, 'w') as out_handler:
            ...
            for line_dic in csv.DictReader(in_handler, delimiter='\t'):
                ...
                out_handler.write(out_demo.format(
                    transcript_id=t_id, transcript_length=length, gene_id=gene_id, **line_dic))
    ...
    class TestFunction(unittest.TestCase):
        ...
        def test(self):
            ...
            data = {
                ...
                "type": "tool",
                "name": "lnc_rna.lncrna_identification.cnci_predictor",
                "instant": False,
                "options": dict(
                    taxonmy="animal",
                    fasta_file="/mnt/ilustre/users/sanger-dev/sg-users/zhaozhipeng/lncrna_tools/test_lnc_predict.fa",
                    cnci_score=0,
                )
            }
