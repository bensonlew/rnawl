...
class Barrnap2Agent(Agent):
    """
    Barrnap v0.9 进行rRNA预测
    """

    def __init__(self, parent):
        super(Barrnap2Agent, self).__init__(parent)
        options = [
    ...
    def end(self):
        super(Barrnap2Agent, self).end()


class Barrnap2Tool(Tool):
    def __init__(self, config):
        super(Barrnap2Tool, self).__init__(config)
        ...
        #self.path = self.config.SOFTWARE_DIR + "/bioinfo/align/hmmer-3.1b2-linux-intel-x86_64/binaries/nhmmer"
        #self.conda = self.config.SOFTWARE_DIR + "/program/miniconda3/bin/"
    ...
    def set_output(self):
        nul_path = self.output_dir + "/" + self.sample_name + ".rRNA.fnn"
        gff = self.output_dir + "/" + self.sample_name + ".rRNA.gff"
        if os.path.getsize(nul_path):
            self.option('seq', nul_path)
        self.option('rna_gff', gff)

    def run(self):
        super(Barrnap2Tool, self).run()
        self.run_barrnap()
        self.end()
