...
class MapAssessmentTestModule(Module):
    """
    ...
    def __init__(self, work_id):
        super(MapAssessmentTestModule, self).__init__(work_id)
        options = [
            ...
            {"name": "chr_info", "type": "string", "default": ''},  # 做染色体圈图统计必传
            # {"name": "circos", "type": "bool", "default": False},
    ...
    def check_options(self):
        ...
        for analysis in self.option("analysis").split(","):
            ...
            # if analysis == "chr_stat" and self.option("chr_info") == "":
            #     raise OptionError("circos统计需要传入染色体禅读信息表")
    ...
    def chr_stat_run(self):
        n = 0
        for f in self.files:
            chr_stat = self.add_tool("gene_structure.chr_distribution_test")
            self.step.add_steps("chr_distribution_{}".format(n))
            chr_stat.set_options({
                "bam": f,
                "chr_info": self.option("chr_info")
            })
        ...
        for tool in self.tools:
            ...
            for f_name in out:
                ...
                if "chr_stat.xls" in f_name:
                    ...
                    sourse_circos = os.path.join(tool.work_dir, "circos_stat", "all")
                    self.logger.info(sourse_circos)
                    target_circos = os.path.join(self.output_dir, "chr_stat", f_name + ".circos")
                    if os.path.exists(target):
                        os.remove(target)
                    if os.path.exists(target_circos):
                        os.remove(target_circos)
                    os.link(fp, target)
                    os.link(sourse_circos, target_circos)
    ...
    def run(self):
        super(MapAssessmentTestModule, self).run()
    ...
    def end(self):
        ...
        super(MapAssessmentTestModule, self).end()
