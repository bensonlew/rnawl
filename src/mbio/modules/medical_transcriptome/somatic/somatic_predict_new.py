...
class SomaticPredictNewModule(Module):
    """
    bam文件分析前处理
    """
    def __init__(self, work_id):
        super(SomaticPredictNewModule, self).__init__(work_id)
        options = [
        ...
        self.dbnsfp_tools = self.add_tool("medical_transcriptome.somatic.dbnsfp_tools_new")
    ...
    def run_vcf_split(self):
        ...
        self.vcf_split.on("end", self.run_somatic_predict)
        self.vcf_split.run()

    def run_somatic_predict(self):
        self.dbnsfp_tools.set_options({
            "vcf_dir": self.vcf_split.output_dir,
        })
    ...
    def run_predict_stat(self):
        self.predict_stat.set_options({
            "predict_result":os.path.join(self.dbnsfp_tools.output_dir,"final_result"),
        })
    ...
    def set_output(self):
        allfiles = os.listdir(self.predict_stat.output_dir)
        resultfiles = [os.path.join(self.predict_stat.output_dir, i) for i in allfiles]
    ...
    def run(self):
        super(SomaticPredictNewModule, self).run()
        self.logger.info("开始准备切割vcf文件")
        # self.genome_split()
    ...
    def end(self):
        super(SomaticPredictNewModule, self).end()
