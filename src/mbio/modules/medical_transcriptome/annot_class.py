...
class AnnotClassModule(Module):
    ...
    def __init__(self, work_id):
        ...
        options = [
            ...
            {"name": "blast_nr_table", "type": "infile", "format": "ref_rna_v2.blast_table"},
            {"name": "blast_string_table", "type": "infile", "format": "ref_rna_v2.blast_table"},
            {"name": "blast_kegg_table", "type": "infile", "format": "ref_rna_v2.blast_table"},
            {"name": "known_ko", "type": "string", "default": None},
            {"name": "blast_swissprot_table", "type": "infile", "format": "ref_rna_v2.blast_table"},
            {"name": "pfam_domain", "type": "infile", "format": "ref_rna_v2.kegg_list"},
            {"name": "gos_list_upload", "type": "infile", "format": "ref_rna_v2.anno_upload"},   # 客户上传go注释文件
            {"name": "kos_list_upload", "type": "infile", "format": "ref_rna_v2.anno_upload"},  # 客户上传kegg注释文件
            {"name": "gene_file", "type": "infile", "format": "ref_rna_v2.gene_list"},
            {"name": "length_file", "type": "infile", "format": "ref_rna_v2.cog_list"},  # 注释转录本序列的长度
            {"name": "new_gtf", "type": "infile", "format": "ref_rna_v2.gtf"},  # 新转录本gtf文件
            {"name": "gtf", "type": "infile", "format": "ref_rna_v2.gtf"},  # 参考基因组gtf文件
            ...
            {"name": "type", "type": "string", "default": "new"},
            {"name": "des_type", "type": "string", "default": "type3"},  # 基因描述文件类型
            {"name": "enterz", "type": "string", "default": None},
            {"name": "annot_type", "type": "string", "default": "denovo_rna" },
            {"name": "g2t2p", "type": "string", "default": "" }, # 基因转录本蛋白对应关系列表
            {"name": "blast2go_annot", "type": "infile", "format": "ref_rna_v2.blast2go_annot"}
        ]
        self.add_option(options)
        self.swissprot_annot = self.add_tool('ref_rna_v2.annotation.xml2table')
        self.nr_annot = self.add_tool('ref_rna_v2.annotation.xml2table')
        self.eggnog_annot = self.add_tool('ref_rna_v2.annotation.xml2table')
        self.eggnog_cog = self.add_tool('ref_rna_v2.annotation.eggnog2cog')
        self.ncbi_taxon = self.add_tool('ref_rna_v2.annotation.ncbi_taxon')
        self.go_annot = self.add_tool('ref_rna_v2.annotation.go_annotation')
        self.go_upload = self.add_tool('ref_rna_v2.annotation.go_upload')
        self.string_cog = self.add_tool('ref_rna_v2.annotation.string2cogv9')
        self.kegg_annot = self.add_tool('ref_rna_v2.annotation.kegg_annotation')
        self.swissprot_annot = self.add_tool("ref_rna_v2.annotation.swissprot")
        self.kegg_upload = self.add_tool('ref_rna_v2.annotation.kegg_upload')
        self.anno_stat = self.add_tool('ref_rna_v2.annotation.ref_anno_stat')
        self.anno_query = self.add_tool('ref_rna_v2.annotation.ref_anno_query')
        self.step.add_steps('blast_statistics', 'nr_annot', 'ncbi_taxon', 'go_annot', 'go_upload', 'kegg_annot', 'kegg_upload', 'eggnog_annot', 'eggnog_cog', 'cog_annot', 'anno_stat', 'swissprot_annot', 'anno_query')
        ...
        if self.option('annot_type') == "denovo_rna":
            if self.option('anno_statistics'):
                if not self.option('gene2trans').is_set:
                    if self.option('gtf').is_set and self.option('g2t2p') != "":
                        ...
                        raise OptionError('进行注释统计的tool必须要设置gtf+ g2t2p 或者 gene2trans', code = "23700101")
            ...
            if self.option('anno_statistics'):
                if not self.option('gene_file').is_set:
                    raise OptionError('进行注释统计的tool必须要设置gene_file', code = "23700102")
                if not self.option('ref_genome_gtf').is_set and not self.option('new_gtf').is_set:
                    raise OptionError('缺少gtf文件', code = "23700103")
                if not self.option("length_file").is_set:
                    raise OptionError("缺少注释转录本序列的长度文件", code = "23700104")
    ...
    def run(self):
        ...
        if len(self.all_end_tool) > 1:
            ...
            self.set_error('不需要进行任何注释工作', code="23700105")
    ...
    def run_annot_stat(self):
        ...
        if 'kegg' in self.anno_database:
            ...
            opts['known_ko'] = self.option('known_ko')
        ...
        # 参考注释使用黄色标记
        if self.option('type') == "ref":
            opts['link_bgcolor'] = "yellow"
            opts['png_bgcolor'] = "#FFFF00"
    ...
    def set_xml(self):
        ...
        os.mkdir(xml_dir)
        # 上传xml结果
        # os.link(self.option('blast_nr_xml').prop['path'], os.path.join(xml_dir, "Trinity_vs_nr.xml") )
        # os.link(self.option('blast_swissprot_xml').prop['path'], os.path.join(xml_dir, "Trinity_vs_swissprot.xml") )
        # os.link(self.option('blast_string_xml').prop['path'], os.path.join(xml_dir, "Trinity_vs_string.xml") )
        # os.link(self.option('blast_kegg_xml').prop['path'], os.path.join(xml_dir, "Trinity_vs_kegg.xml") )
    ...
    def linkdir(self, olddir, newname, mode='link'):
        ...
        if not os.path.isdir(olddir):
            self.set_error('需要移动到output目录的文件夹不存在。', code = "23700106")
