...
class RnaseqMappingModule(Module):
    ...
    def __init__(self, work_id):
        ...
        options = [
            ...
            {"name": "assemble_method", "type": "string", "default": "None"},  # 拼接方法，cufflinks or stringtie
    ...
    def get_opts(self):
        ...
        if self.option("strand_specific"):
            self.tool_opts.update({"strand_specific": True})
            if self.option("mapping_method").lower() != "star":
                self.tool_opts.update({'strand_direct': self.option('strand_direct')})
        return True
        ...
        if self.option("seq_method") == "PE":
            for f in sorted(self.samples):
                ...
                if tool == "star":
                    ...
                    mapping_tool = self.add_tool('medical_transcriptome.' + tool)
    ...
    def run_star(self):
        ...
        tool = None
        for f in sorted(self.samples):
            tool = self.add_tool("medical_transcriptome.star")
            if self.option("seq_method") == "PE":
                fq_l = os.path.join(self.option('fastq_dir').prop["path"], self.samples[f]["l"])
                fq_r = os.path.join(self.option('fastq_dir').prop["path"], self.samples[f]["r"])
                opts.update({
    ...
    def set_output(self, event):
        ...
        if event["data"] == "hisat":
            ...
            with open(Comparison_results, "w") as map_info:
                map_info.write("Sample" + "\t" + "Total_reads" + "\t" + "Total_mapped" + "\t" + "Multiple_mapped" +
                               "\t" + "Unique_mapped" + "\n")
                ...
                for fs in stat_files:
                    ...
                    if len(values) == 13:
                        ...
                        multi_mapped = str(multi_mapped) + "(" + str(
                            float("%0.4f" % (multi_mapped / int(total_reads))) * 100) + "%" + ")"
                    map_info.write("{}\t{}\t{}\t{}\t{}\n".format(
                        specimen_name, total_reads, map_reads, multi_mapped, uniq_mapped))
                ...
                for fs in stat_files:
                    ...
                    for line in f:
                        ...
                        if re.match(r"Right", line):
                            ...
                            map_reads += int(f.next().split()[2])
                            multiple += int(f.next().split()[2])
                    mapping_reads_final = str(map_reads) + "(" + str(
                        float("%0.4f" % (map_reads / total_reads)) * 100) + "%" + ")"
                    multiple_mapped_final = str(multiple) + "(" + str(
                        float("%0.4f" % (multiple / total_reads)) * 100) + "%" + ")"
                    uniq_mapped_final = str(total_reads - multiple) + "(" + str(
                        float("%0.4f" % ((map_reads - multiple) / total_reads)) * 100) + "%" + ")"
                    map_info.write("{}\t{}\t{}\t{}\t{}\n".format(
                        specimen_name, total_reads, mapping_reads_final, multiple_mapped_final, uniq_mapped_final))
        else:
            ...
            with open(Comparison_results, "w") as map_info:
                ...
                for fs in stat_files:
                    ...
                    if int(values[5]) > 150:
                        ...
                        map_info.write("{}\t{}\t{}\t{}\t{}\n".format(
                            specimen_name, total_reads, map_reads_f, multi_mapped_f, uniq_mapped_f))
                    else:
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            "type": "module",
            "name": "medical_transcriptome.rnaseq_mapping",
            "instant": False,
            "options": dict(
                ...
                seq_method="PE",
                mapping_method="star",
                fastq_dir = "/mnt/ilustre/users/sanger-dev/workspace/20200807/Refrna_tsg_38276/FastpRna/output/fastq",
                # fastq_dir="/mnt/ilustre/users/sanger-dev/workspace/20190617/Single_HiseqQc_437/HiseqQc/output"
                #           "/sickle_dir",
