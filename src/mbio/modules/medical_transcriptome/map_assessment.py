...
class MapAssessmentModule(Module):
    def __init__(self, work_id):
        ...
        options = [
            ...
            {'name': 'bam', 'type': 'infile', 'format': 'align.bwa.bam_dir'},  # bam格式文件,排序过的
    ...
    def satur_run(self):
        n = 0
        for f in self.files:
            satur = self.add_tool('medical_transcriptome.mapping.rpkm_saturation')
    ...
    def coverage_new_run(self):
        ...
        n = 0
        for f in self.files:
            coverage_new = self.add_tool('medical_transcriptome.mapping.coverage')
    ...
    def coverage_run(self):
        n = 0
        for f in self.files:
            coverage = self.add_tool('medical_transcriptome.mapping.coverage')
            self.step.add_steps('coverage_{}'.format(n))
    ...
    def distribution_run(self):
        n = 0
        for f in self.files:
            distribution = self.add_tool('medical_transcriptome.mapping.bam_readsdistribution')
    ...
    def chr_stat_run(self):
        n = 0
        for f in self.files:
            chr_stat = self.add_tool('medical_transcriptome.mapping.chr_distribution')
...
class TestFunction(unittest.TestCase):
    def test(self):
        ...
        data = {
            'id': 'map_assessment_{}_{}'.format(random.randint(1000, 9999), random.randint(1000, 9999)),
            'type': 'module',
            'name': 'medical_transcriptome.map_assessment',
            'instant': False,
            'options': dict(
                bam='/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38312/RnaseqMapping/output/bam',
                # bam='/mnt/ilustre/users/sanger-dev/sg-users/qinjincheng/ref_rna_v3/mouse/bam',
                analysis='saturation,coverage,distribution,chr_stat',
                bed = "/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38312/FileCheck/Mus_musculus.GRCm38.100.gtf.bed"
