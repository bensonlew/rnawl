...
class RefrnaAssembleModule(Module):
    ...
    def __init__(self, work_id):
        ...
        options = [
            ...
            {"name": "sample_gtf", "type": "outfile", "format": "medical_transcriptome.gtf"},  # 输出的gtf文件
            {"name": "merged_gtf", "type": "outfile", "format": "medical_transcriptome.gtf"},  # 输出的合并文件
            {"name": "cuff_gtf", "type": "outfile", "format": "medical_transcriptome.gtf"},  # compare后的gtf文件
            {"name": "new_transcripts_gtf", "type": "outfile", "format": "medical_transcriptome.gtf"},  # 新转录本注释文件
            {"name": "new_transcripts_fa", "type": "outfile", "format": "medical_transcriptome.fasta"},  # 新转录本序列文件
            {"name": "all_transcripts_fa", "type": "outfile", "format": "medical_transcriptome.fasta"},  # 新转录本+已知转录本序列文件
            {"name": "new_gene_gtf", "type": "outfile", "format": "medical_transcriptome.gtf"},  # 新基因注释文件
            {"name": "change_id_gtf", "type": "outfile", "format": "medical_transcriptome.gtf"},  # 转换ID后的注释文件
            {"name": "ref_and_new_gtf", "type": "outfile", "format": "medical_transcriptome.gtf"},
            {"name": "trans2gene", "type": "outfile", "format": "medical_transcriptome.common"},
    ...
    def stringtie_run(self):
        ...
        for f in samples:
            ...
            stringtie = self.add_tool('medical_transcriptome.assemble.stringtie')
    ...
    def stringtie_merge_run(self):
        self.get_list()
        stringtie_merge = self.add_tool("medical_transcriptome.assemble.stringtie_merge")
    ...
    def cufflinks_run(self):
        ...
        for f in samples:
            f = os.path.join(self.option('sample_bam_dir').prop['path'], f)
            cufflinks = self.add_tool('medical_transcriptome.assemble.cufflinks')
    ...
    def cuffmerge_run(self):
        self.get_list()
        self.cuffmerge = self.add_tool("medical_transcriptome.assemble.cuffmerge")
    ...
    def gffcompare_run(self):
        ...
        gffcompare = self.add_tool("medical_transcriptome.assemble.gffcompare")
            ...
            if not os.path.exists(merged_gtf) and not os.path.exists(tmap):
                ...
                if not os.path.exists(merged_gtf) and not os.path.exists(tmap):
                    ...
                    if not os.path.exists(merged_gtf) and not os.path.exists(tmap):
                        ...
                        merged_gtf = os.path.join(self.work_dir, "StringtieMerge__3/output/merged.gtf")
        new_transcripts = self.add_tool("medical_transcriptome.assemble.new_transcripts")
    ...
    def refassemble_stat_run(self):
        ...
        self.refassemble_stat = self.add_tool("medical_transcriptome.assemble.refassemble_stat")
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data5 = {
            ...
            "type": "module",
            "name": "medical_transcriptome.refrna_assemble",
            "instant": False,
            "options": dict(
                ...
                sample_bam_dir="/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/RnaseqMapping/output/bam",
                fr_stranded="fr-unstranded",
                ref_gtf="/mnt/ilustre/users/sanger-dev/workspace/20200810/Refrna_tsg_38314/FileCheck/Homo_sapiens.GRCh38.96.gtf",
