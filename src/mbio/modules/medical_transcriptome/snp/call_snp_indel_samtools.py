...
class CallSnpIndelSamtoolsModule(Module):
    """
    star:序列比对
    picard:处理比对结果sam文件
    gatk：snp calling软件
    ...
    def __init__(self, work_id):
        super(CallSnpIndelSamtoolsModule, self).__init__(work_id)
        options = [
        ...
        self.samtools_path = Config().SOFTWARE_DIR + "/miniconda2/bin/samtools"
    ...
    def make_bamlist(self):
        with open(os.path.join(self.work_dir, "bam_list"), "w") as blst:
            for i in sorted(os.listdir(os.path.join(self.option("in_bam").prop["path"]))):
                if i.endswith("bam"):
                    bam_path = os.path.join(self.option("in_bam").prop["path"], i)
                    blst.write(bam_path + "\n")
    ...
    def samtool_run(self):
        ...
        for m in files_list:
            self.samtool = self.add_tool('medical_transcriptome.snp.snp')
            self.samtool.set_options({
                "trinity_fa": self.option("ref_genome_custom"),
                "bamlist": self.bam_list,
                "bed_file": m,
            })
    ...
    def bcftools_vcf_run(self):
        self.bcftools_vcf = self.add_tool("medical_transcriptome.snp.bcftool_vcf")
        self.get_vcf_list()
    ...
    def vcf_filter_run(self):
        self.vcf_filter=self.add_tool('medical_transcriptome.snp.vcf_filter_samtools')
    ...
    def snp_anno(self):
        annovar = self.add_tool('medical_transcriptome.snp.annovar')
    ...
    def predeal(self, event):
        ...
        predeal = self.add_tool('medical_transcriptome.snp.predeal_snpresults')
        options = {
            "method": "gatk",
    ...
    def run(self):
        super(CallSnpIndelSamtoolsModule, self).run()
    ...
    def end(self):
        ...
        super(CallSnpIndelSamtoolsModule, self).end()
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "snp_test_samtools" + str(random.randint(1, 10000))+"-xxx",
            "type": "module",
            "name": "whole_transcriptome.call_snp_indel_samtools",
            "instant": False,
            "options": dict(
