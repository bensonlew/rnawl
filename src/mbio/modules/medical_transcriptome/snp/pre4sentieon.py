...
class Pre4sentieonModule(Module):
    ...
    def __init__(self, work_id):
        ...
        options = [
            ...
            {"name": "algorithm", "type": "string", "default": "HaplotypeCaller"}  # 算法选择：HaplotypeCaller,DNAscope add by fwy 20201130
        ]
    ...
    def run_bam_addrg(self):
        self.bam_addrg=self.add_tool("medical_transcriptome.snp.bam_addrg")
        opts = {
        ...
        if self.option("align_method").lower() == "tophat":
            opts.update({"in_reference" : self.option("ref_fasta").prop["path"]})
        self.bam_addrg.set_options(opts)
    ...
    def run_bam_sort_index(self):
        self.bam_sort_index=self.add_tool("medical_transcriptome.snp.bam_sort")
        if self.option("analysis_format").lower() == "bam":
    ...
    def run_bam2cram(self):
        self.bam2cram = self.add_tool("medical_transcriptome.snp.bam2cram")
    ...
    def run_sentieon_haplptyper(self):
        self.sentieon_haplptyper=self.add_tool("medical_transcriptome.snp.sentieon_haplotyper")
