...
class OnlyTestModule(Module):
    ...
    def __init__(self, work_id):
        super(OnlyTestModule, self).__init__(work_id)
        options = [
    ...
    def picard_run(self):
        for i in os.listdir(self.option("in_bam").prop["path"]):
            ...
            picard = self.add_tool('medical_transcriptome.picard_rna')
            picard.set_options({
                "in_bam": f_path
            })
    ...
    def gatk_run(self, event):
        ...
        gatk = self.add_tool('medical_transcriptome.gatk')
    ...
    def snp_anno(self, event):
        ...
        if self.end_times == len(self.samples):
            annovar = self.add_tool('medical_transcriptome.annovar')
            options = {
    ...
    def predeal(self, event):
        obj = event["bind_object"]
        predeal = self.add_tool('medical_transcriptome.predeal_snpresults')
        options = {
            "method": "gatk",
