...
class HiseqQcModule(Module):
    ...
    def check_options(self):
        ...
        if not self.option("fastq_dir").is_set:
            raise OptionError("需要传入fastq文件或者文件夹", code = "23700501")
        if self.option("fastq_dir").is_set:
            list_path = os.path.join(self.option("fastq_dir").prop["path"], "list.txt")
            if not os.path.exists(list_path):
                OptionError("缺少list文件", code = "23700502")
            row_num = len(open(list_path, "r").readline().split())
            if self.option('fq_type') == "PE" and row_num != 3:
                raise OptionError("PE序列list文件应该包括文件名、样本名和左右端说明三列", code = "23700503")
            elif self.option('fq_type') == "SE" and row_num != 2:
                raise OptionError("SE序列list文件应该包括文件名、样本名两列", code = "23700504")
        ...
        with open(os.path.join(self.output_dir, "rawdata_md5"), "w") as m:
            with open(list_path, "r") as l:
                for line in l.readlines():
                    ...
                    m.write(line[1] + "\t")
                    md5_num = self.get_md5num(os.path.join(self.option("fastq_dir").prop["path"], line[0]))
                    m.write(md5_num + "\n")
    ...
    def fastp_run(self):
        ...
        for f in self.samples:
            ...
            fastp = self.add_tool('medical_transcriptome.fastp')
    ...
    def clipper_run(self):
        ...
        for f in self.samples:
            fq_s = os.path.join(self.option("fastq_dir").prop["path"], self.samples[f])
            clipper = self.add_tool('medical_transcriptome.fastx_clipper')
            self.step.add_steps('clipper_{}'.format(n))
            clipper.set_options({
                "fastq_s": fq_s,
                'adapter':self.option("adapter_a")
            })
    ...
    def clipper_run_phred64(self):
        ...
        self.samples = self.get_list()
        for f in self.samples:
            ## 增加判断的原因是跑hiseqreadsstat时已经对文件完成解压并修改list.txt文件，会导致后续运行失败
            if os.path.exists(os.path.join(self.work_dir + "/fastp_output", self.samples[f])):
                fq_s = os.path.join(self.work_dir + "/fastp_output", self.samples[f])
            elif os.path.exists(os.path.join(self.work_dir + "/fastp_output", self.samples[f]) + ".gz"):
                fq_s = os.path.join(self.work_dir + "/fastp_output", self.samples[f]) + ".gz"
            elif os.path.exists(os.path.join(self.work_dir + "/fastp_output",self.samples[f].split(".")[0]+"fq.gz")):
                fq_s=os.path.join(self.work_dir + "/fastp_output",self.samples[f].split(".")[0]+".fq.gz")
            clipper = self.add_tool('medical_transcriptome.fastx_clipper')
            self.step.add_steps('clipper_{}'.format(n))
            clipper.set_options({
                "fastq_s": fq_s,
    ...
    def seqprep_run(self):
        ...
        for f in self.samples:
            ...
            seqprep = self.add_tool('medical_transcriptome.seq_prep')
    ...
    def sickle_se_run(self, event):
        ...
        if not os.path.exists(obj.output_dir + "/" + event["data"] + "_clip_s.fastq"):
    ...
    def run(self):
        super(HiseqQcModule, self).run() ## 初始化时间比较久，会影响监听的触发时间，需要一开始就super run
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "HiseqQc_SE_33_sada_" + str(random.randint(1, 10000)),
            "type": "module",
            "name": "medical_transcriptome.hiseq_qc",
            "instant": False,
            "options": dict(
                ...
                fq_type="PE",
                adapter_a="AGATCGGAAGAGCACACGTC",
                adapter_b="AGATCGGAAGAGCGTCGTGT",
