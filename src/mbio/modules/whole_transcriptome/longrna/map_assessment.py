...
class MapAssessmentModule(Module):
    ...
    def satur_run(self):
        n = 0
        for f in self.files:
            satur = self.add_tool('ref_rna_v3.mapping.rpkm_saturation')
    ...
    def coverage_new_run(self):
        ...
        n = 0
        for f in self.files:
            coverage_new = self.add_tool('ref_rna_v3.mapping.coverage')
    ...
    def coverage_run(self):
        n = 0
        for f in self.files:
            coverage = self.add_tool('ref_rna_v3.mapping.coverage')
            self.step.add_steps('coverage_{}'.format(n))
    ...
    def distribution_run(self):
        n = 0
        for f in self.files:
            distribution = self.add_tool('ref_rna_v3.mapping.bam_readsdistribution')
    ...
    def chr_stat_run(self):
        n = 0
        for f in self.files:
            chr_stat = self.add_tool('ref_rna_v3.mapping.chr_distribution')
...
class TestFunction(unittest.TestCase):
    def test(self):
        ...
        data = {
            ...
            'type': 'module',
            'name': 'ref_rna_v3.map_assessment',
            'instant': False,
            'options': dict(
                bam='/mnt/ilustre/users/sanger-dev/workspace/20191031/Longrna_tsg_36023/RnaseqMapping/output/bam',
                analysis='saturation,coverage,distribution,chr_stat',
                bed='/mnt/ilustre/users/sanger-dev/workspace/20191031/Longrna_tsg_36023/FileCheck/Homo_sapiens.GRCh38.96.gtf.bed'
            )
        }
