...
class CallSnpIndelGatkModule(Module):
    ...
    def __init__(self, work_id):
        ...
        self.gatks = []
        self.bampredeals = []
        self.step.add_steps('samtools_presnp', 'gatk',"vcffilter")  # 添加步骤
        self.count = 0
    ...
    def check_options(self):
        ...
        if self.option("ref_genome") == "customer_mode" and not self.option("ref_genome_custom").is_set:
            raise OptionError("请传入自定义参考序列!", code = "25600202")
    ...
    def run_bam_predeal(self):
        for i in os.listdir(self.option("in_bam").prop["path"]):
            ...
            bampredeal.on("end", self.gatk_run, i[:-4])  # event["data"]传为样本名称
        if len(self.bampredeals) == 1:
            self.bampredeals[0].on("end", self.finish_update, "samtools_presnp")
        else:
            self.on_rely(self.bampredeals, self.finish_update, 'samtools_presnp')
    ...
    def gatk_run(self, event):
        ...
        gatk = self.add_tool('whole_transcriptome.snp.gatk')
    ...
    def vcf_filter_run(self,event):
        ...
        if self.end_times == len(self.samples):
            ...
            self.vcffilter.on("end",self.snp_anno, event['data'] )
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            "type": "module",
            "name": "whole_transcriptome.call_snp_indel_gatk",
            "instant": False,
            "options": dict(
                ref_genome="customer_mode",
