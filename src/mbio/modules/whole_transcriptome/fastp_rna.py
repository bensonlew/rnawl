...
class FastpRnaModule(Module):
    ...
    def fastp_run(self):
        n = 0
        fastq_dir = os.path.dirname(self.option('sample_path').prop['path'])
        for sample in self.sample_info:
            fastp = self.add_tool('datasplit.fastp_rna')
            self.step.add_steps('fastp{}'.format(n))
            opts = {
                "fq1": os.path.join(fastq_dir, self.sample_info[sample][0]),
                "fq2": os.path.join(fastq_dir, self.sample_info[sample][1]),
    ...
    def get_info(self):
        ...
        with open(self.option('sample_path').prop['path'])as fr:
            for line in fr:
                ...
                if len(tmp) != 3:
                    raise Exception(
                        '进行fastp的sample_path：{}文件必须是三列，请检查！'.format(self.option('sample_path').prop['path']))
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "LncrnaQc_fastp_" + str(random.randint(1, 10000)),
            "type": "module",
            "name": "whole_transcriptome.fastp_rna",
            "instant": False,
            "options": dict(
                sample_path="/mnt/ilustre/users/sanger-dev/sg-users/shicaiping/whole_transcriptome/demo_human/longrna"
                            "/rawdata/list.txt",
            )
        }
    ...
    def test_medip(self):
        ...
        from biocluster.wsheet import Sheet
        data = {
            'id': 'fastp_rna_{}_{}'.format(random.randint(1000, 9999), random.randint(1000, 9999)),
            'type': 'module',
            'name': 'whole_transcriptome.fastp_rna',
            'instant': False,
            'options': dict(
                sample_path='/mnt/ilustre/users/sanger-dev/sg-users/qinjincheng/medip/raw_data/list.txt',
            )
        }
        wsheet = Sheet(data=data)
        wf = SingleWorkflow(wsheet)
    ...
    def test_bacilli(self):
        ...
        data = {
            ...
            'instant': False,
            'options': dict(
                sample_path='/mnt/ilustre/users/sanger-dev/sg-users/qinjincheng/data/Geobacillus_thermodenitrificans'
                            '/raw_data/list.txt',
            )
        }
        wsheet = Sheet(data=data)
...
if __name__ == '__main__':
    ...
    suite.addTests([TestFunction('test_bacilli')])
