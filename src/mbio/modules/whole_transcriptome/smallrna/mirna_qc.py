...
class MirnaQcModule(Module):
    ...
    def run_mirna_qc(self):
        ...
        for s in self.fastqs.keys():
            ...
            self.single_mirna_qc  = self.add_module("whole_transcriptome.smallrna.single_mirna_qc")
    ...
    def set_output(self, event):
        ...
        for f in os.listdir(olddir):
            ...
            if re.search(r".+.trimmed", f):
                f1 = sample + "_clip_s.fastq.trimmed"
            if re.search(r".+.length.txt", f):
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "whole_transcriptome.smallrna.mirna_qc" + datetime.datetime.now().strftime('%H-%M-%S'),
            "type": "module",
            "name": "whole_transcriptome.smallrna.mirna_qc",
            "instant": False,
            "options": dict(
                list_file = test_dir + "/" + "list_re.txt",
            )
        }
