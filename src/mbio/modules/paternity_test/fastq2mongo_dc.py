...
class Fastq2mongoDcModule(Module):
    ...
    def __init__(self, work_id):
        ...
        self.fastq2tab = self.add_tool("paternity_test.family2tab_dc")
        self._end_info = 0
    ...
    def fastq2tab_run(self):
        ...
        self.fastq2tab.on('end', self.set_output, 'fastq2tab')
        self.fastq2tab.on('start', self.set_step, {'start': self.step.fastq2tab})
        self.fastq2tab.on('end', self.set_step, {'end': self.step.fastq2tab})
        self.fastq2tab.on('end', self.end)
        self.fastq2tab.run()
    ...
    def set_output(self, event):
        ...
        elif event['data'] == 'fastq2tab':
            self.linkdir(obj.output_dir, 'fastq2tab')

    def run(self):
        super(Fastq2mongoDcModule, self).run()
        if self.option('type') == 'pt' or self.option('type') == 'ppt':
            self.fastq2bam.on('end', self.bam2tab_run)
            self.fastq2bam_run()
        elif self.option('type') == 'dcpt':
            self.fastq2tab_run()
    ...
    def end(self):
        ...
        # print regexps
        super(Fastq2mongoDcModule, self).end()