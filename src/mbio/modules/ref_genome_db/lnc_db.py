...
class LncDbModule(Module):
    def __init__(self, work_id):
        ...
        :param wsheet_object:
        """
        super(LncDbModule, self).__init__(work_id)
        options = [
            {'name': 'ref_fa', 'type': 'infile', 'format': 'lnc_rna.lnc_fasta'},
            {'name': 'ref_gtf', 'type': 'infile', 'format': 'lnc_rna.lnc_gtf'},
            {'name': 'ref_lnc_list', 'type': 'infile', 'format': 'ref_genome_db.common'},
            {'name': 'ref_lnc_gtf', 'type': 'infile', 'format': 'ref_genome_db.gtf'},
            {'name': 'ref_lnc_db', 'type': 'string', 'default': ''},
    ...
    def fa_filter(self):
        ...
        for db_name, dic in self.params_dic.items():
            ...
            if 'ids_mapping'in dic and os.path.exists(dic["ids_mapping"]):
                ...
                if dic['db_name'].lower() == "ncbi" and self.option("ref_lnc_db").lower() == "ensembl":
                    tool_params["ids_mapping"] = self.config.SOFTWARE_DIR + "/lnc_rna/ncbi/ncbi_rna_2_ensembl_rna.txt"
                elif dic['db_name'].lower() == "noncode" and self.option("ref_lnc_db").lower() == "ensembl":
                    tool_params["ids_mapping"] = self.config.SOFTWARE_DIR + "/lnc_rna/noncode/noncode_id_2_ensembl_id.txt"
        ...
        if params_list:
            ...
            options = {
                'params': params_str,
                'ref_db_name': self.option("ref_lnc_db"),
                'ref_lnc_list': self.option("ref_lnc_list").prop["path"]
            }
            ...
            self.single_db_tool.set_options(dict(
                ...
                gtf=self.option('ref_lnc_gtf').prop["path"],
                db_name=self.option("ref_lnc_db")
            ))
            ...
            self.single_db_tool.on('end', self.set_output)
    ...
    def seq_align(self):
        ...
        params_dic = {
            ...
            'ref_db_gtf': self.option("ref_lnc_gtf").prop["path"],
    ...
    def merge_info(self):
        ...
        self.merge_info_tool.on('end', self.set_output)
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        params = [
            {
                'db_name': 'ncbi',
                'fasta': '/mnt/ilustre/users/sanger-dev/sg-users/shicaiping/ref_genome_db/Homo_sapiens/ncbi/ncbi_lncrna.fa',
                'ids_mapping': '/mnt/ilustre/users/sanger-dev/app/database/lnc_rna/ncbi/ncbi_rna_2_ensembl_rna.txt'
            },
            {
                'db_name': 'noncode',
                'fasta': '/mnt/ilustre/users/sanger-dev/app/database/lnc_rna/noncode/NONCODEv5_human.fa',
                'ids_mapping': '/mnt/ilustre/users/sanger-dev/app/database/lnc_rna/noncode/noncode_id_2_ensembl_id.txt'
            }
        ]
        params = json.dumps(params)
        data = {
            'id': 'lnc_db_japonica_{}_{}'.format(random.randint(1000, 10000), random.randint(1000, 10000)),
            'type': 'module',
            'name': 'ref_genome_db.lnc_db',
            'options': {
                # {'name': 'ref_fa', 'type': 'infile', 'format': 'lnc_rna.lnc_fasta'},
                # {'name': 'gmap_db_dir', 'type': 'infile', 'format': 'lnc_rna.common_dir'},
                # {'name': 'gmap_db_name', 'type': 'string'},
                # {'name': 'dbs_info', 'type': 'string'}
                'ref_fa': '/mnt/ilustre/users/sanger-dev/sg-users/shicaiping/ref_genome_db/Homo_sapiens/ensembl/Homo_sapiens.GRCh38.dna.toplevel.fa',
                'ref_gtf': '/mnt/ilustre/users/sanger-dev/sg-users/shicaiping/ref_genome_db/Homo_sapiens/ensembl/Homo_sapiens.GRCh38.96.gtf',
                'gmap_db_dir': '/mnt/ilustre/users/sanger-dev/sg-users/shicaiping/ref_genome_db/Homo_sapiens/gmap/GRCh38',
                'ref_lnc_gtf': '/mnt/ilustre/users/sanger-dev/workspace/20190530/Single_extract_lncrna_8550_8607/ExtractLncrna/lncrna.gtf',
                'ref_lnc_list': '/mnt/ilustre/users/sanger-dev/workspace/20190530/Single_extract_lncrna_8550_8607/ExtractLncrna/lncrna.txt',
                'ref_lnc_db': 'ensembl',
                'gmap_db_name': 'GRCh38',
                'dbs_info': params
            }
