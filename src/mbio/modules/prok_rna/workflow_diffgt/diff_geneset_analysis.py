...
class DiffGenesetAnalysisModule(Module):
    def __init__(self, work_id):
        ...
        options = [
            ...
            {'name': 'cog_class', 'type': 'infile', 'format': 'medical_transcriptome.common'},
            {'name': 'go_class', 'type': 'infile', 'format': 'medical_transcriptome.common'},
            {'name': 'go_class_detail', 'type': 'infile', 'format': 'medical_transcriptome.common'},
            {'name': 'do_class', 'type': 'infile', 'format': 'medical_transcriptome.common'},
        ]
        self.add_option(options)
        self.go_class_tool = None
        self.kegg_class_tool = self.add_tool("prok_rna.geneset.kegg_class")
        self.go_enrich_tool = self.add_tool("prok_rna.geneset.go_enrich")
        self.kegg_enrich_tool = self.add_module("prok_rna.workflow_diffgt.diff_kegg_enrich")
        ...
        self.analysis_funcs = [self.run_cog_class,self.run_go_class,self.run_kegg_class,self.run_go_enrich,self.run_kegg_enrich]
        self.result_dict = OrderedDict()


    def check_options(self):
        self.logger.info("本次分析参数如下")
        self.logger.info("kegg {}".format(self.option("kegg_version")))
    ...
    def set_output(self):
        ...
        if self.option("genes_num") == 0 :
            ...
            if "diff_cog_class" in self.tools_names:
                ...
                try:
                    os.link(cog_class_path,
                            os.path.join(self.output_dir, "diff_cog_class", os.path.basename(cog_class_path)))
                except:
            ...
            if "diff_go_class" in self.tools_names:
                ...
                go_detail_path = self.option("go_class_detail").prop["path"]
                try:
                    os.link(go_class_path,os.path.join(self.output_dir,"diff_go_class",os.path.basename(go_class_path)))
                    os.link(go_detail_path,
                            os.path.join(self.output_dir, "diff_go_class", os.path.basename(go_detail_path)))
                except:
