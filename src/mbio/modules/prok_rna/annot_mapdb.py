...
class AnnotMapdbModule(Module):
    ...
    def __init__(self, work_id):
        ...
        options = [
            {"name": "query", "type": "infile", "format": "prok_rna.fasta"},  # 输入文件
            {"name": "querypep", "type": "infile", "format": "prok_rna.fasta"},  # 输入文件
            ...
            {"name": "nr_db", "type": "string", "default": "bacteria"},
        ...
        self.catblast = self.add_tool("prok_rna.annotation.cat_blastout")
        self.splitfasta = self.add_tool("prok_rna.annotation.split_fasta")
        self.xmlfilter = self.add_tool("prok_rna.annotation.filter_annot")
        self.step.add_steps('blast2go', 'split_fasta',
                            'blast_nr', 'blast_string', 'blast_kegg', 'blast_swissprot', 'blast_eggnog',
                            'merge_nr', 'merge_string', 'merge_kegg', 'merge_swissprot', 'merge_eggnog', 'merge_cog')
        self.merge_step = {
    ...
    def check_options(self):
        if not self.option("query").is_set:
            raise OptionError("必须设置参数query", code = "25000301")
        if self.option('query_type') not in ['nucl', 'prot']:
            raise OptionError("query_type查询序列的类型为nucl（核酸）或者prot（蛋白）：%s",
                              variables = (self.option('query_type')), code = "25000302")
        if self.option('outfmt') not in [5, 6]:
            raise OptionError("outfmt遵循blast+输出规则，目前只支持5，6输出格式：%s",
                              variables = (self.option('outfmt')), code = "25000303")
        if not 1 > self.option('evalue') >= 0:
            raise OptionError("E-value值设定必须为[0-1)之间：%s",
                              variables = (self.option('evalue')), code = "25000304")
        if not isinstance(self.option('lines'), int):
            raise OptionError("行数必须为整数", code = "25000305")
        return True
    ...
    def run_blast_nr(self):
        ...
        for f in os.listdir(self.splitfasta.output_dir):
            ...
            for opt in ["nr_version"]:
                opts.update({opt: self.option(opt)})

            if self.option('method') == "blast":
                blast_tool = self.add_tool('prok_rna.annotation.blast')
            else:
                blast_tool = self.add_tool('prok_rna.annotation.diamond')
            ...
            nr2go = self.add_tool('prok_rna.annotation.nr2go')
            nr2ncbitaxon = self.add_tool('prok_rna.annotation.ncbi_taxon')
    ...
    def run_nr2go(self, event):
        ...
        for opt in ["pir_version"]:
            options.update({opt: self.option(opt)})
    ...
    def run_blast_string(self):
        ...
        for f in os.listdir(self.splitfasta.output_dir):
            ...
            string2cog_tool = self.add_tool('prok_rna.annotation.string2cog')
    ...
    def run_blast_cog(self):
        '''
        ncbi cog 数据库比对
        '''
        opts = self.blast_opts.copy()
        opts.update({"database": "cog"})
        opts.update({"version": self.option("version")})
        opts.update({"cog_version": self.option("cog_version")})
        opts.update({"diamond_version": self.option("diamond_version")})
        if self.option("diamond_version") >= "v2.0.13":
            opts.update({"sensitive": 1})
        for f in os.listdir(self.splitfasta.output_dir):
            ...
            blast_tool.set_options(opts)
            # blast_tool.run()
            self.blast_cog_tools.append(blast_tool)
        if len(self.blast_cog_tools) == 1:
            self.blast_cog_tools[0].on("end", self.run_catblastout, "cog")
        else:
            self.on_rely(self.blast_cog_tools, self.run_catblastout, "cog")
        for tool in self.blast_cog_tools:
            tool.run()
    ...
    def run(self):
        ...
        for db in self.option("database").split(";"):
            if db == 'nr':
                self.merge_tools['nr'] = self.add_tool("prok_rna.annotation.cat_blastout")
                self.merge_tools['GO'] = self.add_tool("prok_rna.annotation.cat_table")
            elif db == 'string':
                self.merge_tools['string'] = self.add_tool("prok_rna.annotation.cat_blastout")
            elif db == 'swissprot':
                self.merge_tools['swissprot'] = self.add_tool("prok_rna.annotation.cat_blastout")
            elif db == 'kegg':
                self.merge_tools['kegg'] = self.add_tool("prok_rna.annotation.cat_blastout")
            elif db == 'eggnog':
                self.merge_tools['eggnog'] = self.add_tool("prok_rna.annotation.cat_blastout")
            elif db == 'cog':
                self.merge_tools['cog'] = self.add_tool("prok_rna.annotation.cat_blastout")
            else:
                self.logger.info("不支持该类型{}的数据库".format(db))
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            "instant": False,
            "options": dict(
                query = test_dir + "/test_data1/" + "ref_cds_from_genomic.fna",
                # database="nr",
                method = "diamond",
                nr_db = "bacteria",
                lines = 5000
