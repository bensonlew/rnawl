...
class DiffexpBatchNewModule(Module):
    ...
    def __init__(self, work_id):
        ...
        options = [
            ...
            dict(name='exp_matrix', type='infile', format='denovo_rna_v2.express_matrix'),
    ...
    def set_output(self):
        ...
        diff_option_file = os.path.join(self.tool.work_dir, "option_file")
        if os.path.exists(os.path.join(self.work_dir, "option_file")):
            os.remove(os.path.join(self.work_dir, "option_file"))
        os.link(diff_option_file, os.path.join(self.work_dir, "option_file"))
        self.end()
    ...
    def run_tool(self):
        opts = dict(
            is_workflow=self.option("is_workflow"),
    ...
    def run_uniform(self):
        ...
        diff_method = diff_options["method"]
        opts = {
             # 'method': self.option('method'),
            'method':diff_method,
        ...
        if self.option('method').lower() in ['deseq2', 'degseq', 'edger', 'limma']:
            pvalue_pajdust = diff_options["pvalue_padjust"]
            opts.update({'pvalue_padjust': pvalue_pajdust})
