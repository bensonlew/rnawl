...
class AnnotOrfpfamModule(Module):
    ...
    def __init__(self, work_id):
        ...
        options = [
            ...
            {"name": "pep", "type": "infile", "format": "prok_rna.fasta"},
            ...
            {"name": "gtf", "type": "infile", "format": "prok_rna.gtf"},#实际为一个文件，只是不检查
    ...
    def check_options(self):
        ...
        if self.option('fasta').is_set:
            ...
            if not self.option('gtf').is_set:
                raise OptionError('指定蛋白序列时需同时指定gtf文件')
            if self.option('g2t2p') == "":
                raise OptionError('指定蛋白序列时需同时指定基因转录本蛋白对应关系文件')
            '''
        else:
            raise OptionError("必须输入fasta, 序列或蛋白序列", code = "25000401")
        '''
        if self.option("isoform_unigene") == "" and not self.option('gtf').is_set:
            raise OptionError('必须提供gtf文件或transcript基因对应关系文件')
        '''
        return True
    ...
    def longorf_run(self):
        self.longorf = self.add_tool('prok_rna.annotation.longorf')
        opts = ({
    ...
    def run_hmm(self):
        ...
        for f in os.listdir(self.splitfasta.output_dir):
            ...
            hmm_tool = self.add_tool('prok_rna.annotation.hmm')
    ...
    def run_cathmmout(self):
        ...
        self.predict_tool = self.add_tool('prok_rna.annotation.predict')
