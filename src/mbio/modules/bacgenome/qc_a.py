...
class QcAModule(Module):
    """
    fastp的质控module
    """

    def __init__(self, work_id):
        super(QcAModule, self).__init__(work_id)
        option = [
        ...
        self.qc_tool = self.add_module("meta.qc.fastp_qc")
    ...
    def run_raw_stat(self):
        self.raw_stat_tool.set_options({
            "list": self.raw_list,
    ...
    def run_qc_stat(self):
        data_path = self.set_clean_output()
        self.qc_stat_tool.set_options({
            "list": data_path + "/list.txt",
            "fastq_dir": data_path
        })
    ...
    def set_clean_output(self):
        data_path = os.path.join(self.output_dir, "cleandata")
        if not os.path.isdir(data_path):
            os.mkdir(data_path)
        qc_dir = os.path.join(self.qc_tool.output_dir, "after_qc_dir")
        with open(qc_dir + "/list.txt", "r") as f1, open(data_path + "/list.txt", "w") as f2:
            lines = f1.readlines()
            for line in lines:
                tmp = line.strip().split("\t")
                file_name = tmp[0]
                new_file_name = re.sub(r"fastq$", "fq", file_name)
                link_file(qc_dir + "/" + file_name, data_path + "/" + new_file_name)
                f2.write(new_file_name + "\t" + tmp[1] + "\t" + tmp[2] + "\n")
        return data_path
