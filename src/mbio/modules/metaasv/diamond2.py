...
class Diamond2Module(Module):
    def __init__(self, work_id):
        super(Diamond2Module, self).__init__(work_id)
    ...
    def check_options(self):
        ...
        if self.option('outfmt') not in [5, 6]:
            raise OptionError('outfmt遵循blast+输出规则，目前只支持5，6输出格式：%s', variables=(self.option('outfmt')), code="21100203")
    ...
    def run_blast(self):
        ...
        for f in os.listdir(self.splitfasta.output_dir):
            ...
            blast_tool = self.add_tool('meta.diamond2')
