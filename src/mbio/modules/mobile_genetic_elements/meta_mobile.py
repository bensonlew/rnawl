...
class MetaMobileModule(Module):
    """
    宏基因组数据可移动元件的预测.分五部分，IS、island、integron、prompter、transposon
    author: gaohao
    last_modify: 2020.11.04
    """
    def __init__(self, work_id):
        super(MetaMobileModule, self).__init__(work_id)
        options = [
        ...
        self.island = self.add_module('mobile_genetic_elements.mg_island')
    ...
    def run_mobile_stat(self):
        ...
        if os.path.getsize(self.enzyme.output_dir + "/" + self.option("sample") + ".enzyme.xls") >0 :
        ...
        if os.path.exists(self.integron.output_dir + "/" + self.option("sample") + ".integrons"):
            num = self.get_num(self.integron.output_dir + "/" + self.option("sample") + ".integrons")
            if num >2:
    ...
    def run(self):
        ...
        :return:
        """
        super(MetaMobileModule, self).run()
    ...
    def end(self):
        super(MetaMobileModule, self).end()
