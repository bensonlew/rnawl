...
class MicrobialGenomeQcModule(Module):
    """
    微生物基因组质控，MP、PE文库用同一种质控方式 20181107
    """
    def __init__(self, work_id):
        super(MicrobialGenomeQcModule, self).__init__(work_id)
        options = [
            {"name": "sample_path", "type": "infile", "format": "sequence.file_sample"},  # 第一列fastq路径，第二列样本名，第三列序列类型 l or r
        ...
        with open(self.option('sample_path').prop['path'])as fr:
            for line in fr:
                ...
                try:
                    ...
                    self.logger.info('需要质控的序列样本{}没有相关的样本信息，请核实！'.format(tmp[1]))
        ...
        for f in os.listdir(old):
            ...
            if name:
                if f.endswith("clean.1.fastq.gz"):
                    f2 = new + "/" + name + ".clean.1.fq.gz"
                else:
                    f2 = new + "/" + name + ".clean.2.fq.gz"
            else:
    ...
    def set_output(self, event):
        ...
        self.end_times += 1
        if self.end_times == len(self.mp_samples):
            self.end()
