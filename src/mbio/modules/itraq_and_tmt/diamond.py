...
class DiamondModule(Module):
    def __init__(self, work_id):
        ...
        self.catblast = self.add_tool("itraq_and_tmt.annotation.cat_blastout")
        self.splitfasta = self.add_tool("itraq_and_tmt.annotation.split_fasta")
        self.xmlfilter = self.add_tool("itraq_and_tmt.annotation.filter_annot")
    ...
    def check_options(self):
        if not self.option("query").is_set:
            raise OptionError("必须设置参数query", code = "22500101")
        if self.option('query_type') not in ['nucl', 'prot']:
            raise OptionError('query_type查询序列的类型为nucl(核酸)或者prot(蛋白):%s', variables = (self.option('query_type')), code = "22500102")
        if self.option('outfmt') not in [5, 6]:
            raise OptionError('outfmt遵循blast+输出规则，目前只支持5，6输出格式：%s', variables = (self.option('outfmt')), code = "22500103")
        if not 1 > self.option('evalue') >= 0:
            raise OptionError('E-value值设定必须为[0-1)之间：%s', variables = (self.option('evalue')), code = "22500104")
        if not isinstance(self.option('lines'), int):
            raise OptionError("行数必须为整数", code = "22500105")
        return True
    ...
    def run_blast(self):
        ...
        for f in os.listdir(self.splitfasta.output_dir):
            ...
            blast_tool = self.add_tool('itraq_and_tmt.annotation.diamond')
