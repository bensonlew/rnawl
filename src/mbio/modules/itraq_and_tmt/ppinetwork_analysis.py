...
class PpinetworkAnalysisModule(Module):
    def __init__(self, work_id):
        ...
        options = [
            {"name": "diff_exp_gene", "type": "infile", "format": "itraq_and_tmt.common"},
            {"name": "species", "type": "int", "default": 9606},
            {"name": "combine_score", "type": "int", "default": 300},
            {"name": "seq", "type": "string", "default": ""}
        ]
        self.add_option(options)
        self.map = self.add_tool("itraq_and_tmt.proteinset.ppinetwork_map")
        self.map_blast = self.add_tool("itraq_and_tmt.proteinset.ppinetwork_mapblast")
        self.ppinetwork_predict = self.add_tool("itraq_and_tmt.proteinset.ppinetwork_predict")
        self.ppinetwork_topology = self.add_tool("itraq_and_tmt.proteinset.ppinetwork_topology")
        self._end_info = 0
    ...
    def ppinetwork_predict_run(self):
        ...
        if not line:
            self.set_error("基因集中的基因不能匹配到string数据库中id，请您确认基因id为Ensemble或者Entrez GeneID！", code = "22500204")
