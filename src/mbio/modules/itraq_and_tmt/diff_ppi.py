    ...
    def run_ppi(self):
        ...
        for diff in diff_files:
            ...
            if len(uplist + downlist) > 10:
                de_module = self.add_module('itraq_and_tmt.ppinetwork_analysis')
                de_module.set_options(options)
                self.modules.append(de_module)
            if len(uplist) > 10:
                up_module = self.add_module('itraq_and_tmt.ppinetwork_analysis')
                options.update(dict(diff_exp_gene=acc_up))
                up_module.set_options(options)
                self.modules.append(up_module)
            if len(downlist) > 10:
                down_module = self.add_module('itraq_and_tmt.ppinetwork_analysis')
