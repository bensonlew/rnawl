...
class DenovoAssemble2FilterModule(Module):
    """
    拼接以及新转录本预测
    version v1.0.1
    author: liubinxu
    ...
    def __init__(self, work_id):
        super(DenovoAssemble2FilterModule, self).__init__(work_id)
        options = [
            ###组装选项
    ...
    def check_options(self):
        ...
        if self.option('assemble_fa') and self.option('assemble_g2t'):
            ...
            raise OptionError("样本列表文件依次为sample_id\tleft.fq\tright.fq", code = "22001501")

        if self.option('fq_type') == "PE":
            with open(self.option('sample_fq_list'), 'r') as f:
                if len(f.readline().strip().split("\t")) < 3:
                    raise OptionError("样本文件格式错误 样本列表文件依次为sample_id\tleft.fq\tright.fq", code = "22001502")
        elif self.option('fq_type') == "SE":
            with open(self.option('sample_fq_list'), 'r') as f:
                if len(f.readline().strip().split("\t")) < 2:
                    raise OptionError("样本文件格式错误 样本列表文件依次为sample_id\tleft.fq", code = "22001503")
        return True
    ...
    def run_trinity_stat(self):
        ...
        '''
        self.logger.info('** gene2trans结果为 {}'.format(self.trinity_merge.option("gene2trans")))
        self.logger.info('** fasta结果为 {}'.format(self.trinity_merge.option("trinity_fa").prop['path']))
    ...
    def get_filter_fa(self):
        ...
        获取过滤得到的fasta文件
        '''
        filter_fa = self.option("assemble_fa")
        if self.option('cdhit_filter'):
    ...
    def get_unfilter_fa(self):
        ...
        获取需要过滤的fasta文件
        '''
        un_filter_fa = self.option("assemble_fa")
    ...
    def run(self):
        ...
        设置运行顺序
        '''
        super(DenovoAssemble2FilterModule, self).run()
        if self.option('assemble_fa') and self.option('assemble_g2t'):
        ...
        '''
            ...
            self.fastq_merge.on('end', self.run_trinity)
            self.trinity.on('end', self.run_trinity_dis)
            self.trinity_merge.on('end', self.run_busco)
            self.trinity_merge.on('end', self.set_output, 'trinity')
            self.on_rely([self.fastq_merge, self.trinity_merge], self.run_transrate)
            self.on_rely([self.trinity_merge, self.busco, self.transrate], self.run_trinity_stat)
        '''
        if self.option('TPM_filter'):
        ...
        if self.option('assemble_fa') and self.option('assemble_g2t'):
            self.trinity_merge.option("trinity_fa", self.option('assemble_fa'))
            self.trinity_merge.option("gene2trans", self.option('assemble_g2t'))
    ...
    def set_output(self, event):
        ...
        if event['data'] == 'trinity_stat':
            self.logger.info("param is {} {} {}".format(self.option('cdhit_filter'), self.option('TPM_filter'), self.option('transrate_filter')))
        ...
        if event['data'] == 'transrate':
            if (not self.option('cdhit_filter')) and (not self.option('TPM_filter')) and self.option('transrate_filter'):
                ...
                if os.path.exists(self.output_dir + "/Trinity.filter.fasta"):
                    os.remove(self.output_dir + "/Trinity.filter.fasta")
                    os.link(filter_fa, out)
                else:
                    os.link(filter_fa, out)
            elif (not self.option('cdhit_filter')) and (not self.option('TPM_filter')) and (not self.option('transrate_filter')):
                out = os.path.join(self.output_dir, 'Trinity.filter.fasta')
                filter_fa = self.get_filter_fa()
                if os.path.exists(self.output_dir + "/Trinity.filter.fasta"):
                    os.remove(self.output_dir + "/Trinity.filter.fasta")
    ...
    def linkdir(self, olddir, newname, mode='link'):
        ...
        if not os.path.isdir(olddir):
            self.set_error('需要移动到output目录的文件夹不存在。', code = "22001501")
    ...
    def end(self):
        ...
        self.option("filter_fa").delete_property()
        ...
        '''
        super(DenovoAssemble2FilterModule, self).end()
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "denovo_assemble2filter" + str(random.randint(1, 10000)),
            "type": "module",
            "name": "denovo_rna_v2.denovo_assemble2_filter",
            "instant": False,
            "options": dict(
                sample_fq_list = "/mnt/ilustre/users/sanger-dev/workspace/20190617/Denovorna_tsg_34448/HiseqQc/output/sickle_dir/fq_list.txt",
                assemble_fa = "/mnt/ilustre/users/sanger-dev/workspace/20190617/Single_denovo_assemble3390/DenovoAssemble3/output/assemble_raw.fasta",
                assemble_g2t = "/mnt/ilustre/users/sanger-dev/workspace/20190617/Single_denovo_assemble3390/DenovoAssemble3/output/assemble_raw.gene_trans_map"
            )
        }
