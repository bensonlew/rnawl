...
class HiseqQcModule(Module):
    ...
    def check_options(self):
        ...
        if not self.option("fastq_dir").is_set:
            raise OptionError("需要传入fastq文件或者文件夹", code = "22000801")
        if self.option("fastq_dir").is_set:
            ...
            if self.option('fq_type') == "PE" and row_num != 3:
                raise OptionError("PE序列list文件应该包括文件名、样本名和左右端说明三列", code = "22000802")
            elif self.option('fq_type') == "SE" and row_num != 2:
                raise OptionError("SE序列list文件应该包括文件名、样本名两列", code = "22000803")
    ...
    def clipper_run(self):
        ...
        for f in self.samples:
            fq_s = os.path.join(self.option("fastq_dir").prop["path"], self.samples[f])
            clipper = self.add_tool('denovo_rna_v2.fastx_clipper')
    ...
    def seqprep_run(self):
        ...
        for f in self.samples:
            ...
            seqprep = self.add_tool('denovo_rna_v2.seq_prep')
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "test_HiseqQc_" + str(random.randint(1, 10000))+"-yyyyyyyy",
            "type": "module",
            "name": "denovo_rna_v2.hiseq_qc",
            "instant": False,
            "options": dict(
