...
class CdsTfModule(Module):
    """
    denovo_rna_v2运行tool下面的Pepsplit.py和Pepsplitfinal.py
    """

    def __init__(self, work_id):
        super(CdsTfModule, self).__init__(work_id)
        options = [
    ...
    def check_options(self):
        ...
        if not self.option('fasta'):
            raise OptionError('必须输入trinity_fa作为比对参考文件', code = "22000301")
        return True
