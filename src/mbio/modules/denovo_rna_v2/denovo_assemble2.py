...
class DenovoAssemble2Module(Module):
    """
    拼接以及新转录本预测
    version v1.0.1
    author: liubinxu
    ...
    def __init__(self, work_id):
        super(DenovoAssemble2Module, self).__init__(work_id)
        options = [
            ###组装选项
    ...
    def check_options(self):
        ...
        if self.option('assemble_fa') and self.option('assemble_g2t'):
            ...
            raise OptionError("样本列表文件依次为sample_id\tleft.fq\tright.fq", code = "22000601")
    ...
    def run_trinity(self):
        ...
        if self.option("fq_type") == "PE":
            options.update({
                "fq_l": self.fastq_merge.option('merge_left').prop['path'],
                "fq_r": self.fastq_merge.option('merge_right').prop['path'],
            })
        elif self.option("fq_type") == "SE":
            options.update({
                "fq_s": self.fastq_merge.option('merge_left').prop['path'],
            })
        else:
            pass
    ...
    def run_busco(self):
        ...
        options = {
            ...
            "g2t": self.trinity_merge.option("gene2trans").prop['path'],
            "marker": True,
            "fa": self.trinity_merge.option("trinity_fa")
        }
    ...
    def run_transrate(self):
        ...
        options = {
            ...
            "assembly": self.trinity_merge.option("trinity_fa").prop['path']
        }
        if self.option("fq_type") == "PE":
            options.update({
    ...
    def run_trinity_stat(self):
        ...
        options = {
            "fasta": self.trinity_merge.option("trinity_fa").prop['path'],
            "gene2trans": self.trinity_merge.option("gene2trans").prop['path'],
    ...
    def get_filter_fa(self):
        ...
        获取过滤得到的fasta文件
        '''
        filter_fa = self.trinity_merge.option("trinity_fa").prop['path']
        if self.option('cdhit_filter'):
    ...
    def get_unfilter_fa(self):
        ...
        获取需要过滤的fasta文件
        '''
        un_filter_fa = self.trinity_merge.option("trinity_fa").prop['path']
    ...
    def run(self):
        ...
        设置运行顺序
        '''
        super(DenovoAssemble2Module, self).run()
        if self.option('assemble_fa') and self.option('assemble_g2t'):
    ...
    def linkdir(self, olddir, newname, mode='link'):
        ...
        if not os.path.isdir(olddir):
            self.set_error('需要移动到output目录的文件夹不存在。', code = "22000604")
    ...
    def end(self):
        ...
        '''
        super(DenovoAssemble2Module, self).end()
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "denovo_assemble2" + str(random.randint(1, 10000)),
            "type": "module",
            "name": "denovo_rna_v2.denovo_assemble2",
            "instant": False,
            "options": dict(
                sample_fq_list = "/mnt/ilustre/users/sanger-dev/workspace/20190416/Denovorna_tsg_33857/HiseqQc/output/sickle_dir/fq_list.txt",
            )
        }
