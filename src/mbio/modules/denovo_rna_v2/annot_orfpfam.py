...
class AnnotOrfpfamModule(Module):
    ...
    def check_options(self):
        ...
        if self.option('fasta').is_set:
            ...
            if not self.option('gtf').is_set:
                raise OptionError('指定蛋白序列时需同时指定gtf文件', code = "22001401")
            if self.option('g2t2p') == "":
                raise OptionError('指定蛋白序列时需同时指定基因转录本蛋白对应关系文件', code = "22001402")
        else:
            raise OptionError('必须输入fasta, 序列或蛋白序列', code = "22001403")

        if self.option("isoform_unigene") == "" and not self.option('gtf').is_set:
            raise OptionError('必须提供gtf文件或transcript基因对应关系文件', code = "22001404")
        return True
    ...
    def run_cathmmout(self):
        ...
        self.predict_tool = self.add_tool('denovo_rna_v2.annotation.predict')
