...
class DiffGenesetAnalysisModule(Module):
    def __init__(self, work_id):
        ...
        options = [
            ...
            {'name': 'go_list', 'type': 'infile', 'format': 'denovo_rna_v2.go_list'},
            {'name': 'go_version', 'type': 'string', 'default': '20200628'},
            {'name': 'kegg_table', 'type': 'infile', 'format': 'denovo_rna_v2.common'},
            {'name': 'kegg_table2', 'type': 'infile', 'format': 'denovo_rna_v2.common'},
            {'name': 'all_list', 'type': 'infile', 'format': 'denovo_rna_v2.common'},
            {'name': 'add_info', 'type': 'infile', 'format': 'denovo_rna_v2.common'},
            # {'name': 'reactome_annot', 'type': 'infile', 'format': 'denovo_rna_v2.common'},
            # {'name': 'do_list', 'type': 'infile', 'format': 'denovo_rna_v2.common'},
            ...
            {'name': 'gene_list', 'type': 'infile', 'format': 'denovo_rna_v2.common'},
            {'name': 'gene_multi', 'type': 'infile', 'format': 'denovo_rna_v2.common'},
            {'name': 'go_class', 'type': 'infile', 'format': 'denovo_rna_v2.common'},
            {'name': 'cog_class', 'type': 'infile', 'format': 'denovo_rna_v2.common'},
            {'name': 'method', 'type': 'string', 'default': ''},
            # {'name': 'do_class', 'type': 'infile', 'format': 'denovo_rna_v2.common'},
        ]
        self.add_option(options)
        self.go_class_tool = None
        self.kegg_class_tool = self.add_tool("denovo_rna_v2.diffgt4work.diff_kegg_class")
        self.go_enrich_tool = self.add_tool("denovo_rna_v2.diff_geneset.diff_go_enrich")
        self.kegg_enrich_tool = self.add_module("denovo_rna_v2.workflow_diffgt.diff_kegg_enrich")
        self.tools_names = ["diff_cog_class", "diff_go_class","diff_kegg_class","diff_go_enrich","diff_kegg_enrich"]
    ...
    def check_options(self):
        ...
        if self.option("genes_num") >= 50:
            ...
            self.logger.info("基因数量不足")
        # if self.option("genes_num") > 0:
            # if self.option("level") == "G":
            #     self.logger.info("水平为基因水平,进行reactome分析")
            #     self.reactome_class_tool = self.add_tool("denovo_rna_v2.diff_geneset.diff_reactome_class")
            ...
            #     self.reactome_enrich_tool = self.add_module("denovo_rna_v2.diff_geneset.diff_reactome_enrich")
            ...
            #         self.do_class_tool = self.add_tool("denovo_rna_v2.diff_geneset.diff_do_class")
            ...
            #         self.do_enrich_tool = self.add_tool("denovo_rna_v2.diff_geneset.diff_do_enrich")
    ...
    def prepare_file_json(self):
        option_names = ["level","species","go_list","kegg_table",
                        "kegg_table2","all_list","add_info","kegg_version",
                        "geneset_name","genes_num","geneset_path","level","regulate",
                        "gene_list","gene_multi","go_class"]
    ...
    def replace_kegg_link(self):
        ...
        替换kegg链接
        '''
        enrich_result = glob.glob("{}/*.xls".format(self.kegg_enrich_tool.output_dir))[0]
        annot_result = self.kegg_class_tool.output_dir + '/kegg_stat.xls'
    ...
    def set_output(self):
        ...
        if self.option("genes_num") < 0:
            ...
            if "diff_cog_class" in self.tools_names:
                self.logger.info("开始整理cog注释相关文件")
                if os.path.exists(os.path.join(self.output_dir,"diff_cog_class")):
                    shutil.rmtree(os.path.join(self.output_dir,"diff_cog_class"))
                os.makedirs(os.path.join(self.output_dir,"diff_cog_class"))
                cog_class_path = self.option("cog_class").prop["path"]
                try:
                    os.link(cog_class_path,os.path.join(self.output_dir,"diff_cog_class",os.path.basename(cog_class_path)))
                except:
                    raise Exception("diff_cog_class文件整理失败")
                self.result_dict["geneset_results"]["diff_cog_class"] = os.path.join(self.output_dir,"diff_cog_class")
                self.logger.info("cog注释相关文件整理完成")
    ...
    class TestFunction(unittest.TestCase):
        ...
        def test(self):
            ...
            data = {
                ...
                "type": "module",
                "name": "denovo_rna_v2.diff_geneset.diff_geneset_analysis",
                "instant": False,
                "options": dict(
