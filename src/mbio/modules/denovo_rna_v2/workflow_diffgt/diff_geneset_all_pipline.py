...
class DiffGenesetAllPiplineModule(Module):
    ...
    def __init__(self, work_id):
        ...
        self.file_prepare = self.add_module("denovo_rna_v2.workflow_diffgt.diff_geneset_all")
        self.cluster = self.add_tool("denovo_rna_v2.exp_cluster")
        ...
        if not select_genesets:
            ...
            for geneset_info in sorted(gensets_infos.keys()):
                if geneset_info in select_genesets:
                    ...
                    geneset_analysis = self.add_module("denovo_rna_v2.workflow_diffgt.diff_geneset_analysis")
    ...
    def set_output(self):
        ...
        tre_files = glob.glob(cluster_dir + "/*tre")
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        pipe_path = "/mnt/lustre/users/sanger-dev/wpm2/workspace/20210728/Denovorna_8hdg_b7jpuuo4soclc8v34be5kr/"

        data = {
            "id": task_id,
            "type": "module",
            "work_dir": "/mnt/lustre/users/sanger-dev/wpm2/workspace/{}/{}".format(date, task_id),
            "name": "denovo_rna_v2.workflow_diffgt.diff_geneset_all_pipline",
            "instant": False,
            "options": dict(
                diff_path= pipe_path + "DiffexpBatchNew/DiffexpBatch/output",
                annot_result =pipe_path + "AnnotClassBeta/output",
                diff_method="DESeq2",
                kegg_version ="202007",
                level = "G",
                gene_count_file =pipe_path + "Quant/output/gene.count.matrix",
                gene_exp_file = pipe_path + "Quant/output/gene.tpm.matrix",
                group = pipe_path + "remote_input/group/default_group.txt",
            )
           }
