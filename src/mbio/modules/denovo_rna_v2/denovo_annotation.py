...
class DenovoAnnotationModule(Module):
    ...
    def __init__(self, work_id):
        ...
        options = [
            ...
            {"name": "blast_eggnog_xml", "type": "infile", "format": "align.blast.blast_xml"},
        ...
        self.swissprot_annot = self.add_tool('denovo_rna_v2.annotation.xml2table')
        self.nr_annot = self.add_tool('denovo_rna_v2.annotation.xml2table')
        self.ncbi_taxon = self.add_tool('denovo_rna_v2.annotation.ncbi_taxon')
        self.go_annot = self.add_tool('denovo_rna_v2.annotation.go_annotation')
        self.go_upload = self.add_tool('denovo_rna_v2.annotation.go_upload')
        self.string_cog = self.add_tool('denovo_rna_v2.annotation.string2cogv9')
        self.kegg_annot = self.add_tool('denovo_rna_v2.annotation.kegg_annotation')
        self.swissprot_annot = self.add_tool("denovo_rna_v2.annotation.swissprot")
        self.kegg_upload = self.add_tool('denovo_rna_v2.annotation.kegg_upload')
        self.anno_stat = self.add_tool('denovo_rna_v2.annotation.ref_anno_stat')
        self.anno_query = self.add_tool('denovo_rna_v2.annotation.ref_anno_query')
        self.step.add_steps('blast_statistics', 'nr_annot', 'ncbi_taxon', 'go_annot', 'go_upload', 'kegg_annot', 'kegg_upload', 'cog_annot', 'anno_stat', 'swissprot_annot', 'anno_query')

    def check_options(self):
        if self.option('annot_type') == "denovo_rna":
            if self.option('anno_statistics'):
                if not self.option('gene2trans').is_set:
                    raise OptionError('进行注释统计的tool必须要设置gene_file 或者 gene2trans', code = "22000401")
        elif self.option('annot_type') == "ref_rna":
            if self.option('anno_statistics'):
                if not self.option('gene_file').is_set:
                    raise OptionError('进行注释统计的tool必须要设置gene_file', code = "22000402")
                if not self.option('ref_genome_gtf').is_set and not self.option('new_gtf').is_set:
                    raise OptionError('缺少gtf文件', code = "22000403")
                if not self.option("length_file").is_set:
                    raise OptionError("缺少注释转录本序列的长度文件", code = "22000404")
    ...
    def run_string2cog(self):
        if self.option('blast_string_xml').is_set:
            xml_file = self.option('blast_string_xml')
        else:
            xml_file = self.option('blast_eggnog_xml')
        options = {
            'blastout': xml_file,
    ...
    def run_annot_stat(self):
        ...
        if 'cog' in self.anno_database:
            if self.option('blast_string_xml').is_set:
                ...
                opts['string_xml'] = self.option('blast_eggnog_xml')
    ...
    def run(self):
        ...
        if self.option('go_annot'):
            ...
            if self.option("gos_list_upload").is_set:
                ...
                self.run_go_anno()
        if self.option('blast_string_xml').is_set or self.option('blast_string_table').is_set or self.option('blast_eggnog_xml').is_set:
        ...
        if len(self.all_end_tool) > 1:
            ...
            self.set_error('不需要进行任何注释工作', code = "22000405")
    ...
    def set_xml(self):
        ...
        if self.option('blast_string_xml').is_set:
            ...
            os.link(self.option('blast_eggnog_xml').prop['path'], os.path.join(xml_dir, "Trinity_vs_eggnog.xml") )
    ...
    def set_output(self, event):
        ...
        if event['data'] == 'blast_stat':
            ...
            if os.path.exists(self.output_dir + "/anno_stat/trans_anno_detail.xls"):
                os.remove(self.output_dir + "/anno_stat/trans_anno_detail.xls")
    ...
    def linkdir(self, olddir, newname, mode='link'):
        ...
        if not os.path.isdir(olddir):
            self.set_error('需要移动到output目录的文件夹不存在。', code = "22000406")
