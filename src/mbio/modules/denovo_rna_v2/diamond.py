...
class DiamondModule(Module):
    ...
    def check_options(self):
        if not self.option("query").is_set:
            raise OptionError("必须设置参数query", code = "22000701")
        if self.option('query_type') not in ['nucl', 'prot']:
            raise OptionError('query_type查询序列的类型为nucl(核酸)或者prot(蛋白):%s', variables = (self.option('query_type')), code = "22000702")
        if self.option('outfmt') not in [5, 6]:
            raise OptionError('outfmt遵循blast+输出规则，目前只支持5，6输出格式：%s', variables = (str(self.option('outfmt'))), code = "22000703")
        if not 1 > self.option('evalue') >= 0:
            raise OptionError('E-value值设定必须为[0-1)之间：%s', variables = (str(self.option('evalue'))), code = "22000704")
        if not isinstance(self.option('lines'), int):
            raise OptionError("行数必须为整数", code = "22000705")
        return True
    ...
    def run_blast(self):
        ...
        for f in os.listdir(self.splitfasta.output_dir):
            ...
            blast_tool = self.add_tool('denovo_rna_v2.annotation.diamond')
