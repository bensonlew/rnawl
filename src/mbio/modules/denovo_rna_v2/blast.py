...
class BlastModule(Module):
    def __init__(self, work_id):
        ...
        self.catblast = self.add_tool("denovo_rna_v2.annotation.cat_blastout")
    ...
    def check_options(self):
        if not self.option("query").is_set:
            raise OptionError("必须设置参数query", code = "22000201")
        if self.option('query_type') not in ['nucl', 'prot']:
            raise OptionError('query_type查询序列的类型为nucl(核酸)或者prot(蛋白):%s', variables = (self.option('query_type')), code = "22000202")
        else:
            if self._fasta_type[self.option('query').prop['seq_type']] != self.option('query_type'):
                raise OptionError(
                    '文件检查发现查询序列为:%s, 而说明的文件类型为:%s', variables = (
                        self._fasta_type[self.option('query').prop['seq_type'], self.option('query_type')]), code = "22000203")
        if self.option("database") == 'customer_mode':
            if not self.option("reference").is_set:
                raise OptionError("使用自定义数据库模式时必须设置reference", code = "22000204")
            if self.option('reference_type') not in ['nucl', 'prot']:
                raise OptionError('reference_type参考序列的类型为nucl(核酸)或者prot(蛋白):%s', variables = (self.option('query_type')), code = "22000205")
            else:
                if self._fasta_type[self.option('reference').prop['seq_type']] != self.option('reference_type'):
                    raise OptionError(
                        '文件检查发现参考序列为:%s, 而说明的文件类型为:%s', variables = (
                            self._fasta_type[self.option('reference').prop['seq_type'], self.option('reference_type')]), code = "22000206")
        elif self.option("database").lower() not in ["nt", "nr", "string", 'kegg', 'swissprot']:
            raise OptionError("数据库%s不被支持", variables = (self.option("database")), code = "22000207")
        else:
            self.option('reference_type', self._database_type[self.option("database").lower()])
        if self.option('outfmt') not in [5, 6]:
            raise OptionError('outfmt遵循blast+输出规则，目前只支持5，6输出格式：%s', variables = (str(self.option('outfmt'))), code = "22000208")
        if not 1 > self.option('evalue') >= 0:
            raise OptionError('E-value值设定必须为[0-1)之间：%s', variables = (str(self.option('evalue'))), code = "22000209")
        if not 0 < self.option('num_alignment') < 1001:
            raise OptionError('序列比对保留数必须设置在1-1000之间:%s', variables = (str(self.option('num_alignment'))), code = "22000210")
        if self.option('blast') not in self._blast_type[self.option('query_type')][self.option('reference_type')]:
            raise OptionError('程序不试用于提供的查询序列和库的类型，请仔细检查，核酸比对核酸库只能使用blastn或者tblastn，核酸比对蛋白库只能使用blastp， 蛋白比对蛋白库只能使用blastp, 或者没有提供blast参数', code = "22000211")
        if not isinstance(self.option('lines'), int):
            raise OptionError("行数必须为整数", code = "22000212")
        # if self.option('lines') % 2:
        #     raise OptionError("行数必须为整除2")
        if self.option('lines') <= 0:
            raise OptionError("行数小于等于0，请重设！", code = "22000213")
        return True
    ...
    def run_blast(self):
        ...
        for f in os.listdir(self.splitfasta.output_dir):
            ...
            blast_tool = self.add_tool('denovo_rna_v2.annotation.blast')
