...
class AnnotMapdbModule(Module):
    ...
    def __init__(self, work_id):
        ...
        options = [
            {"name": "query", "type": "infile", "format": "denovo_rna_v2.fasta"},  # 输入文件
    ...
    def check_options(self):
        if not self.option("query").is_set:
            raise OptionError("必须设置参数query", code = "22001801")
        if self.option('query_type') not in ['nucl', 'prot']:
            raise OptionError('query_type查询序列的类型为nucl(核酸)或者prot(蛋白):%s', variables = (self.option('query_type')), code = "22001802")
        if self.option('outfmt') not in [5, 6]:
            raise OptionError('outfmt遵循blast+输出规则，目前只支持5，6输出格式：%s', variables = (self.option('outfmt')), code = "22001803")
        if not 1 > self.option('evalue') >= 0:
            raise OptionError('E-value值设定必须为[0-1)之间：%s', variables = (self.option('evalue')), code = "22001804")
        if not isinstance(self.option('lines'), int):
            raise OptionError("行数必须为整数", code = "22001805")
        return True
    ...
    def run_blast_nr(self):
        ...
        self.logger.info("nr version is {} {} {}".format(self.option("version"), self.option("diamond_version"), self.option("nr_version")))
        i = 0
