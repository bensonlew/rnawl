...
class WgsConfigModule(Module):
    """
    样本基因组SSR分析及引物设计
    """
    def __init__(self, work_id):
        super(WgsConfigModule, self).__init__(work_id)
        options = [
            {"name": "downloadfa", "type": "string"},       # 下载的fa文件
            {"name": "downloadgff", "type": "string"},      # 有没有gff的check程序！
            {"name": "info", "type": "string"},              #  info.log 应该check列数；请确保信息完整，列数一致
    ...
    def check_options(self):
        if not self.option("downloadfa"):
            raise OptionError("请设置downloadfa", code="24501801")
        if not self.option("downloadgff"):
            raise OptionError("请设置downloadgff", code="24501802")
        if not self.option("info"):
            raise OptionError("请设置info文件", code="24501803")
    ...
    def makedir_info(self): # mkdir 
        ...
        if not os.path.exists(self.file_path):
            ...
            print("##### database/dna_wgs_geneome 基因组存放路径不存在，重新建立!")
        with open(self.option("info"), 'r') as file:
            data = file.readlines()   # 怎么增加判断 data存在不为空！
            for line in data:
                i=line.strip()
                temp = i.split('\t')
                print(temp)
                print(len(temp) )
                if len(temp) >= 10:    # 核查列数
                    self.strain_characteristic = temp[1]
                    self.link = temp[2]
                    self.genome_size = temp[6]
                    self.n50 = temp[7]
                    self.assembly = temp[8]
                    self.web_dir = temp[9]
                    self.species = sym1.join(re.split(' |_|-',temp[0])).capitalize()
                    self.source = temp[3].upper()
                    self.edition = temp[4]
    ...
    def openjson(self):
        abspath = os.path.join(self.species, self.source, self.edition, self.release_time)
        time_edition = self.release_time + "||" + self.edition
        ...
        os.system("cp {} {}".format(self.file_path + "/wgs_genome.json", Config().SOFTWARE_DIR + "/database/dna_wgs_geneome/z.wgs_json/wgs_genome.json_" + b))
        print("##### 物种是" + self.species)
        print("##### 物种版本是" + time_edition)        
        print("##### 已有物种名如下" )
        print(self.wgsjson.keys())
        self.change = 0
        print(self.option("update"))
        print("#######\n")
        if self.species not in self.wgsjson.keys() or (self.species in self.wgsjson.keys() and time_edition not in self.wgsjson[self.species].keys()):
            self.change = 1                      
        elif int(self.option("update")) == 1:
            self.change = 1
            print("222222\n")
        else:
            print(self.species +"\t" + time_edition + "该物种id无需导入 ! ")
            print("无需导入，正常结束")
            self.end()
    ...
    def run_genome_grename(self): 
        ...
        """
        opti = {"fasta": self.option("downloadfa"), "gff": self.option("downloadgff")}
        if self.option("chromosomelist"):
            opti["chromosomelist"] = self.option("chromosomelist")
        self.genome_grename.set_options(opti)
        self.genome_grename.on("end", self.run_samtools_faidx)
    ...
    def fa_link(self):
        ...
        """
        namelist = ["ref.fa", "ref.gff", "ref.changelog", "info.log"]
        for i in namelist:
            oldpathname = os.path.join(self.genome_grename.output_dir, i)
            newpathname = os.path.join(self.species_path, i)
            print(oldpathname)
            print(newpathname)            
            self.checklink(newpathname)  
            if i is "info.log":
                os.link(self.option("info"), newpathname)   # 输入的info文件被链接更名
            else:
                os.link(oldpathname, newpathname)           # genome_grename输出文件被链接
    ...
    def run_gene_anno(self):   # 4 ssr的module
        """
    ...
    def hardlink(self, event):
        ...
        if event['data'] == 'ref.chrlist':
            ...
            self.linkdir(obj.output_dir + "/gene_anno/anno.summary", self.species_path)
    ...
    def run_update_json(self):
        while self.change:
            data = {
                ...
                'ref': abspath + "/ref.fa",
                'gff': abspath + "/ref.gff",
                'anno': abspath + "/anno.summary",
                'change_log': abspath + "/ref.changelog",
                'info_log': abspath + "/info.log",
                'ref_fa_amb': abspath + "/ref.fa.amb",
                'ref_fa_ann': abspath + "/ref.fa.ann",
                'ref_fa_bwt': abspath + "/ref.fa.bwt",
                'ref_fa_fai': abspath + "/ref.fa.fai",
                'ref_fa_pac': abspath + "/ref.fa.pac",
                'ref_fa_sa': abspath + "/ref.fa.sa",
                'ref_dict': abspath + "/ref.dict",
                'ref_chrlist': abspath + "/ref.chrlist",
                'total_chrlist': abspath + "/total.chrlist",
                'snpeff_path': abspath + "/snpEff.config",
                'ssr_path': abspath + "/",
                'web_dir': self.web_dir
            }
            if self.species not in self.wgsjson.keys():
                self.wgsjson[self.species] = {time_edition: data}
                print("物种存入")
            else:
                self.wgsjson[self.species][time_edition] = data
                print("物种另一个版本存入")
            with open(self.file_path + "/wgs_genome.json", "w") as dump_f:
                json.dump(self.wgsjson, dump_f, indent=4)
    ...
    def run(self):
        super(WgsConfigModule, self).run()
        self.makedir_info()
        self.openjson()
        self.run_genome_grename()
        # self.run_stat_annotation()
        # self.run_getgenefasta()
        self.on_rely([self.bwa_config, self.circos_chrlist, self.snpeff_index, self.ssr_ref_primer, self.makeblastdb, self.gene_anno, self.circos_ref2bit], self.run_update_json)        

    def end(self):
        super(WgsConfigModule, self).end()
