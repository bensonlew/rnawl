...
class PpinetworkAnalysisModule(Module):
    def __init__(self, work_id):
        ...
        options = [
            {"name": "diff_exp_gene", "type": "infile", "format": "ref_rna.common"},
            {"name": "species", "type": "int", "default": 9606},
            {"name": "combine_score", "type": "int", "default": 300},
            {"name": "seq", "type": "string", "default": ""}
        ]
        self.add_option(options)
        self.map = self.add_tool("ref_rna.geneset.ppinetwork_map")
        self.map_blast = self.add_tool("ref_rna.geneset.ppinetwork_mapblast")
        self.ppinetwork_predict = self.add_tool("ref_rna.geneset.ppinetwork_predict")
        self.ppinetwork_topology = self.add_tool("ref_rna.geneset.ppinetwork_topology")
        self._end_info = 0
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        test_dir = '/mnt/ilustre/users/sanger-dev/sg-users/liubinxu/test_ref_rna/data4'
        data = {
            "id": "ppinetwork" + str(random.randint(1, 10000)),
            "type": "module",
            "name": "ref_rna.ppinetwork_analysis",
            "instant": False,
            "options": dict(
