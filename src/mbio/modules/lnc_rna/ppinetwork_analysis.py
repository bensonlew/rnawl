...
class PpinetworkAnalysisModule(Module):
    def __init__(self, work_id):
        ...
        options = [
            {"name": "diff_exp_gene", "type": "infile", "format": "lnc_rna.common"},
            {"name": "species", "type": "int", "default": 9606},
            {"name": "combine_score", "type": "int", "default": 300},
            {"name": "seq", "type": "string", "default": ""}
        ]
        self.add_option(options)
        self.map = self.add_tool("lnc_rna.geneset.ppinetwork_map")
        self.map_blast = self.add_tool("lnc_rna.geneset.ppinetwork_mapblast")
        self.ppinetwork_predict = self.add_tool("lnc_rna.geneset.ppinetwork_predict")
        self.ppinetwork_topology = self.add_tool("lnc_rna.geneset.ppinetwork_topology")
        self._end_info = 0
    ...
    def check_options(self):
        ...
        if int(self.option('species')) not in species_list:
            raise OptionError("不能进行蛋白质互作分析，因为string数据库中不存在该物种的蛋白质互作组数据！", code = "23700803")
        '''
        if self.option('seq') == "":
            raise OptionError("seq参数不可以为空", code = "23700804")
        return True
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            "type": "module",
            "name": "lnc_rna.ppinetwork_analysis",
            "instant": False,
            "options": dict(
