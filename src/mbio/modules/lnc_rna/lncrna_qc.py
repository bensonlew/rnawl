...
class LncrnaQcModule(Module):
    ...
    def __init__(self, work_id):
        super(LncrnaQcModule, self).__init__(work_id)
        options = [
            ...
            {"name": "fq_list", "type": "outfile", "format": "lnc_rna.fastq_list"},
        ]
    ...
    def fastp_run(self):
        ...
        for f in sorted(self.samples):
            ...
            fastp = self.add_tool('lnc_rna.fastp')
    ...
    def clipper_run(self):
        ...
        for f in sorted(self.samples):
            fq_s = os.path.join(self.option("fastq_dir").prop["path"], self.samples[f])
            clipper = self.add_tool('lnc_rna.fastx_clipper')
    ...
    def seqprep_run(self):
        ...
        for f in sorted(self.samples):
            ...
            seqprep = self.add_tool('lnc_rna.seq_prep')
    ...
    def run(self):
        super(LncrnaQcModule, self).run() ## 初始化时间比较久，会影响监听的触发时间，需要一开始就super run
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "LncrnaQc_" + str(random.randint(1, 10000)),
            "type": "module",
            "name": "lnc_rna.lncrna_qc",
            "instant": False,
            "options": dict(
