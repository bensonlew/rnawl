...
class RnaseqMappingModule(Module):
    ...
    def __init__(self, work_id):
        ...
        options = [
            ...
            {"name": "ref_genome_custom", "type": "string"},
            {"name": "ref_gtf_custom", "type": "string"},  # 参考基因组模式选项 用户自定义、选择已有生物物种
            ...
            {"name": "bamlist", "type": "outfile", "format": "lnc_rna.common"},  # 输出的bamlist
            ...
            {"name": "bam_loc", "type": "outfile", "format": "lnc_rna.loc2name"}
        ]
    ...
    def check_options(self):
        ...
        if self.option("fastq_dir").is_set:
            ...
            if self.option('seq_method') == "PE" and row_num != 3:
                ...
                raise OptionError("SE序列list文件应该包括文件名、样本名两列", code = "23701202")
        ...
        if not self.option("mapping_method").lower() in ["tophat", "hisat","star"]:
            raise OptionError("比对软件只能为tophat or hisat or star", code = "23701208")
        ...
        if self.option("seq_method") == "PE":
            for f in self.samples:
                ...
                if tool == "star":
                    ...
                    mapping_tool = self.add_tool('lnc_rna.' + tool)
                self.tool_opts.update({
                    'left_reads': fq_l,
                    'right_reads': fq_r,
                    'sample': f
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            "id": "RnaseqMapping_star_test" + str(random.randint(1, 10000)),
            "type": "module",
            "name": "lnc_rna.rnaseq_mapping",
            "instant": False,
            "options": dict(
