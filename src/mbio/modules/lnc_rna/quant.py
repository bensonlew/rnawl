...
class QuantModule(Module):
    ...
    def __init__(self, work_id):
        ...
        options = [
            ...
            dict(name="bamlist", type="outfile", format="lnc_rna.bamlist"),
            ...
            dict(name="tpm_threshold",type="float",default="0"),#TPM阈，过滤平均TPM值小于该阈值的id
            dict(name="gene_count", type="outfile", format="lnc_rna.express_matrix"),  # for output
            dict(name="transcript_count", type="outfile", format="lnc_rna.express_matrix"),  # for output
            dict(name="transcript_tpm", type="outfile", format="lnc_rna.express_matrix"),  # for output
            dict(name="gene_tpm", type="outfile", format="lnc_rna.express_matrix"),  # for output
            dict(name="gene_fpkm", type="outfile", format="lnc_rna.express_matrix"),  # for output
            dict(name="transcript_fpkm", type="outfile", format="lnc_rna.express_matrix"),  # for output
    ...
    def tool_run(self):
        ...
        for sample, fq_list in sorted(fastq_dict.items()):
            ...
            tool = self.add_tool('lnc_rna.single_quant')
    ...
    def set_output(self):
        ...
        tpm_threshold=self.option("tpm_threshold")
        #新增 表达量过滤 去除在所有样品中表达量均为0的id 2019.04.12 fwy
        tmp_matrix =  self.work_dir + "/gene.tpm.matrix"
        exp_df=pd.read_table(tmp_matrix, index_col=0, header=0)
        num_samples=exp_df.shape[1]
        exp_final_df=exp_df.loc[exp_df.sum(axis=1)/num_samples>float(tpm_threshold)]
        exp_final_df.to_csv(tmp_matrix, sep='\t')

        with open(self.option('t2g'), 'r') as f:
            t2g_dict = dict([(line.strip().split("\t")[0], line.strip().split("\t")[1]) for line in f.readlines()])

        gene_list = list(exp_final_df.index)
        trans_list = list(set([trans for trans in t2g_dict if t2g_dict[trans] in gene_list]))

        for each in ['transcript.tpm.matrix', 'transcript.fpkm.matrix', 'transcript.count.matrix']:
            if os.path.exists(self.work_dir + "/" + each):
                fname=os.path.basename(self.work_dir + "/" + each)
                exp_df=pd.read_table(self.work_dir + "/" + each,index_col=0, header=0)
                num_samples=exp_df.shape[1]
                exp_final_df=exp_df.loc[trans_list]
                exp_final_df.to_csv(os.path.join(self.work_dir,fname), sep='\t')

        for each in ['gene.count.matrix', 'gene.fpkm.matrix']:
            if os.path.exists(self.work_dir + "/" + each):
                fname=os.path.basename(self.work_dir + "/" + each)
                exp_df=pd.read_table(self.work_dir + "/" + each,index_col=0, header=0)
                num_samples=exp_df.shape[1]
                exp_final_df=exp_df.loc[gene_list]
                exp_final_df.to_csv(os.path.join(self.work_dir,fname), sep='\t')
        ...
        for each in matrix_files:
            ...
            if each.endswith("transcript.count.matrix"):
                ...
                continue
        #新增输出文件trans_ids和gene_ids 2019.04.12 fwy

        trans_ids_file = os.path.join(self.output_dir, 'trans_ids.list')
        gene_ids_file=os.path.join(self.output_dir,'gene_ids.list')
        trans_ids_set=set()
        gene_ids_set=set()
        for each in matrix_files:
            if each.endswith("transcript.count.matrix"):
                df = pd.read_table(each, index_col=0)
                exp_sum = df.sum(axis=1)
                for t_id, value in exp_sum.items():
                    if value > 0:
                        trans_ids_set.add(t_id)
            elif each.endswith("gene.count.matrix"):
                df=pd.read_table(each, index_col=0)
                exp_sum = df.sum(axis=1)
                for g_id, value in exp_sum.items():
                    if value > 0:
                        gene_ids_set.add(g_id)
            else:
                continue
        with open(trans_ids_file,"w") as trans_ids_out:
            trans_ids_out.write('\n'.join(trans_ids_set))
        with open(gene_ids_file,"w") as gene_ids_out:
            gene_ids_out.write('\n'.join(gene_ids_set))
        self.end()
