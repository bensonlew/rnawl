...
class AsprofileModule(Module):
    ...
    def run_count_ref(self):
        self.count_ref = self.add_tool('tool_lab.ASprofile.count_ref')
    ...
    def run_gtf_compare(self):
        ...
        for sample, gtf_list in sp2fqs_dict.items():
            self.gffcompare = self.add_tool('ref_rna_v2.assemble.gffcompare')
    ...
    def run_asprofile(self):
        ...
        sp2fqs_dict = dict()
        for tool in self.tools:
            sp2fqs_dict[tool.option('sample')] = tool.option('cuff_gtf').path

        for sample, gtf_list in sp2fqs_dict.items():
            self.asprofile = self.add_tool('tool_lab.ASprofile.asprofile')
            opts = {'transcripts': gtf_list,
    ...
    def run_merge(self):
        ...
        self.merge = self.add_tool('tool_lab.ASprofile.merge')
        ...
        with open(result_list, 'w') as handle:
            for module in self.modules:
                handle.write('{}\t{}\n'.format(module.option('sample'), module.option('as_result').path))
