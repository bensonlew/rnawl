...
class TfPredictModule(Module):
    ...
    def __init__(self, work_id):
        ...
        options = [
            ...
            {'name': 'hmmscan', 'type': 'string',
             'default': '/mnt/ilustre/users/sanger-dev/sg-users/litangjian/hmm/hmmer-3.1b2-linux-intel-x86_64/binaries/hmmscan',
             'format': 'None'},
            {'name': 'hmmdb', 'type': 'string', 'default': '/mnt/ilustre/users/sanger-dev/app/database/pfam_31/Pfam-A.hmm',
             'format': 'None'},
            {'name': 'seqfile', 'type': 'infile', 'default': '/mnt/ilustre/users/sanger-dev/sg-users/deqing/TestFiles/TF/example.fa',
            ...
            {'name': 'tfdb', 'type': 'string', 'default': '/mnt/ilustre/users/sanger-dev/app/database/TFDB/',
             'format': 'None'},
            {'name': 'diamond', 'type': 'string', 'default': '/mnt/ilustre/users/sanger-dev/app/bioinfo/align/diamond-0.8.35/diamond',
             'format': 'None'},
            {'name': 'evalue', 'type': 'float', 'default': '0.0001', 'format': 'None'}, ]
        self.add_option(options)
        self.split_fasta = self.add_tool('ref_rna_v2.split_fasta')
        self.tools = list()
    ...
    def run_tf_predict(self):
        ...
        for fasta in fasta_files:
            ...
            tool = self.add_tool('ref_rna_v2.tf_predict')
    ...
    def merge_result(file_list, out_path, has_header=True):
        ...
        with open(out_path, 'w') as fw:
            if out_path.endswith("diamond.out.txt"):
                fw.write("Query-Name\tHit-Name\tIdentity_percent\tHsp_align-len\tMismatch\tHsp_gaps\tHsp_query-from\tHsp_query-to\tHsp_hit-from\tHsp_hit-to\tHsp_evalue\tHsp_bit-score\n")
            fr = open(file_list[0])
            fw.write(fr.read())
            fr.close()
