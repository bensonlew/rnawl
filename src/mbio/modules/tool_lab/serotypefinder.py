...
class SerotypefinderModule(Module):
    """
    多个样品的Serotypefinder分析
    """

    def __init__(self, work_id):
        super(SerotypefinderModule, self).__init__(work_id)
        option = [
    ...
    def run_serotypefinder(self):
        for sample in self.samples:
            serotypefinder = self.add_tool('tool_lab.serotypefinder')
            opts = {
                "fasta": self.option("fasta_dir").prop['path'] + "/" + self.fastas[sample],
                "sample_name": sample,
            }
            serotypefinder.set_options(opts)
            self.all_modules.append(serotypefinder)
    ...
    def run(self):
        super(SerotypefinderModule, self).run()
        self.run_serotypefinder()

    def end(self):
        super(SerotypefinderModule, self).end()