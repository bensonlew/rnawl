...
class BowtieAssemblyModule(Module):
    """
    宏基因组binning组装前的比对
    """
    def __init__(self, work_id):
        super(BowtieAssemblyModule, self).__init__(work_id)
        options = [
    ...
    def run_split_fastq(self):
        ...
        for sample, value in self.samples.items():
    ...
    def run_align(self):
        ...
        for sample in self.samples.keys():
            self.read1_path = (self.work_dir + "/" + sample + "_fastq_1")
            self.read2_path = (self.work_dir + "/" + sample + "_fastq_2")
    ...
    def run(self):
        super(BowtieAssemblyModule, self).run()
        self.run_build_index()

    def end(self):
        super(BowtieAssemblyModule, self).end()