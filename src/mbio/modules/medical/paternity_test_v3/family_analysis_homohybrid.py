...
class FamilyAnalysisHomohybridModule(Module):
    """
    做亲子鉴定一个家系分析的完整流程--胎儿位点是纯合与杂合的分析
    lasted modified by hongdong@20180821
    包含tool：family_merge_homohybrid、family_analysis, result_info_homohybrid
    """
    def __init__(self, work_id):
        super(FamilyAnalysisHomohybridModule, self).__init__(work_id)
        ...
        self.family_merge = self.add_tool("medical.paternity_test_v3.family_merge_homohybrid")
        self.father_analysis = self.add_tool("medical.paternity_test_v3.father_analysis")
        self.result_info = self.add_tool("medical.paternity_test_v3.result_info_homohybrid")
    ...
    def analysis_run(self):
        ...
        rdata = ""
        for f in results:
            if re.match(r'.*family_joined_tab_update\.Rdata$', f):
                rdata = f
    ...
    def run(self):
        super(FamilyAnalysisHomohybridModule, self).run()
    ...
    def end(self):
        super(FamilyAnalysisHomohybridModule, self).end()
