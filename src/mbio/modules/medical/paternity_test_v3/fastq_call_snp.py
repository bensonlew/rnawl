...
class FastqCallSnpModule(Module):
    """
        ...
        last modified by hongyuchen 20180810
    """
    def __init__(self, work_id):
        super(FastqCallSnpModule, self).__init__(work_id)
        self.step.add_steps('fastq2bam', 'fastq2bam_v3', 'bam2tab_v3', 'bam2tab', 'bam2tab_dc', 'pol_degree')
        options = [
            ...
            {"name": "split_type", "type": "string", "default": "PE"},   # 是单端还是双端
        ]
        self.add_option(options)
        self.fastq2bam = self.add_tool("medical.paternity_test_v3.fastq2bam")
        self.bam2tab = self.add_tool("medical.paternity_test_v3.bam2tab")
        self.bam2tab_dc = self.add_tool("medical.paternity_test_v3.bam2tab_dc")
        self.pol_degree = self.add_tool("medical.paternity_test_v3.pol_degree")
        self.fastq2bam_v3 = self.add_tool("medical.paternity_test_v3.fastq2bam_v3")
        self.bam2tab_v3 = self.add_tool("medical.paternity_test_v3.bam2tab_v3")
    ...
    def check_options(self):
        ...
        if not self.option('analysis_type'):
            raise OptionError("必须指定分析类型")
        if self.option('analysis_type') not in ["dcpt", "pt", "wqcf"]:
            raise OptionError("测序类型必须是dcpt/pt/wqcf")
        if self.option("split_type") not in ["PE", "SE"]:
            raise OptionError("拆分类型必须为PE/SE")
    ...
    def fastq2bam_run(self):
        ...
        if self.option('analysis_type') not in ["dcpt", "pt"]:
            self.set_error("tool: medical.paternity_test_v3.fastq2bam只允许analysis_type为dcpt/pt, 当前值：{}".format(
                self.option('analysis_type')))

        is_dcpt = True if self.option('analysis_type') in ["dcpt", "wqcf"] else False
    ...
    def fastq2bam_v3_run(self):
        self.logger.info(self.option("fastq_path"))

        self.fastq2bam_v3.set_options({
            "fastq": self.option("sample_id"),
            "is_dcpt": True,
            "ref_fasta": self.option("ref_fasta"),
            "targets_bedfile": self.option("targets_bedfile"),
        ...
        self.fastq2bam_v3.on('end', self.set_output, 'fastq2bam_v3')
        self.fastq2bam_v3.on('start', self.set_step, {'start': self.step.fastq2bam_v3})
        self.fastq2bam_v3.on('end', self.set_step, {'end': self.step.fastq2bam_v3})
        self.fastq2bam_v3.run()
    ...
    def bam2tab_v3_run(self):
        bam_path = os.path.join(self.fastq2bam_v3.output_dir, self.option("sample_id") + ".mem.sort.hit.filter.bam")
        self.logger.info(bam_path)
        self.bam2tab_v3.set_options({
            "sample_id": self.option("sample_id"),
            "bam_path": bam_path,
            "ref_fasta": self.option("ref_fasta"),
            "targets_bedfile": self.option("targets_bedfile"),
        ...
        self.bam2tab_v3.on('end', self.set_output, 'bam2tab_v3')
        self.bam2tab_v3.on('start', self.set_step, {'start': self.step.bam2tab_v3})
        self.bam2tab_v3.on('end', self.set_step, {'end': self.step.bam2tab_v3})
        self.bam2tab_v3.run()
    ...
    def pol_degree_run(self):
        if self.option('analysis_type') == "dcpt":
            ...
            dp = self.api.api('medical.paternity_test_v3.paternity_test_v3').query_dp(self.option("sample_id"))
            print 'dp:', dp
        ...
        elif self.option('analysis_type') == "wqcf":
            ref_dp_xls = os.path.join(self.bam2tab_v3.output_dir, self.option("sample_id") + ".ref.dp.xls")
            dp = self.api.api('medical.paternity_test_v3.paternity_test_v3').query_dp(self.option("sample_id"))
            print 'dp:', dp
            self.pol_degree.set_options({
                "ref_dp_xls": ref_dp_xls,
    ...
    def set_output(self, event):
        ...
        elif event['data'] == 'fastq2bam_v3':
            self.linkdir(obj.output_dir, 'fastq2bam_v3')
        elif event['data'] == 'bam2tab' or event['data'] == 'bam2tab_dc':
            self.linkdir(obj.output_dir, "bam2tab")
        elif event['data'] == 'bam2tab_v3':
            self.linkdir(obj.output_dir, "bam2tab_v3")
    ...
    def run(self):
        ...
        if self.option('analysis_type') == "pt":
            ...
            self.fastq2bam_v3.on('end', self.bam2tab_v3_run)
            self.bam2tab_v3.on('end', self.pol_degree_run)
            self.fastq2bam_v3_run()
        else:
