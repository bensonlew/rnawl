...
class DailyDedupModule(Module):
    ...
    def check_table(self):
        ...
        if not os.path.exists(self.output_dir + '/' + self.option("preg_id") + '.tab'):
            self.api_pt.export_tab_file(self.option("preg_id"), self.output_dir)
        else:
            self.logger.info("样本{}tab文件已经存在！".format(self.option("preg_id")))

    def dedup_run(self):
        n = 0
        # for p in range(2, self.option('err_min')):
        dedup = self.add_tool("medical.paternity_test_v2.dedup_multi")
        ...
        dedup.set_options({
            ...
            "father_path": self.option("ref_data"),
            "mem": self.option("mem")
        })
