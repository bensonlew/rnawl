...
class ProcrustesModule(Module):
    ...
    普氏分析module
    author: liulinmeng
    last_modify: 2018.08.14
    """
    
    def __init__(self,work_id):
        super(ProcrustesModule, self).__init__(work_id)
        options = [
            {'name': 'metab_table', 'type': 'infile', 'format': 'metabolome.metab_abun,sequence.profile_table','required':True},
            {'name': 'asso_table', 'type': 'infile', 'format': 'metabolome.metab_abun,sequence.profile_table,meta.otu.otu_table','required':True},
        ...
        self.procrustes_tool = self.add_tool("metabolome.metabset.procrustes")
        self.run_tools = [] #并行运行的tool

    def check_options(self):
        self.method = self.option('method')
        if self.method not in ["pca", "pcoa"]:
            raise OptionError("PARAMETERS ERROR:wrong value of method (%s), pca or pcoa expected!" % self.method)
        return True
    ...
    def discalc_run_ref(self):
        ...
        """
        options = {
            "otutable": self.option('metab_table').prop['path'],
            "method": self.option("metab_dist")
        }
