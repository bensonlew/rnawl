...
class CoverageWindowModule(Module):
    ...
    def check_options(self):
        ...
        if not self.option("step_num") in [5000, 10000, 50000, 100000, 200000, 500000]:
            raise OptionError("step_num 只能是5000，10000，50000,100000，200000，500000", code="24901002")
        return True
    ...
    def coverage_window_run(self):
        for f in os.listdir(self.option("bam_dir").prop["path"]):
            if re.match(".*bai$", f):   # 筛选去掉bai文件
                ...
                coverage_window = self.add_tool("wgs_v2.coverage_window")
    ...
    def set_db(self):
        ...
        if self.option("main_id"):
            api_coverage = self.api.api("wgs_v2.coverage_window")
