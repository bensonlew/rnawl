...
class AssemblyModule(Module):
    """
    用于对数据组装_v2版中捆绑bam2fastq, make_config, ngsqc_stat, soap_denovo
    """
    ...
    def assembly_stat_run(self):
        if len(self.soap_denovo_tools) == 1:
            cal_path = self.soap_denovo_tools[0].output_dir
        else:
            cal_path = self.output_dir + '/soap_denovo'
        for m in os.listdir(cal_path):
            n = re.match('(.*)\.denovo\.scafSeq', m)
            if n:
                assembly_stat = self.add_tool("wgs.assembly_stat")
                assembly_stat.set_options({
                    "denovo_scafseq": cal_path + "/{}.denovo.scafSeq".format(n.group(1)),
                    "denovo_scafstatistics": cal_path + "/{}.denovo.scafStatistics".format(n.group(1))
                })
                self.assembly_stat_tools.append(assembly_stat)
        for j in range(len(self.assembly_stat_tools)):
            self.assembly_stat_tools[j].on("end", self.set_output, 'assembly_stat')
        if self.assembly_stat_tools:
            if len(self.assembly_stat_tools) > 1:
                self.on_rely(self.assembly_stat_tools, self.end)
            elif len(self.assembly_stat_tools) == 1:
                self.assembly_stat_tools[0].on('end', self.end)
            for tool in self.assembly_stat_tools:
                gevent.sleep(1)
                tool.run()
        else:
            self.set_error("soap_denovo_tools列表为空！")
