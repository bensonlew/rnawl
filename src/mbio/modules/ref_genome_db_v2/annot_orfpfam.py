...
class AnnotOrfpfamModule(Module):
    ...
    def longorf_run(self):
        self.longorf = self.add_tool('ref_genome_db_v2.annotation.longorf')
        opts = ({
    ...
    def run_hmm(self):
        ...
        for f in os.listdir(self.splitfasta.output_dir):
            ...
            hmm_tool = self.add_tool('ref_genome_db_v2.annotation.hmm')
    ...
    def run_cathmmout(self):
        ...
        self.predict_tool = self.add_tool('ref_genome_db_v2.annotation.predict')
        ...
        ### 重运行会根据文件跳过相关步骤， 所以删除
        if os.path.exists(self.predict_tool.work_dir + "/" + self.fasta_name + "." + 'transdecoder_dir.__checkpoints/'):
            shutil.rmtree(self.predict_tool.work_dir + "/" + self.fasta_name + "." + 'transdecoder_dir.__checkpoints/')
