...
class AnnotMapdbModule(Module):
    ...
    def __init__(self, work_id):
        ...
        self.catblast = self.add_tool("ref_genome_db_v2.annotation.cat_blastout")
        self.splitfasta = self.add_tool("ref_genome_db_v2.annotation.split_fasta")
        self.xmlfilter = self.add_tool("ref_genome_db_v2.annotation.filter_annot")
    ...
    def run_blast_nr(self):
        ...
        for f in os.listdir(self.splitfasta.output_dir):
            ...
            nr2go = self.add_tool('ref_genome_db_v2.annotation.nr2go')
            nr2ncbitaxon = self.add_tool('ref_genome_db_v2.annotation.ncbi_taxon')
    ...
    def run_blast_string(self):
        ...
        for f in os.listdir(self.splitfasta.output_dir):
            ...
            string2cog_tool = self.add_tool('ref_genome_db_v2.annotation.string2cog')
    ...
    def run(self):
        ...
        for db in self.option("database").split(";"):
            if db == 'nr':
                self.merge_tools['nr'] = self.add_tool("ref_genome_db_v2.annotation.cat_blastout")
                self.merge_tools['GO'] = self.add_tool("ref_genome_db_v2.annotation.cat_table")
            elif db == 'string':
                self.merge_tools['string'] = self.add_tool("ref_genome_db_v2.annotation.cat_blastout")
            elif db == 'swissprot':
                self.merge_tools['swissprot'] = self.add_tool("ref_genome_db_v2.annotation.cat_blastout")
            elif db == 'kegg':
                self.merge_tools['kegg'] = self.add_tool("ref_genome_db_v2.annotation.cat_blastout")
            elif db == 'eggnog':
                self.merge_tools['eggnog'] = self.add_tool("ref_genome_db_v2.annotation.cat_blastout")
            else:
                self.logger.info("不支持该类型{}的数据库".format(db))
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            "type": "module",
            "name": "ref_genome_db_v2.annot_mapdb",
            "instant": False,
            "options": dict(
