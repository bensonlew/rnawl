...
class SingleMirnaQcModule(Module):
    ...
    def __init__(self, work_id):
        ...
        options = [
            ...
            {"name": "skip_qc", "type": "string", "default": "no"},
        ]
        self.add_option(options)
        self.cut_left = self.add_tool("small_rna.qc.cut_left51")
    ...
    def run_fastx_clipper(self):
        ...
        self.fastx_clipper = self.add_tool("small_rna.qc.fastx_clipper")
    ...
    def run_dynamic_trim(self):
        ...
        self.dynamic_trim = self.add_tool("small_rna.qc.dynamic_trim")
    ...
    def run_fastq_fasta(self):
        if self.option("skip_qc") == "yes":
            options = {
                "fastq_input": self.option("fastq"),
            }
        else:
            options = {
                "fastq_input": self.dynamic_trim.option("out_fastq")
            }
        self.fastq_fasta = self.add_tool("sequence.fastq_to_fasta")
    ...
    def run_trim_seq(self):
        ...
        self.trim_seq = self.add_tool("small_rna.qc.trim_seq")
    ...
    def set_output(self, event):
        ...
        if event["data"] == "trim_seq":
            ...
            if self.option("skip_qc") == "yes":
                ##原始数据
                fastq_name  =os.path.basename(self.option("fastq").prop['path'])
                count = -1
                ...
                if os.path.exists(os.path.join(self.output_dir, fastq_name)):
                    os.remove(os.path.join(self.output_dir, fastq_name))
                os.link(self.option("fastq").prop['path'], os.path.join(self.output_dir, fastq_name))
                self.option("raw_fastq", os.path.join(self.output_dir, fastq_name))
                trim_fq_name = self.option("sample_name") +  "_clip_s.fastq.trimmed"

                if os.path.exists(os.path.join(self.output_dir, trim_fq_name)):
                    os.remove(os.path.join(self.output_dir, trim_fq_name))
                os.link(self.option("fastq").prop['path'], os.path.join(self.output_dir, trim_fq_name))
                self.option("out_fastq", os.path.join(self.output_dir, trim_fq_name))
            ...
            if os.path.exists(os.path.join(self.output_dir, "Sample_QC_stat.xls")):
                ...
                if self.option("skip_qc") == "yes":
                    with open(os.path.join(self.output_dir, "Sample_QC_stat.xls"), 'w') as stat_out:
                        stat_out.write("Sample\tRaw_reads\tAdapter_only\tN_reads\t<18nt\t>32nt\tClean_reads\tAdapter%\n")
                        stat_out.write("{}\t{}\t0\t0\t0\t0\t{}\t0\n".format(self.option("sample_name"), seq_num, seq_num))
    ...
    def run(self):
        ...
        if self.option("skip_qc") == "yes":
            self.run_fastq_fasta()
        elif self.option("cut_left") > 0 or self.option("length_contain") > 0:
            self.run_cut_left()
        else:
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            "type": "module",
            "name": "small_rna.single_mirna_qc",
            "instant": False,
            "options": dict(
                fastq = test_dir + "/" + "data0/sample1_1.fq",
            )
        }
