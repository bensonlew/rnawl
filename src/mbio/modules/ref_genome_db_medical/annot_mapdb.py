...
class AnnotMapdbModule(Module):
    ...
    def __init__(self, work_id):
        ...
        options = [
            ...
            {"name": "lines", "type": "int", "default": 1000000},  # 将fasta序列拆分此行数的多个文件
            {"name": "query_type", "type": "string", "default": "nucl"},  # 输入的查询序列的格式，为nucl或者prot
            # {""},
            {"name": "database", "type": "string", "default": "nr;kegg;uniprot;swissprot;eggnog"},
            ...
            {"name": "uniprot_db", "type": "string", "default": "Mammalia"},
        ...
        self.catblast = self.add_tool("medical_transcriptome.annotation.cat_blastout")
        self.splitfasta = self.add_tool("medical_transcriptome.annotation.split_fasta")
        self.xmlfilter = self.add_tool("medical_transcriptome.annotation.filter_annot")
        self.step.add_steps('blast2go', 'split_fasta',
                            'blast_nr', 'blast_string', 'blast_kegg', 'blast_swissprot', 'blast_uniprot', 'blast_eggnog',
                            'merge_nr', 'merge_string', 'merge_kegg', 'merge_swissprot', 'merge_uniprot', 'merge_eggnog')
        self.merge_step = {
            "nr": self.step.merge_nr,
            "string": self.step.merge_string,
            "kegg": self.step.merge_kegg,
            "swissprot": self.step.merge_swissprot,
            "uniprot": self.step.merge_uniprot,
    ...
    def run_blast_nr(self):
        ...
        for f in os.listdir(self.splitfasta.output_dir):
            ...
            if self.option('method') == "blast":
                blast_tool = self.add_tool('medical_transcriptome.annotation.blast')
            else:
                blast_tool = self.add_tool('medical_transcriptome.annotation.diamond')
            ...
            nr2go = self.add_tool('medical_transcriptome.annotation.nr2go')
            nr2ncbitaxon = self.add_tool('medical_transcriptome.annotation.ncbi_taxon')
    ...
    def run_blast_uniprot(self):
        '''
        uniprot blast比对
        '''
        opts = self.blast_opts.copy()
        opts.update({"database": self.option("uniprot_db")})
        opts.update({"db": "uniprot"})
        opts.update({"version": self.option("version")})
        opts.update({"uniprot_version": self.option("uniprot_version")})
        opts.update({"diamond_version": self.option("diamond_version")})
        if self.option("diamond_version") >= "v2.0.13":
            opts.update({"sensitive": 1})
        i = 0
        for f in os.listdir(self.splitfasta.output_dir):
            ...
            if self.option('method') == "blast":
                ...
                blast_tool = self.add_tool('medical_transcriptome.annotation.diamond')
            blast_tool.set_options(opts)
            self.blast_uniprot_tools.append(blast_tool)

        if len(self.blast_uniprot_tools) == 1:
            self.blast_uniprot_tools[0].on("end", self.run_catblastout, "uniprot")
        else:
            self.on_rely(self.blast_uniprot_tools, self.run_catblastout, "uniprot")
        for tool in self.blast_uniprot_tools:
            tool.run()
    ...
    def run_blast_string(self):
        ...
        for f in os.listdir(self.splitfasta.output_dir):
            ...
            string2cog_tool = self.add_tool('medical_transcriptome.annotation.string2cog')
    ...
    def run_catblastout(self, event):
        ...
        elif event['data'] == 'uniprot':
            tools = self.blast_uniprot_tools
    ...
    def run(self):
        ...
        for db in self.option("database").split(";"):
            if db == 'nr':
                self.merge_tools['nr'] = self.add_tool("medical_transcriptome.annotation.cat_blastout")
                self.merge_tools['GO'] = self.add_tool("medical_transcriptome.annotation.cat_table")
            elif db == 'string':
                self.merge_tools['string'] = self.add_tool("medical_transcriptome.annotation.cat_blastout")
            elif db == 'swissprot':
                self.merge_tools['swissprot'] = self.add_tool("medical_transcriptome.annotation.cat_blastout")
            elif db == 'uniprot':
                self.merge_tools['uniprot'] = self.add_tool("medical_transcriptome.annotation.cat_blastout")
            elif db == 'kegg':
                self.merge_tools['kegg'] = self.add_tool("medical_transcriptome.annotation.cat_blastout")
            elif db == 'eggnog':
                self.merge_tools['eggnog'] = self.add_tool("medical_transcriptome.annotation.cat_blastout")
            else:
                self.logger.info("不支持该类型{}的数据库".format(db))
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        test_dir = '/mnt/ilustre/users/sanger-dev/sg-users/liubinxu/medical_transcriptome'
        data = {
            "id": "annot_db" + "rerun",
            "type": "module",
            "rerun": True,
            "skip_all_success": True,
            "name": "medical_transcriptome.annot_mapdb",
            "instant": False,
            "options": dict(
                query = test_dir + "/assemble_out/NewTranscripts/new_transcripts.fa",
                # database="nr",
                method = "diamond",
                nr_db = "animal",
                lines = 5000
