...
class QuantLargeModule(Module):
    """
    对所有样本进行定量
    """
    def __init__(self, work_id):
        super(QuantLargeModule, self).__init__(work_id)
        options = [
        ...
        self.tools = list()
        self.samples = list()
        self.merge_tool = self.add_tool('ref_rna_v3.large.quant_merge')
    ...
    def run(self):
        super(QuantLargeModule, self).run()
        self.tool_run()
    ...
    def tool_run(self):
        ...
        for sample, fq_list in sorted(fastq_dict.items()):
            ...
            tool = self.add_tool('ref_rna_v2.single_quant')
    ...
    def prepare_quant_files_json(self):
        quant_dict = {
        ...
        with open(os.path.join(self.work_dir,"quant_large.json"), 'w') as json_f:
            json.dump(quant_dict, json_f, sort_keys=True, indent=4)
        return os.path.join(self.work_dir,"quant_large.json")



    def set_output(self):
        self.logger.info("Set output of expression quantification")
        self.logger.info("首先准备json文件")
        target_json = self.prepare_quant_files_json()
        opts = {
            "merge_file" : target_json
        }
        self.merge_tool.set_options(opts)
        self.merge_tool.on("end", self.set_opts)
        self.merge_tool.run()


    def set_opts(self):
        self.logger.info("Set output of expression quantification")
        opts_json = self.merge_tool.option("options_dict").prop["path"]
        with open(opts_json, 'r') as f:
            opts_json_dict = json.loads(f.read())
        def link_file(raw_path,link_path):
            if os.path.exists(link_path):
                os.remove(link_path)
            os.link(raw_path,link_path)
        for opt in opts_json_dict:
            if opt == "bamlist" :
                raw_path = opts_json_dict[opt]
                link_path = os.path.join(self.work_dir,os.path.basename(raw_path))
                link_file(raw_path,link_path)
                self.option(opt).set_path(link_path)
            elif opt == "align_rate" :
                raw_path = opts_json_dict[opt]
                link_path = os.path.join(self.work_dir, os.path.basename(raw_path))
                link_file(raw_path, link_path)
                self.set_options(dict(align_rate=link_path))
            else:
                raw_path = opts_json_dict[opt]
                link_path = os.path.join(self.output_dir, os.path.basename(raw_path))
                link_file(raw_path, link_path)
                self.option(opt).set_path(link_path)
        self.end()
    ...
    def end(self):
        ...
        super(QuantLargeModule, self).end()
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            "type": "module",
            "name": "ref_rna_v2.quant_large",
            "instant": False,
            "options": dict(
                transcriptome="/mnt/ilustre/users/isanger/workspace/20210331/Refrna_olnf_29205qlouth3epbqigcf0p/RefrnaAssemble/output/NewTranscripts/all_transcripts.fa",
                fastq="/mnt/ilustre/users/isanger/workspace/20210331/Refrna_olnf_29205qlouth3epbqigcf0p/FastpRna/output/fastq/fq_list.txt",
                method="rsem",
                t2g="/mnt/ilustre/users/isanger/workspace/20210331/Refrna_olnf_29205qlouth3epbqigcf0p/RefrnaAssemble/output/NewTranscripts/trans2gene",
                pool=6,
                thread=6,
                output=None,
                read_len=149,
                read_len_sd=30,
                map_tool="bowtie2",
                ref_gtf = "/mnt/ilustre/users/isanger/workspace/20210331/Refrna_olnf_29205qlouth3epbqigcf0p/FileCheck/final.gtf"
            )
        }
