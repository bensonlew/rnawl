...
import unittest
from mbio.packages.labelfree.kegg_enrichment import multtest_correct
...
class DiffGoEnrichModule(Module):
    def __init__(self, work_id):
        ...
        options = [
            ...
            {'name': 'go_list', 'type': 'infile', 'format': 'labelfree.common'},
    ...
    def run_go_enrich(self):
        ...
        for diff in diff_files:
            ...
            if len(uplist + downlist) > 10:
                de_tool = self.add_tool("labelfree.proteinset.go_enrich")
                de_tool.set_options(options)
                self.tools.append(de_tool)
            if len(uplist) > 10:
                up_tool = self.add_tool("labelfree.proteinset.go_enrich")
                options.update(dict(diff_list=acc_up))
                up_tool.set_options(options)
                self.tools.append(up_tool)
            if len(downlist) > 10:
                down_tool = self.add_tool("labelfree.proteinset.go_enrich")
