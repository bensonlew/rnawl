...
class DiffClusterModule(Module):
    def __init__(self, work_id):
        ...
        options = [
            ...
            {'name': 'exp', 'type': 'infile', 'format': 'labelfree.ratio_exp'},
            {'name': 'group', 'type': 'infile', 'format': 'labelfree.group_table'},
    ...
    def prepare_exp(self):
        ...
        if len(diff_list) < 5:
            self.logger.info('差异蛋白只有%s个，cluster退出运行')
            return
    ...
    def run_clusters(self):
        ...
        for i in [5, 10]:
            if exp_df.shape[0] < i:
                i = exp_df.shape[0]
            cluster_yes = self.add_tool("labelfree.exp_cluster")
            cluster_no = self.add_tool("labelfree.exp_cluster")
            options = dict(
                exp=self.option('exp'),
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        ...
        data = {
            ...
            "type": "module",
            "name": "labelfree.diff_cluster",
            "instant": False,
            "options": dict(
                diff_path="/mnt/ilustre/users/sanger-dev/workspace/20190624/Labelfree_tsg_34554/Diff/output",
                exp="/mnt/ilustre/users/sanger-dev/workspace/20190624/Labelfree_tsg_34554/treat_ref",
                group="/mnt/ilustre/users/sanger-dev/workspace/20190624/Labelfree_tsg_34554/remote_input/protein_group/group.txt",
            )
        }
