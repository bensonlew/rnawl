...
class GenomesPredictModule(Module):
    """
    所有基因组的结果统计
    """
    def __init__(self, work_id):
        super(GenomesPredictModule, self).__init__(work_id)
        options = [
    ...
    def run_genomepredict(self):
        for file in os.listdir(self.option("genomes").prop['path']):
            genome_pre = self.add_module('toolapps.genome_predict')
            opts = {
    ...
    def run(self):
        ...
        :return:
        """
        super(GenomesPredictModule, self).run()
    ...
    def set_output(self):
        ...
        if len(self.modules) > 1:
            ...
            file3 = os.path.basename(self.modules[0].option("stat").prop['path'])
    ...
    def end(self):
        super(GenomesPredictModule, self).end()