...
class CdhitParaModule(Module):
    ...
    def check_query(self, fa):
        '''
        检测自比的输入是否为空，如果为空则写入本轮database fa的第一条序列
        '''
        with open(fa, 'r') as r:
            has_seq = r.readline().startswith('>')
        if not has_seq:
            pre_db = self.option("in_dir") + "/gene.geneset.tmp.fa.div-" + str(self.option("first") - 1) + "-/o"
            with open(pre_db, 'r') as r, open(fa, 'w') as w:
                w.write(r.readline())
                while 1:
                    l = r.readline()
                    if l.startswith('>'):
    ...
    def single_run(self):
        ...
        query = self.option("in_dir") + "/gene.geneset.tmp.fa.div-" + str(self.option("first")) + "-/vs." + str(self.option("first") - 1)
        self.check_query(query)
        self.compare_single.set_options({
            # "query": self.option("in_dir") + "/gene.geneset.tmp.fa.div-" + str(self.option("first")),
