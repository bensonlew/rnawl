...
class CazyAlignAnnoModule(Module):
    def __init__(self, work_id):
        super(CazyAlignAnnoModule, self).__init__(work_id)
        options = [
    ...
    def run_hmmscan(self):
        ...
        self.hmmscan.set_options({
            ...
            'lines': n,
            "database": "cazy_v20200408",## add by qingchen.zhang@20200922
        })
    ...
    def set_output(self, event):
        ...
        if event['data'] == 'anno':
            ...
            self.option('cazy_family_profile', obj.output_dir + "/cazy.family.profile.xls")
            self.option('cazy_class_profile', obj.output_dir + "/cazy.class.profile.xls")
            self.end()

    def run(self):
        super(CazyAlignAnnoModule, self).run()
        self.run_hmmscan()

    def end(self):
        super(CazyAlignAnnoModule, self).end()
