...
class DiffexpBatchWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self.tool = self.add_tool("ref_rna_v3.batch.diffexp_batch")
        ...
        if self._sheet.rerun:
            ...
            interactiondelete =  InteractionDelete(bind_object=self,project_type="ref_rna_v2",main_id=self.option('diff_main_id'))
    ...
    def set_db(self):
        ...
        if self.option("method").lower() in ["degseq", "edger", "deseq2", 'limma', 'svaseqlimma']:
            self.all_exp.add_diffexp(self.tool.output_dir,
                                     ...
                                     s3_output_dir=self._sheet.output
                                     )
        else:
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("ref_rna_v2", table="sg_diff", main_id=self.option('diff_main_id'),
    ...
    def set_output(self):
        diff_total = pd.read_table(
            os.path.join(self.tool.output_dir, "total_diff_stat.{}.xls".format(self.option("method").lower())),
            index_col="seq_id")
        diff_files_stat = glob.glob(
            os.path.join(self.tool.output_dir, '*_vs_*.{}.annot.xls'.format(self.option("method").lower())))
        ...
        df1_geneanno = pd.DataFrame(df_anno,
                                    columns=["nr", "go", "KO_id", "KO_name", "paths", "cog", "cog_description", "pfam",
                                             "swissprot", "entrez"])
        ...
        dff2.to_csv(self.tool.output_dir + "/total_diff_stat.{}.annot.xls".format(self.option("method").lower()), sep="\t")


        # files = glob.glob(self.tool.output_dir + "/*.annot.xls") + \
        #         glob.glob(self.tool.output_dir + "/diff_summary_*.xls") + \
        #         glob.glob(self.tool.output_dir + "/*.pdf") + \
        #         glob.glob(self.tool.output_dir + "/total_diff_stat*.xls")
        #modify by fwy 20210622 为了
        files = glob.glob(self.tool.output_dir + "/*.annot.xls") + \
                glob.glob(self.tool.output_dir + "/diff_summary_*.xls") + \
                glob.glob(self.tool.output_dir + "/*.pdf")
        for b in files:
            if os.path.exists(os.path.join(self.output_dir,os.path.basename(b))):
                os.remove(os.path.join(self.output_dir,os.path.basename(b)))
            if not os.path.basename(b).startswith("total_diff_stat"):
                os.link(b, os.path.join(self.output_dir, os.path.basename(b)))
            else:
                if os.path.exists(os.path.join(self.output_dir, os.path.basename(b).replace("annot.",""))):
                    os.remove(os.path.join(self.output_dir, os.path.basename(b).replace("annot.","")))
                os.link(b, os.path.join(self.output_dir, os.path.basename(b).replace("annot.","")))
    ...
    def paste_annotation(self):
        ...
        for each in glob.glob(os.path.join(self.tool.output_dir, '*_vs_*.*.xls')):
            ...
            if self.option("method").lower() in ["degseq", "edger", "deseq2", 'limma', 'svaseqlimma']:
                ...
                try:
                    b = pd.DataFrame(diff_pd,
                                     columns=['{}_mean'.format(ctrl), '{}_mean'.format(test), "fc", "log2fc", "theta",
                                              "D", 'prob', "significant", "regulate"])
                    c = diff_pd.drop(
                        ['{}_mean'.format(ctrl), '{}_mean'.format(test), "fc", "log2fc", "theta", "D", 'prob',
                         "significant", "regulate"], axis=1)
                except:
                    b = pd.DataFrame(diff_pd,
                                     columns=['{}_mean'.format(ctrl), '{}_mean'.format(test), "fc", "log2fc", "D",
                                              'prob', "significant", "regulate"])
                    c = diff_pd.drop(
                        ['{}_mean'.format(ctrl), '{}_mean'.format(test), "fc", "log2fc", "D", 'prob', "significant",
                         "regulate"], axis=1)
            ...
            each  = os.path.join(self.tool.output_dir,os.path.basename(each).split('.xls')[0] + '.annot.xls')
