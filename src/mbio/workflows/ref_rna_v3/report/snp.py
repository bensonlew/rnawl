...
class SnpWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            dict(name = "analysis_format", type="string", default="bam"),
        ...
        self._sheet.output = self._sheet.output.replace('interaction_results', 'interaction_results/05 SNP_InDel_Analysis')
        self.inter_dirs = []
        if self._sheet.rerun:
            ...
            interactiondelete = InteractionDelete(bind_object=self, project_type="ref_rna_v2",
                                                  main_id=self.option('snp_main_id'))
            interactiondelete.delete_interactions_records()

        self.sopts ={}
        if self.option('method_type').lower() == 'gatk':
            self.gatk = self.add_module("ref_rna_v3.snp_rna")
        if self.option('method_type').lower() == 'samtools':
            self.samtools = self.add_module("ref_rna_v3.sam_rna")
        if self.option("method_type").lower() =='sentieon':
            # self.realign=self.add_module("ref_rna_v3.bam_realign")
            self.sentieon=self.add_module("ref_rna_v3.call_snp_indel_sentieon")
    ...
    def run(self):
        ...
        if self.option("method_type").lower() == "sentieon":
            # self.realign.on("end",self.sentieon_run)
            self.sentieon.on("end",self.set_db)
            self.logger.info("在sentieon的run_tool之前")
            self.run_tool()
            self.logger.info("在sentieon的run_tool之后")
    ...
    def set_db(self):
        ...
        保存结果表到mongo数据库中
        """
        self.api_snp = self.api.api("ref_rna_v3.ref_snp")
        # add result info
        if self.option('method_type').lower() == 'samtools':
            snp_anno = self.samtools.output_dir
            self.api_snp.add_snp_main(snp_anno=snp_anno, group=self.option('sample_list_str'), main_id=self.option('snp_main_id'), new_output=self.output_dir)
        if self.option('method_type').lower() == 'gatk':
    ...
    def chart(self):
        ...
        if self.option('method_type').lower() == 'samtools':
            snp_result = self.samtools.output_dir
        elif self.option('method_type').lower() == 'gatk':
            snp_result = self.gatk.output_dir
        elif self.option('method_type').lower() == 'sentieon':
            snp_result = os.path.join(self.sentieon.output_dir,"predeal")
        self.logger.info("dir is {}".format(snp_result))
    ...
    def end(self):
        ...
        if self.option('method_type').lower() == 'samtools':
            ...
            self.inter_dirs = [
                ["05 SNP_InDel_Analysis", "", "SNP/InDel分析结果目录", 0],
        ...
        if self.option('method_type').lower() == 'sentieon':
            ...
            result_dir.add_relpath_rules([
                [".", "", "SNP_InDel分析文件", 0, "211267"],
                ["snp_anno.xls", "", "SNP分析结果注释详情表", 0, "211268"],
                ["indel_anno.xls", "", "InDel分析结果注释详情表", 0, "211269"],
                ["snp_transition_tranversion_statistics.xls", "", "SNP类型统计结果表", 0, "211270"],
                #["snp_freq_statistics.xls", "", "SNP频率统计结果表格", 0, "211263"],
                ["snp_depth_statistics.xls", "", "SNP深度统计结果表", 0, "211271"],
                ["snp_position_distribution.xls", "", "SNP不同区域分布结果表", 0, "211272"],
                ["indel_position_distribution.xls", "", "InDel不同区域分布结果表", 0, "211273"],
                ['run_parameter.txt', 'txt', '运行参数日志', 0],
                ['*.pdf', 'txt', "snp统计图", 0],
                ])
    ...
    def run_tool(self):
        ...
        if self.option('method_type').lower() == 'sentieon':
            ...
            self.sentieon.set_options(opts2)
