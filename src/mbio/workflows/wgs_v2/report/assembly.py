...
class AssemblyWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            {"name": "is_bucket", "type": "bool", "default": False},
        ...
        self.path_ = ''
        self.base_genome = Config().SOFTWARE_DIR + "/database/dna_geneome/" if \
            self.option('project_type') == 'dna_wgs_v2' else Config().SOFTWARE_DIR + "/database/dna_wgs_geneome/"
        # self.base_genome = Config().SOFTWARE_DIR + "/database/dna_wgs_geneome/"  # 测试先用，上线后换成上面的

    def check_options(self):
        if not self.option("samples"):
            raise OptionError("缺少samples参数")
        if not self.option("poss"):
            raise OptionError("缺少poss参数")
        if not self.option("unmapping"):
            raise OptionError("缺少unmapping参数")
        if not self.option("posname"):
            raise OptionError("缺少posname参数")
        return True
    ...
    def set_which_analysis(self):
        ...
        for pos in poss:
            ...
            if chr_ and chr_ not in chrlist:
                self.set_error("染色体编号%s不在该物种基因组中存在！正常结束，请修改参数" % chr_)
    ...
    def get_chr_list(self):
        chrlist_path = self.base_genome + os.path.join(os.path.dirname(os.path.dirname(self.ref_db)).lstrip('/'),
                                                       "total.chrlist")
        chrlist = []
    ...
    def get_chr_max_len(self):
        ...
        :return:
        """
        ref_dict = self.base_genome + os.path.join(os.path.dirname(os.path.dirname(self.ref_db)).lstrip('/'),
                                                   "ref.dict")
        if not os.path.exists(ref_dict):
            self.set_error("文件%s不存在！" % ref_dict)
        chr_len = {}
    ...
    def set_chr_len(self, pos, chr_len):
        """
        这里初始化位置信息，根据前端传进来的chr1:1-200格式进行初始化
        :param pos:
        :param chr_len:
        :return:
        """
        ...
        chr_, postion = pos.split(':')
        start1, end1 = postion.split('-')
        temp = [chr_, start1, end1]
        if temp[1] or temp[2]:
            if not temp[0]:
                self.set_error("输入参数%s不合法,有起始碱基位置与终止碱基位置，必须要有染色体编号！" % pos)
        if temp[1]:
            # noinspection PyBroadException
            try:
                start_ = int(temp[1])
            except:
                self.set_error("输入参数不合格，起始碱基位置必须为整型")
            else:
        ...
        if temp[2]:
            ...
            try:
                ...
                self.set_error("输入参数不合格，终止碱基位置必须为整型")
            else:
                if end_ < 0:
                    end = ""
    ...
    def check_bam_is_ok(self):
        ...
        for m in self.option("samples").split(","):
            if not os.path.exists(os.path.join(self.path_, "{}.sort.bam".format(m))):
                self.set_error("文件%s不存在！".format(os.path.join(self.path_, "{}.sort.bam".format(m))))

    def run_assembly(self):
        for item in self.analysis_num:
            assembly = self.add_module("wgs_v2.assembly")
            options = {
        ...
        if self.assembly_modules:
            ...
            self.set_error("assembly_modules列表为空！")

    def gene_anno_run(self):
        soap_data = self.set_soap_file()
        for m in soap_data:
            anno = self.add_module("wgs.gene_anno_v2")
            options = {
                "fasta": m.values()[0],
                "sample_id": m.keys()[0]
            }
    ...
    def set_soap_file(self):
        ...
        if len(self.assembly_modules) > 1:
            ...
            path = os.path.join(self.assembly_modules[0].output_dir, "soap_denovo")
    ...
    def blast_run(self):
        ...
        for m in soap_data:
            ...
            options = {
                "query_fa": m.values()[0],
                "sample_id": m.keys()[0],
                "dbname_nsq": self.base_genome + self.ref_db,
                "outfmt": "6"
            }
        ...
        if self.blastn:
            ...
            self.set_error("blastn列表为空！")

    def summary_set_run(self):
        for m in os.listdir(self.output_dir + "/gene_anno"):
            if len(self.blastn) > 1:
                blast_file = self.output_dir + "/blast/{}.blast".format(m.split('.')[0])
            else:
                blast_file = self.blastn[0].output_dir + "/{}.blast".format(m.split('.')[0])
            if not os.path.exists(blast_file):
                self.set_error("文件%s不存在！" % blast_file)
        ...
        if self.summary_set:
            ...
            self.set_error("summary_set列表为空！")
    ...
    def set_db(self):
        ...
        # reads 数量统计表
        possname = self.make_region_name()
        self.logger.info("possname:{}".format(possname))
        file_reads = os.path.join(self.output_dir, "qc_stat")
        self.assembly_api.add_qc_file(file_reads, self.option("main_id"), possname)
        assembly_stat = os.path.join(self.output_dir, "assembly_stat")
        self.assembly_api.add_pc_file(assembly_stat, self.option("main_id"), possname)
        ...
        if len(self.summary_set) == 1:
            self.assembly_api.add_anno_file(self.summary_set[0].output_dir,  self.option("main_id"), possname)
        else:
    ...
    def make_region_name(self):
        possname = {}
        pos = self.option("poss").split("|")
        posname = self.option("posname").split("|")
        for i in range(0, len(pos)):
            possname[pos[i]] = posname[i]
        return possname
    ...
    def set_output_file(self):
        ...
        rm_dir.append(self.output_dir + "/kegg_kobas")
        rm_dir.append(self.output_dir + "/pfam_scan")
        rm_dir.append(self.output_dir + "/interpro_scan")
        rm_dir.append(self.output_dir + "/diamond")
        for dirs in rm_dir:
