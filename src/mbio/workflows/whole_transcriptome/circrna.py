...
class CircrnaWorkflow(Workflow):
    ...
    def check_mapping(self):
        if self.option('mapping_stop'):
            if self.modules['rnaseq_mapping'].option('mapping_sample_percent') > self.option('mapping_sample_percent'):
                ...
                self.on_rely([self.modules['map_assessment'], self.modules['diff_exp_c']], self.pack_and_compress)
    ...
    def run_circ_brush(self):
        ...
        if self.genome_doc_lnc_dir:
            annotate = os.path.join(self.db_path, self.genome_doc_lnc_dir, 'mrna.gtf')
        else:
            annotate = os.path.join(self.db_path, self.genome_doc_gtf)
    ...
    def export_qc(self):
        ...
        api.add_qc_detail(qc_id, qc_stat_before, qc_stat_after, 'circle', group=self.option('group_table').path)
    ...
    def export_mapping(self, assess=True):
        ...
        api.add_mapping_stat(stat_file=stat_file, library='circle', method=method, group=self.option('group_table').path)
        if assess:
            ...
            api.add_chrom_distribution_table(chr_dir, params=params, library='circle', group=self.option('group_table').path)
            api.add_coverage_table(cov_dir, params=params, detail=True, library='circle')
            api.add_distribution_table(dis_dir, params=params, library='circle', group=self.option('group_table').path)
    ...
    def export_expression_stat_circ(self):
        ...
        api.add_expression_statcirc(stat, self.task_id, self.project_sn, group=self.option('group_table').path)
