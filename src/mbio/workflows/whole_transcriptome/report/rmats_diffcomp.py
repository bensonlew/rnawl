...
class RmatsDiffcompWorkflow(Workflow):
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("whole_transcriptome", table="splicing_rmats_diffcomp", main_id=self.option('main_id'),
    ...
    def run_rmats_diffcomp(self):
        ...
        self.rmats_diffcomp = self.add_tool('whole_transcriptome.structure.rmats_diffcomp')
    ...
    @workfuncdeco
    def set_db(self):
        api = self.api.api('whole_transcriptome.rmats_diffcomp')
...
class TestFunction(unittest.TestCase):
    ...
    '''
    def test(self):
        from mbio.workflows.whole_transcriptome.report.rmats_diffcomp import RmatsDiffcompWorkflow
        ...
        data = {
            ...
            'options': {
                's3_file_list': '/mnt/ilustre/users/sanger-dev/workspace/20191018/RmatsDiffcomp_tsg_34883_4776_5307/table.list',
                'delta_psi': '0.0',
                'significant_diff': 'fdr',
                'significant_value': '0.05',
                'main_id': '5da927cf17b2bf11432136f1'
            }
        }
        wsheet = Sheet(data=data)
        wf = RmatsDiffcompWorkflow(wsheet)
        wf.sheet.id = 't33555_8908_5404'
