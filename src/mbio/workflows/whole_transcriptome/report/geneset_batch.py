...
class GenesetBatchWorkflow(Workflow):
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            dict(name='category', type='string'),
        ...
        self.tool = self.add_tool("whole_transcriptome.export_diff_genesets")
    ...
    def set_db(self):
        geneset_self = self.api.api("whole_transcriptome.geneset_batch")
        geneset_file = os.path.join(self.tool.output_dir, "genesets")
        geneset_self.add_diff_genesets(self.option('task_id'), geneset_file, self.option('category'),
                                       self.option('geneset_type'), self.option('geneset_suffix'),
                                       self.option('diff_id'))
        self.end()
