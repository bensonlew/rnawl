...
class GenesetClusterWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self.tool = self.add_tool("whole_transcriptome.exp_cluster")
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("whole_transcriptome", table="geneset_cluster", main_id=self.option('cluster_main_id'),
    ...
    def chart(self):
        ...
        chart.chart_geneset_cluster(cluster_exp, cluster_tree, sample_tree, subcluster_list, group_dict=group_dict,level=self.level)
        chart.to_pdf()
        ...
        for p in pdf_file:
            print
            "copy", p, self.tool.output_dir + "/" + os.path.basename(p)
    ...
    def end(self):
        ...
        upload_name = glob.glob(self.tool.output_dir + "/*.xls")
        for files in upload_name:
            shutil.copy(files, self.output_dir)
