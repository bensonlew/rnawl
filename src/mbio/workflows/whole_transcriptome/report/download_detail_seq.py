...
class DownloadDetailSeqWorkflow(Workflow):
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            {'name': 'category', 'type': 'string', 'default': 'mrna'},
        ...
        self._sheet.output = self._sheet.output.replace('interaction_results',
                                                        'interaction_results/08 Seq_Download')
        self.download = self.add_tool("whole_transcriptome.extract_detail_seq")
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("meidical_transcriptome", table="sg_seq_extract", main_id=self.option('main_id'),
    ...
    def end(self):
        ...
        result_dir.add_relpath_rules([
            ...
            ['./extract_gene_seqs.fa', '', '基因序列', 0],
            ['./extract_trans_seqs.fa', '', '转录本序列', 0],
            ['./extract_cds_seqs.fa', '', 'CDS序列', 0],
            ['./extract_pep_seqs.fa', '', '氨基酸序列', 0],
            ['./extract_lnc_seqs.fa', '', 'lncRNA序列', 0],
            ['./extract_mir_seqs.fa', '', 'miRNA序列', 0],
            ['./extract_circ_seqs.fa', '', 'circRNA序列', 0],
        ])
