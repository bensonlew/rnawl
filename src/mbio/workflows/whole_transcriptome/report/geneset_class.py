...
class GenesetClassWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        if self.option("anno_type") == "cog":
            self._sheet.output = self._sheet.output.replace('interaction_results','interaction_results/04 GeneSet/02 COG_Annotion')
        else:
            self._sheet.output = self._sheet.output.replace('interaction_results','interaction_results/04 GeneSet/03_GO Annotion')
        self.inter_dirs = []
    ...
    def get_run_log(self):
        if self.option("anno_type") == "cog":
            get_run_log = GetRunLog("whole_transcriptome", table="geneset_cog_class", main_id=self.option('main_table_id'),
                                    dir_path=self.work_dir)
        else:
            get_run_log = GetRunLog("whole_transcriptome", table="geneset_go_class", main_id=self.option('main_table_id'),
                                    dir_path=self.work_dir)
    ...
    def chart(self):
        ...
        if self.option("anno_type") == "cog":
            ...
            chart.chart_geneset_class_cog(cog_class_table,level=self.level)
        elif self.option("anno_type") == "go":
    ...
    def end(self):
        ...
        self.inter_dirs = [
            ...
            ["04 GeneSet/02 COG_Annotion", "", "COG功能注释", 0],
            ["04 GeneSet/03 GO_Annotion", "", "GO功能注释", 0]
        ]
        if self.option("anno_type") == "go":
            ...
            result_dir.add_relpath_rules([
                [".", "", "COG功能注释文件",0],
                ["cog_class_table.xls", "", "COG分类统计表",0],
                ['run_parameter.txt', 'txt', '运行参数日志', 0],
                ["*.pdf", "pdf", "COG分类统计图", 0],
            ])
