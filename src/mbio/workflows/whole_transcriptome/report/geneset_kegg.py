...
class GenesetKeggWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self._sheet.output = self._sheet.output.replace('interaction_results','interaction_results/04 GeneSet/04 KEGG_Annotion')
        self.inter_dirs = []
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("whole_transcriptome", table="geneset_kegg_class", main_id=self.option('main_table_id'),
    ...
    def set_db(self):
        ...
        api_geneset.add_kegg_regulate_new(self.option("main_table_id"), self.option("geneset_id"), output_file,
                                          self.option("kegg_table_2"), self.work_dir, self.option("geneset_type"),self.option("geneset_kegg"),
        ...
        conn = api_geneset.db["geneset_kegg_class"]
    ...
    def chart(self):
        ...
        chart.chart_geneset_class_kegg(kegg_class_table,level=self.level)
        chart.to_pdf()
    ...
    def end(self):
        ...
        statis_new = statis.drop(['kegg_id', 'level', 'geneset_id'], axis=1)
        ...
        shutil.rmtree(self.kegg_class.output_dir + '/kegg_pathways')
        ...
        self.inter_dirs = [
            ...
            ["04 GeneSet/04 KEGG_Annotion", "", "KEGG功能注释", 0]
        ]
        result_dir.add_relpath_rules([
            [".", "", "KEGG功能注释文件",0],
            ["kegg_pathways", "", "KEGG通路图",0],
            ["pathways.tar.gz", "", "KEGG通路图", 0],
            ["kegg_analysis_detail.xls", "", "KEGG分类统计表",0],
            ["kegg_statistics.xls", "", "Pathway分类统计表",0],
            ['run_parameter.txt', 'txt', '运行参数日志', 0],
            ["*.pdf", "pdf", "KEGG分类统计图", 0],
        ])
        result_dir.add_regexp_rules([
            [r'kegg_pathways/map.*\.html', '', 'KEGG通路html文件',0],
            [r'kegg_pathways/map.*\.png', '', 'KEGG通路图片png',0],
        ])
