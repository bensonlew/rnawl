...
class SmallrnaWorkflow(Workflow):
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            {"name": "mirna_database", "type": "string", "default": "mirbase"}, # 已知miRNA鉴定的参考数据库, mirbase or pmiren or none
    ...
    def load_libraries(self):
        ...
        self.tools['mapper_and_stat'] = self.add_tool('small_rna.mapper_and_stat')
        self.tools['tar'] = self.add_tool('whole_transcriptome.tar')
        self.modules['srna'] = self.add_module("small_rna_v2.srna.srna")
        self.modules['diff_exp'] = self.add_module('whole_transcriptome_v1_1.diff_exp')
        self.on_rely([
    ...
    def run_mapper_and_stat(self):
        ...
        self.tools['mapper_and_stat'].on('end', self.run_srna_new)
        self.tools['mapper_and_stat'].run()

    def run_srna_new(self):
        self.logger.info("开始运行srna注释")
        reference = os.path.join(self.db_path, self.genome_doc['dna_fa'])
        ref_index = os.path.join(self.db_path, self.genome_doc["dna_index"])
        ...
        if self.option("input_type") == "raw":
            opts.update({'clean_fa': clean_fa})
        else:
            opts.update({'clean_fa': os.path.join(self.tools['mapper_and_stat'].output_dir, "uniq_mapped.fasta")})
    ...
    def export_qc(self):
        ...
        api.add_qc_detail(qc_id, qc_stat_before, qc_stat_after, 'small', qc_result_dir, group=self.option('group_table').path)
    ...
    def export_mapping(self):
        ...
        params = json.dumps({'task_id': self.task_id, 'submit_location': 'mapping', 'task_type': 2}, sort_keys=True,
                            separators=(',', ':'))
        api.add_mapping_stat(stat_file=stat_file, library='small', method=method, sample_list_file=sample_list_file, group=self.option('group_table').path)
    ...
    def export_srna(self):
        ...
        api.add_mirna_stat(mirna_stat=mirna_stat, task_id=self.task_id, project_sn=self.project_sn, params=params, group=self.option('group_table').path)
        api.add_srna_stat(srna_stat=srna_stat, srna_stat_for_graph=srna_stat_for_graph, task_id=self.task_id,
                          project_sn=self.project_sn, params=params, group=self.option('group_table').path)
