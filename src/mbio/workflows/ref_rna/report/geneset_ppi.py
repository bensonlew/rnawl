...
class GenesetPpiWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self.ppinetwork = self.add_module('ref_rna.ppinetwork_analysis')
    ...
    def set_db(self):
        ...
        报存分析结果到mongo数据库中
        """
        api_ppinetwork = self.api.api('ref_rna.geneset_ppi')
    ...
    def run(self):
        self.get_seq_fasta(self.work_dir + '/set.fa')
        self.run_ppinetwork()
        super(GenesetPpiWorkflow, self).run()


    def get_seq_fasta(self, fasta_path):
        '''
        根据sqlite 数据库获得蛋白集fasta文件
        '''
        conn = sqlite3.connect(self.option("seqdb_path").prop['path'])
        cursor = conn.cursor()
        with open(fasta_path, 'w') as fw:
            with open(self.option('geneset_list').prop['path'], 'r') as f:
                for gene in f.readlines()[1:]:
                    gene = gene.strip()
                    cursor.execute('select sequence from {} where seq_id="{}"'.format(self.option('type'), gene))
                    seq = str(cursor.fetchall()[0][0])
                    fw.write(">{}\n{}\n".format(gene, seq))
