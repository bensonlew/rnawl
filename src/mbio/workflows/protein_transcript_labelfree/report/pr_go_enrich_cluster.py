...
class PrGoEnrichClusterWorkflow(Workflow):
    """
    蛋白集富集分析
    """
    def __init__(self, wsheet_object):
        ...
        self.rpc = False
        super(PrGoEnrichClusterWorkflow, self).__init__(wsheet_object)
        options = [
            {"name": "gene_list", "type": "string"},
            {"name": "protein_list", "type": "string"},
            {"name": "protein_go", "type": "string"},
            {"name": "gene_go", "type": "string"},
        ...
        self.rna_tools = list()
        self.cluster_tool = self.add_tool("protein_transcript_labelfree.enrich_cluster")
        self._sheet.output = self._sheet.output.replace('interaction_results', 'interaction_results/7_relate/03_relate_anno/03_relate_goenrich_cluster')
        self.inter_dirs = []
    ...
    def send_log(self, data):
        ...
        super(PrGoEnrichClusterWorkflow, self).send_log(data)

    def run_tools(self):
        for protein_list in self.option('protein_list').split(','):
            tool = self.add_tool("labelfree.proteinset.go_enrich")
            options = {
                "diff_list": protein_list,
                "go_list": self.option("protein_go"),
    ...
    def run_cluster(self):
        ...
        for tool in self.protein_tools + self.rna_tools:
            ...
            enrich_df_col = enrich_df.set_index('GO')
            ...
            if self.option('cluster_col').lower() == 'p_value':
                enrich_df_col = enrich_df_col['p_uncorrected']
            if self.option('cluster_col').lower() == 'q_value':
                enrich_df_col = enrich_df_col['p_corrected']
            enrich_df_col.name = name
            enrich_df_col.index.name = 'go'
        ...
        for dict_ in info_list_tmp:
            if dict_['GO'] not in go_list:
                self.info_list.append(dict_)
                go_list.add(dict_['GO'])
        cluster_pd = pd.concat(col_list, axis=1)
        cluster_pd.index.name = 'go'
    ...
    def run(self):
        ...
        for tool in self.protein_tools + self.rna_tools:
            tool.run()
        super(PrGoEnrichClusterWorkflow, self).run()

    def set_db(self):
        """
        保存结果指数表到mongo数据库中
        """
        api_goenrich = self.api.api('protein_transcript_labelfree.relaset')
        ...
        api_goenrich.add_enrich_cluster(self.cluster_tool.output_dir, main_id=self.option('main_table_id'), type_ = 'go')
        self.end()
    ...
    def chart(self):
        ...
        pt_goenrich_cluster_matrix = os.path.join(self.cluster_tool.output_dir, "cluster_matrix.xls")
        pt_goenrich_cluster_tree = os.path.join(self.cluster_tool.output_dir, "seq.cluster_tree.txt")
        if os.path.exists(pt_goenrich_cluster_matrix) and os.path.exists(pt_goenrich_cluster_tree):
            chart.chart_pt_goenrich_cluster(pt_goenrich_cluster_matrix, pt_goenrich_cluster_tree)
            chart.to_pdf()
            # move pdf to result dir
            pdf_file = os.path.join(self.work_dir, "pt_goenrich_cluster.heatmap_new.pdf")
            if os.path.exists(pdf_file):
                os.link(pdf_file, os.path.join(self.output_dir, "heat.pdf"))
            pdf_files = glob.glob(self.work_dir + "/pt_goenrich_cluster__*.pdf")
            for pdf_file in pdf_files:
                os.link(pdf_file, os.path.join(self.output_dir, "sub"+os.path.basename(pdf_file)[21:].split('.')[0].split('_')[1]+'.pdf'))
    ...
    def end(self):
        ...
        self.inter_dirs = [
            ...
            ["7_relate/03_relate_anno/03_relate_goenrich_cluster", "", "GO富集热图", 0],
        ]
        ...
        super(PrGoEnrichClusterWorkflow, self).end()
