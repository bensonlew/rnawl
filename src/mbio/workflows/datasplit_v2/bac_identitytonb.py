...
class BacIdentitytonbWorkflow(Workflow):
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(BacIdentitytonbWorkflow, self).__init__(wsheet_object)
        options = [
        ...
        # self.bac_identools = self.add_tool("tool_lab.bac_identity")
        self.bac_datapre = self.add_tool("tool_lab.bac_datapretonb")
    ...
    def set_output(self, event):
        ...
        if event['data'] == 'datapre':
            self.linkdir(obj.output_dir, 'datapre')
    ...
    def run(self):
        ...
        super(BacIdentitytonbWorkflow, self).run()
    ...
    def end(self):
        ...
        transfer.add_upload(self.output_dir, self._sheet.output,self.output_dir)
        transfer.perform()
        self.post_sample_upload()
        super(BacIdentitytonbWorkflow, self).end()

    def post_sample_upload(self):
        """
        启动接口，进行数据上传
        """
        
        list_url_s3_path = '{}{}'.format(self._sheet.output, 'datapre/list.xls')
        sample_info_url_s3_path = '{}{}'.format(self._sheet.output, 'datapre/sample.detail.xls')
        fasta_dir_s3_path = '{}{}'.format(self._sheet.output, 'datapre/fasta_dir/')
        new_pictrue_name_s3_path = '{}{}'.format(self._sheet.output, 'datapre/')
        tid = self.option("task_id")
        bac_id = self.option("main_id")
        # tid = '20211209bac_identity_uploaddata
        upload_params = {
            "tid": tid,
            "bac_id": bac_id,
            "list_url": list_url_s3_path,
            "sample_info_url": sample_info_url_s3_path,
            "fasta_dir": fasta_dir_s3_path,
            "new_pictrue_name": new_pictrue_name_s3_path
            }
        type = "sanger"
        self.logger.info(upload_params)
        result = Submit(upload_params, "/s/datasplit/bacidentity_upload", type).webapitest()
        self.logger.info(result)
        if result["success"]:
            self.logger.info("请求数据上传成功！")
        else:
            self.logger.info("请求数据上传失败！")
    ...
    #         "fasta_dir": os.path.join(self.bac_datapre.output_dir, "fasta_dir"),
    #         "list_xls": os.path.join(self.bac_datapre.output_dir, "list.xls"),
    #         "sample_info": os.path.join(self.bac_datapre.output_dir, "sample.detail.xls")
    #         })
    ...
    #     api_bac_ind.update_bac_db_record(main_id, s3_upload_dir,self.dir_name)
        ...
        # api_bac_ind.add_blastnt_tax(main_id , os.path.join(ot_dir, "{}/NCBI/blastM8.xls".format(self.dir_name)))
...
if __name__ == '__main__':
    ...
    wsheet =Sheet(data=data)
    wf = BacIdentitytonbWorkflow(wsheet)
    wf.run()
