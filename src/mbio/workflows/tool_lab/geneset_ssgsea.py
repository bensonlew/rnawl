...
class GenesetSsgseaWorkflow(Workflow):
    """
    基因集富集分析
    """
    def __init__(self, wsheet_object):
        ...
        self.rpc = False
        super(GenesetSsgseaWorkflow, self).__init__(wsheet_object)
        options = [
            ...
            {"name": 'method', 'type': 'string', 'default': 'ssgsea'},
        ...
        self.gsva_tool.on('end', self.set_db)
    ...
    def run(self):
        self.gsva_runner()
        super(GenesetSsgseaWorkflow, self).run()
    ...
    def set_db(self):
        ...
        保存结果指数表到mongo数据库中
        """
        api_gsva = self.api.api('tool_lab.ssgsea')
        main_id = api_gsva.add_ssgsea(self.option("main_id"), self.gsva_tool.option('es_exp_table').path, self.gmx)
        self.set_output()
        self.end()
    ...
    def end(self):
        ...
        # ])
        super(GenesetSsgseaWorkflow, self).end()
