...
class PacbioAsWorkflow(Workflow):
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(PacbioAsWorkflow, self).__init__(wsheet_object)
        options = [
            {'name': 'flnc', 'type': 'infile', 'format':'medical_transcriptome.common'},
            {'name': 'update_info', 'type': 'string'},
            {'name': 'main_id', 'type': 'string'}
        ]
        self.add_option(options)
        self.revise_infiles()
        self.tool = self.add_tool("tool_lab.pacbio_as")
        self.set_options(self._sheet.options())

    def run(self):
        self.run_tool()
        super(PacbioAsWorkflow, self).run()

    def run_tool(self):
        opts = {
            'flnc': self.option('flnc').path,
        }
    ...
    def end(self):
        ...
        result_dir.add_relpath_rules([
            [".", "", "Pacbio无参全长转录组可变剪切分析", 0],
        ])
        super(PacbioAsWorkflow, self).end()
