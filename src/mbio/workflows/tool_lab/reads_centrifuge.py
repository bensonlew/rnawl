...
class ReadsCentrifugeWorkflow(Workflow):
    """
    基于reads的humann2分析
    ...
    def __init__(self, wsheet_object):
        ...
        super(ReadsCentrifugeWorkflow, self).__init__(wsheet_object)
        options = [
            {'name': 'in_fastq', 'type': 'infile', 'format': 'sequence.fastq_dir'},  # 输入的fq文件夹
            {'name': 'data_type', 'type': 'string'},  # 是否需要质控
    ...
    def run(self):
        self.run_main()
        super(ReadsCentrifugeWorkflow, self).run()

    def run_main(self):
        self.main = self.add_module("toolapps.centrifuge")
    ...
    def set_db(self):
        ...
        api_main.add_main_table("reads_centrifuge", main_id = self.option('main_id'))
    ...
    def end(self):
        ...
        result_dir.add_relpath_rules([
            [".", "", "Centrifuge分析结果文件目录",0],
        ])
        super(ReadsCentrifugeWorkflow, self).end()