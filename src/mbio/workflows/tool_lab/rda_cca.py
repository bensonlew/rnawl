...
class RdaCcaWorkflow(Workflow):
    """
    RDA/CCA小工具
    """
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(RdaCcaWorkflow, self).__init__(wsheet_object)
        options = [
        ...
        self.rda_cca = self.add_tool('meta.beta_diversity.rda_cca')
    ...
    #     self.matrix.on('end', self.rda_cca_run)
    #     self.matrix.run()
    
    def rda_cca_run(self):
        # if self.option("otutable").format == "Dsdsd":
        #     pass
        # else:
        #     self.logger.info(self.option("otutable").format)
        self.otu_table = self.option('otutable').prop['path']
        self.group_table = self.option("group_table").prop['path']
        self.rda_cca.set_options ({"otutable": self.otu_table,
        ...
        if not self.skip_ellipse:
            self.rda_cca.on("end",self.run_ellipse)
        else:
            self.rda_cca.on("end",self.set_output)
        self.rda_cca.run()
    ...
    def set_output(self):
        ...
        rda_files = os.listdir(self.rda_cca.output_dir)
        if 'cca_sites.xls' in rda_files:
            plot_species_path = self.rda_cca.output_dir.rstrip('/') + '/cca_plot_species_data.xls'
            plot_feature_path = self.rda_cca.output_dir.rstrip('/') + '/cca_plot_feature_data.xls'
            os.rename(plot_species_path,plot_feature_path)
        elif 'rda_sites.xls' in rda_files:
            plot_species_path = self.rda_cca.output_dir.rstrip('/') + '/rda_plot_species_data.xls'
            plot_feature_path = self.rda_cca.output_dir.rstrip('/') + '/rda_plot_feature_data.xls'
            os.rename(plot_species_path,plot_feature_path)
        try:
            shutil.copytree(self.rda_cca.output_dir,
                            os.path.join(self.output_dir, "rda_cca"))
    ...
    def set_db(self):
        ...
        api = self.api.api("tool_lab.rda_cca")
        sn_group = {}
        ...
        for i in feature_list:
            feature_index[i] = n
            n += 1 
        dir_path = self.rda_cca.output_dir
        ...
        if os.path.exists(ellispe_table):
    ...
    def run_ellipse(self):
        ...
        options = {
            ...
            "analysis" : "rda_cca",
            "meta" : "rda_cca",
            'pc_table':self.rda_cca.output_dir
            }
        rda_files = os.listdir(self.rda_cca.output_dir)
        # if 'cca_sites.xls' in rda_files:
        #     options['pc_table'] = self.rda_cca.output_dir + "/cca_sites.xls"
        # elif 'rda_sites.xls' in rda_files:
        #     options['pc_table'] = self.rda_cca.output_dir + "/rda_sites.xls"
...
if __name__ == '__main__':
    ...
    data = {
        'name': 'test_rda_cca',
        'id': 'rda_cca_' +  str(random.randint(1, 10000)),
        'type': 'workflow',
        'options': {
        "otutable": "/mnt/ilustre/users/sanger-dev/sg-users/xueqinwen/tool_lab_tools/rda_cca/otu_taxon.txt",
        "envtable": "/mnt/ilustre/users/sanger-dev/sg-users/xueqinwen/tool_lab_tools/rda_cca/env_table.txt",
        "group_table": "/mnt/ilustre/users/sanger-dev/sg-users/xueqinwen/tool_lab_tools/rda_cca/group.txt",
        "main_id" : "5e9e6a6017b2bf2049a81be1"
        }
    }
    wsheet = Sheet(data=data)
    wf = RdaCcaWorkflow(wsheet)
    wf.run()
