...
class PiceditWorkflow(Workflow):
    """
    ...
    def __init__(self, wsheet_object):
        ...
        super(PiceditWorkflow, self).__init__(wsheet_object)
        options = [
            {'name': 'picture_dir', 'type': 'infile', 'format': 'ref_rna_v2.common_dir'},
            {'name': 'edit_file', 'type': 'infile', 'format': 'ref_rna_v2.common'},
            {'name': 'update_info', 'type': 'string'},
            {'name': 'main_id', 'type': 'string'}
        ]
        self.add_option(options)
        self.revise_infiles()
        self.tool = self.add_tool("tool_lab.picedit")
        self.set_options(self._sheet.options())

    def run(self):
        self.run_tool()
        super(PiceditWorkflow, self).run()

    def run_tool(self):
        opts = {
            'picture_dir': self.option('picture_dir').path,
            'edit_file': self.option('edit_file').path
        }
    ...
    def end(self):
        ...
        super(PiceditWorkflow, self).end()
