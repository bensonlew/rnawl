...
class BacIdentityTestWorkflow(Workflow):
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(BacIdentityTestWorkflow, self).__init__(wsheet_object)
        options = [
    ...
    def run_bac_datapre(self):
        ...
        self.bac_datapre.on("end",self.run_bac_indent,"datapre")
    ...
    def run_zip_result(self):
        ...
        with open(os.path.join(self.bac_datapre.output_dir, "sample.detail.xls"),"r") as sinfo:
            ...
            line = sinfo.readline()
            temp= line.split("\t")
            self.dir_name = "{}-{}-{}-{}-{}sample".format(temp[0],temp[1],temp[5],temp[4],temp[6])
            self.result_name = "{}-{}-{}-{}-{}sample".format(temp[0],temp[1],temp[5],temp[4],temp[6])
    ...
    def set_db(self,s3_upload_dir):
        ...
        api_bac_ind = self.api.api("tool_lab.bac_identity")
        ...
        api_bac_ind.update_bac_db_record(main_id, s3_upload_dir)
    ...
    def run(self):
        ...
        super(BacIdentityTestWorkflow, self).run()
    ...
    def end(self):
        ...
        super(BacIdentityTestWorkflow, self).end()
...
if __name__ == '__main__':
    ...
    wsheet =Sheet(data=data)
    wf = BacIdentityTestWorkflow(wsheet)
    wf.run()