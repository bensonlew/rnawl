...
class DuogeneWorkflow(Workflow):
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(DuogeneWorkflow, self).__init__(wsheet_object)
        options = [
            {'name': 'gene_file', 'type': 'infile', 'format': 'medical_transcriptome.common'},
            {'name': 'update_info', 'type': 'string'},
            {'name': 'main_id', 'type': 'string'}
        ]
        self.add_option(options)
        self.revise_infiles()
        self.tool = self.add_tool("tool_lab.duogene")
        self.set_options(self._sheet.options())

    def run(self):
        self.run_tool()
        super(DuogeneWorkflow, self).run()

    def run_tool(self):
        opts = {
            'gene_file': self.option('gene_file').path,
        }
    ...
    def set_db(self):
        ...
        s3_path = '{}/{}'.format(self._sheet.output, 'gene_structure.pdf')
        duogene = self.api.api('tool_lab.duogene')
        duogene.add_duogene(self.option('main_id'), s3_path)
        self.set_output()
    ...
    def end(self):
        ...
        super(DuogeneWorkflow, self).end()
