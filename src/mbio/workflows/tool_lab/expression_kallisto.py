...
class ExpressionKallistoWorkflow(Workflow):

    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(ExpressionKallistoWorkflow, self).__init__(wsheet_object)
        options = [
    ...
    def run(self):
        self.run_tool()
        super(ExpressionKallistoWorkflow, self).run()
    ...
    def end(self):
        ...
        super(ExpressionKallistoWorkflow, self).end()
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        from mbio.workflows.tool_lab.expression_kallisto import ExpressionKallistoWorkflow
        from biocluster.wsheet import Sheet
        import random
        test_dir = "/mnt/ilustre/users/sanger-dev/app/database/Genome_DB_finish/fungi/Saccharomyces_cerevisiae/Ensemble_release_39/"
        data = {
            "id": "expression_kallisto" + str(random.randint(1, 10000)),
            "type": "workflow",
            "name": "tool_lab.expression_kallisto",
            "options":   {'fq_type': 'PE',
        ...
        wf =ExpressionKallistoWorkflow(wsheet)
        wf.sheet.id = 'expression_kallisto'
        wf.sheet.project_sn = 'expression_kallisto'
