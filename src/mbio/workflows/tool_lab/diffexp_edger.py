...
class DiffexpEdgerWorkflow(Workflow):
    """
    差异分析
    """
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(DiffexpEdgerWorkflow, self).__init__(wsheet_object)
        options = [
    ...
    def run(self):
        ...
        self.prepare()
        self.run_tool()
        super(DiffexpEdgerWorkflow, self).run()
    ...
    def end(self):
        ...
        super(DiffexpEdgerWorkflow, self).end()
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        from mbio.workflows.tool_lab.diffexp_edger import DiffexpEdgerWorkflow
        from biocluster.wsheet import Sheet
        import random
        data = {
            'id': 'diff_exp_{}_{}'.format(random.randint(1000, 9999), random.randint(1000, 9999)),
            'type': 'workflow',
            'name': 'tool_lab.diffexp_edger',
            "options": dict(
        ...
        wf = DiffexpEdgerWorkflow(wsheet)
