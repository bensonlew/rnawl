...
class DiffexpDegseqWorkflow(Workflow):
    """
    差异分析
    """
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(DiffexpDegseqWorkflow, self).__init__(wsheet_object)
        options = [
    ...
    def run(self):
        ...
        self.prepare()
        self.run_tool()
        super(DiffexpDegseqWorkflow, self).run()
    ...
    def set_db(self):
        ...
        #
        #
        # if 'DEGseq'.lower() in ["degseq", "edger", "deseq2", 'limma', 'svaseqlimma']:
        #     self.diffexp.add_diffexp(self.tool.output_dir,
    ...
    def end(self):
        ...
        result_dir.add_regexp_rules([
            ...
            [r'.*_vs_.*\.pdf', 'pdf', '差异表达基因火山图'],
        ])
        super(DiffexpDegseqWorkflow, self).end()
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        from mbio.workflows.tool_lab.diffexp_degseq import DiffexpDegseqWorkflow
        from biocluster.wsheet import Sheet
        import random
        data = {
            'id': 'diff_exp_{}_{}'.format(random.randint(1000, 9999), random.randint(1000, 9999)),
            'type': 'workflow',
            'name': 'tool_lab.diffexp_degseq',
            "options": dict(
        ...
        wf = DiffexpDegseqWorkflow(wsheet)
