...
class RrnaFilterWorkflow(Workflow):
    """
    功能: 统计fastq序列中核糖体rna的含量
    """
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(RrnaFilterWorkflow, self).__init__(wsheet_object)
        options = [
            ...
            {"name": "filter_soft", "type": "string", "default": "bowtie2"},  # 评估软件 ["bowtie2","blast"]
            # {"name": "assess_method", "type": "string", "default": "both"},  # 评估方式 ["R1","R2","both"]
            # {"name": "seq_num", "type": "int", "default": False},  # 抽取评估序列长度
    ...
    def check_options(self):
        if  self.option("seq_type") == "fasta" and self.option("filter_soft") == "bowtie2":
        ...
        if self.option("fastq_dir").is_set:
            ...
            # elif self.fq_type == "SE" and row_num != 2:
    ...
    def run_tools(self):
        if self.option("filter_soft").lower() == "blast":
            self.rfam_run()
        if self.option("filter_soft").lower() == "bowtie2":
            self.rrna_run()
    ...
    def run(self):
        if self.if_gz:
            ...
            self.run_tools()
        super(RrnaFilterWorkflow, self).run()
    ...
    def rrna_run(self):
        ...
        for f in sorted(self.samples):
            options = {}
            mapping_tool = self.add_tool('tool_lab.rfam_filter.bowtie2')
            if self.fq_type == "PE":
    ...
    def rfam_run(self):
        for n, sample_name in enumerate(self.samples):
            ...
            rfam = self.add_tool('tool_lab.rfam_filter.blast_to_rfam')
            if self.option("seq_type") == "fastq":
                ...
                if self.fq_type == "PE":
                    opts.update({"query1" : os.path.join(self.option('fastq_dir').path, self.samples[sample_name]["r"])})
                rfam.set_options(opts)
    ...
    def set_output(self):
        ...
        elif self.option("filter_soft").lower() == "bowtie2":
        ...
        if self.option("filter_soft").lower() == "blast":
            for tool in self.tools:
                if self.option("seq_type") == "fastq":
                    out_fqs = glob.glob(tool.output_dir + '/' + "*input_rmrRNA*fastq")
                    for fq in out_fqs:
                        os.link(fq, os.path.join(self.output_dir,os.path.basename(fq)))
                else:
                    out_fas = glob.glob(tool.output_dir + '/' + "*_input_rmrRNA.fasta")
                    for fa in out_fas:
                        os.link(fa, os.path.join(self.output_dir, os.path.basename(fa)))
        elif self.option("filter_soft").lower() == "bowtie2":
            for tool in self.tools:
                out_fqs = glob.glob(tool.output_dir + '/' + "*input_rmrRNA*fastq")
                for fq in out_fqs:
    ...
    def end(self):
        ...
        result_dir.add_regexp_rules([
            [r'.*\.rmrRNA\.1\.fastq', '', '过滤后fastq文件r1端', 0],
            [r'.*\.rmrRNA\.2\.fastq', '', '过滤后fastq文件r2端', 0],
            [r'.*\.rmrRNA\.fastq', '', '过滤后fastq文件s端', 0],
            [r'.*fasta', '', '过滤后fasta', 0]
        ])
        super(RrnaFilterWorkflow, self).end()
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        from mbio.workflows.tool_lab.rrna_filter import RrnaFilterWorkflow
        from biocluster.wsheet import Sheet
        import random
        data = {
            "id": "rfam_stat_" + str(random.randint(1, 10000)),
            "type": "workflow",
            "name": "tool_lab.rrna_filter",
            "options": dict(
