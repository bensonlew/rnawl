...
class DiffexpDeseq2Workflow(Workflow):
    """
    差异分析
    """
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(DiffexpDeseq2Workflow, self).__init__(wsheet_object)
        options = [
    ...
    def run(self):
        ...
        self.prepare()
        self.run_tool()
        super(DiffexpDeseq2Workflow, self).run()
    ...
    def set_db(self):
        ...
        #
        # if 'DESeq2'.lower() in ["degseq", "edger", "deseq2", 'limma', 'svaseqlimma']:
        #     self.diffexp.add_diffexp(self.tool.output_dir,
    ...
    def end(self):
        ...
        super(DiffexpDeseq2Workflow, self).end()
    ...
    def run_tool(self):
        if self.option('test').lower() == 'wald':
            self.option('test', 'Wald')
        if self.option('test').lower() == 'lrt':
            self.option('test', 'LRT')
        opts = dict(
            #基本参数
