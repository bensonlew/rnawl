...
class BugbaseGroupWorkflow(Workflow):
    ...
    def set_db(self):
        ...
        """
        if self.option("group_method") == "two":
            api_two_group = self.api.api("metaasv.bugbase_group")
            ...
            api_two_group.add_species_difference_check_detail(statfile=stat_path, cifiles=[ci_path],
                                                              ...
                                                              coll_name="bugbase_group_detail",
                                                              main_coll="bugbase_group", bar_name = "bugbase_group_bar",task_type="two_group")
            api_two_group.add_species_difference_check_boxplot(boxfile_path, self.option('main_id'),
                                                               correlation_key="bugbase_id",
                                                               coll_name="bugbase_group_box",coll_name2="bugbase_group_bar",
                                                               main_coll="bugbase_group", task_type="two_group")
            #api_two_group.add_species_difference_check_barplot(bar_path, self.option('main_id'),
            #                                                   correlation_key="bugbase_id",
            #                                                   coll_name="bugbase_group_bar",
            #                                                   main_coll="bugbase_group", task_type="two_group")
            api_two_group.update_species_difference_check(self.option('main_id'), stat_path, ci_path,
                                                          'bugbase_group')
        else:
            api_multiple = self.api.api("metaasv.bugbase_group")
            stat_path = self.output_dir + '/' + self.option("test") + '_result.xls'
