...
class FungiWorkflow(Workflow):
    def __init__(self, wsheet_object):
        """
        真菌基因组workflow option参数设置
        """
        self._sheet = wsheet_object
        super(FungiWorkflow, self).__init__(wsheet_object)
        options = [
        ...
        # try:
        #     sanger_prefix = Config().get_project_region_bucket(project_type="fungi")
        #     self.remote_dir = sanger_prefix + self._sheet.output.split(":")[-1].lstrip("/")
        # except:
        #     self.remote_dir = self._sheet.output.split(":")[-1].lstrip("/")
        self.remote_dir = self._sheet.output
        

        #self.file_dir = os.path.join(sanger_prefix ,'rerewrweset/fungi/' + self._sheet.id + '/data')
    ...
    def delete_mongo_data(self):
        delete = DeleteDemoMongo(self._sheet.id, 'fungigenome')
        try:
            delete.run()
        except:
    ...
    def run_fungigenome(self):
        self.get_list()
        protein = ''
        if self.option('faa_file').is_set:
            protein = self.option('faa_file').prop['path']
        elif self.option('genbank'):
            protein = self.config.SOFTWARE_DIR + '/database/NCBI_fungi/faa/' + self.option('genbank') + '.faa'
        elif self.option('ncbi'):
            protein = self.config.SOFTWARE_DIR + '/database/NCBI_maker2_protein/fungi_uniprot.fasta'
        busco =self.config.SOFTWARE_DIR + '/database/BUSCO/' + self.option('busco_database')
        for sample in self.samples.keys():
            asse_path =self.work_dir + '/' + sample +'/assemble'
            raw_path = self.work_dir + '/' + sample + '/data'
            #self.bacgenome = self.add_batch('bacgenome.bacgenome', ignore_error=True, batch_type="module")
            self.fungigenome =self.add_module('fungi_genome.fungi')
            self.step.add_steps('fungi{}'.format(' analysis, sample: '+ sample))
            opts = ''
            ...
            self.fungigenome.set_options(opts)
            step = getattr(self.step, 'fungi{}'.format(' analysis, sample: '+ sample))
            step.start()
            self.step.update()
            self.fungigenome.on('end', self.finish_update, 'fungi{}'.format(' analysis, sample: '+ sample))
            self.modules.append(self.fungigenome)
    ...
    def run(self):
        ...
        self.run_split_dir()
        time.sleep(2)
        task_info = self.api.api('task_info.fungi_task_info')
        task_info.add_task_info()
        self.run_fungigenome()
        super(FungiWorkflow, self).run()
    ...
    def run_api(self, test=False):
        ...
        self.software = self.api.api("fungi_genome.software")
        seq_type = ''
        ...
        #resu = self.remote_dir.rsplit('rerewrweset')[1]
        ...
        for sample in self.samples.keys():
            ...
            if self.option("analysis") in ["uncomplete"] and self.option('raw_dir').is_set:
                self.assess_gc.add_assess_gc_detail(gc_id, sample, "10k", self.remote_dir + sample + '/genomic_assessment/GC_depth/GC_depth_png/depth_gc_10000/')
                self.assess_gc.add_assess_gc_detail(gc_id, sample, "20k", self.remote_dir + sample + '/genomic_assessment/GC_depth/GC_depth_png/depth_gc_20000/')
                self.assess_gc.add_assess_gc_detail(gc_id, sample, "30k", self.remote_dir + sample + '/genomic_assessment/GC_depth/GC_depth_png/depth_gc_30000/')
                self.assess_gc.add_assess_gc_detail(gc_id, sample, "50k", self.remote_dir + sample + '/genomic_assessment/GC_depth/GC_depth_png/depth_gc_50000/')
                self.assess_gc.add_assess_gc_detail(gc_id, sample, "100k", self.remote_dir + sample + '/genomic_assessment/GC_depth/GC_depth_png/depth_gc_100000/')
        ...
        self.bacgenome_model = FungiGenome()
        ...
        for sample in self.samples.keys():
            ...
            if os.path.exists(os.path.join(dir_o, sample + "/genomic_assessment/kmer_frequency/" + sample + '_Kmer_frequency.xls')):
                ...
                for i in ['depth_gc_10000', 'depth_gc_20000', 'depth_gc_30000', 'depth_gc_50000', 'depth_gc_100000']:
                    if os.path.exists(os.path.join(dir_o, sample + "/genomic_assessment/" + i)):
                        self.move_dir(os.path.join(dir_o, sample + "/genomic_assessment/" + i),
                        os.path.join(dir_up,  sample + "/genomic_assessment/GC_depth/GC_depth_png/" + i))   #guanqing.zou 20181010
            ...
            if os.path.exists(os.path.join(dir_o, sample + "/pathogenic_system/secretion_system")):  #add 20203
                self.move_dir(os.path.join(dir_o, sample + "/pathogenic_system/secretion_system"),
                              os.path.join(dir_up, sample + "/pathogenic_system/secretion_system"))

            repaths += [
                ["%s" % sample, "", "样品%s流程分析结果目录" % sample,0,'140001'],
                ["%s/project_overview" % sample, "", "样品%s的基因组总览目录" % sample,0,'140002'],
                ["%s/project_overview/All_predict_info" % sample, "", "样品%s的基因组预测注释文件结果目录" % sample,0,"140109"],
                ...
                ["%s/genomic_assessment/GC_depth/GC_depth_png" % sample, "", "" ,0],  #guanqing.zou 20181010
                ...
                ["%s/assembly_predict/assembly/scaffold" % sample, "", "" ,0],   ##guanqing.zou 20181010
                ["%s/assembly_predict/assembly/contig" % sample, "", "" ,0],   ##guanqing.zou 20181010
            ...
            regexps += [
                ...
                [r"%s/project_overview/All_predict_info/.+.gff" % sample, "", "基因组的gff文件",0,"140110"],
                [r"%s/project_overview/All_predict_info/.+.gbk" % sample, "", "基因组的gbk文件",0,"140111"],
