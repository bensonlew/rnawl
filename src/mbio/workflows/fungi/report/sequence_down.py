    ...
    def run_seq_down(self):
        ...
        for k in sorted(specimen_gene.keys()):
            ...
            if re.search(r'://', sample_seq_path[0]):
                ...
                seq_path_ori = sample_seq_path[0] + k + sample_seq_path[1]
                self.logger.info("seq_path_ori: " + seq_path_ori)
                seq_path = self.work_dir + '/' + self.option('type') + '/' + k + '/' + k + sample_seq_path[2]
                transfer.add_download(seq_path_ori,self.work_dir + '/' + self.option('type') + '/' + k + '/')
                self.logger.info("seq_path: " + seq_path)
                transfer.perform()
