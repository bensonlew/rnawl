...
class BugbaseWorkflow(Workflow):
    def __init__(self, wsheet_object):
        ...
        self.bugbase = self.add_tool('meta.bugbase_predict')
    ...
    def set_db(self):
        ...
        self.api_bugbase = self.api.api("bugbase")
        prediction_path = self.bugbase.output_dir+"/bugbase_predictions.txt"
        threshold_path = self.bugbase.output_dir+"/contributing_otus.txt"
        normalized_path = self.bugbase.output_dir + "/16s_normalized_otus.txt"
    ...
    def end(self):
        save_params(self.bugbase.output_dir, self.id)
        result_dir = self.add_upload_dir(self.bugbase.output_dir)
        result_dir.add_relpath_rules([
            [".", "", "BugBase表型预测结果目录",0],
            ["./16s_normalized_otus.txt", "txt", "Bugbase表型预测标准化OTU丰度表",0],
            ...
            ["./contributing_otus.txt", "txt", "Bugbase表型预测贡献度结果表", 0],
        ])
