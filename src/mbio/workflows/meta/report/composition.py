...
class CompositionWorkflow(Workflow):
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            {"name": "type","type":"string","default":"ternary"} #guanqing.zou 20180508
        ]
    ...
    def set_db(self):
        ...
        api_otu.add_sg_otu_detail(self.sort_samples.option("level_otu_table").prop["path"], self.option("main_id"), self.option("input_otu_id"),self.option("type")) # guanqing.zou 20180411
    ...
    def save_pdf(self):
        ...
        if self.pdf_status:
            name = get_name(self.option("main_id"),"sg_composition")
            self.figsave = self.add_tool("meta.fig_save")
            self.figsave.on('end', self.end)
            if self.option("type") == "ternary":
                submit_loc = "ternary"
            else:
                submit_loc = "composition"
    ...
    def end(self):   # modify by zhouxuan 2016.11.23
        ...
        result_dir.add_relpath_rules([
            ...
            ["Circos图.pdf", "pdf", "样本与物种关系Circos图", 0, ""],
            ["三元相图.pdf", "pdf", "三元相图", 0, ""]
        ])
