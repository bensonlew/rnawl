...
class BugbaseGroupWorkflow(Workflow):
    """
    bugbase组间差异检验
    """
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(BugbaseGroupWorkflow, self).__init__(wsheet_object)
        options = [
    ...
    def run_two_group(self):
        if self.option("test") == "student":
            options = {
                "student_input": self.option("bugbase_table"),
            ...
            options = {
                "mann_input": self.option("bugbase_table"),
            ...
            options = {
                "welch_input": self.option("bugbase_table"),
    ...
    def run_multiple(self):
        if self.option("test") == "anova":
            options = {
                "anova_input": self.option("bugbase_table"),
            ...
            options = {
                "kru_H_input": self.option("bugbase_table"),
    ...
    def end(self):
        if self.pdf_status:
            ...
            if self.option("group_method") == "two":
                if os.path.exists(self.output_dir+"/多组差异检验柱形图.pdf"):
                    os.remove(self.output_dir+"/多组差异检验柱形图.pdf")
        ...
        if self.option("group_method") == "two":
            ...
            result_dir.add_relpath_rules([
                ...
                ["BugBase表型两组差异检验柱形图.pdf", "pdf", "两组差异检验柱形图", 0, ""]
            ])
            ...
            result_dir.add_relpath_rules([
                ...
                ["BugBase表型多组差异检验柱形图.pdf", "pdf", "多组差异检验柱形图", 0, ""]
            ])
            result_dir.add_regexp_rules([
        ...
        super(BugbaseGroupWorkflow, self).end()
    ...
    def set_db(self):
        ...
        if self.option("group_method") == "two":
            ...
            api_two_group.add_species_difference_check_detail(statfile=stat_path, cifiles=[ci_path],
                                                              ...
                                                              correlation_key="bugbase_id",bar_name = "sg_bugbase_group_bar",
                                                              coll_name="sg_bugbase_group_detail", main_coll="sg_bugbase_group",task_type="two_group")
            api_two_group.add_species_difference_check_boxplot(boxfile_path, self.option('main_id'),
                                                               correlation_key="bugbase_id", coll_name="sg_bugbase_group_box",
                                                               main_coll="sg_bugbase_group",task_type="two_group")
            #api_two_group.add_species_difference_check_barplot(bar_path, self.option('main_id'),
            #                                                   correlation_key="bugbase_id", coll_name="sg_bugbase_group_bar",
            #                                                   main_coll="sg_bugbase_group",task_type="two_group")
            api_two_group.update_species_difference_check(self.option('main_id'), stat_path, ci_path, 'sg_bugbase_group')
        else:
    ...
    def save_pdf(self):
        ...
        if self.pdf_status:
            name = get_name(self.option("main_id"), "sg_bugbase_group")
            ...
            self.figsave.set_options({
                ...
                "table_name": name,
                "project": "meta",
                "submit_loc": "bugbase_group",
                "interaction": 1,
                "main_table": "sg_bugbase_group",
            })
            self.figsave.run()
        else:
            self.end()
    ...
    def run(self):
        if self.option("group_method") == "two":
            ...
            self.run_multiple()
        super(BugbaseGroupWorkflow, self).run()
