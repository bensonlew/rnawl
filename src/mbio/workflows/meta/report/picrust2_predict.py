...
class Picrust2PredictWorkflow(Workflow):
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            {"name": "predict_id", "type": "string"},##主表id
        ...
        self.api_picrust2 = self.api.picrust2_predict
    ...
    def check_options(self):
        ...
        if not self.option("otu_fasta").is_set:
            raise OptionError("请传入otu_reps序列！", code="12705101")
        if not self.option("otu_table").is_set:
            raise OptionError("请传入otu的丰度表文件！", code="12705102")
    ...
    def check_params(self):
        ...
        if (self.option("analysis_type") in ["16S"]) and (database not in ['silva138/16s_archaea', 'silva138/16s_bacteria', 'silva138/16s', 'silva123/16s_bacteria', 'silva123/16s_archaea', 'silva123/16s','silva119/16s_bacteria', 'silva119/16s_archaea', 'silva119/16s','silva128/16s_archaea', 'silva128/16s_bacteria', 'silva128/16s','silva132/16s_archaea', 'silva132/16s_bacteria', 'silva132/16s','greengenes135/16s', 'greengenes135/16s_archaea', 'greengenes135/16s_bacteria','rdp11.5/16s', 'rdp11.5/16s_bacteria', 'rdp11.5/16s_archaea', 'fgr/amoA', 'fgr/nosZ', 'fgr/nirK', 'fgr/nirS','fgr/nifH', 'fgr/pmoA', 'fgr/mmoX', 'fgr/mcrA','fgr/amoA_archaea', 'fgr/amoA_bacteria','maarjam081/AM', 'Human_HOMD', 'Human_HOMD_v15.2', 'Human_HPB', 'Protist_PR2_v4.5', "nt", 'nt_v20200327/16s_archaea', 'nt_v20200327/16s_bacteria', 'nt_v20200327/16s', "custom_mode"]):
            raise OptionError("注释数据库%s不是16s数据库，不能进行Picrust2分析！", variables=(database), code="12705103")
        elif (self.option("analysis_type") in ["18S"]) and (database not in ['silva138/18s_eukaryota',"silva123/18s_eukaryota", 'silva119/18s_eukaryota', 'silva128/18s_eukaryota', 'silva132/18s_eukaryota','fgr/amoA', 'fgr/nosZ', 'fgr/nirK', 'fgr/nirS','fgr/nifH', 'fgr/pmoA', 'fgr/mmoX', 'fgr/mcrA','fgr/amoA_archaea', 'fgr/amoA_bacteria','maarjam081/AM', 'Human_HOMD', 'Human_HPB', 'Protist_PR2_v4.5', "nt", 'nt_v20200327/18s_eukaryota', "custom_mode"]):
            raise OptionError("注释数据库%s不是18s数据库，不能进行Picrust2分析！", variables=(database), code="12705104")
        elif (self.option("analysis_type") in ["ITS"]) and (database not in ['unite8.0/its_fungi', 'unite7.2/its_fungi', 'unite7.0/its_fungi', 'fgr/amoA', 'fgr/nosZ', 'fgr/nirK', 'fgr/nirS','fgr/nifH', 'fgr/pmoA', 'fgr/mmoX', 'fgr/mcrA','fgr/amoA_archaea', 'fgr/amoA_bacteria','maarjam081/AM', 'Human_HOMD', 'Human_HPB', 'Protist_PR2_v4.5', "nt", 'nt_v20200327/its_fungi', "custom_mode"]):
            raise OptionError("注释数据库%s不是its数据库，不能进行Picrust2分析！", variables=(database), code="12705105")
        else:
    ...
    def set_db(self):
        ...
        api_picrust2 = self.api.picrust2_predict
        main_id = self.option("predict_id")
        ...
        if os.path.exists(cog_path):
            ...
            if self.option("analysis_type") in ['16s', "16S"]:
                self.set_error("未能成功正确的生成cog结果文件或结果文件为空，请检查！", code="12705101")
            else:
        ...
        if os.path.exists(function_path):
            ...
            if self.option("analysis_type") in ['16s', "16S"]:
                self.set_error("未能成功正确的生成cog结果文件或结果文件为空，请检查！", code="12705102")
            else:
                self.logger.info("18S和ITS没有cog功能分析")
        ...
        if os.path.exists(ko_path):
            ...
            if self.option("analysis_type") in ['16s', "16S"]:
                self.set_error("未能成功正确的生成ko结果文件或结果文件为空，请检查！", code="12705103")
            else:
        ...
        if os.path.exists(module_path):
            ...
            if self.option("analysis_type") in ['16s', "16S"]:
                self.set_error("未能成功正确的生成module结果文件或结果文件为空，请检查！", code="12705104")
            else:
        ...
        api_picrust2.add_path(main_id=main_id,path=remote_dir)
        #self.end()
        self.save_pdf()
    ...
    def save_pdf(self):
        ...
        if self.pdf_status:
            name = get_name(self.option("predict_id"), "sg_picrust2")
            ...
            for file in os.listdir(pdf_outs):
                if file == "COG功能分类统计柱形图.pdf":
                    ...
                    if not os.path.exists(self.output_dir+"/KEGG"):
                        os.mkdir(self.output_dir+"/KEGG")
                    os.link(pdf_outs+"/"+file,self.output_dir+"/KEGG/"+file)
        save_params(self.output_dir, self.id)
        result_dir = self.add_upload_dir(self.output_dir)
        result_dir.add_relpath_rules([
            [".", "dir", "PICRUSt2功能预测结果目录",0,"110273"],
            ["./COG/", "dir", "COG功能预测结果目录",0,"110274"],
            ["./KEGG/", "dir", "KEGG功能预测结果目录",0,"110275"],
            ["./MetaCyc/", "dir", "MetaCyc功能预测结果目录",0,"110276"],
            [r"./COG/prediction_cog.xls", "xls", "COG样品丰度表",0,"110277"],
            [r"./COG/prediction_function.xls", "xls", "Function样品丰度表",0,"110278"],
            [r"./COG/weighted_nsti.xls", "xls", "COG功能预测的NSTI值文件",0,"110279"],
            [r"./KEGG/prediction_KO.xls", "xls", "KO样品丰度表",0,"110280"],
            [r"./KEGG/prediction_pathway.L1.xls", "xls", "代谢通路level 1丰度表",0,"110281"],
            [r"./KEGG/prediction_pathway.L2.xls", "xls", "代谢通路level 2丰度表",0,"110282"],
            [r"./KEGG/prediction_pathway.L3.xls", "xls", "代谢通路level 3丰度表",0,"110283"],
            [r"./KEGG/kegg.pathway.profile.xls", "xls", "各个样品的pathway丰度统计",0,"110284"],
            [r"./KEGG/prediction_enzyme.xls", "xls", "enzyme样品丰度表",0,"110285"],
            [r"./KEGG/weighted_nsti.xls", "xls", "KEGG功能预测的NSTI值文件",0,"110286"],
            [r"./KEGG/prediction_module.xls", "xls", "module样品丰度表",0,"110287"],
            [r"./MetaCyc/MetaCyc_pathway_pred.xls", "xls", "MetaCyc pathway样品丰度表",0,"110288"],
            [r"./COG_predicted.xls", "xls", "预测的OTU中COG的数量",0,"110289"],
            [r"./EC_predicted.xls", "xls", "预测的OTU中enzyme的数量",0,"110290"],
            [r"./KO_predicted.xls", "xls", "预测的OTU中KO的数量",0,"110291"],
            [r"./marker_predicted_and_nsti.xls", "xls", "预测的16S拷贝数和NSTI值",0,"110292"],
            [r"./KEGG/Enzyme丰度Heatmap图.pdf", "pdf", "KEGG功能预测Enzyme丰度Heatmap图",0,""],
            [r"./KEGG/KO丰度Heatmap图.pdf", "pdf", "KEGG功能预测KO丰度Heatmap图", 0, ""],
            [r"./KEGG/Module丰度Heatmap图.pdf", "pdf", "KEGG功能预测Module丰度Heatmap图", 0, ""],
            [r"./KEGG/Pathway Level1丰度Heatmap图.pdf", "pdf", "KEGG功能预测pathway level1丰度Heatmap图", 0, ""],
            [r"./KEGG/Pathway Level2丰度Heatmap图.pdf", "pdf", "KEGG功能预测pathway level2丰度Heatmap图", 0, ""],
            [r"./KEGG/Pathway Level3丰度Heatmap图.pdf", "pdf", "KEGG功能预测pathway level3丰度Heatmap图", 0, ""],
