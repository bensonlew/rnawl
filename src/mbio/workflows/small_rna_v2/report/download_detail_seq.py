...
class DownloadDetailSeqWorkflow(Workflow):
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            {'name': 'seq_db_mirna_novol', 'type': 'infile', 'format': 'ref_rna_v2.common'},
            {'name': 'seq_db_mirna_known', 'type': 'infile', 'format': 'ref_rna_v2.common'},
        ...
        self._sheet.output = self._sheet.output.replace('interaction_results',
                                                        'interaction_results/20 Seq_Download')
        self.download = self.add_tool("small_rna_v2.extract_detail_seq")
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("small_rna", table="sg_seq_extract", main_id=self.option('main_id'),
    ...
    def run_extract(self):
        ...
        if self.option('level') == 'M':
            options = {
            'seq_db_mirna_novol': self.option('seq_db_mirna_novol').path,
            'seq_db_mirna_known': self.option('seq_db_mirna_known').path,
            'geneset_extract': self.option("geneset_extract"),
            'level': self.option("level"),
            'extract_info': self.option("extract_info"),
            't2g_file': self.option('t2g_file')
            }
    ...
    def transfer(self):
        ...
        cmd = "scp -r -i ~/.ssh/id_rsa {} dongmei.fu@10.2.4.236:/mnt/ilustre/users/meng.luo/sanger_data".format(
