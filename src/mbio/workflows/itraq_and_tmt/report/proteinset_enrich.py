...
class ProteinsetEnrichWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        if self.option("anno_type") == "go":
            self.enrich_tool = self.add_tool("itraq_and_tmt.proteinset.go_enrich")
        else:
            self.enrich_tool = self.add_tool("itraq_and_tmt.proteinset.kegg_rich")
        self.output_dir = self.enrich_tool.output_dir
        #
        if self.option("anno_type") == "go":
            self._sheet.output = self._sheet.output.replace('interaction_results', 'interaction_results/5_Proteinset/04_Enrich/01_GO')
        if self.option("anno_type") == "kegg":
            self._sheet.output = self._sheet.output.replace('interaction_results', 'interaction_results/5_Proteinset/04_Enrich/02_KEGG')
        self.inter_dirs = []
    ...
    def send_log(self, data):
        ...
        super(ProteinsetEnrichWorkflow, self).send_log(data)
    ...
    def set_db(self):
        ...
        if self.option("anno_type") == "kegg":
            ...
            if isinstance(record_id, types.StringTypes):
                ...
                self.set_error("main_id参数必须为字符串或者ObjectId类型!", code = "12501201")
    ...
    def chart(self):
        ...
        if self.option("anno_type") == "go":
            ...
            chart.chart_proteinsetcluster_gorich_web(go_enrich_all_file)
            chart.to_pdf()
            for old,new in [["enrichgo.densebubble.pdf",'enrichgo.bubble2.pdf'],["enrichgo.scatterbubble.pdf",'enrichgo.bubble.pdf'],["enrichgo.shadowbar.pdf",'enrichgo.bar.pdf']]:
        ...
        if self.option("anno_type") == "kegg":
            ...
            chart.chart_proteinsetcluster_keggrich_web(kegg_enrich_all_file)
            chart.to_pdf()
            for old,new in [["enrichkegg.densebubble.pdf",'enrichkegg.bubble2.pdf'],["enrichkegg.scatterbubble.pdf",'enrichkegg.bubble.pdf'],["enrichkegg.shadowbar.pdf",'enrichkegg.bar.pdf']]:
                if os.path.exists(os.path.join(self.work_dir, old)):
                    os.link(os.path.join(self.work_dir, old), os.path.join(self.enrich_tool.output_dir, new))
    ...
    def end(self):
        ...
        if self.option("anno_type") == "go":
            ...
            result_dir.add_relpath_rules([
                ...
                ["enrichgo.bar.pdf", "", "kegg富集分析直方图", 0, "220076"],
                ["enrichgo.bubble.pdf", "", "kegg富集分析气泡图", 0, "220076"],
                ["enrichgo.bubble2.pdf", "", "kegg富集分析气泡图", 0, "220076"],
            ])
    ...
    def multtest_correct(self, p_values, methods=3): #复制于package下的kegg_rich.py
        ...
        def fdrcorrection(pvals, alpha=0.05, method='indep', is_sorted=False):
            ...
            if method in ['i', 'indep', 'p', 'poscorr']:
                ...
                self.set_error('only indep and negcorr implemented', code="12501202")
