...
class ToolCircularbarWorkflow(Workflow):
    """
    Go分类环形柱状图
    """
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(ToolCircularbarWorkflow, self).__init__(wsheet_object)
        options = [
            {"name": "task_id", "type": "string"},
            {"name": "submit_location", "type": "string"},
            {"name": "update_info", "type": "string"},
            {"name": "go_id_level", "type": "string"},
    ...
    def set_db(self):
        tool_lab_api = self.api.api("itraq_and_tmt.tool_lab_api")
        self.s3_path = '{}/{}'.format(self._sheet.output, 'circularbar.txt')
        tool_lab_api.add_circularbar(self.option('main_id'))
        self.set_output()

    def set_output(self):
        circularbar_path = os.path.join(self.work_dir, 'circularbar.txt')
        os.link(circularbar_path, os.path.join(self.output_dir, 'circularbar.txt'))
        self.end()
    ...
    def end(self):
        ...
        super(ToolCircularbarWorkflow, self).end()
