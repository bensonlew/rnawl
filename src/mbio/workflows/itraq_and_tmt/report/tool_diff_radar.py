...
class ToolDiffRadarWorkflow(Workflow):
    """
    基因集venn图
    """
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(ToolDiffRadarWorkflow, self).__init__(wsheet_object)
        options = [
            ...
            {'name': "tool_type", 'type': 'string', 'default': 'diff_radar'},
    ...
    def end(self):
        ...
        super(ToolDiffRadarWorkflow, self).end()

    def set_db(self):
        diff_plot = self.api.api("itraq_and_tmt.tool_lab_api")
        self.s3_path = '{}/{}'.format(self._sheet.output, 'diff_radar.txt')
        diff_plot.add_diff_radar(self.option('main_id'))
        self.set_output()

    def set_output(self):
        diff_plot_path = os.path.join(self.work_dir, 'diff_radar.txt')
        if os.path.exists(os.path.join(self.output_dir, 'diff_radar.txt')):
            os.remove(os.path.join(self.output_dir, 'diff_radar.txt'))
        os.link(diff_plot_path, os.path.join(self.output_dir, 'diff_radar.txt'))
        self.end()
