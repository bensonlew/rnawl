...
class ExpPcaWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self.tool = self.add_tool("itraq_and_tmt.exp_pca_meta")
    ...
    def chart(self):
        ...
        sample_pca_sites = os.path.join(self.tool.work_dir, "output", "pca_sites.xls")
        sample_pca_propor = os.path.join(self.tool.work_dir, "output", "pca_importance.xls")
        chart.chart_sample_pca(sample_pca_sites, sample_pca_propor)
        chart.to_pdf()

        # move pdf to result dir
        pdf_file = glob.glob(self.work_dir + "/*.pdf")[0]
        os.link(pdf_file, self.tool.output_dir + "/SamPCA.pdf")
    ...
    def end(self):
        ...
        self.inter_dirs = [
            ["2_SampleComp", "", "样本比较分析",0],
            ["2_SampleComp/01_SamPca", "", "PCA分析图", 0]
        ]
