...
class ItraqtmtWorkflow(Workflow):
    def __init__(self, wsheet_object):
        """
        workflow option参数设置
        """
        self._sheet = wsheet_object
        super(ItraqtmtWorkflow, self).__init__(wsheet_object)
        options = [
            ##选择输入文件
            {"name": "itraq_or_tmt", "type": "string", "default": ""},  # 分析流程, itraq or tmt
            {"name": "ref_set", "type": "bool", "default": False},# 是否设置了内参
            {"name": "protein", "type": "infile", "format": "itraq_and_tmt.common"},  # 输入蛋白鉴定表
            {"name": "psm", "type": "infile", "format": "itraq_and_tmt.common"},  # 输入PSM表
            {"name": "peptide", "type": "infile", "format": "itraq_and_tmt.common"},  # 输入肽段表
            {"name": "protein_information", "type": "infile", "format": "itraq_and_tmt.common"},  # 输入蛋白信息表
            {"name": "ratio_exp", "type": "infile", 'format': "itraq_and_tmt.ratio_exp"},  # 输入表达量ratio表
            #{"name": "scaled_exp", "type": "infile", "format": "itraq_and_tmt.scaled_exp"},  # 输入表达量Abundance表
            {"name": "protein_fasta", "type": "infile", "format": "itraq_and_tmt.common"},  # 输入蛋白fasta序列文件
            {"name": "protein_group", "type": "infile", "format": "itraq_and_tmt.group_table"},  # 输入分组文件
            {"name": "protein_control", "type": "infile", "format": "itraq_and_tmt.compare_table"},  # 输入对照组文件
            ...
            {"name": "diamand_version", "type": "string", "default": "v0.9.24.125"}
        ...
        if re.match(r'tsanger:',self.workflow_output_tmp):
            ...
            self.set_error("json output wrong", code="12500408")
        ...
        try:
            protein_df = pd.read_table(self.option('protein').prop['path'], sep='\t')
        except:
            try:
                ...
                try:
                    ...
                    raise OptionError("The file format of protein table is wrong, only support three(txt,csv,xls) formats!")
        ...
        try:
            peptide_df = pd.read_table(self.option('peptide').prop['path'], sep='\t')
        except:
            try:
                ...
                try:
                    ...
                    raise OptionError("The file format of peptide table is wrong, only support three(txt,csv,xls) formats!")
        ...
        try:
            psm_df = pd.read_table(self.option('psm').prop['path'], sep='\t')
        except:
            try:
                ...
                try:
                    ...
                    raise OptionError("The file format of psm table is wrong, only support three(txt,csv,xls) formats!")
        ...
        #添加tool/module
        self.filecheck = self.add_tool("itraq_and_tmt.filecheck_itraq")
        self.searchdb = self.add_tool("itraq_and_tmt.searchdb")
        self.sam_corr = self.add_tool("itraq_and_tmt.exp_corr")
        # self.sam_pca = self.add_tool("itraq_and_tmt.exp_pca")
        self.sam_pca = self.add_tool("itraq_and_tmt.exp_pca_meta")
        # 置信圈
        self.ellipse = self.add_tool("graph.ellipse")
        self.diff_pep = self.add_tool("itraq_and_tmt.diff")
        self.annotation = self.add_module("itraq_and_tmt.protein_annotation")

        # 工作流中跑蛋白集相关内容
        self.diff_cluster = self.add_module("itraq_and_tmt.diff_cluster")
        self.diff_cog_class = self.add_module("itraq_and_tmt.diff_cog_class")
        self.diff_go_class = self.add_module("itraq_and_tmt.diff_go_class")
        self.diff_kegg_class = self.add_module("itraq_and_tmt.diff_kegg_class")
        self.diff_pfam_stat = self.add_module("itraq_and_tmt.diff_pfam_stat")
        self.diff_subloc_stat = self.add_module("itraq_and_tmt.diff_subloc_stat")
        self.diff_ipath = self.add_module("itraq_and_tmt.diff_ipath")
        self.diff_ppi = self.add_module("itraq_and_tmt.diff_ppi")
        self.diff_string_picture = self.add_module("itraq_and_tmt.diff_string_pictures")
        self.diff_go_enrich = self.add_module("itraq_and_tmt.diff_go_enrich")
        self.diff_kegg_enrich = self.add_module("itraq_and_tmt.diff_kegg_enrich")
        self.diff_circle = self.add_module("itraq_and_tmt.diff_circle")


        #判断流程结束tool/module list
        self.final_tools = [self.searchdb, self.sam_corr, self.sam_pca, self.diff_cluster,
                            self.diff_cog_class,
                            self.diff_go_class, self.diff_kegg_class, self.diff_pfam_stat, self.diff_subloc_stat,
                            self.diff_ipath, self.diff_ppi, self.diff_circle]

        # 添加step，显示在页面进度条
        # 添加step，显示在页面进度条
        all_steps = ["filecheck", "searchdb", "sam_corr", "sam_pca", "diff_pep",
                     "annotation", "diff_cluster", "diff_cog_class", "diff_go_class",
                     "diff_kegg_class", "diff_pfam_stat", "diff_subloc_stat", "diff_ipath", "diff_ppi",
                     "diff_string_picture",
    ...
    def delete_mongo_data(self):
        delete = DeleteDemoMongo(self.task_id, 'itraq_tmt')
        try:
            delete.run()
        except:
    ...
    def check_options(self):
        ...
        try:
            ...
            raise OptionError("传入的evalue值不符合规范", code = "12500401")
        else:
    ...
    def run(self):
        ...
        self.run_filecheck()
        super(ItraqtmtWorkflow, self).run()
    ...
    def run_chart(self):
        ...
        try:
            self.tmp_add_proteininfo(proteininfo_exp=self.option("protein_information").prop['path'])
        except:
            pass
        self.tmp_add_pepnum(pepnum_exp=self.option("protein").prop['path'])
        self.tmp_add_peplen(peplen_exp=self.option("peptide").prop['path'])
        self.tmp_add_peperror(peperror_exp=self.option("psm").prop['path'])
        #
        self.chart = self.add_tool('itraq_and_tmt.chart')
        chart_dict = {
            # "type": "workflow",
            # "diff_group":[i[1]+"_vs_"+i[0] for i in self.option("protein_control").prop["cmp_list"]],
            # "protein_mw_distribution":self.work_dir+"/qc/Protein_mw_distribution.xls",
            # "protein_seq_cover_distribution":self.work_dir+"/qc/Protein_seq_cover_distribution.xls",
            # "protein_infomation":self.work_dir+"/qc/Protein_infomation.xls",
            # "peptite_number_distribution":self.work_dir+"/qc/Peptite_number_distribution.xls",
            # "peptite_length_distribution":self.work_dir+"/qc/Peptite_length_distribution.xls",
            # "peptide_dmass":self.work_dir+"/qc/dMass.xls",
        ...
        chart_dict2.update({"diff_path_for_proteinset_for_venn":self.diff_pep.work_dir,"type": "workflow", "diff_group":[i[1]+"_vs_"+i[0] for i in self.option("protein_control").prop["cmp_list"]]})
    ...
    def run_searchdb(self):
        ...
        self.searchdb.set_options({
            ...
            'ratio_exp': self.option('ratio_exp').prop['path']
        })
    ...
    def treat(self):
        ...
        def scale_func(row):
            ...
            scale_row = row/mean_row
            return scale_row
        df_scaled = df.apply(scale_func, axis=1)
        df_scaled.to_csv(self.work_dir + "/treat_ref", sep = '\t')

    def run_sam_corr(self):
        self.logger.info("开始运行样本相关性分析")
        if self.option("ref_set") == True:
            self.sam_corr.set_options({'exp': self.option("ratio_exp").prop['path']})
        else:
            self.treat()
            self.sam_corr.set_options({'exp': self.work_dir + "/treat_ref"})
    ...
    def run_sam_pca(self):
        ...
        self.sam_pca.set_options(dict(
            exp_file=self.option('ratio_exp').prop['path'],
            group_file=self.option('protein_group').prop['path'],
        )
            # 'exp': self.option('ratio_exp').prop['path']
            # 'otutable': self.option('ratio_exp').prop['path']
    ...
    def run_diamond(self):
        ...
        if 'cog' in self.option('database'):
            self.diamond_eggnog = self.add_module("itraq_and_tmt.diamond")
            blast_opts.update(
                {
                    'database': 'eggnog',
            ...
            self.diamond_string = self.add_module("itraq_and_tmt.diamond")
            blast_opts.update(
                {
                    'database': 'string',
                    'evalue': self.option('cog_evalue'),
        ...
        if 'kegg' in self.option('database'):
            self.diamond_kegg = self.add_module("itraq_and_tmt.diamond")
            blast_opts.update(
                {
                    'database': 'kegg',
        ...
        if 'pfam' in self.option("database"):
            self.pfam = self.add_tool("itraq_and_tmt.annotation.pfam")
            opts = {
    ...
    def run_diff_cluster(self):
        self.logger.info("开始运行差异蛋白cluster分类注释")
        opts = {
    ...
    def run_diff_string_picture(self):
        self.logger.info("开始运行差异蛋白数据库图片等的爬取")
        useblast = 'no'
    ...
    def move2outputdir(self, olddir, newname, mode='link'):
        ...
        if not os.path.isdir(olddir):
            self.set_error('需要移动到output目录的文件夹不存在。', code = "12500407")
    ...
    def end(self):
        ...
        # 声明是V2
        col2.update({"task_id": self.task_id}, {"$set": {"version": "V2.1"}}, upsert=True)
        ...
        super(ItraqtmtWorkflow, self).end()
    ...
    def modify_output(self):
        ...
        ratio_exp = self.option('ratio_exp').prop["path"]
        os.link(ratio_exp, target_dir + "/Express/ratio_exp.xls")
        if self.option("ref_set") == False:
            scaled_exp = os.path.join(self.work_dir, "treat_ref")
            os.link(scaled_exp, target_dir + "/Express/scaled_exp.xls")
        ...
        for file in os.listdir(diff_path):
            ...
            try:
                os.link(os.path.join(diff_path, 'diff_protein_summary_itraq_and_tmt.xls'), os.path.join(tmp, 'diff_protein_summary_itraq_and_tmt.xls'))
            except:
                pass
        for dir in ['Express', 'SamCorr', 'SamPca', 'DiffPep', 'Ellipse']:
            try:
        ...
        for i in ["pep_len.pdf","pep_num.pdf","pep_error.pdf","info.pdf","mw.pdf","seq_cov.pdf"]:
            pdf_file = os.path.join(self.work_dir, 'Chart/output/', tmp_work_dir_name, i)
            if os.path.exists(pdf_file):
                os_link(pdf_file, os.path.join(self.output_dir, '1_DataInfo', '02_QcInfo', i))
        #
        ...
        #
        for i in ["sam_pca.pdf"]:
            pdf_file = os.path.join(self.work_dir, 'Chart/output/', tmp_work_dir_name, i)
            if os.path.exists(pdf_file):
                os_link(pdf_file, os.path.join(self.output_dir, '2_SampleComp', '01_SamPca', i))
        for i in ["sam_corr.pdf"]:
            pdf_file = os.path.join(self.work_dir, 'Chart/output/', tmp_work_dir_name, i)
            if os.path.exists(pdf_file):
                os_link(pdf_file, os.path.join(self.output_dir, '2_SampleComp', '02_SamCorr', i))
        for i in ["anno_stat.pdf"]:
            pdf_file = os.path.join(self.work_dir, 'Chart/output/', tmp_work_dir_name, i)
            if os.path.exists(pdf_file):
                os_link(pdf_file, os.path.join(self.output_dir, '3_Annotation', '01_Stat', i))
        for i in ["go_lev2_bar.pdf","go_lev2_pie.pdf","go_lev3_bar.pdf","go_lev3_pie.pdf","go_lev4_bar.pdf","go_lev4_pie.pdf"]:
            pdf_file = os.path.join(self.work_dir, 'Chart/output/', tmp_work_dir_name, i)
            if os.path.exists(pdf_file):
                os_link(pdf_file, os.path.join(self.output_dir, '3_Annotation', '02_GO', i))
        for i in ["path_class.pdf","key_path.pdf"]:
            pdf_file = os.path.join(self.work_dir, 'Chart/output/', tmp_work_dir_name, i)
            if os.path.exists(pdf_file):
                os_link(pdf_file, os.path.join(self.output_dir, '3_Annotation', '03_KEGG', i))
        for i in ["cog_bar.pdf"]:
            pdf_file = os.path.join(self.work_dir, 'Chart/output/', tmp_work_dir_name, i)
            if os.path.exists(pdf_file):
                os_link(pdf_file, os.path.join(self.output_dir, '3_Annotation', '04_COG', i))
        for i in ["pfam_bar.pdf"]:
            pdf_file = os.path.join(self.work_dir, 'Chart/output/', tmp_work_dir_name, i)
            if os.path.exists(pdf_file):
                os_link(pdf_file, os.path.join(self.output_dir, '3_Annotation', '05_Pfam', i))
        for i in ["subloc_bar.pdf"]:
            pdf_file = os.path.join(self.work_dir, 'Chart/output/', tmp_work_dir_name, i)
            if os.path.exists(pdf_file):
                os_link(pdf_file, os.path.join(self.output_dir, '3_Annotation', '06_SubLoc', i))
        for i in ["_scatter.pdf","_volcano.pdf"]:
            for pre in all_cmp_names_list:
                pdf_file = os.path.join(self.work_dir, 'Chart/output/', tmp_work_dir_name, pre+i)
                if os.path.exists(pdf_file):
                    if not os.path.exists(os.path.join(self.output_dir, '4_ExpDiff', pre)):
                        os.makedirs(os.path.join(self.output_dir, '4_ExpDiff', pre))
                    os_link(pdf_file, os.path.join(self.output_dir, '4_ExpDiff', pre, pre+i))
        ...
        for i in ["venn.pdf"]:
            ...
            if os.path.exists(pdf_file):
                ...
                os_link(pdf_file, os.path.join(self.output_dir, '5_Proteinset', '02_PsetVenn', i))
        for i in ['_all_go.pdf','_up_down_go.pdf','_up_down_go_neg.pdf']:
        ...
        for i in ['_all_pfam.pdf','_up_down_pfam.pdf']:
            for comp in all_cmp_names_list:
                one_pdf = comp+'____'+comp+i
                one_pdf_full = os.path.join(self.work_dir, 'Chart/output/', tmp_work_dir_name, one_pdf)
                if os.path.exists(one_pdf_full):
                    if not os.path.exists(os.path.join(self.output_dir, '5_Proteinset', '03_PsetAnno','04_PsetPfam', comp)):
                        os.makedirs(os.path.join(self.output_dir, '5_Proteinset', '03_PsetAnno','04_PsetPfam', comp))
                    os_link(one_pdf_full, os.path.join(self.output_dir, '5_Proteinset', '03_PsetAnno', '04_PsetPfam', comp, comp+i))
        for i in ['_all_subloc.pdf','_up_down_subloc.pdf']:
            for comp in all_cmp_names_list:
                one_pdf = comp+'____'+comp+i
                one_pdf_full = os.path.join(self.work_dir, 'Chart/output/', tmp_work_dir_name, one_pdf)
                if os.path.exists(one_pdf_full):
                    if not os.path.exists(os.path.join(self.output_dir, '5_Proteinset', '03_PsetAnno','05_PsetSubLoc', comp+i[:-11])):
                        os.makedirs(os.path.join(self.output_dir, '5_Proteinset', '03_PsetAnno','05_PsetSubLoc', comp+i[:-11]))
                    os_link(one_pdf_full, os.path.join(self.output_dir, '5_Proteinset', '03_PsetAnno', '05_PsetSubLoc', comp+i[:-11], comp+i))
        ...
        repaths = [
            [".", "", "流程分析结果目录", 0, "220001"],
            ["SequenceDatabase", "", "蛋白序列数据库文件", 0, "220002"],
            ["SequenceDatabase/seq_db.sqlite3", "", "", 1, "220003"],
            ["ProteinFasta", "", "蛋白序列FASTA文件", 0, "220004"],
            ["SearchDB", "", "搜库结果目录", 0, "220005"],
            ["SearchDB/protein_sliced.xls", "", "搜库得到的蛋白详细信息", 0, "220006"],
            ["SearchDB/search_db.xls", "", "搜库得到的蛋白详细信息以及在各个样本中的丰度", 0, "220007"],
            ["SamPca", "", "样本PCA分析结果目录", 0, "220008"],
            ["SamPca/pca_rotation.xls", "", "蛋白相关主成分贡献度", 0, "220009"],
            ["SamPca/pca_sites.xls", "", "PCA分析样本坐标表", 0, "220010"],
            ["SamPca/pca_importance.xls", "", "PCA主成分解释表", 0, "220011"],
            ["SamCorr", "", "样本相关性分析结果目录", 0, "220012"],
            ["SamCorr/sample_correlation.xls", "", "样本间相关性系数表", 0, "220013"],
            ["SamCorr/expression_matrix.xls", "", "蛋白表达丰度矩阵表", 0, "220014"],
            ["Annotation", "", "蛋白注释结果目录", 0, "220015"],
            ["Annotation/anno_stat", "", "蛋白注释统计结果目录", 0, "220016"],
            ["Annotation/anno_stat/proteins_anno_detail.xls", "", "鉴定蛋白附加6数据库详细信息的表", 0, "220017"],
            ["Annotation/anno_stat/all_annotation_statistics.xls", "", "6个数据库中相关蛋白数以及总蛋白占比", 0, "220018"],
            ["Annotation/anno_stat/venn", "", "鉴定结果中蛋白在各个数据库注释结果统计VENN图目录", 0, "220019"],
            ["Annotation/anno_stat/venn/pfam_venn.txt", "", "有与pfam数据库相关信息的蛋白", 0, "220020"],
            ["Annotation/anno_stat/venn/kegg_venn.txt", "", "有与kegg数据库相关信息的蛋白", 0, "220021"],
            ["Annotation/anno_stat/venn/go_venn.txt", "", "有与go数据库相关信息的蛋白", 0, "220022"],
            ["Annotation/anno_stat/venn/cog_venn.txt", "", "有与cog数据库相关信息的蛋白", 0, "220023"],
            ["Annotation/anno_stat/venn/nr_venn.txt", "", "有与nr数据库相关信息的蛋白", 0, "220024"],
            ["Annotation/anno_stat/venn/subloc_venn.txt", "", "有与subloc数据库相关信息的蛋白", 0, "220025"],
            ["Annotation/go", "", "鉴定蛋白与GO数据库比对结果详情目录", 0, "220026"],
            ["Annotation/go/blast2go.annot", "", "鉴定蛋白与GO的对应关系以及GOterm名称", 0, "220027"],
            ["Annotation/go/query_gos.list", "", "鉴定蛋白与GO的对应关系", 0, "220028"],
            ["Annotation/go/go12level_statistics.xls", "", "鉴定蛋白的GO1、2层级的详细信息", 0, "220029"],
            ["Annotation/go/go123level_statistics.xls", "", "鉴定蛋白的GO1、2、3层级的详细信息", 0, "220030"],
            ["Annotation/go/go1234level_statistics.xls", "", "鉴定蛋白的GO1、2、3、4层级的详细信息", 0, "220031"],
            ["Annotation/cog", "", "鉴定蛋白与COG数据库比对结果详情目录", 0, "220032"],
            ["Annotation/cog/cog_table.xls", "", "鉴定蛋白序列文件与COG比对结果表", 0, "220033"],
            ["Annotation/cog/cog_summary.xls", "", "COG数据库注释结果汇总", 0, "220034"],
            ["Annotation/cog/cog_list.xls", "", "鉴定蛋白与COG的对应关系", 0, "220035"],
            ["Annotation/kegg", "", "鉴定蛋白与KEGG数据库比对结果详情目录", 0, "220036"],
            ["Annotation/kegg/pathways", "", "KEGG通路注释结果图片目录", 0, "220037"],
            ["Annotation/kegg/pathways/map*.png", "", "通路注释结果图片-位图", 0, "220038"],
            ["Annotation/kegg/pathways/map*.pdf", "", "通路注释结果图片-矢量图", 0, "220039"],
            ["Annotation/kegg/pathway_table.xls", "", "每行以通路为单位显示通路注释详情表", 0, "220040"],
            ["Annotation/kegg/kegg_table.xls", "", "每行以蛋白为单位显示通路注释详情表", 0, "220041"],
            ["Annotation/kegg/pid.txt", "", "每个注释通路和映射上去的蛋白联系表", 0, "220042"],
            ["Annotation/subloc", "", "蛋白亚细胞定位注释结果目录", 0, "220043"],
            ["Annotation/subloc/multiloc_stat.xls", "", "亚细胞位置对应蛋白的数量表", 0, "220044"],
            ["Annotation/subloc/multiloc.xls", "", "亚细胞定位结果表", 0, "220045"],
            ["Annotation/blast_xml", "", "蛋白比对注释结果目录", 1, "220046"],
            ["Annotation/blast_xml/kegg.xml", "", "鉴定蛋白与数据库比对结果文件目录", 0, "220047"],
            ["Annotation/blast_xml/string.xml", "", "鉴定蛋白与STRING数据库比对结果xml形式", 0, "220048"],
            ["Annotation/blast_xml/pfam_domain", "", "鉴定蛋白与pfam数据库比对结果xml形式", 0, "220049"],
            ["Annotation/blast_xml/nr.xml", "", "鉴定蛋白与NR数据库比对结果xml形式", 0, "220050"],
            ["Annotation/blast_xml/protein.xls", "", "鉴定蛋白详情表", 0, "220051"],
            ["Annotation/blast_xml/protein.fa", "", "蛋白序列", 0, "220052"],
            ["DiffPep", "", "全部样本差异表达详情目录", 0, "220053"],
            ["DiffPep/diff_protein_summary.xls", "", "所有组差异信息汇总表", 0, "220054"],
            ["DiffPep/*.diff.xls", "", "样本间差异比较信息表", 0, "220055"],
            ["DiffPep/*_diff.detail.xls", "", "样本间差异比较信息与蛋白详细注释信息表", 0, "220056"],
            ["Express", "", "蛋白定量分析结果目录", 0, "220057"],
            ["Express/ratio_exp.xls", "", "蛋白在各个样本间质谱峰面积比值表", 0, "220058"],
            ["Express/scaled_exp.xls", "", "蛋白在各个样本间质谱峰面积表", 0, "220059"],
            ["1_DataInfo", "", "搜库数据信息"],
            ...
            ["2_SampleComp/02_SamCorr/sam_corr.pdf", "", "相关性热图"],
            ...
            ["3_Annotation/02_GO/go_lev2_bar.pdf", "", "直方图"],
            ["3_Annotation/02_GO/go_lev2_pie.pdf", "", "饼图"],
            ["3_Annotation/02_GO/go_lev3_bar.pdf", "", "直方图"],
            ["3_Annotation/02_GO/go_lev3_pie.pdf", "", "饼图"],
            ["3_Annotation/02_GO/go_lev4_bar.pdf", "", "直方图"],
            ["3_Annotation/02_GO/go_lev4_pie.pdf", "", "饼图"],
            ...
            ["5_Proteinset/02_PsetVenn/venn.pdf", '', '韦恩图'],
        ...
        if self.option("report_img"):
            dmass_origin = os.path.join(self.plot_static_peperr.output_dir, 'dMass.png')
            dmass_target = os.path.join(self.chart.work_dir, 'png', 'dMass.png')
            if os.path.exists(dmass_target):
                os.remove(dmass_target)
            os.link(dmass_origin, dmass_target)
            self.logger.info("开始上传报告中的png图片")
            s3 = self._sheet.output.split(":")[0]
            report_img_dir = self.chart.work_dir + '/png/'
            report_img_s3 = s3 + "://commonbucket/files/report_img/itraq/" + self.task_id + "/"
    ...
    def run_api(self, test=False):
        ...
        task_info = self.api.api('task_info.itraq_task_info')
    ...
    def export_report_img(self):
        ...
        api =  self.api.api('itraq_and_tmt.report_model')
        s3 = self._sheet.output.split(":")[0]
        report_img_s3 = s3 + ":commonbucket/files/report_img/itraq/" + self.task_id
    ...
    def export_specimen(self):
        gevent.sleep()
        self.api_specimen = self.api.api("itraq_and_tmt.specimen")
        self.api_specimen.add_specimen(group_txt=self.option("protein_group").prop["path"], params=None,
                project_sn=self.project_sn, task_id=self.task_id, type=self.option("itraq_or_tmt"))
    ...
    def export_searchdb(self):
        ...
        if self.option('change_des'):
            ...
            acc2des = dict()
            hit_description = [i.strip().split('>')[0] for i in nr_df['Hit-Description'].tolist()]
            # for acc, des in zip(nr_df['Query-Name'], nr_df['Hit-Description']):
            for acc, des in zip(nr_df['Query-Name'], hit_description):
        ...
        self.api_searchdb.add_searchdb(project_sn=self.project_sn,
                task_id=self.task_id, name=None, protein_selected = self.searchdb.work_dir + "/protein_sliced.xls")

    @time_count
    def export_sam_corr(self):
        gevent.sleep()
        self.api_sam_corr = self.api.api("itraq_and_tmt.all_exp")
        params = dict(
    ...
    def export_qc(self):
        ...
        self.api_express = self.api.api("itraq_and_tmt.express")
        ...
        self.api_proteininfo = self.api.api("itraq_and_tmt.proteininfo")
        ...
        self.api_express.add_express(params=params, main_id=None,
                                     ...
                                     express_exp=self.option("ratio_exp").prop['path'])
        if self.option("ref_set") == True:
            self.api_express.add_express(params=params, main_id=None,
    ...
    def export_pep_diff(self):
        gevent.sleep()
        self.api_diff = self.api.api("itraq_and_tmt.diff")
        ...
        #     pvalue=self.option('pvalue'), diff_method=self.option('method_type'),control_id=str(self.control_id), task_type=2, sig_type='pvalue')
        params = dict(
            task_id=self.task_id,
            submit_location="diff",
            group_id=str(self.group_id),
            group_dict=self.option('protein_group').prop['group_dict'],
            type= "origin", correct_method=self.option('correct_method'), fc_up=str(self.option('fc_up')), fc_down=str(self.option('fc_down')),
            pvalue=str(self.option('pvalue')), diff_method=self.option('method_type'),control_id=str(self.control_id), task_type=2, sig_type='pvalue')
        params = json.dumps(params, sort_keys=True, separators=(',', ':'))
        work_dir = self.diff_pep.work_dir
        compare_dict_xls = work_dir + "/compare_dict.xls"
        num_summary_xls = work_dir + "/num_summary.xls"
        allsummary_xls = work_dir + "/allsummary.xls"
    ...
    def export_annotation(self):
        ...
        '''
        gevent.sleep()
        self.annotation_api = self.api.api("itraq_and_tmt.protein_annotation")
    ...
    def export_proteinset(self):
        ...
        self.export_proteinset = self.api.api("itraq_and_tmt.proteinset")
    ...
    def build_seq_database(self):
        ...
        self.export_seq = self.api.api("itraq_and_tmt.protein_seq")
    ...
    def add_anno_to_diff(self):
        ...
        if self.option("ref_set"):  # modified by zhangyitong on 20211124
            treat_ref_df = pd.read_table(self.option("ratio_exp").prop['path'], sep="\t", header=0, index_col=0)
        else:
            treat_ref_df = pd.read_table(self.work_dir + "/treat_ref", sep="\t", header=0, index_col=0)
        # treat_ref_df = pd.read_table(self.work_dir + "/treat_ref", sep="\t", header=0, index_col=0)
