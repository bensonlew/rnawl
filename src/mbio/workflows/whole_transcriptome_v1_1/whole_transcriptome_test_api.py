...
class WholeTranscriptomeTestApiWorkflow(Workflow):
    ...
    def enter_pipeline(self):
        ...
        if ',' in str(self.option('mirbase_specie')):
            mirbase_specie = self.option('mirbase_specie').split(',')[0]
        else:
    ...
    def run_whole_snp(self):
        ...
        large_chr = {'Ginkgo_biloba': ['v1.0'], 'Triticum_turgidum': ['ensembl_45', 'iwgsc_refseq']}
        if organism_name in large_chr and annot_version in large_chr[self.organism_name]:
    ...
    def export_gensets_analysis(self):
        ...
        diff_geneset_pipline_result = "/mnt/lustre/users/sanger-dev/wpm2/workspace/20210818/test_for_whole_transcriptome_4/DiffGenesetAllPipline/output"
        diff_id = None
        task_id = "vf87_p8fpav2hapqsbfarvsamqj"
        analysis_names = ["kegg", "go", "cog"]
        file_json_path = "/mnt/lustre/users/sanger-dev/wpm2/workspace/20210818/test_for_whole_transcriptome_4/DiffGenesetAllPipline/DiffGenesetAll/output/prepare_json"
    ...
    def export_qc_l(self):
        ...
        api.add_qc_detail(qc_id, qc_stat_before, qc_stat_after, 'long')
    ...
    def export_qc_s(self):
        if self.lib_dict['small']:
            ...
            api.add_sample_info(sample_list=sample_list, library='small', group_file=group_file)
    ...
    def export_qc_c(self):
        if self.lib_dict['circle']:
            ...
            api.add_sample_info(sample_list=sample_list, library='circle', group_file=group_file)
    ...
    def export_mapping_l(self, assess=True):
        ...
        api.add_mapping_stat(stat_file=stat_file, library='long', method=method)
        if assess:
            ...
            params = json.dumps({'task_id': self.task_id, 'submit_location': 'mapping', 'task_type': 2}, sort_keys=True,
                                separators=(',', ':'))
            api.add_chrom_distribution_table(chr_dir, params=params, library='long')
            api.add_coverage_table(cov_dir, params=params, detail=True, library='long')
            api.add_distribution_table(dis_dir, params=params, library='long')
    ...
    def export_srna(self):
        if self.lib_dict['small']:
            ...
            params = json.dumps(
                {'task_id': self.task_id, 'submit_location': 'srna', 'task_type': 2, 'method': 'mirdeep'},
                sort_keys=True, separators=(',', ':'))
            api.add_mirna_stat(mirna_stat, project_sn=self.project_sn, task_id=self.task_id, params=params)
            params = json.dumps(
                {'task_id': self.task_id, 'submit_location': 'srna', 'task_type': 2, 'method': 'mirdeep'},
                sort_keys=True, separators=(',', ':'))
    ...
    def export_snp(self):
        ...
        api.add_snp_main(snp_anno=snp_anno, params=params, method_type=method_type, task_id=task_id,
    ...
    def def_upload_dir(self):
        ...
        sdir.add_regexp_rules([
            ...
            [r'circrna/05_Diff_Express/.*_vs_.*_{}.xls'.format(c_method), 'xls', '差异分析结果表', 0],
        ])
...
class TestFunction(unittest.TestCase):
    ...
    def test(self):
        from mbio.workflows.whole_transcriptome.whole_transcriptome import WholeTranscriptomeTestApiWorkflow
