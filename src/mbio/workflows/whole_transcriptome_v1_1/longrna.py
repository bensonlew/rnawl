...
class LongrnaWorkflow(Workflow):
    ...
    def load_libraries(self):
        ...
        self.modules['diff_exp_g'] = self.add_module('whole_transcriptome.diff_exp')
        self.modules['diff_exp_t'] = self.add_module('whole_transcriptome.diff_exp')
    ...
    def enter_pipeline(self):
        ...
        self.on_rely(relies_2, self.set_db)
    ...
    def run_assembly(self):
        ...
        ref_fa = os.path.join(self.db_path, self.genome_doc['dna_fa'])
        opts = {
            'bamlist': bamlist,
            'strand_specific': self.option('strand_specific'),
    ...
    def run_large_gush(self):
        ...
        if 'lnc_dir' in self.genome_doc:
            ref_mrna_gtf = os.path.join(self.db_path, self.genome_doc['lnc_dir'], 'mrna.gtf')
            ref_lncrna_gtf = os.path.join(self.db_path, self.genome_doc['lnc_dir'], 'lncrna.gtf')
            ref_mrna_fasta = os.path.join(self.db_path, self.genome_doc['lnc_dir'], 'mrna.fa')
            ref_lncrna_fasta = os.path.join(self.db_path, self.genome_doc['lnc_dir'], 'lncrna.fa')
            ids_mapping = os.path.join(self.db_path, self.genome_doc['lnc_dir'], 'ids_matrix.xls')
        else:
            ref_mrna_gtf = os.path.join(self.db_path, self.genome_doc['gtf'])
            ref_lncrna_gtf = os.path.join(self.modules['large_gush'].work_dir, 'lncrna.gtf')
            ref_mrna_fasta = os.path.join(self.db_path, self.genome_doc['transcript'])
        ...
        if False:
            known_ko = os.path.join(self.db_path, self.genome_doc['kegg'])
    ...
    def run_exp_make(self):
        ...
        opts.update({'t_tpm': t_tpm, 'g_tpm': g_tpm, 't_fpkm': t_fpkm, 'g_fpkm': g_fpkm, 't_count': t_count,
                     'g_count': g_count, 'annot_table': annot_table, 'relat_table': relat_table})
