...
class LabelfreeWorkflow(Workflow):
    ...
    def run(self):
        ...
        if self.option('DIA'):  # modified by zhangyitong on 20211109
            self.on_rely(self.final_tools, self.end)
        else:
    ...
    def run_chart(self):
        ...
        #
        self.chart = self.add_tool('labelfree.chart')
        chart_dict = {
            # "type": "workflow",
    ...
    def end(self):
        ...
        # 声明是V2
        col2.update({"task_id" : self.task_id}, {"$set": {"version": "V2.2"}}, upsert=True)
    ...
    def modify_output(self):
        ...
        for file in os.listdir(diff_path):
            ...
            try:
                os.link(os.path.join(diff_path, 'diff_protein_summary_labelfree.xls'), os.path.join(tmp, 'diff_protein_summary_labelfree.xls'))
            except:
                pass
        ...
        for i in ["exp_venn.pdf"]:
            pdf_file = os.path.join(self.work_dir, 'Chart/output/', tmp_work_dir_name, i)
            if os.path.exists(pdf_file):
                os_link(pdf_file, os.path.join(self.output_dir, '2_SampleComp', '03_ExpVenn', i))
        ...
        if self.option("report_img"):
            ...
            report_img_s3 = s3 + "://commonbucket/files/report_img/labelfree/" + self.task_id + "/"
    ...
    def export_report_img(self):
        ...
        api =  self.api.api('labelfree.report_model')
        s3 = self._sheet.output.split(":")[0]
        report_img_s3 = s3 + ":commonbucket/files/report_img/labelfree/" + self.task_id
    ...
    def export_qc(self):
        ...
        if self.new:
            ...
            self.api_proteininfo._project_type = 'labelfree'
    ...
    def export_pep_diff(self):
        ...
        #     group_id=str(self.group_id),
        #     group_dict=self.option('protein_group').prop['group_dict'],
        #     type= "origin", correct_method=correct_method, fc_up=self.option('fc_up'), fc_down=self.option('fc_down'),
        ...
        params = dict(
            ...
            group_id=str(self.group_id),
            group_dict=self.option('protein_group').prop['group_dict'],
            type= "origin", correct_method=correct_method, fc_up=str(self.option('fc_up')), fc_down=str(self.option('fc_down')),
