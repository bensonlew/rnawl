...
import json
import re
from mbio.packages.labelfree.wgcna.wgcna import Wgcna
...
class WgcnaPipelineWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self.dump_tool = self.api.api("labelfree.wgcna")
        self.labelfree =Wgcna() # add by fwy 20210108 mongo库升级
        # self.labelfree = Labelfree()  # 用于创建主表
    ...
    def set_db(self):
        ...
        pipeline_params = self.labelfree.get_main_info(self.option("main_id"),'sg_wgcna_pipeline', self.option("task_id"))['params']
...
#   dump data to db for each step
    def add_prepare_result(self):
        # create main table
        exp_info = self.labelfree.get_main_info(self.option("exp_id"), 'sg_express', self.option("task_id"))
        ...
        main_info = dict(
            ...
            status="start")
        main_id = self.labelfree.insert_main_table('sg_wgcna_prepare', main_info)
        # add result info
    ...
    def add_module_result(self, wgcna_prepare_id):
        ...
        main_id = self.labelfree.insert_main_table('sg_wgcna_module', main_info)
        # add result info
    ...
    def add_network_result(self, wgcna_module_id, wgcna_relate_id):
        for each_tool in self.wgcna_network:
            ...
            main_info = dict(
                ...
                status="start")
            main_id = self.labelfree.insert_main_table("sg_wgcna_network", main_info)
            # add network detail
