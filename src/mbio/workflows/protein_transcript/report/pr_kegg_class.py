...
class PrKeggClassWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self.kegg_class = self.add_tool("protein_transcript.kegg_class")
        self._sheet.output = self._sheet.output.replace('interaction_results', 'interaction_results/7_relate/03_relate_anno/04_relate_kegg')
        self.inter_dirs = []
            ...
            for line in info_r:
                ...
                if map in map2pr:
                    ...
                    for gene in genes:
                        tmp_dict[gene] = 'pink'
                    map2pr[map] = tmp_dict
                # with open(self.work_dir + "/" + map + ".dict", 'w') as f:
                #     f.write("{} {}".format(map, map2pr[map]))
                            ...
                            for p in prs:
                                if p not in p_list:
                                    ...
                                    if p not in ko2bg:
                                        ...
                                        if ko2bg[p] != 'orange':
                                            ko2bg[p] = tmp[pr]
                            # self.logger.info("path is {} K01641 is {}".format(map, ko2bg.get("K01641", "NONE")))
                        else:
                            if pr not in ko2bg:
                                ko2bg[pr] = tmp[pr]
                            else:
    ...
    def set_db(self):
        ...
        保存结果指数表到mongo数据库中
        """
        api_kegg = self.api.api('protein_transcript.relaset')
    ...
    def chart(self):
        ...
        if os.path.exists(pt_proteinset_file) and os.path.exists(pt_kegg_stat_file) and os.path.exists(pt_gene_kegg_level_table_xls):
