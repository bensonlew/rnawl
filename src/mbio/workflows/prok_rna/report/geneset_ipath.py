...
class GenesetIpathWorkflow(Workflow):
    ...
    def set_db(self):
        ...
        if isinstance(record_id, types.StringTypes):
            ...
            self.set_error("main_id参数必须为字符串或者ObjectId类型", code = "15000301")
    ...
    def end(self):
        ...
        result_dir.add_relpath_rules([
            [".", "", "基因集Ipath结果目录"],
            ["Biosynthesis_of_secondary_metabolities.svg", "", "次级代谢产物合成通路图"],
            ["Regulatory_pathways.svg", "", "调控通路图"],
            ["Metabolic_pathways.svg", "", "iPath代谢通路图"],
            ["gene_ipath_input.xls", "", "iPath分析输入文件"],
        ])
