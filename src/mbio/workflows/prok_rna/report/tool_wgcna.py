...
class ToolWgcnaWorkflow(Workflow):
    """
    基因集venn图
    """
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(ToolWgcnaWorkflow, self).__init__(wsheet_object)
        options = [
    ...
    def end(self):
        ...
        main_info = dict(
            ...
            relate_id=main_id,
            group_path=self.s3_path_group,
            exp_path=self.s3_path_exp
        )
        ...
        super(ToolWgcnaWorkflow, self).end()

    def set_db(self):
        api = self.api.api("prok_rna.tool_lab_api")
        self.s3_path_exp = '{}/{}'.format(self._sheet.output, 'wgcna_exp.txt')
        self.s3_path_group = '{}/{}'.format(self._sheet.output, 'group_table.txt')
        api.add_wgcna(self.option('main_id'))
        self.set_output()

    def set_output(self):
        if os.path.exists(os.path.join(self.output_dir, 'wgcna_exp.txt')):
            os.remove(os.path.join(self.output_dir, 'wgcna_exp.txt'))
        os.link(os.path.join(self.work_dir, 'wgcna_exp.txt'), os.path.join(self.output_dir, 'wgcna_exp.txt'))
        self.end()
