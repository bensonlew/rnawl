...
import json
import re
from mbio.packages.dia_v3.wgcna.wgcna import Wgcna
...
class WgcnaPipelineWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self.wgcna_prepare = self.add_tool("labelfree.wgcna.wgcna_prepare")
        self.wgcna_module = self.add_tool("labelfree.wgcna.wgcna_module")
        self.wgcna_relate = self.add_tool("labelfree.wgcna.wgcna_relate")
        # self.wgcna_network = self.add_tool("rna.wgcna.wgcna_network")
        self.wgcna_network = list()
        self.dump_tool = self.api.api("dia.wgcna")
        self.dia =Wgcna() # add by fwy 20210108 mongo库升级
        # self.dia = Dia()  # 用于创建主表
    ...
    def set_db(self):
        ...
        pipeline_params = self.dia.get_main_info(self.option("main_id"),'sg_wgcna_pipeline', self.option("task_id"))['params']
    ...
    def end(self):
        ...
        result_dir.add_relpath_rules([
            ...
            ["./wgcna_relate/*protein_trait.correlation.pdf", "", "蛋白与表型相关性热图", 0],
...
#   run tools
    ...
    def run_wgcna_network(self):
        ...
        for module in target_modules:
            tool = self.add_tool("labelfree.wgcna.wgcna_network")
            options = dict(
                module=module,
...
#   dump data to db for each step
    def add_prepare_result(self):
        # create main table
        exp_info = self.dia.get_main_info(self.option("exp_id"), 'sg_express', self.option("task_id"))
        ...
        main_info = dict(
            ...
            status="start")
        main_id = self.dia.insert_main_table('sg_wgcna_prepare', main_info)
        # add result info
    ...
    def add_module_result(self, wgcna_prepare_id):
        ...
        main_id = self.dia.insert_main_table('sg_wgcna_module', main_info)
        # add result info
    ...
    def add_relate_result(self, wgcna_module_id):
        ...
        main_id = self.dia.insert_main_table('sg_wgcna_relate', main_info)
        # add result info
    ...
    def add_network_result(self, wgcna_module_id, wgcna_relate_id):
        for each_tool in self.wgcna_network:
            ...
            main_info = dict(
                ...
                status="start")
            main_id = self.dia.insert_main_table("sg_wgcna_network", main_info)
            # add network detail
