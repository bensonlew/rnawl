...
class ProteinsetClassWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        if self.option("anno_type") == "cog":
            self._sheet.output = self._sheet.output.replace('interaction_results',
                                                            'interaction_results/5_Proteinset/03_Anno/03_COG')
        if self.option("anno_type") == "go":
            self._sheet.output = self._sheet.output.replace('interaction_results',
                                                            'interaction_results/5_Proteinset/03_Anno/01_GO')
    ...
    def chart(self):
        ...
        if self.option("anno_type") == 'go':
            ...
            files = glob.glob(os.path.join(self.work_dir, '*webmulti_go.go_bar.pdf'))
            for f in files:
                geneset = os.path.basename(f).split('.', 1)[0]
                if geneset == 'webmulti_go':
                    newname = 'all_go_lev2.pdf'
                else:
                    newname = geneset + '_go_lev2.pdf'
                if os.path.exists(os.path.join(self.output_dir, newname)):
                    os.remove(os.path.join(self.output_dir, newname))
                os.link(os.path.join(self.work_dir, f), os.path.join(self.output_dir, newname))

        elif self.option("anno_type") == 'go2':
            proteinsetgo_class_file = os.path.join(self.output_dir, "go_class_table.xls")
            chart.chart_proteinsetcluster_goclass_bothdir_web(proteinsetgo_class_file)
            chart.to_pdf()
            # move pdf to result dir
            if os.path.exists(os.path.join(self.output_dir, 'go_lev2_both.pdf')):
                os.remove(os.path.join(self.output_dir, 'go_lev2_both.pdf'))
            if os.path.exists(os.path.join(self.work_dir, 'webmulti_go_neg.bar_neg.pdf')):
                os.link(os.path.join(self.work_dir, 'webmulti_go_neg.bar_neg.pdf'), os.path.join(self.output_dir, 'go_lev2_both.pdf'))
            ...
            chart.to_pdf()
            # move pdf to result dir
            if os.path.exists(os.path.join(self.output_dir, "cog.pdf")):
                os.remove(os.path.join(self.output_dir, "cog.pdf"))
