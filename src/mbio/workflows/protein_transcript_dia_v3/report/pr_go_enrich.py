...
class PrGoEnrichWorkflow(Workflow):
    """
    蛋白集富集分析
    """
    def __init__(self, wsheet_object):
        ...
        self.rpc = False
        super(PrGoEnrichWorkflow, self).__init__(wsheet_object)
        options = [
        ...
        self._sheet.output = self._sheet.output.replace('interaction_results',
                                                        'interaction_results/7_relate/03_relate_anno/02_relate_goenrich')
        self.inter_dirs = []
    ...
    def send_log(self, data):
        ...
        super(PrGoEnrichWorkflow, self).send_log(data)
    ...
    def run(self):
        ...
        for tool in self.protein_tools + self.rna_tools:
            tool.run()
        super(PrGoEnrichWorkflow, self).run()
    ...
    def set_db(self):
        ...
        def change_qvalue(file):
            ...
            for col in pf:
                if col.startswith('p_fdr'):
                    pf = pf.drop(columns=[col])
            os.remove(file)
        ...
        api_goenrich.add_go_enrich_pr(self.option("main_table_id"), protein_file, rna_file)
        self.end()
    ...
    def chart(self):
        ...
        pt_goenrich = self.output_dir
        chart.chart_pt_goenrich(pt_goenrich)
        chart.to_pdf()
        # move pdf to result dir
        pdf_file = os.path.join(self.work_dir, "pt_goenrich.shadow_bar_groups.pdf")
        if os.path.exists(pdf_file):
            if os.path.exists(os.path.join(self.output_dir, "bar.pdf")):
                os.remove(os.path.join(self.output_dir, "bar.pdf"))
            os.link(pdf_file, os.path.join(self.output_dir, "bar.pdf"))
        pdf_file = os.path.join(self.work_dir, "pt_goenrich_bubble.bubble_groups.pdf")
        if os.path.exists(pdf_file):
            if os.path.exists(os.path.join(self.output_dir, "bubble.pdf")):
                os.remove(os.path.join(self.output_dir, "bubble.pdf"))
    ...
    def end(self):
        ...
        self.inter_dirs = [
            ...
            ["7_relate/03_relate_anno/02_relate_goenrich", "", "GO富集", 0],
        ]
        ...
        super(PrGoEnrichWorkflow, self).end()
