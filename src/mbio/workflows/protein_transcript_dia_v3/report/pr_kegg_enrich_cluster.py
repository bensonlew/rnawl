...
class PrKeggEnrichClusterWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self._sheet.output = self._sheet.output.replace('interaction_results',
                                                        'interaction_results/7_relate/03_relate_anno/06_relate_keggenrich_cluster')
        self.inter_dirs = []
    ...
    def set_db(self):
        ...
        保存结果指数表到mongo数据库中
        """
        api_keggenrich = self.api.api('protein_transcript_dia_v3.relaset')
    ...
    def chart(self):
        ...
        if os.path.exists(pt_keggenrich_cluster_matrix) and os.path.exists(pt_keggenrich_cluster_tree):
            ...
            for pdf_file in pdf_files:
                os.link(pdf_file, os.path.join(self.output_dir,
                                               "sub" + os.path.basename(pdf_file)[23:].split('.')[0].split('_')[
                                                   1] + '.pdf'))
