...
class AnnoTcdbWorkflow(ComfunWorkflow):
    """
    宏基因组TCDB注释交互分析
    """
    ...
    def set_db(self):
        ...
        保存结果到mongo数据库中
        """
        api_anno = self.api.api("metagenomic.tcdb")
        ...
        if not isinstance(main_id, ObjectId):
            if isinstance(main_id, types.StringTypes):
                ...
                self.set_error('main_id必须为ObjectId对象或其对应的字符串！', code="12803601")
    ...
    def end(self):
        ...
        result_dir.add_relpath_rules([
            [".", "", "TCDB功能注释结果目录", 0, "120058"],
            ["class_abund_out.xls", "xls", "各样品Class丰度表", 0, "120060"],
            ["subclass_abund_out.xls", "xls", "各样品SubClass丰度表",0,"120273"],
            ["family_abund_out.xls", "xls", "各样品family丰度表",0,"120274"],
            ["tcdb_abund_out.xls", "xls", "各样品tcdb丰度表",0,"120275"],
            ["Class_gene_stat.xls", "xls", "各样品Class基因列表", 0, "120060"],
            ["SubClass_gene_stat.xls", "xls", "各样品SubClass基因列表",0,"120276"],
            ["Family_gene_stat.xls", "xls", "各样品family基因列表",0,"120277"],
            ["TCDB_gene_stat.xls", "xls", "各样品tcdb基因列表",0,"120278"],
            ["function_select_anno.xls", "xls", "每条基因的TCDB功能注释表",0,"120279"],
            ["TCDB_class_bar.pdf", "pdf", "class丰度图"]
        ])
