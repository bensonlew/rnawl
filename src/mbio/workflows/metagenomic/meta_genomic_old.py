...
class MetaGenomicWorkflow(Workflow):
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            {'name': 'anno_nr', 'type': 'string', 'default': 'none'},  # 适应页面参数, "nr"
            {'name': 'anno_list', 'type': 'string', 'default': 'none'},
            # 功能注释列表，逗号分割，适应页面参数, 'kegg,cog,cazy,ardb,card,vfdb'
        ...
        '''
        if self.option("unique_fa").is_set:
    ...
    def check_options(self):
        ...
        if self.option('old_task_id'):
            self.pipeline = 'pipeline2.1'
            if self.option('old_task_db') == 0:
                db = get_old_mongo()[1]
            else:
                db = get_mongo()[1]
            if not db['sg_task'].find_one({'task_id': self.option('old_task_id')}):
                self.set_error('输入的task_id为{}, {}中没有相关记录，请确保task_id正确且运行成功'.format(self.option('old_task_id'), db))
        elif self.option("unique_fa").is_set:
            self.pipeline = "pipeline2"
        else:
            self.pipeline = "pipeline1"
    ...
    def send_files(self):  # 原modify_output
        ...
        repaths = [
            ...
            ["rawdata/base_info", "", "原始序列质量统计目录", 0, "120003"],
            ["rawdata/data", "", "原始序列文件路径", 0, "120005"],
            ["rawdata/data/reads.rawData.stat.xls", "", "各样品原始数据统计表", 0, "120006"],
            ["qc", "", "质控结果目录", 0, "120007"],
            ["qc/qc_stat", "", "质控后质量统计结果", 0, "120008"],
            ["qc/qc_stat/reads.cleanData.stat.xls", "", "各样品过滤后数据统计表", 0, "120009"],
            ...
            ["kegg/kegg_merge.xml", "", "", 1, ""],  # 临时添加
            ["kegg/pathway_img", "", "", 1, ""],  # 图片路径，临时添加
        ...
        regexps = [
            [r"rawdata/base_info/.+\.fastxstat\.txt", "", "原始序列碱基质量统计文件", 0, "120004"],
    ...
    def export_specimen_group(self):
        ...
        if self.option('envtable').is_set:
            ...
            self.env_id = self.api_dic["envtable"].add_env_table(self.option('envtable').prop['path'],
                                                                 self.spname_spid, env_true=self.env_true_name)
    ...
    def export_qc(self):
        ...
        if self.option('rm_host'):
            ...
            software_ver = {'bwa_ver': 'bwa-0.7.17'}  # 增加软件版本信息
    ...
    def export_geneset(self):
        ...
        self.geneset_id = self.api_dic["geneset"].add_geneset(self.output_dir + "/geneset", geneset_path, 1, identity=self.option('cdhit_identity'))  # mark add by qingchen.zhang@20190621
    ...
    def export_kegg(self):
        ...
        xml_path = os.path.join(self.remote_dir, "kegg/kegg_merge.xml")  # 此处已将文件路径上传，导表函数只用来保存路径
        ...
        graph_path = os.path.join(self.remote_dir, "kegg/pathway_img")
    ...
    def export_vfdb(self):
        ...
        return_id = self.api_dic["vfdb"].add_anno_vfdb(self.geneset_id, "All",
                                                       ...
                                                       group_detail=vfdb_group_detail)
