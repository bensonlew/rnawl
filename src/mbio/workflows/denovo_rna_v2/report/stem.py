...
class StemWorkflow(Workflow):
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("denovo_rna_v2", table="sg_stem", main_id=self.option('main_id'),
    ...
    def run_stem(self):
        ...
        flag = True
        n = {'sanger-dev': 40, 'sanger': 60, 'isanger': 50}[self.config.wpm_user]
        while flag and n:
            n -= 1
    ...
    @reminder
    def set_db(self):
        self.database = self.api.api('denovo_rna_v2.stem')
    ...
    def end(self):
        ...
        if os.path.exists(os.path.join(self.output_dir, "cluster.tsv")):
            os.remove(os.path.join(self.output_dir, "cluster.tsv"))
        if os.path.exists(os.path.join(self.output_dir, "detail.tsv")):
            os.remove(os.path.join(self.output_dir, "detail.tsv"))
        ...
        result_dir.add_regexp_rules([
            ...
            [r'.*profile.*.pdf', 'pdf', 'profile 趋势图 ', 0],
            [r'.*trend.pdf', 'pdf', '时序趋势分析图', 0]
        ])
