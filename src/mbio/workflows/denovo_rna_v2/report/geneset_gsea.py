...
class GenesetGseaWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            {"name": "max_num", "type": "string"},
            {"name": "min_num", "type": "string"},
            ...
            {"name": "plot_top_x", "type": "int", "default": 20},
        ...
        self._sheet.output = self._sheet.output.replace('interaction_results', 'interaction_results/06 Advanced_Analysis/04 GSEA_Analysis')
        self.inter_dirs = []
    ...
    def gsea_runner(self):
        ...
        self.gsea_tool.set_options({
            ...
            'plot_top_x': self.option('plot_top_x'),
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("denovo_rna_v2", table="sg_geneset_gsea", main_id=self.option('main_table_id'),
    ...
    def set_db(self):
        ...
        保存结果指数表到mongo数据库中
        """
        api_geneset = self.api.api('denovo_rna_v2.gsea')
    ...
    def end(self):
        ...
        os.system('cp {} {}'.format(os.path.join(self.gsea_tool.output_dir,'all_exp.detail'), os.path.join(self.output_dir,'all_exp_detail.xls')))
        os.system('cp {} {}'.format(os.path.join(self.gsea_tool.output_dir,'all_sets.detail'),os.path.join(self.output_dir,'all_sets_detail.xls')))
        os.system('cp {} {}'.format(os.path.join(self.gsea_tool.output_dir,'gsea_report.xls'),os.path.join(self.output_dir,'gsea_stat.xls')))
        os.system('cp {} {}'.format(os.path.join(self.gsea_tool.output_dir,'enplot*'),self.output_dir))
        os.system('cp {} {}'.format(os.path.join(self.gsea_tool.output_dir,'run_parameter.txt'),self.output_dir))
        result_dir = self.add_upload_dir(self.output_dir)
        self.inter_dirs = [
            ["06 Advanced_Analysis", "", "高级分析结果目录",0],
            ["06 Advanced_Analysis/04 GSEA_Analysis", "", "GSEA分析结果目录", 0]
        ]
        result_dir.add_relpath_rules([
            [".", "", "GSEA分析文件",0],
            ["./*.pdf", "", "该基因集富集结果图pdf",0],
            ["./*.png", "", "该基因集富集结果图png",0],
            ["./all_sets_detail.xls", "xls", "所有基因集富集结果详情表",0,"201395"],
            ["./gsea_stat.xls", "xls", "所有基因集富集结果统计表",0,"201396"],
            ["./all_exp_detail.xls", "xls", "所有基因集leading基因表达量信息",0,"201397"],
