...
import json
import re
from mbio.packages.denovo_rna_v2.wgcna.wgcna import Wgcna
...
class WgcnaPipelineWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self.dump_tool = self.api.api("denovo_rna_v2.wgcna")
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("denovo_rna_v2", table="sg_wgcna_pipeline", main_id=self.option('main_id'),
    ...
    def set_db(self):
        ...
        self.dump_tool.update_db_record('sg_wgcna_pipeline', self.option('main_id'), main_id=ObjectId(self.option('main_id')),
                                        output_dir=workflow_output, params=new_params)
        # add result info
    ...
    def add_network_result(self, wgcna_module_id, wgcna_relate_id):
        for each_tool in self.wgcna_network:
            ...
            main_info = dict(
                ...
                # output_dir=self.get_workflow_output_dir() + '/wgcna_network/' +'network.json',
                status="start")
...
#   export file for toolq
    def export_wgcna_g2n_matrix(self, exp_matrix):
        ...
        "因为denovo_rna_v2无法获得gene_name字段，为了代码兼容，让gene_name和seq_id相等"
        output = os.path.join(self.wgcna_module.output_dir, "seq_id2gene_name.txt")
        gene2name = pd.read_table(exp_matrix, header=0,).iloc[:, [0]]
        gene2name['gene_name'] = gene2name.iloc[:, [0]]
