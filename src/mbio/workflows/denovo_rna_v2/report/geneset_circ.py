...
class GenesetCircWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self.circ = self.add_tool("denovo_rna_v2.geneset.enrich2circ")
        self._sheet.output = self._sheet.output.replace('interaction_results', 'interaction_results/04 GeneSet/07 Enrich_Circ')
        self.inter_dirs = []
    ...
    def chart(self):
        ...
        chart.denovo_chart_geneset_enrich_circ(circ_table, circ_zscore_input)
        chart.to_pdf()
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("denovo_rna_v2", table="sg_geneset_circ", main_id=self.option('main_table_id'),
    ...
    def end(self):
        self.chart()
        rm_file = glob.glob(self.circ.output_dir + "/enrich_zscore") + glob.glob(
            self.circ.output_dir + "/go_enrich_choose.table")
        for file in rm_file:
            os.remove(file)
        ...
        os.system('cp {} {}'.format(os.path.join(self.circ.output_dir,'*enrich_detail.table'),self.output_dir))
        os.system('cp {} {}'.format(os.path.join(self.circ.output_dir,'run_parameter.txt'),self.output_dir))
        ...
        result_dir.add_relpath_rules([
            [".", "", "富集弦图文件",0],
            ['enrich_circ.pdf', 'pdf', '富集弦图', 0],
            ['run_parameter.txt', 'txt', '运行参数日志', 0]
        ])
        result_dir.add_regexp_rules([
            [r".*enrich_detail\.table", "", "富集弦图数据表",0,"201377"]
        ])
