...
class ExpCorrsfWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self.tool = self.add_tool("denovo_rna_v2.exp_corrsf")
        all_annot=pd.read_table(self.option("anno").prop["path"], header=0)
        if "is_gene" in all_annot.columns:
            if self.option("gt").upper() == 'G':
            ...
            if self.option("gt").upper() == 'G':
                annot_pd = all_annot.set_index('gene_id')
            else:
                annot_pd = all_annot.set_index('transcript')
        self.anno=os.path.join(self.work_dir,"all_anno.xls")
        annot_pd.to_csv(self.anno,sep="\t")
        self._sheet.output = self._sheet.output.replace('interaction_results', 'interaction_results/04 GeneSet/09 Exp_Network')
        self.inter_dirs = []
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("denovo_rna_v2", table="sg_exp_corrsf", main_id=self.option('corr_main_id'),
    ...
    def set_db(self):
        ...
        保存结果表到mongo数据库中
        """
        exp_corrsf = self.api.api("denovo_rna_v2.exp_corrsf")
        # add result info
    ...
    def end(self):
        ...
        os.system('cp {} {}'.format(os.path.join(self.tool.output_dir,'express_correlation_info.xls'),self.output_dir))
        os.system('cp {} {}'.format(os.path.join(self.tool.output_dir,'run_parameter.txt'),self.output_dir))
        os.system('cp {} {}'.format(os.path.join(self.tool.output_dir, 'record.json'), self.output_dir))
