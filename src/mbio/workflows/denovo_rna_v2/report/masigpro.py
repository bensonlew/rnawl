...
from mbio.packages.denovo_rna_v2.functions import reminder
import json
import re
...
class MasigproWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self._sheet.output = self._sheet.output.replace('interaction_results', 'interaction_results/06 Advanced_Analysis/03 Time_series_Analysis')
        self.inter_dirs = []
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("denovo_rna_v2", table="sg_masigpro", main_id=self.option('main_id'),
    ...
    def run_masigpro(self):
        ...
        self.masigpro = self.add_tool('denovo_rna_v2.masigpro')
        options = {
    ...
    @reminder
    def set_db(self):
        self.database = self.api.api('denovo_rna_v2.masigpro')
    ...
    def end(self):
        ...
        self.inter_dirs = [
            ...
            ["06 Advanced_Analysis/03 Time_series_Analysis", "", "时序分析结果目录", 0]
        ]
        ...
        result_dir.add_regexp_rules([
            [r'result.xls', 'xls', '时序差异分析详情表',0,"201427"],
            [r'heatmap.*.pdf', 'pdf', '时序差异热图',0,"201428"],
            [r'heatmap.*.png', 'png', '时序差异热图',0,"201429"],
            [r'heatmap.*.svg', 'svg', '时序差异热图',0,"201430"],
            [r'groups.*.pdf', 'pdf', '不同处理在不同时间点的表达模式图',0,"201431"],
            [r'groups.*.png', 'png', '不同处理在不同时间点的表达模式图',0,"201432"],
            [r'groups.*.svg', 'svg', '不同处理在不同时间点的表达模式图',0,"201433"],
            [r'profile.*.pdf', 'pdf', '聚类指定基因在所有样本中的表达模式图',0,"201434"],
            [r'profile.*.png', 'png', '聚类指定基因在所有样本中的表达模式图',0,"201435"],
            [r'profile.*.svg', 'svg', '聚类指定基因在所有样本中的表达模式图',0,"201436"],
        ])
