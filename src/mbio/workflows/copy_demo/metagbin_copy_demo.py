...
from mbio.packages.metagbin.metagbin_copy_demo import MetagbinCopyDemo
from biocluster.core.exceptions import OptionError

class MetagbinCopyDemoWorkflow(Workflow):
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(MetagbinCopyDemoWorkflow, self).__init__(wsheet_object)
        options = [
    ...
    def run(self):
        self.start_listener()
        self.fire("start")
        copy_task = MetagbinCopyDemo(self.option("task_id"), self.option("target_task_id"), self.option("target_project_sn"),
                             self.option("target_member_id"), self.option("target_member_type"))
        copy_task.run()
        self.end()

    def end(self):
        super(MetagbinCopyDemoWorkflow, self).end()