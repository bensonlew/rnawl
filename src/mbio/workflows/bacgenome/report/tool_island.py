...
class ToolIslandWorkflow(Workflow):
    """
    细菌基因组打通的基因组岛圈图
    """
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(ToolIslandWorkflow, self).__init__(wsheet_object)
        options = [
    ...
    def run(self):
        ...
        super(ToolIslandWorkflow, self).run()
    ...
    def set_output(self):
        ...
        self.download_file()
        self.table = self.common_api.get_table_file(self.work_dir, self.option("task_id"), self.option("specimen_id"), "island")
        with open(self.table) as f, open(self.work_dir+"/"+self.option('specimen_id_new') + ".island.xls", "w") as g:
            g.write("Location\tElement ID\tStart\tEnd\tLength\n")
            lines =f.readlines()
            for line in lines[1:]:
                lin =line.strip().split("\t")
                g.write("{}\t{}\t{}\t{}\t{}\n".format(lin[0],lin[1],lin[2],lin[3],lin[4]))
        self.logger.info("开始设置结果文件目录！")
        link_file(self.work_dir+"/"+self.option('specimen_id_new') + ".island.xls", self.output_dir+"/"+self.option('specimen_id_new') + ".island.xls")
        ...
        self.remot2 = self.file_path + '/' + self.option('specimen_id_new') + ".island.xls"
    ...
    def set_db(self):
        ...
        self.common_api.update_data(self.option("main_id"), "sg_tool_lab_island", dict)
    ...
    def end(self):
        ...
        super(ToolIslandWorkflow, self).end()