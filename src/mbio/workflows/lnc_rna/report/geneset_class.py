...
from mbio.packages.lnc_rna.geneset.kegg_regulate import KeggRegulate
...
class GenesetClassWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            {"name": "kegg_table", "type": "infile", "format": "lnc_rna.kegg_table"},
        ...
        if self.option("anno_type") == "cog":
            self._sheet.output = self._sheet.output.replace('interaction_results',
                                                            'interaction_results/04 GeneSet/02 COG_Annotation')
        elif self.option("anno_type") == "go":
            self._sheet.output = self._sheet.output.replace('interaction_results',
                                                            'interaction_results/04 GeneSet/03 GO_Annotation')
        self.inter_dirs = []
    ...
    def end(self):
        ...
        if self.option("anno_type") == "go":
            self.inter_dirs = [
                ...
                ["04 GeneSet/03 GO_Annotation", "", "GO功能注释", 0],
            ]
            result_dir.add_relpath_rules([
                [".", "", "GO功能注释文件", 0],
                ['./go_class_table.xls', 'xls', 'GO分类统计表',0],
                ['run_parameter.txt', 'txt', '运行参数日志', 0]
            ])
        elif self.option("anno_type") == "cog":
            self.inter_dirs = [
                ["04 GeneSet", "", "基因集分析结果目录", 0],
                ["04 GeneSet/02 COG_Annotation", "", "COG功能注释", 0],
            ]
            result_dir.add_relpath_rules([
                [".", "", "COG功能注释文件", 0],
                ['./cog_class_table.xls', 'xls', 'COG分类统计表', 0],
                ['run_parameter.txt', 'txt', '运行参数日志', 0]
            ])
