...
class RmatsStatWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self._sheet.output = self._sheet.output.replace('interaction_results',
                                                        'interaction_results/05 AS/AS_diff_stats')
        self.inter_dirs = []
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("lnc_rna", table="sg_splicing_rmats_stats", main_id=self.option('main_id'),
                                dir_path=self.work_dir)
        self.run_log = get_run_log.run()

    def run_rmats_stat(self):
        self.step.add_steps('rmats_stat')
        self.rmats_stat = self.add_tool('lnc_rna.structure.rmats_stat')
    ...
    def set_output(self, event):
        ...
        for basename in os.listdir(self.rmats_stat.output_dir):
            source = os.path.join(self.rmats_stat.output_dir, basename)
    ...
    def set_db(self):
        ...
        self.database = self.api.api('lnc_rna.rmats_stat')
        self.database.add_rmats_stat_detail(
    ...
    def end(self):
        ...
        if os.path.exists(self.output_dir + '/upload'):
            shutil.rmtree(self.output_dir + '/upload')
        os.mkdir(self.output_dir + '/upload')
        up_files = glob.glob(self.output_dir + '/*splicing*stats.xls')
        for each in up_files:
            shutil.copyfile(each, os.path.join(self.output_dir, 'upload', os.path.basename(each)))
        result_dir = self.add_upload_dir(self.output_dir+'/upload')
        self.inter_dirs = [
            ["05 AS", "", "可变剪切分析结果目录", 0],
            ["05 AS/AS_diff_stats", "", "差异可变剪切统计文件", 0],
        ]
        relpath = [
            ['.', '', '差异可变剪切事件统计', 0],
