...
class WgcnaNetworkWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            dict(name="step3output", type="infile", format='lnc_rna.common_dir'),
            dict(name="step2output", type="infile", format='lnc_rna.common_dir'),
            # to update sg_status
            {"name": "update_info", "type": "string"},
        ]
        self.add_option(options)
        self.set_options(self._sheet.options())
        self.tool = self.add_tool("lnc_rna.wgcna.wgcna_network")
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("lnc_rna", table="sg_wgcna_network", main_id=self.option('main_id'),
    ...
    def end(self):
        ...
        files = glob.glob(self.tool.output_dir + '/network.*')
        for each in files:
            os.rename(each, self.tool.output_dir+'/'+self.option('module')+'_'+os.path.basename(each))
        ...
        result_dir.add_regexp_rules([
            [r'.*network\.nodes\.xls', 'xls', '该模块网络图节点列表', 0],
            [r'.*network\.xls', 'xls', '该模块网络图数据', 0],
            [r'.*network\.edges\.xls', 'xls', '该模块网络图边列表', 0],
        ])
