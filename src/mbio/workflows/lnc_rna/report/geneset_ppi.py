...
class GenesetPpiWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        options = [
            {"name": "geneset_list", "type": "infile", "format": "lnc_rna.common"},  # 基因列表
            {"name": "species", "type": "int", "default": 9606},
            {"name": "combine_score", "type": "int", "default": 1000},  # 设定蛋白质间的相互作用可能性值前300个互作组
            {"name": "update_info", "type": "string"},
            {"name": "geneset_id", "type": "string"},
            {"name": "seq", "type": "infile", "format": "lnc_rna.common"},
            {"name": "type", "type":"string"},
            {"name": "t2g_dir", "type": "infile", "format": "lnc_rna.common"},
            {"name": "ppi_id", "type": "string"}
        ]
        self.add_option(options)
        self.set_options(self._sheet.options())
        self.ppinetwork = self.add_module('lnc_rna.ppinetwork_analysis')
        if self.option('type') == 'G':
            self.set_options({'seq': self.trans_t2g()})
        self._sheet.output = self._sheet.output.replace('interaction_results',
                                                        'interaction_results/07 Advanced_Analysis/04 PPI_Analysis')
        self.inter_dirs = []
    ...
    def end(self):
        ...
        result_dir.add_relpath_rules([
            [".", "", "蛋白互作网络分析", 0],
            ["./map", "", "与String数据库比对结果文件", 0],
            ["./ppinetwork_predict", "", "蛋白质相互作用组预测文件目录", 0, "211083"],
            ["./ppinetwork_topology", "", "蛋白质互作网络拓扑属性文件目录", 0, "211084",],
            ["./map/diff_exp_mapped.txt", "txt", "含有STRINGid的差异基因列表", 0, "211085"],
            ['./map/seq_mapped.txt', 'txt', '比对到数据库的序列信息表',0],
            ['./map/unmapped_seq.txt', 'txt', '未比对到数据库用的序列信息表', 0],
    ...
    def set_db(self):
        ...
        报存分析结果到mongo数据库中
        """
        api_ppinetwork = self.api.api('lnc_rna.geneset_ppi')
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("lnc_rna", table="sg_geneset_ppi", main_id=self.option('ppi_id'),
