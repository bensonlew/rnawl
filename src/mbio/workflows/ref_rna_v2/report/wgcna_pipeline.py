...
class WgcnaPipelineWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self.dump_tool = self.api.api("ref_rna_v2.wgcna")
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("ref_rna_v2", table="sg_wgcna_pipeline", main_id=self.option('main_id'),
    ...
    def end(self):
        def make_dir(path):
            if os.path.exists(path):
                shutil.rmtree(path)
            os.mkdir(path)
        make_dir(self.output_dir + '/' + 'wgcna_prepare')
        make_dir(self.output_dir + '/' + 'wgcna_module')
        make_dir(self.output_dir + '/' + 'wgcna_relate')
        make_dir(self.output_dir + '/' + 'wgcna_network')
