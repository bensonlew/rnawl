...
class TfPredictWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self.tf_predict2 = self.add_tool("rna.tf_predict2")
        self.dump_tool = self.api.api("ref_rna_v2.tf_predict")
    ...
    def run(self):
        ...
        if self.option("s") == "jaspar":
            self.tf_predict2.on("end", self.set_db_jasar)
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("ref_rna_v2", table="sg_tf_predict", main_id=self.option('main_id'),
    ...
    def set_db(self):
        ...
        tf_result_file_path = os.path.join(self.tf_predict.output_dir,"final_tf_predict.xls")
        if not os.path.exists(tf_result_file_path) :
            self.set_error(u"本次预测无结果,请更换参数重新投递")
