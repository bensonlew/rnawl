...
class QuantWorkflow(Workflow):
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            dict(name="t2g", type="infile", format="ref_rna_v2.common"),
        ...
        self.tool = self.add_module("ref_rna_v2.quant")
        self._sheet.output = self._sheet.output.replace('interaction_results', 'interaction_results/01 Express/01 Exp_Analysis')
        self.inter_dirs = []
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("ref_rna_v2", table="sg_exp", main_id=self.option('gene_main_id'),
    ...
    def update_main_document(self):
        database = Config().get_mongo_client(mtype='ref_rna_v2')[Config().get_mongo_dbname('ref_rna_v2')]
        collection = database['sg_exp']
        collection.update({'main_id': ObjectId(self.option('gene_main_id'))},
                          {'$set': {'count_file': self.gene_count_file}}, upsert=True)
        if self.option('transcript_main_id') is not None:
            collection.update({'main_id': ObjectId(self.option('transcript_main_id'))},
                              {'$set': {'count_file': self.transcript_count_file}}, upsert=True)
    ...
    def end(self):
        ...
        result_dir.add_relpath_rules([
            [".", "", "表达量分析文件", 0, "211576"],
            ['./gene.fpkm.matrix.xls', '', '基因fpkm表达定量结果表', 0, "211577"],
            ['./gene.fpkm.matrix.annot.xls', '', '基因fpkm表达定量注释结果表', 0, "211578"],
            ['./transcript.fpkm.matrix.xls', '', '转录本fpkm表达定量结果表', 0, "211579"],
            ['./transcript.fpkm.matrix.annot.xls', '', '转录本fpkm表达定量注释结果表', 0, "211580"],
            ['./gene.tpm.matrix.annot.xls', '', '基因tpm表达定量注释结果表', 0, "211581"],
            ['./transcript.tpm.matrix.annot.xls', '', '转录本tpm表达定量注释结果表', 0, "211582"],
            ['./gene.tpm.matrix.xls', '', '基因tpm表达定量结果表', 0, "211583"],
            ['./transcript.tpm.matrix.xls', '', '转录本tpm表达定量结果表', 0, "211584"],
    ...
    #         download(path, to_path)
    #     else:
    #         self.set_error('file can not find', code="13702107")
    #     return to_path

    def dump_fpkm(self, task_id, transcript_main_id, gene_main_id):
        all_exp = self.api.api("ref_rna_v2.all_exp")
        collection = all_exp.db['sg_exp']
        results = collection.find({"task_id": task_id, "method": "RSEM", 'status': 'end'})
        ...
        for each in results:
            if each["params"]:
                if each['exp_level'] == 'T' and 'count_file' in each and 'batch_method' not in each:
                    self.transcript_count_file = each['count_file']
                    transcript_fpkm_file = each['count_file'].replace('.count.matrix', '.fpkm.matrix')
                    transcript_fpkm_file = self.download_s3_file(transcript_fpkm_file,
                                                                 os.path.basename(transcript_fpkm_file))
                    ...
                    if exists(transcript_fpkm_anno_file):
                        transcript_fpkm_anno_file = self.download_s3_file(transcript_fpkm_anno_file,
                                                                          os.path.basename(transcript_fpkm_anno_file))
                    else:
                        transcript_fpkm_anno_file = None
                if each['exp_level'] == 'G' and 'count_file' in each and 'batch_method' not in each:
                    self.gene_count_file = each['count_file']
                    ...
                    if exists(gene_fpkm_anno_file):
                        gene_fpkm_anno_file = self.download_s3_file(gene_fpkm_anno_file,
                                                                    os.path.basename(gene_fpkm_anno_file))
                    else:
                        gene_fpkm_anno_file = None
        ...
        if transcript_fpkm_file is not None:
            all_exp.add_exp(transcript_fpkm_file, quant_method="RSEM", exp_level='T',
                            main_id=transcript_main_id, exp_type='fpkm', add_distribution=False,
                            task_id=self.option('raw_task_id'))
        else:
            pass
        ...
        if gene_fpkm_file is not None:
            all_exp.add_exp(gene_fpkm_file, quant_method="RSEM", exp_level='G',
                            main_id=gene_main_id, exp_type='fpkm', add_distribution=False,
                            task_id=self.option('raw_task_id'))
        else:
        ...
        elif transcript_fpkm_file is not None:
    ...
    def dump_tpm(self, task_id, transcript_main_id, gene_main_id):
        ...
        if transcript_fpkm_file is not None:
            all_exp.add_exp(transcript_fpkm_file, quant_method="RSEM", exp_level='T',
                            main_id=transcript_main_id, exp_type='TPM', add_distribution=False,
                            task_id=self.option('raw_task_id'))
        else:
            self.set_error('transcript_tpm/count_file is not found', code="13702103")
        if gene_fpkm_file is not None:
            all_exp.add_exp(gene_fpkm_file, quant_method="RSEM", exp_level='G',
                            main_id=gene_main_id, exp_type='TPM', add_distribution=False,
                            task_id=self.option('raw_task_id'))
        else:
