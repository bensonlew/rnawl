...
class GenesetKeggWorkflow(Workflow):
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("ref_rna_v2", table="sg_geneset_kegg_class", main_id=self.option('main_table_id'),
    ...
    def end(self):
        ...
        statis = pd.read_table(self.work_dir + "/kegg_statistic", sep="\t", header=0)
        statis_new = statis.drop(['kegg_id', 'geneset_type', 'geneset_id'], axis=1)
        statis_new.to_csv(self.output_dir + "/kegg_statistics", sep='\t', index=False)
        # shutil.copyfile(self.work_dir + "/kegg_statistics", self.kegg_class.output_dir + "/kegg_statistics.xls")

        ## set output

        linkdir(self.kegg_class.output_dir + '/pathways', self.output_dir + '/kegg_pathways')
        mark_file = glob.glob(self.output_dir + "/kegg_pathways/*.html.mark")
        for file in mark_file:
            os.remove(file)
