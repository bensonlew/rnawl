...
class ExpPcaWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self.tool = self.add_tool("ref_rna_v2.exp_pca")
        self.ellipse = None
        ...
        try:
            ...
            self.rerun = False
        if self.rerun:
            interactiondelete =  InteractionDelete(bind_object=self,project_type="ref_rna_v2",main_id=self.option('pca_main_id'))
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("ref_rna_v2", table="sg_exp_pca", main_id=self.option('pca_main_id'),
    ...
    def chart(self):
        ...
        for file in pdf_files:
            if os.path.basename(file).endswith("ell.scatter.pdf"):
                if os.path.exists(os.path.join(self.tool.output_dir,"sample_pca_ellipse.pdf")):
                    os.remove(os.path.join(self.tool.output_dir,"sample_pca_ellipse.pdf"))
                os.link(file, os.path.join(self.tool.output_dir,"sample_pca_ellipse.pdf"))
            else:
                if os.path.exists(os.path.join(self.tool.output_dir, "sample_pca.pdf")):
                    os.remove(os.path.join(self.tool.output_dir, "sample_pca.pdf"))
                os.link(file, os.path.join(self.tool.output_dir, "sample_pca.pdf"))
