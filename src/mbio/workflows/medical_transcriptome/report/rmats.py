...
import os
import shutil
# from mbio.packages.medical_transcriptome.upload import get_gid2des_dct, export_rmats_detail, export_rmats_diff_stats, export_rmats_psi
...
class RmatsWorkflow(Workflow):
    def __init__(self, wsheet_object):
        ...
        self._sheet.output = self._sheet.output.replace('interaction_results', 'interaction_results/03 Gene_structure_analysis/01 AS/01 AS_diff')
        self.inter_dirs = []
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("medical_transcriptome", table="sg_splicing_rmats", main_id=self.option('splicing_id'),
    ...
    def run_rmats(self):
        ...
        self.rmats = self.add_module('medical_transcriptome.rmats.rmats')
    ...
    def chart(self):
        ...
        splice_diff = glob.glob(self.output_dir + "/*/event_stats.file.txt")[0]
        splice_psi =  glob.glob(self.output_dir + "/*/psi_stats.file.txt")[0]
        num, vs_list = self.option('control_table').get_control_info()
        ctrl, test = vs_list[0]
        cmp_name = "{}_vs_{}".format(ctrl,test)
        chart.chart_splice_diff_stat(splice_diff, splice_psi, cmp_name=cmp_name)
        chart.to_pdf()

        pdf_file = glob.glob(self.work_dir + "/*.pdf")
        for p in pdf_file:
            # os.link(p, self.output_dir +"/" + os.path.basename(p))
            os.link(p, self.target_dir + "/" + os.path.basename(p))
    ...
    def end(self):
        ...
        self.inter_dirs = [
            ...
            ["03 Gene_structure_analysis/01 AS/01 AS_diff", "", "差异可变剪切分析文件", 0]
        ]
...
class TestFunction(unittest.TestCase):
    ...
    '''
    def test(self):
        from mbio.workflows.medical_transcriptome.report.rmats import RmatsWorkflow
        from biocluster.wsheet import Sheet
        import random
        data = {
            'id': 'workflow_{}_{}'.format(random.randint(1000, 9999), random.randint(1000, 9999)),
            'type': 'workflow',
            'name': 'medical_transcriptome.report.rmats',
            'options': {
                ...
                's3_file_list': '/mnt/ilustre/users/sanger-dev/workspace/20200812/Rmats_tsg_38314_3629_4004/bam.list',
                'control_table': '/mnt/ilustre/users/sanger-dev/workspace/20200812/Rmats_tsg_38314_3629_4004/control.txt',
                'group_table': '/mnt/ilustre/users/sanger-dev/workspace/20200812/Rmats_tsg_38314_3629_4004/group.txt',
                'ref_gtf': '/mnt/ilustre/users/sanger-dev/workspace/20200812/Rmats_tsg_38314_3629_4004/remote_input/ref_gtf/all_transcripts.gtf',
                'seq_type': 'paired',
                'lib_type': 'fr-unstranded',
                'splicing_id':"5f33aaeb17b2bf7043eec5d7",
            }
        }
