...
class DiffGenesetEnrichWorkflow(Workflow):
    '''
    last_modify: 2019.06.12
    '''
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(DiffGenesetEnrichWorkflow, self).__init__(wsheet_object)
        options = [
    ...
    def send_log(self, data):
        ...
        super(DiffGenesetEnrichWorkflow, self).send_log(data)
    ...
    def run(self):
        ...
        self.get_run_log()
        self.run_tool()
        super(DiffGenesetEnrichWorkflow, self).run()

    def get_run_log(self):
        if self.option('anno_type') == 'go':
            table = "sg_diff_geneset_go_enrich"
            get_run_log = GetRunLog("medical_transcriptome", table=table, main_id=self.option('main_id'),
                                    dir_path=self.work_dir)
        elif self.option('anno_type') == 'kegg':
            table = "sg_diff_geneset_kegg_enrich"
    ...
    def set_db(self):
        if self.option('anno_type') == 'go':
            api = self.api.api('medical_transcriptome.diff_geneset')
            ...
            api_geneset.update_db_record('sg_diff_geneset_kegg_enrich', self.option('main_table_id'), result_dir= workflow_output)
            ...
            api_geneset.update_db_record('sg_diff_geneset_kegg_enrich', self.option('main_table_id'), graph_dir=graph_dir)
    ...
    def end(self):
        ...
        super(DiffGenesetEnrichWorkflow, self).end()
