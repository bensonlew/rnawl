...
class GenesetCircWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            {"name": "diff_fc", "type": "infile", "format": "medical_transcriptome.common"},
        ...
        self._sheet.output = self._sheet.output.replace('interaction_results', 'interaction_results/04 GeneSet/03 Enrich/06 Enrich_Circ')
        self.inter_dirs = []
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("medical_transcriptome", table="sg_geneset_circ", main_id=self.option('main_table_id'),
    ...
    def end(self):
        ...
        for file in rm_file:
            os.remove(file)
        detail = os.path.join(self.circ.output_dir, '{}_enrich_detail.table'.format(self.option("enrich_type").lower()))
        new = os.path.join(self.circ.output_dir, '{}_enrich_detail.xls'.format(self.option("enrich_type").lower()))
        os.rename(detail, new)
        ...
        self.inter_dirs = [
            ...
            ["04 GeneSet/03 Enrich/06 Enrich_Circ", "", "基因集富集弦图", 0]
        ]
        ...
        result_dir.add_regexp_rules([
            [r'(go|do|disgenet|kegg|reactome)_enrich_detail.xls', "xls", "富集弦图数据表", 0]
        ])
