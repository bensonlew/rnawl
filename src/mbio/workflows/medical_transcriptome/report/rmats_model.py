...
class RmatsModelWorkflow(Workflow):
    def __init__(self, wsheet_object):
        ...
        self._sheet.output = self._sheet.output.replace('interaction_results', 'interaction_results/03 Gene_structure_analysis/01 AS/04 AS_diff_model')
        self.inter_dirs = []
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("medical_transcriptome", table="sg_splicing_rmats_model", main_id=self.option('main_id'),
    ...
    def run_rmats_model(self):
        ...
        self.rmats_model = self.add_tool('medical_transcriptome.structure.rmats_model')
    ...
    @workfuncdeco
    def set_db(self):
        self.database = self.api.api('medical_transcriptome.rmats_model')
    ...
    def end(self):
        ...
        self.inter_dirs = [
            ...
            ["03 Gene_structure_analysis/01 AS/04 AS_diff_model", "", "差异可变剪切事件模式图文件", 0]
        ]
        relpath = [
        ...
        result_dir.add_regexp_rules([
            [r".*\.pdf", 'pdf', "差异可变剪接事件模式图 ", 0],
            [r".*\.png", 'png', "差异可变剪接事件模式图 ", 0]
        ])
