...
class GenesetClusterWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            dict(name="geneset_name", type="string"),
            # to update sg_status
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("medical_transcriptome", table="sg_geneset_cluster", main_id=self.option('cluster_main_id'),
    ...
    def end(self):
        ...
        # os.rename(os.path.join(self.tool.output_dir, 'expression_matrix_annot.xls'),
        #           os.path.join(self.tool.output_dir, 'expression_matrix.xls'))

        df = pd.read_table(os.path.join(self.tool.output_dir, 'expression_matrix_annot.xls'))
        df = df.drop_duplicates()
        df.to_csv(os.path.join(self.tool.output_dir, 'expression_matrix.xls'), sep='\t', index=False)

        rm_files = glob.glob(self.tool.output_dir + '/sample*cluster*tree.txt')
