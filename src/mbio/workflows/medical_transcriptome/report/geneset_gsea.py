...
class GenesetGseaWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            {'name': 'do_genesets', 'type': 'string', 'default': 'all'},
        ...
        self._sheet.output = self._sheet.output.replace('interaction_results', 'interaction_results/05 Advanced_Analysis/03 GSEA_Analysis')
        self.inter_dirs = []
        ...
        if self.option("do_list").is_set:
            self.do_gene_gmx = self.add_tool('medical_transcriptome.geneset.do_gene_gmx')
        if self.option('preranked'):
            self.gsea_tool = self.add_tool('medical_transcriptome.geneset.gsea_preranked')
        else:
            self.gsea_tool = self.add_tool('medical_transcriptome.geneset.gsea')
    ...
    def get_do_gmx(self):
        opts = {
            ...
            "min_num": self.option("min_num"),
            "max_num": self.option("max_num"),
            "do_sets": self.option("do_genesets")
        }
        self.do_gene_gmx.set_options(opts)
        self.do_gene_gmx.run()

    def gsea_runner(self):
        # 输出: biomart.xls, biomart.json
        if self.option("go_list").is_set:
            gmx = self.go_gene_gmx.output_dir + '/go.gmt'
        elif self.option("do_list").is_set:
            gmx = self.do_gene_gmx.output_dir + '/do.gmt'
        else:
            gmx =self.option('gmx')
    ...
    def run(self):
        ...
        if self.option("preranked"):
            if self.option("go_list").is_set:
                ...
                self.do_gene_gmx.on('end', self.gsea_preranked_runner)
                self.get_do_gmx()
            else:
            ...
            if self.option("go_list").is_set:
                ...
                self.do_gene_gmx.on('end', self.gsea_runner)
                self.get_do_gmx()
            else:
                self.gsea_runner()
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("medical_transcriptome", table="sg_geneset_gsea", main_id=self.option('main_table_id'),
    ...
    def set_db(self):
        ...
        保存结果指数表到mongo数据库中
        """
        api_geneset = self.api.api('medical_transcriptome.gsea')
    ...
    def end(self):
        ...
        self.inter_dirs = [
            ...
            ["05 Advanced_Analysis/03 GSEA_Analysis", "", "GSEA分析结果目录", 0]
        ]
