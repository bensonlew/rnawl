...
class GenesetKeggWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self._sheet.output = self._sheet.output.replace('interaction_results', 'interaction_results/04 GeneSet/02 Annotation/02 KEGG')
        self.inter_dirs = []
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("medical_transcriptome", table="sg_geneset_kegg_class", main_id=self.option('main_table_id'),
    ...
    def set_db(self):
        ...
        if results_num>1:
            ...
            conn.update({"_id": record_id}, {"$set": {'graph_dir': graph_dir,'status':'end'}}, upsert=True)
        else:
            record_id = self.option("main_table_id")
            if isinstance(record_id, types.StringTypes):
                record_id = ObjectId(record_id)
                ...
                record_id = record_id
            conn = api_geneset.db["sg_geneset_kegg_class"]
            conn.update({"_id": record_id}, {"$set": {'has_results': False, 'status': 'end'}}, upsert=True)
        self.end()
    ...
    def chart(self):
        ...
        if self.has_results:
            ...
            for p in pdf_file:
                print "copy", p, self.output_dir + "/" + os.path.basename(p)
    ...
    def end(self):
        ...
        if self.has_results:
            needed_add_genename_file = self.work_dir + "/kegg_analysis_of_anotate"
            add_name = GeneInfoSupple(task_id=self.option("task_id"), level=self.option("geneset_type"))
            try:
                geneid_list = [i for i in open(needed_add_genename_file).readline().strip().split('\t') if "_genes" in i]
                add_name.add_gene_name(needed_add_genename_file, ',', "kegg", geneid_list, needed_add_genename_file+'_addgenename')
                needed_add_genename_file__AddGenename = needed_add_genename_file+'_addgenename'
                print "geneid_list:    "+str(geneid_list)
            except Exception as e:
                self.logger.info("添加基因name 失败 {}".format(e))
                needed_add_genename_file__AddGenename = needed_add_genename_file

            # shutil.rmtree(self.kegg_class.output_dir + "/ko")
            # os.remove(self.output_dir + "/kegg_stat.xls")

            shutil.copyfile(needed_add_genename_file__AddGenename,
                            self.output_dir + "/kegg_analysis_detail.xls")
            ...
            shutil.copyfile(self.work_dir + "/kegg_statistics", self.output_dir + "/kegg_statistics.xls")
            ...
            #     if os.path.exists(self.kegg_class.output_dir + '/pathways'):
            #         shutil.rmtree(self.kegg_class.output_dir + '/pathways')
            if os.path.exists(self.kegg_class.output_dir + '/pathways.tar.gz'):
                linkfile(os.path.join(self.kegg_class.output_dir,'pathways.tar.gz'),os.path.join(self.output_dir,'pathways.tar.gz'))
        ...
        self.inter_dirs = [
            ...
            ["04 GeneSet/02 Annotation/02 KEGG", "", "基因集KEGG功能注释", 0]
        ]
        result_dir.add_relpath_rules([
            [".", "", "基因集KEGG功能注释文件",0],
            ["kegg_pathways", "", "基因集KEGG通路图",0],
            ["kegg_analysis_detail.xls", "", "基因集Pathway分类统计表",0],
            ["kegg_statistics.xls", "", "基因集KEGG分类统计表",0],
            ['./run_parameter.txt', 'txt', '运行参数日志', 0],
            ["*.pdf", "pdf", "KEGG分类统计图",0],
        ])
        result_dir.add_regexp_rules([
            [r'kegg_pathways/.*\.html', '', 'KEGG通路html文件', 0, "211558"],
            [r'kegg_pathways/.*\.png', '', 'KEGG通路图片png', 0, "211559"],
        ])
