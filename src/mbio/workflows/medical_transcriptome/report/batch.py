...
class BatchWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            {'name': 'exp_all', 'type': 'infile', 'format': 'ref_rna_v2.common'},
            {'name': 'exp_level', 'type': 'string', 'default': 'T'},
        ...
        self._sheet.output = self._sheet.output.replace('interaction_results', 'interaction_results/02 Annotation_Express/03 Exp_Batch')
        self.inter_dirs = []
        self.project_type = 'medical_transcriptome'
    ...
    def get_run_log(self):
        get_run_log = GetRunLog("medical_transcriptome", table="sg_exp_batch", main_id=self.option('sg_exp_batch_id'),
                                dir_path=self.work_dir)
        self.run_log = get_run_log.run()

    def run_batch(self):
        self.batch = self.add_tool('medical_transcriptome.batch.batch')
    ...
    def run_SVA(self):
        self.SVA = self.add_tool('medical_transcriptome.batch.sva')
    ...
    def run_exp_pca(self):
        self.exp_batch_pca = self.add_tool("medical_transcriptome.exp_pca")
        if self.option('has_batch') == True:
    ...
    def run_exp_corr(self):
        self.exp_batch_corr = self.add_tool("medical_transcriptome.exp_corr")
        if self.option('has_batch') == True:
    ...
    def chart(self):
        ...
        os.link(pdf_file, self.output_dir + "/sample_correlation.pdf")
    ...
    def set_db(self):
         ...
         保存结果标准化数据到mongo数据库中
         """
         s3_result = self._sheet.output + '/' + 'count_batch.txt'
         record_id = self.option('main_id_exp')
         exp_id = self.option('exp_id')
         sg_exp_batch_id = self.option('sg_exp_batch_id')
         exp_batch = self.api.api('medical_transcriptome.batch')
         exp_batch.add_exp_batch(self.option('count_batch').path, self.option('exp_all').path, self.option('exp_level'),
                                 record_id, exp_id,self.option('sg_exp_batch_id'), self.option('batch_version'),
                                 self.option('task_id'), self.option('params'), s3_result)



         # add result info
         exp_batch.add_exp_batch_pca(self.exp_batch_pca.output_dir, self.option('batch_version'), self.option('task_id'), self.option('exp_level'), main_id=self.option('sg_exp_batch_id'), record_id=record_id)
         if self.option('ellipse') == 'yes':
             exp_batch.insert_ellipse_table(self.ellipse.work_dir + '/ellipse_out.xls', self.option('batch_version'), self.option('task_id'), self.option('exp_level'), main_id=self.option('sg_exp_batch_id'), record_id=record_id)
         if self.option('ellipse') == 'no':
             pass
         exp_batch.add_exp_batch_corr(self.exp_batch_corr.work_dir, self.option('batch_version'), self.option('task_id'), self.option('exp_level'), main_id=self.option('sg_exp_batch_id'), record_id=record_id)
         self.end()
    ...
    def end(self):
        ...
        self.inter_dirs = [
            ...
            ["02 Annotation_Express/03 Exp_Batch", "", "批次效应评估", 0]
        ]
