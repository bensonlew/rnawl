...
class ToolDiffPlotWorkflow(Workflow):
    """
    基因集venn图
    """
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(ToolDiffPlotWorkflow, self).__init__(wsheet_object)
        options = [
    ...
    def end(self):
        ...
        super(ToolDiffPlotWorkflow, self).end()

    def set_db(self):
        diff_plot = self.api.api("denovo_rna_v3.tool_lab_api")
        self.s3_path = '{}/{}'.format(self._sheet.output, 'diff_plot.txt')
        diff_plot.add_diff_plot(self.option('main_id'))
        self.set_output()

    def set_output(self):
        diff_plot_path = os.path.join(self.work_dir, 'diff_plot.txt')
        if os.path.exists(os.path.join(self.output_dir, 'diff_plot.txt')):
            os.remove(os.path.join(self.output_dir, 'diff_plot.txt'))
        os.link(diff_plot_path, os.path.join(self.output_dir, 'diff_plot.txt'))
        self.end()
