...
class NiptAnalysisWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        self.niptanalysis = self.add_tool('medical.nipt_v2.bed_analysis')
    ...
    def run_niptanalysis(self):
        ...
        bed_file = self.api.api('medical.nipt_a'
                                'nalysis_v2').export_bed_file(sample=self.option("bed_file"), dir_path=self.work_dir)
        options = {
            'bed_file': bed_file,
            'bw': self.option('bw'),
            'bs': self.option('bs'),
    ...
    def set_db(self):
        ...
        报存分析结果到mongo数据库中
        """
        api_niptanalysis = self.api.api('medical.nipt_analysis_v2')
