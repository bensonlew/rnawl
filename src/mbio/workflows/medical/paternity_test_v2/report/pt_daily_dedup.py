...
class PtDailyDedupWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            {"name": "err_min", "type": "int", "default": 11},
    ...
    def get_ref_list(self):
        ...
        for m in ref_dad:
            self.ref_list.append(m.strip().split('.')[0])
        self.ref_set = set(self.ref_list)
    ...
    def get_dad_no_analysis(self, mom_id, preg_id):
        ...
        :return:
        """
        father_dedup_id = self.api_pt.find_dedup_err_id(2, mom_id, preg_id)
        if father_dedup_id:
            dad_list_ = self.api_pt.find_dedup_father_id(father_dedup_id)
            analysed = set(dad_list_)
            not_analysed = self.ref_set - analysed
            dad_list = list(not_analysed)
        else:
            dad_list = self.ref_list
            self.logger.info("库中未查找到该M-S的查重结果，后面将进行全库查重分析！")
        self.logger.info(dad_list)
        return dad_list
    ...
    def dedup_run(self):
        ...
        for m_s_id in self.ms_ids:
            ...
            if not dad_list:
                ...
                continue
            pt_daily_dedup = self.add_module("medical.paternity_test_v2.daily_dedup")
        ...
        if self.modules_dedup:
            ...
            self.exit(0, "没有需要查重的M-S,正常退出", True)
