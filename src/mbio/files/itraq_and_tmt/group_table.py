    ...
    def parse_file(self):
        ...
        with open(self.prop['path']) as f:
            ...
            if not header_line.startswith('#'):
                raise FileError("该group文件不含表头，group表第一列应该以#号开头", code = "42501201")
            header_list = header_line.lstrip('#').rstrip().split()[1:]
            for line in f:
                if not line.strip():
                    continue
                tmp_list = line.strip().split()
                if tmp_list[0] in sample_list:
                    raise FileError("样本{}在分组方案中重复出现，请核查！".format(tmp_list[0]))
        ...
        if not group_dict:
            raise FileError("分组信息内容为空", code = "42501202")
