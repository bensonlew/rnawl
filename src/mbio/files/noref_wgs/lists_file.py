...
class ListsFileFile(File):

    def __init__(self):
        super(ListsFileFile, self).__init__()

    def check(self):
        if super(ListsFileFile, self).check():
            if not os.path.exists(self.prop["path"]):
                raise FileError("%s文件不存在" , variables=( self.prop["path"]), code="45500403")
            # else:
            #     self.check_len()
            return True
