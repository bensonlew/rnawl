...
class BlastXmlFile(File):
    ...
    def get_xml_info(self):
        ...
        try:
            ...
            raise FileError('传入文件类型不正确，无法解析，请检查文件是否正确，或者生成文件的blast版本不正确，本程序测试版本blast+2.3.0', code="41100501")
        except Exception as e:
            raise FileError('未知原因导致blastxml文件解析失败:%s', variables=(e), code="41100502")
    ...
    def xml_to_xml(self,outfile):
        """
        调用packages中的extract_xml方法将一个基因多条记录转换成一个基因只有一条最佳记录的xml文件
        """
        from mbio.packages.align.blast.xml2table import extract_xml
        extract_xml(self.path,outfile)
