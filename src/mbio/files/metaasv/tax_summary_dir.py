...
class TaxSummaryDirFile(Directory):
    ...
    def get_table(self, level, full_path=False):
        ...
        for file_ in list_:
            if full_path:
                pattern = r"ASV_taxon_" + level + r"\.full\.xls"
                if re.search(pattern, file_, re.IGNORECASE):
                    return os.path.join(self.prop['path'], file_)
            else:
                pattern = r"ASV_taxon_" + level + r"\.xls"
                if re.search(pattern, file_, re.IGNORECASE):
                    return os.path.join(self.prop['path'], file_)
        raise ValueError("未找到文件，输入的level为:{}，正确的level应该为['Domain', 'Kingdom', 'Phylum', 'Order', 'Family', 'Genus','Class' , 'Species', 'asv']中的一个".format(level))
    ...
    def get_biom(self, level):
        ...
        list_ = os.listdir(self.prop['path'])
        for file_ in list_:
            pattern = r"ASV_taxon_" + level + r"\.biom"
            if re.search(pattern, file_, re.IGNORECASE):
                return os.path.join(self.prop['path'], file_)
