            ...
            for line in f:
                ...
                if l_str.startswith('<BlastOutput_version>'):
                    if '2.3.0+' not in line:
                        raise FileError('blast 程序版本不是2.3.0版本，此处报错不是必须用2.3.0版本，而是b2gPipe程序需要版本为2.2.25，此处特殊改为2.2.25，依然可以进行blast2go，\
                            但是不代表后续版本仍然可以使用，既然blast版本修改，请重新检查', code = "43700805")
