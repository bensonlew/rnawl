...
class FastqDirFile(Directory):
    ...
    def get_info(self):
        ...
        if 'path' in self.prop.keys() and os.path.isdir(self.prop['path']):
            ...
            raise FileError("文件夹路径不正确，请设置正确的文件夹路径!", code="43703210")
    ...
    def get_fastq_number(self):
        ...
        if os.path.exists(list_txt):
            ...
            if not len(filesample.prop["file_names"]):
                raise FileError('Fastq 序列文件夹为空，请检查确认', code="43703211")
            for filename in filesample.prop["file_names"]:
                ...
                try:  ######
                    ...
                    raise FileError("file: %s, %s" , variables=(fq_path, e), code="43703212")
            ...
            if not len(filelist):
                raise FileError('Fastq 序列文件夹为空，请检查确认', code="43703213")
            for filename in filelist:
                if os.path.isdir(filename):
                    raise FileError('fastq文件夹中不应该存在文件夹：%s！！！',variables=(filename), code="43703214")
    ...
    def covert_to_fasta(self):
        ...
        if not self.has_work_dir:
            raise FileError("还未建立工作路径！", code="43703215")
        if not self.is_convert:
            ...
            if self.has_unziped:
                ...
                raise FileError('文件还没有解压', code="43703217")
            ...
            for fastq in self.prop["fastq_basename"]:
                ...
                if re.search(r'\.(fastq|fq)\.gz', fastq):
                    ...
                    try:
                        ...
                        raise FileError("解压缩文件失败!", code="43703218")
    ...
    def prepare(self):
        ...
        self.se_fastq = dict()
        self.samples = set()
        for n, line in enumerate(open(os.path.join(self.path, 'list.txt'))):
            items = line.strip().split('\t')
            if len(items) == 3:
                if items[2] == 'l':
                    self.pe1_fastq[items[1]] = os.path.join(self.path, items[0])
                if items[2] == 'r':
                    self.pe2_fastq[items[1]] = os.path.join(self.path, items[0])
            elif len(items) == 2:
                self.se_fastq[items[1]] = os.path.join(self.path, items[0])
            else:
                continue
            self.samples.add(items[1])
