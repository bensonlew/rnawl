...
class FastqDirFile(Directory):
    ...
    def get_info(self):
        ...
        if 'path' in self.prop.keys() and os.path.isdir(self.prop['path']):
            ...
            raise FileError("文件夹路径不正确，请设置正确的文件夹路径!", code="44000801")
    ...
    def extract_list(self):
        ...
        :return:
        """
        os.rename(self.prop['path'] + "/list.txt",self.prop['path']+"/list.txt_bak")
        with open(self.prop['path'] + "/list.txt","w") as t,open(self.prop['path']+"/list.txt_bak") as f:
            for i in f:
                if i.strip().split("\t")[0].endswith(".txt"):
                    pass
                else:
                    t.write(i)
        os.remove(self.prop['path']+"/list.txt_bak")
    ...
    def get_fastq_number(self):
        ...
        if os.path.exists(list_txt):
            ...
            if not len(filesample.prop["file_names"]):
                raise FileError('Fastq 序列文件夹为空，请检查确认', code="44000802")
            for filename in filesample.prop["file_names"]:
                ...
                try:  ######
                    ...
                    raise FileError("file: %s, %s" % (fq_path, e))
            ...
            if not len(filelist):
                raise FileError('Fastq 序列文件夹为空，请检查确认', code="44000803")
            for filename in filelist:
                if os.path.isdir(filename):
                    raise FileError('fastq文件夹中不应该存在文件夹：%s！！！',variables=(filename), code="44000804")
                if re.match(r'\S+(fastq|fastq.gz|fq|fq.gz)$', filename):
                    my_fastq = FastqFile()
                    fq_path = os.path.join(self.prop['path'], filename)
                    my_fastq.set_path(fq_path)
                    my_fastq.get_info()
                    ...
                    raise FileError('Fastq 序列文件夹不能包含fastq|fastq.gz|fq|fq.gz|.txt以外后缀文件，请检查确认')
    ...
    def covert_to_fasta(self):
        ...
        if not self.has_work_dir:
            raise FileError("还未建立工作路径！", code="44000805")
            ...
            if self.has_unziped:
                for fastq in self.unzip_file:
                    ...
                    if mycmd.returncode == 0:
                        ...
                        raise FileError("fastq转化fasta失败！\n%s \n%s", variables=(convert_str,res[0]),
                                        code="44000806")
            else:
                raise FileError('文件还没有解压', code="44000807")
    ...
    def unzip_fastq(self):
        ...
        """
        if not self.has_work_dir:
            self.bind_object.set_error("还未建立工作路径！", code="44000808")
            ...
            for fastq in self.prop["fastq_basename"]:
                ...
                if re.search(r'\.(fastq|fq)\.gz', fastq):
                    ...
                    try:
                        ...
                        raise FileError("解压缩文件失败!", code="44000809")
                        # raise Exception("解压缩文件失败!", code="44000805")
                else:
