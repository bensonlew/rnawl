...
class KeggTableFile(File):
    ...
    def __init__(self, kegg_version=None):
        ...
        if kegg_version == "201909":
            self.path2ko = Config().SOFTWARE_DIR + "/database/Annotation/other2019/kegg201909/pathway"
        '''
    ...
    def get_path2ko(self,  limit_ko=None):
        '''
        获取数据库pathway ko对应编号
        '''
        if limit_ko != None:
    ...
    def get_pathway_koid(self,  limit_ko=None, kegg_version=None):
        '''
        返回两个字典：
    ...
    def get_gene2path(self, outdir, limit_ko=None):
        if limit_ko != None:
            limit_kos = list()
            with open(limit_ko) as f:
                limit_kos = [line.split("\t")[0] for line in f.readlines()[1:]]
