#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
@time    : 2019/5/30 10:28
@file    : geneset_gmt.py
"""
import csv
import os
import re

from biocluster.iofile import File


class GenesetGmtFile(File):

    def check(self):
        super(GenesetGmtFile, self).check()
        with open(self.path) as in_handler:
            for line in in_handler:
                try:
                    name, _, gene_set = line.strip().split('\t', 2)
                    re.split('\s', gene_set)
                except:
                    self.set_error(
                        '基因集文件格式错误，文件应该三列: geneset_name<TAB>description<TAB>gene_list[空格或者tab分隔]')
                else:
                    return True

    def hard_link(self, new_path):
        if os.path.isdir(os.path.dirname(new_path)):
            cmd = 'ln {old} {new}'.format(old=self.path, new=new_path)
            return_code = os.system(cmd)
            if return_code not in (0, '0'):
                cmd = 'cp {old} {new}'.format(old=self.path, new=new_path)
                os.system(cmd)
        else:
            self.set_error('文件创建link失败，请确认文件路径是否正确', code="43703504")

    def basename(self):
        return os.path.basename(self.path)

    def csv_reader(self, remove_header=False):
        with self.get_reader() as in_handler:
            if remove_header:
                in_handler.readline()
            for item in csv.reader(in_handler, delimiter='\t'):
                yield item
