                            ...
                            for raw in raw_path:
                                ...
                                for file_path in files_path:
                                    if file_path not in filelist:
                                        raise FileError('原始序列文件夹的序列文件名称与list.txt不同，请检查确认', code="41400803")
                        else:
                            files_path = line2[1].split(',')
                            for file_path in files_path:
                                if file_path not in filelist:
                                    raise FileError('原始序列文件夹的序列文件名称与list.txt不同，请检查确认', code="41400804")
                    else:
                        raise FileError('list.txt文件格式有误', code="41400805")
