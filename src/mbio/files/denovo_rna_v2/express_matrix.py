...
class ExpressMatrixFile(File):
    ...
    def parse_file(self):
        ...
        if not os.path.exists(exp_file):
            raise FileError('文件不存在：%s', variables = (exp_file), code = "42000701")
        table = pd.read_table(exp_file, index_col=0, header=0)
        gene_number = table.shape[0]
        if gene_number <= 2:
            raise FileError('表达矩阵近乎为空：%s', variables = (exp_file), code = "42000702")
        sample_number = table.shape[1]
        # if sample_number <= 1:
        #     raise FileError('表达矩阵包含的样本太少，无法做差异分析: %s', variables = (exp_file), code = "42000703")
