    ...
    def format_check(self):
        with open(self.prop['path'], 'r') as f:
            ...
            if not re.search("^#", line[0]):
                raise FileError("该alias文件不含表头，alias表第一列应该以#号开头", code="42000101")
            line = line.split("\t")
            length = len(line)
            if length < 2:
                raise FileError('alias_table文件至少应该有两列', code="42000102")
            for i in line[1:]:
                if re.search("\s", i):
                    raise FileError('别名表里不可以包含空格', code="42000103")
        with open(self.prop['path'], 'r') as f:
            for line in f:
                if line.startswith("#"):
                    continue
                ...
                for l in line:
                    if re.search("\s", l):
                        raise FileError('别名表里不可以包含空格', code="42000104")
                len_ = len(line)
                if len_ != length:
                    raise FileError("文件的列数不相等", code="42000105")

    def check(self):
        if super(AliasTableFile, self).check():
            self.get_info()
            if self.prop['sample_number'] == 0:
                raise FileError('应该至少包含一个样本', code="42000106")
