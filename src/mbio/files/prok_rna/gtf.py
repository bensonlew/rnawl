...
class GtfFile(File):
        ...
        for line in open(self.path):
            ...
            if content_m:
                if not {content_m.captures(5)[0], content_m.captures(8)[0]} == {'transcript_id', 'gene_id'}:
                    raise FileError("line error: %s the No9 line must contain gene_id txptid",
                                    variables = (line.strip()), code = "45000701")
                continue
            if not (comment_m or content_m):
                raise FileError("line %s is illegal in gtf file %s: it is not comment line (start with #) or tab-delimeted 9 colomuns line",
                                variables = (line.strip(), self.path), code = "45000702")
    ...
    def __check_gtf_bio_logic(self, log_file):
        ...
        if self._validate_gtf_tool:
            ...
            raise FileError("failed to run validate_gtf.pl", code = "45000703")
    ...
    def _check_chars(self, merged=False):
        ...
        for line in open(self.path):
            ...
            content_m = regex.match(
                ...
                line.strip())
            if not (comment_m or content_m):
                raise FileError("substandard string found in %s", variables = (line), code = "45000704")
            ...
            if content_m:
                ...
                if merged:
                    ...
                    if not merged_m:
                        raise FileError("illegal merged gtf", code = "45000705")
                if not (contig_m and seq_type_m and start_m and frame_m and end_m and strand_m):
                    raise FileError("line %s in gtf file %s is not legal",
                                    variables = (line.strip(), self.path), code = "45000706")
    ...
    def __check_hierachy(self):
        ...
        for line in open(self.path):
            ...
            if not (comment_m or content_m or regex.match(r'^$', line.strip())):
                raise FileError("line %s is illegal in gtf file %s: it is not comment line (start with #) or tab-delimeted 9 colomuns line",
                                variables = (line.strip(), self.path), code = "45000707")
            ...
            if content_m:
                ...
                if not (contig_m and seq_type_m and start_m and frame_m and end_m and strand_m):
                    raise FileError("line %s in gtf file %s is not legal",
                                    variables = (line.strip(), self.path), code = "45000708")
    ...
    def to_isoform2unigene(self, t2g_file, g2t2p=None):
        ...
        t2g2l_sort = sorted(t2g2l, key= lambda x:x[2])
        gene_list = []
        ...
        with open(t2g_file, 'wb') as f:
            for tran, gene, length in t2g2l_sort:
                ...
                if gene.startswith("MSTRG") or gene.startswith("CUFF"):
                    if gene in gene_list:
                        f.write("{}\t{}\t{}\t{}\t{}\n".format(tran, gene, "no", length, pep))
                    else:
                        f.write("{}\t{}\t{}\t{}\t{}\n".format(tran, gene, "yes", length, pep))
                        gene_list.append(gene)
                elif not (tran.startswith("MSTRG") or tran.startswith("CUFF")):
                    if gene in gene_list:
                        f.write("{}\t{}\t{}\t{}\t{}\n".format(tran, gene, "no", length, pep))
                    else:
                        f.write("{}\t{}\t{}\t{}\t{}\n".format(tran, gene, "yes", length, pep))
    ...
    def to_bed(self):
        ...
        try:
            ...
            os.remove(bed)
            raise FileError("failed to accomplish to_bed", code = "45000709")
        pass
