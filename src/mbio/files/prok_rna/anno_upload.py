                ...
                for line in lines:
                    try:
                        ...
                        if len(line) == 3:
                            ...
                            raise FileError("文件每行需为三列", code = "45000101")
                        if line[1] not in ["transcript", "gene"]:
                            raise FileError("文件每行第二列要为gene或transcript，便于区分是基因注释还是转录本注释", code = "45000102")
                        items = line[2].split(";")
                        for item in items:
                            if re.match(r"K|GO.+$", item):
                                pass
                            else:
                                raise FileError("文件第三列应为注释到KEGG数据库中的gene ID", code = "45000103")
                    except:
                        raise FileError("文件每行不是以\t分隔", code = "45000104")
