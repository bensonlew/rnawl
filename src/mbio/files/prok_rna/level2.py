    ...
    def get_table_info(self):
        with open(self.path) as f:
            ...
            if len(header.split('\t')) != 6:
                raise FileError("文件表头存在错误，默认表头：term_type term GO number percent sequence", code = "45000801")
            count = 0
            for record in go_info[1:]:
                if record != '':
                    ...
                    if len(record_info) != 6:
                        raise FileError("文件中存在格式不整齐的行", code = "45000802")
                    if record_info[0] not in ['biological_process', 'cellular_component', 'molecular_function']:
                        raise FileError("文件中存在错误的一级分类：%s", variables = (record_info[0]), code = "45000803")
        return count, header
