...
class FastaFile(CommonFile):
    ...
    def check(self):
        ...
        if super(FastaFile, self).check():
            if self.prop['file_format'] != 'FASTA':
                raise FileError("文件格式错误", code = "45000501")
            if self.prop["seq_number"] < 1:
                raise FileError("应该至少含有一条序列", code = "45000502")
        return True
    ...
    def ncbi_blast_tool_check(self):
        ...
        if self.check():
            if self.prop['seq_type'] not in {"DNA", "Protein"}:
                raise FileError("不支持此类型的Fasta进行blast比对", code = "45000503")
        return True
    ...
    def get_seq_info(self):
        ...
        try:
            ...
            raise FileError("seqstat 运行出错", code = "45000504")
    ...
    def get_all_seq_name(self):
        ...
        for k in seq_name.iterkeys():
            ...
            if len(dup_list) > 0:
                str_ = "; ".join(dup_list)
                raise FileError("序列名：%s在输入的fasta文件里面重复", variables = (str_), code = "45000505")
        return seq_name
            ...
            for i in f:
                ...
                if i[0] == '>':
                    ...
                    if i.startswith('>TRINITY_'):
                        ...
                        raise FileError("文件不是trinity生成的文件", code = "45000506")
        return True
    ...
    def split_single_seq(self, output_dir):
        ...
        if self.check():
            try:
                ...
                raise FileError("get split fa to single seqs failed", code = "45000507")
