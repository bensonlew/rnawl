...
class LocalActor(gevent.Greenlet):
    ...
    #     if workflow.sheet.WPM:
    #         if workflow.rpc_server.last_none_block and \
    #                         (datetime.datetime.now() - workflow.rpc_server.last_none_block).seconds > 3:
    #             return
    #         if workflow.rpc_server.check_rpc(self._agent.id):
    #             return
    #     if self._agent.is_end or self.auto_break:
    #         return
    #
    #     if self._update is not None:
    #         if ((datetime.datetime.now() - self._update).seconds -
    #                 (self._kao_fire_times * self._config.MAX_KEEP_ALIVE_TIME)) > self._config.MAX_KEEP_ALIVE_TIME:
    ...
    #                 if (datetime.datetime.now() - self._agent.job.submit_time).seconds \
    #                         - self._run_startout_times * self._config.RUN_START_TIMEOUT \
    #                         > self._config.RUN_START_TIMEOUT:
    #                     if self.last_runstartout and (datetime.datetime.now() - self.last_runstartout).seconds\
    #                             < self._config.RUN_START_TIMEOUT:
    #                         return
    #                     self._run_startout_times += 1
    #                     self._agent.fire('runstartout', self._run_startout_times)
    #                     self.last_runstartout = datetime.datetime.now()
    #
    #                 if (datetime.datetime.now() - self._agent.job.submit_time).seconds - \
    ...
    def _run(self):
        ...
        while self.running:
            ...
            if self.skip and workflow.is_start:
                self._agent.logger.info("跳过Job运行...")
                self._agent.finish_callback()
            if self._agent.is_end or self.auto_break:
                break
            if len(self._history_messages) > 0 and self._agent.is_start and workflow.is_start:
                with self.sem:
                    messages = copy.copy(self._history_messages)
                for msg in messages:
                    self.receive(msg)
                    with self.sem:
                        self._history_messages.remove(msg)
            gevent.sleep(3)
