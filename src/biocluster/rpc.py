...
# import json
import grpc
from .proto import workflow_guide_pb2_grpc, public_pb2
import os
import copy
import signal
import socket
...
def get_state_string(state):
    return "workflowid: %s, toolid: %s, state: %s jobid: %s jobtype: %s host: %s version: %s data: %s" % (state.workflow_id, state.tool_id, state.state,state.jobid, state.jobtype, state.host, state.version, state.data)
...
class Report(object):
    ...
    def report(self, state, agent):
        """
        用于传递消息的方法。

        :param state: 通过gRPC服务接收的远程消息
        :param agent:
        """
        # if is_rpc is True:
        #     try:
        ...
        # agent = self.workflow.find_tool_by_id(state.tool_id)
        # if not agent:
        #     self.workflow.logger.error("Server在workflow中找不到对应的tool: {} !".format(state.tool_id))
        # else:
        #     agent.send_exit_action("Server在workflow中找不到对应的tool: {} !".format(msg['id']), msg["version"])
        #     if (not isinstance(msg, digentct)) or ('id' not in msg.keys()) or ("version" not in msg.keys()):
        #         self.workflow.logger.error("Server接收到不符合规范的消息: 不是字典类型或没有key值'id'或者'version'!")
        self.workflow.logger.info("收到信息, workflowid: %s, toolid: %s, state: %s jobid: %s jobtype: %s host: "
                                  "%s version: %s data: %s" % (state.workflow_id, state.tool_id, state.state,
                                                               state.jobid, state.jobtype, state.host,
                                                               state.version, state.data))
        if state.version != agent.version:
            self.workflow.logger.error("接收到已经重新投递任务的历史版本信号，丢弃: %s ！" % get_state_string(state))
            agent.send_exit_action("此版本号tate已经更新过期", state.version)
        else:
            # self.workflow.last_update = datetime.datetime.now()
            # if agent.job:
            agent.actor.receive(state)
            # else:
            #     agent.send_exit_action(reason="Job尚未开始却接收到信息", version=agent.version)
            # if not self.workflow.sheet.instant:
            # return agent.get_callback_action(agent.version)
...
class RPC(object):
    ...
    def run(self):
        ...
        gevent.spawn(self._check_unfind_state)
        while not self._closed:
            # self.last_none_block = datetime.datetime.now()
            gevent.sleep(1)

    def get_tool_state(self, signum, frame):
        self._workflow.logger.info("接收到Linux signal %s 信号, 开始获取tool状态信息 " % signum)
        self._get_tool_state_times += 1
        hostname = socket.gethostname()
        ip = socket.gethostbyname(hostname)
        self._workflow.logger.debug("ip:%s port:%s" % (ip, self._workflow.sheet.wfm_port))
        try:
            with grpc.insecure_channel('localhost:%s' % self._workflow.sheet.wfm_port) as channel:
                ...
                responses = stub.GetToolsStates(public_pb2.Workflow(instant=self._workflow.sheet.instant,
                                                                    workflow_id=self._workflow.id,
                                                                    process_id=int(os.getpid())))
                for state in responses:
                    # self._workflow.logger.info("请求tool state rpc：{}".format(state))
                    agent = self._workflow.find_tool_by_id(state.tool_id)
                    if not agent:
                        if state.jobtype == "batch":
                            batch = self._workflow.get_batch(state.tool_id)
                            if batch:
                                self._workflow.logger.debug("接收到Batch 子任务 %s State数据并发送给batch: %s " %
                                                            (state.tool_id, get_state_string(state)))
                                gevent.spawn(self._report.report(state, batch))
                            else:
                                self._workflow.logger.debug("接收到Batch 子任务 %s State数据但是没有找到对应Agent,"
                                                            "稍后尝试: %s " % (state.tool_id, get_state_string(state)))
                                self._unfind_states.append(state)
                        else:
                            self._workflow.logger.debug("接收到State数据但是没有找到对应Agent,稍后尝试: %s " % get_state_string(state))
                            self._unfind_states.append(state)
                    else:
                        self._workflow.logger.debug("接收到State数据并发送给Agent: %s " % get_state_string(state))
                        # self._report.report(state, agent)
                        gevent.spawn(self._report.report(state, agent))
                self._get_tool_state_times = 0
        except Exception as e:
            exstr = traceback.format_exc()
            print(exstr)
            sys.stdout.flush()
            if self._get_tool_state_times > 5:
                self._workflow.logger.error("获取tool状态信息发生错误超过3次,退出运行: %s" % e)
                raise e
            else:
                self._workflow.logger.error("获取tool状态信息发生错误10秒后重试: %s" % e)
                gevent.sleep(20)
                self.get_tool_state(signum, frame)

    def _check_unfind_state(self):
        while True:
            if not self._workflow.is_start:
                gevent.sleep(1)
                continue
            if self._closed:
                break
            states = copy.copy(self._unfind_states)
            for i in range(len(states)):
                state = self._unfind_states[i]
                agent = self._workflow.find_tool_by_id(state.tool_id)
                if agent:
                    self._workflow.logger.debug("历史State数据找到并发送给Agent: %s " % get_state_string(state))
                    self._report.report(state, agent)
                    self._unfind_states.pop(i)
            gevent.sleep(10)
    ...
    # def check_rpc(self, tid):
    #     if not self._workflow.sheet.WPM:
    #         return False
    #     message_queue = self._workflow.rpc_message_queue
    #     count = 0
    #     message_list = []
    ...
    #                     message_queue[i] = ("", "", "", "", "", 0, 0)
    #                     count += 1
    #                 elif v.id.split(".")[0] == self._workflow.id:
    #                     message_list.append(v.to_dict())
    #                     # self._workflow.logger.debug("检测到%s!" % v)
    #                     message_queue[i] = ("", "", "", "", "", 0, 0)
    #     for msg in message_list:
    ...
    #             self._set_callback_queue(msg)
    #     if count > 0:
    #         # self._workflow.logger.debug("检测RPC信息，获取到%s条新RPC消息...!" % count)
    #         return True
