...
class RefRnaV2(Base):
    def __init__(self, bind_object=None):
        ...
        super(RefRnaV2, self).__init__(bind_object=self._bind_object)
        self._project_type = 'ref_rna_v2'
        self.db1 = Config().get_mongo_client(mtype=self._project_type, dydb_forbid=True)[Config().get_mongo_dbname(self._project_type, dydb_forbid=True)]
    ...
    def get_main_info_by_record(self, main_table, **kwargs):
        ...
        :return:
        """
        if main_table == 'sg_genome_db':
            collection = self.db1[main_table]
        else:
