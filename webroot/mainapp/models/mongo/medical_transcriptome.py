...
class MedicalTranscriptome(Base):
    def __init__(self, bind_object=None):
        self._bind_object = bind_object
        super(MedicalTranscriptome, self).__init__(bind_object=self._bind_object)
    ...
    def get_diff_table_name(self,diff_id,task_id):
        ...
        print(diff_id)
        diff_collection = self.db['sg_diff']
        try:
            col_result = diff_collection.find_one({"main_id": diff_id,"task_id":task_id})
            return  col_result
        except:
            print("不能找到对应table_id {} in {}".format(diff_id, "sg_diff"))
            raise Exception("不能找到对应table_id {} in {}".format(diff_id, "sg_diff"))

    def get_diff_table_infos(self, main_id, task_id):
        """
        根据diff_id获得整个差异主表/记录的信息
        :param main_id:
        :param collection_name:
        :return: 整个主表信息
        """
    ...
    def delete_difftable(self,diff_id,task_id):
        ...
        try:
            ...
            status_collection.update({"table_id": diff_id,"task_id":task_id}, {"$set": col_result})
        except:
            print("不能找到对应table_id {} in {}".format(diff_id, 'sg_status'))
        col = self.db["sg_diff"]
        try:
    ...
    def delete_geneset(self, geneset_id, task_id):
        if isinstance(geneset_id, types.StringTypes):
            ...
            raise Exception("输入geneset_id参数必须为字符串或者ObjectId类型!")
        # 补救代码,为配合主工作流部分分析模块没有导入sg_geneset_info信
        # 聚类
        collection_diff_cluster = self.db['sg_diff_geneset_cluster']
        target_results = collection_diff_cluster.find({"task_id": task_id, "status": "end"})
        for target_result in target_results:
            if target_result["params"] != "":
                params = json.loads(target_result["params"])
                print(params["geneset_id"])
                if params["geneset_id"] == str(geneset_id):
                    print("该表格需删除")
                    target_result["params"] = ""
                    collection_diff_cluster.update_one({"_id": target_result["_id"], "task_id": task_id},
                                                       {"$set": target_result})

        # venn
        collection_diff_venn = self.db['sg_diff_geneset_venn']
        target_results = collection_diff_venn.find({"task_id": task_id, "status": "end"})
        for target_result in target_results:
            if target_result["params"] != "":
                params = json.loads(target_result["params"])
                if str(geneset_id) in params["geneset_ids"]:
                    print("该表格需删除")
                    target_result["params"] = ""
                    collection_diff_venn.update_one({"_id": target_result["_id"], "task_id": task_id},
                                                    {"$set": target_result})
        collection = self.db['sg_geneset_info']
        status_collection = self.db['sg_status']
        ...
        for result in results:
            ...
            try:
                ...
                if col_result["task_id"] == task_id:
                    col_result["params"] = ""
                    col.update_one({"_id": col_id}, {"$set": col_result})
            except:
                print("不能找到对应id {} in {}".format(col_id, col_name))
            try:
                col_result = status_collection.find_one({"table_id": col_id})
                col_result["status"] = "deleted"
                status_collection.update_one({"table_id": col_id}, {"$set": col_result})
            except:
    ...
    def get_pep(self, task_id, table_name="sg_annotation_stat"):
        ...
        if "genome_id" in result_task.keys():
            genome_id = result_task["genome_id"]
            db = Config().get_mongo_client(mtype='ref_rna_v2', dydb_forbid=True)[
                Config().get_mongo_dbname('ref_rna_v2', dydb_forbid=True)]
            col = db["sg_genome_db"]
            genome_info = col.find_one({"genome_id": genome_id})
            db_path = Config().SOFTWARE_DIR + "/database/Genome_DB_finish"
