...
class WholeTranscriptome(Base):
    def __init__(self, bind_object=None):
        self._bind_object = bind_object
        super(WholeTranscriptome, self).__init__(bind_object=self._bind_object)
    ...
    def check_geneset_regulate(self, main_id):
        collection = self.db['geneset_detail']
        geneset_detail = collection.find_one({'geneset_id': main_id})
        if "regulate_list" in geneset_detail:
            return True
        else:
            return False

    def get_geneset_batch_flag(self, task_id):
        collection = self.db["geneset_batch"]
    ...
    def update_db_record1(self, table_name, record_id=None, query_dict=None, insert_dict=None, **kwargs):
