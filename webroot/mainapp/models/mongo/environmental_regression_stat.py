...
class EnvironmentalRegressionStat(Base):
    ...
    def create_environmental_regression(self, params, group_id=0, from_otu_table=0, level_id=0, env_id=0, name=None):
        ...
        if env_id != 0 and not isinstance(env_id, ObjectId):
            if isinstance(env_id, StringTypes):
                env_id = ObjectId(env_id)
            else:
                raise Exception("env_id必须为ObjectId对象或其对应的字符串!")
        ...
        desc = "environmental_regression分析"
        insert_data = {
            "project_sn": project_sn,
            "task_id": task_id,
            "otu_id": from_otu_table,
            "name":  name if name else "environmental_regression_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S"),
            "params": params,
            "desc": desc,
            "status": "end",
            "created_ts": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        }
        collection = self.db["sg_environmental_regression"]
