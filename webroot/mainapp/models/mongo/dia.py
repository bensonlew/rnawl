...
dia里面定义了很多函数，可以直接获取mongodb里面的数据，获得具体的参数值，如文件路径信息，task_id信息,根据这里面的函数的需求
'''
class Dia(Base):
    def __init__(self, bind_object=None):
        self._bind_object = bind_object
        super(Dia, self).__init__(bind_object=self._bind_object)
    ...
    def get_exp_params_info_new2(self, task_id=None, name=None):
        collection = self.db['sg_express']
        result = collection.find_one({'name': name, 'task_id': task_id})
        return result
