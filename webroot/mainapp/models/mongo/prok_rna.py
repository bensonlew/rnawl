...
class ProkRNA(Base):
    def __init__(self, bind_object=None):
        ...
        super(ProkRNA, self).__init__(bind_object=self._bind_object)
    ...
    def get_count_path(self, main_id):
        ...
        if 'count_file' in result and result['count_file'].startswith("s3"):
            ...
            result1 = collection.find_one({"task_id": result["task_id"], "method": "RSEM"})
    ...
    def consistent_in_enrich_and_diff(self, main_id, diff_id, typee="KEGG", table_name="sg_geneset_kegg_enrich"):
        ...
        diff_type = json.loads(diff_main['params'])['type']
        if diff_type == 'ref':
            if has_new:
                return False
            else:
...
#------针对获取基因上下游序列的几个函数------------------
    def get_rockhopper_info(self, task_id):
        """
        根据task_id到sg_task获得相关记录信息
        :param task_id:
        :return:
        """
        collection = self.db['sg_srna_predict']
        result = collection.find_one({"task_id": task_id})
        return result

    def insert_or_replace_main_table(self, collection_name, data):
        """
        新建主表或替换主表之前的内容，并返回该条记录的_id
        :param collection_name:
        :param data:
        :return: 插入记录的id
        """
        ...
        result = collection.find_one({"task_id": task_id})
        if result:
            collection.update({"task_id": task_id}, {"$set": data})
            main_id = result['_id']
        else:
            main_id = collection.insert_one(data).inserted_id
        # conn = self.db[collection_name]
        # task_id = conn.find_one({'_id': main_id})['task_id']
        collection.update({'_id': main_id}, {"$set": {'main_id': main_id}}, upsert=True)
        return main_id
