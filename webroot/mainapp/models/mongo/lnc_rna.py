...
class LncRna(Base):
    def __init__(self, bind_object=None):
        ...
        super(LncRna, self).__init__(bind_object=self._bind_object)
    ...
    def insert_geneset_info(self, geneset_id, col_name, col_id):
        ...
        geneset_list.extend([ObjectId(x) for x in geneset_id.split(",") if x != ""])
    ...
    def get_count_path(self, main_id):
        ...
        if result["exp_type"] == "FPKM":
            result1 = collection.find_one({"task_id": result["task_id"], "method" : "RSEM", "exp_level": result["exp_level"]})
            return result1['count_file']
        else:
            return result['count_file']

    def get_seq_type_path(self,task_id,exp_level):
        collection=self.db['sg_task']
        result=collection.find_one({"task_id":task_id})
        if exp_level=="T":
            return result['all_trans_type']
        if exp_level == "G":
            return result['all_gene_type']
