...
class WgcnaModuleAction(LabelfreeController):
    def __init__(self):
    ...
    def POST(self):
        ...
        prepare_info = self.labelfree.get_main_info(data.wgcna_prepare_id, 'sg_wgcna_prepare', data.task_id)
        project_sn = prepare_info["project_sn"]
        task_id = data.task_id
        if data.power == "" or data.power == "NaN":
            prepare_info = self.labelfree.get_power(data.wgcna_prepare_id)
        ...
        # prepare to file
        to_files = ["labelfree.export_wgcna_exp_matrix(exp_matrix)"]

        # 把参数交给workflow运行相应的tool， 其中to_file用于准备tool的输入文件
        task_name = 'labelfree.report.wgcna_module'
