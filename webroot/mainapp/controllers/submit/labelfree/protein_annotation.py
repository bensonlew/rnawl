...
class ProteinAnnotationAction(LabelfreeController):
    """
    蛋白注释重运行接口
    """
    def __init__(self):
    ...
    def POST(self):
        ...
        stat_info = self.labelfree.get_main_info_by_record("sg_annotation_stat", task_id=str(data.task_id), type="origin")
        if not stat_info:
            info = {"success": False, "info": "stat_id不存在,请确认参数是否正确"}
            return json.dumps(info)
        print stat_info

        run_info = self.labelfree.get_main_info_by_record("sg_annotation_stat", task_id=str(data.task_id), status="start", type="latest")
        if run_info:
            info = {"success": False, "info": "任务{}正在计算，请等待其完成在下次运行".format(run_info['name'])}
            return json.dumps(info)

        latest_info = self.labelfree.get_main_info_by_record("sg_annotation_stat", task_id=str(data.task_id), status="end",  type="latest")
        latest_main_id = ""
        ...
        main_table_id = self.labelfree.insert_main_table("sg_annotation_stat", mongo_data)
        ...
        to_file = []
        self.set_sheet_data(name="labelfree.report.protein_annotation",
                            options=options,
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/labelfree/protein_annotation "
