...
class ProteinsetStringPictureAction(LabelfreeController):
    """
    蛋白质互作网络的接口
    """
    ...
    def POST(self):
        ...
        if int(data.species) not in self.species_list:
            ...
            return json.dumps(info)
        '''
        task_name = 'labelfree.report.proteinset_string_picture'
        task_type = 'workflow'
        params_json = {
        ...
        stat_info = self.labelfree.get_main_info_by_record("sg_annotation_stat", task_id=str(data.task_id),
                                                           type="origin")
        origin_result = stat_info["result_dir"]
        if not origin_result.endswith("/"):
            origin_result = origin_result + "/"

        main_table_name = 'ProteinsetStrPic_' + datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        ...
        main_table_id = self.labelfree.insert_main_table('sg_proteinset_string_picture', mongo_data)
        ...
        to_file = "labelfree.export_protein_list_pfam(proteinset_list)"
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/labelfree/proteinset_string_picture "
