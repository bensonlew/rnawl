...
class WgcnaPipelineAction(LncRnaController):
    def __init__(self):
        super(WgcnaPipelineAction, self).__init__(instant=False)
        self.task_name = 'lnc_rna.report.wgcna_pipeline'
        self.expected_args = ["task_id", "submit_location", 'task_type']
        self.expected_args += ['group_id', 'group_dict', 'exp_id', 'me',  'exp_level', 'cv']
        ...
        self.choose_args = ['geneset_id', 'lncset_id']
    ...
    def check_params(self):
        ...
        if self.input_data.geneset_id == "" and self.input_data.lncset_id == "":
            info = {'success': False, 'info': "mRNA基因集, lncRNA基因集至少选择一个"}
            return json.dumps(info)
        # do some other check
    ...
    def pack_params(self):
        ...
        params_dict = dict()
        for each in self.expected_args + self.choose_args:
            if each in input_data_dict:
                if each == "task_type":
    ...
    def create_main_table(self, packed_params, collection_name):
        ...
        """
        result_info = self.lnc_rna.get_main_info(self.input_data.exp_id, 'sg_exp', self.input_data.task_id)
        ...
        if hasattr(self.input_data, 'geneset_id') and (self.input_data.geneset_id.lower() not in ["all", "refall", "none"]):
            if self.input_data.geneset_id != "":
                self.lnc_rna.insert_geneset_info(self.input_data.geneset_id, collection_name, str(main_id))
        if hasattr(self.input_data, 'lncset_id') and (self.input_data.lncset_id.lower() not in ["all", "refall", "none"]):
            if self.input_data.lncset_id != "":
                self.lnc_rna.insert_geneset_info(self.input_data.lncset_id, collection_name, str(main_id))
        return main_id, name
    ...
    def prepare_workflow_options(self, main_id, collection_name, main_table_name):
        ...
        options.update({
            ...
            "main_id": str(main_id),
            "exp_matrix": self.input_data.exp_id+";"+self.input_data.geneset_id+";"+self.input_data.lncset_id,
            "gene_info": self.input_data.geneset_id+";"+self.input_data.lncset_id,
        ...
        if self.input_data.geneset_id != "":
            options.update({
                "geneset_id" : self.input_data.geneset_id
            })
        if self.input_data.lncset_id != "":
            options.update({
                "lncset_id" : self.input_data.lncset_id
            })
        # prepare to file
        to_files = ["lnc_rna.export_geneset_lncset_exp_matrix(exp_matrix)",
                    "lnc_rna.export_gene_name_type2(gene_info)"]
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        args = dict(
            ...
            networkType="signed",
            minKMEtoStay="0.5",
            trait_path="/mnt/ilustre/users/sanger-dev/sg-users/liubinxu/test_lnc_rna/test_trait.txt",
            trait_type="continue",
            corr_method="pearson",
            file_id="6919032",
            threshold="0.03",
