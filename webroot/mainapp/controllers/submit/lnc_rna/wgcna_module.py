...
class WgcnaModuleAction(LncRnaController):
    def __init__(self):
    ...
    def POST(self):
        ...
        exp_info = self.lnc_rna.get_main_info(data.wgcna_prepare_id, 'sg_wgcna_prepare', data.task_id)
        project_sn = exp_info["project_sn"]
        task_id = data.task_id
        if data.power == "" or data.power == "NaN":
            prepare_info = self.lnc_rna.get_power(data.wgcna_prepare_id)
        ...
        main_id = self.lnc_rna.insert_main_table('sg_wgcna_module', main_info)
        ...
        # prepare to file
        to_files = ["lnc_rna.export_wgcna_exp_matrix(exp_matrix)"]

        # 把参数交给workflow运行相应的tool， 其中to_file用于准备tool的输入文件
        task_name = 'lnc_rna.report.wgcna_module'
