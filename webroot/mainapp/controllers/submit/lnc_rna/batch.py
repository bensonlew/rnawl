...
class BatchAction(LncRnaController):
    ...
    def POST(self):
        ...
        main_id_exp = self.lnc_rna.insert_main_table('sg_exp', main_info_exp)

        name = 'Exp_batch_{}'.format(time_now.strftime('%Y%m%d_%H%M%S'))
        task_info = self.lnc_rna.get_task_info(task_id=data.task_id)
        main_info_batch = {
            'task_id': data.task_id,
        ...
        main_id_batch = self.lnc_rna.insert_main_table('sg_exp_batch', main_info_batch)
        new_task_id = self.lnc_rna.get_new_id(data.task_id)
        ...
        options = {
            "exp_matrix": data.exp_id + ';' + sample_str + ';' + 'all',
        ...
        #     })
        # prepare to file
        to_files = ["lnc_rna.export_exp_batch_matrix(exp_matrix)",
                    "lnc_rna.export_group_detail(group_table)",
                    "lnc_rna.export_exp_other_matrix(exp_other)"
                    ]
        task_name = 'lnc_rna.report.batch'
