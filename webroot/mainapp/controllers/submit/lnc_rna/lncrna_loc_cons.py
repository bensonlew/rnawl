...
class LncrnaLocConsAction(LncRnaController):
    '''
    last_modify: 2019.04.24
    '''
    def __init__(self):
        super(LncrnaLocConsAction, self).__init__(instant=False)

    @check_sig
    def POST(self):
        data = web.input()
        args = ['task_id', 'submit_location', 'task_type', 'genome', 'ways', 'sequence']
        for arg in args:
        ...
        name = 'Lncrna_loc_cons_{}'.format(time_now.strftime('%Y%m%d_%H%M%S'))
        desc = 'lncRNA location conservation main table'
        param_dict.update({
            'task_id': task_id,
        ...
        main_id = self.lnc_rna.insert_main_table('sg_lncrna_loc_cons', main_info)
        ...
        options.update({
            ...
            'main_id': str(main_id),
            'update_info': json.dumps({str(main_id): 'sg_lncrna_loc_cons'})
        })
        self.set_sheet_data(
            name='lnc_rna.report.lncrna_loc_cons',
            options=options,
        ...
        run_info = super(LncrnaLocConsAction, self).POST()
...
class TestFunction(unittest.TestCase):
    ...
    def test_lncrna_id(self):
        ...
        cmd += ' s/lnc_rna/lncrna_loc_cons'
        args = {
            'task_id': 'tsg_33915',
            'task_type': '2',
            'submit_location': 'lncrna_loc_cons',
