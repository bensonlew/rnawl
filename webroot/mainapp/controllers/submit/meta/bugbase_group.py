...
class BugbaseGroupAction(MetaController):
    ...
    def POST(self):
        ...
        task_name = 'meta.report.bugbase_group'
        task_type = 'workflow'  # 可以不配置
        print data.bugbase_id
        bugbase_info = self.meta.get_bugbase_info(data.bugbase_id)
        print bugbase_info
        if not bugbase_info:
        ...
        if data.group_method != "two":
            if len(table_dict) <= 2 or data.group_id == 'all':
                ...
                return json.dumps(info)
        task_info = self.meta.get_task_info(bugbase_info['task_id'])
        ...
        main_table_id = self.meta.insert_none_table('sg_bugbase_group')
        update_info = {str(main_table_id): 'sg_bugbase_group'}
        if data.group_method == "two":
            options = {
        ...
        to_file = ["meta.export_bugbase_table_by_bugbase_id(bugbase_table)", "meta.export_group_table_by_detail(group_file)"]
