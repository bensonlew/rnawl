...
class BugbaseGroupAction(MetaasvController):

    def __init__(self):
        super(BugbaseGroupAction, self).__init__(instant=False)
    ...
    def POST(self):
        ...
        task_name = 'metaasv.report.bugbase_group'
        task_type = 'workflow'  # 可以不配置
        bugbase_info = self.metaasv.get_bugbase_info(data.bugbase_id)
        if not bugbase_info:
           info = {"success": False, "info": "bugbase分析不存在，请确认参数是否正确！!"}
           return json.dumps(info)
        ...
        if data.group_method != "two":
            if len(table_dict) <= 2 or data.group_id == 'all':
                ...
                return json.dumps(info)
        task_info = self.metaasv.get_task_info(bugbase_info['task_id'])
        main_table_name = 'BugbaseGroup_' + datetime.datetime.now().strftime("%Y%m%d_%H%M%S%f")[:-3]
        groupname = json.loads(data.group_detail).keys()
        groupname.sort()
        category_name = ','.join(groupname)
        my_param = dict()
        my_param['bugbase_id'] = data.bugbase_id
        ...
        mongo_data = {
            ...
            "group_id": self.metaasv.get_group_name(data.group_id),
        ...
        main_table_id = self.metaasv.insert_none_table('bugbase_group')
        # print main_table_id, type(main_table_id)
        update_info = {str(main_table_id): 'bugbase_group'}
        if data.group_method == "two":
            options = {
        ...
        to_file = ["metaasv.export_bugbase_table_by_bugbase_id(bugbase_table)", "metaasv.export_group_table_by_detail(group_file)"]

        self.set_sheet_data(name=task_name, options=options, main_table_name="BugbaseGroup/" + main_table_name, module_type=task_type, to_file=to_file)
        task_info = super(BugbaseGroupAction, self).POST()
    ...
    def check_options(self, data):
        ...
        检查网页端传进来的参数是否正确
        """
        params_name = ['bugbase_id', 'group_detail', 'group_id',  'correction', 'test', 'coverage', 'submit_location']
        success = []
