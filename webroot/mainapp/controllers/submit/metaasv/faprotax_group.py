...
class FaprotaxGroupAction(MetaasvController):
    ...
    def POST(self):
        ...
        task_name = 'metaasv.report.faprotax_group'
        task_type = 'workflow'  # 可以不配置
        print data.faprotax_id
        faprotax_info = self.metaasv.get_faprotax_info(data.faprotax_id)
        print faprotax_info
        ...
        if data.group_method != "two":
            if len(table_dict) <= 2 or data.group_id == 'all':
                ...
                return json.dumps(info)
        task_info = self.metaasv.get_task_info(faprotax_info['task_id'])
        ...
        main_table_id = self.metaasv.insert_none_table('faprotax_group')
        update_info = {str(main_table_id): 'faprotax_group'}
        if data.group_method == "two":
            options = {
        ...
        to_file = ["metaasv.export_faprotax_table_by_faprotax_id(faprotax_table)", "metaasv.export_group_table_by_detail(group_file)"]
