...
class ProteinsetPpiAction(DiaController):
    """
    蛋白质互作网络的接口
    """
    ...
    def POST(self):
        ...
        if int(data.species) not in self.species_list:
            ...
            return json.dumps(info)
        '''
        task_name = 'dia_v3.report.proteinset_ppi'
        task_type = 'workflow'
        params_json = {
        ...
        main_table_name = 'ProteinsetPPI_' + datetime.datetime.now().strftime("%Y%m%d_%H%M%S%f")[:-3]
        ...
        main_table_id = self.dia.insert_main_table('sg_proteinset_ppi', mongo_data)
        ...
        to_file = "dia.export_protein_list_ppi(proteinset_list)"
