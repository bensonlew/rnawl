...
class WgcnaNetworkAction(DiaController):
    def __init__(self):
        super(WgcnaNetworkAction, self).__init__(instant=False)
        self.task_name = 'dia_v3.report.wgcna_network'
    ...
    def POST(self):
        ...
        if 'group_id' in self.input_data and str(self.input_data.group_id).lower() != 'all':
            _ = self.dia.update_group_is_use(self.input_data.task_id, self.input_data.group_id)
        if 'control_id' in self.input_data:
            _ = self.dia.update_group_compare_is_use(self.input_data.task_id, self.input_data.control_id)
        ##
    ...
    def create_main_table(self, packed_params, db_name):
        result_info = self.dia.get_main_info(self.input_data.wgcna_relate_id, 'sg_wgcna_relate', self.input_data.task_id)
        ...
        main_info = dict(
            ...
            status="start")
        main_id = self.dia.insert_main_table(db_name, main_info)
        return main_id, name

    def prepare_workflow_options(self, main_id, db_name, main_table_name):
        relate_output = self.dia.get_main_info(self.input_data.wgcna_relate_id,'sg_wgcna_relate', self.input_data.task_id)['output_dir']
        if not relate_output.endswith('/'):
            relate_output += '/'
        module_output = self.dia.get_main_info(self.wgcna_module_id, 'sg_wgcna_module', self.input_data.task_id)['output_dir']
