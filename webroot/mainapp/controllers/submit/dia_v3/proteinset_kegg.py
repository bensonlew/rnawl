...
class ProteinsetKeggAction(DiaController):
    def __init__(self):
    ...
    def POST(self):
        ...
        task_name = 'dia_v3.report.proteinset_kegg'
        params_json = {
        ...
        for proteinset in data.proteinset_id.split(","):
            proteinset_info = self.dia.get_main_info(proteinset, 'sg_proteinset', data.task_id)
        ...
        to_file = ['dia.export_multi_protein_list(proteinset_kegg)',
                   "dia.export_kegg_table(kegg_table)",
                   "dia.export_kegg_level_table(kegg_table_2)",
        ...
        main_table_name = 'Proteinset' + table_name + "_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S%f")[:-3]
        ...
        proteinset_info = self.dia.insert_proteinset_info(data.proteinset_id, collection_name, str(main_table_id))
        ##
