...
class WgcnaPrepareAction(DiaController):
    def __init__(self):
    ...
    def POST(self):
        ...
        exp_info = self.dia.get_main_info(data.exp_id, 'sg_express', data.task_id)
        ...
        if data.proteinset_id.lower() not in ["all", "none"]:
            self.dia.insert_proteinset_info(data.proteinset_id, 'sg_wgcna_prepare', str(main_id))
        ...
        task_name = 'dia_v3.report.wgcna_prepare'
