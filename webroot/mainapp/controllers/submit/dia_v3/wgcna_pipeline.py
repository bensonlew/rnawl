...
class WgcnaPipelineAction(DiaController):
    def __init__(self):
        super(WgcnaPipelineAction, self).__init__(instant=False)
        self.task_name = 'dia_v3.report.wgcna_pipeline'
    ...
    def create_main_table(self, packed_params, collection_name):
        ...
        result_info = self.dia.get_main_info(self.input_data.exp_id, 'sg_express', self.input_data.task_id)
        ...
        main_info = dict(
            ...
            status="start")
        main_id = self.dia.insert_main_table(collection_name, main_info)
        if self.input_data.proteinset_id.lower() not in ["all", "none"]:
            self.dia.insert_proteinset_info(self.input_data.proteinset_id, collection_name, str(main_id))
        return main_id, name
    ...
    def prepare_workflow_options(self, main_id, collection_name, main_table_name):
        ...
        # prepare to file
        to_files = ["dia.export_proteinset_exp_matrix(exp_matrix)"]
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/dia_v3/wgcna_pipeline "
        cmd += "-b http://bcl.tsg.com "
        args = dict(
            task_id="dia_test",
            task_type="2",
            exp_id='5fc737f517b2bf708ed8252e',
            ...
            networkType="signed",
            minKMEtoStay="0.5",
            trait_path="/mnt/ilustre/users/sanger-dev/sg-users/xuxi/tmp_trait.xls",
            trait_type="discrete",
            corr_method="pearson",
            file_id="6919032",
            threshold="0.03",
