...
class ProteinsetClassAction(DiaController):
    def __init__(self):
    ...
    def POST(self):
        ...
        if data.anno_type == "go2":
            if len(num) != 2:
                info = {'success': False, 'info': "必须选择两个蛋白集"}
                return json.dumps(info)
        elif data.anno_type == "go" or data.anno_type == "cog":
            if len(num) < 1 :
                info = {'success': False, 'info': "至少选择一个蛋白集"}
                return json.dumps(info)
        else:
            info = {'success': False, 'info': "不支持该注释类型"}
            return json.dumps(info)

        task_name = 'dia_v3.report.proteinset_class'
        params_json = {
