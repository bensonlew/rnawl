...
class ProteinsetCircAction(DiaController):
    def __init__(self):
    ...
    def POST(self):
        ...
        task_name = 'dia_v3.report.proteinset_circ'
        params_json = {
        ...
        # 判断传入的富集id是否存在
        if data.enrich_type == "GO":
            enrich_info = self.dia.get_main_info(data.enrich_id, 'sg_proteinset_go_enrich', data.task_id)
        elif data.enrich_type == "KEGG":
            enrich_info = self.dia.get_main_info(data.enrich_id, 'sg_proteinset_kegg_enrich', data.task_id)
        else:
        ...
        task_info = self.dia.get_task_info(enrich_info['task_id'])
        ...
        if data.enrich_type == "GO":
            to_file = ['dia.export_go_enrich_matrix(enrich_table)',
                       "dia.export_compare_exp_fc(diff_fc)"]
        elif data.enrich_type == "KEGG":
            to_file = ['dia.export_kegg_enrich_matrix(enrich_table)',
                       "dia.export_compare_exp_fc(diff_fc)"]

        # proteinset_type只有2种，T或者G,s而页面限制只能同时取G或者T，所以这里用一个proteinset_id作为示例即可
        main_table_name = "Circ_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S%f")[:-3]
