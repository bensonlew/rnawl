...
class GroupsDiffAction(DiaController):
    def __init__(self):
    ...
    def POST(self):
        ...
        for arg in basic_args:
            ...
            if arg.lower() == "null":
                ...
                return json.dumps(info)
        exp_info = self.dia.get_task_info(data.task_id)
        ...
        main_id = self.dia.insert_main_table('sg_groups_diff', main_info)
        ...
        options = {
            ...
            "protein_table": data.express_id,
            "post_hoc": data.post_hoc,
            "test_method": data.test_method,
            "update_info": json.dumps({str(main_id): "sg_groups_diff"}),
        }
        # prepare to file
        to_files = ["dia.export_exp_matrix2(protein_table)",
                    "dia.export_group(group)"]

        # 把参数交给workflow运行相应的tool， 其中to_file用于准备tool的输入文件
        task_name = 'dia_v3.report.groups_diff'
