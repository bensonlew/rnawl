...
class ProteinsetStringPictureAction(DiaController):
    """
    蛋白质互作网络的接口
    """
    ...
    def POST(self):
        ...
        if int(data.species) not in self.species_list:
            ...
            return json.dumps(info)
        '''
        task_name = 'dia_v3.report.proteinset_string_picture'
        task_type = 'workflow'
        params_json = {
        ...
        proteinset_info = self.dia.get_main_info(data.proteinset_id, 'sg_proteinset', data.task_id)
        ...
        stat_info = self.dia.get_main_info_by_record("sg_annotation_stat", task_id=str(data.task_id),
                                                           type="origin")
        origin_result = stat_info["result_dir"]
        if not origin_result.endswith("/"):
            origin_result = origin_result + "/"

        main_table_name = 'ProteinsetStringPicture_' + datetime.datetime.now().strftime("%Y%m%d_%H%M%S%f")[:-3]
        ...
        main_table_id = self.dia.insert_main_table('sg_proteinset_string_picture', mongo_data)
        ...
        to_file = "dia.export_protein_list_pfam(proteinset_list)"
        ...
        proteinset_info = self.dia.insert_proteinset_info(data.proteinset_id, "sg_ppinetwork", str(main_table_id))
        ##
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/dia_v3/proteinset_string_picture "
