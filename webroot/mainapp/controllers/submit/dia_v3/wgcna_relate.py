...
class WgcnaRelateAction(DiaController):
    def __init__(self):
    ...
    def POST(self):
        ...
        main_info = self.dia.get_main_info(data.wgcna_module_id, 'sg_wgcna_module', data.task_id)
        ...
        module_info = self.dia.get_main_info_by_record('sg_wgcna_module', main_id=wgcna_module_id, task_id=data.task_id)
        ...
        # prepare to file
        to_files = ["dia.export_wgcna_relate_input(exp_eigengenes)"]

        # 把参数交给workflow运行相应的tool， 其中to_file用于准备tool的输入文件
        task_name = 'dia_v3.report.wgcna_relate'
