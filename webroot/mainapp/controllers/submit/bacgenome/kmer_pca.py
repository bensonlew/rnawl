...
class KmerPcaAction(BacgenomeController):
    def __init__(self):
        super(KmerPcaAction, self).__init__(instant=False)
    ...
    def POST(self):
        ...
        task_name = 'bacgenome.report.pca'
        ...
        main_table_name = data.sample + "_" + data.sof_type + '_' + 'KmerPca_' + datetime.datetime.now().strftime("%Y%m%d_%H%M%S%f")[:-3]
        update_info = {}
        mongo_data = [
        ...
        main_id = self.bacgenome.insert_main_table('kmer_pca', mongo_data)
        self.bacgenome.insert_main_table_new("kmer_pca", str(main_id), {"main_id": main_id})
        update_info[str(main_id)] = 'kmer_pca'
        options = {
                   'samp': data.sample,
        ...
        self.set_sheet_data(name=task_name,
                            options=options,
                            main_table_name="KmerPca/" + main_table_name,
                            module_type=module_type,
                            project_sn=project_sn,
                            task_id=data.task_id,
                            params=params,
                            to_file=to_file)
        task_info = super(KmerPcaAction, self).POST()
