...
class PtAnalysisAction(PtController):
    ...
    def POST(self):
        ...
        task_name = 'medical.paternity_test_v2.report.pt_analysis'
        task_type = 'workflow'
        options = {
        ...
        if hasattr(data, 'samples'):  # call snp
            ...
            family_result = PT("pt_v2").get_one("sg_family", "case_name", str(data.dad_id).strip().split('-')[0])
            if not family_result:
                info = {"success": False, "info": "家系编号{}在数据库中不存在！".format(str(data.dad_id).strip().split('_')[0])}
                return json.dumps(info)
                # raise Exception("家系编号{}在数据库中不存在！".format(str(data.dad_id).strip().split('_')[0]))
            else:
                family_id = family_result['_id']
            main_table_name = data.dad_id + '_' + data.mom_id + '_' + data.son_id
            mongo_data = [
                ('member_id', data.member_id),
                ('batch_id', ObjectId(data.batch_id)),
                ('family_id', family_id),
                ...
                ('preg_id', str(data.son_id).strip().split('_')[0]),
                ('desc', '亲子鉴定家系分析'),
                ('status', 'start'),
            ...
            main_table_id = PT('pt_v2').add_main_id('sg_father')
        ...
        self.set_sheet_data_(name=task_name, options=options, module_type=task_type, params=params,
                             db_type='pt_v2', analysis_name=analysis_name)
    ...
    def check_options(self, data):
        ...
        # if not PT("pt_v2").get_one("sg_sample_tab", "sample_id", data.mom_id):
        #     success.append("母本{}tab文件不存在，不能进行后续分析！".format(data.mom_id))
        # if not PT("pt_v2").get_one("sg_sample_tab", "sample_id", data.dad_id):
        #     success.append("父本{}tab文件不存在，不能进行后续分析！".format(data.dad_id))
        return success
