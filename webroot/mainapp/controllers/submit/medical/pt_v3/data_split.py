...
class DataSplitAction(MedController):
    ...
    板号名字（前端不想查找，所以就我们这边进行个判断）
       ...
       然后启动后面的工作流
    last modified by HONGDONG 20180830
    """
    def __init__(self):
        super(DataSplitAction, self).__init__(instant=False)
        self.datatype = "notwailai"
    ...
    def POST(self):
        ...
        # 判断是都外来数据
        if re.match(r".*_wailaidata_.*", str(board_batch_name)):
            self.datatype = "iswailai"
        if self.datatype == "iswailai":
            if not hasattr(data, "aliyun_path"):
                info = {'success': False, 'info': '外送数据必须要提供阿里云的路径'}
                return json.dumps(info)
        self.check_can_split(board_batch_name)
        indextype = 'single'
        board = MedMongo("pt_v3").get_one('sg_datasplit', 'board_batch', board_batch_name)
        if not board:
            ...
            datasplit_batch_id = MedMongo("pt_v3").add_one('sg_datasplit', split_data)
            record_data = {
            ...
            MedMongo("pt_v3").add_one('sg_datasplit_records', record_data)
            return json.dumps(info)
        else:
            if 'indextype' in board.keys():
                indextype = board['indextype']
            if hasattr(data, "start_type") and str(data.start_type) == 'auto':
                MedMongo("pt_v3").update_table("sg_datasplit", "_id", board['_id'], {'board_path': data.board_batch})
            init_data = {'status': "start", "desc": "", "created_ts": datetime.datetime.now().strftime("%Y-%m-%"
                                                                                                       "d %H:%M:%S")}
            MedMongo("pt_v3").update_table("sg_datasplit", "_id", board['_id'], init_data)

        if data.project_types in ['all', 'pt']:
            ana = MedMongo("pt_v3").get_one_bymore('sg_analysis_status',
            ...
            if ana:
                ...
                MedMongo("pt_v3").update_table("sg_analysis_status", "batch_id", ObjectId(board['_id']), analysis_data)
        if data.project_types in ['all', 'nipt']:
            ana = MedMongo("pt_v3").get_one_bymore('sg_analysis_status', {'batch_id': ObjectId(board['_id']),
                                                                          'type': 'nipt'})
            if ana:
                analysis_data = {
                    "member_id": data.member_id,
        ...
        if self.datatype != "iswailai":
            ...
            if root.find("IsPairedEnd").text == 'true':
                ...
                return json.dumps(info)
            MedMongo("pt_v3").update_table('sg_datasplit', 'board_path', board_batch_path, {'split_type': split_type})
        # 调用WPM运行数据拆分
        task_name = 'medical.data_split_v3.data_split'
        options = {
        ...
        if self.datatype == "iswailai":
            options.update({
                "aliyun_path": data.aliyun_path,
                "datatype": self.datatype
            })
        self.set_sheet_data_(name=task_name, options=options, module_type="workflow", db_type='pt_v3',
    ...
    def check_can_split(self, board_batch):
        ...
        :return:
        """
        board_one = "/mnt/ilustre/upload/nextseq1/" + board_batch
        board_two = "/mnt/ilustre/upload/nextseq/" + board_batch
        board_three = "/mnt/ilustre/users/yixuezhuanhua/raw-data/" + board_batch
        board_four = "/mnt/clustre/upload/nextseq1/" + board_batch
        board_five = "/mnt/clustre/upload/nextseq/" + board_batch
        board_six = "/mnt/clustre/users/yixuezhuanhua/raw-data/" + board_batch
        if os.path.exists(board_one):
            ...
            if not os.path.exists(board_two + "/RTAComplete.txt") or not os.path.isfile(board_two + "/RTAComplete.txt"):
                info = {'success': False, 'info': '板子{}还没有完全下机不能进行拆分！'.format(board_batch)}
                return json.dumps(info)
        elif os.path.exists(board_three):
            if not os.path.exists(board_three + "/RTAComplete.txt") or not \
                    os.path.isfile(board_three + "/RTAComplete.txt"):
                info = {'success': False, 'info': '板子{}还没有完全下机不能进行拆分！'.format(board_batch)}
                return json.dumps(info)
        elif os.path.exists(board_four):
            if not os.path.exists(board_four + "/RTAComplete.txt") or not \
                    os.path.isfile(board_four + "/RTAComplete.txt"):
                info = {'success': False, 'info': '板子{}还没有完全下机不能进行拆分！'.format(board_batch)}
                return json.dumps(info)
        elif os.path.exists(board_five):
            if not os.path.exists(board_five + "/RTAComplete.txt") or not \
                    os.path.isfile(board_five + "/RTAComplete.txt"):
                info = {'success': False, 'info': '板子{}还没有完全下机不能进行拆分！'.format(board_batch)}
                return json.dumps(info)
        elif os.path.exists(board_six):
            if not os.path.exists(board_six + "/RTAComplete.txt") or not \
                    os.path.isfile(board_six + "/RTAComplete.txt"):
                info = {'success': False, 'info': '板子{}还没有完全下机不能进行拆分！'.format(board_batch)}
                return json.dumps(info)
        elif self.datatype == "iswailai":
            pass
        else:
