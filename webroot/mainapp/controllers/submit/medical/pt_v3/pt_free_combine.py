...
class PtFreeCombineAction(PtController):
    ...
    def POST(self):
        ...
        task_name = 'medical.paternity_test_v3.report.pt_free_combine'
        task_type = 'workflow'
        params_json = {
            ...
            'dp_correction': data.dp_correction if hasattr(data, "dp_correction") else 'false'
        }
        ...
        if str(data.is_report) == "true":
            ...
            if hasattr(data, "dp_correction") and data.dp_correction == "true":
                remark = "{}改为{};{}改为{};{}_{}改为" \
                         "{}".format(data.case_id + '-' + data.dad_id, data.new_dad_id, data.mom_id,
                                     data.new_mom_id, data.preg_id, data.mom_id, data.new_preg_id)
                task_id += "_dp_correction"
            # dad_id = "{}-{}".format(data.case_id, data.dad_id)
            # mom_id = data.mom_id
            preg_id = data.preg_id
            dad_id = data.new_dad_id
            mom_id = data.new_mom_id
        ...
        main_table_id = PT('pt_v3').add_main_id('sg_free_combine')
        ...
        self.set_sheet_data_(name=task_name, options=options, module_type=task_type, params=params, db_type='pt_v3')
    ...
    def check_options(self, data):
        ...
        :return: 检查结果
        """
        params_name = ['mom_id', 'case_id', 'preg_id', 'err_min', 'dad_id', 'member_id', 'is_report', "dp_correction"]
        success = []
