...
class WgcnaNetworkAction(RefRnaController):
    def __init__(self):
        super(WgcnaNetworkAction, self).__init__(instant=False)
        self.task_name = 'ref_rna.report.wgcna_network'
    ...
    def check_params(self):
        ...
        if len(self.input_data.module) < 1:
            info = {'success': False, 'info': "module: {}".format("选择的模块为空")}
            return json.dumps(info)
        return True
    ...
    def create_main_table(self, packed_params, db_name):
        result_info = self.ref_rna.get_main_info(self.input_data.wgcna_relate_id, 'sg_wgcna_relate')
        ...
        main_info = dict(
            ...
            status="start")
        main_id = self.ref_rna.insert_main_table(db_name, main_info)
        return main_id, name

    def prepare_workflow_options(self, main_id, db_name, main_table_name):
        relate_output = self.ref_rna.get_main_info(self.input_data.wgcna_relate_id,'sg_wgcna_relate')['output_dir']
        if not relate_output.endswith("/"):
            relate_output += "/"
        module_output = self.ref_rna.get_main_info(self.wgcna_module_id, 'sg_wgcna_module')['output_dir']
