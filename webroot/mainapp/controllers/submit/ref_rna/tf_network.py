...
class TfNetworkAction(RefRnaController):
    ...
    def __init__(self):
        ...
        self.task_name = 'ref_rna.report.tf_network'
    ...
    def check_params(self):
        ...
        # do some other check
        main_info = self.get_main_info(self.input_data.tfbs_predict_id, 'sg_tfbs_predict')
        ...
        if pre_thresh < float(self.input_data.thresh):
            info = {'success': False, 'info': "FIMO对应的pvalue阈值不能大于上次TFBS预测时使用的阈值{}".format(pre_thresh)}
            return json.dumps(info)
        if pre_corr_cutoff > float(self.input_data.corr_cutoff):
            info = {'success': False, 'info': "相关系数的阈值不能小于上次TFBS预测时使用的阈值{}".format(pre_corr_cutoff)}
            return json.dumps(info)
        if pre_corr_pvalue < float(self.input_data.corr_pvalue):
            info = {'success': False, 'info': "相关系数的pvalue阈值不能大于上次TFBS预测时使用的阈值{}".format(pre_corr_pvalue)}
            return json.dumps(info)
    ...
    def prepare_workflow_options(self, main_id, collection_name, main_table_name):
        ...
        # prepare to file
        to_files = ["ref_rna.export_predict_result(tfbs_predict)"]
