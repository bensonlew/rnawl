...
class TfStatAction(RefRnaController):
    ...
    def check_params(self):
        ...
        main_info = self.get_main_info(self.input_data.tf_predict_id, 'sg_tf_predict')
        pre_evalue = float(json.loads(main_info['params'])['E'])
        if pre_evalue < float(self.input_data.e_value):
            info = {'success': False, 'info': "evalue阈值不能大于上次TF预测时使用的阈值{}".format(pre_evalue)}
            return json.dumps(info)
        # do some other check
        '''
    ...
    def prepare_workflow_options(self, main_id, collection_name, main_table_name):
        ...
        # prepare to file
        # to_files = ["ref_rna.YOUR_tofile_Func(tofile_Param)", ]
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        args = dict(
            ...
            geneset_id="all",
            families="all",
            tf_predict_id="5af114ffa4e1af1ee6f998eb",
            e_value="0.00001",
        )
