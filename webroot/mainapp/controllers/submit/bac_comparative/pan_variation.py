...
class PanVariationAction(BacComparativeController):
    def __init__(self):
        super(PanVariationAction, self).__init__(instant=False)
    ...
    def POST(self):
        ...
        task_name = 'bac_comp_genome.report.pan_variation'
        ...
        main_table_name = "Pan_mutation_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S%f")[:-3]
        update_info = {}
        mongo_data = [
        ...
        main_id = self.bac_comparative.insert_main_table('pan_var', mongo_data)
        update_info[str(main_id)] = 'pan_var'
        ...
        self.set_sheet_data(name=task_name,
                            options=options,
                            main_table_name="Pan_mutation/" + main_table_name,
                            module_type=module_type,
                            project_sn=project_sn,
                            task_id=data.task_id,
                            params=params)
        task_info = super(PanVariationAction, self).POST()
