...
class HclusterAction(BacComparativeController):
    def __init__(self):
    ...
    def POST(self):
        data = web.input()
        default_argu = ['task_id', 'function', 'submit_location', 'task_type', 'group_detail', 'group_id','file_path', "dis_method", "hcluster_method", "is_group"]
        ...
        main_table_name = 'Hcluster_' + data.function + "_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S%f")[:-3]
        update_info = {}
        mongo_data = [
        ...
        main_id = self.bac_comparative.insert_main_table('hcluster', mongo_data)
        update_info[str(main_id)] = 'hcluster'
        ...
        self.set_sheet_data(name=task_name,
                            options=options,
                            main_table_name="HCLUSTER/" + main_table_name,
