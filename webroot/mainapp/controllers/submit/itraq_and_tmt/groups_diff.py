...
class GroupsDiffAction(ItraqTmtController):
    def __init__(self):
    ...
    def POST(self):
        data = web.input()
        print("groups_diff controller input data:"+str(data))
        ...
        main_id = self.itraq_tmt.insert_main_table('sg_groups_diff', main_info)
        ...
        # prepare to file
        to_files = ["itraq_tmt.export_exp_matrix2(protein_table)",
                    "itraq_tmt.export_group(group)"]

        # 把参数交给workflow运行相应的tool， 其中to_file用于准备tool的输入文件
        task_name = 'itraq_and_tmt.report.groups_diff'
