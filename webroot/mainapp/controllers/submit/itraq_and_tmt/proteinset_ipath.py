...
class ProteinsetIpathAction(ItraqTmtController):
    def __init__(self):
    ...
    def POST(self):
        ...
        for argu in default_argu:
            if not hasattr(data, argu):
                info = {'success': False, 'info':"Lack argument: %s", "variables":[argu], "code" : "C1901201"}
                return json.dumps(info)

        task_name = 'itraq_and_tmt.report.proteinset_ipath'
        params_json = {
        ...
        for proteinset in data.proteinset_id.split(","):
            ...
            if not proteinset_info:
                info = {"success": False, "info": "proteinset not exist", "code" : "C1901202"}
                return json.dumps(info)
        if len(data.proteinset_id.split(",")) > 2:
            info = {"success": False, "info": "Select a maximum of two protein sets!", "code" : "C1901203"}
            return json.dumps(info)
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/itraq_and_tmt/proteinset_ipath "
