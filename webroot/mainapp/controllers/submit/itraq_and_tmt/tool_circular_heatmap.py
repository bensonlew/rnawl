...
class ToolCircularHeatmapAction(ItraqTmtController):
    def __init__(self):
    ...
    def POST(self):
        ...
        basic_args += ['group_dict', 'proteinset_id', 'tool_type']
        # check arg
        ...
        main_id = self.itraq_tmt.insert_main_table('sg_tool_lab_circular_heatmap', main_info)
        # prepare option for workflow
        options = {
            "group_dict": data.group_dict,
            "metab_table": data.proteinset_id,
            'tool_type': data.tool_type,
            'params': params,
            "main_id": str(main_id),
        ...
        to_files = ["itraq_tmt_tool.export_proteinset_exp(metab_table)"]
        task_name = 'itraq_and_tmt.report.tool_circular_heatmap'
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/itraq_and_tmt/tool_circular_heatmap "
        cmd += "-b http://wpm2.sanger.com "
        args = dict(
            task_id="jssn_svfguokp3g35cbuue5v31s",
            task_type="2",
            submit_location="CircularHeatmap",
            group_dict=r'{"DMSO": ["DMSO_1", "DMSO_2"], "CG3556": ["CG3556_1", "CG3556_2"]}'.replace('"', '\\"'),
            proteinset_id='60f52cc5a511db2424dda5e1',
