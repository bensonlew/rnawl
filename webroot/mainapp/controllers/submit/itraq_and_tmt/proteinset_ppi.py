...
class ProteinsetPpiAction(ItraqTmtController):
    """
    蛋白质互作网络的接口
    """
    ...
    def POST(self):
        ...
        for argu in default_argu:
            if not hasattr(data, argu):
                info = {'success': False, 'info':"Lack argument: %s", "variables":[argu], "code" : "C1901401"}
                return json.dumps(info)
        if str(data.species) == '' or str(data.species) == "0":
            info = {"success": False, "info": "Please select reference species!", "code" : "C1901402"}
            return json.dumps(info)
        '''
        if int(data.species) not in self.species_list:
            ...
            return json.dumps(info)
        '''
        task_name = 'itraq_and_tmt.report.proteinset_ppi'
        task_type = 'workflow'
        params_json = {
        ...
        if not proteinset_info:
            info = {"success": False, "info": "proteinset not exist", "code" : "C1901403"}
            return json.dumps(info)
        ...
        if task_info.has_key('protein_fa'):
            ...
            info = {"success": False, "info": "The sequence file could not be found!", "code" : "C1901404"}
            return json.dumps(info)
        ...
        main_table_id = self.itraq_tmt.insert_main_table('sg_proteinset_ppi', mongo_data)
        ...
        to_file = "itraq_tmt.export_protein_list_ppi(proteinset_list)"
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/itraq_and_tmt/proteinset_ppi "
