...
class WgcnaNetworkAction(ItraqTmtController):
    def __init__(self):
        super(WgcnaNetworkAction, self).__init__(instant=False)
        self.task_name = 'itraq_and_tmt.report.wgcna_network'
        self.expected_args = ["task_id", "submit_location", 'task_type']
        self.expected_args += ['module', 'wgcna_relate_id', 'threshold', 'top']
    ...
    def create_main_table(self, packed_params, db_name):
        result_info = self.itraq_tmt.get_main_info(self.input_data.wgcna_relate_id, 'sg_wgcna_relate', self.input_data.task_id)
        ...
        main_info = dict(
            ...
            status="start")
        main_id = self.itraq_tmt.insert_main_table(db_name, main_info)
        return main_id, name

    def prepare_workflow_options(self, main_id, db_name, main_table_name):
        relate_output = self.itraq_tmt.get_main_info(self.input_data.wgcna_relate_id,'sg_wgcna_relate', self.input_data.task_id)['output_dir']
        if not relate_output.endswith('/'):
            relate_output += '/'
        module_output = self.itraq_tmt.get_main_info(self.wgcna_module_id, 'sg_wgcna_module', self.input_data.task_id)['output_dir']
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/itraq_and_tmt/wgcna_network "
        cmd += "-b http://bcl.tsg.com "
        args = dict(
            task_id="tsg_34739",
            task_type="2",
            submit_location="WgcnaNetwork",
            wgcna_relate_id="5d25b09f17b2bf759fca506e",
            module='red,blue',
            threshold='0.03',
            top='30',
        )
