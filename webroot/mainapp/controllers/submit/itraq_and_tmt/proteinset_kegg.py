...
class ProteinsetKeggAction(ItraqTmtController):
    def __init__(self):
    ...
    def POST(self):
        ...
        for argu in default_argu:
            if not hasattr(data, argu):
                info = {'success': False, 'info':"Lack argument: %s", "variables":[argu], "code" : "C1901301"}
                return json.dumps(info)

        task_name = 'itraq_and_tmt.report.proteinset_kegg'
        params_json = {
        ...
        if len(data.proteinset_id.split(",")) >2:
            info = {"success": False, "info": "Only one or two protein sets are supported!", "code" : "C1901302"}
            return json.dumps(info)

        for proteinset in data.proteinset_id.split(","):
            proteinset_info = self.itraq_tmt.get_main_info(proteinset, 'sg_proteinset', data.task_id)
            if not proteinset_info:
                info = {"success": False, "info": "proteinset not exist", "code" : "C1901303"}
                return json.dumps(info)
        ...
        to_file = ['itraq_tmt.export_multi_protein_list(proteinset_kegg)',
                   "itraq_tmt.export_kegg_table(kegg_table)",
                   "itraq_tmt.export_kegg_level_table(kegg_table_2)",
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/itraq_and_tmt/proteinset_kegg "
