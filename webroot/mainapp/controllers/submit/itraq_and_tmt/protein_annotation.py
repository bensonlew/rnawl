...
class ProteinAnnotationAction(ItraqTmtController):
    """
    蛋白注释重运行接口
    """
    def __init__(self):
    ...
    def POST(self):
        ...
        if return_result:
            info = {"success": False, "info": return_result, "code" : "C1900601"}
            return json.dumps(info)
        stat_info = self.itraq_tmt.get_main_info_by_record("sg_annotation_stat", task_id=str(data.task_id), type="origin")
        if not stat_info:
            info = {"success": False, "info": "stat_id not exist", "code" : "C1900602"}
            return json.dumps(info)
        print stat_info

        run_info = self.itraq_tmt.get_main_info_by_record("sg_annotation_stat", task_id=str(data.task_id), status="start", type="latest")
        if run_info:
            info = {"success": False, "info": "task %s is running，please wait for it to complete before running again", "variables":[run_info['name']], "code" : "C1900603"}
            return json.dumps(info)

        latest_info = self.itraq_tmt.get_main_info_by_record("sg_annotation_stat", task_id=str(data.task_id), status="end",  type="latest")
        latest_main_id = ""
        ...
        main_table_id = self.itraq_tmt.insert_main_table("sg_annotation_stat", mongo_data)
        ...
        to_file = []
        self.set_sheet_data(name="itraq_and_tmt.report.protein_annotation",
                            options=options,
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/itraq_and_tmt/protein_annotation "
