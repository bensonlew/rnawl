...
class WgcnaPipelineAction(ItraqTmtController):
    def __init__(self):
        super(WgcnaPipelineAction, self).__init__(instant=False)
        self.task_name = 'itraq_and_tmt.report.wgcna_pipeline'
    ...
    def POST(self):
        ...
        if 'group_id' in self.input_data and str(self.input_data.group_id).lower() != 'all':
            _ = self.itraq_tmt.update_group_is_use(self.input_data.task_id, self.input_data.group_id)
        if 'control_id' in self.input_data:
            _ = self.itraq_tmt.update_group_compare_is_use(self.input_data.task_id, self.input_data.control_id)
    ...
    def create_main_table(self, packed_params, collection_name):
        ...
        result_info = self.itraq_tmt.get_main_info(self.input_data.exp_id, 'sg_express', self.input_data.task_id)
        ...
        main_info = dict(
            ...
            status="start")
        main_id = self.itraq_tmt.insert_main_table(collection_name, main_info)
        if self.input_data.proteinset_id.lower() not in ["all", "none"]:
            self.itraq_tmt.insert_proteinset_info(self.input_data.proteinset_id, collection_name, str(main_id))
        return main_id, name
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/itraq_and_tmt/wgcna_pipeline "
        cmd += "-b http://bcl.tsg.com "
        args = dict(
            task_id="tsg_36324",
            task_type="2",
            exp_id='5de730a617b2bf1bd53ac3b4',
            group_id='5de7309217b2bf1bd53aafe7',
            group_dict=json.dumps({"DGZ":["DGZ25_30","DGZ25_31","DGZ5_8"],"DSZ":["DSZ22_9","DSZ26_32","DSZ28_27"],"MGZ":["MGZ10_1","MGZ17_28","MGZ1_11"],"MSZ":["MSZ12_17","MSZ7_4","MSZ7_5"]}).replace(
                '"', '\\"'),
            submit_location="wgcnapipeline",
            proteinset_id="5de78883c6598d481f8b4569",
            me='0',
            cv='0',
            mergeCutHeight="0.45",
            power="7",
            ...
            networkType="signed",
            minKMEtoStay="0.5",
            trait_path="/mnt/ilustre/users/sanger-dev/sg-users/deqing/projects_test_input/traits_fyt_itraq_.xls",
            trait_type="discrete",
            corr_method="pearson",
            file_id="6919032",
            threshold="0.03",
