...
class DeCarrierAction(BasicMolecularController):
# class BacIdentityAction(BacidentityController):
    ...
    def POST(self):
        ...
        result = Datasplit("basic_molecular").coll_find_one("sg_decarrier", {"task_id": data.tid})
        iid = result["_id"]
        update_info = {str(iid):"sg_decarrier"}
        options = {"update_info": json.dumps(update_info), "main_id": str(iid)}
        options["list_file"] = str(iid)
        to_file = ["basic_molecular.export_decarrier_params(list_file)"]
        self.set_sheet_data(name="basic_molecular.de_carrier", options=options, table_id=data.tid,to_file=to_file, seq_number="de_carrier", module_type="workflow")
        task_info = super(DeCarrierAction, self).POST()
        update_dict = {
                # "statue": split_status,
                "status": "start",
                "desc":"任务开始运行",
        ...
        Datasplit("basic_molecular").update_db_record("sg_decarrier", {"_id": ObjectId(iid)}, update_dict)
