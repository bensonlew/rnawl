...
class ExpDiffv2Action(MetabolomeController):
    def __init__(self):
        super(ExpDiffv2Action, self).__init__(instant=False)
    ...
    def POST(self):
        ...
        task_name = 'metabolome.report.exp_diffv2'
        ...
        for eachgroup in group_detail.keys():
            ...
            if sam_len >= 4:
                group_len4 += 1
        # if data.test_method in ["t-test", "welch", "t-test-paired", "welch-paired",]:
        #     if not len(group_detail) == grouo_len3:
        #         info = {'success': False, 'info': 't-test检验和welch差异检验方法时每组样本数需不少于3!', 'code':'C2300503'}
        #         return json.dumps(info)
        # elif data.test_method in ["wilcox", "wilcox-paired"]:
        ...
        for each in [data.plsda_replace, data.oplsda_replace]:
            if not 19 < int(each) < 1001:
                ...
                return json.dumps(info)
        if not data.test_method in ["t-test", 'welch', 'wilcox',"t-test-paired", 'welch-paired', 'wilcox-paired']:
        ...
        if hasattr(data, 'paired_id'):  #paried_id 是 specimen_group_paired的_id
            params_json['paired_id'] = str(data.paired_id)
            metabolome.update_mongo('specimen_group_paired',{"_id":ObjectId(data.paired_id)}, {"is_use":1})
        ...
        to_file = []
        if hasattr(data, 'paired_id'):
            options['paired_id'] = data.paired_id
            to_file.append('metabolome.export_paired_sample(group)')
        else:
        ...
        task_info = super(ExpDiffv2Action, self).POST()
