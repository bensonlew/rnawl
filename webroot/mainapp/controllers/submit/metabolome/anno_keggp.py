...
class AnnoKeggpAction(MetabolomeController):
    def __init__(self):
        super(AnnoKeggpAction, self).__init__(instant=False)

    @check_sig
    def POST(self):
        data = web.input()
        for arg in ['metab_table', 'organism', 'task_id', 'submit_location', 'task_type']:  # table_type
            if not hasattr(data, arg):
                info = {'success': False, 'info': 'Argument:{} required'.format(arg)}
                return json.dumps(info)
        task_name = 'metabolome.report.anno_keggp'
        # print data.organism
        ...
        main_table_name = "Anno_keggp_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S%f")[:-3]
        ...
        #v2 = metabolome.conmon_find_one('sg_task', {"task_id": data.task_id, "version": "2.0"})
        ...
        if  'version' in v2  and float(v2['version'])>1:
            ...
            if hasattr(data, 'table_type'):
                ...
                table_type = 'pos'
        main_table_id = metabolome.insert_main_table('anno_keggp', mongo_data)
        metabolome.insert_main_id('anno_keggp', main_table_id)
        update_info = {str(main_table_id): 'anno_keggp'}
        options = {
        ...
        if "save_pdf" in task_info and int(task_info["save_pdf"]) == 1:
            options["save_pdf"] = 1
            m_table_name = '3.Anno/02.AnnoKeggp/' + main_table_name
        else:
        ...
        post_info = super(AnnoKeggpAction, self).POST()
