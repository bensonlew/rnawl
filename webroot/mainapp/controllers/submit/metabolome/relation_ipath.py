...
class RelationIpathAction(MetabolomeController):
    def __init__(self):
        super(RelationIpathAction, self).__init__(instant=False)

    @check_sig
    def POST(self):
        data = web.input()
        args = ['metab_set', 'gene_set', 'metab_annot', 'gene_annot', 'task_id', 'submit_location', 'task_type']
        for arg in args:
            if not hasattr(data, arg):
                info = {'success': False, 'info': 'Argument:{} required'.format(arg)}
                return json.dumps(info)
        task_name = 'metabolome.report.relation_ipath'
        ...
        main_table_name = "RelationIpath_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S%f")[:-3]
        ...
        main_table_id = metabolome.insert_main_table('relation_ipath', mongo_data)
        ...
        metabolome.insert_main_id('relation_ipath', main_table_id)
        update_info = {str(main_table_id): 'relation_ipath'}
        options = {
        ...
        post_info = super(RelationIpathAction, self).POST()
        if post_info['success']:
            post_info['content'] = {'ids': {'id': str(main_table_id), 'name': main_table_name}}
        self.Metabolome.insert_set_info(data.metab_set, "relation_ipath", main_table_id)
