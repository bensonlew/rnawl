...
class GenesetBatchAction(DenovoRnaV2Controller):
    def __init__(self):
    ...
    def POST(self):
        ...
        diff_info = self.denovo_rna_v2.get_main_info(input_data.diff_id, "sg_diff", input_data.task_id)
        ...
        flag = int(self.denovo_rna_v2.get_geneset_batch_flag(input_data.task_id))
        if "is_batch" in params:
        ...
        geneset_names = self.denovo_rna_v2.get_genesets(input_data.task_id, geneset_type, is_use=1)
        ...
        main_id = self.denovo_rna_v2.insert_main_table('sg_geneset_batch', main_info)
        new_task_id = self.denovo_rna_v2.get_new_id(input_data.task_id)
        ...
        # prepare to file
        # to_files = ["denovo_rna_v2.export_diff_genesets(genesets)"]

        # 把参数交给workflow运行相应的tool， 其中to_file用于准备tool的输入文件
        task_name = 'denovo_rna_v3.report.geneset_batch'
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/denovo_rna_v3/geneset_batch "
        cmd += "-b http://bcl.tsg.com "
        args = dict(
            task_id="tsg_219005",
            task_type="2",
            submit_location="geneset_batch",
            diff_id='5f6d4d9c17b2bf14b6c92bce'
        )
