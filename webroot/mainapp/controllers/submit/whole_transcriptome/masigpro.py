...
class MasigproAction(WholeTranscriptomeController):
    ...
    def POST(self):
        ...
        args = ['task_id', 'submit_location', 'task_type', 'level',
                'design', 'cluster', 'method', 'design_id', 'geneset_id', 'category']
        for arg in args:
        ...
        way = self.whole_transcriptome.db['exp'].find_one({'main_id': ObjectId(exp_id)})['way']
        options = {
            'matrix': exp_id,
            'geneset': data.geneset_id + "," + data.category + "," + data.level,
            'design': design,
            'cluster': cluster,
            'method': data.method,
            'level': data.level,
        ...
        to_files = [
            ...
            'whole_transcriptome.whole_transcriptome.export_masigpro_matrix(matrix)'
        ]
        self.set_sheet_data(
