...
class WgcnaRelateAction(WholeTranscriptomeController):
    def __init__(self):
    ...
    def POST(self):
        ...
        main_info = self.whole_transcriptome.get_main_info(data.wgcna_module_id, 'wgcna_module', data.task_id)
        ...
        module_info = self.whole_transcriptome.get_main_info_by_record('wgcna_module', main_id=wgcna_module_id,
                                                                       task_id=data.task_id)
        module_rdata = os.path.join(module_info["output_dir"], 'blockwiseModules_result.RData')

        name = "Relate" + '_' + module_info.get("category", "mRNA") + '_'
        ...
        options = {
            ...
            "level": data.level,
            "update_info": json.dumps({str(main_id): "wgcna_relate"})  # to update status
        }
        # prepare to file
        to_files = ["whole_transcriptome.advance.export_wgcna_relate_input(exp_eigengenes)"]

        # 把参数交给workflow运行相应的tool， 其中to_file用于准备tool的输入文件
        task_name = 'whole_transcriptome.report.wgcna_relate'
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/whole_transcriptome/wgcna_relate "
        cmd += "-b http://bcl.tsg.com "
        args = dict(
            task_id="tsg_36088",
            task_type="2",
            submit_location="wgcna_relate",
            wgcna_module_id="5ddcf68017b2bf364af24eb1",
            level="G",
