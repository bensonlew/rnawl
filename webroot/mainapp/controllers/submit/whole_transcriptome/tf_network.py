...
class TfNetworkAction(WholeTranscriptomeController):
    ...
    def __init__(self):
        ...
        self.task_name = 'whole_transcriptome.report.tf_network'
    ...
    def check_params(self):
        ...
        # do some other check
        main_info = self.get_main_info(self.input_data.tfbs_predict_id, 'sg_tfbs_predict', self.input_data.task_id)
        ...
        if pre_thresh < float(self.input_data.thresh):
            ...
            info = {'success': False, 'info': "FIMO对应的pvalue阈值不能大于上次TFBS预测时使用的阈值%s" % pre_thresh, 'code': 'C2902902',
                    'variables': variables}
            return json.dumps(info)
        if pre_corr_cutoff > float(self.input_data.corr_cutoff):
            variables = []
            variables.append(pre_corr_cutoff)
            info = {'success': False, 'info': "相关系数的阈值不能小于上次TFBS预测时使用的阈值%s" % pre_corr_cutoff, 'code': 'C2902903',
                    'variables': variables}
            return json.dumps(info)
        if pre_corr_pvalue < float(self.input_data.corr_pvalue):
            variables = []
            variables.append(pre_corr_pvalue)
            info = {'success': False, 'info': "相关系数的pvalue阈值不能大于上次TFBS预测时使用的阈值%s" % pre_corr_pvalue, 'code': 'C2902904',
                    'variables': variables}
            return json.dumps(info)
        '''
    ...
    def prepare_workflow_options(self, main_id, collection_name, main_table_name):
        ...
        # prepare to file
        to_files = ["whole_transcriptome.export_predict_result(tfbs_predict)"]
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/whole_transcriptome/tf_network "
        cmd += "-b http://192.168.12.102:9090 "
        args = dict(
            task_id="tsg_29268",
            task_type="2",  # maybe you need to change it
            submit_location="tfnetwork",
            tfbs_predict_id="5b02868aa4e1af73f824c705",
            thresh="0.0001",
            qv_thresh="0",
            corr_pvalue="0.05",
            corr_cutoff="0.5",
