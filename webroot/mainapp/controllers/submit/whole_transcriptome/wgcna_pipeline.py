...
class WgcnaPipelineAction(WholeTranscriptomeController):
    def __init__(self):
        ...
        self.expected_args += ["category", 'group_id', 'group_dict', 'me', 'geneset_id', 'level', 'cv']
    ...
    def check_params(self):
        ...
        if os.path.exists(trait_path):
            pass
        elif not self.input_data.trait_path:
            info = {'success': False, 'info': "表型数据表路径为空"}
            return json.dumps(info)
            ...
            info = {'success': False, 'info': "表型数据表路径错误{}".format(self.input_data.trait_path)}
            return json.dumps(info)
    ...
    def create_main_table(self, packed_params, collection_name):
        ...
        exp_type = "RSEM"
        try:
            quant_method = self.exp_info["way"]
        except:
            quant_method = "tpm"
        # exp_info["method"]
    ...
    def prepare_workflow_options(self, main_id, collection_name, main_table_name):
        ...
        options.update({
            ...
            "main_id": str(main_id),
            "exp_id": str(self.exp_info['main_id']),
            "exp_matrix": ";".join([str(self.exp_info['main_id']), self.input_data.geneset_id, self.input_data.category,
                                    self.input_data.level]),
            "gene_name": str(self.exp_info['main_id']) + ";" + self.input_data.geneset_id,
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/whole_transcriptome/wgcna_pipeline "
