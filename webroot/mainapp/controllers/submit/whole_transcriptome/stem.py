...
class StemAction(WholeTranscriptomeController):
    ...
    def POST(self):
        ...
        args = ['task_id', 'submit_location', 'task_type', 'level', 'pheno_file', 'method', 'pheno_id', 'category']
        for arg in args:
        ...
        options = {
            'matrix': exp_id,
            'pheno': pheno_file,
            'geneset': data.geneset_id + "," + data.category + "," + data.level + "," + str(exp_id),
            'method': data.method,
            'main_id': str(main_id),
        ...
        to_files = [
            ...
            'whole_transcriptome.whole_transcriptome.export_stem_matrix(matrix)'
        ]
        self.set_sheet_data(
