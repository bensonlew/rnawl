...
class TfbsPredictAction(WholeTranscriptomeController):
    ...
    def __init__(self):
        ...
        self.task_name = 'whole_transcriptome.report.tfbs_predict'
    ...
    def check_params(self):
        ...
        table_info = self.whole_transcriptome.get_main_info(self.input_data.tf_predict_id, 'tf_predict',
    ...
    def prepare_workflow_options(self, main_id, collection_name, main_table_name, genome_id):
        ...
        # prepare to file
        to_files = [
            "whole_transcriptome.advance.export_geneid2tfid_file(tf_predict_id)",
            "whole_transcriptome.advance.get_gene_detail(seq2name)",
        ]
        if 'all' not in self.input_data.geneset_id.lower():
            to_files.append("whole_transcriptome.advance.export_gene_list(geneset_list)")
        else:
            pass
        ...
        if self.input_data.exp_id.lower() != "none":
            options['exp_matrix'] = self.input_data.exp_id + ";all;mRNA;G"
            to_files.append("whole_transcriptome.advance.export_geneset_exp_matrix(exp_matrix)", )
        else:
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/whole_transcriptome/tfbs_predict "
        ...
        args = dict(
            ...
            seqdb="None",
            seqdb_file_id="",
            exp_id="5dd5167617b2bf7328fad45d",
            group_id="all",
            group_dict=json.dumps({"A": ["A1", "A2"], "C": ["C2", "C3"], "S": ["S1", "S3"]}).replace('"', '\\"'),
            tf_predict_id="5ddb92a317b2bf704845e814",
            thresh="0.0001",
            qv_thresh="0",
            corr_cutoff="0.5",
            corr_pvalue="0.05",
