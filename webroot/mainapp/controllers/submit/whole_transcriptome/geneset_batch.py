...
class GenesetBatchAction(WholeTranscriptomeController):
    def __init__(self):
    ...
    def POST(self):
        ...
        diff_info = self.whole_transcriptome.get_main_info(input_data.diff_id, "diff", input_data.task_id)
        diff_name = diff_info["name"]
        project_sn = diff_info["project_sn"]
        geneset_type = diff_info["level"]
        category = diff_info["category"]
        ...
        flag = int(self.whole_transcriptome.get_geneset_batch_flag(input_data.task_id))
        ...
        geneset_names = self.whole_transcriptome.get_genesets(input_data.task_id, geneset_type, is_use=1)
        for cmp in cmp_detail:
            if geneset_suffix:
                geneset_name = geneset_type + "_" + category + "_" + cmp.replace("|", "_vs_") + geneset_suffix
            else:
                geneset_name = geneset_type + "_" + category + "_" + cmp.replace("|", "_vs_")
        ...
        main_id = self.whole_transcriptome.insert_main_table('geneset_batch', main_info)
        new_task_id = self.whole_transcriptome.get_new_id(input_data.task_id)
        ...
        options = {
            ...
            "update_info": json.dumps({str(main_id): "geneset_batch"})  # to update sg_status
        }

        # 把参数交给workflow运行相应的tool， 其中to_file用于准备tool的输入文件
        task_name = 'whole_transcriptome.report.geneset_batch'
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/whole_transcriptome/geneset_batch "
        cmd += "-b http://bcl.tsg.com "
        args = dict(
            task_id="tsg_36458",
            task_type="2",
            submit_location="geneset_batch",
            diff_id='5df8982217b2bf4a63778e03'
        )
