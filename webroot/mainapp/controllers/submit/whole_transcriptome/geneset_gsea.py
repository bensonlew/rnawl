...
class GenesetGseaAction(WholeTranscriptomeController):
    ...
    def POST(self):
        ...
        required = ['preranked', 'submit_location', 'task_type', 'task_id', 'group_id', 'group_dict', 'geneset_source',
                    'task_id', 'min_num', 'max_num', 'level', 'geneset_source']
        ...
        if hasattr(data, "preranked") and data.preranked == 'true':
            ...
            if len(group_dict.keys()) != 2:
                ...
                if min([len(x) for x in group_dict.values()]) < 2:
                    ...
                    return json.dumps(error)
        w_options['matrix'] = ";".join([str(self.exp_info['main_id']), data.geneset_id, 'mRNA', data.level])
        if len(group_dict.keys()) == 2:
        ...
        to_file.append('whole_transcriptome.advance.export_geneset_exp_matrix(matrix)')
    ...
    class TestFunction(unittest.TestCase):
        ...
        def test_this(self):
            ...
            args = dict(
                ...
                geneset_type="G",
                group_id="all",
                group_dict=json.dumps({"A": ["A1", "A2"], "C": ["C2", "C3"]}).replace('"', '\\"'),
