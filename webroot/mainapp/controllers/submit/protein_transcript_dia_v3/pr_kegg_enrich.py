...
class PrKeggEnrichAction(ProteinTranscriptDiaV3Controller):
    def __init__(self):
        super(PrKeggEnrichAction, self).__init__(instant=False)
    ...
    def POST(self):
        ...
        main_info = dict(
            ...
            desc='the kegg enrich of one proteinset and one geneset',
            params=params,
            status="start",
            version='v2.2'
        )
        main_id = self.dia.insert_main_table('sg_pr_kegg_enrich', main_info)
        ...
        options = dict(
            ...
            update_info=json.dumps({str(main_id): "sg_pr_kegg_enrich"})  # to update sg_status
        )


        # prepare to file
        to_files = ["protein_transcript_dia_v3.export_proteinsets_list(protein_list)",
                    "protein_transcript_dia_v3.export_genesets_list(gene_list)",
                    "protein_transcript_dia_v3.export_kegg_table_rna(gene_kegg)",
                    "protein_transcript_dia_v3.export_add_info_rna(gene_info)",
                    "protein_transcript_dia_v3.export_kegg_table_protein(protein_kegg)",
                    "protein_transcript_dia_v3.export_add_info_protein(protein_info)",
                    ]
        # 把参数交给workflow运行相应的tool， 其中to_file用于准备tool的输入文件
        task_name = 'protein_transcript_dia_v3.report.pr_kegg_enrich'
        ...
        # 运行workflow 并传回参数
        task_info = super(PrKeggEnrichAction, self).POST()
        task_info['content'] = {
            'ids': {
                'id': str(main_id),
                'name': name
