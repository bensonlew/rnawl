...
from mainapp.models.mongo.prok_rna import *
from mbio.api.to_file.prok_rna import *
from bson.objectid import ObjectId
from collections import OrderedDict


class BatchAction(ProkRNAController):
    ...
    def POST(self):
        ...
        basic_args += ["exp_id", "has_batch", 'batch_method']
        ...
        group_dict = OrderedDict(zip(category_names, samples))
        ...
        try:
            ...
            count_file = ''
        try:
            exp_level = record_exp['exp_level']
        except:
            exp_level = ''
        try:
            result_dir = record_exp['result_dir']
        except:
            result_dir = ''
        name_batch = "ExpBatch" + '_' + quant_method + '_' + exp_type + '_'
        ...
        main_id_exp = self.prok_rna.insert_main_table('sg_exp', main_info_exp)

        name = 'Exp_batch_{}'.format(time_now.strftime('%Y%m%d_%H%M%S'))
        task_info = self.prok_rna.get_task_info(task_id=data.task_id)
        main_info_batch = {
            'task_id': data.task_id,
        ...
        main_id_batch = self.prok_rna.insert_main_table('sg_exp_batch', main_info_batch)
        ...
        # prepare to file
        to_files = ["prok_rna.export_exp_batch_matrix(exp_matrix)",
                    "prok_rna.export_group_detail(group_table)",
                    "prok_rna.export_exp_other_matrix(exp_other)"
                    ]
        task_name = 'prok_rna.report.batch'
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        args = dict(
            ...
            has_batch="True",
            batch_matrix='/mnt/ilustre/users/sanger-dev/sg-users/zoujiaxun/prok_rna_v3/batch/batch.txt',
            batch_method='combat'
