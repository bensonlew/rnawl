...
class WgcnaPipelineAction(WholeTranscriptomeController):
    def __init__(self):
        super(WgcnaPipelineAction, self).__init__(instant=False)
        self.task_name = 'whole_transcriptome.report.wgcna_pipeline'
        self.expected_args = ["task_id", "submit_location", 'task_type']
        self.expected_args += ["category", 'group_id', 'group_dict', 'me', 'geneset_id', 'level', 'exp_id', 'cv']
        ...
        self.collection = "wgcna_pipeline"
    ...
    def create_main_table(self, packed_params, collection_name):
        ...
        self.exp_dict = self.whole_transcriptome.get_main_info_by_record("exp", level=self.input_data.level,
                                                                      task_id=self.input_data.task_id, main_id=ObjectId(self.input_data.exp_id))
        self.is_rmbe = str(self.exp_dict['is_rmbe'])
        if 'is_rmbe' not in self.exp_dict or self.is_rmbe == 'false':
            self.exp_id = str(self.exp_dict['main_id'])
        if self.is_rmbe == 'true':
            self.exp_id = str(self.exp_dict['batch_main_id'])



        self.project_sn = self.exp_dict["project_sn"]
        '''
        exp_info = json.loads(self.exp_info['params'])
        exp_level = self.input_data.exp_level[0].upper()
        exp_type = exp_info["level"]
        '''
        exp_level = self.exp_dict["level"]
        exp_type = "RSEM"
        try:
            quant_method = self.exp_dict["way"]
        except:
            quant_method = "tpm"
        # exp_info["method"]
        # name = "WgcnaPipeline" + '_' + self.input_data.category + '_' + quant_method + '_' + exp_type.upper() + '_'
        name = "WgcnaPipeline" + '_' + self.input_data.category + '_'
        ...
        if self.input_data.geneset_id.lower() not in ["all", "refall", "none"]:
            self.whole_transcriptome.insert_geneset_info(self.input_data.geneset_id, collection_name, str(main_id))
        return main_id, name
    ...
    def prepare_workflow_options(self, main_id, collection_name, main_table_name):
        ...
        options.update({
            ...
            "exp_matrix": ";".join([self.exp_id, self.input_data.geneset_id, self.input_data.category,
                                    self.input_data.level, self.is_rmbe]),
            "gene_name": str(self.exp_id) + ";" + self.input_data.geneset_id,
        ...
        to_files = ["whole_transcriptome_v1_1.advance.export_geneset_exp_matrix(exp_matrix)",
                    "whole_transcriptome.advance.export_geneset_genes_name(gene_name)"
                    ]
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/whole_transcriptome_v1_1/wgcna_pipeline "
        cmd += "-b http://wpm2.sanger.com  "
        args = {
            'db_version': '0',
            'group_dict': json.dumps({"all":["R_F1","R_F2","R_F3","R_M1","R_M2","R_M3","S_F1","S_F2","S_F3","S_M1","S_M2","S_M3"]}).replace('"', '\\"'),
            'geneset_id': 'All',
            'networkType': 'signed',
            'threshold': '0.02',
            'trait_type': 'discrete',
            'cv': '0.1',
            'category': 'mRNA,lncRNA',
            'minModuleSize': '30',
            'task_type': '2',
            'top': '30',
            'mergeCutHeight': '0.25',
            'timestamp': '1627954178',
            'exp_id': '5fe95ff717b2bf100b4d53f4',
            'nonce': '8895',
            'power': '0',
            'minKMEtoStay': '0.3',
            'submit_location': 'wgcnapipeline',
            'trait_path': 's3://commonbucket/files/m_188/188_5fced081d6f85/tsg_248932/trait.txt',
            'file_id': '62073608',
            'amp': '',
            'me': '1',
            'machine_id': '0',
            'corr_method': 'spearman',
            'task_id': 'i7bo_hnrnh48l7qjohu7d6lioin',
            'level': 'T',
            'client': 'client03',
            'signature': '35305e22b597f491607970a657f02690bdf33cfc',
            'group_id': 'all'
        }
        # args = dict(
        #     task_id="tsg_36088",
        #     task_type="2",
        #     # exp_id='5da1457517b2bf7dbbe973df',
        ...
        #     networkType="signed",
        #     minKMEtoStay="0.5",
        #     trait_path="/mnt/ilustre/users/sanger-dev/sg-users/liubinxu/whole_rna/test.trait.txt",
        #     trait_type="continus",
        #     corr_method="pearson",
        #     file_id="6919032",
        #     threshold="0.03",
