...
class EnterotypeAction(MetagenomicController):
    def __init__(self):
        super(EnterotypeAction, self).__init__(instant=False)
    ...
    def POST(self):
        ...
        default_argu = ['submit_location', 'group_id', 'group_detail', 'geneset_id', 'anno_type',
                        'distance_method', 'method', 'task_type']
        ...
        if info != "":
            return json.dumps(info)
        task_name = 'metagenomic.report.enterotyping'
        ...
        main_table_name = EnterotypeAction.get_main_table_name(data.anno_type, level)
        ...
        main_table_id = self.metagenomic.insert_main_table('enterotype', mongo_data)
    ...
    def get_main_table_name(anno_type, level):
        ...
        return 'Enterotypes_%s%s_%s' % (database, level, time_now)
