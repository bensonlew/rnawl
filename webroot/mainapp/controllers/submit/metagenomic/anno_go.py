...
class AnnoGoAction(MetagenomicController):
    def __init__(self):
        super(AnnoGoAction, self).__init__(instant=False)
    ...
    def POST(self):
        ...
        for argu in default_argu:
            if not hasattr(data, argu):
                info = {"success": False, "info": "PARAMETERS MISSING: %s" , "variables": [argu], "code" : "C2404201"}
                return json.dumps(info)
        if not data.database == "go":
            info = {"success": False, "info": "PARAMETERS ERROR: wrong value of database (%s), pfam expected!" , "variables": [data.database], "code" : "C2404202"}
            return json.dumps(info)
        task_name = "metagenomic.report.anno_go"#调用的workflow
        ...
        lowest_level_file = os.path.join(gene_anno_dir, "go.level4.xls")
        level1234_file = os.path.join(gene_anno_dir, "go1234level_statistics.xls")
        ...
        name = "GO_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        params_json = {
        ...
        if hasattr(data, "sample_filter"):
            ...
            if len(samples_ids) < int(sam_num):
                info = {"success": False, "info":"样本数应少于分组方案中所选样本数！", 'variables': '', "code" : "C2404203"}
                return json.dumps(info)
        ...
        if hasattr(data, "read_filter"):
            ...
            if float(abu_pro) < 0 or float(abu_pro) > 1:
                info = {"success": False, "info": "丰度占比必须在0-1之间的小数！", "variables": '', "code" : "C2404204"}
                return json.dumps(info)
        ...
        main_table_id = self.metagenomic.insert_main_table('anno_go', mongo_data)
        update_info = {str(main_table_id):"anno_go"}
        ...
        task_info = super(AnnoGoAction, self).POST()
