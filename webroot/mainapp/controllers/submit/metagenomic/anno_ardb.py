...
class AnnoArdbAction(MetagenomicController):
    def __init__(self):
        super(AnnoArdbAction, self).__init__(instant=False)
    ...
    def POST(self):
        ...
        for argu in default_argu:
            if not hasattr(data, argu):
                info = {"success": False, "info": "PARAMETERS MISSING: %s" , "variables": [argu], "code" : "C2400105"}
                return json.dumps(info)
        if not data.database == "ardb":
            info = {"success": False, "info": "PARAMETERS ERROR: wrong value of database (%s), ardb expected!" , "variables": [data.database], "code" : "C2400106"}
            return json.dumps(info)
        task_name = "metagenomic.report.anno_ardb"  # 调用workflow
        ...
        gene_anno_info = metagenomic.get_anno_path(gene_origin_id, "ardb", task_id)
        ...
        lowest_level_file = os.path.join(gene_anno_dir, "ardb_ARG_profile.xls")
        ...
        name = "ARDB_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        params_json = {
        ...
        if hasattr(data, "sample_filter"):
            ...
            if len(samples_ids) < int(sam_num):
                info = {"success": False, "info": "样本数应少于分组方案中所选样本数!", 'code':'C2400101', 'variables':''}
                return json.dumps(info)
        ...
        if hasattr(data, "reads_filter"):
            ...
            if float(abu_pro) < 0 or float(abu_pro) > 1:
                info = {"success": False, "info": "丰度占比必须为在0-1之间的小数！", 'code':'C2400102', 'variables':''}
                return json.dumps(info)
        ...
        main_table_id = self.metagenomic.insert_main_table("anno_ardb", mongo_data)  # webroot/models/mongo/meta.py
        update_info = {str(main_table_id): "anno_ardb"}
        ...
        task_info = super(AnnoArdbAction, self).POST()
