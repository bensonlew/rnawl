...
class AnosimAction(MetagenomicController):
    def __init__(self):
    ...
    def POST(self):
        ...
        default_argu = ['submit_location', 'group_id', 'group_detail', 'geneset_id', 'anno_type',
                        'permutation', 'distance_method', 'method', 'task_type']
        ...
        if info != "":
            return json.dumps(info)
        task_name = 'metagenomic.report.anosim'
        ...
        main_table_name = AnosimAction.get_main_table_name(data.anno_type, level)
        ...
        main_table_id = self.metagenomic.insert_main_table('anosim', mongo_data)
        update_info = {str(main_table_id): 'anosim'}
    ...
    def get_main_table_name(anno_type, level):
        ...
        return 'ANOSIM_%s%s_%s' % (database, level, time_now)
