...
class AnnoCazyAction(MetagenomicController):
    def __init__(self):
        super(AnnoCazyAction, self).__init__(instant=False)
    ...
    def POST(self):
        ...
        for argu in default_argu:
            if not hasattr(data, argu):
                info = {"success": False, "info": "PARAMETERS MISSING: %s" , "variables": [argu], "code" : "C2400305"}
                return json.dumps(info)
        # database_type = ["nr", "kegg", "cog", "vfdb", "ardb", "card", "cazy"]
        if not data.database == "cazy":
            info = {"success": False, "info": "PARAMETERS ERROR: wrong value of database (%s), cazy expected!" , "variables": [data.database], "code" : "C2400306"}
            return json.dumps(info)
        print data.group_detail
        task_name = "metagenomic.report.anno_cazy"  # 调用workflow
        ...
        gene_anno_info = metagenomic.get_anno_path(gene_origin_id, "cazy", task_id)
        ...
        lowest_level_file = os.path.join(gene_anno_dir, "cazy_family_profile.xls")
        ...
        name = "CAZY_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        params_json = {
        ...
        if hasattr(data, "sample_filter"):
            ...
            if len(samples_ids) < int(sam_num):
                info = {"success": False, "info": "样本数应少于分组方案中所选样本数!","code":"C2400301", "variables":""}
                return json.dumps(info)
        ...
        if hasattr(data, "reads_filter"):
            ...
            if float(abu_pro) < 0 or float(abu_pro) > 1:
                info = {"success": False, "info": "丰度占比必须为在0-1之间的小数！","code":"C2400302", "variables":""}
                return json.dumps(info)
        ...
        if float(data.identity) != 0:
            ...
            if identity < 0 or identity > 1 or len(str(data.identity).split(".")[1]) > 2:
                info = {"success": False, "info": "identity必须为在0-1之间的小数,且小数位数不超过2位！","code":"C2400303", "variables":""}
                return json.dumps(info)
            params_json["identity"] = identity
            options['identity'] = identity
        if data.align_length != 0 or "0":
            if int(data.align_length) < 0:
                info = {"success": False, "info": "align_length必须为正整数！","code":"C2400304", "variables":""}
                return json.dumps(info)
        ...
        main_table_id = self.metagenomic.insert_main_table("anno_cazy", mongo_data)  # webroot/models/mongo/meta.py
        update_info = {str(main_table_id): "anno_cazy"}
        ...
        task_info = super(AnnoCazyAction, self).POST()
