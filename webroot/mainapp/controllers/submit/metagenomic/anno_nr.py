...
class AnnoNrAction(MetagenomicController):
    def __init__(self):
        super(AnnoNrAction, self).__init__(instant=False)

    @check_sig
    def POST(self):
        data = web.input()
        # default_argu = ["database", "geneset_id", "group_detail", "identity", "align_length","level_filter", "sample_filter","reads_filter", "submit_location"]
        default_argu = ["database", "geneset_id", "group_id", "group_detail", "identity", "align_length",
        ...
        if not data.database == "nr":
            info = {"success": False, "info": "PARAMETERS ERROR: wrong value of database (%s), nr expected!" % data.database}
            return json.dumps(info)
        task_name = "metagenomic.report.anno_nr"  # 调用workflow
        ...
        gene_anno_info = metagenomic.get_anno_path(gene_origin_id, "nr", task_id, nr_method=data.nr_method)
        gene_anno_file = gene_anno_info["anno_file"]
        gene_anno_dir = "/".join(gene_anno_file.split("/")[0:len(gene_anno_file.split("/")) - 1])
        lowest_level_file = os.path.join(gene_anno_dir, "tax_s.xls")
        ...
        name = "NR_"+ data.nr_method +"_"+ datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        params_json = {
        ...
        options = {
            ...
            "gene_nr_anno": self.use_s3(gene_anno_file),
        ...
        if hasattr(data, "reads_filter"):
            ...
            reads_filter = eval(data.reads_filter)[0]
            abu_pro_type = reads_filter['type']
            abu_pro = reads_filter['proportion']
            if float(abu_pro) < 0 or float(abu_pro) > 1:
                info = {"success": False, "info": "丰度占比必须为在0-1之间的小数！", "code": "C2400602", "variables": ""}
                return json.dumps(info)
        ...
        if float(data.identity) != 0:
            ...
            if identity < 0 or identity > 1 or len(str(data.identity).split(".")[1]) > 2:
                info = {"success": False, "info": "identity必须为在0-1之间的小数,且小数位数不超过2位！", "code": "C2400603", "variables": ""}
                return json.dumps(info)
            params_json["identity"] = identity
            options['identity'] = identity
        if data.align_length != 0 or "0":
            if int(data.align_length) < 0:
                info = {"success": False, "info": "align_length必须为正整数！", "code": "C2400604", "variables": ""}
                return json.dumps(info)
        ...
        if len(set(select_samples_ids)) != len(set(all_samples_ids)):
            options['group'] = 2
        if hasattr(data, "nr_method"):
            params_json["nr_method"] = data.nr_method
        # options['params'] = json.dumps(params_json, sort_keys=True, separators=(",", ":"))
        mongo_data.append(('params', json.dumps(params_json, sort_keys=True, separators=(",", ":"))))
        main_table_id = self.metagenomic.insert_main_table("anno_nr", mongo_data)  # webroot/models/mongo/meta.py
        update_info = {str(main_table_id): "anno_nr"}
        ...
        task_info = super(AnnoNrAction, self).POST()
