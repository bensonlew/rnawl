...
class AnnoMvirdbAction(MetagenomicController):
    def __init__(self):
        super(AnnoMvirdbAction, self).__init__(instant=False)
    ...
    def POST(self):
        ...
        for argu in default_argu:
            if not hasattr(data, argu):
                info = {"success": False, "info": "PARAMETERS MISSING: %s" % argu, "code" : "C2403008"}
                return json.dumps(info)
        if not data.database == "mvirdb":
            info = {"success": False, "info": "PARAMETERS ERROR: wrong value of database (%s), mvirdb expected!" % data.database, "code" : "C2403002"}
            return json.dumps(info)
        task_name = "metagenomic.report.anno_mvirdb"  # 调用workflow
        ...
        gene_anno_info = metagenomic.get_anno_path(gene_origin_id, "mvir", task_id)
        ...
        lowest_level_file = os.path.join(gene_anno_dir, "Factor_abund_out.xls")
        ...
        name = "MvirDB_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        params_json = {
        ...
        #lowest_level_file = "/mnt/ilustre/users/sanger-dev/workspace/20171121/MetaGenomic_metagenome_6/output/mvirdb"
        options = {
            "geneset_id": data.geneset_id,
        ...
        if hasattr(data, "sample_filter"):
            ...
            if len(samples_ids) < int(sam_num):
                info = {"success": False, "info": "样本数应少于分组方案中所选样本数!", 'code':'C2403003', 'variables':''}
                return json.dumps(info)
        ...
        if hasattr(data, "reads_filter"):
            ...
            if float(abu_pro) < 0 or float(abu_pro) > 1:
                info = {"success": False, "info": "丰度占比必须为在0-1之间的小数！", 'code':'C2403004', 'variables':''}
                return json.dumps(info)
        ...
        if float(data.identity) != 0:
            ...
            if identity < 0 or identity > 1 :
                info = {"success": False, "info": "identity必须为在0-1之间的小数,且小数位数不超过2位！", 'code':'C2403005', 'variables':''}
                return json.dumps(info)
            sp_identity = str(data.identity).split(".")
            if len(sp_identity) > 1:
                if len(sp_identity[1]) > 2:
                    info = {"success": False, "info": "identity必须为在0-1之间的小数,且小数位数不超过2位！", 'code':'C2403006', 'variables':''}
                    return json.dumps(info)
            if identity==0.0:
                params_json["identity"] = 0
            else:
                params_json["identity"] = identity
            options['identity'] = identity
        if data.align_length != 0 or "0":
            if int(data.align_length) < 0:
                info = {"success": False, "info": "align_length必须为正整数！", 'code':'C2403007', 'variables':''}
                return json.dumps(info)
        ...
        main_table_id = self.metagenomic.insert_main_table("anno_mvir", mongo_data)  # webroot/models/mongo/meta.py
        update_info = {str(main_table_id): "anno_mvir"}
        ...
        task_info = super(AnnoMvirdbAction, self).POST()
