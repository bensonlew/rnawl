...
class AnnoTcdbAction(MetagenomicController):
    def __init__(self):
        super(AnnoTcdbAction, self).__init__(instant=False)
    ...
    def POST(self):
        ...
        default_argu = ["database", "geneset_id", "group_id", "group_detail","submit_location","task_type"] #, "identity", "align_length", "task_type",

        for argu in default_argu:
            if not hasattr(data, argu):
                info = {"success": False, "info": "PARAMETERS MISSING: %s" , "variables": [argu], "code" : "C2402904"}
                return json.dumps(info)
        if not data.database == "tcdb":
            info = {"success": False, "info": "PARAMETERS ERROR: wrong value of database (%s), tcdb expected!" , "variables": [data.database], "code" : "C2402905"}
            return json.dumps(info)
        task_name = "metagenomic.report.anno_tcdb"  # 调用workflow
        ...
        gene_anno_info = metagenomic.get_anno_path(gene_origin_id, "tcdb", task_id)
        ...
        lowest_level_file = os.path.join(gene_anno_dir, "tcdb_abund_out.xls")
        ...
        name = "TCDB_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        params_json = {
        ...
        #lowest_level_file = "/mnt/ilustre/users/sanger-dev/workspace/20171121/MetaGenomic_metagenome_6/output/tcdb"
        options = {
            "geneset_id": data.geneset_id,
        ...
        if hasattr(data, "sample_filter"):
            ...
            if len(samples_ids) < int(sam_num):
                info = {"success": False, "info": "样本数应少于分组方案中所选样本数!", "code" : "C2402906"}
                return json.dumps(info)
        ...
        if hasattr(data, "reads_filter"):
            ...
            if float(abu_pro) < 0 or float(abu_pro) > 1:
                info = {"success": False, "info": "丰度占比必须为在0-1之间的小数！", "code" : "C2402907"}
                return json.dumps(info)
        ...
        if hasattr(data, 'identity') :
            ...
            if identity < 0 or identity > 1 :
                info = {"success": False, "info": "identity必须为在0-1之间的小数,且小数位数不超过2位！", "code" : "C2402908"}
                return json.dumps(info)
            sp_identity = str(data.identity).split(".")
            if len(sp_identity) > 1:
                if len(sp_identity[1]) > 2:
                    info = {"success": False, "info": "identity必须为在0-1之间的小数,且小数位数不超过2位！", "code" : "C2402909"}
                    return json.dumps(info)
            params_json["identity"] = identity
            options['identity'] = identity
        if hasattr(data, 'align_length'):
            if int(data.align_length) < 0:
                info = {"success": False, "info": "align_length必须为正整数！", "code" : "C2402910"}
                return json.dumps(info)
        ...
        main_table_id = self.metagenomic.insert_main_table("anno_tcdb", mongo_data)  # webroot/models/mongo/meta.py
        update_info = {str(main_table_id): "anno_tcdb"}
        ...
        task_info = super(AnnoTcdbAction, self).POST()
