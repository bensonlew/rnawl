...
class AnnoKeggAction(MetagenomicController):
    def __init__(self):
        super(AnnoKeggAction, self).__init__(instant=False)
    ...
    def POST(self):
        ...
        for argu in default_argu:
            if not hasattr(data, argu):
                info = {"success": False, "info": "%s参数缺少!" , "variables": [argu], "code" : "C2400501"}
                return json.dumps(info)
        # database_type = ["nr", "kegg", "cog", "vfdb", "ardb", "card", "cazy"]
        if not data.database == "kegg":
            info = {"success": False, "info": "KEGG数据库类型错误!", "code" : "C2400502"}
            return json.dumps(info)
        task_name = "metagenomic.report.anno_kegg"  # 调用workflow
        ...
        geneset_table = self.use_s3(geneset_info["gene_list_length"])
        ...
        gene_anno_file = self.use_s3(gene_anno_info["anno_file"])
        ...
        lowest_level_file = os.path.join(gene_anno_dir, "kegg_gene_profile.xls")
        ...
        name = "KEGG_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        params_json = {
        ...
        if hasattr(data, "sample_filter"):
            ...
            if len(samples_ids) < int(sam_num):
                info = {"success": False, "info": "样本数应少于分组方案中所选样本数!", "code" : "C2400503"}
                return json.dumps(info)
        ...
        if hasattr(data, "reads_filter"):
            ...
            if float(abu_pro) < 0 or float(abu_pro) > 1:
                info = {"success": False, "info": "丰度占比必须为在0-1之间的小数！", "code" : "C2400504"}
                return json.dumps(info)
        ...
        main_table_id = self.metagenomic.insert_main_table("anno_kegg", mongo_data)  # webroot/models/mongo/meta.py
        update_info = {str(main_table_id): "anno_kegg"}
        ...
        task_info = super(AnnoKeggAction, self).POST()
    ...
    def create_link_dir(self, task_id, main_table_id):
        # data = web.input()
        ...
        # target_dir = path + '/rerewrweset/metag/KEGG_Pathway/' + str(task_id) + '/' + str(main_table_id) + '/'
        target_dir = 'metag/KEGG_Pathway/' + str(task_id) + '/' + str(main_table_id) + '/'
        return target_dir
