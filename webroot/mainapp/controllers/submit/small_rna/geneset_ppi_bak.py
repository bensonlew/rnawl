...
class GenesetPpiAction(SmallRnaController):
    ...
    def POST(self):
        ...
        info = {"success": False, "info": None}
        if geneset_info['type'] != data.geneset_type:
            info["info"] = "基因集属性为{} 但筛选基因集属性为{}".format(geneset_info['type'], data.geneset_type)
            return json.dumps(info)
        if not geneset_info:
            info["info"] = "geneset不存在，请确认参数是否正确！!"
            return json.dumps(info)
    ...
    class TestFunction(unittest.TestCase):
        ...
        def test_this(self):
            ...
            args = dict(
                ...
                geneset_id="5bc05fe4a4e1af0f2b2e0e5a",
                taxon='Plants',
                species='4081',
                # species = '3702',
                combine_score='300',
