...
class BatchAction(SmallRnaController):
    ...
    def POST(self):
        ...
        exp_info = self.small_rna.get_exp_params_info(data.exp_id, data.task_id)
        ...
        name_batch = "ExpBatch" + '_' + exp_type + '_'
        ...
        main_id_exp = self.small_rna.insert_main_table('sg_exp', main_info_exp)

        name = 'Exp_batch_{}'.format(time_now.strftime('%Y%m%d_%H%M%S'))
        task_info = self.small_rna.get_task_info(task_id=data.task_id)
        main_info_batch = {
            'task_id': data.task_id,
        ...
        main_id_batch = self.small_rna.insert_main_table('sg_exp_batch', main_info_batch)
        new_task_id = self.small_rna.get_new_id(data.task_id)
        ...
        #     })
        # prepare to file
        to_files = ["small_rna.export_exp_batch_matrix(exp_matrix)",
                    "small_rna.export_group_detail(group_table)",
                    "small_rna.export_exp_other_matrix(exp_other)"
                    ]
        task_name = 'small_rna.report.batch'
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/small_rna/batch "
        cmd += "-b http://bcl.tsg.com "
        args = dict(
            task_id="tsg_248838",
            task_type="2",
            submit_location="batch",
            exp_id="5fbd6d4517b2bf6a7d4518f8",
            has_batch="True",
            batch_matrix='/mnt/ilustre/users/sanger-dev/sg-users/zoujiaxun/smallrna/batch/batch.txt',
            batch_method='combat'
