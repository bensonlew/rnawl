...
class GenesetCircAction(SmallRnaController):
    def __init__(self):
    ...
    def POST(self):
        ...
        task_name = 'smallrna.report.geneset_circ'
        params_json = {
        ...
        # 判断传入的富集id是否存在
        if data.enrich_type == "GO":
            enrich_info = self.smallrna.get_main_info(data.enrich_id, 'sg_geneset_go_enrich', data.task_id)
        elif data.enrich_type == "KEGG":
            enrich_info = self.smallrna.get_main_info(data.enrich_id, 'sg_geneset_kegg_enrich', data.task_id)
        else:
        ...
        if not self.smallrna.consistent_in_enrich_and_diff(data.enrich_id, data.diff_id, data.enrich_type):
            info = {"success": False, "info": "富集结果里存在新基因，但差异结果中没有"}
            return json.dumps(info)

        task_info = self.smallrna.get_task_info(enrich_info['task_id'])
        ...
        to_file = []
        if data.enrich_type == "GO":
            to_file = ['smallrna.export_go_enrich_matrix(enrich_table)',
                       "smallrna.export_compare_exp_fc(diff_fc)",
                       "smallrna.get_gene_detail(gene_detail)"
            ]
        elif data.enrich_type == "KEGG":
            to_file = ['smallrna.export_kegg_enrich_matrix(enrich_table)',
                       "smallrna.export_compare_exp_fc(diff_fc)",
                       "smallrna.get_gene_detail(gene_detail)"
            ]

        # proteinset_type只有2种，T或者G,s而页面限制只能同时取G或者T，所以这里用一个geneset_id作为示例即可
        main_table_name = enrich_info['name'] + "Circ_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S%f")[:-3]
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/smallrna/geneset_circ "
        cmd += "-b http://192.168.12.102:9090 "
        args = dict(
            task_id="RefrnaV2_7320",
            task_type="2",
            submit_location="genesetcirc",
            enrich_id="5b1e1a3ea4e1af1b2af0825a",
            enrich_type="KEGG",
            compare_group="A1|B1",
            diff_id="5b1e0419a4e1af72844960a6",
            p_thre="1",
            p_type="padjust",
            anno_num_thre="15",
            geneset_type='G',
