...
class GenesetPpiAction(MedicalTranscriptomeController):
    """
    蛋白质互作网络的接口
    """
    ...
    def POST(self):
        data = web.input()
        default_argu = ['geneset_id', 'species', 'submit_location', 'combine_score', 'level', 'task_type', 'task_id']
        ...
        if int(data.species) not in self.species_list:
            ...
            return json.dumps(info)
        '''
        task_name = 'medical_transcriptome.report.geneset_ppi'
        task_type = 'workflow'
        params_json = {
        ...
        if geneset_info['level'] != data.level:
            ...
            info = {"success": False, "info": "基因集属性为%s 但筛选基因集属性为%s" % (geneset_info['level'], data.level), 'code': 'C2901703', 'variables': variables}
            return json.dumps(info)
        if not geneset_info:
        ...
        # annot_stat = self.medical_transcriptome.get_annotation_stat_info(geneset_info['task_id'])
        # if not annot_stat:
        ...
        main_table_id = self.medical_transcriptome.insert_main_table('sg_geneset_ppi', mongo_data)
        ...
        options = {
            ...
            # 'anno': result_dir,
            'anno': task_info['task_id'] + '|' + data.level,
            "species": data.species,
        ...
        to_file = ["medical_transcriptome.export_gene_list_ppi(geneset_list)",
                   "medical_transcriptome.get_gene_detail_new(anno)"]
        ...
        geneset_info = self.medical_transcriptome.insert_geneset_info(data.geneset_id, "sg_ppinetwork", str(main_table_id))
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/medical_transcriptome/geneset_ppi "
