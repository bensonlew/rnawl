...
class CoxAction(MedicalTranscriptomeController):
    def __init__(self):
        super(CoxAction, self).__init__(instant=False)

    @check_sig
    def POST(self):
        data = web.input()
        basic_args = ["task_id", "submit_location", 'task_type']
        basic_args += ['exp_file', 'meta_file']
        ...
        if hasattr(data, 'factor_list'):
            params.update({
                'factor_list': data.factor_list
            })
        if hasattr(data, 'gene_list'):
            params.update({
        ...
        main_id = self.medical_transcriptome.insert_main_table('sg_tool_cox', main_info)
        ...
        options = dict(
            ...
            main_id=str(main_id),
            update_info=json.dumps({str(main_id): "sg_tool_cox"})  # to update sg_status
        )
        ...
        task_name = 'medical_transcriptome.report.cox'
        ...
        # 运行workflow 并传回参数
        task_info = super(CoxAction, self).POST()
        task_info['content'] = {
            'ids': {
                'id': str(main_id),
                'name': name
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/medical_transcriptome/cox "
        ...
        args = dict(
            ...
            factor_list='/mnt/ilustre/users/sanger-dev/sg-users/zoujiaxun/tool_lab/nomogram/test2medical/factor',
