...
class ImmunedeconvAction(MedicalTranscriptomeController):
    def __init__(self):
        super(ImmunedeconvAction, self).__init__(instant=False)

    @check_sig
    def POST(self):
        data = web.input()
        basic_args = ["task_id", "submit_location", 'task_type']
        basic_args += ['exp_file', 'method', 'species', ]
        ...
        main_id = self.medical_transcriptome.insert_main_table('sg_tool_immunedeconv', main_info)
        ...
        options = dict(
            ...
            main_id=str(main_id),
            update_info=json.dumps({str(main_id): "sg_tool_immunedeconv"})  # to update sg_status
        )
        ...
        task_name = 'medical_transcriptome.report.immunedeconv'
        ...
        # 运行workflow 并传回参数
        task_info = super(ImmunedeconvAction, self).POST()
        task_info['content'] = {
            'ids': {
                'id': str(main_id),
                'name': name
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/medical_transcriptome/immunedeconv "
        cmd += "-b http://bcl.tsg.com "
        # cmd += "-b http://192.168.12.102:9090 "
        args = dict(
            task_id="medical_transcriptome",
            task_type="2",
            submit_location="immunedeconv",
