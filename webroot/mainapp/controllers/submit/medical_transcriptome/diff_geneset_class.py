...
from mbio.api.to_file.medical_transcriptome_new.geneset import *


class DiffGenesetClassAction(MedicalTranscriptomeController):
    def __init__(self):
        super(DiffGenesetClassAction, self).__init__(instant=False)

    @check_sig
    def POST(self):
        data = web.input()
        default_argu = ['geneset_id', 'level', 'submit_location', 'task_type']
        ...
        task_name = 'medical_transcriptome.report.diff_geneset_class'
        params_json = {
        ...
        if anno_type == "go":
            table_name = "GO"
            collection_name = "sg_diff_geneset_go_class"
            to_file = 'medical_transcriptome_new.diff_geneset.export_go_class(geneset_go)'
            option = {"geneset_go": data.geneset_id}

        elif anno_type == "cog":
            table_name = "COG"
            collection_name = "sg_diff_geneset_cog_class"
            to_file = 'medical_transcriptome_new.diff_geneset.export_cog_class(geneset_cog)'
        ...
        task_info = super(DiffGenesetClassAction, self).POST()
        ...
        geneset_info = self.medical_transcriptome.insert_geneset_info(data.geneset_id, collection_name,
                                                                    str(main_table_id))
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/medical_transcriptome/diff_geneset_class "
        cmd += "-b http://bcl.tsg.com "
        args = dict(
            submit_location="diffgenesetcog",
