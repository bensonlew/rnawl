...
class TfPredictAction(MedicalTranscriptomeController):
    ...
    def __init__(self):
        ...
        self.task_name = 'medical_transcriptome.report.tf_predict'
        ...
        self.add_params = ['blast_all', 'seqfile', 'E'] #医学转录组只做人，大鼠，小鼠，所以tf_sub_db默认为vertebrates， 故页面不在传该参数
    ...
    def create_main_table(self, packed_params, collection_name):
        task_info = self.medical_transcriptome.get_table_info_by_task_id(self.input_data.task_id, 'sg_task')
    ...
    def prepare_workflow_options(self, main_id, collection_name, main_table_name):
        ...
        seqdb_path = self.medical_transcriptome.get_task_info(self.input_data.task_id)['refrna_seqdb']
        ...
        options.update({
            ...
            "seq_db": seqdb_path,
            "gene_detail": self.input_data.task_id + '|' + 'T',
        })

        # prepare to file
        to_files = ["medical_transcriptome.get_all_pep_seq(seq_db)", "medical_transcriptome.get_gene_detail_new(gene_detail)"]
...
class TestFunction(unittest.TestCase):
    ...
    #     cmd += "s/medical_transcriptome/tf_predict "
