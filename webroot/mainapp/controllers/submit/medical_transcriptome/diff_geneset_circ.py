...
class DiffGenesetCircAction(MedicalTranscriptomeController):
    def __init__(self):
        super(DiffGenesetCircAction, self).__init__(instant=False)
    ...
    def POST(self):
        ...
        task_name = 'medical_transcriptome.report.diff_geneset_circ'
        params_json = {
        ...
        if hasattr(data, "anno_list"):
            if data.anno_list == "":
                ...
                if data.enrich_type == 'GO':
                    ...
                    # if not "map" in data.anno_list.lower() and :
        ...
        if data.enrich_type.upper() == "GO":
            enrich_info = self.medical_transcriptome.get_enrich_info_by_geneset(data.enrich_id, 'sg_diff_geneset_go_enrich', data.task_id)
        #差异基因弦图中，当且仅当选DISGENET时,输入为enrich_id,其余均为geneset_id
        elif data.enrich_type.upper() == "DISGENET":
            enrich_info = self.medical_transcriptome.get_main_info(data.enrich_id, 'sg_diff_geneset_disgenet_enrich',
                                                                   data.task_id)
        elif data.enrich_type.upper() == "REACTOME":
            enrich_info = self.medical_transcriptome.get_enrich_info_by_geneset(data.enrich_id, 'sg_diff_geneset_reactome_enrich',
                                                                   data.task_id)
        elif data.enrich_type.upper() == "DO":
            enrich_info = self.medical_transcriptome.get_enrich_info_by_geneset(data.enrich_id, 'sg_diff_geneset_do_enrich', data.task_id)
        elif data.enrich_type == "KEGG":
            enrich_info = self.medical_transcriptome.get_enrich_info_by_geneset(data.enrich_id, 'sg_diff_geneset_kegg_enrich',
                                                                   data.task_id)
        else:
        ...
        enrich_main_id = str(enrich_info["main_id"])
        if not self.medical_transcriptome.consistent_in_enrich_and_diff_diff(enrich_main_id, data.diff_id, data.enrich_type):
        ...
        # 判断传入的差异分析是否存在 ，暂时没写
        # table_name = "Kegg"
        collection_name = "sg_diff_geneset_circ"
        to_file = []
        if data.enrich_type == "GO":
            to_file = ['medical_transcriptome_new.geneset_circ.export_diff_go_enrich_matrix(enrich_table)',
            ...
            to_file = ['medical_transcriptome_new.geneset_circ.export_diff_kegg_enrich_matrix(enrich_table)',
                       "medical_transcriptome_new.geneset_circ.export_compare_exp_fc(diff_fc)",
                       "medical_transcriptome_new.geneset_circ.get_gene_detail_new(gene_detail)"
            ]
        elif data.enrich_type.upper() == "DISGENET":
            to_file = ['medical_transcriptome_new.geneset_circ.export_diff_disgenet_enrich_matrix(enrich_table)',
                       "medical_transcriptome_new.geneset_circ.export_compare_exp_fc(diff_fc)",
                       "medical_transcriptome_new.geneset_circ.get_gene_detail_new(gene_detail)"
            ]
        elif data.enrich_type.upper() == "DO":
            to_file = ['medical_transcriptome_new.geneset_circ.export_diff_do_enrich_matrix(enrich_table)',
                       "medical_transcriptome_new.geneset_circ.export_compare_exp_fc(diff_fc)",
                       "medical_transcriptome_new.geneset_circ.get_gene_detail_new(gene_detail)"
            ]
        elif data.enrich_type.upper() == "REACTOME":
            to_file = ['medical_transcriptome_new.geneset_circ.export_diff_reactome_enrich_matrix(enrich_table)',
                       "medical_transcriptome_new.geneset_circ.export_compare_exp_fc(diff_fc)",
                       "medical_transcriptome_new.geneset_circ.get_gene_detail_new(gene_detail)"
            ]

        # proteinset_type只有2种，T或者G,s而页面限制只能同时取G或者T，所以这里用一个geneset_id作为示例即可
        main_table_name = "Diff_EnrichCirc_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        ...
        option = {
            "diff_fc": "diff_fc",
            "gene_detail": task_info['task_id']+"|"+data.level,
        ...
        task_info = super(DiffGenesetCircAction, self).POST()
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/medical_transcriptome/diff_geneset_circ "
        cmd += "-b http://bcl.tsg.com "
        args = dict(
            task_id="8ju59of6lkdojsbreb482jveeb",
            task_type="2",
            submit_location="diffgenesetcirc",
            enrich_id="5facb20d17b2bf65566b8881",
            enrich_type="GO",
            compare_group="Para_carcinoma_tissue|Carcinoma_tissue",
