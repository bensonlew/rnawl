...
class CollineAction(MetagbinController):
    def __init__(self):
    ...
    def POST(self):
        ...
        name = "Colline_" + data.query +"_"+ datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        params_json = {
            'query': data.query,
        ...
        main_table_id = Metagbin().insert_main_table("colline", mongo_data)
        Metagbin().insert_main_table_new("colline", str(main_table_id), {"main_id": main_table_id})
        update_info[str(main_table_id)] = 'colline'
        options = {
        ...
        if data.ref == "user_defined" and hasattr(data, "ref_path"):
            ...
            options["ref"] = Metagbin().get_genome(data.task_id, data.ref)
        self.set_sheet_data(
            name=task_name,
            options=options,
