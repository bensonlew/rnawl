...
class IdentifGenusAction(MetagbinController):
    def __init__(self):
        super(IdentifGenusAction, self).__init__(instant=False)
    ...
    def POST(self):
        ...
        name = "POCP_" + data.query +"_"+ datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        params_json = {
            'query': data.query,
        ...
        main_table_id = Metagbin().insert_main_table("identif_genus", mongo_data)
        Metagbin().insert_main_table_new("identif_genus", str(main_table_id), {"main_id": main_table_id})
        update_info[str(main_table_id)] = 'identif_genus'
        options = {
            "update_info": json.dumps(update_info),
            "main_id": str(main_table_id),
            "bin_name": data.query,
            "method": "pocp",
            "query": Metagbin().get_sample_genefile(data.task_id, data.query, type="faa"),
            "task_id": data.task_id
            # ("ref", data.ref)
        }
        if data.ref == "user_defined" and hasattr(data, "ref_path"):
            ...
            ref_path = os.path.join(Config().SOFTWARE_DIR, "database/NCBI_bacgenome/faa", data.accession_value + ".faa")
            options["ref"] = ref_path
        else:
            options["ref"] = Metagbin().get_sample_genefile(data.task_id, data.ref, type="faa")
        self.set_sheet_data(
            name=task_name,
            options=options,
        ...
        task_info = super(IdentifGenusAction, self).POST()
