...
class GenesetIpathAction(RefRnaV2Controller):
    def __init__(self):
    ...
    def POST(self):
        ...
        task_name = 'ref_rna_v2.report.geneset_ipath'
        params_json = {
        ...
        for geneset in data.geneset_id.split(","):
            ...
            if geneset_info['type'] != data.geneset_type:
                ...
                info = {"success": False, "info": "基因集属性为%s 但筛选基因集属性为%s" % (geneset_info['type'], data.geneset_type), 'code': 'C2901502', 'variables': variables}
                return json.dumps(info)
            if not geneset_info:
                variables = []
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/ref_rna_v2/geneset_ipath "
