...
class GenesetKeggenrichstatAction(RefRnaV2Controller):
    def __init__(self):
    ...
    def POST(self):
        ...
        for arg in basic_args:
            if not hasattr(data, arg):
                ...
                info = {'success': False, 'info': "Lack argument:   %s" % arg, 'code': 'C2903804', 'variables': variables}
        ...
        if data.draw_for_ids == "yes":
            if not data.pathway_ids:
                ...
                info = {'success': False, 'info': "Lack argument:   %s" % lag_info, 'code': 'C2903805','variables': variables}
                return json.dumps(info)
        else:
            ...
            for arg in other_args:
                if not hasattr(data,arg):
                    variables = []
                    variables.append(arg)
                    info = {'success': False, 'info': "Lack argument:   %s" % arg, 'code': 'C2903806','variables': variables}
                    return json.dumps(info)

        enrich_info = self.ref_rna_v2.get_genesetkeggenrich_params_info(data.kegg_enrich_id, data.task_id)
        ...
        geneset_info = self.ref_rna_v2.get_main_info(geneset_id, 'sg_geneset', data.task_id)
        ...
        main_id = self.ref_rna_v2.insert_main_table('sg_geneset_kegg_enrich_stat', main_info)
        ...
        if data.draw_for_ids == "yes":
            ...
            to_files = ["ref_rna_v2.export_kegg_enrich_info_ids(geneset_kegg_enrich_info)"]
        else:
            options = {
                "geneset_kegg_enrich_info": data.kegg_enrich_id,
                "geneset_kegg_enrich_stat_id": str(main_id),
            ...
            to_files = ["ref_rna_v2.export_kegg_enrich_info_filter(geneset_kegg_enrich_info)",
                        'ref_rna_v2.export_gene_list(geneset_list)']
        # prepare to file


        # 把参数交给workflow运行相应的tool， 其中to_file用于准备tool的输入文件
        task_name = 'ref_rna_v2.report.geneset_keggenrichstat'
