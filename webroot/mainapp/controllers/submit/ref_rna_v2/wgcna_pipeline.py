...
class WgcnaPipelineAction(RefRnaV2Controller):
    def __init__(self):
        super(WgcnaPipelineAction, self).__init__(instant=False)
        self.task_name = 'ref_rna_v2.report.wgcna_pipeline'
    ...
    def check_trait(self, trait_path):
        ...
        with open(trait_path, 'rb') as f:
            ...
            if len(trait_samples) != len(select_samples):
                ...
                return json.dumps({'success': False, 'info': "表型样本与预处理样本不一致！注:上传的表型数据表中样本名称必须是原始样本名称，否则无法运行!", 'variables': ''})
            elif len(set(trait_samples).intersection(set(select_samples))) != len(select_samples):
                print trait_samples, select_samples
                return json.dumps({'success': False, 'info': "表型样本与预处理样本不一致！注:上传的表型数据表中样本名称必须是原始样本名称，否则无法运行!", 'variables': ''})
    ...
    def create_main_table(self, packed_params, collection_name):
        ...
        result_info = self.ref_rna_v2.get_main_info(self.input_data.exp_id, 'sg_exp', self.input_data.task_id)
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/ref_rna_v2/wgcna_pipeline "
        ...
        args = dict(
            ...
            networkType="signed",
            minKMEtoStay="0.5",
            trait_path="/rerewrweset/files/m_188/188_59f029eed6609/tsg_28226/trait_data/test.txt",
            trait_type="discrete",
            corr_method="pearson",
            file_id="6919032",
            threshold="0.03",
