...
class MasigproAction(RefRnaV2Controller):
    '''
    ...
    def POST(self):
        ...
        if list(df.columns)[:3] != req_columns:
            ...
            return json.dumps(info)
        sp2exp = self.ref_rna_v2.db['sg_exp_detail'].find_one(
        ...
        main_id = self.ref_rna_v2.insert_main_table('sg_masigpro', main_info)
        new_task_id = self.ref_rna_v2.get_new_id(data.task_id)
        main_table_data = {'run_id': new_task_id}

        exp_type = self.ref_rna_v2.db['sg_exp'].find_one({'main_id': ObjectId(data.exp_id)})['exp_type']
        options = {
            'matrix': data.exp_id,
        ...
        to_files = [
            'ref_rna_v2.export_masigpro_geneset(geneset)',
            'ref_rna_v2.export_masigpro_matrix(matrix)'
        ]
        self.set_sheet_data(
