...
class DiffexpAction(RefRnaV2Controller):
    def __init__(self):
    ...
    def POST(self):
        ...
        compare_names=self.ref_rna_v2.get_control_id(data.control_id)
        cmp_info = json.loads(compare_names)
        for cmp in cmp_info:
            ctrl,test = cmp.split("|")
            reverse_cmp = test+"|" + ctrl
            if reverse_cmp in cmp_info:
                success.append('同一个两两比较分组中出现不同的对照组，请检查')
        ...
        task_name = 'ref_rna_v2.report.diffexp'
