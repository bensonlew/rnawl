...
class RefAnnotationAction(RefRnaV2Controller):
    """
    refrna 注释重运行接口
    """
    def __init__(self):
        super(RefAnnotationAction, self).__init__(instant=False)
    ...
    def POST(self):
        ...
        if return_result:
            ...
            info = {"success": False, "info": '%s' % return_result, 'code': 'C2902201', 'variables': variables}
            return json.dumps(info)
        stat_info = self.ref_rna_v2.get_main_info_by_record("sg_annotation_stat", task_id=str(data.task_id), type="origin")
        if not stat_info:
            info = {"success": False, "info": "stat_id不存在,请确认参数是否正确", 'code': 'C2902202', 'variables': ''}
            return json.dumps(info)
        print(stat_info)

        run_info = self.ref_rna_v2.get_main_info_by_record("sg_annotation_stat", task_id=str(data.task_id), status="start", type="latest")
        if run_info:
            variables = []
            variables.append(run_info['name'])
            info = {"success": False, "info": "任务%s正在计算，请等待其完成在下次运行" % run_info['name'], 'code': 'C2902203', 'variables': variables}
            return json.dumps(info)

        latest_info = self.ref_rna_v2.get_main_info_by_record("sg_annotation_stat", task_id=str(data.task_id), status="end",  type="latest")
        latest_main_id = ""
        ...
        if stat_info.has_key("exp_level"):
            exp_level = stat_info["exp_level"]
        else:
        ...
        mongo_data = [
            ...
            ("exp_level", exp_level),
            ("type", "latest"),
            ("has_new", stat_info["has_new"]),
            ("species_name", stat_info["species_name"]),
            ("created_ts", datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
            ("params", json.dumps(params_json, sort_keys=True, separators=(',', ':')))
        ]
        main_table_id = self.ref_rna_v2.insert_main_table("sg_annotation_stat", mongo_data)
        ...
        to_file = []
        self.set_sheet_data(name="ref_rna_v2.report.ref_annotation",
                            options=options,
                            ...
                            project_sn=stat_info["project_sn"],
                            to_file=to_file, )
        task_info = super(RefAnnotationAction, self).POST()
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/ref_rna_v2/ref_annotation "
