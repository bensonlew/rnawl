...
class WgcnaNetworkAction(RefRnaV2Controller):
    def __init__(self):
        super(WgcnaNetworkAction, self).__init__(instant=False)
        self.task_name = 'ref_rna_v2.report.wgcna_network'
    ...
    def create_main_table(self, packed_params, db_name):
        result_info = self.ref_rna_v2.get_main_info(self.input_data.wgcna_relate_id, 'sg_wgcna_relate', self.input_data.task_id)
        ...
        main_info = dict(
            ...
            status="start")
        main_id = self.ref_rna_v2.insert_main_table(db_name, main_info)
        return main_id, name

    def prepare_workflow_options(self, main_id, db_name, main_table_name):
        relate_output = self.ref_rna_v2.get_main_info(self.input_data.wgcna_relate_id,'sg_wgcna_relate', self.input_data.task_id)['output_dir']
        if not relate_output.endswith('/'):
            relate_output += '/'
        module_output = self.ref_rna_v2.get_main_info(self.wgcna_module_id, 'sg_wgcna_module', self.input_data.task_id)['output_dir']
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/ref_rna_v2/wgcna_network "
        cmd += "-b http://192.168.12.102:9090 "
        args = dict(
            task_id="tsg_28226",
            task_type="2",
            submit_location="WgcnaNetwork",
            wgcna_relate_id="5abf33cea4e1af79707a4142",
            module='red,blue',
            threshold='0.03',
            exp_level="gene",
            top='30',
