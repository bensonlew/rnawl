...
class TfbsPredictAction(RefRnaV2Controller):
    ...
    def __init__(self):
        ...
        self.task_name = 'ref_rna_v2.report.tfbs_predict'
    ...
    def check_params(self):
        ...
        if self.input_data.motifs_user.lower() != 'none':
            ...
            self.motifs_user = self.input_data.motifs_user
        if hasattr(self.input_data, "seqdb") and self.input_data.seqdb.lower() != 'none':
        ...
        if self.input_data.motifs_user.lower() == 'none' and self.input_data.tf_selected.lower() in ['none', '']:
            info = {'success': False, 'info': "请选择已知转录因子或按照指定要求上传Motif文件", 'code': 'C2902802', 'variables': ''}
            return json.dumps(info)


        table_info = self.ref_rna_v2.get_main_info(self.input_data.tf_predict_id, 'sg_tf_predict', self.input_data.task_id)
    ...
    def create_main_table(self, packed_params, collection_name):
        ...
        if self.input_data.geneset_id.lower() not in ["all", "refall", "none"]:
            self.ref_rna_v2.insert_geneset_info(self.input_data.geneset_id, collection_name, str(main_id))
    ...
    def prepare_workflow_options(self, main_id, collection_name, main_table_name, genome_id):
        ...
        options.update({
            ...
            "seq2name": self.input_data.task_id,
            "genome_id": genome_id,
        })
        # get motif species
        table_info = self.ref_rna_v2.get_main_info(self.input_data.tf_predict_id, 'sg_tf_predict', self.input_data.task_id)
        ...
        # prepare to file
        to_files = [
            "ref_rna_v2.export_geneid2tfid_file(tf_predict_id)",
            "ref_rna_v2.get_gene_detail(seq2name)",
        ]
        if 'all' not in self.input_data.geneset_id.lower():
            to_files.append("ref_rna_v2.export_gene_list(geneset_list)")
        else:
            pass
        if "All" in input_data_dict['tf_selected']:
        ...
        if self.input_data.exp_id.lower() != "none":
            ...
            to_files.append("ref_rna_v2.export_geneset_exp_matrix(exp_matrix)",)
        else:
    ...
    def POST(self):
        ...
        if 'group_id' in self.input_data and str(self.input_data.group_id).lower() != 'all' and str(self.input_data.group_id).lower() != 'none':
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        args = dict(
            ...
            motifs_file_id="none",
            seqdb_file_id="none",
            tf_selected="gene11711|MA1664.1,gene13275|MA0571.1,gene13410|MA1205.1",
            geneset_id="all",
            organism="Capsicum_annuum",
            seqdb="None",
            exp_id="5d2f8c1217b2bf3136aac01d",
            group_id="all",
            group_dict=json.dumps({"T1" : [ "T1_1",  "T1_2", "T1_3"],"CK1" : [ "CK1_1", "CK1_2", "CK1_3"],"T2" : [ "T2_1", "T2_2", "T2_3"],"CK2" : [ "CK2_1", "CK2_2", "CK2_3"],"T3" : [ "T3_1", "T3_2", "T3_3"],"CK3" : [ "CK3_1", "CK3_2", "CK3_3"],"T4" : [ "T4_1", "T4_2", "T4_3"],"CK4" : [ "CK4_1", "CK4_2", "CK4_3"]}).replace('"', '\\"'),
            tf_predict_id="5ecb8f3c17b2bf2f02743869",
            thresh="0.0001",
            qv_thresh="0",
            corr_cutoff="0.5",
            corr_pvalue="0.05",
