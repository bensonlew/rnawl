...
class WgcnaModuleAction(RefRnaV2Controller):
    def __init__(self):
    ...
    def POST(self):
        ...
        exp_info = self.ref_rna_v2.get_main_info(data.wgcna_prepare_id, 'sg_wgcna_prepare', data.task_id)
        project_sn = exp_info["project_sn"]
        task_id = data.task_id
        if data.power == "" or data.power == "NaN":
            prepare_info = self.ref_rna_v2.get_power(data.wgcna_prepare_id)
        ...
        main_id = self.ref_rna_v2.insert_main_table('sg_wgcna_module', main_info)
        ...
        # prepare to file
        to_files = ["ref_rna_v2.export_wgcna_exp_matrix(exp_matrix)"]

        # 把参数交给workflow运行相应的tool， 其中to_file用于准备tool的输入文件
        task_name = 'ref_rna_v2.report.wgcna_module'
