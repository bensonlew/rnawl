...
class ExpPcaAction(DenovoRnaV2Controller):
    def __init__(self):
    ...
    def POST(self):
        ...
        basic_args += [ 'draw_in_groups']  #last 新增参数draw_in_groups
        # check arg
        for arg in basic_args:
            if not hasattr(data, arg):
                var = []
                var.append(arg)
                info = {'success': False, 'info': "Lack argument: %s"%(arg), "code": 'C1600701', "variables": var}
        ...
        main_id = self.denovo_rna_v2.insert_main_table('sg_exp_pca', main_info)
        ...
        to_files = ["denovo_rna_v2.export_exp_matrix(exp_matrix)",
                    "denovo_rna_v2.export_group(group_table)",]

        # 把参数交给workflow运行相应的tool， 其中to_file用于准备tool的输入文件
        task_name = 'denovo_rna_v2.report.exp_pca'
