...
class WgcnaRelateAction(DenovoRnaV2Controller):
    def __init__(self):
        ...
        with open(trait_path, 'rb') as f:
            ...
            if self.input_data.trait_type.lower() == "discrete":
                if len(phenoype_list) >= 2:
                    return json.dumps({'success': False, 'info': "选择非连续类型时，表型数据只可以为1列", "code" : "C1602502"})
            trait_samples = list()
            for line in f:
                cols = line.strip().split()
                if len(cols) < len(phenoype_list) + 1:
                    return json.dumps({'success': False, 'info': "样本%s表型数据缺失", "variables":[cols[0]], "code" : "C1602503"})
                for n, p in enumerate(phenoype_list):
                    phe = cols[n+1]
                    if self.input_data.trait_type.lower() == "discrete":
                        if is_number(phe):
                            return json.dumps({'success': False, 'info': "选择非连续类型时，表型数据不应该是数字", 'code': 'C1602504', 'variables': ''})
                    else:
                        if not is_number(phe):
                            return json.dumps({'success': False, 'info': "选择连续类型时，表型数据必须是数字", 'code': 'C1602505', 'variables': ''})
                    phenoypes[p].append(phe)
                trait_samples.append(cols[0])
            select_samples = self.samples
            if len(trait_samples) != len(set(trait_samples)):
                return json.dumps({'success': False, 'info': "每个样本表型数据只可以有一行", "code" : "C1602506"})
            if len(trait_samples) != len(select_samples):
                print trait_samples, select_samples
                return json.dumps({'success': False, 'info': "表型样本与预处理样本不一致！", 'code': 'C1602507', 'variables': ''})

            for p, phe in phenoypes.items():
                if len(set(phe)) == 1:
                    return json.dumps({'success': False, 'info': "表型%s只有单一的值请删除该列", "variables":[p], "code" : "C1602508"})
        return True
    ...
    def POST(self):
        ...
        for arg in basic_args:
            if not hasattr(data, arg):
                var = []
                var.append(arg)
                info = {'success': False, 'info': "Lack argument: %s" % (arg), "code": 'C1602501', "variables": var}
                return json.dumps(info)
        main_info = self.denovo_rna_v2.get_main_info(data.wgcna_module_id, 'sg_wgcna_module', data.task_id)
        ...
        if clean_path:
            ...
            return json.dumps({'success': False, 'info': "不支持除了Unicode文本和excel之外的格式", "code" : "C1602509"})
        ...
        if isinstance(data.wgcna_module_id, types.StringTypes):
            ...
            raise Exception("wgcna_module_id参数必须为字符串或者ObjectId类型!")
        module_info = self.denovo_rna_v2.get_main_info_by_record('sg_wgcna_module', main_id=wgcna_module_id, task_id=data.task_id)
        ...
        main_id = self.denovo_rna_v2.insert_main_table('sg_wgcna_relate', main_info)
        ...
        # prepare to file
        to_files = ["denovo_rna_v2.export_wgcna_relate_input(exp_eigengenes)"]

        # 把参数交给workflow运行相应的tool， 其中to_file用于准备tool的输入文件
        task_name = 'denovo_rna_v2.report.wgcna_relate'
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/denovo_rna_v2/wgcna_relate "
        cmd += "-b http://192.168.12.101:51232 "
        args = dict(
            task_id="denovo_rna_v2_upgrade",
            task_type="2",
            submit_location="wgcna_relate",
            wgcna_module_id="5d88206017b2bf70f338a12d",
            exp_level="G",
            trait_path="/mnt/ilustre/users/sanger-dev/sg-users/liubinxu/test_denovo_rna_v2/test.trait.txt",
            trait_type="continues",
            corr_method="pearson",
            file_id="6919032",
        )
