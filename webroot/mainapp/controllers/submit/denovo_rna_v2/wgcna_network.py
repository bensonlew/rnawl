...
class WgcnaNetworkAction(DenovoRnaV2Controller):
    def __init__(self):
        super(WgcnaNetworkAction, self).__init__(instant=False)
        self.task_name = 'denovo_rna_v2.report.wgcna_network'
    ...
    def check_params(self):
        for arg in self.expected_args:
            if not hasattr(self.input_data, arg):
                var = []
                var.append(arg)
                info = {'success': False, 'info': "Lack argument: %s" % (arg), "code": 'C1602201', "variables": var}
                return json.dumps(info)
        # do some other check
        if len(self.input_data.module) < 1:
            info = {'success': False, 'info': "module: 选择的模块为空", "code": 'C1602202', "variables": ''}
            return json.dumps(info)
        return True
    ...
    def create_main_table(self, packed_params, db_name):
        result_info = self.denovo_rna_v2.get_main_info(self.input_data.wgcna_relate_id, 'sg_wgcna_relate', self.input_data.task_id)
        ...
        if isinstance(self.input_data.wgcna_relate_id, types.StringTypes):
            ...
            info = {'success': False, 'info': "wgcna_relate_id参数必须为字符串或者ObjectId类型!", "code": 'C1602203', "variables": ''}
            return json.dumps(info)
        main_info = dict(
            ...
            status="start")
        main_id = self.denovo_rna_v2.insert_main_table(db_name, main_info)
        return main_id, name

    def prepare_workflow_options(self, main_id, db_name, main_table_name):
        relate_output = self.denovo_rna_v2.get_main_info(self.input_data.wgcna_relate_id,'sg_wgcna_relate', self.input_data.task_id)['output_dir']
        if not relate_output.endswith('/'):
            relate_output += '/'
        module_output = self.denovo_rna_v2.get_main_info(self.wgcna_module_id, 'sg_wgcna_module', self.input_data.task_id)['output_dir']
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/denovo_rna_v2/wgcna_network "
        cmd += "-b http://192.168.12.102:9090 "
        args = dict(
            task_id="denovo_rna_v2",
            task_type="2",
            submit_location="WgcnaNetwork",
            wgcna_relate_id="5b288ca6a4e1af7451972033",
            module='red,blue',
            threshold='0.03',
            exp_level="gene",
            top='30',
