...
class WgcnaModuleAction(DenovoRnaV2Controller):
    def __init__(self):
    ...
    def POST(self):
        ...
        for arg in basic_args:
            if not hasattr(data, arg):
                var = []
                var.append(arg)
                info = {'success': False, 'info': "Lack argument: %s"%(arg), "code": 'C1602101', "variables": var}
                return json.dumps(info)
        if not data.power.isdigit():
            info = {'success': False, 'info': 'soft power must be integer rathar than %s', "variables":[data.power], "code" : "C1602102"}
            return json.dumps(info)
        exp_info = self.denovo_rna_v2.get_main_info(data.wgcna_prepare_id, 'sg_wgcna_prepare', data.task_id)
        ...
        if isinstance(data.wgcna_prepare_id, types.StringTypes):
            ...
            info = {'success': False, 'info': "wgcna_prepare_id参数必须为字符串或者ObjectId类型!", "code": 'C1602101', "variables": ''}
            return json.dumps(info)
        main_info = dict(
        ...
        main_id = self.denovo_rna_v2.insert_main_table('sg_wgcna_module', main_info)
        ...
        # prepare to file
        to_files = ["denovo_rna_v2.export_wgcna_exp_matrix(exp_matrix)"]

        # 把参数交给workflow运行相应的tool， 其中to_file用于准备tool的输入文件
        task_name = 'denovo_rna_v2.report.wgcna_module'
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/denovo_rna_v2/wgcna_module "
        cmd += "-b http://192.168.12.102:9090 "
        args = dict(
            task_id="denovo_rna_v2",
            task_type="2",
            submit_location="wgcna_module",
            wgcna_prepare_id="5b2889a1a4e1af7451972031",
            exp_level="gene",
            mergeCutHeight="0.25",
            power="7",
            minModuleSize="30",
