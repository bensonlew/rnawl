...
class WgcnaPipelineAction(DenovoRnaV2Controller):
    def __init__(self):
        super(WgcnaPipelineAction, self).__init__(instant=False)
        self.task_name = 'denovo_rna_v2.report.wgcna_pipeline'
        ...
        with open(trait_path, 'rb') as f:
            ...
            for line in f:
                ...
                if len(cols) < len(phenoype_list) + 1:
                    return json.dumps({'success': False, 'info': "样本%s表型数据缺失", "variables":[cols[0]], "code" : "C2903412"})
    ...
    def check_params(self):
        for arg in self.expected_args:
            if not hasattr(self.input_data, arg):
                var = []
                var.append(arg)
                info = {'success': False, 'info': "Lack argument: %s" % (arg), "code": 'C1602301', "variables": var}
    ...
    def create_main_table(self, packed_params, collection_name):
        ...
        """
        result_info = self.denovo_rna_v2.get_main_info(self.input_data.exp_id, 'sg_exp', self.input_data.task_id)
        self.project_sn = result_info["project_sn"]
        # exp_info = json.loads(result_info['params'])
        exp_level = self.input_data.exp_level[0].upper()
        exp_type = result_info["exp_type"]
        quant_method = result_info["method"]
        ...
        main_info = dict(
            ...
            status="start")
        main_id = self.denovo_rna_v2.insert_main_table(collection_name, main_info)
        if self.input_data.geneset_id.lower() not in ["all", "refall", "none"]:
            self.denovo_rna_v2.insert_geneset_info(self.input_data.geneset_id, collection_name, str(main_id))
        return main_id, name
...
class TestFunction(unittest.TestCase):
    ...
    def test_this(self):
        ...
        cmd += "s/denovo_rna_v2/wgcna_pipeline "
