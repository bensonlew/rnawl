...
class GenesetGseaAction(DenovoRnaV2Controller):
    ...
    def POST(self):
        ...
        preranked_file: path
        dict(name="rnk", type="infile", format="denovo_rna_v2.ref_common"),
        ...
        # 基因集
        dict(name="gmx", type="infile", format="denovo_rna_v2.ref_common"),
        ...
        if hasattr(data, "preranked") and data.preranked == 'true':
            ...
            if len(group_dict.keys()) != 2:
                error = {'success': False, 'info': '所选分组数量必须为两个', "code" : "C1603502"}
                print "return error {}".format(json.dumps(error))
                return json.dumps(error)
            else:
                if min([len(x) for x in group_dict.values()]) < 2:
                    error = {'success': False, 'info': '每个分组样本数量必须大于二个', "code" : "C1603503"}
                    return json.dumps(error)

        if data.exp_id == "":
             error = {'success': False, 'info': '请选择表达统计表'}
             return json.dumps(error)
        w_options['matrix'] = data.exp_id
        if len(group_dict.keys()) == 2:
        ...
        to_file.append('denovo_rna_v2_new.geneset.export_exp_matrix(matrix)')
        ...
        if data.geneset_source == 'user_defined':
            ...
            annot_info = self.denovo_rna_v2.get_main_info_by_record("sg_annotation_stat", task_id=str(data.task_id),  type="origin")
            species = annot_info["species_name"].replace("_", " ")
            if species in ["Mus musculus", "Homo sapiens", "Rattus norvegicus"]:
                pass
            else:
                error = {'success': False, 'info': '该项目物种不可以使用msigdb数据库进行分析', "code" : "C1603504"}
                return json.dumps(error)
            ...
            to_file.append('denovo_rna_v2_new.geneset.export_msigdb_genesets(gmx)')
            ...
            if data.anno_type == "GO":
                ...
                to_file.append('denovo_rna_v2_new.geneset.export_anno_genesets(go_list)')
            if data.anno_type == "KEGG":
                ...
                if hasattr(data, "kegg_genesets"):
                    ...
                    w_options['gmx'] += "all"
                to_file.append('denovo_rna_v2_new.geneset.export_anno_genesets(gmx)')
        w_options['genes_detail'] = {'task_id': data.task_id, 'type': 'origin'}
        to_file.append('denovo_rna_v2_new.geneset.export_genes_detail(genes_detail)')
        ...
        task_name = 'denovo_rna_v2.report.geneset_gsea'
...
if __name__ == '__main__':
    class TestFunction(unittest.TestCase):
        ...
        def test_this(self):
            ...
            cmd += "s/denovo_rna_v2/geneset_gsea "
            ...
            args = dict(
                ...
                exp_id="5d41459117b2bf10c8b4b896",
                geneset_type="G",
