...
class MetagbinController(Base):
    """
    设置项目基本信息
    """
    def __init__(self, bind_object=None, instant=False):
        super(MetagbinController, self).__init__(bind_object)
        ...
        self.metagbin = Metagbin(bind_object=bind_object)
    ...
    def _update_status_api(self):
        ...
        if client == 'client01':
            return 'metagbin.update_status'
        else:
            return 'metagbin.tupdate_status'
    ...
    def set_sheet_data(self, name, options, main_table_name, task_id, project_sn,module_type="workflow", params=None, to_file=None):
        ...
        self._sheet_data = {
            ...
            'db_type': 'metagbin',  # 需要根据自己连接的哪个数据库来修改实际的名字
    ...
    def _create_output_dir(self, task_id, main_table_name):
        ...
        """
        data = web.input()
        task_info = self.metagbin.get_task_info(task_id)
        part_dir = 'files/' + str(task_info['member_id']) + \
                      '/' + str(task_info['project_sn']) + '/' + \
                      task_id + '/interaction_results/' + main_table_name + "/"
        my_bucket = Config().get_project_region_bucket(project_type="metagbin")
