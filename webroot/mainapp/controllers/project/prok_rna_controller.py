...
class ProkRNAController(Base):
    def __init__(self, bind_object=None, instant=False, ):
        super(ProkRNAController, self).__init__(bind_object)
    ...
    def _update_status_api(self):
        """
        根据client决定接口api为
        1. mbio.api.web.prok_rna.update_status
        2. mbio.api.web.prok_rna.tupdate_status
        """
        data = web.input()
        client = data.client if hasattr(data, "client") else web.ctx.env.get('HTTP_CLIENT')
        if client == 'client01':
            return 'prok_rna.update_status'
        else:
            return 'prok_rna.tupdate_status'
    ...
    def roll_back(self):
        ...
        try:
            ...
            for i in update_info:
                if i == "batch_id":
                    continue
                self.prok_rna.update_status_failed(update_info[i], i)
    ...
    def set_sheet_data(self, name, options, main_table_name, task_id, project_sn,
        ...
        self._sheet_data = {
            ...
            'db_type': 'prok_rna',  # 需要根据自己连接的哪个数据库来修改实际的名字
    ...
    def set_sheet_data_1(self, name, options, task_id, project_sn,
    ...
    def _create_output_dir(self, task_id, main_table_name):
        data = web.input()
        task_info = self.prok_rna.get_task_info(task_id)
        config = Config()
        if config.RGW_ENABLE:
            target_dir = os.path.join(config.get_project_region_bucket(project_type="prok_rna"), "files",
