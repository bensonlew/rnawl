...
class BasicMolecularController(object):
    ...
    def _update_status_api(self):
        """
        根据client决定接口api为databasic_molecular.update_status/basic_molecular.tupdate_status
        """
        data = web.input()
        client = data.client if hasattr(data, "client") else web.ctx.env.get('HTTP_CLIENT')
        if client == 'client01':
            return 'basic_molecular.update_status'
        else:
            return 'basic_molecular.tupdate_status'
    ...
    def set_sheet_data(self, name, options, table_id, to_file=None, seq_number=None, module_type="workflow", params=None):
        ...
        if self.data.client == "client01":  # 线上拆分指定任务投递到isanger上
            self._sheet_data['cluster'] = "isanger"
        return self._sheet_data
    ...
    def _create_output_dir(self, seq_number, new_task_id):
        ...
        # "/" + seq_number + '/' + new_task_id + '/'
        config = Config()
        bucket = config.get_project_region_bucket(project_type="basic_molecular").rstrip("/")
