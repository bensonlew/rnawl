...
class MetaasvController(Base):

    def __init__(self, bind_object=None, instant=False):
        super(MetaasvController, self).__init__(bind_object)
    ...
    def POST(self):
        ...
        try:
            ...
            self.roll_back()
            return {"success": False, "info": "Failed to submit your task because too many tasks are being calculated on the cluster. Please try again!"}
    ...
    def roll_back(self):
        ...
        try:
            ...
            for i in update_info:
                if i == "batch_id":
                    continue
                self.metaasv.update_status_failed(update_info[i], i)
    ...
    def set_sheet_data(self, name, options, main_table_name, module_type="workflow", params=None, to_file=None, main_id=None, collection_name=None):
        ...
        if not main_id:
            main_id = self.data.asv_id
            collection_name = 'asv'

        table_info = self.metaasv.get_main_info(main_id=main_id, collection_name=collection_name)
        # table_info = self.get_main_info(main_id=main_id, collection_name=collection_name)  # modified by guhaidong 20171026
        # print table_info
        if "project_sn" in table_info:
        ...
        if "Sample_check" not in main_table_name:
            self._sheet_data['output'] = self._create_output_dir(task_id, main_table_name)
        if self.instant:
    ...
    def _create_output_dir(self, task_id, main_table_name):
        ...
        """
        data = web.input()
        task_info = self.metaasv.get_task_info(task_id)
        part_dir = str(task_info['member_id']) + '/' + str(task_info['project_sn']) + '/' + task_id + '/interaction_results/' + main_table_name
        sanger_prefix = Config().get_project_region_bucket(project_type="metaasv")
        target_dir = os.path.join(sanger_prefix, 'files/' + part_dir + '/')
        return target_dir
    ...
    def _update_status_api(self):
        ...
        if client == 'client01':
            return 'metaasv.update_status'
        else:
            return 'metaasv.tupdate_status'
