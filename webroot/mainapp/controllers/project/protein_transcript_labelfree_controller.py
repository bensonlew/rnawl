...
class ProteinTranscriptLabelfreeController(Base):
    def __init__(self, bind_object=None, instant=False, ):
        super(ProteinTranscriptLabelfreeController, self).__init__(bind_object)
    ...
    def _update_status_api(self):
        ...
        if client == 'client01':
            return 'labelfree.update_status'
        else:
            return 'labelfree.tupdate_status'
    ...
    def roll_back(self):
        ...
        try:
            ...
            for i in update_info:
                if i == "batch_id":
                    continue
                self.labelfree.update_status_failed(update_info[i], i)
    ...
    def set_sheet_data(self, name, options, main_table_name, task_id, project_sn,
        ...
        new_task_id = self.labelfree.get_new_id(task_id)
        self._sheet_data = {
            'id': new_task_id,
            'stage_id': 0,
            'interaction': True,
            ...
            'db_type': 'labelfree',  # 需要根据自己连接的哪个数据库来修改实际的名字
    ...
    def _create_output_dir(self, task_id, main_table_name):
        data = web.input()
        task_info = self.labelfree.get_task_info(task_id)
        config = Config()
        if config.RGW_ENABLE:
            target_dir = os.path.join(config.get_project_region_bucket(project_type="labelfree"), "files",
