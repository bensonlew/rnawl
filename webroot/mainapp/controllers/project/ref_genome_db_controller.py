...
from mainapp.models.mongo.ref_genome_db import RefGenomeDb
...
class RefGenomeDbController(Base):
    def __init__(self, bind_object=None, instant=False, ):
        super(RefGenomeDbController, self).__init__(bind_object)
        ...
        self.ref_genome_db = RefGenomeDb(bind_object=bind_object)
    ...
    def _update_status_api(self):
        """
        根据client决定接口api为
        1. mbio.api.web.ref_rna_v2.update_status
        2. mbio.api.web.ref_rna_v2.tupdate_status
        """
        data = web.input()
        client = data.client if hasattr(data, "client") else web.ctx.env.get('HTTP_CLIENT')
        if client == 'client01':
            return 'ref_genome_db.update_status'
        else:
            return 'ref_genome_db.tupdate_status'
    ...
    def roll_back(self):
        ...
        :return:
        """
        print("INFO: 任务提交出错，尝试更新主表为原始状态。")
        try:
            self.ref_genome_db.rollback_status()
            print("INFO: 更新主表状态为原始状态成功")
        except Exception as e:
            print('ERROR:尝试回滚主表状态为原始状态失败:{}'.format(e))
    ...
    def set_sheet_data(self, name, options, main_table_name, task_id, project_sn, client="client03",
        ...
        new_task_id = self.ref_genome_db.get_new_id(task_id)
        self._sheet_data = {
            'id': new_task_id,
            'stage_id': 0,
            'interaction': True,
            ...
            'output': self._create_output_dir(client, task_id, main_table_name),
            'project_sn': project_sn,
            'IMPORT_REPORT_DATA': True,
            'UPDATE_STATUS_API': self._update_status_api(),
            'db_type': 'ref_genome_db',  # 需要根据自己连接的哪个数据库来修改实际的名字
    ...
    def _create_output_dir(self, client, task_id, main_table_name):
        data = web.input()
        task_info = self.ref_genome_db.get_task_info(client, task_id)
        config = Config()
        if config.RGW_ENABLE:
            target_dir = os.path.join(config.get_project_region_bucket(project_type="ref_rna_v2"), "files",
    ...
    def use_s3(self, path):
        ...
        # elif "/mnt/lustre" in path:
        #     if os.path.exists(path):
        #         return path
        #     else:
        #         return {"success": False, "info": "{} not exists".format(path)}
        # elif "://" in path:
        #     return path
        # else:
        #     if exists("s3://" + path):
        #         return "s3://" + path
        #     elif exists("s3nb://" + path):
        #         return "s3nb://" + path
        #     else:
        #         return {"success": False, "info": "{} not exists".format(path)}
