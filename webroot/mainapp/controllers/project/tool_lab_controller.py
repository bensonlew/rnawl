...
class ToolLabController(Base):
    def __init__(self, bind_object=None, instant=False, ):
        super(ToolLabController, self).__init__(bind_object)
        self._project_type = "tool_lab"
        self._instant = instant
        self._post_data = None
        self._sheet_data = None
        self._return_msg = None
        self.tool_lab = ToolLab(bind_object=bind_object)
    ...
    def roll_back(self):
        ...
        try:
            ...
            for i in update_info:
                if i == "batch_id":
                    continue
                self.tool_lab.update_status_failed(update_info[i], i)
    ...
    def set_sheet_data(self, name, options, main_table_name, task_id, project_sn,
        ...
        new_task_id = self.tool_lab.get_new_id(task_id)
        self._sheet_data = {
            'id': new_task_id,
            'stage_id': 0,
            'interaction': True,
            ...
            'db_type': 'tool_lab',  # 需要根据自己连接的哪个数据库来修改实际的名字
    ...
    def _create_output_dir(self, task_id, main_table_name):
        data = web.input()
        # task_info = self.tool_lab.get_task_info(task_id)
        config = Config()
        if config.RGW_ENABLE:
            # target_dir = os.path.join(config.get_project_region_bucket(project_type="toollab"), "files",
            #                           str(task_info['member_id']), str(task_info['project_sn']),
            #                           task_id, 'interaction_results', main_table_name)
            target_dir = os.path.join(config.get_project_region_bucket(project_type="tool_lab"), "files",
                                      'm_188', 'tool',
                                      task_id, 'interaction_results', main_table_name)
        else:
            client = data.client if hasattr(data, "client") else web.ctx.env.get('HTTP_CLIENT')
