...
class LncRnaController(Base):
    ...
    def _update_status_api(self):
        """
        根据client决定接口api为
        1. mbio.api.web.lnc_rna.update_status
        2. mbio.api.web.lnc_rna.tupdate_status
        """
        data = web.input()
        client = data.client if hasattr(data, "client") else web.ctx.env.get('HTTP_CLIENT')
        if client == 'client01':
            return 'lnc_rna.update_status'
        else:
            return 'lnc_rna.tupdate_status'
    ...
    def roll_back(self):
        ...
        try:
            ...
            for i in update_info:
                if i == "batch_id":
                    continue
                self.lnc_rna.update_status_failed(update_info[i], i)
    ...
    def set_sheet_data(self, name, options, main_table_name, task_id, project_sn,
        ...
        if not new_task_id:
            new_task_id = self.lnc_rna.get_new_id(task_id)
        self._sheet_data = {
            'id': new_task_id,
            'stage_id': 0,
            'interaction': True,
            ...
            'db_type': 'lnc_rna',  # 需要根据自己连接的哪个数据库来修改实际的名字
    ...
    def _create_output_dir(self, task_id, main_table_name):
        data = web.input()
        task_info = self.lnc_rna.get_task_info(task_id)
        config = Config()
        if config.RGW_ENABLE:
            target_dir = os.path.join(config.get_project_region_bucket(project_type="lnc_rna"), "files",
    ...
    def download_from_s3(self, from_file, to_path="download/", inter_dir="", cover=True):
        ...
        if os.path.exists(from_file):
            to_file = from_file
        elif os.path.exists(to_file) and os.path.getsize(to_file) != 0:
            pass
        else:
