...
class WholeTranscriptomeController(Base):
    def __init__(self, bind_object=None, instant=False, ):
        super(WholeTranscriptomeController, self).__init__(bind_object)
        self._project_type = "whole_transcriptome"
        self._instant = instant
        self._post_data = None
        self._sheet_data = None
        self._return_msg = None
        self.whole_transcriptome = WholeTranscriptome(bind_object=bind_object)
    ...
    def _update_status_api(self):
        """
        根据client决定接口api为
        1. mbio.api.web.whole_transcriptome.update_status
        2. mbio.api.web.whole_transcriptome.tupdate_status
        """
        data = web.input()
        client = data.client if hasattr(data, "client") else web.ctx.env.get('HTTP_CLIENT')
        if client == 'client01':
            return 'whole_transcriptome.update_status'
        else:
            return 'whole_transcriptome.tupdate_status'
    ...
    def roll_back(self):
        ...
        try:
            ...
            for i in update_info:
                if i == "batch_id":
                    continue
                self.whole_transcriptome.update_status_failed(update_info[i], i)
    ...
    def set_sheet_data(self, name, options, main_table_name, task_id, project_sn,
        ...
        self._sheet_data = {
            ...
            'db_type': 'whole_transcriptome',  # 需要根据自己连接的哪个数据库来修改实际的名字
    ...
    def _create_output_dir(self, task_id, main_table_name):
        data = web.input()
        task_info = self.whole_transcriptome.get_task_info(task_id)
        config = Config()
        if config.RGW_ENABLE:
            target_dir = os.path.join(config.get_project_region_bucket(project_type="whole_transcriptome"), "files",
