    ...
    def POST(self):
        ...
        for i in requires:
            if not (hasattr(data, i)):
                return json.dumps({"success": False, 'info': "Lack argument: %s", "variables":[i], "code" : "C1900101"})
        ...
        if data.type == "itraq_tmt":
            data_json = {
                'id': workflow_id,
                'stage_id': 0,
                'name': "itraq_and_tmt.copy_demo",  # 需要配置
                'type': 'workflow',  # 可以配置
                'client': data.client,
            ...
            info = {"success": False, "info": "project type：%s is not illegal!", "variables":[data.type], "code" : "C1900102"}
