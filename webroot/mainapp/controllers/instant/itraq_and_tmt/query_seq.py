...
class QuerySeqAction(ItraqTmtController):
    def __init__(self):
    ...
    def POST(self):
        ...
        for arg in ['seq_id', 'seq_db', 'task_id']:
            if not hasattr(data, arg):
                info = {'success': False, 'info': 'Argument:%s required', "variables":[arg], "code" : "C1901601"}
        ...
        if not os.path.exists(data['seq_db']):
            info = {'success': False, 'info': data['seq_db'] + ' not exist', "code" : "C1901602"}
            return json.dumps(info)
