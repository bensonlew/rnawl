...
class DiffGenesetVennAction(MedicalTranscriptomeController):
    def __init__(self):
        super(DiffGenesetVennAction, self).__init__(instant=True)

    @check_sig
    def POST(self):
        data = web.input()
        basic_args = ["task_id", "submit_location", 'task_type']
        basic_args += ['geneset_ids', 'level']
        # check arg
        ...
        params = dict(
            ...
            geneset_ids=','.join(geneset_id_list),
        )
        params = json.dumps(params, sort_keys=True, separators=(',', ':'))
        name = "Diff_GenesetVenn" + '_' + data.level + '_'
        ...
        main_id = self.medical_transcriptome.insert_main_table('sg_diff_geneset_venn', main_info)
        ...
        options = {
            ...
            "update_info": json.dumps({str(main_id): "sg_diff_geneset_venn"})  # to update sg_status
        }

        # 把参数交给workflow运行相应的tool， 其中to_file用于准备tool的输入文件
        task_name = 'medical_transcriptome.report.diff_geneset_venn'
        ...
        # 运行workflow 并传回参数
        task_info = super(DiffGenesetVennAction, self).POST()
        task_info['content'] = {
            'ids': {
                'id': str(main_id),
                'name': name
        ...
        # 更新基因集的使用信息
        self.medical_transcriptome.insert_geneset_info(data.geneset_ids, "sg_diff_geneset_venn", str(main_id))
