...
class PearsonCorrelation(MetaController):
    ...
    def __init__(self):
        super(PearsonCorrelation, self).__init__(instant=True)
    ...
    def POST(self):
        ...
        if not hasattr(data, 'env_id'):  # modified by hongdongxuan 20170310
            info = {'success': False, 'info': '缺少环境因子参数!', 'code':'C2202801'}
        ...
        if len(str(data.env_labs).split(",")) < 2:
            info = {'success': False, 'info': '相关性Heatmap分析环境因子数量必须大于等于2!', 'code':'C2202802'}
            return json.dumps(info)
        task_name = 'meta.report.pearson_correlation'
        task_type = 'workflow'
        otu_info = self.meta.get_otu_table_info(data.otu_id)
        if not otu_info:
            info = {"success": False, "info": "OTU不存在，请确认参数是否正确！!", 'code':'C2202803'}
            return json.dumps(info)
