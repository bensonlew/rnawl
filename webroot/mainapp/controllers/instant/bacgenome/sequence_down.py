...
class SequenceDownAction(BacgenomeController):
    def __init__(self):
        super(SequenceDownAction, self).__init__(instant=True)

    @check_sig
    def POST(self):
        data = web.input()
        #print data
        default_argu = ['task_id', 'specimen_gene', 'type', 'submit_location', 'specimen_gene_new','client']
        for argu in default_argu:
            if not hasattr(data, argu):
                info = {'success': False, 'info': 'parameters missing:%s' % argu}
                return json.dumps(info)
        project_sn = self.bacgenome.get_projectsn(data.task_id)
        task_name = 'bacgenome.report.sequence_down'
        module_type = 'workflow'
        params_json = {
            "type": data.type,
            "specimen_gene": json.loads(data.specimen_gene),
            "task_type": int(data.task_type),
            "submit_location": data.submit_location,
            "task_id": data.task_id,
            "specimen_gene_new":json.loads(data.specimen_gene_new),
        }
        main_table_name = 'SeqDown_' + data.type.capitalize() + '_' + datetime.datetime.now().strftime("%Y%m%d_%H%M%S%f")[:-3]
        file_seqpath = self.bacgenome.get_genefile_bysample(data.task_id, "KKKKKKKKKKKKKKKK", type="fnn",
                                                            predict=data.type)
