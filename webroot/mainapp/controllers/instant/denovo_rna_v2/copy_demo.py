    ...
    def POST(self):
        ...
        for i in requires:
            if not (hasattr(data, i)):
                var = []
                var.append(i)
                return json.dumps({"success": False, "info": "缺少%s参数!" % i, "code": 'C1600101', "variables": var})
        ...
        if data.type == "denovo_rna_v2":
            data_json = {
                'id': workflow_id,
                'stage_id': 0,
                'name': "denovo_rna_v2.copy_demo",  # 需要配置
                'type': 'workflow',  # 可以配置
                'client': data.client,
            ...
            info = {"success": False, "info": "项目类型type：%s不合法!"%(data.type), "code": 'C1600102', "variables": var}
        ...
        try:
            ...
            return json.dumps({"success": False, "info": "运行出错: %s" % filter_error_info(str(e)), "code": 'C1600103', "variables": var})
