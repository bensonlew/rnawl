...
class StopAction(DiaController):
    def __init__(self):
    ...
    def create_main_table(self, packed_params):
        status_info = self.dia.get_main_info_by_record('sg_status', _id=ObjectId(self.input_data.main_id))
        print(status_info)
        if status_info:
            if status_info['status'] == 'end':
                ...
                if 'run_id' in status_info and status_info['run_id']:
                    ...
                    main_info = dict(
                        ...
                        status="start")
                    main_id = self.dia.insert_main_table('sg_cancel', main_info)
    ...
    def modify_status(self, status):
        ...
        self.dia.update_db_record('sg_status', query_dict=query_dict, insert_dict=update_dict)
        # 更改主表状态
        status_info = self.dia.get_main_info_by_record('sg_status', _id=ObjectId(self.input_data.main_id))
        table_name = status_info['type_name']
        table_id = status_info['table_id']
        self.dia.update_db_record(table_name, query_dict={'_id': table_id}, insert_dict=update_dict)

    def update_main(self, status, desc, query_dict):
        update_dict = {'status': status, 'desc': desc}
        self.dia.update_db_record('sg_cancel', query_dict=query_dict, insert_dict=update_dict)
