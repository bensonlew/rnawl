...
class ProteinsetSelfAction(DiaController):
    def __init__(self):
    ...
    def POST(self):
        ...
        task_info = self.dia.get_task_info(task_id=input_data.task_id)
        ...
        main_id = self.dia.insert_main_table('sg_proteinset', main_info)
        ...
        # prepare to file
        to_files = ["dia.export_proteinset_from_query(proteins)"]

        # 把参数交给workflow运行相应的tool， 其中to_file用于准备tool的输入文件
        task_name = 'dia_v3.report.proteinset_self'
        ...
        #
        if 'group_id' in input_data and str(input_data.group_id).lower() != 'all':
            _ = self.dia.update_group_is_use(input_data.task_id, input_data.group_id)
        if 'control_id' in input_data:
            _ = self.dia.update_group_compare_is_use(input_data.task_id, input_data.control_id)
        ##
