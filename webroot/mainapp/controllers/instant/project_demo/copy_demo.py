...
import unittest
import os
from mbio.packages.project_demo.copy_demo import CopyDemoMongo
...
class CopyDemoAction(Base):
    ...
    def POST(self):
        ...
        if hasattr(self.input_data, 'db_version'):
            db_version = self.input_data.db_version

        try:
            copy_demo = CopyDemoMongo(old_task_id, new_task_id, new_project_sn, new_member_id, project_type, db_version=db_version)
            copy_demo.run()
            ...
            result = col.find_one({"task_id": demo_task_id})
            if result:
                ...
                return json.dumps({"success": False, "info": "拉取demo失败!"})
