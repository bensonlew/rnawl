...
class DeleteTaskAction(Base):
    def __init__(self):
        ...
        :return:
        """
        super(DeleteTaskAction, self).__init__()
    ...
    def POST(self):
        ...
        print("new_task: %s" %self.input_data.task_id)
        # new_task = json.loads(self.input_data.task_id)
        new_task = (self.input_data.task_id)
        if isinstance(new_task, list):
            task_id = (",".join(self.input_data.task_id)).encode("utf-8")
        elif isinstance(new_task, str):
            task_id = new_task.encode("utf-8")
        else:
            task_id = new_task
        ...
        if hasattr(self.input_data, "main_id"):#
            data_json["main_id"] = self.input_data.main_id
